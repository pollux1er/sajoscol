...
if(isset($_GET['del_id_info'])) {
	...
	if(del_info_action($_GET['del_id_info'])) {
		...
		$msg.="Error during suppression of the action n° ".$_GET['del_id_info'].".<br />";
	}
}
...
if((($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite'))&&(isset($_GET['del_id_acces_cdt']))) {
	...
	if(del_acces_cdt($_GET['del_id_acces_cdt'])) {
		...
		$msg.="Error during suppression of access CDT n° ".$_GET['del_id_acces_cdt'].".<br />";
	}
}
...
if (!suivi_ariane($_SERVER['PHP_SELF'],"Home"))
...
if ($_SESSION['statut'] == "administrateur") {
	...
	if (!check_backup_directory()) {
		$afficheAccueil->message_admin[] = "There was a problem with the update of the backup directory.
Please ensure that the /backup directory is writtable for the server (the server *only* !)";
    }


	if (!check_user_temp_directory()) {		
		$afficheAccueil->message_admin[] = "There was a problem with the update of the temp directory.
Please ensure that the /temp directory is writtable for the server (the server *only* !)";
	}else{
		$_SESSION['user_temp_directory']='y';
	}

	if ((getSettingValue("disable_login"))!='no'){
	  $afficheAccueil->message_admin[] = "The site is down for maintenance.";
	}
...
// christian : demande d'enregistrement
	if (($force_ref)) {
		$afficheAccueil->referencement[]=array("texte"=>"Your institution is not listed among the users.","lien"=>"$gepiPath/referencement.php?etape=explication","titre"=>"Why is it useful ?");
		$afficheAccueil->referencement[]=array("texte"=>"Your institution is not listed among the users.","lien"=>"$gepiPath/referencement.php?etape=1","titre"=>"Reference your school");
	}
...
// fin christian demande d'enregistrement
	...
	if (
		...
		$afficheAccueil->probleme_dir[]="Unsecured connection ! You *should* login to the system in HTTPS (check the configuration of your web server)";
		$test_https = 'n';
	}

	if (ini_get("register_globals") == "1") {
		$afficheAccueil->probleme_dir[]="PHP potentially badly configured (register_globals=on)! To prevent certain faults of secutity, you *must* configure PHP with the parameter register_globals to off.";
	}
    
    if (version_compare(PHP_VERSION,'5.2.4')<0) {
        $afficheAccueil->probleme_dir[]="Gépi requires a version of php higher than version 5.2.4 to function in an optimal way. It is advised to update your version of PHP.";
    }
    if(file_exists('./lib/global.inc')){
        $afficheAccueil->probleme_dir[]="The file global.inc in the repertory lib is obsolete. You must remove it manually.";
    }
    /**
	...
	if(mysql_num_rows($res_sign)>0) {
		$tbs_signalement="<p class='tbs_signalement'>One or more errors of assignment of students were announced in the following courses&nbsp;:<br />\n";
	...
	if (!check_user_temp_directory()) {
		...
		$afficheAccueil->probleme_dir[]="There was a problem with the update of the temp directory.";
		$afficheAccueil->probleme_dir[]="Please contact the administrator to fix.";
