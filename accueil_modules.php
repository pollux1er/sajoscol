...
$titre_page = "Home - Administration of the modules";
$racine_gepi = 'yes';
...
/* ===== Titre du menu ===== */
	...
	$menuTitre[$a]->texte='Administration of the modules';
...
// cahier de texte
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="Textbook" ;
		$nouveauItem->expli="To manage the textbooks, (general configuration, ...)" ;
...
// cahier de notes
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="Report cards" ;
		$nouveauItem->expli="To manage the report cards (General configuration, ...)" ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Absences
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->expli="To manage the module absences" ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Absences 2
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->expli="To manage the module absences 2 (under development)" ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Emploi du temps
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="Timetable" ;
		$nouveauItem->expli="To manage the opening of the timetable of Gepi." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Mise à jour automatisée
	if ($force_msj) {
		...
		if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
		{
			...
			$nouveauItem->titre="Automated update" ;
			$nouveauItem->expli="To manage the module of update of GEPI" ;
			$menuPage[]=$nouveauItem;
		}
		unset($nouveauItem);
	}
...
// Trombinoscope
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->expli="To manage the module trombinoscope." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Notanet/Fiches Brevet
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->expli="To manage the Notanet module/Cards Patent" ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Inscription
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->expli="To simply manage the inscriptions  of ".$gepiSettings['denomination_professeurs']." for example for training courses or many interventions in the colleges" ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Flux rss
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->expli="Management of flows rss of the textbooks produced by Gepi" ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Autorisation des statuts personnalisés
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="To create personalized statutes" ;
		$nouveauItem->expli="To define additional statutes by personalizing the access rights ." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Années antérieures
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="Former years" ;
		$nouveauItem->expli="To manage the module former Years." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Module ateliers
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->expli="Management and installation of workshops of the conferences type (management of the workshops, speakers, inscriptions...)." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
// Module discipline
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->expli="To manage the module Disciplines." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
//Module modèle Open_Office
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="OpenOffice model" ;
		$nouveauItem->expli="To manage the OpenOffice models of Gepi." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
//Module ECTS
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="Typing ECTS" ;
		$nouveauItem->expli="To manage appropriations ECTS allotted for each course." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
//Module Plugins
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="To manage the plugins" ;
		$nouveauItem->expli="Administration interface  of the personal plugins of the school." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
//Module Génèse des classes
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="Genesis of the classes" ;
		$nouveauItem->expli="To manage the Genesis module of the classes." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
//Module Epreuve blanche
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="White tests" ;
		$nouveauItem->expli="To manage white tests (anonymity of the copies,...)." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
//Module Examen blanc
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="Practice tests" ;
		$nouveauItem->expli="To manage practice tests." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
//Module Gestionnaires d'AID
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		...
		$nouveauItem->titre="Managers of IDA" ;
		$nouveauItem->expli="To open the possibility of defining managers for each IDA." ;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
