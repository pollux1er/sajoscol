...
while ($ind < $nb_messages) {
	...
	//if ((strtolower($_SESSION['login'])==strtolower($statuts_destinataires1)) || ((strpos(strtolower($statuts_destinataires1), substr(strtolower($_SESSION['statut']), 0, 1)))&&(substr($statuts_destinataires1,0,1)=="_"))) {
		...
		if(strstr($content, '_DECOMPTE_')) {
			...
			elseif($nb_sec==0) {
				$decompte_remplace=" <span style='color:red'>You are at the moment T</span> ";
			}
			else {
				$nb_sec=$nb_sec*(-1);
				$decompte_remplace=" <span style='color:red'>date exceeded of</span> ";
			}
...
if ((basename($_SERVER['SCRIPT_NAME'])=="accueil_simpl_prof.php") && ($affiche_messages == 'yes')) {
	echo "<table id='messagerie' summary=\"This table contains information on which one want to draw the attention\">\n";
