...
if (isset($is_posted) and ($is_posted =="1")) {
    ...
    if ($test_id != 0) {
       ...
       if (!$reg_data) {
          ...
          $msg = "AID enregistrée !";
          if ($count == "1") {
              $msg=$msg." Attention, an IDA bearing the same name existed déja !";
          } else if ($count > 1) {
              $msg=$msg." Attention, several IDA bearing the same name existed déja !";
          }
...
//**************** EN-TETE *********************
if ($action == "modif_aid") {
	$titre_page = "IDA Management | Modify an IDA";
}
else {
	$titre_page = "IDA Management | Add an IDA";
}
require_once("../lib/header.inc");
//**************** FIN EN-TETE *****************
?>
<p class=bold>
|<a href="index.php?indice_aid=<?php echo $indice_aid; ?>"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return</a>
...
<?php
	if ($action == "modif_aid") {
		...
		if($id_aid_prec!=-1) {
			echo " | <a href='".$_SERVER['PHP_SELF']."?action=modif_aid&amp;aid_id=$id_aid_prec&amp;indice_aid=$indice_aid' onclick=\"return confirm_abandon (this, change, '$themessage')\">Previous IDA</a>";
		}
		if($id_aid_suiv!=-1) {
			echo " | <a href='".$_SERVER['PHP_SELF']."?action=modif_aid&amp;aid_id=$id_aid_suiv&amp;indice_aid=$indice_aid' onclick=\"return confirm_abandon (this, change, '$themessage')\">Next IDA</a>";
		}
	}
?>
...
<?php if ($action == "add_aid") { ?>
    ...
    <p><label for="aidRegNom">Name : <input type="text" id="aidRegNom" name="reg_nom" size="100" <?php if (isset($reg_nom)) { echo "value=\"".$reg_nom."\"";}?> /></label></p>

    <p><label for="aidRegNum">Number (fac.) : <input type="text" id="aidRegNom" name="reg_num" size="4" <?php if (isset($reg_num)) { echo "value=\"".$reg_num."\"";}?> /></label></p>
...
if ($action == "modif_aid") { ?>
    ...
    <p>Enter the new name in the place of old : </p>
    ...
    <p><label for="aidRegNom">Name : <input type="text" id="aidRegNom" name="reg_nom" size="100" <?php echo "value=\"".$aid_nom."\"";?> /></label></p>

    <p><label for="aidRegNum">Number (fac.) : <input type="text" id="aidRegNum" name="reg_num" size="4" <?php echo "value=\"".$aid_num."\"";?> /></label></p>
