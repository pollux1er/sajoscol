...
echo "Click on the symbol <img src=\"../images/plier.png\" alt=\"More details\" title=\"More details\" style=\"vertical-align: middle;\"/> in front of each project to post or hide the details of the project. You can also ";
...
echo " all details.<br />\n";
if ($_login!="")
    echo "According to the parameter setting carried out by the administrator, some of these cards are partly <a href=\"../public/index_fiches.php\">freely consultable and accessible to the public</a>.<br />";
echo "<br />";
$i = 0;
while ($i < $nombreligne){
    ...
    if ((VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'adresse1','',$annee)) and ($adresse1 != "")) {
        if (($affiche_adresse1 == 'y')or(($affiche_adresse1 != 'y') and ($_login!="") )) {
            if ((substr($adresse1,0,4) == "http") or (substr($adresse1,0,3) == "ftp")) {
                ...
                echo ">Click to access the site</a></span>";
             } else
                echo "<span class=\"small\"> -  Public access : <b>".$adresse1."</b></span>";
        }
    }
    // Adresse privée :
    if ((VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'adresse2','',$annee)) and ($adresse2 != "")) {
        if ($adresse2 != "")  {
            if ((substr($adresse2,0,4) == "http") or (substr($adresse2,0,3) == "ftp"))
                echo "<span class=\"small\"> -  Restricted access : <a href='".$adresse2."' title='".$adresse2."' >Click to access the site</a></span>";
            else
                echo "<span class=\"small\"> -  Restricted access : <b>".$adresse2."</b></span>";
        }
    }
    echo "<br />";
    ...
    if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'resume','',$annee)) {
        echo "<span class='medium'><b>Summary : </b>";
        //Résumé
    ...
    if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'famille','',$annee)) {
        // Famille
        echo "<span class='medium'><b>Project classified in the family : </b>";
    ...
    if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'mots_cles','',$annee)) {
        ...
        echo "<br /><span class='medium'><b>Key words : </b>";
        if ($aff_motcle == "")
            echo $non_defini;
        else
            echo $aff_motcle;
    ...
    if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'public_destinataire','',$annee)) {
        ...
        echo "<br /><span class='medium'><b>Recipient public: </b>";
        if ($liste == "")
            echo $non_defini;
        else
            echo $liste;
    ...
    if ($annee=='') {
      ...
      if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'eleve_peut_modifier','',$annee)) {
        if ($eleve_peut_modifier == "y")
          echo "<li>The responsible student can modify this card.</li>";
        else
          echo "<li>The responsible student cannot modify this card.</li>";
      }
      if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'prof_peut_modifier','',$annee)) {
        if ($prof_peut_modifier == "y")
          echo "<li>The responsible professors can modify this card.</li>";
        else
          echo "<li>The responsible professors cannot modify this card.</li>";
      }
      if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'cpe_peut_modifier','',$annee)) {
        if ($cpe_peut_modifier == "y")
          echo "<li>The CPE can modify this card.</li>";
        else
          echo "<li>The CPE cannot modify this card.</li>";
      }
      if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'fiche_publique','',$annee)) {
        if ($fiche_publique == "y")
          echo "<li>This card is on line in <a href=\"javascript:centrerpopup('../public/index_fiches.php',800,500,'scrollbars=yes,statusbar=no,resizable=yes')\">the public interface</a>.</li>";
        else
          echo "<li>This card is not available in <a href=\"javascript:centrerpopup('../public/index_fiches.php',800,500,'scrollbars=yes,statusbar=no,resizable=yes')\">the public interface</a>.</li>";
      }
      ...
      if (VerifAccesFicheProjet($_login,$aid_id,$indice_aid,'eleves_profs','',$annee)) {
        echo "<hr /><b>Other information : </b><ul>";
        echo "<li>student responsible for the project :".$eleves_resp."</li>";
        echo "<li>Professors responsible for the project :".$responsables."</li>";
        echo "<li>Student belonging to the project :".$eleves."</li>";
        echo "</ul>";
      }
