...
if (isset($_POST['sup'])) {
  ...
  if ($sup_all=='no') $msg = "One or more ida categories could not be removed because they contain aid.";
...
echo "| <a href=\"config_aid.php\">Add a category of IDA</a> |";
...
if ($test_outils_comp != 0) {
    echo " <a href=\"config_aid_fiches_projet.php\">Configure the cards project</a> |";
}
...
if ($nb_aid == 0) {
   echo "<p class='grand'>There is currently no category of IDA</p>";
} else {
    ...
    echo "<tr><th><p>Name - Modifications</p></th>\n";
    echo "<th><p>List of ida in the category</p></th>\n";
    echo "<th><p>Complete name of the IDA</p></th>\n";
    echo "<th><p><input type=\"submit\" name=\"sup\" value=\"Delete\" /></p></th></tr>\n";
    ...
    while ($i < $nb_aid) {
        ...
        if ($outils_complementaires=='y')
            $display_outils = "<br /><span class='small'>(Activated complementary tools)</span>";
        else
            $display_outils="";
        if ((getSettingValue("num_aid_trombinoscopes")==$indice_aid) and (getSettingValue("active_module_trombinoscopes")=='y'))
            $display_trombino = "<br /><span class='small'>(Management of the student access  to the trombinoscope)</span>";
        else
            $display_trombino="";

		$alt=$alt*(-1);
        echo "<tr class='lig$alt'><td><p><a href='config_aid.php?indice_aid=$indice_aid'>$nom_aid</a> $display_outils $display_trombino</p></td>";
        echo "<td><p><a href='index2.php?indice_aid=$indice_aid'>List of Ida in the category</a></p></td>";
