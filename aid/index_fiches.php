...
if (!VerifAidIsAcive($indice_aid,"")) {
    echo "<p>You try to reach tools which are not activated. please contact the administrator.</p></body></html>";
    die();
}
...
if (!isset($_GET['action'])) {
    echo "<p class='bold'>";
    echo "<a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return</a>";
    echo "</p>";
    echo "<b>Your choice : </b>\n";
    echo "<ul>";
    echo "<li><a href=\"index_fiches.php?action=liste_projet&amp;indice_aid=".$indice_aid."\"><b>Table of all the cards ".$nom_projet."</b> (".$gepiSettings['denomination_professeurs']." responsables, ".$gepiSettings['denomination_eleves']." responsables";
    if (isset($test_salle) && $test_salle != 0)
        echo ", salles";
    if (($feuille_presence == 'y') and ($_SESSION["statut"] != "eleve") and ($_SESSION["statut"] != "responsable"))
        echo ", attendance sheets";
    echo ")</a></li>\n";
    echo "<li><a href=\"visu_fiches.php?indice_aid=".$indice_aid."\"><b>Table of all the cards ".$nom_projet."</b> (résumé, productions attendues, ...)</a></li>\n";
    echo "<li><a href=\"index_fiches.php?action=liste_eleves&amp;indice_aid=".$indice_aid."\"><b>List of all ".$gepiSettings['denomination_eleves']."</b> (possible classifications by name, first name, class, project)</a></li>\n";
    if (($_SESSION["statut"] == "administrateur") or ($_SESSION["statut"] == "cpe"))
        echo "<li><a href=\"index_fiches.php?action=liste_eleves_sans_projet&amp;indice_aid=".$indice_aid."\"><b>List of ".$gepiSettings['denomination_eleves']." not affected</b></a></li>\n";
    if (($feuille_presence == 'y') and ($_SESSION["statut"] != "eleve") and ($_SESSION["statut"] != "responsable"))
        echo "<li><a href=\"index_fiches.php?action=liste_presence&amp;indice_aid=".$indice_aid."\"><b>Edition of all attendance sheets</b></a></li>\n";
...
if ((isset($_GET['action'])) and ($_GET['action']=="liste_projet")) {
    ...
    if (($feuille_presence == 'y') and ($_SESSION["statut"] != "eleve") and ($_SESSION["statut"] != "responsable"))
            echo "<br /><span class='medium'><a href='index_fiches.php?action=liste_presence&amp;aid_id=".$aid_id."&amp;indice_aid=".$indice_aid."' title=\"The attendance sheet opens in a new window.\"  ><i>Attendance sheet</i></a></span>\n";
        echo "<br /><span class='medium'><i>Complete card : </i>\n";
        echo "<a href='modif_fiches.php?aid_id=$aid_id&amp;indice_aid=$indice_aid&amp;retour=index_fiches.php' >Visualize</a></span>\n";
        if (VerifAccesFicheProjet($_SESSION['login'],$aid_id,$indice_aid,'','')) {
            echo " - <a href='modif_fiches.php?aid_id=$aid_id&amp;indice_aid=$indice_aid&amp;action=modif&amp;retour=index_fiches.php'>Modify</a>";
        }
        echo "</td>\n";
...
if ((isset($_GET['action'])) and ($_GET['action']=="liste_presence")) {
    ...
    while ($i < $nombreligne){
        ...
        echo "<tr><td style=\"width:50%\"><b>Name First name</b></td><td><b>Absences / Delay (beginning of meeting)</b></td><td><b>Absences / Delay (end of meeting)</b></td></tr>";
        $nombre = mysql_num_rows($call_liste_data);
        $j = "0";
        while ($j < $nombre) {
            $vide = 0;
        ...
        echo "</table>\n";
        if ($vide == 1) {
            echo "<br /><font color = red>There is currently none ".$gepiSettings['denomination_eleve']." in this IDA !</font>\n";
        } else {
            echo "<table style=\"width:90%\" border=\"0\" cellpadding=\"5\">\n";
            echo "<tr><td style=\"width:50%\">&nbsp;</td><td><b>Signature of ".$gepiSettings['denomination_professeur']."/".$gepiSettings['denomination_professeurs']." responsible(s)</b></td></tr>\n";
            echo "</table>";
        }
...
if ((isset($_GET['action'])) and ($_GET['action']=="liste_eleves")) {
    ...
    echo "<a href=\"./index_fiches.php?indice_aid=".$indice_aid."\"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Retour</a>";
    echo "</p>";
    echo "<h2>List of".$gepiSettings['denomination_eleves']."</h2>";
    echo "Click on the heading of the first line to classify them ".$gepiSettings['denomination_eleves']." by name and first name, class or project<br /><br />";
    ...
    echo "<tr>
    <td style=\"width:50%\"><b><a href='index_fiches.php?order_by2=nom&amp;action=liste_eleves&amp;indice_aid=".$indice_aid."'>Name First name</a></b></td>\n
    <td style=\"width:50%\"><b>Identifier</b></td>\n
    <td><b><a href='index_fiches.php?order_by2=classe&amp;action=liste_eleves&amp;indice_aid=".$indice_aid."'>Class</a></b></td>\n
    ...
    echo "</table>\n";
    if ($vide == 1) {
        echo "<br /><font color = red>There is currently none ".$gepiSettings['denomination_eleve']." registered in ".$nom_projet." !</font>\n";
    }
...
if ((isset($_GET['action'])) and ($_GET['action']=="liste_eleves_sans_projet")) {
  ...
  if (!isset($_GET['choix_classes'])) {
    ...
    echo "<p>Select classes to exclude from research&nbsp;:\n";
    echo "</p>\n";
    // Affichage sur 4/5 colonnes
    $nb_classes_par_colonne=round($nb_classes/4);
    echo "<table style=\"width:100%\" summary='Choice of classes'>\n";
    ...
    echo "</tr>\n";
    echo "</table>\n";
    echo "<p><input type='submit' name='choix_classes' value='Validate' /></p>\n";
    ...
    if ($order_by2 == "nom") $order_by2 = "e.nom, e.prenom";
    if ($order_by2 == "classe") $order_by2 = "c.classe, e.nom, e.prenom";
    echo "Click on the heading of the first line to classify ".$gepiSettings['denomination_eleves']." by name and first name or class<br /><br />";
    ...
    echo "<table style=\"width:90%\" border=\"1\" cellpadding=\"3\">\n";
    echo "<tr>
      <td style=\"width:50%\"><b><a href='index_fiches.php?order_by2=nom&amp;action=liste_eleves_sans_projet&amp;indice_aid=".$indice_aid."&amp;id_classe_serie=".serialize($id_classe)."&amp;choix_classes=y'>Name First name</a></b></td>\n
    <td style=\"width:50%\"><b>Identifiant</b></td>\n
    <td><b><a href='index_fiches.php?order_by2=classe&amp;action=liste_eleves_sans_projet&amp;indice_aid=".$indice_aid."&amp;id_classe_serie=".serialize($id_classe)."&amp;choix_classes=y'>Class</a></b></td>\n
    </tr>\n";
    $nombre = mysql_num_rows($call_liste_data);
    $i = "0";
    $vide = 1;
    ...
    echo "</table>\n";
    if ($vide == 1) {
        echo "<br /><font color = red>Currently all ".$gepiSettings['denomination_eleves']." are registered in a project !</font>\n";
    }
  }
}
