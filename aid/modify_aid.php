...
if ((NiveauGestionAid($_SESSION["login"],$indice_aid) >= 5) and (isset($add_prof) and ($add_prof == "yes"))) {
    ...
    if ($test2 != "0") {
        ...
        if ($reg_prof_login != '') {
            ...
            if (!$reg_data) { $msg = "Error during the addition of the professor !"; } else { $msg = "The professor was added !"; }
        }
    }
    $flag = "prof";
}
...
if ((NiveauGestionAid($_SESSION["login"],$indice_aid) >= 10) and (isset($add_prof_gest) and ($add_prof_gest == "yes"))) {
    ...
    $test2 = mysql_num_rows($test);
    if ($test2 != "0") {
        $msg = "The user that you tried to add belongs already to the list of the managers of this IDA";
    } else {
        if ($reg_prof_login != '') {
            ...
            if (!$reg_data) { $msg = "Error during the addition of the user !"; } else { $msg = "The user was added !"; }
        }
    }
    $flag = "prof_gest";
}
...
if (isset($add_eleve) and ($add_eleve == "yes")) {
    ...
    $test2 = mysql_num_rows($test);
    if ($test2 != "0") {
        $msg = "The student that you tried to add belongs already to a IDA";
    } else {
        if ($reg_add_eleve_login != '') {
            ...
            if (!$reg_data) { $msg = "Error during the addition of the student"; } else { $msg = "The student was added."; }
        }
    }
    $msg .= "<br />The modifications were recorded.";
    $flag = "eleve";
...
if (isset($_POST["toutes_aids"]) and ($_POST["toutes_aids"] == "y")) {
    ...
    $query = mysql_query($sql) OR DIE('Error in the request : '.mysql_error());
    ...
    while ($i < $nombreligne){
        ...
        if ($test1 == 0) {
          ...
          foreach($liste_profs as $key){
              ...
              if (!$reg_data) { $msg .= "Error during the addition of the professor $key !<br />"; }
          }
        }
        $i++;
    }
    $flag = "prof";
    if ($msg == '') $msg = "The modifications were recorded.";
}
    ...
    foreach($_POST["liste_aids"] as $key1) {
      foreach($liste_profs as $key2){
        ...
        if ($test == -1) {
          ...
          if (!$reg_data) { $msg .= "Error during addition of the professor $key2 to the ida whose identifier is $key1 !<br />"; }
        }
     }
    }
    $flag = "prof";
    ...
    while ($i < $nombreligne){
        ...
        if ($test1 == 0) {
          ...
          foreach($liste_profs as $key){
              ...
              if (!$reg_data) { $msg .= "Error during the addition of the user $key !<br />"; }
          }
        }
        $i++;
    }
...
//**************** EN-TETE *********************
$titre_page = "Management of $nom_aid | Modify them $nom_aid";
...
$query = mysql_query($sql) OR DIE('Error in the request select * from aid : '.mysql_error());
...
for($a = 0; $a < $nbre; $a++){
	...
	if ($a != 0) {
		...
		if ($aid_p[$a]["id"] == $aid_id) {
			...
			$aff_precedent = '
			<a href="modify_aid.php?flag='.$flag.'&amp;indice_aid='.$indice_aid.'&amp;aid_id='.$aid_precedent.'">Previous IDA&nbsp;</a>';
		}
	}
	...
	if ($a < ($nbre - 1)) {
		...
		if ($aid_p[$a]["id"] == $aid_id) {
			...
			$aff_suivant = '
			<a href="modify_aid.php?flag='.$flag.'&amp;indice_aid='.$indice_aid.'&amp;aid_id='.$aid_suivant.'">&nbsp;Next IDA  </a>';
		}
	}
}
...
if ($flag == "prof") { ?>
   ...
   <?php
    ...
    if ($nombre !=0) {
    ?>
        <p class='bold'>List of responsible professors :</p>
        The names of the professors below appear (according to the configuration) on the official bulletins and/or the simplified bulletins.<br />
        <?php
        if ($activer_outils_comp == "y")
              echo "Moreover these professors can modify the cards project (if the administrator activated this possibility).";
    ...
    while ($i < $nombre) {
        ...
        echo "<tr><td><b>";
        echo "$nom_prof $prenom_prof</b></td><td><a href='../lib/confirm_query.php?liste_cible=$login_prof&amp;liste_cible2=$aid_id&amp;liste_cible3=$indice_aid&amp;action=del_prof_aid".add_token_in_url()."'>\n<font size=2><img src=\"../images/icons/delete.png\" title=\"Delete this professor\" alt=\"Delete\" /></font></a></td>\n";
        echo "</tr>";
    $i++;
    }

    if ($nombre == 0) {
        echo "<h4 style=\"color: red;\">There is currently any responsible professor !</h4>";
    } else {
        echo "</table>";
    }
    ?>
    <p class='bold'>Add a professor responsible to the list for the IDA :</p>
    ...
    ?>
      ...
      <hr /><H2>Assign this list to IDAs without responsible professor</H2>
      If you click on the button below, the professors of the list above will be also assigned to all  IDAs of this category not having yet of responsible professor.
      <?php
		echo add_token_field();
      ...
      echo "<br /><input type=\"submit\" value=\"Assign the list to Idas without professor\" />\n";
      echo "</form>";
      ?>
      <form enctype="multipart/form-data" action="modify_aid.php" method="post">
      <hr /><H2>Assign this list to Idas selected</H2>
      <?php
		echo add_token_field();
      ...
      echo "<br />If you click on the button below, professors of the list of this IDA will be also assigned to all IDAs selected above.<br />";
      echo "<br /><input type=\"submit\" value=\"Assign the list to Idas selected above\" />\n";
      echo "</form>";
...
if ($flag == "prof_gest") { ?>
   ...
   <?php
    ...
    if ($nombre !=0) {
    ?>
        The managers can add or remove students in this IDA.
        <p class='bold'>List of Managers users :</p>
        <?php
        echo "<hr /><table class=\"aid_tableau\" border=\"0\" summary=''>\n";
    }
    $i = "0";
    ...
    if ($nombre == 0) {
        echo "<h4 style=\"color: red;\">There is not currently a manager user !</h4>";
    } else {
        echo "</table>";
    }
    ?>
    <p class='bold'>Add a user to the list of the managers of the IDA :</p>
    <select size=1 name="reg_prof_login">
    <option value=''>(none)</option>
    <?php
    ...
    ?>
      <form enctype="multipart/form-data" action="modify_aid.php" method="post">
      <hr /><H2>Assign this list to Idas without manager</H2>
      If you click on the button below, the users of the list above will be also assigned to all IDAs of this category not having yet a manager.
      <?php
		echo add_token_field();
      ...
      echo "<input type=\"hidden\" name=\"indice_aid\" value=\"".$indice_aid."\" />\n";
      echo "<input type=\"hidden\" name=\"aid_id\" value=\"".$aid_id."\" />\n";
      echo "<br /><input type=\"submit\" value=\"Assign the list to Idas without manager\" />\n";
      echo "</form>";
   }
}
...
?>
    ...
    <p class = 'bold'>List of students of the IDA <?php echo $aid_nom ?> :</p>
    <hr />
    <?php
    $vide = 1;
    // Ajout d'un tableau
echo "<form enctype=\"multipart/form-data\" action=\"modify_aid.php\" method=\"post\">\n";
    ...
    echo $nombre." élève".$s.".\n</td><td></td>";
    if ($activer_outils_comp == "y") {
      echo "<td>student responsible (*)</td>";
    }
    echo "</tr>\n";
    $i = "0";
    while ($i < $nombre) {
        ...
        echo "<tr><td>\n";
        echo "<b>$nom_eleve $prenom_eleve</b>, $classe_eleve </td>\n<td> <a href='../lib/confirm_query.php?liste_cible=$login_eleve&amp;liste_cible2=$aid_id&amp;liste_cible3=$indice_aid&amp;action=del_eleve_aid".add_token_in_url()."'><img src=\"../images/icons/delete.png\" title=\"Delete this student\" alt=\"Delete\" /></a>\n";
...
//		        if($photo!=""){
      	  ...
      	  $tabdiv_infobulle[]=creer_div_infobulle('info_popup_eleve'.$v_elenoet,$titre,"",$info,"",14,0,'y','y','n','n');
	          ...
	          echo "<img src='../images/icons/buddy_no.png' alt='No photo' />";
          }
        }
    ...
    if ($vide == 1) {
        echo "<br /><p style=\"color: red;\">There are no currently student in this IDA !</p>";
    }
    if ($autoriser_inscript_multiples == 'y')
    ...
    if ($nombreligne != 0) {
        ...
        if (getSettingValue("num_aid_trombinoscopes")==$indice_aid) {
            echo "<br />Below, list only the student without photography <input type=\"checkbox\" name=\"eleves_sans_photos\" value=\"y\" ";
            if(isset($_SESSION['eleves_sans_photos'])) echo " checked ";
            echo "/>";
        }

        echo "<br />\n<p><span class = 'bold'>Ajouter un élève à la liste de l'AID :</span>\n";
        echo "<a href=\"modify_aid_new.php?id_aid=".$aid_id."&amp;indice_aid=".$indice_aid."\">List the student by class</a>\n";
...
<?php } else {
        echo "<p>All the students of the base have a IDA. Impossible to add a student to this IDA !</p>";
    }
    ?>
    ...
    if ($activer_outils_comp == "y") {?>
    <p><br />(*) The student responsible can for example access under certain conditions to the edition of IDA cards.
    <?php }
