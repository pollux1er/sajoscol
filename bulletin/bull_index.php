 ...
 *
 * $Id: bull_index.php 8084 2011-08-31 16:00:21Z crob $
 *
...
$titre_page = empty($lang) ? ${'titre_page' . $lang} = "Edition of bulletins" : $titre_page;	
...
The generation of the pdf will fail because this information of debuggage is posted,<br />
but it may be that you have precise details thus on what poses problem.<br />
</p>\n";
	}
...
if(!isset($tab_id_classe)) {
	...
	(($_SESSION['statut']=='administrateur')&&(getSettingValue('GepiAdminImprBulSettings')=='yes'))) {
		echo " | <a href='param_bull.php' target='_blank'>Parameters of impression of the bulletins</a>";
	}
	echo " | <a href='index.php'>Old disposition</a>";
	echo "</p>\n";

	echo "<p class='bold'>Choice of the classes:</p>\n";

	if (($_SESSION['statut'] == 'professeur') and getSettingValue("GepiProfImprBul")!='yes') {
		echo "<p>Insufficient rights to carry out this operation</p>\n";
	...
	if ($_SESSION["statut"] == "scolarite") {
		...
		$message_0="No class (<i>with student</i>) is affected to you.";
	}
	...
	elseif (($_SESSION["statut"] == "administrateur")||($_SESSION["statut"] == "secours")) {
		...
		$message_0="No class with student was found.";
	}
	elseif ($_SESSION["statut"] == "professeur") {
		$sql="SELECT DISTINCT c.* FROM classes c, j_eleves_professeurs jep, j_eleves_classes jec WHERE (jep.professeur='".$_SESSION['login']."' AND jep.login = jec.login AND jec.id_classe = c.id);";
			...
			$message_0="No class (<i>with student</i>) is affected to you for the edition of the bulletins.";
	}
	else {
	...
	echo "<table width='100%' summary='Choice of classes'>\n";
	...
	while($lig_clas=mysql_fetch_object($call_classes)) {
		...
		echo "<label id='label_tab_id_classe_$cpt' for='tab_id_classe_$cpt' style='cursor: pointer;'><input type='checkbox' name='tab_id_classe[]' id='tab_id_classe_$cpt' value='$lig_clas->id' onchange='change_style_classe($cpt)' /> $lig_clas->class</label>";
		echo "<br />\n";
		$cpt++;
	}
	...
	echo "<p><a href='#' onClick='ModifCase(true)'>Check all</a> / <a href='#' onClick='ModifCase(false)'>Uncheck all</a></p>\n";
...
elseif((!isset($choix_periode_num))||(!isset($tab_periode_num))) {
	...
	echo "<p class='bold'>Choice of periods:</p>\n";
	...
	*/
	...
	for($i=0;$i<count($tab_id_classe);$i++) {
		...
		if((strlen(preg_replace("/[0-9]/","",$tab_id_classe[$i])))||($tab_id_classe[$i]=="")) {
			echo "<p>Erroneous identifier of class: <span style='color:red'>".$tab_id_classe[$i]."</span></p>\n";
			require("../lib/footer.inc.php");
			die();
		}
			...
			while($lig_per=mysql_fetch_object($res_per)) {
				...
				elseif($lig_per->verouiller=="N") {
					...
					*/
					...
					$ligne_debug[$i].="<td style='background-color:orange; text-align:center;'>Partially closed</td>\n";
				}
	...
	echo "<table class='boireaus' summary='Choice of periods'>\n";
	echo "<tr>\n";
	echo "<th>Classe</th>\n";
	for($i=1;$i<=$max_per;$i++) {
	...
	for($i=1;$i<=$max_per;$i++) {
		if(!in_array($i,$tab_periode_num_excluses)) {
			...
			echo "<td style='background-color:red; text-align:center;'>Nonclosed period<br />for a class at least";
			if($_SESSION['statut']=='scolarite') {
				echo " <a href='verrouillage.php' target='_blank'><img src='../images/icons/configure.png' width='16' height='16' title='Locking/unlocking'/></a>\n";
			}
			echo "</td>\n";
		}
	}
...
elseif(!isset($_POST['valide_select_eleves'])) {
	...
	echo "<p class='bold'>";
	echo "<a href='".$_SERVER['PHP_SELF']."'>Choose other classes</a>\n";
	//echo " | <a href='".$_SERVER['PHP_SELF']."'>Choisir d'autres périodes</a>\n";
	//echo " | <a href='#' onClick='history.go(-1);'>Choisir d'autres périodes</a>\n";
	echo " | <a href='".$_SERVER['PHP_SELF']."' onClick=\"document.forms['form_retour'].submit();return false;\">Choose other periods</a>\n";
	if((($_SESSION['statut']=='scolarite')&&(getSettingValue('GepiScolImprBulSettings')=='yes'))||
	(($_SESSION['statut']=='professeur')&&(getSettingValue('GepiProfImprBulSettings')=='yes'))||
	(($_SESSION['statut']=='administrateur')&&(getSettingValue('GepiAdminImprBulSettings')=='yes'))) {
		echo " | <a href='param_bull.php' target='_blank'>Parameters impression of the bulletins</a>";
	}
	echo "</p>\n";

	//===========================
	// FORMULAIRE POUR LE RETOUR AU CHOIX DES PERIODES
	echo "\n<!-- Form for the return to the choice of periods -->\n";
	...
	echo "<p class='bold'>Selection of the students and parameters:</p>\n";

	echo "\n<!-- Form of selection of students  and parameter setting -->\n";
	...
	echo "<table border='0' summary='Choice of bulletin type'>\n";
	echo "<tr>\n";
	echo "<td valign='top'>\n";
	...
	echo "</td>\n";
	echo "<td>\n";
	echo "<label for='mode_bulletin_html' id='texte_mode_bulletin_html' style='cursor:pointer;'>Report card HTML</label>\n";
	echo "</td>\n";
	echo "</tr>\n";
	...
	if ($test == -1) {
		echo "&nbsp;</td>\n";
		echo "<td valign='top'>\n";
		echo "Pdf models  use a table 'modele_bulletin' who seems absent.<br />\n";
		if($_SESSION['statut']=='administrateur') {
			echo "Visit the page of <a href='test_modele_bull.php'>creation of this table</a> according to the old table 'model_bulletin' to allow the impression of PDF bulletins .";
		}
		else {
			echo "The filling of the table must be carried out by administrator.";
		}
	}
	else {
		...
		if(mysql_num_rows($test)==0) {
			echo "&nbsp;</td>\n";
			echo "<td valign='top'>\n";
			echo "Pdf models pdf use a table 'modele_bulletin' who seems absent.<br />\n";
			if($_SESSION['statut']=='administrateur') {
				echo "Visit the page of <a href='test_modele_bull.php'>creation of this table</a> according to the old one table 'model_bulletin' to allow the impression of pdf bulletins.";
			}
			else {
				echo "The filling of the table must be carried out by administrator.";
			}
			...
			echo "</td>\n";
			echo "<td>\n";
			echo "<label for='mode_bulletin_pdf' id='texte_mode_bulletin_pdf' style='cursor:pointer;'>PDF Report card </label>\n";
			...
			echo "<div id='div_modele_bulletin_pdf'>\n";
				...
				if (($option_modele_bulletin==2)||($option_modele_bulletin==3)) { //Par défaut  le modèle défini pour les classes
					echo "Choose the model of bulletin<br />\n";
					...
					if ($option_modele_bulletin==2) { //Par défaut  le modèle défini pour les classes
						echo "<option value=\"-1\">Use the models preselected by class</option>\n";
					}
			...
			echo "<input type='checkbox' name='use_cell_ajustee' id='use_cell_ajustee' value='n' onchange=\"checkbox_change(this.id)\" /><label for='use_cell_ajustee' id='texte_use_cell_ajustee' style='cursor: pointer;'> Don't use the new function use_cell_ajustee() for the writing of the appreciations.</label>";

			$titre_infobulle="Fonction cell_ajustee()\n";
			$texte_infobulle="For the appreciations on the bulletins, statements,... was using the function DraxTextBox() of FPDF.<br />This function had sometimes a curious behavior with truncated texts or much smaller in the cell than what seemed to be able to hold in the
box.<br />The function cell_ajustee() is a function developped to try to do better than DraxTextBox().<br />As it was not tested by several people on trunk, we put a check box which makes it possible to use the old function DrawTextBox() if cell_ajustee() did not appear as well rotten as hope for we it;o).<br />\n";
			//$texte_infobulle.="\n";
			...
			// Debug
			echo "<input type='checkbox' name='bull_pdf_debug' id='bull_pdf_debug' value='y' onchange=\"checkbox_change(this.id)\" />&nbsp;<label for='bull_pdf_debug' id='texte_bull_pdf_debug' style='cursor: pointer;'>Activate the debug to display the errors disturbing the generation of pdf.</label>\n";

			$texte_infobulle="It happens that the generation of pdf fails.<br />The reasons can be variable (<em>Miss of server resources, bug,...</em>).<br />In this case, the presence of a plugin reader pdf can prevent from seeing which
errors cause the failure.<br />By checking the box DEBUG, you will obtain the display of the errors and thus you will be able to obtain from the assistance more easily onthe list 'gepi-users'<br />\n";
	...
	echo "<table border='0' summary='Table of parameters'>\n";
	echo "<tr><td valign='top'><input type='checkbox' name='un_seul_bull_par_famille' id='un_seul_bull_par_famille' value='oui' onchange=\"checkbox_change(this.id)\" /></td><td><label for='un_seul_bull_par_famille' id='texte_un_seul_bull_par_famille' style='cursor: pointer;'>Don't print a bulletin for the second parent<br />(<i>even in the case of separate parents</i>).</label></td></tr>\n";
	...
	//if($test_coef>0){
		...
		*/
		echo "<tr><td valign='top'><input type='checkbox' name='coefficients_a_1' id='coefficients_a_1' value='oui' onchange=\"checkbox_change(this.id)\" /></td><td><label for='coefficients_a_1' id='texte_coefficients_a_1' style='cursor: pointer;'>Force, in the calculation of the general averages, coefficients of the course to 1, independently of the coefficients typed in the parameters of the class.</label></td></tr>\n";
		...
		echo "<tr><td valign='top'><input type='checkbox' name='tri_par_etab_orig' id='tri_par_etab_orig' value='y' onchange=\"checkbox_change(this.id)\" /></td><td><label for='tri_par_etab_orig' id='texte_tri_par_etab_orig' style='cursor: pointer;'>Sort the bulletins by school of origin.</label></td></tr>\n";
		...
		*/
		...
		if(($_SESSION['statut']=='scolarite')||($_SESSION['statut']=='administrateur')) {
			echo "<tr><td valign='top'><input type='checkbox' name='forcer_recalcul_moy_conteneurs' id='forcer_recalcul_moy_conteneurs' value='y' onchange=\"checkbox_change(this.id)\" /></td><td><label for='forcer_recalcul_moy_conteneurs' id='texte_forcer_recalcul_moy_conteneurs' style='cursor: pointer;'>Force the recalculation of the averages of containers.</label></td></tr>\n";

			echo "<tr><td valign='top'><input type='checkbox' name='forcer_recalcul_rang' id='forcer_recalcul_rang' value='y' onchange=\"checkbox_change(this.id)\" /></td><td><label for='forcer_recalcul_rang' id='texte_forcer_recalcul_rang' style='cursor: pointer;'>Force the recalculation of the rows.</label></td></tr>\n";
		}
	//}
	echo "</table>\n";
	...
	echo "<br />\n";
	echo "<p><b>Block responsible addresses and title page&nbsp;:</b></p>\n";
	echo "<blockquote>\n";
	echo "<input type='radio' name='b_adr_pg' id='b_adr_pg_xx' value='xx' ";
	if($b_adr_pg_defaut=="xx") {
		echo "checked='checked' ";
	}
	echo "/><label for='b_adr_pg_xx' style='cursor:pointer'> According to the parameters of HTML bulletin </label><br />\n";
	...
	echo "/><label for='b_adr_pg_yy' style='cursor:pointer'> with addresses  block and title page</label><br />\n";
	echo "</blockquote>\n";
	echo "</div>\n";
	//===========================================
	...
	if (mysql_num_rows($res_verif_droit)>0) {
		echo "<p><input type='checkbox' name='intercaler_releve_notes' id='intercaler_releve_notes' value='y' onchange='display_param_releve();checkbox_change(this.id)' /> <label for='intercaler_releve_notes' id='texte_intercaler_releve_notes' style='cursor: pointer;'>Intercalate the bulletins</label>\n";
		echo "<span id='pliage_param_releve'>\n";
		echo "(<i>";
		echo "<a href='#' onclick=\"document.getElementById('div_param_releve').style.display='';return false;\">Display</a>";
		echo " / \n";
		echo "<a href='#' onclick=\"document.getElementById('div_param_releve').style.display='none';return false;\">Hide</a>";
		echo " les paramètres du relevé de notes</i>).";
		echo "</span>\n";

		echo "<br />\n";
		echo "(<i>to allow for example a recto/verso impression</i>).";
		echo "</p>\n";
...
";
	...
	echo "<p align='center'><input type='submit' name='bouton_valide_select_eleves1' value='Validate' /></p>\n";
	echo "</div>\n";
	...
	echo "<input type='submit' name='bouton_valide_select_eleves1' value='Validate' />\n";
	echo "</div>\n";
	*/
	//=======================================
	...
	if(count($tab_id_classe)>1) {
		...
		echo " <a href='#' onClick='cocher_tous_eleves();return false;'>check all students</a> / <a href='#' onClick='decocher_tous_eleves();return false;'>Uncheck all students</a></p>\n";
	}
	...
	for($i=0;$i<count($tab_id_classe);$i++) {
		...
		echo "<table class='boireaus' summary='Choice of Students'>\n";
		echo "<tr>\n";
		echo "<th>Elèves</th>\n";
		for($j=0;$j<count($tab_periode_num);$j++) {
			// Est-ce bien un entier?
			if((strlen(preg_replace("/[0-9]/","",$tab_periode_num[$j])))||($tab_periode_num[$j]=="")) {
				echo "<td>Erroneous identifier of period: <span style='color:red'>".$tab_periode_num[$j]."</span></td></tr></table></form>\n";
				require("../lib/footer.inc.php");
				die();
			}
			...
			echo "<a href=\"javascript:CocheColonneSelectEleves(".$i.",".$j.");changement();\"><img src='../images/enabled.png' width='15' height='15' alt='Checck All' /></a> / <a href=\"javascript:DecocheColonneSelectEleves(".$i.",".$j.");changement();\"><img src='../images/disabled.png' width='15' height='15' alt='Uncheck All' /></a>\n";
...
display_param_b_adr_pg();
	...
	echo "<p><input type='submit' name='bouton_valide_select_eleves2' value='Validate' /></p>\n";
	echo "</form>\n";
}
//=======================================================
...
<table class='boireaus' width='100%' summary='Table of progression'>
<tr>
<th colspan='3' id='td_info'></th>
</tr>
<tr>
<th width='33%'>Class</th>
<th width='33%'>Period</th>
<th>Mark</th>
</tr>
<tr>
<td id='td_classe'></td>
...
<td id='td_ele'></td>
	...
	if($mode_bulletin=="html") {
		echo "<script type='text/javascript'>
	document.getElementById('titre_infodiv').innerHTML='Report cards';
	...
	for($loop_classe=0;$loop_classe<count($tab_id_classe);$loop_classe++) {
		...
		if((isset($_POST['forcer_recalcul_rang']))&&($_POST['forcer_recalcul_rang']=='y')) {
			...
			if(mysql_num_rows($res_per)>0) {
				...
				$res=mysql_query($sql);
				if(!$res) {
					$msg.="<br />Error during the programming of the recalculation of the rank for the class ".get_nom_classe($tab_id_classe[$loop_classe]).".";
				}
			}
				...
				if($nb_cat_priorites!=$nb_cat) {
					...
					if($nb_cat_priorites_glob!=$nb_cat) {
						if($mode_bulletin!="pdf") {
							...
							echo "<p>You asked to post the categories of course, but the priorities of display of the categories are not correctly
defined, nor at the global level in course management, nor at the particular level in Management of the classes/&lt;Class&gt; Parameters<br />Two categories shouldn't have the same priority without what it can occur of the anomalies of order of the courses on the bulletin.</p>\n";
							require("../lib/footer.inc.php");
							die();
						}
						else {
							...
							$pdf->SetXY(20,40);
							$pdf->SetFontSize(10);
							$pdf->Cell(150,7, "you have asked to display the categories of courses,",0,2,'');
							$pdf->SetXY(20,45);
							$pdf->Cell(150,7, "but the priorities of display of the categories are not correctly defined,",0,2,'');
							$pdf->SetXY(20,50);
							$pdf->Cell(150,7, "nor at the global level in course management,",0,2,'');
							$pdf->SetXY(20,55);
							$pdf->Cell(150,7, "nor at the particular level in Management of the classes/<Classe> Lesson",0,2,'');
							$pdf->SetXY(20,65);
							$pdf->Cell(150,7, "Two categories should not have the same priority",0,2,'');
							$pdf->SetXY(20,70);
							$pdf->Cell(150,7, "without what it can occur anomalies of order of the course on the bulletin.",0,2,'');
			...
			if($eff_classe==0) {
				echo "<p>The class '$classe' is empty during the period '$periode_num'.<br />It is not possible to continue.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
	...
	document.getElementById('td_ele').innerHTML='".$current_eleve_login[$i]."';
					...
					if(mysql_num_rows($res_resp)>2) {
						if($mode_bulletin=="html") {
							echo "<div class='alerte_erreur'><b style='color:red;'>Error:</b>";
							echo $tab_ele['nom']." ".$tab_ele['prenom']." has more than two legal persons in charge 1 and 2.<br />It is an anomaly.<br />";
							for ($z=0;$z<count($tab_ele['resp']);$z++) {
								echo $tab_ele['resp'][$z]['nom']." ".$tab_ele['resp'][$z]['prenom']." (<i>legal responsible ".$tab_ele['resp'][$z]['resp_legal']."</i>)<br />";
							}
							echo "Only the two first will appears on bulletins.";
							echo "</div>\n";
						}
					}
					...
					if ($bull_affiche_aid == 'y') {
						...
						if($mode_bulletin=="html") {
							...
							decompte_debug($motif,"$motif student $i (".$current_eleve_login[$i].") before IDA");
							flush();
						}
						//==============================
						...
						if($mode_bulletin=="html") {
							$motif="Temoin_eleve".$id_classe."_".$periode_num."_".$i;
							decompte_debug($motif,"$motif student $i (".$current_eleve_login[$i].") after IDA");
							flush();
						}
						//==============================
					}
	...
	document.getElementById('td_periode').innerHTML='".$periode_num."';
			...
			if($mode_bulletin=="html") {
				...
				echo "<p>Manpower of the class: ".$tab_bulletin[$id_classe][$periode_num]['eff_classe']."</p>\n";
				echo "</div>\n";
			}
	...
	document.getElementById('td_ele').innerHTML='".$tab_bulletin[$id_classe][$periode_num]['eleve'][$rg[$i]]['login']."';
							...
							if($mode_bulletin=="html") {
								//$motif="Report card_eleve".$id_classe."_".$periode_num."_".$i;
								//decompte_debug($motif,"$motif élève $i avant");
								$motif="Report card_eleve".$id_classe."_".$periode_num."_".$rg[$i];
								decompte_debug($motif,"$motif student $rg[$i] before");
								flush();
								...
								$motif="Report card_eleve".$id_classe."_".$periode_num."_".$rg[$i];
								decompte_debug($motif,"$motif student $rg[$i] after");
								flush();
							}
							else {
							...
							if($mode_bulletin=="html") {
								echo "<div class='espacement_bulletins'><div align='center'>Spacing (nonprinted) between the bulletins</div></div>\n";
							}
...
<p>TO VERIFY:</p>
<ul>
<li>HTML bulletins use the infos display_rang, display_coef,... table 'classes'.<br />
Pdf bulletins  use the infos of the table 'modele_bulletin' it seems to me.<br />
Perhaps it would be necessary to re-examine the device to adopt the same strategy.<br />
we should also add fields in the table 'classes' for the statements of notes,... another structure should be considered?</li>
</ul>
</div>\n";
