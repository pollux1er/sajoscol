...
if (isset($_POST['ok'])) {
							...
							if(mysql_num_rows($test_conteneur)>0) {
								...
								if((isset($_POST['creer_conteneur']))&&($_POST['creer_conteneur']=="y")) {
									...
									if($insert_conteneur=mysql_query($sql)) {
										...
										$msg.="The container/box for teaching ".$tmp_group["name"]." (n°".$tmp_group["id"].") en ".$tmp_group["classlist_string"]." was not created.<br />";
									}
								}
    ...
    if(mysql_num_rows($test_bulletin)>0) {
        $msg.=" CAUTION: Notes are present on the bulletins. If you modified a coefficient, notes,... think of updating the recopy towards the bulletins.";
    }
    //==========================================================
...
//**************** EN-TETE *****************
$titre_page = "Report card - Addition/modification of an evaluation";
/**
 * Entête de la page
 */
...
echo "<p class='bold'> Class : $nom_classe | Matière : ".htmlentities("$matiere_nom ($matiere_nom_court)")."| Période : $nom_periode[$periode_num] <input type=\"submit\" name='ok' value=\"Register\" style=\"font-variant: small-caps;\" /></p>\n";
echo "</div>";


echo "<h2 class='gepi'>Configuration of the evaluation :</h2>\n";
...
if($interface_simplifiee=="y"){
	...
	if($aff_note_autre_que_referentiel=='y'){
		if(getSettingValue("note_autre_que_sur_referentiel")=="V") {
			...
			echo "</tr>\n";
			echo "<tr>\n";
			echo "<td style='background-color: #aae6aa; font-weight: bold; vertical-align: top;'>Bring back the note on ".getSettingValue("referentiel_note")."<br />during the calculation of the average : </td>\n";
    		echo "<td><input type='checkbox' name='ramener_sur_referentiel' value='V' "; if ($ramener_sur_referentiel == 'V') {echo " checked";} echo " /><br />\n";
			echo "<span style=\"font-size: x-small;\">Example with 3 notes : 18/20 ; 4/10 ; 1/5<br />\n";
			echo "box checked: average = 18/20 + 8/20 + 4/20 = 30/60 = 10/20<br />\n";
			echo "boc unchecked: average = (18 + 4 + 1) / (20 + 10 + 5) = 23/35 &asymp; 13,1/20</span><br /><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
	} else {
	...
	if($aff_date_ele_resp=='y'){
		echo "<tr>\n";
		echo "<td style='background-color: #aae6aa; font-weight: bold;'>Date of visibility<br />of note for<br />student and responsible:</td>\n";
		echo "<td>\n";
	...
	if($aff_boite=='y'){
		echo "<tr>\n";
		echo "<td style='background-color: #aae6aa; font-weight: bold;'>Position of the evaluation:</td>\n";
		echo "<td>\n";
	...
	echo "<table summary='Name and container of the Exam'>\n";
	...
	echo "<br />\n";
	echo "<table summary='Position of the exam'><tr><td><h3 class='gepi'>Position of the evaluation : </h3></td>\n<td>";
	echo "<select size='1' name='id_emplacement'>\n";
	$appel_conteneurs = mysql_query("SELECT * FROM cn_conteneurs WHERE id_racine ='$id_racine' order by nom_court");
	$nb_cont = mysql_num_rows($appel_conteneurs);
	$i = 0;
	...
	echo "<h3 class='gepi'>Coefficient of the evaluation</h3>\n";
	echo "<table summary='Ponderation'><tr><td>Value of weighting in the calculation of the average (if 0, the note of the evaluation do not intervene in the calculation of the average) : </td>";
	...
	if(getSettingValue("note_autre_que_sur_referentiel")=="V") {
	    ...
	    echo "<table summary='Referentiel'><tr><td>Note on: </td>";
	    ...
	    echo "<tr><td>Bring back the note on ".getSettingValue("referentiel_note")." during the calculation of the average : <br />";
		echo "<span style=\"font-size: x-small;\">Example with 3 notes : 18/20 ; 4/10 ; 1/5<br />";
		echo "Box checked: average = 18/20 + 8/20 + 4/20 = 30/60 = 10/20<br />";
		echo "Box not checked : average = (18 + 4 + 1) / (20 + 10 + 5) = 23/35 &asymp; 13,1/20</span><br /><br />\n";
		echo "</td>";
	...
	echo "<h3 class='gepi'>Statute of the evaluation</h3>\n";
	echo "<table summary='Statute of the exam'><tr><td><input type='radio' name='facultatif' id='facultatif_O' value='O' "; if ($facultatif=='O') echo "checked"; echo " /></td><td>";
	...
	echo "Only the notes of the evaluation higher than 10 enter the calculation of the average.";
	echo "</label>";
	...
	echo "The note of the evaluation enters the calculation of the average only if it improves the average.";
	echo "</label>";
	echo "</td></tr></table>\n";
	...
	echo "<a name=\"calend\"></a><h3 class='gepi'>Date of the evaluation (format jj/mm/aaaa) : </h3>
	<b>Notice</b> : it is this date which is taken into account for the edition of the statements of notes at various periods of the year.
	...
	echo "<a name=\"calend\"></a><h3 class='gepi'>Date of visibility of the evaluation for the students and responsibles (format jj/mm/aaaa) : </h3>
	<b>Notice</b> : This date makes it possible to make the note visible when the exam is corrected in class.
	...
	echo "<h3 class='gepi'>Display on the statement of notes</h3>\n";
	echo "<table summary='Visibilité'>\n";
	echo "<tr><td><label for='display_parents' style='cursor: pointer;'>";
	echo "<b>Display this evaluation</b> on the <b>statement of notes</b> of the student : ";
	echo "</label>";
	...
	echo "<b>The appreciation</b> of the evaluation is displayable on <b>statement of notes</b> of the student (if the preceding option were validated) :";
	echo "</label>";
...
if ($new_devoir=='yes') {
	...
	if(count($tab_group)>1) {
		...
		echo "<input type='checkbox' id='creation_dev_autres_groupes' name='creation_dev_autres_groupes' value='y' onchange=\"display_div_autres_groupes()\" /><label for='creation_dev_autres_groupes'>Create the same exam for other lesson.</label><br />\n";
	
		echo "<div id='div_autres_groupes'>\n";
		echo "<table class='boireaus' summary='Other lesson'>\n";
		echo "<tr>\n";
		echo "<th rowspan='2'>";
		...
		echo "</table>\n";
		// A METTRE AU POINT: 
		echo "<input type='checkbox' name='creer_conteneur' id='creer_conteneur' value='y' /><label for='creer_conteneur'>Create if necessary ";
...
echo "<div style='display:none'><input type=\"hidden\" name='ok' value=\"Save\" /></div>\n";
echo "<p style='text-align:center;'><input type=\"submit\" name='ok1' value=\"Save\" style=\"font-variant: small-caps;\" /><br/>\n";
