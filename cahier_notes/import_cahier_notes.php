...
//**************** EN-TETE *****************
$titre_page = "Importation of exams in the report card ";
/**
 * Entête de la page
 */
...
//debug_var();
?>
<p class='bold'><a href="index.php?id_racine=<?php echo $id_racine;?>"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return </a></p>
<?php

$titre=htmlentities($current_group['description'])." (".$nom_periode.")";
$titre .= " - IMPORT";

echo "<center><h3 class='gepi'>Importation of exams in the report card </h3></center>\n";

if (!isset($is_posted)) {
	echo "<p>Import exams in the report card, you must provide a correctly formatted file...</p>";
	echo "<p>Please specify the complete name of the <b>CSV</b>  file to import.";
	...
	echo "<p><i>NOTE&nbsp;</i>: The format of the CSV is a little complex.<br />
The first line is used to locate the fields.<br />
The first field of the  student lines must contain the chains GEPI_LOGIN_ELEVE.</p>
...
<p>It is simpler to create an evaluation and export the report card then (<i>even empty</i>) to have a correctly formatted CSV.</p>\n";
	...
	if(!isset($_POST['valide_insertion_devoirs'])) {
		...
		if (trim($csv_file['name'])=='') {
			...
			echo "<a href='".$_SERVER['PHP_SELF']."?id_racine=$id_racine'>Click here</a> to restart !</center></p>";
		}
		else{
			...
			if(!$fp){
				...
				echo "<p><a href='".$_SERVER['PHP_SELF']."?id_racine=$id_racine'>Click here</a> restart !</center></p>\n";
			}
			else{
				...
				if($temoin!=count($tabchamps)){
					echo "<p><b>ERROR:</b> The heading line of the file is not in conformity with what is awaited.</p>\n";
					echo "<p><a href='".$_SERVER['PHP_SELF']."?id_racine=$id_racine'>Click here</a> restart!</center></p>\n";
					/**
                     * Pied de page
                     */
								...
								if(trim($tabligne[$tabindice[1]])!=""){
									...
									if(strlen(preg_replace("/[A-Z0-9_]/","",$tabligne[$tabindice[1]]))==0){
										...
										if(mysql_num_rows($test)>0){
											...
											$info_erreur.=$tab_dev[$cpt_ele]['login']." is not member of the group over the selected period. <br />\n";
										}
									}
								}
								break;
				...
				if(count($nomc_dev)==0){
					echo "<p><b>Erreur:</b> No name of exam was found.</p>\n";
					echo "<p><a href='".$_SERVER['PHP_SELF']."?id_racine=$id_racine'>Click here</a> to restart !</center></p>\n";
					/**
                     * Pied de page
                     */
                    require("../lib/footer.inc.php");
				...
				if(count($nomc_dev)!=count($date_dev)
					...
					echo "<p><b>Erreur:</b> The number of fields does not coincide for the short names, coefficients and dates.</p>\n";
					echo "<p><a href='".$_SERVER['PHP_SELF']."?id_racine=$id_racine'>Click here</a> to restart !</center></p>\n";
					/**
                     * Pied de page
                     */
 ...
 */
		...
		if(!isset($nomc_dev)){
			echo "<p>ERROR: No imported exams.</p>\n";
			/**
             * Pied de page
             */
            require("../lib/footer.inc.php");
		...
		if(!isset($date_dev)){
			echo "<p>ERROR: No definite date.</p>\n";
			/**
             * Pied de page
             */
            require("../lib/footer.inc.php");
		...
		if(!isset($coef_dev)){
			echo "<p>ERROR: No definite coefficient.</p>\n";
			/**
             * Pied de page
             */
            require("../lib/footer.inc.php");
		...
		if(!isset($note_sur_dev)){
			echo "<p>ERROR: No definite reference of notation.</p>\n";
			/**
             * Pied de page
             */
            require("../lib/footer.inc.php");
		...
		if(!isset($login_ele)){
			echo "<p>ERROR: No imported student.</p>\n";
			/**
             * Pied de page
             */
            require("../lib/footer.inc.php");
			...
			if($nomc_dev[$i]!=""){
				if(isset($valide_import_dev[$i])){
					...
					if($res_insert){
						...
						echo "<p><b>Error</b> during the creation of the exam n°$i (<i>$nomc_dev[$i]</i>).</p>\n";
						echo "<p><a href='".$_SERVER['PHP_SELF']."?id_racine=$id_racine'>Click here</a> to restart !</center></p>\n";
						/**
                         * Pied de page
                         */
		...
		echo "<p><a href='saisie_notes.php?id_conteneur=$id_racine'>Visualize the exams</a></p>\n";
	}
}
echo "<p><br /></p>\n";
/**
