 ...
 * 
 * $Id: index.php 8427 2011-10-04 17:13:37Z crob $
 *
...
if (is_numeric($id_groupe) && $id_groupe > 0) {
    ...
    if(mysql_num_rows($test_prof_groupe)==0) {
		...
		tentative_intrusion(1, "Attempt at access to a report card which does not belong to him (id_groupe=$id_groupe)");
...
if ((isset($_POST['id_racine'])) or (isset($_GET['id_racine']))) {
    ...
    if (!(Verif_prof_cahier_notes ($_SESSION['login'],$id_racine))) {
        $mess=rawurlencode("You try to penetrate in a report card which does not belong to you !");
		unset($_SESSION['id_groupe_session']);
		tentative_intrusion(1, "Attempt at access to a report card which does not belong to him (id_racine=$id_racine)");
        header("Location: index.php?msg=$mess");
        die();
    }
}
...
if(isset($_GET['clean_anomalie_cn'])) {
	...
	if(preg_match('/^[0-9]*$/', $suppr_id_dev)) {
		...
		if(mysql_num_rows($test)>0) {
			...
			if(!$suppr) {
				$msg.="Error during the suppression of the notes associated with the exam n°$suppr_id_dev.<br />";
			}
			else {
				$sql="DELETE FROM cn_devoirs WHERE id='".$suppr_id_dev."';";
				$suppr=mysql_query($sql);
				if(!$suppr) {
					$msg.="Error during the suppression of the exam n°$suppr_id_dev.<br />";
				}
				else {
					$msg.="Exam n°$suppr_id_dev deleted.<br />";
				}
			}
		}
		else {
			$msg.="You try to remove an exam which does not belong to you.<br />";
		}
	}
}
...
if(isset($_GET['clean_anomalie_dev'])) {
	...
	if((!isset($tmp_id_groupe))||(!isset($tmp_periode_num))) {
		$msg="The group or the period is not defined.<br />";
	}
	else {
		...
		if(mysql_num_rows($test_cn)==0) {
			$msg="Attempt at access to an exam nonassociated with one of your report card.<br />";
		}
		else {
			...
			if(mysql_num_rows($res_a)==0) {
				$msg="No anomaly is raised for the exam n°".$_GET['clean_anomalie_dev'].".<br />";
			}
			else {
				if(!isset($msg)) {$msg="";}
				while($lig_a=mysql_fetch_object($res_a)) {
					...
					$del=mysql_query($sql);
					if($del) {
						$msg.="cleaning one for $lig_a->login : $lig_a->note - $lig_a->statut on the exam n°".$_GET['clean_anomalie_dev'].".<br />\n";
					}
					else {
						$msg.="Error during the cleaning for $lig_a->login : $lig_a->note - $lig_a->statut on the exam n°".$_GET['clean_anomalie_dev'].".<br />\n";
					}
				}
			}
		}
...
if ((isset($_GET['creer_structure'])) and ($current_group["classe"]["ver_periode"]["all"][$periode_num] >= 2)) {
  ...
  function recopie_arbo($id_racine, $id_prec,$id_new) {
    ...
    if ($vide == 'yes') {
		echo "<p><center><b><font color='red'>Empty structure : no";
		if(getSettingValue('gepi_denom_boite_genre')=="f") {$accord_f="e";} else {$accord_f="";}
		echo " ";
		echo getSettingValue('gepi_denom_boite');
		echo " was not cré in the report card of the previous period.</font></b></center></p><hr />";
	}
}
/*
$fich=fopen("/tmp/test_img.txt","a+");
...
if  (isset($id_racine) and ($id_racine!='')) {
	...
	*/
    ...
    if ((isset($_GET['del_cont'])) and ($_GET['js_confirmed'] ==1)) {
	    ...
	    //if((isset($_GET['alea']))&&($_GET['alea']==$_SESSION['gepi_alea'])) {
			...
			if (($nb_dev != 0) or ($nb_cont != 0)) {
				...
				echo 'alert("Impossible to remove a box which is not empty !");\n';
				echo "</script>\n";
			} else {
    ...
    echo "<a href='index.php?id_groupe=no_group'> My lesson </a> | \n";
...
if(($_SESSION['statut']=='professeur')||($_SESSION['statut']=='secours')) {
    ...
    echo "<a href='index.php?id_groupe=" . $current_group["id"] . "'> Choose another period</a> | \n";
...
var tab_per_cn=new Array();\n";
	...
	function confirm_changement_periode(thechange, themessage)
	{
		...
		echo "<span title='Access the report card of the period (only the periods for which the report card was
initialized  are proposed )'>Period</span> <select name='id_racine' id='id_racine' onchange=\"confirm_changement_periode(change, '$themessage');\">\n";
    ...
    echo "<a href='export_cahier_notes.php?id_racine=".$id_racine."'>Export the notes</a> | \n";
	//==================================
    ...
    if ($current_group["classe"]["ver_periode"]["all"][$periode_num] >= 2) {
		...
		//==================================
		// AJOUT: boireaus EXPORT...
		echo "<a href='import_cahier_notes.php?id_racine=".$id_racine."'>Import the notes</a> | \n";
		//==================================

        echo "<a href='add_modif_conteneur.php?id_racine=$id_racine&amp;mode_navig=retour_index'> Create a";
        ...
        if ($periode_num!='1')  {
            $themessage = 'By clicking on OK, you will create the same structure of boxes as that of the previous period. If boxes already exist, they will not be removed.';
            echo "<a href='index.php?id_groupe=$id_groupe&amp;periode_num=$periode_num&amp;creer_structure=yes".add_token_in_url()."'  onclick=\"return confirm_abandon (this, 'yes', '$themessage')\"> Create the same structure as the previous period</a> | \n";
        }
    }
	...
	echo "<a href=\"../groupes/signalement_eleves.php?id_groupe=$id_groupe&amp;chemin_retour=../cahier_notes/index.php?id_groupe=$id_groupe\"> Indicate assignment errors </a>";
	...
	if((isset($tab_anomalie_cn_pour_groupe_hors_cn))&&(count($tab_anomalie_cn_pour_groupe_hors_cn)>0)) {
		...
		foreach($tab_anomalie_cn_pour_groupe_hors_cn as $tmp_id_groupe => $tmp_cn) {
			...
			if(mysql_num_rows($res_cn_dev)>0) {
				if($info_anomalie=="") {
					$info_anomalie="<div style='border:1px solid red; margin: 1em;'><p><span style='color:red; font-weight:bold;'>ANOMALIE&nbsp;:</span> At least one exam was created in a course which should not normally appear in the Report cards.<br />It would be advisable to remove it, or to transfer it (<em>if for example, it was created in a sub-group, instead of the class group</em>).</p>\n";
				}

				$tmp_group=get_group($tmp_id_groupe);
				$info_anomalie.="<p class='bold'>Exam(s) in ".$tmp_group['name']." (<em>".$tmp_group['description']."</em>) en ".$tmp_group['classlist_string']."&nbsp;:</p>\n";
				$info_anomalie.="<ul>\n";
				while($lig_dev=mysql_fetch_object($res_cn_dev)) {
					...
					if(mysql_num_rows($test_notes_dev)==0) {
						$info_anomalie.=" - No note - ";
					}
					else {
						$info_anomalie.=" - <a href='export_cahier_notes.php?id_racine=".$tmp_cn."' target='_blank'>Export the notes</a> - ";
					}

					$info_anomalie.="<a href='".$_SERVER['PHP_SELF']."?clean_anomalie_cn=y&amp;&amp;id_groupe=$id_groupe&amp;periode_num=$periode_num&amp;suppr_id_dev=".$lig_dev->id.add_token_in_url()."'><img src='../images/delete16.png' width='16' height='16' alt='Delete this Exam' title='Delete this Exam' /></a>\n";
					$info_anomalie.="</li>\n";
				}
				$info_anomalie.="</ul>\n";
			}
		...
		if($info_anomalie!="") {
			$info_anomalie.="<p>If you think that one of this lesson should appear in the report cards, contact the administration of your school.</p>\n";
			$info_anomalie.="</div>\n";
		}
    ...
    echo "<h3 class='gepi'>List evaluations of the report card</h3>\n";
    $empty = affiche_devoirs_conteneurs($id_racine,$periode_num, $empty, $current_group["classe"]["ver_periode"]["all"][$periode_num]);
    echo "</ul>\n";
    if ($empty == 'yes') echo "<p><b>Currently, no evaluation.</b> You must create at least an evaluation.</p>\n";
    if ($empty != 'yes') {
		...
		if(mysql_num_rows($test_jgv)==0) {
			if ($current_group["classe"]["ver_periode"]["all"][$periode_num] >= 2) {
				...
				echo "<ul><li><a href='../saisie/saisie_notes.php?id_groupe=$id_groupe&amp;periode_cn=$periode_num&amp;retour_cn=yes'>Typing of the averages</a> $info_mn</li>\n";
				echo "<li><a href='../saisie/saisie_appreciations.php?id_groupe=$id_groupe&amp;periode_cn=$periode_num'>Typing of the appreciations</a> $info_ma</li></ul>\n";
			} else {
				echo "<h3 class='gepi'>Visualization of bulletin ($nom_periode[$periode_num])</h3>\n";
				echo "<ul><li><a href='../saisie/saisie_notes.php?id_groupe=$id_groupe&amp;periode_cn=$periode_num&amp;retour_cn=yes'>Visualization of averages</a> (<b>".$gepiClosedPeriodLabel."</b>).</li>\n";
				echo "<li><a href='../saisie/saisie_appreciations.php?id_groupe=$id_groupe&amp;periode_cn=$periode_num'>Visualization of appreciations</a> (<b>".$gepiClosedPeriodLabel."</b>).</li></ul>\n";
			}
		}
    }
...
if (isset($_GET['id_groupe']) and !(isset($_GET['periode_num'])) and !(isset($id_racine))) {
    ...
    echo "<p class=bold>";
    echo "<a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to Home </a>|";
    echo "<a href='index.php?id_groupe=no_group'> My lesson </a>|</p>\n";
    echo "<p class='bold'>Lesson : ".htmlentities($current_group["description"])." (" . $current_group["classlist_string"] .")</p>\n";

    echo "<H3>Visualization/modification - Choose the period : </H3>\n";
    $i="1";
	...
	if(mysql_num_rows($res_test)==0){
		echo " (<i>closed period</i>)";
	}

	echo "</p>\n";
    $i++;
    }
    echo "<h3>Visualization only : </h3>\n";
    echo "<p><a href='toutes_notes.php?id_groupe=$id_groupe'>See all the evaluations of the year</a></p>\n";

}

if (!(isset($_GET['id_groupe'])) and !(isset($_GET['periode_num'])) and !(isset($id_racine))) {
    ?>
    <p class=bold><a href="../accueil.php"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return to Home</a></p>
    <p>Access to the report card : </p>
    <?php
    $groups = get_groups_for_prof($_SESSION["login"],"classe puis matière");

    if (empty($groups)) {
        echo "<br /><br />";
        echo "<b>No report card is available.</b>";
        echo "<br /><br />";
    }
