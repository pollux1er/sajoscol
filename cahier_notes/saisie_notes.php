...
if ($id_devoir)  {
	...
	if(mysql_num_rows($query)==0) {
		$msg="The container number $id_conteneur do not exist. It is an anomaly.";
...
if (isset($_POST['is_posted'])) {
    ...
    if(mysql_num_rows($test_bulletin)>0) {
        $msg=" CAUTION: Notes are present on the bulletin.<br />If you modified or added notes, think of updating the recopy towards the bulletin.";
    }
    //==========================================================
...
if (isset($_POST['import_sacoche'])) {
	$message_enregistrement = "Your notes are not recorded yet, please check them and click on the button of recording";
} else {
	$message_enregistrement = "The modifications were recorded !";
}
$themessage  = 'Notes were modified. Do you really want to leave without recording ?';
//**************** EN-TETE *****************
$titre_page = "Typing of the notes";
    /**
     * Entête de la page
     */
...
if ($id_devoir != 0) {$titre .= " - TYPING";} else {$titre .= " - VISUALIZATION";}
...
echo "<p class='bold'>\n";
echo "<a href=\"../accueil.php\" onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to Home </a>|";
echo "<a href='index.php'  onclick=\"return confirm_abandon (this, change, '$themessage')\"> My lessons </a>|";
...
if(mysql_num_rows($res_cn)>1) {
	...
	echo "<span title='Access the report card of the period (only the periods for which the book of notes was initialized are proposed)'>Period </span><select name='tmp_id_conteneur' id='id_conteneur' onchange=\"confirm_changement_periode(change, '$themessage');\">\n";
	echo $chaine_options_periodes;
	echo "</select> | \n";

}

echo "<a href=\"index.php?id_racine=$id_racine\" onclick=\"return confirm_abandon (this, change, '$themessage')\"> My evaluations</a> |";
...
if((isset($id_devoir))&&($id_devoir!=0)) {echo "<a href=\"saisie_notes.php?id_conteneur=$id_racine\" onclick=\"return confirm_abandon (this, change, '$themessage')\"> Visualization of CN </a>|";}
...
if ($current_group["classe"]["ver_periode"]["all"][$periode_num] >= 2) {
	...
	echo "<a href='add_modif_conteneur.php?id_racine=$id_racine&amp;mode_navig=retour_saisie&amp;id_retour=$id_conteneur' onclick=\"return confirm_abandon (this, change,'$themessage')\"> Create a";

	if(getSettingValue("gepi_denom_boite_genre")=='f'){echo "e";}

	echo " ".htmlentities(strtolower(getSettingValue("gepi_denom_boite")))." </a>|";

	echo "<a href='add_modif_dev.php?id_conteneur=$id_racine&amp;mode_navig=retour_saisie&amp;id_retour=$id_conteneur' onclick=\"return confirm_abandon (this, change,'$themessage')\"> Create an evaluation </a>|";
}
echo "<a href=\"../fpdf/imprime_pdf.php?titre=$titre_pdf&amp;id_groupe=$id_groupe&amp;periode_num=$periode_num&amp;nom_pdf_en_detail=oui\" onclick=\"return VerifChargement()\"> Print in the PDF format  </a>|";

echo "<a href=\"../groupes/signalement_eleves.php?id_groupe=$id_groupe&amp;chemin_retour=../cahier_notes/saisie_notes.php?id_conteneur=$id_conteneur\"> Indicate assignment errors </a>";
echo "</p>\n";
echo "</form>\n";
...
if ($id_devoir == 0) {
	...
	echo "<table summary='Paramètres'><tr><td>Mask the columns \"comments\" not empty (visualization mode only) :
...
$detail = "Mode of calculation of the average :\\n";
$detail = $detail."The average is carry out on the columns located by the cells of color violet.\\n";
if (($nb_dev_sous_cont != 0) and ($_SESSION['affiche_tous'] == 'no'))
	$detail = $detail."CAUTION : click on \' Display the evaluations of sub".htmlentities(strtolower(getSettingValue("gepi_denom_boite")))."s\' to reveal all the evaluations which intervene in the average.\\n";
if ($arrondir == 's1') $detail = $detail."The average is round to the tenth of higher mark.\\n";
if ($arrondir == 's5') $detail = $detail."The average is round to the half-mark superior.\\n";
if ($arrondir == 'se') $detail = $detail."The average is round at the higher whole mark.\\n";
if ($arrondir == 'p1') $detail = $detail."The average is round to the nearest tenth of mark .\\n";
if ($arrondir == 'p5') $detail = $detail."The average is round to the neares thalf-mark .\\n";
if ($arrondir == 'pe') $detail = $detail."The average is round at the nearest whole mark .\\n";
if ($ponderation != 0) $detail = $detail."Weighting : ".$ponderation." (is added to the coefficient of best notes each pupil).\\n";
...
if (($nb_dev == 0) and ($nb_sous_cont==0)) {
	...
	echo htmlentities(strtolower(getSettingValue("gepi_denom_boite")))." $nom_conteneur don't contains any evaluation. </p>\n";
...
if ($id_devoir != 0) {
        $appel_note_sur = mysql_query("SELECT NOTE_SUR FROM cn_devoirs WHERE id = '$id_devoir'");
        $note_sur_verif = mysql_result($appel_note_sur,'0' ,'note_sur');
	echo "<p class='cn'>Type a note from 0 to ".$note_sur_verif." for each student, or the code 'a' for 'absent', the code 'd' for 'exempted', the code '-' or 'n' for absence of note.</p>\n";
	echo "<p class='cn'>You also can <b>import your notes directly by \"copy/paste\"</b> from a spreadsheet or of another application : see at bottom of this page.</p>\n";

}
echo "<p class=cn><b>Course : ".$current_group['description']." (" . $current_group["classlist_string"] . ")";
echo "</b></p>\n";
...
if ($nb_dev != 0) {
	if($nom_conteneur!=""){
		...
		echo "<div style='float:right; width:16;'><a href='javascript:affichage_quartiles();'><img src='../images/icons/histogramme.png' width='16' height='16' alt='Display the quartiles' title='Display the quartiles' /></a></div>\n";
...
if ($id_devoir==0) {
	...
	while ($i < $nb_sous_cont) {
		...
		echo "<a href=\"./saisie_notes.php?id_conteneur=$id_sous_cont[$i]\"  onclick=\"return confirm_abandon (this, change,'$themessage')\">Visualization</a>\n";
	...
	if (($id_devoir==0) or ($id_dev[$i] == $id_devoir)) {
		...
		if ($current_group["classe"]["ver_periode"]["all"][$periode_num] >= 2) {
			...
			if($display_parents[$i]!=0) {
				echo " <img src='../images/icons/visible.png' width='19' height='16' title='Evaluation Visible on the report booklet' alt='Evaluation Visible on the report booklet' />\n";
			}
			else {
				echo " <img src='../images/icons/invisible.png' width='19' height='16' title='Evaluation not Visible on the report booklet' alt='Evaluation not Visible on the report booklet' />\n";
			}
			echo "</center></td>\n";
		}
		else {
			...
			if($display_parents[$i]!=0) {
				echo " <img src='../images/icons/visible.png' width='19' height='16' title='Evaluation Visible on the report booklet' alt=Evaluation Visible on the report booklets' />\n";
			}
			else {
				echo " <img src='../images/icons/invisible.png' width='19' height='16' title='Evaluation not Visible on the report booklet' alt='Evaluation not Visible on the report booklet' />\n";
			}
...
if ($id_devoir==0) {
	...
	if((isset($id_conteneur))&&($current_group["classe"]["ver_periode"]["all"][$periode_num] >= 2)) {
		echo "<br /><a href='".$_SERVER['PHP_SELF']."?id_conteneur=$id_conteneur&amp;recalculer=y".add_token_in_url()."'>Recompute</a>";
	}
	echo "</center></td>\n";
	$header_pdf[] = "Moyenne";
	$w_pdf[] = $w2;
...
while ($i < $nb_dev) {
	...
	if (($id_devoir==0) or ($id_dev[$i] == $id_devoir)) {
		if ($id_dev[$i] == $id_devoir) {
			echo "<td class=cn valign='top'><center><a href=\"saisie_notes.php?id_conteneur=$id_conteneur&amp;id_devoir=0\" onclick=\"return confirm_abandon (this, change,'$themessage')\">Visualize</a></center></td>\n";
			echo "<td class=cn valign='top'>&nbsp;</td>\n";
		} else {
			if ($current_group["classe"]["ver_periode"]["all"][$periode_num] >= 2) {
				echo "<td class=cn valign='top'><center><a href=\"saisie_notes.php?id_conteneur=$id_conteneur&amp;id_devoir=$id_dev[$i]\" onclick=\"return confirm_abandon (this, change,'$themessage')\">Type</a></center></td>\n";
			}
			else {
				echo "<td class=cn valign='top'>&nbsp;</td>\n";
			}
...
if ($id_devoir==0) {
	...
	while ($i < $nb_sous_cont) {
		if ($_SESSION['affiche_tous'] == 'yes') {
			$m = 0;
			while ($m < $nb_dev_s_cont[$i]) {
				echo "<td class=cn valign='top'><center><a href=\"saisie_notes.php?id_conteneur=".$id_sous_cont[$i]."&amp;id_devoir=".$id_s_dev[$i][$m]."\" onclick=\"return confirm_abandon (this, change,'$themessage')\">Type</a></center></td>\n";
				$m++;
			}
		}
...
echo "<tr><td class='cn' valign='top'><b>" .
		"<a href='saisie_notes.php?id_conteneur=".$id_conteneur."&amp;id_devoir=".$id_devoir."&amp;order_by=nom' onclick=\"return confirm_abandon (this, change,'$themessage')\">Name First name</a></b></td>";
if ($multiclasses) {echo "<td><a href='saisie_notes.php?id_conteneur=".$id_conteneur."&amp;id_devoir=".$id_devoir."&amp;order_by=classe' onclick=\"return confirm_abandon (this, change,'$themessage')\">Class</a></td>";}
echo "\n";
	...
	if (($id_devoir==0) or ($id_dev[$i] == $id_devoir)) {
		...
		if(getSettingValue("note_autre_que_sur_referentiel")=="V" || $note_sur[$i]!=getSettingValue("referentiel_note")) {
			...
			if ($ramener_sur_referentiel[$i] != 'V') {
				...
				$tabdiv_infobulle[]=creer_div_infobulle('ramenersurReferentiel_'.$i,"","","The note is brought back on ".getSettingValue("referentiel_note")." for the calculation of the average","",15,0,'n','n','n','n');
				...
				echo "<font size=-2>Note on ".$note_sur[$i];
				echo "</a><br />";
			}
		} else {
			...
			while ($m < $nb_dev_s_cont[$i]) {
				...
				if(getSettingValue("note_autre_que_sur_referentiel")=="V" || $note_sur_s_dev[$i][$m]!=getSettingValue("referentiel_note")) {
					...
					if ($ramener_sur_referentiel_s_dev[$i][$m] != 'V') {
						echo "<font size=-2>Note on ".$note_sur_s_dev[$i][$m]."<br />";
					} else {
						$tabdiv_infobulle[]=creer_div_infobulle("ramenersurReferentiel_s_dev_".$i."_".$m,"","","The note is brought back on ".getSettingValue("referentiel_note")." for the calculation of the average","",15,0,'n','n','n','n');
...
$graphe_titre="Distribution of notes";
...
if(($id_devoir>0)||($nb_sous_cont==0)) {
	for($k=$n_dev_0;$k<$n_dev_fin;$k++) {
		...
		if(isset($tab_graph[$k])) {
			...
			echo ">";
			echo "<img src='../images/icons/histogramme.png' alt='Distribution of notes' />";
			echo "</a>";
		}
		else {
			echo "&nbsp;";
...
while($i < $nombre_lignes) {
	...
	if ($eleve_classe[$i] != $prev_classe && $prev_classe != null && $order_by == "classe") {
		...
		if($id_devoir!=0) {
			...
			echo "\"><img src='../images/icons/buddy.png' width='16' height='16' alt='Afficher la photo élève' title='Display the photo of student' /></a></div>\n";
		}
...
if(getPref($_SESSION['login'], 'cn_avec_mediane_q1_q3', 'y')=='y') {
	...
	echo "<tr>\n";
	echo "<td class='cn bold'><b>Medians&nbsp;:</b></td>\n";
	if ($multiclasses) {
		echo "<td class='cn bold'>&nbsp;</td>\n";
		$data_pdf[$tot_data_pdf][]='';
	}
	...
	echo "<tr>\n";
	echo "<td class='cn bold'><b>1st quartile&nbsp;:</b></td>\n";
	if ($multiclasses) {
		echo "<td class='cn bold'>&nbsp;</td>\n";
		$data_pdf[$tot_data_pdf][]='';
	}
	...
	echo "<tr>\n";
	echo "<td class='cn bold'><b>3rd quartile&nbsp;:</b></td>\n";
	if ($multiclasses) {
		echo "<td class='cn bold'>&nbsp;</td>\n";
		$data_pdf[$tot_data_pdf][]='';
	}
...
<?php if ($id_devoir != 0) echo "<br /><center><div id=\"fixe\"><input type='submit' value='Save' /></div></center>\n"; ?>
</form>
<?php
...
if ($id_devoir) {
	...
	function affiche_lien_tri() {
		...
		//alert(chaine1);
		//alert(chaine2);
		document.getElementById('p_tri').innerHTML='<a href=\'affiche_tri.php?titre=Notes&chaine1='+chaine1+'&chaine2='+chaine2+'\' onclick=\"effectuer_tri(); afficher_div(\'div_tri\',\'y\',-150,20); return false;\" target=\'_blank\'>Display sorted the notes</a>';
	}
	...
	echo add_token_field();
	echo "<h3 class='gepi'>Direct importation of the notes by copy/paste from a spreadsheet</h3>\n";
	echo "<table summary=\"Table of importation\"><tr>\n";
	echo "<td>De la ligne : ";
	...
	echo "</select>\n";
	echo "</td><td>\n";
	echo "Paste the data below to be imported: <br />\n";
	...
	echo "<p><b>Significant remark :</b> the importation consider only the students whose name are displayed above !<br />Be thus vigilant to paste only the notes of these students, in the good order.</p>\n";
	echo "</form></fieldset>\n";

	if (isset($_POST['notes'])) {
		echo "<script type=\"text/javascript\" language=\"javascript\">
		<!--
		alert(\"Caution, the imported notes are not recorded yet in base GEPI. You must confirm the importation (button 'Register') !\");
		changement();
		//-->
		</script>\n";
	}
  ...
  if (isset($_POST['appreciations'])) {
  	echo "<script type=\"text/javascript\" language=\"javascript\">
  	<!--
  	alert(\"Caution, the imported appreciations are not recorded yet in base GEPI. You must confirm the importation (button 'Register') !\");
  	//-->
  	</script>\n";
  }
}
...
if ($id_devoir) {
	...
	echo "<form enctype=\"multipart/form-data\" action=\"saisie_notes.php\" method=post>\n";
	echo add_token_field();
	echo "<h3 class='gepi'>Direct importation of the appreciations by copy/paste from a spreadsheet</h3>\n";
	echo "<table summary=\"Table of importation\"><tr>\n";
	echo "<td>De la ligne : ";
		echo "<SELECT name='debut_import' size='1'>\n";
	$k = 1;
	...
	echo "</select>\n";
	echo "</td><td>\n";
	echo "Paste below the data  to be imported&nbsp;: <br />\n";
	...
	echo "<input type='hidden' name='order_by' value='$order_by' />\n";
	echo "<center><input type='submit' value='Importer'  onclick=\"return confirm_abandon (this, change, '$themessage')\" /></center>\n";
	echo "<p><b>Significant remark :</b> the importation considers only the student whose name is displayed above !<br />Be thus vigilant to paste only the appreciations of these student, in the good order.</p>\n";
	echo "</form></fieldset>\n";
}
...
?>
<br />
* In conformity with the CNIL, the professor commits himself making appear in the report card only notes and comments made available of the student (note and comment present on the copy, ...).
<script type="text/javascript" language="javascript">
chargement = true;
