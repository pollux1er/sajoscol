...
//**************** EN-TETE *****************
$titre_page = "Visualization of all the notes of the year";
...
echo "<a href=\"../fpdf/imprime_pdf.php?titre=$titre_pdf&amp;id_groupe=$id_groupe\" target=\"_blank\" onclick=\"return VerifChargement()\">Print in the Pdf format </a> |";
...
echo "<p class=cn><b>Class : $nom_classe | Course : " . $current_group["description"] . "</b></p>\n";
...
if ($nb_cahier_note == 0) {
   echo "<p class='grand'>No data to display !</p>\n";
   echo "</div>\n";
   echo "</body></html>";
   die();
}
...
//
echo "<table summary='All notes' border='1' cellspacing='2' cellpadding='1'>\n";
...
while ($num_per < $nb_cahier_note) {
	...
	if(getSettingValue("note_autre_que_sur_referentiel")=="V" || $note_sur[$i]!=getSettingValue("referentiel_note")) {
		if ($ramener_sur_referentiel[$i] != 'V') {
			...
			$tabdiv_infobulle[]=creer_div_infobulle('ramenersurReferentiel_'.$i,"Ramener sur referentiel","","The note is brought back on ".getSettingValue("referentiel_note")." for the calculation of the average","",14,0,'y','y','n','n');
    ...
    while ($i < $nb_sous_cont[$num_per]) {
		...
		if ($ramener_sur_referentiel_s_dev[$i][$m] != 'V') {
			...
			$tabdiv_infobulle[]=creer_div_infobulle("ramenersurReferentiel_s_dev_".$i."_".$m,"Bring back on the reference","","The note is brought back on ".getSettingValue("referentiel_note")." for the calculation of the average","",14,0,'y','y','n','n');
...
// quatrième ligne
//
echo "<tr><td class=cn valign='top'><b>Name&nbsp;First name&nbsp;\&nbsp;Coef.</b></td>\n";
...
// Dernière ligne
//
echo "<tr><td class=cn><b>Averages :</b></td>\n";
...
echo "<br /><center><a href=\"../fpdf/imprime_pdf.php?titre=$titre_pdf&amp;id_groupe=$id_groupe\" target=\"_blank\">Print in PDF format</a></center>\n";
