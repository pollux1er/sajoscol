...
// Christian renvoye vers le fichier PDF
	...
	$message_erreur = '';
		if ( !empty($classe[0]) and ( empty($date_debut) or empty($date_fin) ) and !empty($creer_pdf) and empty($selection_eleve) ) { $message_erreur = 'caution don\'t forget not to select a date of beginning and a completion date !'; }
		if ( empty($classe[0]) and ( !empty($date_debut) and !empty($date_fin) ) and !empty($creer_pdf) and empty($selection_eleve) ) { $message_erreur = 'caution don\'t forget to select the class !'; }
		if ( empty($classe[0]) and ( empty($date_debut) or empty($date_fin) ) and !empty($creer_pdf) and empty($selection_eleve) ) { $message_erreur = 'caution don\'t forget to select the class and a date of beginning and a completion date !'; }
...
function releve_notes($current_eleve_login,$nb_periode,$anneed,$moisd,$jourd,$anneef,$moisf,$jourf) {
	...
	if(mysql_num_rows($call_classe)==0) {
		...
		echo "<p><strong><span class=\"bull_simpl_g\">$current_eleve_nom $current_eleve_prenom</span></strong> is not any more in any class over the selected period.</p>\n";
...
// tableau de notes
	...
	echo "<div class='entete_releve_notes'>\n";
		...
		echo "<div class='entete_releve_notes_centre'>\n";
			echo "<p class='bull_simpl_g'>\n";
				echo "Class of $current_eleve_classe_complet<br />Année scolaire ".getSettingValue("gepiYear");
			//modif ERIC
			echo "\n</p>";
				if ($choix_periode==0) {
					echo "<p class='bull_simpl_g'>\n";
						echo "report booklet <strong>$display_date_debut</strong> to <strong>$display_date_fin</strong>";
					echo "\n</p>";
				} else {
					...
					echo "<p class='bull_simpl_g'>\n";
						echo "<strong>".$data_periode['nom_periode']."</strong> : report booklet";
					echo "\n</p>";
				}
	...
	echo "<table class='tab_releve_note' summary='report booklet. Column 1 : Courses and teachers. Column 2 : Notes and other information'>\n";
		echo "<caption class='invisible no_print'>report booklet</caption>\n";
		echo "<tr>\n";
			//=================================
			// MODIF: chapel 20071019
			echo "<th class='bull_simpl tab_relev_note_m'>\n";
				echo "<strong>Courses</strong><br /><em>Professor</em>\n";
			echo "</th>\n";
			...
			// echo "<tr>\n";
				...
				if($rn_sign_chefetab=='y'){
					...
					echo "<div class='cel_signature nb_cel_$nb_cases'>\n";
						echo "<strong>Signature of the head of school:</strong>";
				...
				if($rn_sign_pp=='y'){
					// echo "<div style='width:$largeur_case; border: 1px;'>\n";
					echo "<div class='cel_signature nb_cel_$nb_cases'>\n";
						echo "<strong>Signature of ".getSettingValue("gepi_prof_suivi").":</strong>";
						for($i=0;$i<$rn_sign_nblig;$i++){
							echo "<br />\n";
						}
					echo "</div>\n";
				...
				if($rn_sign_resp=='y'){
					// echo "<div style='width:$largeur_case; border: 1px;'>\n";
					echo "<div class='cel_signature nb_cel_$nb_cases'>\n";
						echo "<strong>Signature of responsible:</strong>";
						for($i=0;$i<$rn_sign_nblig;$i++){
							echo "<br />\n";
						}
					echo "</div>\n";
...
//**************** EN-TETE *******************************
if (!isset($_POST['display_entete'])) $titre_page = "Visualization of the report booklet";
...
// rajout christian
			...
			if(empty($format) AND ($_SESSION["statut"] != 'autre')) {
				echo "<a href='visu_releve_notes.php?format=pdf'>Impression in pdf format</a>\n";
			} else {
				echo "<a href='visu_releve_notes.php?format='>Impression  in HTML format</a>\n";
			}
		...
		*/
			...
			if($nombreligne==0){
				echo "<p>No class is allotted to you.<br />Contact the administrator so that it carries out the suitable parameter setting in the Management of the classes.</p>\n";
			     ...
			     echo "<p>Click on the class for which you wish to extract the  report booklet:</p>\n";
				$i = 0;
				unset($tab_lien);
				unset($tab_txt);
				while ($i < $nombreligne){
				...
				if ($_SESSION["statut"] != 'autre') {
					echo "<p>Ou <a href='visu_releve_notes_bis.php'>access the new device of the report booklet (<em>HTML and PDF</em>)</a></p>\n";
				}
...
// rajout christian
			...
			*/
			...
			//=========================
							...
							echo "<label for='display_date_debut' class='invisible'>Date of beginning</label>\n";
							...
							echo "<label for='display_date_fin' class='invisible'>Completion date</label>\n";
							echo "<a href=\"#calend\" onclick=\"".$cal4->get_strPopup('../lib/calendrier/pop.calendrier_id.php', 350, 170)."\">\n";
								echo "<img src='../lib/calendrier/petit_calendrier.gif' alt='Calendrier' />\n";
							echo "</a>\n";
							echo "<br />\n";
							echo "<p class='info'>\n";
								echo "(Please respect the format jj/mm/aaaa)\n";
							echo "</p>\n";

		 					//echo "<br />\n";
							echo "<h2 class='invisible'>Classes</h2>";
							echo "<select id='classes' name='classe[]' size='6' multiple='multiple' tabindex='3'>\n";
							echo "<label for='classes' class='invisible'>Choice of the class</label>\n";
		  						echo "<optgroup label='----- Lists of classes -----'>\n";
							...
							echo "<h2 class='invisible'>Student</h2>";
		  					echo "<input value=' Student List  >' id='selection_eleve' name='selection_eleve' onclick=\"this.form.submit();this.disabled=true;this.value='In progress'\" type='submit' title='Transfer the student from the selected classes' alt='Transfer the students from the selected classes' />\n";
							echo "<label for='selection_eleve' class='invisible'>Transfer of students</label>\n";
		  					echo "<select id'eleves' name='eleve[]' size='6' multiple='multiple' tabindex='4'>\n";
							echo "<label for='eleves' class='invisible'>Student selection</label>\n";
		  						echo "<optgroup label='----- Lists of Students-----'>\n";
									...
									if(empty($classe[0]) and empty($eleve)) {
										echo "<option value='' disabled='disabled'>empty</option>\n";
									}
						  		echo "</optgroup>\n";
							echo "</select>\n";
							if ( $message_erreur != '' ) {
							...
							echo "<div class='txt_gauche'>\n";
								echo "<span id='ligne_adresse_parent'>\n";
									echo "<br />\n";
							echo "<label for='avec_adresse_responsable' class='invisible'>with responsible address</label>\n";
									...
									echo "<label for='avec_adresse_responsable' class='curseur_pointeur'>\n";
										echo "Display the responsible addresses.\n";
									echo "</label>\n";
									echo "<br />\n";

									echo "<blockquote>\n";
							echo "<!-- BEGINNING ADDITION of Eric 16022008-->\n";
									echo "<strong>choice of the address to print</strong>\n";
									echo "<br />\n";
									...
									echo " />\n";
									echo "<label for='choix_adr_parent1' class='curseur_pointeur'> All the responsibles 1</label>\n";
									echo "<br />\n";
									...
									echo "<label for='choix_adr_parent2' class='curseur_pointeur'>\n";
										echo "All the responsibles 1 and only the responsibles 2 different from the responsibles 1\n";
									echo "</label>\n";
									echo "<br />\n";
									...
									echo " />";
									echo "<label for='choix_adr_parent3' class='curseur_pointeur'>All the responsibles 2</label>";
									echo "<br />";
									...
									echo "<label for='choix_adr_parent4' class='curseur_pointeur'>\n";
										echo "Only  the responsibles 2 different from the responsibles 1\n";
									echo "</label>\n";
							...
							echo "<h2 class='invisible'>Type of impression</h2>";
							echo "Type\n";
							...
							echo "<h2 class='invisible'>Other options</h2>";
			...
			// DEBUT AJOUT chapel 20071026
										echo "Display of the name of the class\n";
			...
			// FIN AJOUT chapel 20071026
										...
										echo "<label for='avec_nom_devoir' class='curseur_pointeur'>\n";
											echo "Display the name of exams.\n";
										echo "</label>\n";
										echo "<br />\n";
										...
										echo "<label for='avec_appreciation_devoir' class='curseur_pointeur'>\n";
											echo "Display the appreciations (when the professor has validated this option).\n";
										echo "</label>\n";
										echo "<br />\n";
										...
										echo "Dsiplay all the coefficients of the exams.\n";
										echo "</label>\n";
										echo "<br />\n";
										echo "&nbsp;&nbsp;&nbsp;&nbsp;\n";
										...
										echo "<label for='avec_coef2' class='curseur_pointeur'>\n";
											echo "Display the coefficients of the exams if different from 1.\n";
										echo "</label>\n";
										echo "<br />\n";
										...
										echo "<label for='avec_date_devoir' class='curseur_pointeur'>\n";
											echo "Display the dates of the exams.\n";
										echo "</label>\n";
										echo "<br />\n";
										...
										echo "<label for='avec_bloc_obser' class='curseur_pointeur'>\n";
											echo "Display the block observation.\n";
										echo "</label>\n";
										echo "<br />\n";
										echo "&nbsp;&nbsp;&nbsp;&nbsp;\n";
										...
										echo "<label for='avec_sign_parent' class='curseur_pointeur'>\n";
											echo "Display the block signature of the parents.\n";
										echo "</label>\n";
										echo "<br />\n";
										echo "&nbsp;&nbsp;&nbsp;&nbsp;\n";
										...
										echo "<label for='avec_sign_pp' class='curseur_pointeur'>\n";
											echo "Display the block of the signature of the principal professor.\n";
										echo "</label>\n";
									echo "</span>\n";
								echo "</div>\n";
							//echo "</div>\n";
...
// fin rajout christian
			...
			if ((getSettingValue("GepiAccesReleveProfP") == "yes") AND (getSettingValue("GepiAccesReleveProf") !="yes") AND (getSettingValue("GepiAccesReleveProfTousEleves") !="yes") AND (getSettingValue("GepiAccesReleveProfToutesClasses") !="yes")) {
				...
				$nombre_classe = mysql_num_rows($call_prof_classe);
					if ($nombre_classe == "0") {
						echo "<p class='message-erreur'>You are not ".getSettingValue("gepi_prof_suivi")." ! You cannot access this page.<p></body></html>\n";
						die();
					} else {
						$i = "0";
						echo "<h2 class='choix_classe'>You are ".getSettingValue("gepi_prof_suivi")." in the class of :</h2>\n";
						while ($i < $nombre_classe) {
							$id_classe = mysql_result($call_prof_classe, $i, "id");
							$classe_suivi = mysql_result($call_prof_classe, $i, "classe");
							echo "<p>$classe_suivi --- <a href='visu_releve_notes.php?id_classe=$id_classe'> report booklet.</a></p>\n";
							$i++;
						}
					}
					...
					if (getSettingValue("GepiAccesReleveProfTousEleves") == "yes" AND (getSettingValue("GepiAccesReleveProfToutesClasses") != "yes")) {
						echo "<p>You can choose to visualize the reports booklets of all the students of the classes in which you teach, or only the students whom you have indeed in progress. If you teach only to whole classes, that remains the same.</p>\n";
						...
						if ($nb_classes == "0") {
							echo "<p>You are not professor in any class !</p>\n";
							echo "</body>\n</html>\n";
							die();
						}
						...
						$i = "0";
						if ($nb_classes == "1") {
							echo "<h2 class='choix_classe'>You are professor in the following class :</h2>\n";
						} else {
							echo "<h2 class='choix_classe'>You are professor in the following classes :</h2>\n";
						}
						while ($i < $nb_classes) {
						...
						echo "<p>You can choose to visualize the report booklets of all the classes, or only for the students whom you have indeed in progress.</p>\n";
						...
						$i = "0";
						if ($nb_classes == "1") {
							echo "<h2 class='choix_classe'>You have access to the following class :</h2>\n";
						} else {
							echo "<h2 class='choix_classe'>You have access to the following classes :</h2>\n";
						}
        ...
        //echo $requete_sql;
					...
					if ($nb_groupes == "0") {
						echo "<p>You are not a professor in any group ! You cannot access the report booklet...</p>\n";
						echo "</body></html>\n";
						die();
					}
					...
					$i = "0";
					if ($nb_groupes == "1") {
						echo "<h2 class='choix_classe'>You are a professor in the following group :</h2>\n";
					} else {
						echo "<h2 class='choix_classe'>You are a professor in the following groups :</h2>\n";
					}
					while ($i < $nb_groupes) {
					...
					tentative_intrusion(2, "Attempt of a professor to access the report booklets without being authorized there.");
					echo "<p>You are not authorized to being here.</p>\n";
					die();
				}
			}
		...
		if (($_SESSION['statut']=='professeur') AND (getSettingValue("GepiAccesReleveProf")!="yes") AND (getSettingValue("GepiAccesReleveProfTousEleves")!="yes") AND (getSettingValue("GepiAccesReleveProfToutesClasses")!="yes") AND (getSettingValue("GepiAccesReleveProfP") != "yes")) {
			tentative_intrusion(2, "Attempt of a professor to access the report booklet without being authorized there.");
			...
			tentative_intrusion(2, "Attempt of a user to access the report booklet without being authorized there.");
			...
			if ($test_classe == '-1') {
				echo "You are not ".getSettingValue("gepi_prof_suivi")." of this class ! You cannot access this page.\n";
				echo "</body>\n";
				echo "</html>\n";
				die();
			}
	...
	*/
			...
			//if (getSettingValue("GepiAccesReleveProfTousEleves") != "yes" AND getSettingValue("GepiAccesReleveProfToutesClasses") == "yes") {
			if (getSettingValue("GepiAccesReleveProfTousEleves") != "yes" AND getSettingValue("GepiAccesReleveProfToutesClasses") != "yes") {
				tentative_intrusion(2, "Attempt of a professor to reach the report booklet of a whole class whereas it is authorized to only reach the report booklet of the student of its groups only.");
				echo "You are not authorized to visualize the whole students of this class ! Select only one group among those to which you teach.</body></html>\n";
				die();
			} else {
				if (getSettingValue("GepiAccesReleveProfToutesClasses") != "yes") {
					...
					$test_classe = sql_query1("SELECT DISTINCT jgc.id_classe FROM " .
	                       ...
	                       ")");
					if ($test_classe == '-1') {
						tentative_intrusion(2, "Attempt of a professor to access the report booklet of a class in which he is not a professor.");
		...
		if (!$current_group) {
			...
			echo "<p class='menu_retour'>\n";
				echo "<a href='../accueil.php'>\n";
					echo "<img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to Home\n";
				echo "</a> | <a href='visu_releve_notes.php'>\n";
					echo "Choose another class</a>\n";
			echo "</p>\n";
			echo "<h2 class='h2_grand'>Class of $nom_classe</h2>\n";
...
//echo "<input type='hidden' name='choix_periode' value='".$choix_periode. "'/>";
						...
						//echo "<div>\n";
							echo "<label for='choix_edit_1' class='curseur_pointeur'>\n";
								echo "report booklet of all the pupils of the class\n";
							echo "</label>\n";
						echo "</div>\n";
					//echo "</tr>\n";
		} else {
				...
				// echo "<div>\n";
					//echo "<tr>\n";
						echo "<p>\n";
							...
							echo "<label for='choix_edit_r1' class='invisible'>report booklet of all the students of the group</label>\n";
						//echo "</div>\n";
						//echo "<div>\n";
							echo "Report booklet of all the Students of the group\n";
						echo "</p>\n";
					//echo "</tr>\n";
		}
	        		//=========================
					...
					if (!$current_group) {
						...
						if ($nb_lignes > 1) {
							...
							//echo "</div>\n";
			         	//echo "<div>n";
							echo "Only the report booklet of the students of which the ".getSettingValue("gepi_prof_suivi")." is :";
	        ...
	        //=========================
					...
					//echo "<div>\n";
						echo "<label for='choix_edit_2' class='curseur_pointeur'>\n";
							echo "Only the report booklet of the selected student: \n";
						echo "</label>\n";
						echo "<label for='select_login_eleve' class='invisible'>Student</label>\n";
...
//echo "<select size=\"1\" name=\"login_eleve\" onchange=\"active(".$indice.")\" onfocus=\"active(".$indice.")\">\n";
		...
		if ($_SESSION['statut'] == "responsable") {
			...
			*/
			...
			if(mysql_num_rows($res_pers_id)==0){
				echo "<p class='menu_retour'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return to Home</a></p>\n";
				echo "<p>Error :It seem that you not have an identifier 'pers_id'. Please contact the administrator.</p>";
				require("../lib/footer.inc.php");
				die();
			}
		...
		if (mysql_num_rows($quels_eleves) == 0) {
			echo "<p class='menu_retour'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to Home</a></p>\n";
			...
			echo "<input type='hidden' id='choix_edit_h2' name='choix_edit' value='2'/>\n</p>\n";
			echo "<p>\n";
			echo "<label for='select_login_eleve1' class='invisible'>Student</label>\n";
	...
	if(($_SESSION['statut']=='eleve')||($_SESSION['statut']=='responsable')) {
		...
		if (($_SESSION['statut']=='eleve') AND (getSettingValue("GepiAccesOptionsReleveEleve") == "yes")) {
   		...
   		echo "<h3 class='h3_gros'>\nDisplay Options : \n</h3>\n<p>\n";
			...
			echo " />\n";
			echo "<label for='avec_nom_devoir' class='curseur_pointeur'> Display the exam's name.</label>\n";
			echo "<br />\n";
			echo "<input type='checkbox' name='avec_appreciation_devoir' id='avec_appreciation_devoir' value='oui' \n";
				if($avec_appreciation_devoir=="y"){echo " checked='checked' ";}
			echo " />\n";
			echo "<label for='avec_appreciation_devoir' class='curseur_pointeur'>Display the appreciations (when the professor has validated this option).</label>\n";
			echo "<br />\n";
			...
			echo " />\n";
			echo "<label for='avec_tous_coef_devoir' class='curseur_pointeur'> Display all the coefficients of the exams.</label>\n";
			echo "<br />\n";
			echo "<input type='checkbox' name='avec_coef_devoir' id='avec_coef_devoir' value='oui'";
				if($avec_coef_devoir=="y"){echo " checked='checked' ";}
			echo " />\n";
			echo "<label for='avec_coef_devoir' class='curseur_pointeur'> Displayt the coefficients of the exams if different coefficients are present.</label>\n";
			echo "<br />\n";
			echo "<input type='checkbox' name='avec_date_devoir' id='avec_date_devoir' value='oui'";
				if($avec_date_devoir=="y"){echo " checked=' checked' ";}
			echo " />\n";
			echo "<label for='avec_date_devoir' class='curseur_pointeur'> Display the dates of the exams.</label>\n";
			echo "</p>";
		}
   ...
   echo "<h3 class='h3_gros'>\nDisplay options : \n</h3>\n<p>\n";
			...
			echo "<input type='checkbox' name='avec_nom_devoir' id='avec_nom_devoir' value='oui'";
				if($avec_nom_devoir=="y"){echo " checked=' checked' ";}
			echo " />\n";
			echo "<label for='avec_nom_devoir' class='curseur_pointeur'> Display the name of exams.</label>\n";
			echo "<br />\n";
			echo "<input type='checkbox' name='avec_appreciation_devoir' id='avec_appreciation_devoir' value='oui'";
				if($avec_appreciation_devoir=="y"){echo " checked='checked' ";}
			echo " />\n";
			...
			echo "<input type='checkbox' name='avec_coef_devoir' id='avec_coef_devoir' value='oui'";
				if($avec_coef_devoir=="y"){echo " checked='checked' ";}
			echo " />\n";
			echo "<label for='avec_coef_devoir' class='curseur_pointeur'> Display the coefficients of the exams if different coefficients are present.</label>\n";
			echo "<br />\n";
			echo "<input type='checkbox' name='avec_date_devoir' id='avec_date_devoir' value='oui' ";
				if($avec_date_devoir=="y"){echo "checked='checked' ";}
			echo "/>\n";
   ...
   echo "<h3 class='h3_gros'>\nOptions d'affichage : \n</h3>\n<p>\n";
		...
		echo "/>\n";
		echo "<label for='rn_sign_chefetab' class='curseur_pointeur'> Display a box for the signature of the head of school.</label>\n";
		echo "<br />\n";
		echo "<input type='checkbox' name='rn_sign_pp' id='rn_sign_pp' value='y' ";
			if($rn_sign_pp=="y"){echo "checked='checked' ";}
		echo "/>\n";
		echo "<label for='rn_sign_pp' class='curseur_pointeur'> Display a box for the signature of ".getSettingValue("gepi_prof_suivi").".</label>\n";
		echo "<br />\n";
		echo "<input type='checkbox' name='rn_sign_resp' id='rn_sign_resp' value='y' ";
			if($rn_sign_resp=="y"){echo "checked='checked' ";}
		echo "/>\n";
		echo "<label for='rn_sign_resp' class='curseur_pointeur'> Display a box for the signature of the parents/responsibles.</label>\n";
		echo "<br />\n";
		echo "<label for='rn_sign_nblig' class='curseur_pointeur'>A number of lines for the signature if a box is displayed: </label>\n";
		...
		echo "<br />\n";
		echo "<label for='rn_formule' class='curseur_pointeur'>Formula to display at the foot of the page:</label>\n";
		echo "<br />\n";
	...
	echo "</p>\n";
	echo "<div class='centre_texte'><input type='submit' id='valider' value='Validate' /></div>\n";
echo "</form>\n";
	...
	if (($_SESSION['statut']=='professeur') AND (getSettingValue("GepiAccesReleveProf")!="yes") AND (getSettingValue("GepiAccesReleveProfTousEleves")!="yes") AND (getSettingValue("GepiAccesReleveProfToutesClasses")!="yes") AND (getSettingValue("GepiAccesReleveProfP") != "yes")) {
		tentative_intrusion(3, "Attempt of a professor to access the report booklet without being authorized there, with voluntary passage of parameters to the page.");
		echo "You cannot acces this page.";
		require("../lib/footer.inc.php");
		die();
	} else if (
    	(($_SESSION['statut'] == 'scolarite') AND (getSettingValue("GepiAccesReleveScol") != "yes"))
    	 ...
    	 OR ($_SESSION['statut'] == 'responsable' AND getSettingValue("GepiAccesReleveParent") != "yes")
    	 OR ($_SESSION['statut'] == 'eleve' AND getSettingValue("GepiAccesReleveEleve") != "yes")) {
		tentative_intrusion(3, "Attempt of an user to reach the report booklet without being authorized there, with voluntary passage of parameters to the page.");
		echo "You cannot access this page.";
		require("../lib/footer.inc.php");
		die();
	} else if (($_SESSION['statut']=='professeur')
		...
		if ($test_classe == '-1') {
			tentative_intrusion(3, "Attempt of a professor to access the report booklet without being authorized there, with voluntary passage of parameters to the page..");
			echo "You are not ".getSettingValue("gepi_prof_suivi")." of this class ! You cannot access this page.</body></html>\n";
			die();
		}
		...
		if (!$current_group) {
            // Dans le cas d'une classe, on vérifie que l'accès est autorisé
			if (getSettingValue("GepiAccesReleveProfTousEleves") != "yes" AND getSettingValue("GepiAccesReleveProfToutesClasses") != "yes") {
				tentative_intrusion(3, "Attempt of a professor to access the report booklet of student for all the class (whereas it is authorized to see only its groups), with voluntary passage of parameters to the page.");
				echo "You are not authorized to visualize the whole of the Students of this class ! Select only one group among those to which you teach.</body></html>\n";
				die();
			} else {
				if (getSettingValue("GepiAccesReleveProfToutesClasses") != "yes") {
	                // il a le droit de visualiser des classes. On vérifie s'il est bien professeur dans la classe demandée
				...
				$test_classe = sql_query1("SELECT DISTINCT jgc.id_classe FROM " .
	                        ...
	                        ")");
				if ($test_classe == '-1') {
					tentative_intrusion(3, "Attempt of a professor to acccess the statements of notes of a class where it does not teach, with voluntary passage of parameters to the page.");
					echo "You are not a professor in this class ! You cannot access this page.\n";
					require("../lib/footer.inc.php");
					die();
				}
			}
		...
		if ($test == "0") {
			tentative_intrusion(3, "Attempt of a professor to access the statements of notes of a group where it does not teach, with voluntary passage of parameters to the page.");
			echo "You are not a professor in this group ! You cannot access this page.\n";
			require("../lib/footer.inc.php");
			die();
		}
	}
...
if ($_SESSION['statut'] == "responsable" OR $_SESSION['statut'] == "eleve") {
	if ($choix_edit != "2") {
		tentative_intrusion(3, "Attempt of a parent or student to differently visualize report booklet than for a single student.");
		echo "You are not authorized to use this mode of visualization.\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	if ($_SESSION['statut'] == "eleve") {
		if (strtoupper($login_eleve) != strtoupper($_SESSION['login'])) {
			tentative_intrusion(3, "Attempt of a pupil to visualize the report booklet of another student.");
			echo "You can visualize only your report booklet.\n";
			require("../lib/footer.inc.php");
			die();
		}
	}
	...
	if ($_SESSION['statut'] == "responsable") {
		...
		if (mysql_result($test, 0) == 0) {
			tentative_intrusion(3, "Attempt of a parent to access to the report booklet of a student of which it is not the legal responsible.");
			echo "You can visualize only the report booklet of the student for whom you are legal responsible.\n";
			require("../lib/footer.inc.php");
			die();
		}
	}
