...
if(isset($id_groupe)) {
	if(($id_groupe=='')||(strlen(my_ereg_replace("[0-9]","",$id_groupe))!=0)) {
		tentative_intrusion(2, "Attempt of a professor to handle the identifier id_groupe by putting at it nonnumerical characters or an empty identifier of group.");
		echo "<p>The identifier of group is erroneous.</p>\n";
	...
	if(mysql_num_rows($test_grp_prof)==0) {
		//intrusion
		tentative_intrusion(2, "Attempt of a professor to access the report booklet of a group with which it is not associated.");
		echo "<p>You are not associated the selected group.</p>\n";
...
if($_SESSION['statut']=='eleve') {
	if(getSettingValue("GepiAccesReleveEleve") != "yes") {
		echo "<p>You are not authorized to access the report booklet.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	if(mysql_num_rows($test_ele_clas)==0) {
		echo "<p>You are not affected in a class and thus not authorized to acces the report booklet.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
...
elseif($_SESSION['statut']=='responsable') {
	...
	$test_ele_clas=mysql_query($sql);
	if(mysql_num_rows($test_ele_clas)==0) {
		echo "<p>None of the students for which you are responsible seems registers in a class; you are thus not authorized to access the report booklet.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	elseif(mysql_num_rows($test_ele_clas)==1) {
...
if ((!isset($tab_id_classe))&&(!isset($id_groupe))) {
	...
	elseif(($_SESSION['statut']=='scolarite')||($_SESSION['statut']=='administrateur')) {
		...
		echo " | ";
		echo "<a href='param_releve_html.php' target='_blank'>Parameters of HTML report</a>";
		echo " | ";
		echo "<a href='visu_releve_notes.php'>Old device</a>";
	}
	else {
		echo "<a href='../accueil.php'>Return to Home</a>";
	}
	echo "</p>\n";

	echo "<p class='bold'>Choice of classes";
	if ($_SESSION['statut']=='professeur') {echo " or of a group";}
	echo ":</p>\n";
	...
	elseif($_SESSION['statut']=='professeur') {
		...
		elseif(getSettingValue("GepiAccesReleveProfP")=="yes") {
			...
			if(mysql_num_rows($test_acces)>0) {
				...
				echo "<p>You are not ".getSettingValue("gepi_prof_suivi").", thus not authorized to access the report booklet of the student.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
		}
		else {
			echo "<p>You are not authorized to access the report booklet of the students.</p>\n";
			require("../lib/footer.inc.php");
			die();
		}
	}
	...
	if($nb_classes==0){
		echo "<p>No class with affected student was found.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	echo "<table width='100%' summary='Table of choice of the classes'>\n";
	...
	while($lig_clas=mysql_fetch_object($call_classes)) {
		...
		echo "<label id='label_tab_id_classe_$cpt' for='tab_id_classe_$cpt' style='cursor: pointer;'><input type='checkbox' name='tab_id_classe[]' id='tab_id_classe_$cpt' value='$lig_clas->id' onchange='unCheckRadio();change_style_classe($cpt)' /> $lig_clas->class</label>";
		echo "<br />\n";
		$cpt++;
	}
...
echo "<script type='text/javascript'>
	...
	if(($_SESSION['statut']=='professeur')&&((getSettingValue("GepiAccesReleveProfToutesClasses") == "yes")||(getSettingValue("GepiAccesReleveProf") == "yes")||(getSettingValue("GepiAccesReleveProfTousEleves") == "yes"))) {
		echo "<p><b>Alternatively</b>, you can choose a group:</p>\n";
...
elseif(!isset($choix_periode)) {
	...
	if($_SESSION['statut']=="professeur") {
		if(isset($id_groupe)) {
			...
			if(mysql_num_rows($res_clas)==0) {
				echo "<p>ERROR: The selected group does not seem associated with any class???</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
	...
	echo "<p class='bold'>Choose the display period for ";
	if(isset($id_groupe)) {
	...
	echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post' name='formulaire'>\n";
	echo "<table border='0' summary='Table of choice of the periods'>\n";
	echo "<tr valign='top'>\n";
	echo "<td>\n";
	...
	echo "<br />\n";
    echo " (<i>please respect the format jj/mm/aaaa</i>)</label>\n";
	echo "</td>\n";
	echo "</tr>\n";
	...
	echo "/><label for='choix_periode' style='cursor: pointer;'> <b>Period</b></label>\n";
	...
	echo "<td>\n";
		...
		echo "<table class='boireaus' border='1' summary='Table of choice of periods'>\n";
		echo "<tr>\n";
		echo "<th>Classe</th>\n";
		for($j=1;$j<=$max_per;$j++) {
	...
	*/
	...
	echo "<p><input type='button' name='valide_choix_periode' value='Validate' onclick='check_et_submit_choix_periode()' /></p>\n";

	echo "<noscript>\n";
	echo "<p><input type='submit' name='valide_choix_periode' value='Validate' /></p>\n";
	echo "</noscript>\n";
	...
	echo "<p><i>Notice&nbsp;:</i></p>\n";
	echo "<blockquote>\n";
	echo "<p>The report of a date to another reveal only the courses in which there are notes.<br />\n";
	echo "The report for a period supplements on the other hand reveal all the courses, even if no note is typed.</p>\n";
	echo "The complete period in general is chosen when we want to print a report at the same time as the bulletin (<i>for example in the verso</i>) and at the end of the period, it is good to have all the courses.</p>\n";
	echo "</blockquote>\n";
...
elseif(!isset($_POST['valide_select_eleves'])) {
	...
	echo "\n<!-- Form of return to the choice of periods -->\n";
	...
	if((isset($_POST['choix_periode']))&&($_POST['choix_periode']=='periode')&&(!isset($_POST['tab_periode_num']))) {
		echo "<p style='color:red'>You chose a report of period, but omitted to choose the period.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}

	echo "<p class='bold'>Selection of the students among the students of ";
	for($i=0;$i<count($tab_id_classe);$i++) {
		if($i>0) {echo ", ";}
		echo get_class_from_id($tab_id_classe[$i]);
	}
	if($choix_periode=='periode') {
		...
		echo "<br />for an extraction of the notes between $display_date_debut and  $display_date_fin";
	}
	echo ":</p>\n";

	echo "\n<!-- Form of choice of the Students and the parameters -->\n";
	...
	echo "<p><input type='button' name='bouton_valide_select_eleves2' value='Validate' onclick='test_check_ele()' /></p>\n";
	...
	echo "<div id='div_param_pdf'>\n";
		...
		echo "/><label for='use_cell_ajustee' style='cursor: pointer;'> Don't use the new function use_cell_ajustee() for the writing of the appreciations.</label>";

		$titre_infobulle="Function cell_ajustee()\n";
		$texte_infobulle="For the appreciations on the bulletins, report,... the function DraxTextBox() of FPDF was used.<br />This function had sometimes a curious behavior with texts truncated or much smaller in the cell than what seemed to be able to hold in the box.<br />The function cell_ajustee() is a function developped at the point to try to do better than DraxTextBox().<br />Like it was not tested per sufficient world on trunk, we put a check box which makes it possible to use the old function DrawTextBox() if cell_ajustee() did not appear as well rotten as we hope for it;o).<br />\n";
		//$texte_infobulle.="\n";
		...
		// Debug
		echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='checkbox' name='releve_pdf_debug' id='releve_pdf_debug' value='y' />&nbsp;<label for='releve_pdf_debug' style='cursor: pointer;'>Activate the debug to post the errors disturbing the generation of pdf.</label>\n";

		$titre_infobulle="Debug\n";
		$texte_infobulle="It happens that the generation of pdf fails.<br />The reasons can be variable (<em>miss of server resources, bug,...</em>).<br />In this case, the presence of a pdf reader plugin mask which errors cause the failure.<br />By checking the box DEBUG, you will obtain the display of the errors and thus you will be able to obtain the assistance more easily on the list 'gepi-users'<br />\n";
		//$texte_infobulle.="\n";
	...
	if ((($_SESSION['statut']=='eleve') AND (getSettingValue("GepiAccesOptionsReleveEleve") != "yes"))||
		...
		echo " / \n";
		echo "<a href='#' onclick=\"document.getElementById('div_param_releve').style.display='none';return false;\">Mask</a>";
		echo " parameters of the reportt booklets</i>).";
		echo "</span>\n";
		echo "</p>\n";
		...
		if (($_SESSION['statut']!='eleve')&&($_SESSION['statut']!='responsable')) {
			...
			echo "/></td><td><label for='un_seul_bull_par_famille' style='cursor: pointer;'>Don't print of report booklet for the second parent<br />(<i>even in the case of separate parents</i>).</label></td></tr>\n";
			...
			echo "/></td><td><label for='deux_releves_par_page' style='cursor: pointer;'>Produce two statements per page (<i>PDF</i>).</label></td></tr>\n";
			...
			echo "/></td><td><label for='tri_par_etab_orig' style='cursor: pointer;'>Sort the report by shool of origin.</label></td></tr>\n";
		...
		echo "<input type='hidden' name='valide_select_eleves' value='y' />\n";
		//echo "<p><input type='submit' name='choix_parametres' value='Validate' /></p>\n";
...
echo "</script>\n";
	...
	if(count($tab_id_classe)>1) {
		...
		*/
		echo " <a href='#' onClick='cocher_tous_eleves();return false;'>Cocher tous les élèves</a> / <a href='#' onClick='decocher_tous_eleves();return false;'>Uncheck all the student</a></p>\n";
	}
	...
	for($i=0;$i<count($tab_id_classe);$i++) {
		...
		echo "<p class='bold'>Class of ".get_class_from_id($tab_id_classe[$i])."</p>\n";

		echo "<table class='boireaus' summary='Tableau of choice of students'>\n";
		echo "<tr>\n";
		echo "<th>Students</th>\n";
		...
		if($choix_periode=='periode') {
			for($j=0;$j<count($tab_periode_num);$j++) {
				...
				echo "<a href=\"javascript:CocheColonneSelectEleves(".$i.",".$j.");\"><img src='../images/enabled.png' width='15' height='15' alt='Check all the student' /></a> / <a href=\"javascript:DecocheColonneSelectEleves(".$i.",".$j.");\"><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all the student' /></a>\n";
		...
		*/
		...
		elseif(($_SESSION['statut'] == 'professeur')&&(getSettingValue("GepiAccesReleveProfP")=="yes")) {
			...
			if(mysql_num_rows($test_acces)>0) {
				...
				echo "<p>You are not ".getSettingValue("gepi_prof_suivi")." of this class, thus not authorized to access the report booklet of these students.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
		}
...
";
	...
	if(eff_eleve_checked==0) {
		...
		//return confirm('No student is selected. Want you nevertheless to generate the report?');
		if(confirm('No student is selected. Want you nevertheless to generate the carryforward?')) {document.forms['formulaire'].submit();}
	}
	else {
	//	alert('At least one student is selected: '+eff_eleve_checked);
...
cocher_tous_eleves();
	...
	*/
	echo "<input type='hidden' name='valide_select_eleves' value='y' />\n";
	//echo "<p><input type='submit' name='bouton_valide_select_eleves2' value='Validate' /></p>\n";
	//echo "<p><input type='submit' name='bouton_valide_select_eleves2' value='Validate' onclick='test_check_ele()' /></p>\n";
	echo "<p><input type='button' name='bouton_valide_select_eleves2' value='Validate' onclick='test_check_ele()' /></p>\n";
	echo "</form>\n";
}
//=======================================================
...
<table class='boireaus' width='100%' summary=\"Table of progression of the extraction/generation\">
<tr>
	...
	*/
	...
	function regime($id_reg) {
		switch($id_reg) {
			...
			case "int.":
				...
				break;
			case "i-e":
				$regime="intern-external";
				break;
			default:
				$regime="Unknown regim???";
				break;
		}
	...
	document.getElementById('td_periode').innerHTML='".$periode_num."';
			...
			if($mode_bulletin!="pdf") {
				...
				echo "<p>Number of student in the class: ".$tab_releve[$id_classe][$periode_num]['eff_classe']."</p>\n";
				//echo "</div>\n";
			}

			//if(!isset($tab_releve[$id_classe][$periode_num]['eleve'])) {
			if((!isset($tab_releve[$id_classe][$periode_num]['eleve']))&&($mode_bulletin!="pdf")) {
				echo "<p>No selected/ checked student in this class for this period.</p>\n";
				echo "</div>\n";
			}
			else {
				...
				if($mode_bulletin!="pdf") {
					...
					echo "<p>".count($tab_releve[$id_classe][$periode_num]['eleve'])." student(s) selected(s) in this class (<i>for this period</i>).</p>\n";
					//+++++++++++++++++++++++++++++++++++
	...
	document.getElementById('td_ele').innerHTML='".$tab_releve[$id_classe][$periode_num]['eleve'][$rg[$i]]['login']."';
										...
										if(($un_seul_bull_par_famille=="non")&&($nb_releves>1)) {$chaine_info_deux_releves=".<br /><span style='color:red'>Several report for the same family&nbsp: the addresses of the two responsible differ.</span><br /><span style='color:red'>If you do not wish a second report, remind to check the box 'Only one report by family'.</span>";}

										echo "<div class='espacement_bulletins'><div align='center'>Spacing (<i>not printed</i>) between the report".$chaine_info_deux_releves."</div></div>\n";
