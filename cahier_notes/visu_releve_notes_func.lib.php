...
/*
*
* $Id: visu_releve_notes_func.lib.php 8460 2011-10-10 20:03:02Z crob $
*
...
width:".$releve_addressblock_logo_etab_prop."%;\n";
			...
			if  ($releve_affiche_eleve_une_ligne == 'no') { // sur plusieurs lignes
				...
				//Eric Ajout
				echo "<br />";
				if ($tab_rel['eleve'][$i]['regime'] == "d/p") {echo "";}
				if ($tab_rel['eleve'][$i]['regime'] == "ext.") {echo "External";}
				if ($tab_rel['eleve'][$i]['regime'] == "int.") {echo "Intern";}
				if ($tab_rel['eleve'][$i]['regime'] == "i-e"){
					echo "Intern&nbsp;external";
					if (strtoupper($tab_rel['eleve'][$i]['sexe'])!= "F") {echo "e";}
				}
				//Eric Ajout
			...
			if($releve_affiche_INE_eleve=="y"){
				echo "<br />\n";
				echo "INE  Number : ".$tab_rel['eleve'][$i]['no_gep'];
			}
			...
			if($releve_addressblock_debug=="y"){echo "border='1' ";}
			echo "summary=\"Table of the heading\" ";
			...
			echo "<p class='bulletin' align='center'><span class=\"releve_grand\">Classe de ".$tab_rel['eleve'][$i]['classe_nom_complet']."<br />School year ".$gepiYear."</span><br />\n";

			if ($choix_periode=='intervalle') {
				echo "Report booklet of <b>".$tab_rel['intervalle']['debut']."</b> to <b>".$tab_rel['intervalle']['fin']."</b></span>";
			} else {
				echo "<b>".$tab_rel['nom_periode']."</b> : Report booklet";
			}
			...
			if  ($releve_affiche_eleve_une_ligne == 'no') { // sur plusieurs lignes
				...
				echo "<b><span class=\"releve_grand\">".$tab_rel['eleve'][$i]['nom']." ".$tab_rel['eleve'][$i]['prenom']."</span></b><br />";
				echo "Born";
				if (strtoupper($tab_rel['eleve'][$i]['sexe'])== "F") {echo " ";}
				echo "&nbsp;le&nbsp;".$tab_rel['eleve'][$i]['naissance'];
				//Eric Ajout
				echo "<br />";
				if ($tab_rel['eleve'][$i]['regime'] == "d/p") {echo "";}
		...
		if((!isset($tab_rel['eleve'][$i]['groupe']))||(count($tab_rel['eleve'][$i]['groupe'])==0)) {
			echo "<div class='noprint' style='background-color: white; color: red; border: 1px solid black;padding: 1em;'>No course is associated to the student";

			if ($choix_periode=='intervalle') {
				echo ",<br />or the student does not have any note on the selected interval of dates (<i>by requiring the display of the report for the whole period, the
course are displayed, even if no note were typed</i>).</p>";
			}
			echo ".<br />\n";

			if ($tab_rel['affiche_categories']) {
				echo "<br />If you think that it is abnormal, it is perhaps due to a bad parameter setting of the categories of courses.<br />";
				...
				echo "It is possible to correct the problem as an administrator by remaking the parameter setting of the categories of matters in 'Management of the bases/Management of the classes/&lt;Une_classe&gt; Parameters' or in 'Management of the bases/Management of the classes/Parameter setting by batches'.\n";
			}
			echo "</div>\n";
		}
		...
		echo "summary=\"Table of the notes\" ";
		echo ">\n";
		echo "<tr>\n";
		echo "<td width=\"$larg_col1\" class='releve'><b>Courses</b><br /><i>Professor</i></td>\n";
		echo "<td width=\"$larg_col2\" class='releve'>Notes on 20</td>\n";
		echo "</tr>\n";
					...
					if(!isset($tab_rel['eleve'][$i]['groupe'][$j]['devoir'])) {
						...
						if(($debug_releve=="y")&&($tab_rel['eleve'][$i]['login']==$debug_ele_login)&&($tab_rel['eleve'][$i]['groupe'][$j]['id_groupe']==$debug_id_groupe)) {
							if(isset($tab_rel['eleve'][$i]['groupe'][$j]['existence_sous_conteneurs'])) {
								...
								echo "<span style='color:red'>No sub-container</span><br />";
							}
						}
		...
		*/
		...
		if(($tab_rel['rn_sign_chefetab']=='y')||($tab_rel['rn_sign_pp']=='y')||($tab_rel['rn_sign_resp']=='y')){
			...
			echo " summary=\"Table of signatures\"";
			echo ">\n";
			echo "<tr>\n";

			if($tab_rel['rn_sign_chefetab']=='y'){
				echo "<td width='$largeur_case'>\n";
				echo "<b>Signature of the head of school:</b>";
			...
			if($tab_rel['rn_sign_pp']=='y'){
				echo "<td width='$largeur_case'>\n";
				echo "<b>Signature of ".$gepi_prof_suivi.":</b>";
				for($m=0;$m<$tab_rel['rn_sign_nblig'];$m++) {
					echo "<br />\n";
				}
				echo "</td>\n";
			...
			if($tab_rel['rn_sign_resp']=='y'){
				echo "<td width='$largeur_case'>\n";
				echo "<b>Signature of responsibles:</b>";
				for($m=0;$m<$tab_rel['rn_sign_nblig'];$m++) {
					echo "<br />\n";
				}
				echo "</td>\n";
		...
		if($tab_rel['rn_formule']!=""){
			...
			echo "<table width='$releve_largeurtableau' style='margin-left:5px; margin-right:5px;' border='0' cellspacing='".$releve_cellspacing."' cellpadding='".$releve_cellpadding."' summary='Formula in bottom of report booklet'>\n";
			echo "<tr>";
...
function releve_pdf($tab_rel,$i) {
	...
	if (!isset($tab_rel['eleve'][$i]['resp'][0])) {
		...
		$tab_adr_ligne1[0]="MISSING ADDRESS";
		$tab_adr_ligne2[0]="";
		$tab_adr_ligne3[0]="";
		$tab_adr_ligne4[0]="";
		$tab_adr_ligne5[0]="";
				...
				if (($tab_rel['eleve'][$i]['resp'][0]['adr_id']==$tab_rel['eleve'][$i]['resp'][1]['adr_id']) OR
					...
					if(($tab_rel['eleve'][$i]['resp'][0]['nom']!=$tab_rel['eleve'][$i]['resp'][1]['nom'])&&
						...
						*/
						...
						if(($tab_rel['eleve'][$i]['resp'][0]['civilite']!="")&&($tab_rel['eleve'][$i]['resp'][1]['civilite']!="")) {
							$tab_adr_ligne1[0]=$tab_rel['eleve'][$i]['resp'][0]['civilite']." and ".$tab_rel['eleve'][$i]['resp'][1]['civilite']." ".$tab_rel['eleve'][$i]['resp'][0]['nom']." ".$tab_rel['eleve'][$i]['resp'][0]['prenom'];
						}
						else {
							$tab_adr_ligne1[0]="M. and Mme ".$tab_rel['eleve'][$i]['resp'][0]['nom']." ".$tab_rel['eleve'][$i]['resp'][0]['prenom'];
						}
					}
...
//echo "\$largeur_cadre_note=$largeur_cadre_note_global - $largeur_utilise = $largeur_cadre_note<br />\n";
		...
		*/
				...
				$pdf->Cell(100,20, "No course is associated.",0,1,'C');
			}
