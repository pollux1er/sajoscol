...
if (isset($id_classe)) {
	...
	if (!is_numeric($id_classe)) {
		tentative_intrusion("3", "Change of the value of id_classe for a nonnumerical type, by changing the value of a field 'hidden' of a form.");
		echo "Error.";
	...
	if ($_SESSION['statut'] == "professeur" AND getSettingValue("GepiAccesMoyennesProfToutesClasses") != "yes") {
		...
		if ($test == "0") {
			tentative_intrusion("3", "Attempt of access by a teacher to a class in which it does not teach, without having the authorization of it. Advanced attempt : change of the values of fields of the type 'hidden' of the form.");
			echo "You cannot access this class because you are not professor there !";
			require ("../lib/footer.inc.php");
			die();
		}
	}
...
if((!isset($id_classe))||(!isset($num_periode))) {
	header("Location: index2.php?msg=".rawurlencode('Choose a class'));
	die();
}
...
//if (($aff_rang) and ($referent=="une_periode")) $ligne1[] = "<IMG SRC=\"../lib/create_im_mat.php?texte=Position&width=22\" WIDTH=\"22\" BORDER=0 ALT=\"Position\">";
...
if (($aff_rang) and ($referent=="une_periode")) {
	//$ligne1[] = "<IMG SRC=\"../lib/create_im_mat.php?texte=".rawurlencode("Position")."&amp;width=22\" WIDTH=\"22\" BORDER=\"0\" ALT=\"Position\" />";
	$ligne1[] = "<a href='#' onclick=\"document.getElementById('col_tri').value='".(count($ligne1)+1)."';".
				"document.getElementById('sens_tri').value='inverse';".
				"document.forms['formulaire_tri'].submit();\">".
				"<IMG SRC=\"../lib/create_im_mat.php?texte=".rawurlencode("Position")."&amp;width=22\" WIDTH=\"22\" BORDER=\"0\" ALT=\"Position\" />".
				"</a>";
	$ligne1_csv[]="Position";
}
...
$col[1][$nb_lignes_tableau+$ligne_supl] = "Average";
...
if ($ligne_supl == 1) {
	...
	foreach($displayed_categories as $cat_id) {
		$nb_col++;
		//$ligne1[$nb_col] = "<IMG SRC=\"../lib/create_im_mat.php?texte=".rawurlencode("Average : " . $cat_names[$cat_id])."&amp;width=22\" WIDTH=\"22\" BORDER=\"0\" alt=\"".$cat_names[$cat_id]."\" />";

		$ligne1[$nb_col] = "<a href='#' onclick=\"document.getElementById('col_tri').value='".$nb_col."';".
				"document.forms['formulaire_tri'].submit();\">".
				"<IMG SRC=\"../lib/create_im_mat.php?texte=".rawurlencode("Average : " . $cat_names[$cat_id])."&amp;width=22\" WIDTH=\"22\" BORDER=\"0\" ALT=\"".$cat_names[$cat_id]."\" />".
				"</a>";

		$ligne1_csv[$nb_col] = "Average : " . $cat_names[$cat_id];
	...
	// La moyenne générale
	$nb_col++;
	//$ligne1[$nb_col] = "<IMG SRC=\"../lib/create_im_mat.php?texte=Average générale&width=22\" WIDTH=\"22\" BORDER=\"0\" />";
	//$ligne1[$nb_col] = "<IMG SRC=\"../lib/create_im_mat.php?texte=".rawurlencode("Average générale")."&amp;width=22\" WIDTH=\"22\" BORDER=\"0\" alt=\"Average générale\" />";
	...
	if (($aff_rang) and ($referent!="une_periode")) {
		...
		*/
        ...
        //$ligne1[$nb_col] = "<IMG SRC=\"../lib/create_im_mat.php?texte=".rawurlencode("Position")."&amp;width=22\" WIDTH=\"22\" BORDER=\"0\" alt=\"Position\" />";
        ...
        $ligne1_csv[$nb_col] = "Position";

		// Remplissage de la ligne coefficients
        $col[$nb_col][0] = "-";

		// Remplissage des lignes Average générale, minimale et maximale
        $col[$nb_col][$nb_lignes_tableau+$ligne_supl] = "-";
        $col[$nb_col][$nb_lignes_tableau+1+$ligne_supl] = "-";
        $col[$nb_col][$nb_lignes_tableau+2+$ligne_supl] = "-";
	}
...
if($vtn_coloriser_resultats=='y') {
	...
	echo "<p class='bold' style='text-align:center;'>Caption of the colour application</p>\n";
	$legende_colorisation="<table class='boireaus' summary='Caption of the colour application'>\n";
	$legende_colorisation.="<thead>\n";
		$legende_colorisation.="<tr>\n";
		$legende_colorisation.="<th>Limit<br />higher</th>\n";
		$legende_colorisation.="<th>Text color </th>\n";
		$legende_colorisation.="<th>Cell color cell</th>\n";
		$legende_colorisation.="</tr>\n";
	$legende_colorisation.="</thead>\n";
...
if ($referent == "une_periode") {
	...
	echo "<p class='bold'>Class : $classe - Averages of the report card of $nom_periode[$num_periode] - (<i>".$la_date."</i>) - ".getSettingValue("gepiYear")."</p>\n";
}
else {
	//echo "<p class=bold>Classe : $classe - Résultats : Moyennes annuelles - Année scolaire : ".getSettingValue("gepiYear")."</p>";
	echo "<p class='bold'>Class : $classe - Annual averages of the report card - (<i>".$la_date."</i>) - ".getSettingValue("gepiYear")."</p>\n";
}
...
function affiche_tableau2($nombre_lignes, $nb_col, $ligne1, $col, $larg_tab, $bord, $col1_centre, $col_centre, $couleur_alterne) {
	...
	echo "<table summary=\"Averages of the report cards\" class='boireaus' border=\"$bord\" cellspacing=\"0\" width=\"$larg_tab\" cellpadding=\"1\">\n";
	echo "<tr>\n";
	$j = 1;
	while($j < $nb_col+1) {
...
if($vtn_coloriser_resultats=='y') {
	echo "<p class='bold'>Caption of the colour application&nbsp;:</p>\n";
	echo $legende_colorisation;
}

echo "<div class='noprint'>\n";
echo "<p><b>Caution:</b> The averages visualized here are photographs at one moment t of what was typed by the professors.<br />\n";
echo "That does not necessarily  correspond to what will appear on the bulletin after the typing of other results and possible adjustments of the coefficients.</p>\n";
echo "</div>\n";

//=======================================================
// MODIF: boireaus 20080424
// Pour permettre de trier autrement...
echo "\n<!-- Display form for sorting on the clicked column -->\n";
