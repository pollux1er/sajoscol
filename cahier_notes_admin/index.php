...
$accessibilite="y";
$titre_page = "Management of the report cards";
$niveau_arbo = 1;
$gepiPathJava="./..";
...
if(isset($_POST['is_posted'])) {
	...
	if (isset($_POST['activer'])) {
		if (!saveSetting("active_carnets_notes", $_POST['activer'])) $msg = "Error during the recording of the activation/desactivation parameter  !";
	}

	if (isset($_POST['export_cn_ods'])) {
		//if (!saveSetting("export_cn_ods", $_POST['export_cn_ods'])) {
		if (!saveSetting("export_cn_ods", 'y')) {
			$msg .= "Error during the recording of the authorization of export to ODS format !";
		}
	}
	else{
		if (!saveSetting("export_cn_ods", 'n')) {
			$msg .= "Error during the recording of the prohibition of export to ODS format !";
		}
	}

	if (isset($_POST['referentiel_note'])) {
		if (!saveSetting("referentiel_note", $_POST['referentiel_note'])) {
			$msg .= "Error during the recording of the note reference !";
		}
	}
	...
	if (isset($_POST['note_autre_que_sur_referentiel'])) {
		if (!saveSetting("note_autre_que_sur_referentiel", $_POST['note_autre_que_sur_referentiel'])) {
			$msg .= "Error during the recording of note_autre_que_sur_referentiel !";
		}
	}

	if (isset($_POST['utiliser_sacoche'])) {
		if (!saveSetting("utiliser_sacoche", 'yes')) {
			$msg .= "Error during the recording of utiliser_sacoche !";
		}
	} else {
