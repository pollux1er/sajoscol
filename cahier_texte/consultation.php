<?php
/*
 * $Id: consultation.php 3680 2009-10-27 11:16:05Z crob $
 *
...
if (getSettingValue("active_cahiers_texte")!='y') {
    tentative_intrusion(1, "Attempt to access to the text book in consultation whereas the module is not activated.");
...
if ($_SESSION['statut'] == 'eleve') {
	...
	if ($selected_eleve) {
		if (strtolower($selected_eleve->login) != strtolower($_SESSION['login'])) {tentative_intrusion(2, "Attempt of a student to access the text book of another student.");}
	}
	...
	if (mysql_num_rows($get_eleves) == 1) {
		...
		if (!$ok) {
			...
			tentative_intrusion(2, "Attempt of access by a parent to the textbook of another student that his .");
			$selected_eleve = false;
		}
	}
	...
	// echo "<tr>\n";
		echo "<div class=\"ct_col_gauche\">\n";
			echo "<p class=\"menu_retour\">\n";
				echo "<a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/>\n";
					echo "Return to Home\n";
				echo "</a>\n";
			echo "</p>\n";
			echo "<p>Today date is :&nbsp;<br />\n";
...
//<p class='menu_retour'>".get_date_php()."</p>\n</noscript>";
		...
		echo "<div class=\"ct_col_centre\">\n";
			echo "<p>\n";
				echo "<span class='grand'>Log books";
...
if (($nb_test == 0) and ($id_classe != null OR $selected_eleve) and ($delai != 0)) {
    if ($delai == "") die("Error : Visualization time of  personal work is not definite . Contact the administrator of GEPI of your school.");
    $nb_dev = 0;
    for ($i = 0; $i <= $delai; $i++) {
	...
	if ($_SESSION['statut'] == "responsable") {
		echo "<p class='gepi_garde'>Choose a student and a course.</p>\n"; //correction Régis : h3 doit venir après h1 et h2
	} elseif ($_SESSION['statut'] == "eleve") {
		echo "<p class='gepi_garde'>Choose a course</p>\n";
	} else {
		echo "<p class='gepi_garde'>Choose a Class and a course.</p>\n";
	}
	//echo "</center>";
	require("../lib/footer.inc.php");
	die();
...
// ---------------------------- Lien vers see_all.php  ----
	   echo "<a href='see_all.php?id_classe=$id_classe&amp;login_eleve=$selected_eleve_login&amp;id_groupe=$id_groupe'>See the whole Log book</a>\n<br />\n";
	// Cela provoque une déconnexion de l'élève et le compte est rendu 'inactif'???
	// ?????????????????????????????????????????????????????????

// ---------------------------- Affichage des devoirs  ---

      if ($delai == "") die("Error : Nondefinite time of visualization of the exams. Contact the GEPI administrator of your school.");
...
// Correction Régis : ajout de class pour gérer la mise en page + <strong> à la place de <strong>
              echo "<p class=\"sem_du_au\"><strong>Week of ".strftime("%d %B", $debutsemaine)." au ".strftime("%d %B %Y", $finsemaine)."</strong></p>\n";
              echo "<h2 class='h2_label'><strong>Personal work of $delai following days</strong></h2>\n";
...
//                  echo "<div>\n";
            ...
            echo "<h3 class='titre_a_faire color_police_travaux'>Personal work for<br />".strftime("%a %d %b", $jour)."</h3>\n";
...
// Correction Régis : remplacement de <strong> par <strong> + ajout de class pour gérer la mise en page
	   echo "<h2 class='h2_label'><strong>General information</strong></h2>\n";
...
// ----------------------------  Titre (div div div) --
            echo "<div class='titre_notice'>\n";
              echo "<h2 class='h2_label'><strong>ten last meetings until ".strftime("%A %d %B %Y", $today)." :</strong></h2>\n";
            echo "</div>\n";
...
// ----------------------------  Jour précédent (div div div div) --
          echo "<div class='cdt_dates_precedent'>\n";
            echo "<a title=\"Go to the previous day\" href=\"consultation.php?year=$yy&amp;month=$ym&amp;day=$yd&amp;id_classe=$id_classe&amp;login_eleve=$selected_eleve_login&amp;id_groupe=$id_groupe\">\n";
              echo "<img src='".$gepiPath."/images/icons/back.png' alt='Previous day' />\n";
            echo "</a>\n";
          echo "</div>\n";
...
// ----------------------------  Jour suivant (div div div div) --
          echo "<div class=\"cdt_dates_suivant droite_texte\">\n";
            echo "<a title=\"Go to the next day\" href=\"consultation.php?year=$ty&amp;month=$tm&amp;day=$td&amp;id_classe=$id_classe&amp;login_eleve=$selected_eleve_login&amp;id_groupe=$id_groupe\">\n";
              echo "<img src='".$gepiPath."/images/icons/forward.png' alt='Next day' />\n";
            echo "</a>\n";
          echo "</div>\n";
          ...
          $date_ct_old = -1;
           while (true) {
            ...
            if ($notice && (!$devoir || $notice->date_ct >= $devoir->date_ct)) {
              ...
              $titre = "";
              if ($not_dev->type == "t") {
                $titre .= "<strong>To do for  the: </strong>\n";
              }
