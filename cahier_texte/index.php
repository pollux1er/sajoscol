...
if (getSettingValue("GepiCahierTexteVersion") == '2') {
    ...
    if (getPref($_SESSION['login'],'cdt_version',"non renseigne") != "1" ) {
		if (strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 6' ) !== FALSE) {
			...
			$message_avertissement_navigateur = "Your navigator is not compatible with the tect book 2, but you can use the version 1.";
		} else {
...
$message_suppression = "Confirmation of suppression";
...
if ((isset($_POST['action'])) and ($_POST['action'] == 'sup_serie') and $valide_form=='yes') {
   ...
   if (($appel_ct) and (sql_count($appel_ct)!=0)) {
     ...
     if ($error == 'no') {
        $msg = "Successful suppression";
     } else {
        $msg = "There was a problem during the suppression.";
     }
   } else {
...
if ((isset($_GET['action'])) and ($_GET['action'] == 'sup_devoirs') and $valide_form=='yes') {
   check_token();
    ...
    $res = sql_query($sql);
    if (($res) and (sql_count($res)!=0)) {
        $msg = "Impossible to remove this notice : You must initially remove the joined documents";
    } else {
	//modif Eric interdire la suppression de notice visée
    $res = mysql_query("delete from ct_devoirs_entry where (id_ct = '".$_GET['id_ct_del']."' and vise != 'y')");
	updateOnline("delete from ct_devoirs_entry where (id_ct = '".$_GET['id_ct_del']."' and vise != 'y')");
        if ($res) $msg = "Successful suppression";
    }
	...
	if (isset($edit_devoir)) {
		...
		if((isset($jour_visibilite))&&(isset($heure_visibilite))) {
			...
			if(!preg_match("/^[0-9]{1,2}:[0-9]{1,2}$/",$heure_visibilite)) {
				...
				//die();
				if (isset($id_ct))  {
					$msg.="Visibility hour badly formatted  : $heure_visibilite.<br />The hour of visibility will not be modified.<br />";
				}
				else {
					$msg.="Visibility hour badly formatted : $heure_visibilite.<br />The current hour will be used : $heure_courante<br />";
				}
			...
			if(!preg_match( '`^\d{1,2}/\d{1,2}/\d{4}$`', $jour_visibilite)) {
				...
				//die();
				if (isset($id_ct))  {
					$msg.="Visibility day is badly formatted : $jour_visibilite.<br />The day of visibility will not be modified.<br />";
				}
				else {
					$msg.="Visibility day is badly formatted : $jour_visibilite.<br />The running day  will be used : $jour_courant<br />";
				}
        ...
        if (preg_match("#([0-9]{2})/([0-9]{2})/([0-9]{4})#", $_POST['display_date'])) {
            ...
            $msg_error_date = "The date chosen for the work is not in conformity";
		}
        ...
        if (!isset($msg_error_date)) {
          ...
          if ($req) {$msg.= "Successful recording.";} else {$msg .= "Problem during the recording !";}
        } else {
          $msg = $msg_error_date;
		}
    } else {
      ...
      updateOnline("UPDATE ct_entry SET contenu = '$contenu_cor', id_login='".$_SESSION['login']."' WHERE id_ct='$id_ct' AND id_groupe='".$current_group["id"]."'");
	  } else {
      ...
      updateOnline("INSERT INTO ct_entry SET id_ct='0', contenu = '$contenu_cor', heure_entry='$heure_entry', id_login='".$_SESSION['login']."', id_groupe='".$id_groupe."', date_ct='$temp'");
		  $id_ct = mysql_insert_id();
//            $today = $temp;
        }
        if ($req) $msg = "Successful recording."; else $msg = "Problem during the recording !";
...
if (($test_ct_vide == 0) and ($test_ct_devoirs_vide != 0)) {$req = mysql_query("INSERT INTO ct_entry SET id_ct='0', contenu = '', id_login='".$_SESSION['login']."', id_groupe='" . $current_group["id"]. "', date_ct=''");updateOnline("INSERT INTO ct_entry SET id_ct='0', contenu = '', id_login='".$_SESSION['login']."', id_groupe='" . $current_group["id"]. "', date_ct=''");}
...
echo "<table width=\"98%\" cellspacing=0 align=\"center\" summary=\"Heading table\">\n";
...
if (empty($groups)) {
    echo "<br /><br />\n";
    echo "<b>No text book is available.</b>";
    echo "<br /><br />\n";
}
	$a = 1;
...
echo "<p><span class='grand'>Log book</span><br />";
if (getSettingValue("GepiCahierTexteVersion") == '2') {
echo "<a href=\"../cahier_texte_2/index.php?cdt_version_pref=2\">\n";
echo "<img src='../images/icons/cdt1_2.png' alt='Utiliser la version 2 du cahier de textes' class='link' title='Use version 2 of text book'/> </a>";
...
if ($id_groupe != null) {
	...
	if(getSettingValue('cahier_texte_acces_public')!='no'){
	    echo "<a href='../public/index.php?id_groupe=" . $current_group["id"] ."' target='_blank'>Visualize the text book in public access</a>\n";
	} else {
		echo "<a href='./see_all.php'>Visualize the text book (restricted access)</a>\n";
	}

    if ((getSettingValue("cahiers_texte_login_pub") != '') and (getSettingValue("cahiers_texte_passwd_pub") != '')) {
       echo "<br />(Identifiant : ".getSettingValue("cahiers_texte_login_pub")." - Password : ".getSettingValue("cahiers_texte_passwd_pub").")\n";
    }
	...
	if ($delai > 0) {
		$cr_cours = "<p style=\"border: 1px solid grey; background-color: ".$color_fond_notices["c"]."; font-weight: bold;\">
			<a href=\"index.php?year=$year&amp;month=$month&amp;day=$day&amp;id_groupe=" . $current_group["id"] ."\" title=\"Create/modify reports of course\">
		...
		$travaux_perso = "<p style=\"border: 1px solid grey; background-color: ".$color_fond_notices["t"]."; font-weight: bold;\">
			<a href=\"index.php?edit_devoir=yes&amp;year=$year&amp;month=$month&amp;day=$day&amp;id_groupe=". $current_group["id"] ."\" title=\"Create/modify notifications of personal work to do\">
			Personal work do</a></p>\n";
	...
	if ($nb_total_notices > 1) {
		$legend = "Currently : ".$nb_total_notices." notices.<br />\n";
	}
	elseif ($nb_total_notices == 1) {
		$legend = "Currently : 1 notice.<br />\n";
	}
	else {
		$legend = "";
	}
	if ($nb_total_notices > 15) {
		...
		if ($_SESSION['type_display_notices'] == "all")  {
			echo "<b>>>&nbsp;&nbsp;Display&nbsp;all&nbsp;&nbsp;notices<<</b><br />\n";
			echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"index.php?year=$year&amp;month=$month&amp;day=$day&amp;id_groupe=".$current_group["id"]."&amp;type_display_notices=15\">Display&nbsp;15&nbsp;notices&nbsp;max.</a>\n";
		} else {
			echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"index.php?year=$year&amp;month=$month&amp;day=$day&amp;id_groupe=".$current_group["id"]."&amp;type_display_notices=all\">Display&nbsp;all&nbsp;notices</a><br />\n";
			echo "<b>>>&nbsp;Display&nbsp;15&nbsp;notices&nbsp;max.<<</b>\n";
		}
		echo "</fieldset>\n";
	} else {
    ...
    echo "<span class='grand'> - Select a group.</span>\n";
}
...
if (($id_groupe == null)) {
    if ((getSettingValue("cahiers_texte_login_pub") != '') and (getSettingValue("cahiers_texte_passwd_pub") != '')) {
       echo " <b>WARNING</b> : Because of the personal character of the contents, the access to <a href=\"../public\">site of public consultation of the text book</a> is restricted.
       To access the text book, the visitor (student, parent, ...) must be in possession of a name of user and a valid password.\n";
    } elseif(getSettingValue('cahier_texte_acces_public') == 'no'){
		echo '<p style="font-weight: bold;">The access to the text book is protected.</p>';
	} else {
       echo " <b><span style='font-weight:bold;'>WARNING</span> : the access to the public interface of consultation of the tect books is entirely free and is no subjected to  restriction.</b>\n";
    }
    echo "<br /><br />By using the electronic text book of GEPI :
    <ul>\n";

	if(getSettingValue('cahier_texte_acces_public')!='no'){
		echo "<li>you accept that your name, initial of first name, classes and courses teached appear on <a href=\"../public\">the public site of consultation of the text books</a>,</li>\n";
	}
	else {
		echo "<li>the access to the text book is limited to the users having an account (<i>it can be the students, parents of student if accounts were created for them, but in this case, the students have access only to the text book of the lesson which they follow and the parents have access only to the text book of their children</i>),</li>\n";
	}

    echo "<li>you accept that all information that you provide in this module is disseminated on this same site.</li>
    <li>you commit yourselves complying with the rules fixed concerning the text books (Circular of May 3, 1961 addressed to the vice-chancellors - RLR, 550-1 b)</li>
    <li>you commit yourselves not making appear personal information concerning the student</li>
    </ul>\n";
    echo "<b>RECALL</b> : the homework diary constitutes a tool of communication for the student, disciplinary teams
    and multi-disciplinary , administration, the head of shool, inspection and families.
    It reports the work completed in class :
    <ul>
    <li>project of the teaching staff,</li>
    <li>teaching contents of each meeting, chronology, aim, work to be made ...</li>
...
echo "<table width=\"100%\" border = 0 align=\"center\" cellpadding=\"10\" summary=\"Display board of notices\">\n";
echo "<tr>\n";
...
foreach ($current_group["classes"]["list"] as $_id_classe) {
    if ($total[$_id_classe] > 0) {
        echo "<p>The class " . $current_group["classes"]["classes"][$_id_classe]["classe"] . " has  <a href=\"javascript:centrerpopup('liste_tous_devoirs.php?classe=$_id_classe&amp;debut=$aujourdhui',260,320,'scrollbars=yes,statusbar=no,resizable=yes');\"><strong>" . $total[$_id_classe] . "</strong> ";
        echo (($total[$_id_classe] == 1) ? "personal work" : "personals works");
        echo "</a> until <strong>" . strftime("%a %d %b %y", $date[$_id_classe]) . "</strong>.</p>\n";
    }
}
...
while (true) {
    ...
    if ($not_dev->type == "c") {
        ...
        $html_balise .=(" ");
        $html_balise .=(
            "<a href=\"index.php?id_ct_del=$not_dev->id_ct&amp;edit_devoir=$edit_devoir&amp;action=sup_entry&amp;uid_post=$uid&amp;id_groupe=".$current_group["id"].add_token_in_url()."\" onclick=\"return confirmlink(this,'removal of the notice of " . strftime("%a %d %b %y", $not_dev->date_ct) . " ?','" . $message_suppression . "')\"><img style=\"border: 0px;\" src=\"../images/delete16.png\" alt=\"Delete\" title=\"Delete\" /></a>\n"
        );
			...
			if ($not_dev->vise == 'y') {
			   $html_balise .= "<i><span  class=\"red\">Signed notice</span></i>";
			}
			}
		} else {
		     // cas d'un visa, on n'affiche rien
	    ...
	    if (($not_dev->vise != 'y') or ($visa_cdt_inter_modif_notices_visees == 'no')) {
        $html_balise .=("<a href=\"index.php?id_ct=$not_dev->id_ct&amp;id_groupe=" . $current_group["id"] . "&amp;edit_devoir=yes\"><img style=\"border: 0px;\" src=\"../images/edit16.png\" alt=\"modify\" title=\"modify\" /></a>\n");
        $html_balise .=(" ");
        $html_balise .=(
            "<a href=\"index.php?id_ct_del=$not_dev->id_ct&amp;edit_devoir=$edit_devoir&amp;action=sup_devoirs&amp;uid_post=$uid&amp;id_groupe=".$current_group["id"].add_token_in_url()."\" onclick=\"return confirmlink(this,'suppression of the exam of " . strftime("%a %d %b %y", $not_dev->date_ct) . " ?','" . $message_suppression . "')\"><img style=\"border: 0px;\" src=\"../images/delete16.png\" alt=\"delete\" title=\"delete\" /></a>\n"
			 );
			if ($not_dev->vise == 'y') {
			   $html_balise .= "<i><span  class=\"red\">Signed notice</span></i>";
			}
    ...
    echo("<table style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice.";\" width=\"100%\" cellpadding=\"1\" bgcolor=\"".$color_fond_notices[$not_dev->type]."\" summary=\"Table of...\">\n<tr>\n<td>\n$html_balise$html</td>\n</tr>\n</table>\n<br/>\n");
...
include "../lib/transform.php";
  ...
  $res = sql_query($sql);
  if (($res) and (sql_count($res)!=0)) {
     $html .= "<small style=\"font-weight: bold;\">Joined Document:</small>\n";
...
$html_balise = "<div style=\"margin: 0px; float: left;\"><a href='index.php?info=yes&amp;id_groupe=" . $current_group["id"] . "'><img style=\"border: 0px;\" src=\"../images/edit16.png\" alt=\"modify\" title=\"modify\" /></a> <a href='index.php?info=yes&amp;id_ct_del=$id_ctexte&amp;action=sup_entry&amp;uid_post=$uid&amp;id_groupe=".$current_group["id"].add_token_in_url()."' onclick=\"return confirmlink(this,'removal of the notice general Information ?','".$message_suppression."')\"><img style=\"border: 0px;\" src=\"../images/delete16.png\" alt=\"delete\" title=\"delete\" /></a>";
...
echo "<table style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."; background-color: ".$color_fond_notices["i"] ."; padding: 2px; margin: 2px;\" width=\"100%\" cellpadding=\"2\" summary=\"Table of...\">\n<tr style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."; background-color: ".$couleur_cellule["i"]."; padding: 0px; margin: 0px;\">\n<td>\n".$html_balise.$html."</td>\n</tr>\n</table>\n<br />";

echo "Export in <a href='../cahier_texte_2/exportcsv.php?id_groupe=".$current_group["id"]."'>csv format </a> / <a href='../cahier_texte_2/export_cdt.php?id_groupe=".$current_group["id"]."'> HTML format</a><br/>";
...
echo "<td valign=\"top\">\n";
$test_ct_vide = sql_count(sql_query("SELECT id_ct FROM ct_entry WHERE (id_groupe='" . $current_group["id"] . "')"));
if ($test_ct_vide == 0) {echo "<b><font color='red'>Currently this text book is empty. It is thus not visible in public space.</font></b>\n";}
...
if ($reduce == "off") {
    ...
    echo "<a href=\"./index.php?reduce=on\">Hide the typing window </a>";
}
else {
    echo "<div style=\"position:absolute;top:350px;left:30%;border:2px solid black;background-color:white;width:610px;height:20px;text-align:center;\">\n";
    echo "<a href=\"./index.php?reduce=off\">Show the typing window </a>";
    echo "</div>\n";
...
if (isset($edit_devoir)) {
    echo "<legend style=\"border: 1px solid grey; background: ".$color_fond_notices[$type_couleur]."; font-variant: small-caps;\"> Personal work";
    ...
    if (isset($id_ct)) {
		...
		echo " - <a href=\"index.php?edit_devoir=yes&amp;year=".$annee_lendemain."&amp;month=".$mois_lendemain."&amp;day=".$jour_lendemain."&amp;id_groupe=". $current_group["id"] ."\" title=\"Type a new personal work to make\">New work</a>";
	} else {
    ...
    if (isset($info))
        echo "<legend style=\"border: 1px solid grey; background: ".$color_fond_notices[$type_couleur]."; font-variant: small-caps;\"> General information ";
    else
        echo "<legend style=\"border: 1px solid grey; background: ".$color_fond_notices[$type_couleur]."; font-variant: small-caps;\"> Report ";
	if (isset($num_notice)) echo " <b>N° ".$num_notice."</b> ";
//    echo "de la séance du " . strftime("%A %d %B %Y", $today);
    if (isset($id_ct)) {
        echo " - <b><font color=\"red\">Modification of the notice</font></b>";
        if (!isset($info))
        echo " - <a href=\"index.php?year=".$year."&amp;month=".$month."&amp;day=".$day."&amp;id_groupe=".$current_group["id"]."&amp;ajout=oui\" title=\"Click to add a report for this day\">Add one notice</a>\n";
    } else
        echo " - <b><font color=\"red\">New notice</font></b>\n";
    echo "</legend>\n";
}
...
<table border="0" width="100%" summary="Table of typing of notice">
<tr>
<td style="width:100%"><b><?php echo $temp; ?></b>&nbsp;
<input type="submit" value="Save the notice" style="font-variant: small-caps;" />
<?php
...
if (isset($edit_devoir)) {
	...
	echo "<br />\n";
	echo "<span title='You can modify the dates and hour of visibility with the arrows UP/DOWN, PageUp/PageDown of the keyboard.' style='font-weight: bold;'>Date of visibility</span>&nbsp;:\n";
	...
	onblur=\"date_v=document.getElementById('jour_visibilite').value;
		...
		if(!checkdate(mois_v, jour_v, annee_v)) {
			alert('The date of visibility typed is not valid.');
		}
	\" />\n";
// onblur='verif_date_visibilite()' />\n";
	...
	onblur=\"instant_v=document.getElementById('heure_visibilite').value;
		...
		if(erreur=='y') {
			alert('The hour of visibility typed is not valid.');
		}
	\" />\n";
}
...
echo "<td>\n";
if (isset($edit_devoir)) {
	echo "<a title=\"Go to the previous day\" href=\"index.php?edit_devoir=yes&amp;year=$yy&amp;month=$ym&amp;day=$yd&amp;id_groupe=" . $current_group["id"] . "\">&lt;&lt;</a></td><td align=center><a href=\"index.php?edit_devoir=yes&amp;id_groupe=" . $current_group["id"] ."&amp;id_matiere=$id_matiere\">Today</a></td><td align=right><a title=\"Go to the next day\" href=\"index.php?edit_devoir=yes&amp;year=$ty&amp;month=$tm&amp;day=$td&amp;id_groupe=" . $current_group["id"]."&amp;id_matiere=$id_matiere\">&gt;&gt;</a>\n";
} else {
	echo "<a title=\"Go to the previous day\" href=\"index.php?year=$yy&amp;month=$ym&amp;day=$yd&amp;id_groupe=" . $current_group["id"] . "\">&lt;&lt;</a></td><td align=center><a href=\"index.php?id_groupe=" . $current_group["id"] . "\">Today</a></td><td align=right><a title=\"Go to the next day\" href=\"index.php?year=$ty&amp;month=$tm&amp;day=$td&amp;id_groupe=" . $current_group["id"]."&amp;id_matiere=$id_matiere\">&gt;&gt;</a>\n";
}
echo "</td>\n";
echo "</tr>\n";
echo "\n";
...
if (isset($id_ct)) {
    ...
    if (($res) and (sql_count($res)!=0)) {
        ...
        echo "<table style=\"border-style:solid; border-width:0px; border-color: ".$couleur_bord_tableau_notice."; background-color: #000000; width: 100%\" cellspacing=\"1\" summary=\"Table of joined documents\">\n";
        echo "<tr style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."; background-color: $couleur_entete_fond[$type_couleur];\"><td style=\"text-align: center;\"><b>Title</b></td><td style=\"text-align: center; width: 100px\"><b>Size in Ko</b></td><td style=\"text-align: center; width: 100px\"></td></tr>\n";
        $nb_doc = 0;
        $id_document = array();
        $ic='1';
        for ($i=0; ($row = sql_row($res,$i)); $i++) {
...
<td style="font-weight: bold; text-align: center; width: 20%">Title (optional)</td>
<td style="font-weight: bold; text-align: center; width: 60%">Position</td>
</tr>
<?php
$nb_doc_choisi='3';
...
if ($last_date != "-1") {
    ...
    <input type="hidden" name="uid_post" value="<?php echo $uid; ?>" />
    <?php
    echo "<input type='submit' value='Validate' onclick=\"return confirmlink(this,'Are you sure that you to want to remove the notices and documents joined until the selected date ?','Confirmation of suppression')\" />\n";
    echo "</form>\n";
    echo "</td>\n</tr>\n</table>\n</fieldset>\n";
    echo "</div>\n";
    echo "</div>\n";
