...
function ajout_doc($doc_file,$id_ct,$doc_name,$cpt_doc) {
    ...
    if ($taille > $max_size) {
        return "Impossible Download: maximum authorized  size : ".$max_size_ko." Ko";
        die();
    }
    if ($taille == 0) {
        return "The selected file seems empty : impossible transfer.";
        die();
    }
    ...
    if (($total+$taille) > $total_max_size) {
        return "Impossible Download : disk space available (".(($total_max_size - $total)/1024)." Ko) insufficient.";
        die();
    }
    ...
    if (creer_repertoire($dest, $dossier)) {
      ...
      return "Problem of writing on the repertory. Please signal this problem to the administrator of the site";
      die();
    }
    ...
    if (strstr($nom_sans_ext, "..")) {
        return "Problem of transfer : the file could not be transferred on the repertory. Please signal this problem to the administrator of the site ";
        die();
    }
...
if ((isset($_GET["action"])) and ($_GET["action"] == 'del')) {
    ...
    if ($empl == -1) {
        ...
        $del = @unlink($empl);
        if (file_exists($empl)) {
            $msg = "Problem : the document have not be removed. Contact the administrator of the site.";
        } else {
	    if (isset($edit_devoir))
		...
		$sql_query = "delete from ct_documents where id = '$id_document'";
            if (sql_query($sql_query)) {
                $msg = "Successful Supression";
            } else {
                $msg = "A problem occurred during the removal of the document. Contact the administrator of the site.";
            }
        }
    }
}
...
if (!empty($doc_name_modif) and !empty($id_document)) {
	...
	if (isset($edit_devoir))
        ...
        if (sql_query($query)) {
            $msg = "Successful renaming";
        } else {
            $msg = "A problem occurred during the renaming of the document. Contact the administrator of the site.";
        }

    } else {
        $msg = "Please choose a valid name.";
    }
}
