...
if ($current_group == null) {
	echo("group not specified or not found.");
	die();
}
...
echo "</select>&nbsp;";
echo "<div id=\"div_chaine_liste_notices\" style=\"display:inline;\"><img id=\"chaine_liste_notice\" onLoad=\"updateChaineIcones()\" HEIGHT=\"16\" WIDTH=\"16\" style=\"border: 0px; vertical-align : middle\" src=\"../images/blank.gif\"  alt=\"Lier\" title=\"Bind the list with the windows edition of notices\" /></div>";
//fin affichage des groupes

echo "<p style='font-size:9pt'>";
if(getSettingValue('cahier_texte_acces_public')!='no'){
	echo "<a href='../public/see_all.php?id_groupe=" . $current_group->getId() ."' target='_blank'>Visualize the public access</a>\n<br>";
} else {
...
echo "Export au <a href='./exportcsv.php?id_groupe=".$current_group->getId()."'>csv format</a> / <a href='./export_cdt.php?id_groupe=".$current_group->getId()."'>HTML format</a><br/>";
...
echo "<br />(Identifier : ".getSettingValue("cahiers_texte_login_pub")." - Mot de passe : ".getSettingValue("cahiers_texte_passwd_pub").")\n";
...
foreach ($current_group->getClasses() as $classe) {
	if ($total[$classe->getId()] > 0) {
		...
		echo (($total[$classe->getId()] == 1) ? " personal work" : "personal works");
		echo "</a> until " . strftime("%A %d %B %Y", $date[$classe->getId()]) . ".\n<br style='font-size:2px;'/>";
	}
}
...
$legend = "Currently : ".$compteur_notices_affiches." notices displayed on a total of ".$compteur_nb_total_notices."<br />";
...
echo "<b>Generals informations</b><br>\n";
$i = 1;
foreach ($ctCompteRenduInfoGenerales as $ctCompteRenduInfoGenerale) {
	...
	echo "<table style=\"border-style:solid; border-width:0px; background-color: ".$color_fond_notices["i"] ."; padding: 2px; margin: 0px;\" width=\"100%\" cellpadding=\"2\" summary=\"Table of general information...\">";
...
echo "<table border='0' width='100%' summary=\"Table of...\">\n";
echo "<tr>\n<td>\n";
echo "<ul style=\"list-style-type:disk\">\n";
echo "<p>\n";
echo "<a href='./exportcsv.php?id_groupe=".$current_group->getId()."'>Export in CSV format</a>";
echo "</p>\n";
echo "<p style='margin-left:1em'>\n";
echo "<em>Note&nbsp;:</em> to open this CSV file with OpenOffice,keep the default settings at the time of the opening of the file.";
echo "</p>\n";
echo "<p>\n";
echo "<a href='./export_cdt.php?id_groupe=".$current_group->getId()."' target=\"_blank\">Export in HTML format </a>.\n";
echo "</p>\n";
echo "</td>\n</tr>\n</table>\n</fieldset>\n";
// fin export
