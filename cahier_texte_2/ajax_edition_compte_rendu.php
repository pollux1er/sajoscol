...
if ($ctCompteRendu != null) {
	...
	if ($groupe == null) {
		echo("Recording error  am: No the group associated with thereport");
		die;
	}
	...
	if ($groupe == null) {
		echo("Edition error of report: no specified group");
		die;
	}
...
(getSettingValue("cdt_autoriser_modif_multiprof")!="yes")) {
	echo("Edition error of report : you do not have the right to modify this notice because it belongs to another professor.");
	die();
}
if ($ctCompteRendu->getVise() == 'y') {
	// interdire la modification d'un visa par le prof si c'est un visa
	echo("Edition error of report : Notices signed, impossible edition");
	die();
}
		...
		if(mysql_num_rows($res)>0) {
			...
			if($res) {
				if($visible_eleve_parent=='1') {
					...
					echo "<img src='../images/icons/invisible.png' width='19' height='16' alt='Document invisible for the students and responsibles' title='Document invisible for the students and responsibles' />";
				}
			}
		}
	}
...
echo "<div id=\"div_chaine_edition_notice\" style=\"display:inline;\"><img id=\"chaine_edition_notice\" onLoad=\"updateChaineIcones()\" HEIGHT=\"16\" WIDTH=\"16\" style=\"border: 0px; vertical-align : middle\" src=\"../images/blank.gif\"  alt=\"Lier\" title=\"Bind the list with the list of the notices\" /></div>&nbsp;\n";
...
echo "<button style='background-color:".$color_fond_notices['p']."' onclick=\"javascript:
					...
					\">Edit the private notices</button>\n";
...
<table border="0" width="100%" summary="Tableau de saisie de notice">
	<tr>
		...
		<button type="submit" style='font-variant: small-caps;'
			onClick="javascript:$('passer_a').value = 'passer_devoir';">Save and pass to the exams of the next day</button>
		<?php } ?>
...
//		$oFCKeditor->Create() ;
		...
		"<tr style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."; background-color: $couleur_entete_fond[$type_couleur];\"><td style=\"text-align: center;\"><b>Title</b></td><td style=\"text-align: center; width: 100px\"><b>Size in KB</b></td><td style=\"text-align: center; width: 100px\"></td></tr>\n";
		if (!empty($documents)) {
			...
			foreach ($documents as $document) {
				...
				echo "<td style=\"text-align: center;\"><a href='#' onclick=\"javascript:suppressionDocument('Suppression of the joined document ".$document->getTitre()." ?', '".$document->getId()."', '".$ctCompteRendu->getIdCt()."','".add_token_in_js_func()."')\">Delete</a></td></tr>\n";
				$nb_documents_joints++;
			}
			echo "</table>\n";
		...
		<table style="border-style:solid; border-width:0px; border-color: <?php echo $couleur_bord_tableau_notice;?> ; background-color: #000000; width: 100%" cellspacing="1" summary="Table of...">
			...
			<tr style="border-style:solid; border-width:1px; border-color: <?php echo $couleur_bord_tableau_notice; ?>; background-color: <?php echo $couleur_entete_fond[$type_couleur]; ?>;">
				<td colspan="<?php echo $nb_col_tableau_pj;?>" style="text-align: center;"><?php  echo "All documents are not accepted, see <a href='javascript:centrerpopup(\"limites_telechargement.php?id_groupe=" . $groupe->getId() . "\",600,480,\"scrollbars=yes,statusbar=no,resizable=yes\")'>limits and restrictions</a>\n"; ?>
				</td>
			</tr>
		</table>
		</td>
