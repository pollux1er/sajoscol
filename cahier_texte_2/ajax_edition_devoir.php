...
if ($ctTravailAFaire != null) {
	...
	if ($groupe == null) {
		echo("Edition error of exam: No group associated to the exam ");
		die;
	}

	// Vérification : est-ce que l'utilisateur a le droit de travailler sur ce groupe ?
	if (!$groupe->belongsTo($utilisateur)) {
		echo "Edition error of exam: the group does not belong to the professor";
		die();
	}
	...
	if ($groupe == null) {
		echo("Edition error of exam: no group specified ");
		die;
	}
...
(getSettingValue("cdt_autoriser_modif_multiprof")!="yes")) {
	echo("Edition error of exam : you do not have the right to modify this notice because it belongs to another professor.");
	die();
}
...
if ($ctTravailAFaire->getVise() == 'y') {
	...
	echo("Edition error of exam: Signed notice, impossible edition ");
	die();
}
		...
		if(mysql_num_rows($res)>0) {
			...
			if($res) {
				if($visible_eleve_parent=='1') {
					echo "<img src='../images/icons/visible.png' width='19' height='16' alt='Document visible  for the students and responsibles' title='Document visible  for the students and responsibles' />";
				}
				else {
					echo "<img src='../images/icons/invisible.png' width='19' height='16' alt='Document not visible  for the students and responsibles' title='Document not visible  for the students and responsibles' />";
				}
			}
		}
	}
...
echo "<button style='background-color:".$color_fond_notices['p']."' onclick=\"javascript:
					...
					\">Edit the privates notices</button>\n";
/*
echo " <button style='background-color:".$color_fond_notices['p']."' onclick=\"javascript:
...
<table border="0" width="99%" summary="Tableau de saisie de notice">
	<tr>
		<td style="width: 80%"><b>For the <?php echo strftime("%A %d %B %Y", $ctTravailAFaire->getDateCt()); ?></b>&nbsp;
		...
		<button type="submit" style='font-variant: small-caps;'
			onClick="javascript:$('passer_a').value = 'passer_compte_rendu';">Save and
		pass to the reports</button>
...
*/
			echo "<br />\n";
			echo "<span title='You can modify the dates and hour of visibility with the arrows Up/Down, PageUp/PageDown du clavier.'>Visibility</span>&nbsp;:\n";
	...
	<tr>
		...
		echo "<table style=\"border-style:solid; border-width:0px; border-color: ".$couleur_bord_tableau_notice."; background-color: #000000; width: 100%\" cellspacing=\"1\" summary=\"Table of the joined documents\">\n";
		"<tr style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."; background-color: $couleur_entete_fond[$type_couleur];\"><td style=\"text-align: center;\"><b>Titre</b></td><td style=\"text-align: center; width: 100px\"><b>Size in Ko</b></td><td style=\"text-align: center; width: 100px\"></td></tr>\n";
		if (!empty($documents)) {
			...
			foreach ($documents as $document) {
				...
				echo "<tr style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."; background-color: #FFFFFF;\"><td>
						...
						echo "<td style=\"text-align: center;\"><a href='#' onclick=\"javascript:suppressionDevoirDocument('suppression du document joint ".$document->getTitre()." ?', '".$document->getId()."', '".$ctTravailAFaire->getIdCt()."', '".$ctTravailAFaire->getIdGroupe()."','".add_token_in_js_func()."')\">Delete</a></td></tr>\n";
				$nb_documents_joints++;
			}
			echo "</table>\n";
			...
			if($nb_documents_joints>0) {
				echo "New name <input type=\"text\" name=\"doc_name_modif\" size=\"25\" /> for\n";
		...
		?>
		<table style="border-style:solid; border-width:0px; border-color: <?php echo $couleur_bord_tableau_notice;?> ; background-color: #000000; width: 100%" cellspacing="1" summary="Table of...">
			<tr style="border-style:solid; border-width:1px; border-color: <?php echo $couleur_bord_tableau_notice; ?>; background-color: <?php echo $couleur_entete_fond[$type_couleur]; ?>;">
				<td style="font-weight: bold; text-align: center; width: 20%">Title
				(optional)</td>
				<td style="font-weight: bold; text-align: center; width: 60%">Position</td>
				<?php
					if(getSettingValue('cdt_possibilite_masquer_pj')=='y') {
						$nb_col_tableau_pj=3;
						echo "<td style='font-weight: bold; text-align: center;'>Hide</td>\n";
					}
					else {
						$nb_col_tableau_pj=2;
					}
			...
			<tr style="border-style:solid; border-width:1px; border-color: <?php echo $couleur_bord_tableau_notice;?>; background-color: <?php echo $couleur_cellule[$type_couleur]; ?>;">
				...
				<button type="submit" id="bouton_enregistrer_2" name="Enregistrer"
					style='font-variant: small-caps;'><?php echo($label_enregistrer); ?></button>
				<button type="submit" style='font-variant: small-caps;'
					onClick="javascript:$('passer_a').value = 'passer_compte_rendu';">Save and pass to the reports</button>
				</td>
			</tr>
			<tr style="border-style:solid; border-width:1px; border-color: <?php echo $couleur_bord_tableau_notice; ?>; background-color: <?php echo $couleur_entete_fond[$type_couleur]; ?>;">
				<td colspan="<?php echo $nb_col_tableau_pj;?>" style="text-align: center;"><?php  echo "All the documents are not accepted, see <a href='javascript:centrerpopup(\"limites_telechargement.php?id_groupe=" . $groupe->getId() . "\",600,480,\"scrollbars=yes,statusbar=no,resizable=yes\")'>limits and restrictions</a>\n"; ?>
				</td>
			</tr>
		</table>
		</td>
