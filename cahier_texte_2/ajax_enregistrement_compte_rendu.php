<?php
/*
 * $Id: ajax_enregistrement_compte_rendu.php 8435 2011-10-06 11:05:39Z crob $
 *
...
if ($uid_post==$uid_prime) {
	if(getSettingValue('cdt2_desactiver_copie_svg')!='y') {
		...
		$sql="INSERT INTO ct_private_entry SET date_ct='$date_ct', heure_entry='".strftime("%H:%M:%S")."', id_login='".$_SESSION['login']."', id_groupe='$id_groupe', contenu='<b>BACKUP COPY</b><br />$contenu_cor';";
		$insert=mysql_query($sql);updateOnline($sql);

		echo("Recording error of report : form already posted previously.\nA backup copy was created in private notice.");
	}
	else {
		echo("Recording error of report : form already posted previously.");
	}
...
if ($id_ct != null) {
	...
	if ($ctCompteRendu == null) {
		echo "Recording error of report: Report not found ";
		die();
	}
	...
	if ($groupe == null) {
		echo("Recording error of report: no the group or bad specified group");
		die;
	}
...
if ($ctCompteRendu->getVise() == 'y') {
	echo("Recording error of report: Signed notice, impossible edition/");
	die();
}
...
if (!empty($doc_name_modif) && (trim($doc_name_modif)) != '' && !empty($id_document)) {
	...
	if (empty($documents)) {
		echo "Recording error of report: document not found.";
		die();
	}
