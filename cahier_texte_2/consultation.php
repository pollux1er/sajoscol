<?php
/*
 * $Id: consultation.php 8736 2011-12-23 16:21:33Z crob $
 *
...
if (getSettingValue("GepiCahierTexteVersion") != '2') {
  tentative_intrusion(1, "Attempt to access to the text book v2 whereas it is not open.");
...
if ($_SESSION['statut'] == 'eleve') {
	...
	if ($selected_eleve) {
		if (strtolower($selected_eleve->login) != strtolower($_SESSION['login'])) {tentative_intrusion(2, "Attempt of a student to access the text book of another studet.");}
	}
	...
	if (mysql_num_rows($get_eleves) == 1) {
		...
		if (!$ok) {
			...
			tentative_intrusion(2, "Attempt by a parent to access to the text book of another pupil than his.");
			$selected_eleve = false;
		}
	}
...
//<p class='menu_retour'>".get_date_php()."</p>\n</noscript>";
			...
			if ($selected_eleve_login != "") {
				...
				echo "<a href='see_all.php?&year=$year&amp;month=$month&amp;day=$day&amp;login_eleve=$selected_eleve_login&amp;id_groupe=Toutes_matieres'>See the whole text book</a>";
			}
		echo "</div>\n";
		//echo "<td align=\"right\">\n";
...
if (($nb_test == 0) and ($id_classe != null OR $selected_eleve) and ($delai != 0)) {
    ...
    if ($delai == "") die("Erreur : Time of visualization of personal work is not defined. Contact the administrator of GEPI of your school.");
    $nb_dev = 0;
    for ($i = 0; $i <= $delai; $i++) {
        ...
        if ($nb_devoirs_cahier_texte != 0) {
          ...
          if ($nb_dev == '1') {
            ...
            echo "<p class=\"centre_texte no_print\">Date selected : ".strftime("%A %d %B %Y", $today)."\n</p>\n";
            echo "<h2 class=\"centre_texte_pt_cap petit_h2\">Personal work of $delai days following the ".strftime("%d %B %Y", $today)."</h2>\n";
...
// ---------------------------- Lien vers see_all.php  ----
	   echo "<a href='see_all.php?id_classe=$id_classe&amp;login_eleve=$selected_eleve_login&amp;id_groupe=$id_groupe'>See the whole text book in $matiere_nom</a>\n<br />\n";
	// Cela provoque une déconnexion de l'élève et le compte est rendu 'inactif'???
	// ?????????????????????????????????????????????????????????

// ---------------------------- Affichage des devoirs  ---

      if ($delai == "") die("Error :the time of visualization of the exams is not defined. Contact the administrator of GEPI of your establishment.");
...
// Correction Régis : ajout de class pour gérer la mise en page + <strong> à la place de <strong>
              ...
              echo "<h2 class='h2_label'><strong>Personal work of $delai next days</strong></h2>\n";
...
//                  echo "<div>\n";
            ...
            echo "<h3 class='titre_a_faire color_police_travaux'>Personal work for the <br />".strftime("%a %d %b", $jour)."</h3>\n";
