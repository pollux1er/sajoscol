...
if($_SESSION['statut']=='professeur') {
	...
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Export of my CDT</a>";
}
else {
...
if(!isset($id_groupe)) {
	...
	if($_SESSION['statut']!='professeur') {
		...
		if((!isset($id_classe))&&(!isset($login_prof))) {
			...
			echo "<p class='bold'>Choice of classes&nbsp;:</p>\n";
			...
			if($nb_profs==0){
				echo "<p>No professor ensuring a teaching was found.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
	
			//echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post' name='formulaire'>\n";
			echo "<p>Or choose a professor&nbsp;:</p>\n";
	
			$nb_prof_par_colonne=round($nb_profs/3);
		
			echo "<table width='100%' summary='Choice of the professeur'>\n";
			echo "<tr valign='top' align='center'>\n";
	...
	function change_style_classe(num) {
		if(document.getElementById('tab_id_classe_'+num)) {
			...
			echo " | <a href='".$_SERVER['PHP_SELF']."'>Return to the choice of the classes or the professor</a>";
			echo "</p>\n";

			echo "<p class='bold'>Choice of courses/lesson&nbsp;:</p>\n";
			echo "<blockquote>\n";
	
			//echo "<p></p>\n";
			if(isset($login_prof)) {
	
				echo "<p class='bold'>Lesson of ".civ_nom_prenom($login_prof)."&nbsp;:</p>\n";
				echo "<blockquote>\n";
				...
				echo "<table class='boireaus' summary='Choice of lesson'>\n";
				echo "<tr>\n";
				echo "<th>\n";
				echo "<a href='#' onClick='tout_cocher(true);return false;'><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a> / <a href='#' onClick='tout_cocher(false);return false;'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
				echo "</th>\n";
				echo "<th>Teaching</th>\n";
				...
				echo "<p><b>Action to realized&nbsp;:</b><br />\n";
				echo "<input type='radio' name='action' id='action_export_zip' value='export_zip' checked onchange='modif_param_affichage()' /><label for='action_export_zip'> Generate an export of text book and the zip it</label><br />\n";
				echo "ou<br />\n";
				echo "Set up an access without authentification to the selected text book<br />(<i>to for example, to allow an inspector to consult the text book of a professor during an inspection</i>)";
				echo "<br />\n";
				echo "<input type='radio' name='action' id='action_acces' value='acces' onchange='modif_param_affichage()' /><label for='action_acces'> ' statics' Access&nbsp;: i.e. only the notices type to date could be consulted.</label>";
				echo "<br />\n";

				echo "<input type='radio' name='action' id='action_acces2' value='acces2' onchange='modif_param_affichage()' /><label for='action_acces2'>  ' dynamics' Accès&nbsp;: i.e. the notices possibly typed in the future could be consulted (<i>until the date above</i>).</label>";
				echo "</p>\n";

				echo "<div id='div_param_action_acces' style='margin-left: 3em;'>\n";
				echo "<p class='bold'>Parameters of the access&nbsp;</p>\n";
				echo "<blockquote>\n";
				echo "<p>Description of the reason for the opening of access&nbsp;:<br />";
				echo "<textarea name='description_acces' cols='50' rows='4'></textarea>";
				echo "</p>";
				echo "<p>Date to which you wish to remove the access&nbsp;: ";
				...
				echo "<br />\n";
				echo "(<i>the suppression is not automatic at the date indicated, but fix a date can help to know if the access must be preserved or not</i>)</p>\n";
	...
	function modif_param_affichage() {
		...
		function change_style_groupe(num) {
			...
			elseif(isset($id_classe)) {
				...
				echo "<p class='bold'>Choice of the courses&nbsp;:</p>\n";
				echo "<blockquote>\n";
				...
				for($i=0;$i<count($id_classe);$i++) {
					...
					echo "<table class='boireaus' summary=\"Choice of the lessons of $classe\">\n";
					echo "<tr>\n";
					echo "<th>\n";
					echo "<a href='#' onClick='tout_cocher(true);return false;'><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a> / <a href='#' onClick='tout_cocher(false);return false;'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
					echo "</th>\n";
				...
				echo "<p><b>Action to realized&nbsp;:</b><br />\n";
				echo "<input type='radio' name='action' id='action_export_zip' value='export_zip' checked onchange='modif_param_affichage()' /><label for='action_export_zip'>Generate an export of textbook and zip it</label><br />\n";
				echo "<input type='radio' name='action' id='action_acces' value='acces' onchange='modif_param_affichage()' /><label for='action_acces'> Set up a access without authentification to the textbook choosen<br />(<i>to for example, to allow an inspector to consult the textbooks of a professor during an inspection</i>)</label><br />The access set up is ' statics', i.e. only the notes typed to date could be consulted.";
				/*
				echo "<br />\n";
				...
				*/
				...
				echo "<textarea name='description_acces' cols='50' rows='4'></textarea>";
				echo "</p>";
				echo "<p>Date at which you wish to remove the access&nbsp;: ";
	...
	function change_style_groupe(num) {
		...
		if((document.getElementById('id_groupe_'+num))&&(document.getElementById('label_groupe_'+num))) {
			if(document.getElementById('id_groupe_'+num).checked) {
				...
				echo "<p style='color:red'>You chose neither professor, nor class.</p>\n";
			}
			echo "</blockquote>\n";
		...
		echo "<p class='bold'>Choice of courses&nbsp;:</p>\n";
	
		echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post' name='formulaire'>\n";
	
		$cpt=0;
		$groups=get_groups_for_prof($_SESSION['login']);
		echo "<table class='boireaus' summary='Choice of courses'>\n";
		echo "<tr>\n";
		echo "<th>\n";
		echo "<a href='#' onClick='tout_cocher(true);return false;'><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a> / <a href='#' onClick='tout_cocher(false);return false;'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
		echo "</th>\n";
		...
		//echo "<p style='color:red'>A FAIRE: Ajouter le choix Du/Au à ce niveau</p>\n";
		echo "<p>";
		echo "<label for='choix_periode_dates' style='cursor: pointer;'> \nExport the textbook of the date : </label>";
...
if(!$dirname) {
	echo "<p style='color:red;'>Problem with the temporary folder.</p>\n";
	require("../lib/footer.inc.php");
	die();
}
...
if($action!='acces2') {
	echo "<p class='bold'>Posting of the extracted textbooks</p>\n";
}
...
if(($action=='acces')||($action=='acces2')) {
	...
	if(!$create) {
		echo "<p style='color:red;'>Problem with the temporary folder../documents/".$dirname."</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
...
if($action=='acces2') {
	...
	$res=enregistrement_creation_acces_cdt($chemin_acces, $description_acces, $date1_acces, $date2_acces, $id_groupe);
	if(!$res) {
		echo "<p style='color:red;'>Error during the recording of the installation of the access.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
...
');
	...
	$chaine_info_texte="<br /><p><b>Information&nbsp;:</b><br />The textbook extracted are accessible  without
authentification at the following address &nbsp;:<br /><a href='../$chemin_acces' target='_blank'>../$chemin_acces</a><br />Consult the page, copy the address from address bar and transmit to which you wish.<br />Do not forget to remove this access when it is not useful any more.<br />&nbsp;</p>";
...
if(($_SESSION['statut']=='professeur')||(isset($login_prof))) {
	...
	$html="";
	//$html.=html_entete();
	$html.="<h1 style='text-align:center;'>Textbooks $chaine_info_prof(".$gepiSchoolName." - ".$gepiYear.")</h1>\n";
	$html.="<p>Textbook (<i>$display_date_debut - $display_date_fin</i>) de&nbsp;:</p>\n";
	$html.="<ul>\n";
	for($i=0;$i<count($id_groupe);$i++) {
		...
		$nom_detaille_groupe[$id_groupe[$i]]=$current_group['name']." (<i>".$current_group['description']." in (".$current_group['classlist_string'].")</i>)";
	
		$nom_detaille_groupe_non_html[$id_groupe[$i]]=$current_group['name']." (".$current_group['description']." in (".$current_group['classlist_string']."))";
	
		$html.="<li><a id='lien_id_groupe_$id_groupe[$i]' href='cahier_texte/$nom_page_html_groupe'>".$current_group['name']." (<i>".$current_group['description']." in (".$current_group['classlist_string'].")</i>)</a></li>\n";
	}
	$html.="</ul>\n";
	...
	$html=html_entete("Index of the textbooks",0).$html;
	$html.=html_pied_de_page();
	...
	//if(count($id_classe)>1) {
		...
		$html="";
		//$html.=html_entete();
		$html.="<h1 style='text-align:center;'>Textbooks of $chaine_classes (".$gepiSchoolName." - ".$gepiYear.")</h1>\n";
	...
	for($j=0;$j<count($id_classe);$j++) {
		...
		$html="";
		//$html.=html_entete();
		$html.="<h1 style='text-align:center;'>Textbooks of ".$nom_classe[$j]."(".$gepiSchoolName." - ".$gepiYear.")</h1>\n";
		$html.="<p>Cahier de textes (<i>$display_date_debut - $display_date_fin</i>) de&nbsp;:</p>\n";
		$html.="<ul>\n";
		...
		$html=html_entete("Index of the textbooks of ".$nom_classe[$j],0).$html;
		$html.=html_pied_de_page();
	...
	if(isset($id_classe)) {
		...
		if(isset($current_group)) {
			...
			if(count($current_group["classes"]["list"])==1) {
				$html.="<a id='lien_retour_".$id_groupe[$i]."' href='../index.html'>Return</a>\n";
			}
			else {
				$chaine_cpt_classe="";
				$cpt=0;
		...
		$html.="<div id='div_lien_retour_".$id_groupe[$i]."' class='noprint' style='float:right; width:6em'><a id='lien_retour_".$id_groupe[$i]."' href='../index.html'>Return</a></div>\n";
	}

	$html.="<a name='cible_lien_id_groupe_".$id_groupe[$i]."'></a>\n";

	$html.="<h1 style='text-align:center;'>Textbooks (".$gepiSchoolName." - ".$gepiYear.")</h1>\n";
	$html.="<p style='text-align:center;'>Extraction  of $display_date_debut au $display_date_fin</p>\n";
	$html.="<h2 style='text-align:center;'>Textbook of ".$nom_detaille_groupe[$id_groupe[$i]]." (<i>$display_date_debut - $display_date_fin</i>)&nbsp;:</h2>\n";
	/*
...
if($action=='export_zip') {
	...
	if($v_list==0) {
		echo "<p>Extracted textbooks&nbsp;: <a href='$dossier_export'>$dossier_export</a></p>\n";
	
		echo "<p style='color:red;'>ERROR during the creation of the archive&nbsp;:<br />";
	...
	if(document.getElementById('div_archive_zip')) {
		document.getElementById('div_archive_zip').innerHTML=\"<p class='bold'>Archive of extracted textbooks&nbsp;: <a href='$fichier_archive'>$basename_fichier_archive</a></p>\"
	}
</script>\n";
	}
...
elseif($action=='acces') {
	...
	$chaine_info_texte="<br /><p><b>Information&nbsp;:</b><br />The extracted textbook is accessible without authentification to the following address&nbsp;:<br /><a href='$dossier_export/index.html' target='_blank'>$dossier_export/index.html</a><br />Consult the page, copy the address out of the address bar and transmit to which you wish.<br />Do not forget to remove this access when it is not useful any more.<br />&nbsp;</p>";
