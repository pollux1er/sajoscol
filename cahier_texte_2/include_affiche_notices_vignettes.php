	...
	if (($devoir->getVise() != 'y') or (isset($visa_cdt_inter_modif_notices_visees) AND $visa_cdt_inter_modif_notices_visees == 'no')) {
		...
		if($liens_edition_suppression=="y") {
			$html_balise .=("<a href=\"#\" onclick=\"javascript:
									...
									");
			$html_balise .=("\">");
			$html_balise .=("<img style=\"border: 0px;\" src=\"../images/edit16.png\" alt=\"modify\" title=\"modify\" /></a>\n");
			$html_balise .=(" ");
			$html_balise .=("<a href=\"#\" onclick=\"javascript:
			...
			if(($devoir->getDateVisibiliteEleve()!="")&&(mysql_date_to_unix_timestamp($devoir->getDateVisibiliteEleve())>time())) {
				$html_balise .=("<img src=\"../images/icons/visible.png\" width=\"19\" height=\"16\" alt=\"Date of visibility of the notice for the students\" title=\"Date of visibility of the notice for the students\" /><span style='font-size: xx-small; color:red;'>&nbsp;".get_date_heure_from_mysql_date($devoir->getDateVisibiliteEleve())."</span>\n");
			}
		}
	} else {
		$html_balise .= "<i><span  class=\"red\">Signed note</span></i>";
	}
	$html_balise .= '</div>';
	echo($html_balise);
	echo "<br/>";
...
function affiche_notice_privee_vignette($notice_privee, $couleur_bord_tableau_notice, $color_fond_notices) {
	...
	echo("<strong>&nbsp;Private notice</strong>\n");
...
function affiche_compte_rendu_vignette($compte_rendu, $couleur_bord_tableau_notice, $color_fond_notices) {
		echo("<table style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."\" width=\"100%\" cellpadding=\"1\" bgcolor=\"".$color_fond_notices["c"]."\" summary=\"Table of...\">\n<tr>\n<td>\n");
	...
	if (($documents) and (count($documents)!=0)) {
		...
		foreach ($documents as $document) {
			...
			if(!$document->getVisibleEleveParent()) {
				$html.="<img src='../images/icons/invisible.png' width='19' height='16' alt='Document invisible  for the student and responsible' title='Document invisible  for the student and responsible' />";
			}
