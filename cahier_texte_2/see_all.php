...
if ($current_imprime == 'n') {
	$imprime='y';
	$text_imprime="Version without stringcourses";
	$largeur = "30%";
}
else {
	$imprime='n';
...
//**************** EN-TETE *****************
if ($current_imprime=='n') $titre_page = "Textbook - Overall view";
...
//On vérifie si le module est activé
if (getSettingValue("active_cahiers_texte")!='y') {
	die("<p class='grand centre_texte'>The textbook is not accessible for the moment.</p>");
}
		...
		echo "<p>\n";
			echo "<span class='grand'>\n";
				...
				if($id_groupe=='Toutes_matieres') {
					echo " - All the courses";
				}
				else {
					if ($current_group) {
						echo " - $matiere_nom";
						echo " - class of ".$current_group['classlist_string'];
					}
				}
				if (($id_classe!=-1)&&($id_classe!='')) {
					echo "<br />\n$classe_nom\n";
	...
	echo "<div class='no_print'>\n";
		if ($current_imprime=='n') {
			if ($_SESSION["statut"] == "professeur" OR $_SESSION["statut"] == "scolarite" OR $_SESSION["statut"] == "cpe" OR $_SESSION["statut"] == "autre") {
				echo "<a href='see_all.php'>\n<img src='../images/icons/back.png' alt='Return' class='back_link'/> Return\n</a> - ";
				if ($_SESSION["statut"] == "professeur") {
					echo "<a href='./index.php'>\n<img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to my textbooks\n</a> - ";
				}
			} else {
...
(($selected_eleve_login!='')||($id_classe!=-1))) {
	...
	for($i=0;$i<count($tab_dates);$i++) {
		...
		echo "<tr>\n";
		echo "<th>Course</th>\n";
		echo "<th>Work to do</th>\n";
		echo "<th>Report of meeting</th>\n";
		echo "</tr>\n";
				...
				if(isset($tab_dev[$tab_dates[$i]][$tab_id_grp[$j]])) {
					...
					foreach($tab_dev[$tab_dates[$i]][$tab_id_grp[$j]] as $key => $value) {
						...
						if($value['date_visibilite_eleve']!='0000-00-00 00:00:00') {
							echo "<div style='float:right; width: 6em; border: 1px solid black; margin: 2px; font-size: xx-small; text-align: center;'>Given the ".formate_date($value['date_visibilite_eleve'])."</div>\n";
						}
...
if ($nb_test == 0) {
	...
	if ($_SESSION['statut'] == "responsable") {
		...
		echo "Choose a class and a course.";
	}
	echo "\n</h2>\n";
	require("../lib/footer.inc.php");
	die();
...
echo "<h2 class='grande_ligne couleur_bord_tableau_notice'>\n<strong>TEXTBOOK: reports of meeting</strong>\n</h2>\n";
	...
	*/
	...
	if(($type_notice!="devoir")||
		...
		if(($type_notice=='devoir')&&($not_dev->date_visibilite_eleve!='0000-00-00 00:00:00')) {
			echo "<div style='float:right; width: 6em; border: 1px solid black; margin: 2px; font-size: xx-small; text-align: center;'>Given the ".formate_date($not_dev->date_visibilite_eleve)."</div>\n";
		}
