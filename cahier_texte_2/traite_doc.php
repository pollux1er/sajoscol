...
function ajout_fichier($doc_file, $dest, $cpt_doc, $id_groupe) {
	...
	if ($row = @sql_row($result,0)) {
		...
		echo ("Error: This type of file is not authorized in download");
		die();
	}
	...
	if ($taille > $max_size) {
		echo "Error:Download impossible: authorized maximum size : ".$max_size_ko." Ko";
		die();
	}
	if ($taille == 0) {
		echo "The selected file seems empty: impossible transfer.";
		die();
	}
	...
	if (($total+$taille) > $total_max_size) {
		echo "Error:Download impossible : disk space available (".(($total_max_size - $total)/1024)." Ko) insufficient.";
		die();
	}

	/* Crétion du répertoire de destination */
	if (!creer_repertoire($dest)) {
		echo "Error: Problem of writing on the repertory. Please signal this problem to the administrator of the site";
                echo $dest;
		die();
	}
	...
	if (strstr($nom_sans_ext, "..")) {
		echo "Error: Transfer problem : the file could not be transferred on the repertory. Please signal this problem to the administrator of the site";
		die();
	}
