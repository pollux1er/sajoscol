...
if (isset($_POST['is_posted'])) {
	...
	if (isset($_POST['activer'])) {
		if (!saveSetting("active_cahiers_texte", $_POST['activer'])) $msg = "Error during the recording of the activation/desactivation parameter  !";
	}
	
	if (isset($_POST['version'])) {
		if (!saveSetting("GepiCahierTexteVersion", $_POST['version'])) $msg = "Error during the recording of the number of version of the textbook !";
	}
	...
	if (isset($_POST['begin_day']) and isset($_POST['begin_month']) and isset($_POST['begin_year'])) {
		...
		if (!saveSetting("begin_bookings", $begin_bookings))
				$msg .= "Error during the recording of begin_bookings !";
	}
	if (isset($_POST['end_day']) and isset($_POST['end_month']) and isset($_POST['end_year'])) {
		...
		if (!saveSetting("end_bookings", $end_bookings))
				$msg .= "Error during the recording of end_bookings !";
	}
	...
	if (isset($_POST['cahier_texte_acces_public'])) {
		...
		if (!saveSetting("cahier_texte_acces_public", $temp)) {
			$msg .= "Error during the recording of cahier_texte_acces_public !";
		}
	}
	...
	if (isset($_POST['visa_cdt_inter_modif_notices_visees'])) {
		...
		if (!saveSetting("visa_cdt_inter_modif_notices_visees", $temp)) {
			$msg .= "Error during the recording of visa_cdt_inter_modif_notices_visees !";
		}
	}
	//Fin ajout Eric
	...
	if (isset($_POST['delai_devoirs'])) {
		if (!saveSetting("delai_devoirs", $_POST['delai_devoirs']))
				$msg .= "Error during the recording of the time of visualization of the exams ";
	}
	...
	if (isset($_POST['is_posted'])) {
		if (isset($_POST['cdt_possibilite_masquer_pj'])) {
			if (!saveSetting("cdt_possibilite_masquer_pj", "y")) {
				$msg .= "Error during the recording of the authorization to mask joined documents.<br />";
			}
		}
		elseif (!saveSetting("cdt_possibilite_masquer_pj", "n")) {
			$msg .= "Error during the recording of prohibition to mask joined documents.<br />";
		}
	}
	...
	if (isset($_POST['cdt_autoriser_modif_multiprof'])) {
		...
		if (!saveSetting("cdt_autoriser_modif_multiprof", $temp)) {
			$msg .= "Error during the recording of cdt_autoriser_modif_multiprof !";
		}
	}
}

// on demande une validation si on quitte sans enregistrer les changements
$messageEnregistrer="Information was modified. Do you really want to leave without recording ?";
...
<center>
	<input type="submit" value="Save" style="font-variant: small-caps;" />
</center>
</form>
<hr />
