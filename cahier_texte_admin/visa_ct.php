...
if (isset($_POST['ok_enr_visa'])) {
	...
	if(isset($_POST['texte_visa_FCK'])) {
			...
			if (!saveSetting("texte_visa_cdt", $txt)) {
				$msg .= "Error during the recording of the text of the visa !";
				$erreur = true;
			}
	}
	if (!$error) {
		$msg = "The visa was recorded.";
	}
}
...
if (isset($_POST['begin_day']) and isset($_POST['begin_month']) and isset($_POST['begin_year'])) {
	...
	if (!saveSetting("date_signature", $date_signature)) $msg .= "Error during the recording of date of signature of the textbooks !";
}
...
if (isset($_POST['visa_ct'])) {
	...
	for ($i=0; ($row=sql_row($query,$i)); $i++) {
		...
		if (isset($_POST[$temp])) {
			...
			//echo "$sql_visa_ct<br />\n";
			$visa_ct = sql_query($sql_visa_ct);updateOnline($sql_visa_ct);
			if(!$visa_ct) {
				$msg.="Error during the recording of the visa of the reports for the group n°$id_groupe and the professor $id_professeur<br />\n";
			}
			...
			$insertion_visa = sql_query($sql_insertion_visa);updateOnline($sql_insertion_visa);
			if ($error == 'no') {
				//$msg .= "Cahier(s) de textes signé(s).<br />\n";
				$nb_cdt_signes++;
			} else {
				$msg .= "There was a problem during the signature of the textbook.<br />\n";
			}
		}
		$iterateur++;
	}
	...
	for ($i=0; ($row=sql_row($query,$i)); $i++) {
		...
		if (isset($_POST[$temp])) {
			...
			if(!$visa_ct) {
				$msg.="Error during the recording of the visa of the notices of exams for the group n°$id_groupe and the professor $id_professeur<br />\n";
			}
		}
		$itera++;
	}
	...
	if($nb_cdt_signes>0) {
		$msg .= "$nb_cdt_signes signed textbook .<br />\n";
	}
}
...
//=============================================
// header
$titre_page = "Signature of textbooks";
...
if ($_SESSION['statut'] == "autre"||$_SESSION['statut']== "scolarite") {
	...
	echo "<p class=\"bold\"><a href=\"index.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to home</a>";
}

if(isset($definir_visa_par_defaut)) {
	echo " | <a href='visa_ct.php'>Sign the CDT</a>";
}
else {
	echo " | <a href='visa_ct.php?definir_visa_par_defaut=y'>Define the text of the default visa</a>";
}
echo "</p>\n";

echo "<h2>Signature of textbooks</h2>\n";
...
if(isset($definir_visa_par_defaut)) {
	...
	echo "<h2 class='gepi' style=\"text-align: center;\">Text of the default visa </h2>\n";
	echo "<p><em>Working of the visa :</em></p><p>\n";
	...
	echo "<input type='submit' name=\"ok_enr_visa\" value='Save the visa' /></p>\n";
	echo "</form>\n";
	echo "<br /><br />";
...
echo "<p>The table below presents all the textbooks currently on line.</p>\n";
echo "<ul>\n";
echo "<li>&nbsp;&nbsp;You can sort the table by the group or the owner of a textbook by clicking on the corresponding link.</li>\n";
echo "<li>&nbsp;&nbsp;You can visualize a textbook.</li>\n";
echo "<li>&nbsp;&nbsp;You can also sign one or more textbooks with the text below.<br />The default text can be defined <a href='visa_ct.php?definir_visa_par_defaut=y'>ici</a></li>\n";
echo "</ul>\n";
...
<?php
	...
	//echo "<form enctype=\"multipart/form-data\" action=\"visa_ct.php\" method=\"post\">\n";
	//echo add_token_field();
	echo "<h2 class='gepi' style=\"text-align: center;\">Text of the visa to be place on the textbooks</h2>\n";
	echo "<p><em>Mise en forme du visa :</em> <a href='visa_ct.php'>Re-initialize to the default visa</a></p><p>\n";
...
<th><b><a href='visa_ct.php?order_by=jc.id_classe,jm.id_matiere'>Class(es)</a></b></th>
<th><b><a href='visa_ct.php?order_by=jm.id_matiere,jc.id_classe'>Groupe</a></b></th>
<th><b><a href='visa_ct.php?order_by=ct.id_login,jc.id_classe,jm.id_matiere'>Owner</a></b></th>
<th><b>Numbers<br />of notices</b></th>
<th><b>Numbers<br />of notices<br />"exams"</b></th>
<th>
<b>Action</b></th>
<th><b><input type="submit" name="visa_ct" value="Sign the books" onclick="return confirmlink(this, 'The signature of a textbook is final.Are you sure you want to continue ?', 'Confirmation of the signature')" /></b>
<p><b>whose date is lower than</b></p>
<?php
...
for ($i=0; ($row=sql_row($query,$i)); $i++) {
	...
	if ($_SESSION["statut"] == 'autre' OR $_SESSION["statut"] == 'administrateur' OR $_SESSION["statut"] == 'scolarite') {
		...
		echo '<td><a href="../cahier_texte/see_all.php?id_groupe='.$id_groupe.'&amp;id_classe='.$id_classe.'&amp;retour_cdt=visa_cdt">See</a></td>';
	}else{
		echo "<td><a href='../public/index.php?id_groupe=".$id_groupe."' target='_blank'>See</a></td>";
	}
