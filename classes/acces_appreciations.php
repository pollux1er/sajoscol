...
if($_SESSION['statut']=="professeur") {
	if(getSettingValue('GepiAccesRestrAccesAppProfP')!="yes") {
		$msg="Prohibited acces to the parameter setting of the access to appreciatons /opinion for the parents and students.";
	...
	if(mysql_num_rows($test)==0){
		...
		$msg="Vous n'êtes pas ".$gepi_prof_suivi.".<br />You should not thus acces to this page.";
		header("Location: ../accueil.php?msg=".rawurlencode($msg));
	    die();
	}
}
...
echo "<p class='bold'><a href='../accueil.php' onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Home</a>\n";
...
if($_SESSION['statut']=="professeur") {
	...
	$test=mysql_query($sql);
	if(mysql_num_rows($test)==0){
		echo "<p>Vous n'êtes pas ".$gepi_prof_suivi.".<br />You should not thus access to this page.</p>\n";
		echo "<p><br /></p>\n";
...
if(mysql_num_rows($res_classe)==0) {
	echo "<p>You do not have access to any class.</p>\n";
	echo "<p><br /></p>\n";
	require("../lib/footer.inc.php");
	exit();
}
	...
	if($poursuivre=="y") {
		if(($periode2!=NULL)&&($choix_date2!=NULL)) {
			...
			while ($lig=mysql_fetch_object($res_classe)) {
				...
				//echo "$sql2<br />";
				$update=mysql_query($sql2);updateOnline($sql2);
			}
		}
	}

	// On refait la requête de liste des classes
	$res_classe=mysql_query($sql);updateOnline($sql);
}
...
echo "<p>You can define here when the accounts users for responsibles and of the students can access the appreciations of the professors and opinion of the staff meeting.<br />
It is often appreciated to prohibit the access to the students and responsibles before the staff meeting was held.<br />
This access is conditioned by the existence of the accounts responsibles and students.</p>\n";
echo "<br />\n";

if($acces_app_ele_resp=='manuel') {
	echo "<p>Click on the key <img src='../images/icons/configure.png' width='16' height='16' alt=\"Manuel\" /> to give or remove the access to the appreciations.</p>\n";
}
elseif($acces_app_ele_resp=='date') {
	echo "<p>Click on the calendar <img src='../images/icons/date.png' width='16' height='16' alt=\"Choice of a date of unlocking\" /> to give or remove the access to the appreciations.</p>\n";
}
else {
	if($_SESSION['statut']=='scolarite') {
		echo "<p>The access is automatically open <b>$delais_apres_cloture</b> days after <a href='../bulletin/verrouillage.php'> period closure</a>.</p>\n";
	}
	else {
		echo "<p>The access is automatically open <b>$delais_apres_cloture</b> days after the period closure.</p>\n";
	}
}
...
//echo "<p align='center'><input type='submit' name='submit' value='Validate' /></p>\n";
...
$texte.="<br />\n";
$texte.="<input type='button' name='choix_date_valider' value='Validate' onclick=\"g_date()\" />\n";
$texte.="</p>\n";
$texte.="</form>\n";
...
$texte.="<br />\n";
//$texte.="<input type='button' name='choix_date_valider2' value='Validate' onclick=\"g_date()\" />\n";
$texte.="<input type='submit' name='choix_date_valider2' value='Validate' />\n";
$texte.="</p>\n";
$texte.="</form>\n";
...
if($acces_app_ele_resp=='manuel') {
	...
	for($i=1;$i<=$max_per;$i++) {
		...
		echo "<a href='#' onclick='modif_periode($i,\"y\");return false;'><img src='../images/enabled.png' width='15' height='15' alt='make available' /></a>/\n";
		echo "<a href='#' onclick='modif_periode($i,\"n\");return false;'><img src='../images/disabled.png' width='15' height='15' alt='make inaccessible' /></a>\n";
...
elseif($acces_app_ele_resp=='date') {
	...
	for($i=1;$i<=$max_per;$i++) {
		...
		echo "<a href='#' onclick=\"$('choix_date_periode2').value=$i;afficher_div('infobulle_choix_date2','y',-100,20);return false;\"><img src='../images/icons/date.png' width='16' height='16' alt=\"Choice of a date of unlocking\" /></a>\n";
			...
			if(count($nom_periode)>0){
				...
				for($i=1;$i<=count($nom_periode);$i++) {
					...
					echo "<td>\n";
						...
						echo "<a href='#' onclick=\"g_periode_close('$id_div', $id_classe, $i,'$current_statut');return false;\"><img src='../images/icons/securite.png' width='16' height='16' alt=\"Closed period\" /></a>\n";
						echo "</div>\n";
						...
						if($accessible=="y") {
							...
							if($display_date!='00/00/0000') {
								...
								echo " <span style='font-size:x-small;'>since the closure of the period</span>";
							}
							...
							if($ver_periode[$i]=='N') {
								echo " <span style='font-size:x-small;'>open period</span>";
							}
							elseif($ver_periode[$i]=='P') {
								echo " <span style='font-size:x-small;'>partially closed period</span>";
							}
							else {
								// On est dans le cas du délais après cloture

								echo " <span style='font-size:x-small;'>$display_date + $delais_apres_cloture day(s)</span>";
...
if($_SESSION['statut']=="administrateur") {
	echo "The access mode to the appréciatons (<i>manual/date/closed period</i>) and the times after end of period are parameterized as an
administrator in <a href='../gestion/param_gen.php#delais_apres_cloture'>General management/General configuration</a>";
}
