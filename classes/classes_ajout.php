...
if(mysql_num_rows($call_classe)==0) {
	echo "The class n°$id_classe (id_classe) do not exist.<br />\n";
	die();
}
...
if (isset($is_posted) and ($is_posted == 1)) {
	...
	*/
			...
			while ($i < $nb_periode) {
				...
				if(isset($tab_per_ajout_eleve[$i])) {
					...
					if(mysql_num_rows($test_clas_per)>0) {
						...
						$msg_complement.=get_nom_prenom_eleve($login_eleve)." is already in another class&nbsp;: ".get_class_from_id($lig_clas_per->id_classe)."<br />\n";
						$reg_ok = 'no';
					}
					else {
						...
						if(mysql_num_rows($res_cpe)==1) {
							...
							$msg_complement.="<br />The student $login_eleve was not associated one CPE.";
						}
						...
						if(mysql_num_rows($res_pp)==1) {
							...
							$msg_complement.="<br />The student $login_eleve was not associated one ".$gepiProfSuivi.".";
						}
					}
				}
				$i++;
	...
	if (($reg_data) == 'yes') {
	$msg = "The recording of the data was correctly done !";
	} else {
...
//**************** EN-TETE **************************************
$titre_page = "Management of the classes | Addition of students to a class";
...
<p class="bold">
<a href="classes_const.php?id_classe=<?php echo $id_classe;?>"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return to the page of management of the students</a>
...
<p><b>Addition of students to the class of <?php echo $classe; ?></b><br />List of students not associated to the class :</p>
...
$nombreligne = mysql_num_rows($call_eleves);
if ($nombreligne == '0') {
	echo "<p>There are no students currently in the base.</p>\n";
} else {
	$eleves_non_affectes = 'no';
	echo "<table class='boireaus' cellpadding='5'>\n";
	echo "<tr>\n";
	echo "<th><p><b>Name First name </b></p></th>\n";
	if($classes_ajout_sans_regime!="y") {
		echo "<th><p><b>Regim</b></p></th>\n";
	}
	echo "<th><p><b>Redoubling</b></p></th>\n";
	$i="1";
	while ($i < $nb_periode) {
		echo "<th><p><b>Add per. $i</b></p></th>\n";
		$i++;
	}

	echo "<th><p style='font-weight:bold; text-align:center;'>check / uncheck <br />all periods</p></th>\n";
	echo "</tr>";
	$k = '0';
	//=========================
	...
	While ($k < $nombreligne) {
		...
		if ($inserer_ligne == 'yes') {
			...
			echo "<td><center><a href=\"javascript:CocheLigne($id_eleve);changement();\"><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a> / <a href=\"javascript:DecocheLigne($id_eleve);changement();\"><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a></center></td>\n";
			echo "</tr>\n";
	...
	if ($eleves_non_affectes == 'no') {
		echo "<p>There is no student available to add !";
	} else {
