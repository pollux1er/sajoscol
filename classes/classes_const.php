<?php
/*
* $Id: classes_const.php 8642 2011-11-19 17:07:32Z crob $
*
...
if (isset($is_posted)) {
	...
	While ($k < $nombreligne) {
		...
		if($num_eleve!=-1){
			...
			if ($nb_test_regime == 0) {
				...
				updateOnline("INSERT INTO j_eleves_regime SET login='$login_eleve',     doublant='$reg_doublant', regime='$reg_regime'");
				...
				updateOnline("UPDATE j_eleves_regime SET doublant = '$reg_doublant', regime = '$reg_regime'  WHERE login='$login_eleve'");
				if (!($reg_data)) $reg_ok = 'no';
			}
			...
			while ($i < $nb_periode) {
				...
				if ($del_eleve[$i] == 'yes') {
					...
					if (($nb_test != 0) or ($nb_test_app != 0) or ($nb_test_app_conseil != 0)) {
						$autorisation_sup = 'no';
						$msg = "<font color = 'red'>--> Impossible to remove the student $eleve_login from the class for the period $i !<br />This one has averages or appreciations for this period. Start by removing the data of the student for this period!</font><br />\n";
						$reg_ok = "impossible";
					} else {
	...
	$msg.="The modifications were recorded !";
		...
		$message_enregistrement = "Impossible operation (See warning message in red).";
		$affiche_message = 'yes';
	} else {
	//$message_enregistrement = "Il y a eu un problème lors de l'enregistrement";
		$message_enregistrement="There was a problem during the recording";
		$affiche_message = 'yes';
	}
	//$affiche_message = 'yes';
}
...
if(isset($_GET['add_eleve_classe'])) {
	...
	if(($num_periode=='')||(preg_match("/[^0-9]/", $num_periode))) {
		$msg="Number of the period $num_periode invalid for the addition of $login_eleve in the class.";
	}
	elseif(($login_eleve=='')||(preg_match("/[^A-Za-z0-9\._-]/", $login_eleve))) {
		$msg="Student login  $login_eleve invalid for the addition in the class in period $num_periode.";
	}
	else {
		...
		if(mysql_num_rows($test)>0) {
			...
			$msg="$login_eleve is already registered in the class ".get_class_from_id($lig->id_classe)." in period $num_periode.";
		}
		else {
			...
			updateOnline($sql);
			if(!$insert) {
				$msg="Error during the addition of $login_eleve in the class in period $num_periode.";
			}
			else {
				$msg="Addition of $login_eleve in the class in period $num_periode done.<br />Reminf to define the <a href='eleve_options.php?login_eleve=$login_eleve&id_classe=$id_classe'>followed courses</a>.";
			}
		}
	}
}
...
//**************** EN-TETE **************************************
$titre_page = "Management of the classes | Management of students";
...
if(!isset($quitter_la_page)){
	...
	echo "<p class='bold'>\n";
	echo "<a href='index.php' onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return </a> | <a href='prof_suivi.php?id_classe=$id_classe' onclick=\"return confirm_abandon (this, change, '$themessage')\">".ucfirst(getSettingValue("gepi_prof_suivi"))." : fast typing</a>\n";
	...
	echo "<form action='".$_SERVER['PHP_SELF']."' name='form1' method='post'>\n";
	echo "<p class='bold'>";
	echo "<a href='index.php' onClick='self.close();return false;'><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Close the page again </a> | <a href='prof_suivi.php?id_classe=$id_classe' onclick=\"return confirm_abandon (this, change, '$themessage')\">".ucfirst(getSettingValue("gepi_prof_suivi"))." : fast typing</a>\n";
	if($id_class_prec!=0){echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_class_prec&amp;quitter_la_page=y' onclick=\"return confirm_abandon (this, change, '$themessage')\">Previous Class</a>";}
	...
	if($id_class_suiv!=0){echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_class_suiv&amp;quitter_la_page=y' onclick=\"return confirm_abandon (this, change, '$themessage')\">Next class</a>";}
	echo "</p>\n";
	echo "</form>\n";
...
<?php
echo "<img src='../images/icons/add_user.png' alt='' /> <a href='classes_ajout.php?id_classe=$id_classe' onclick=\"return confirm_abandon (this, change, '$themessage')\">Add students to the class</a>";
?>
</p>
<p class='small'><b>Notice :</b> during the removing of a student of the class for a given period, this one will be removed from all the courses to which it was registered for the period in question.</p>
<?php
...
$nombreligne = mysql_num_rows($call_eleves);
if ($nombreligne == '0') {
	echo "<p>There are no students currently in this class.</p>\n";
} else {
...
";
	...
	$k = '0';
	echo "<table class='boireaus' border='1' cellpadding='5' class='boireaus' summary='Students'>\n";
	echo "<tr>\n";
	...
	echo "</select>\n";
	echo "&nbsp;<a href='javascript: imposer_pp();' title='Impose for all the students'><img src='../images/icons/wizard.png' width='16' height='16' alt='Impose for all the students' /></a>\n";


	echo "</th>\n";
	echo "<th>CPE responsible\n";
	...
	echo "</select>\n";
	echo "&nbsp;<a href='javascript: imposer_cpe();' title='Impose for all the students'><img src='../images/icons/wizard.png' width='16' height='16' alt='Impose for all the students' /></a>\n";
	echo "</th>\n";

	$i="1";
	while ($i < $nb_periode) {
		//echo "<th><p class=\"small\">Retirer de la classe<br />$nom_periode[$i]</p></th>\n";
		echo "<th><p class=\"small\">Remove from the class<br />$nom_periode[$i]<br />\n";

		echo "<a href=\"javascript:CocheColonne(".$i.");changement();\"><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a> / <a href=\"javascript:DecocheColonne(".$i.");changement();\"><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>";
	...
	While ($k < $nombreligne) {
		...
		echo "<a href='../eleves/modify_eleve.php?eleve_login=".$login_eleve."' onclick=\"return confirm_abandon (this, change, '$themessage')\" title='Edit the student card ' target='_blank'>";
		...
		echo "<br /><b><a href='eleve_options.php?login_eleve=".$login_eleve."&amp;id_classe=".$id_classe."' onclick=\"return confirm_abandon (this, change, '$themessage')\">Followed courses</a></b>";
		echo "</p></td>\n";
		echo "<td style='padding: 0;'>\n";

		echo "";

		echo "<table style='border-collaspe: collapse;' summary='Regim'>\n";
		echo "<tr>\n";
		//=========================
		// MODIF: boireaus 20071010
		/*
		...
		*/
		...
		//=========================
		$profsuivi = '(vide)';
		echo "<option value='$profsuivi'>(empty)</option>\n";
		for($loop=0;$loop<count($tab_profsuivi);$loop++) {
			...
			if ($nb_ligne != 0) {
				...
				if((mysql_num_rows($verif)==0)&&(mysql_num_rows($verif2)==0)) {
					...
					if($_SESSION['statut']=='administrateur') {
						echo "<br />\n";
						echo "<span style='font-size:x-small'><a href='changement_eleve_classe.php?login_eleve=$login_eleve&amp;id_classe=$id_classe&amp;periode_num=$i'>Change class</a></span>";
					}
				}
				else {
					echo "<img src='../images/icons/bulletin_16.png' width='16' height='16' title=\"Nonempty bulletin : The student cannot be removed from the class\" alt=\"Nonempty bulletin : The student cannot be removed from the class\" />";
				}
				...
				if($nom_classe!="") {
					...
					echo "<td style='vertical-align: bottom; text-align: right;'><a href='".$_SERVER['PHP_SELF']."?add_eleve_classe=y&amp;num_periode=$i&amp;id_classe=$id_classe&amp;login_eleve=$login_eleve".add_token_in_url()."' onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/add.png' width='16' height='16' alt=\"Add ".strtoupper($nom_eleve)." ".$prenom_eleve." to the class $classe in period $i\" title=\"Add ".strtoupper($nom_eleve)." ".$prenom_eleve." to the class $classe in period $i\" /></a></td>\n";
				}
			}
			$i++;
		}

		echo "<td><a href=\"javascript:CocheLigne(".$k.");changement()\"><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a> <a href=\"javascript:DecocheLigne(".$k.");changement()\"><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a></td>";
