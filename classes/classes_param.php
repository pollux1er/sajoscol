			...
			if ($nb_periode == $per) {
				...
				if (isset($_POST[$temp])) {
					...
					While ($k < $per+1) {
						...
						updateOnline("UPDATE periodes SET nom_periode='".$_POST[$temp2]."' where (id_classe='".$id_classe."' and num_periode='".$k."')");
					...
					if ($_POST[$temp2] != '') {
						...
						updateOnline("UPDATE classes SET suivi_par='".$_POST[$temp2]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
//                        echo "classe : ".$id_classe." - reg_suivi_par".$per." : ".$_POST[$temp2]."</br>";
					...
					if ($_POST[$temp2] != '') {
						...
						updateOnline("UPDATE classes SET formule='".html_entity_decode($_POST[$temp2])."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
//                        echo "classe : ".$id_classe." - reg_formule".$per." : ".$_POST[$temp2]."</br>";
					...
					if (isset($_POST['nb_'.$per.'_reg_format'])) {
						...
						updateOnline("UPDATE classes SET format_nom='".$tab[2]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
//                        echo "classe : ".$id_classe." - ".$_POST['nb_'.$per.'_reg_format']."</br>";
					...
					if (isset($_POST['display_rang_'.$per])) {
						...
						updateOnline("UPDATE classes SET display_rang='".$_POST['display_rang_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					//====================================
					// AJOUT: boireaus
					if (isset($_POST['display_address_'.$per])) {
						...
						updateOnline("UPDATE classes SET display_address='".$_POST['display_address_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					if (isset($_POST['display_coef_'.$per])) {
						...
						updateOnline("UPDATE classes SET display_coef='".$_POST['display_coef_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					if (isset($_POST['display_nbdev_'.$per])) {
						...
						updateOnline("UPDATE classes SET display_nbdev='".$_POST['display_nbdev_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['display_moy_gen_'.$per])) {
						...
						updateOnline("UPDATE classes SET display_moy_gen='".$_POST['display_moy_gen_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['display_mat_cat_'.$per])) {
						...
						updateOnline("UPDATE classes SET display_mat_cat='".$_POST['display_mat_cat_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if ((isset($_POST['modele_bulletin_'.$per])) AND ($_POST['modele_bulletin_'.$per]!=0)) {
						...
						updateOnline("UPDATE classes SET modele_bulletin_pdf='".$_POST['modele_bulletin_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_nomdev_'.$per])) {
						$register = mysql_query("UPDATE classes SET rn_nomdev='".$_POST['rn_nomdev_'.$per]."' where id='".$id_classe."'");
						updateOnline("UPDATE classes SET rn_nomdev='".$_POST['rn_nomdev_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['rn_toutcoefdev_'.$per])) {
						...
						updateOnline("UPDATE classes SET rn_toutcoefdev='".$_POST['rn_toutcoefdev_'.$per]."' where id='".$id_classe."'");
					...
					if (isset($_POST['rn_coefdev_si_diff_'.$per])) {
						...
						updateOnline("UPDATE classes SET rn_coefdev_si_diff='".$_POST['rn_coefdev_si_diff_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['rn_datedev_'.$per])) {
						...
						updateOnline("UPDATE classes SET rn_datedev='".$_POST['rn_datedev_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['rn_sign_chefetab_'.$per])) {
						...
						updateOnline("UPDATE classes SET rn_sign_chefetab='".$_POST['rn_sign_chefetab_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['rn_sign_pp_'.$per])) {
						...
						updateOnline("UPDATE classes SET rn_sign_pp='".$_POST['rn_sign_pp_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['rn_sign_resp_'.$per])) {
						...
						updateOnline("UPDATE classes SET rn_sign_resp='".$_POST['rn_sign_resp_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['rn_sign_nblig_'.$per])) {
						...
						updateOnline("UPDATE classes SET rn_sign_nblig='".$_POST['rn_sign_nblig_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if (isset($_POST['rn_formule_'.$per])) {
						if ($_POST['rn_formule_'.$per]!='') {
							...
							updateOnline("UPDATE classes SET rn_formule='".$_POST['rn_formule_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
					...
					if (isset($_POST['ects_fonction_signataire_attestation_'.$per])) {
						if ($_POST['ects_fonction_signataire_attestation_'.$per]!='') {
							...
							updateOnline("UPDATE classes SET ects_fonction_signataire_attestation='".$_POST['ects_fonction_signataire_attestation_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
					...
					if (isset($_POST['ects_type_formation_'.$per])) {
						if ($_POST['ects_type_formation_'.$per]!='') {
							...
							updateOnline("UPDATE classes SET ects_type_formation='".$_POST['ects_type_formation_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
                    ...
                    if (isset($_POST['ects_parcours_'.$per])) {
						if ($_POST['ects_parcours_'.$per]!='') {
							...
							updateOnline("UPDATE classes SET ects_parcours='".$_POST['ects_parcours_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
                    ...
                    if (isset($_POST['ects_domaines_etude_'.$per])) {
						if ($_POST['ects_domaines_etude_'.$per]!='') {
							...
							updateOnline("UPDATE classes SET ects_domaines_etude='".$_POST['ects_domaines_etude_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
                    ...
                    if (isset($_POST['ects_code_parcours_'.$per])) {
						if ($_POST['ects_code_parcours_'.$per]!='') {
							...
							updateOnline("UPDATE classes SET ects_code_parcours='".$_POST['ects_code_parcours_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
					...
					while ($row = mysql_fetch_array($get_cat, MYSQL_ASSOC)) {
						...
						updateOnline("INSERT INTO j_matieres_categories_classes SET classe_id = '" . $id_classe . "', categorie_id = '" . $row["id"] . "', priority = '" . $reg_priority . "', affiche_moyenne = '" . $reg_aff_moyenne . "'");
						} else {
					...
					updateOnline("UPDATE j_matieres_categories_classes SET priority = '" . $reg_priority . "', affiche_moyenne = '" . $reg_aff_moyenne . "' WHERE (classe_id = '" . $id_classe . "' and categorie_id = '" . $row["id"] . "')");
					}
						if (!$res) {
							$msg .= "<br />An error occurred during the recording of the data of category.";
						}
					}
					...
					if((isset($_POST['change_coef']))&&($_POST['change_coef']=='y')) {
						if((isset($_POST['coef_enseignements']))&&($_POST['coef_enseignements']!="")) {
							...
							if($coef_enseignements!="") {
								...
								updateOnline("UPDATE j_groupes_classes SET coef='".$coef_enseignements."' WHERE id_classe='".$id_classe."';";
								if(!$update_coef) {
									$msg .= "<br />An error occurred during the update of the coefficients for the class $id_classe.";
								}
							}
						}
					}
					...
					if((isset($_POST['forcer_recalcul_rang']))&&($_POST['forcer_recalcul_rang']=='y')) {
						...
						if(mysql_num_rows($res_per)>0) {
							...
							updateOnline("UPDATE groupes SET recalcul_rang='$recalcul_rang' WHERE id in (SELECT id_groupe FROM j_groupes_classes WHERE id_classe='$id_classe');";
							//echo "$sql<br />";
							$res=mysql_query($sql);
							if(!$res) {
								$msg.="<br />Error during the programming of the recalculation of the ranks for the class ".get_nom_classe($id_classe).".";
							}
						}
						else {
							$msg.="<br />No period is defined for this class.<br />Impossible recalculation of the ranks for the class ".get_nom_classe($id_classe).".";
						}
					}
							...
							if(mysql_num_rows($verif)==0) {
								...
								if($create) {
									...
									if($coef_nouvel_enseignement!="") {
										...
										updateOnline($sql);
										if(!$res_coef) {
											$msg .= "<br />Error during the update of the coefficient of the group n°$create for the class n°$id_classe.";
										}
									}
								}
							}
	...
	if ($reg_ok=='') {
		$message_enregistrement = "No modification was carried out !";
		$affiche_message = 'yes';
	} else if ($reg_ok=='yes') {
		$message_enregistrement = "The modifications were carried out successfully.";
		$affiche_message = 'yes';
	} else {
		$message_enregistrement = "There was a problem during the recording of the modification.";
		$affiche_message = 'yes';
	}
}

//**************** EN-TETE *****************
$titre_page = "Management of the classes - Parameter setting of the classes by batches";
require_once("../lib/header.inc");
//**************** FIN EN-TETE *****************

If ($max_periode <= 0) {
echo "No class comprising periods was defined.";
die();
}
echo "<form action=\"classes_param.php\" method='post'>\n";
echo add_token_field();
echo "<p class=bold><a href=\"index.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return </a>| <input type='submit' name='enregistrer1' value='Save' /></p>";
echo "On this page, you can modify various parameters by batches of classes selected below.";
/*
...
while ($per < $max_periode) {
	...
	If ($nb != 0) {
		echo "<center><p class='grand'>Classes having ".$per." period";
		if ($per > 1) echo "s";
		echo "</p></center>\n";
		...
		while ($i < $nb_ligne) {
			...
			echo "<th>";
			echo "<a href='javascript:modif_case($per,$i,\"lig\",true)'><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a>/\n";
			echo "<a href='javascript:modif_case($per,$i,\"lig\",false)'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
			echo "</th>\n";
		...
		while ($j < 3) {
			echo "<th>\n";
			echo "<a href='javascript:modif_case($per,$j,\"col\",true)'><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a>/\n";
			echo "<a href='javascript:modif_case($per,$j,\"col\",false)'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
			echo "</th>\n";
			$j++;
		}
		//echo "<td>&nbsp;</td>\n";
		echo "<th>";
			...
			echo "<a href='javascript:tout_cocher($per,true)'><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a>/\n";
			echo "<a href='javascript:tout_cocher($per,false)'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
	...
	function change_style_classe(num) {
		...
		?>
		<p class='bold'>For the classes selected above&nbsp;: </p>
		<p>No modification will be made to the empty fields</p>
		...
		<p>First name and name of the signatory of the bulletins<?php if ($gepiSettings['active_mod_ects'] == "y") echo " and of ECTS certificates " ?> (head of school or his representative)&nbsp;:
		<br /><input type="text" size="30" name="<?php echo "nb_".$per."_reg_suivi_par"; ?>" value="" /></p>
        <?php if ($gepiSettings['active_mod_ects'] == "y") { ?>
            <p>Function of the above-named signatory (ex.: "Proviseur")&nbsp;: <br /><input type="text" size="40" name="ects_fonction_signataire_attestation_<?php echo $per;?>" value="" /></p>
        <?php } ?>
		<p>Formula to insert on the bulletins (this formula will be followed by name and first name of the designated person above&nbsp;:
		<br /><input type="text" size="80" name="<?php echo "nb_".$per."_reg_formule"; ?>" value="" /></p>
		<p>Formatting of the identity of the professors&nbsp;:

		<br />
		<input type="radio" name="<?php echo "nb_".$per."_reg_format"; ?>" id="<?php echo "nb_".$per."_reg_format"; ?>_np" value="<?php echo "nb_".$per."_np"; ?>" />
		<label for='<?php echo "nb_".$per."_reg_format"; ?>_np' style='cursor: pointer;'>Name First name (Durand Albert)</label>
		<br />
		<input type="radio" name="<?php echo "nb_".$per."_reg_format"; ?>" id="<?php echo "nb_".$per."_reg_format"; ?>_pn" value="<?php echo "nb_".$per."_pn"; ?>" />
		<label for='<?php echo "nb_".$per."_reg_format"; ?>_pn' style='cursor: pointer;'>First name Name (Albert Durand)</label>
		<br />
		<input type="radio" name="<?php echo "nb_".$per."_reg_format"; ?>" id="<?php echo "nb_".$per."_reg_format"; ?>_in" value="<?php echo "nb_".$per."_in"; ?>" />
		<label for='<?php echo "nb_".$per."_reg_format"; ?>_in' style='cursor: pointer;'>Initial of first name Name (A. Durand)</label>
		<br />
		<input type="radio" name="<?php echo "nb_".$per."_reg_format"; ?>" id="<?php echo "nb_".$per."_reg_format"; ?>_ni" value="<?php echo "nb_".$per."_ni"; ?>" />
		<label for='<?php echo "nb_".$per."_reg_format"; ?>_ni' style='cursor: pointer;'>Name Initial of First name (Durand A.)</label>
		<br />
		<input type="radio" name="<?php echo "nb_".$per."_reg_format"; ?>" id="<?php echo "nb_".$per."_reg_format"; ?>_cnp" value="<?php echo "nb_".$per."_cnp"; ?>" />
		<label for='<?php echo "nb_".$per."_reg_format"; ?>_cnp' style='cursor: pointer;'>Civility Name First name (M. Durand Albert)</label>
		<br />
		<input type="radio" name="<?php echo "nb_".$per."_reg_format"; ?>" id="<?php echo "nb_".$per."_reg_format"; ?>_cpn" value="<?php echo "nb_".$per."_cpn"; ?>" />
		<label for='<?php echo "nb_".$per."_reg_format"; ?>_cpn' style='cursor: pointer;'>Civility First name Name (M. Albert Durand)</label>
		<br />
		<input type="radio" name="<?php echo "nb_".$per."_reg_format"; ?>" id="<?php echo "nb_".$per."_reg_format"; ?>_cin" value="<?php echo "nb_".$per."_cin"; ?>" />
		<label for='<?php echo "nb_".$per."_reg_format"; ?>_cin' style='cursor: pointer;'>Civ. Initial-First name Name (M. A. Durand)</label>
		<br />
		<input type="radio" name="<?php echo "nb_".$per."_reg_format"; ?>" id="<?php echo "nb_".$per."_reg_format"; ?>_cni" value="<?php echo "nb_".$per."_cni"; ?>" />
		<label for='<?php echo "nb_".$per."_reg_format"; ?>_cni' style='cursor: pointer;'>Civ. Name initial-First name (M. Durand A.)</label>
		<br />
<br />
...
<tr>
	...
	<td>
	<input type='checkbox' name='change_coef' id='change_coef' value='y' /> Pass the coefficients of all the courses to&nbsp;:
	</td>
	<td>
...
<tr>
	...
	<input type='checkbox' name='creer_enseignement' id='creer_enseignement' value='y' /> Create a course of&nbsp;:
	</td>
	<?php
		$sql="SELECT DISTINCT matiere,nom_complet FROM matieres ORDER BY nom_complet,matiere;";
		$res_mat=mysql_query($sql);
		if(mysql_num_rows($res_mat)==0) {
			echo "<td>No course is still created.</td>\n";
		}
		else {
			echo "<td colspan='2'>\n";
...
<?php
	...
	$texte="<p>A user encountered one day the following problem&nbsp;:<br />The rank was calculated for the courses, but not for the general rank of the student.<br />This link makes it possible to force the recalculation of the ranks for
the courses as for general rank.<br />The recalculation will be carried out at the time of the next display of bulletin or averages.</p>";
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td><input type='checkbox' name='forcer_recalcul_rang' id='forcer_recalcul_rang' value='y' /><label for='forcer_recalcul_rang'>Force the recalculation of the ranks</label> <a href='#' onclick="afficher_div('recalcul_rang','y',-100,20);return false;"><img src='../images/icons/ico_ampoule.png' width='15' height='25' alt='Force the recalculation of the ranks' title='Force the recalculation of the ranks' /></a>.</td>
</tr>
</table>
...
<tr>
	...
	<!--td style="font-weight: bold;"-->
	<td>
	Display course rubrics on the bulletin (HTML),<br />report booklet (HTML), and tools for visualization&nbsp;:
	</td>
	<td>
	<?php
		//echo "<input type='checkbox' value='y' name='display_mat_cat_".$per."' />\n";
		echo "<input type='radio' value='y' name='display_mat_cat_".$per."' />Yes\n";
		echo "<input type='radio' value='n' name='display_mat_cat_".$per."' />No\n";
	?>
	</td>
</tr>
<tr>
	...
	<td valign="top">
	Parameter setting of the categories of course for this class<br />
	(<i>the consideration of this parameter is conditioned<br />
	by the fact of check the box<br />
	'Display course rubrics...' above</i>)
	</td>
	<td>
		<table style='border: 1px solid black;'>
		<tr>
			<td style='width: auto;'>Category</td><td style='width: 100px; text-align: center;'>Priority of display</td><td style='width: 100px; text-align: center;'>Display the average on the bulletin</td>
		</tr>
		<?php
		$get_cat = mysql_query("SELECT id, nom_court, priority FROM matieres_categories");
		while ($row = mysql_fetch_array($get_cat, MYSQL_ASSOC)) {
	...
	<tr>
	<td colspan='3'>
	<h2><b>Parameters of HTML bulletin &nbsp;: </b></h2>
	</td>
	<td>
	</td>
	</tr>
	...
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td valign="top">
		Display on the bulletin the rank of each student&nbsp;: 
	</td>
	<td valign="bottom">
		<input type="radio" name="<?php echo "display_rang_".$per; ?>" value="y" />Yes	
		<input type="radio" name="<?php echo "display_rang_".$per; ?>" value="n" />No
	</td>
	</tr>
	...
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td valign="top">
	Display the block addresses of the responsible of the student&nbsp;: 
	</td>
	<td valign="bottom">
		<input type="radio" name="<?php echo "display_address_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "display_address_".$per; ?>" value="n" />No
	</td>
	</tr>
	...
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td valign="top">
	Display coefficients of the courses<br />(<i>only if at least a coef is different from 0</i>)&nbsp;: 
	</td>
	<td valign="bottom">
		<input type="radio" name="<?php echo "display_coef_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "display_coef_".$per; ?>" value="n" />No
	</td>
	</tr>
	...
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td valign="top">
	Display general averages on the bulletins<br />(<i>only if at least a coef different from 0</i>)&nbsp;: 
	</td>
	<td valign="bottom">
		<input type="radio" name="<?php echo "display_moy_gen_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "display_moy_gen_".$per; ?>" value="n" />No
	</td>
	</tr>
	...
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td valign="top">
	Display on the bulletin the number of exams&nbsp;: 
	</td>
	<td valign="bottom">
		<input type="radio" name="<?php echo "display_nbdev_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "display_nbdev_".$per; ?>" value="n" />No
	</td>
	</tr>
	<tr>
	<td colspan='3'>
	<h2><b>Parameters of PDF bulletin &nbsp;: </b></h2>
	</td>
	<td>
	</td>
	</tr>
	...
	Select the model of bulletin for the impression in pdf&nbsp;:
	</td>
	<td><?php
...
<tr>
	<td colspan='3'>
	<h2><b>Parameters of reports booklet&nbsp;: </b></h2>
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Display the name of the exams&nbsp;:</td>
	<td>
		<input type="radio" name="<?php echo "rn_nomdev_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "rn_nomdev_".$per; ?>" value="n" />No
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Display all coefficients of the exams&nbsp;:</td>
	<td>
		<input type="radio" name="<?php echo "rn_toutcoefdev_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "rn_toutcoefdev_".$per; ?>" value="n" />No
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Display coefficients of the exams if different coefficients are present&nbsp;:</td>
	<td>
		<input type="radio" name="<?php echo "rn_coefdev_si_diff_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "rn_coefdev_si_diff_".$per; ?>" value="n" />No
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Display exams dates&nbsp;:</td>
	<td>
		<input type="radio" name="<?php echo "rn_datedev_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "rn_datedev_".$per; ?>" value="n" />No
	</td>
</tr>

<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Formula/Message to be inserted under the report booklet&nbsp;:</td>
	<td><input type=text size=40 name="rn_formule_<?php echo $per;?>" value="" /></td>
</tr>

<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Display a box for the signature of the head of school&nbsp;:</td>
	<td>
		<input type="radio" name="<?php echo "rn_sign_chefetab_".$per; ?>" value="y" />Yes	
		<input type="radio" name="<?php echo "rn_sign_chefetab_".$per; ?>" value="n" />No
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Display a box for the signature of the principal teacher&nbsp;:</td>
	<td>
		<input type="radio" name="<?php echo "rn_sign_pp_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "rn_sign_pp_".$per; ?>" value="n" />No
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Display a box for the signature of the parents/responsibles&nbsp;:</td>
	<td>
		<input type="radio" name="<?php echo "rn_sign_resp_".$per; ?>" value="y" />Yes
		<input type="radio" name="<?php echo "rn_sign_resp_".$per; ?>" value="n" />No
	</td>
</tr>

<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Number of lines for the signature&nbsp;:</td>
...
<tr>
	<td colspan='3'>
	  <h2><b>Parameters of ECTS certificates &nbsp;: </b></h2>
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">Type of formation (ex: "Scientific preparatory class")&nbsp;:</td>
    <td><input type="text" size="40" name="ects_type_formation_<?php echo $per;?>" value="" /></td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">Complete name of the formation (ex: "BCPST (Biology, Chemistry, Physics and Sciences of the Earth)")&nbsp;:</td>
    <td><input type="text" size="40" name="ects_parcours_<?php echo $per;?>" value="" /></td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">Short name of the formation (ex: "BCPST")&nbsp;:</td>
    <td><input type="text" size="40" name="ects_code_parcours_<?php echo $per;?>" value="" /></td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">Fields of study (ex: "Biology, Chemistry, Physics, Mathematics, Sciences of the Earth")&nbsp;:</td>
...
<center><input type='submit' name='enregistrer2' value='Save' /></center>
