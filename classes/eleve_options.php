...
if($_SESSION['statut']=="scolarite") {
	...
	if ($test == "0") {
		tentative_intrusion("2", "Attempt at access by an account schooling to a class with which it is not associated.");
		echo "You cannot access to this class because you are not referent there !";
			...
			while ($i < $nombre_ligne) {
				...
				if (isset($_POST[$id_group[$j]])) {
					...
					if (($nb_test1 != 0) or ($nb_test2 != 0)) {
						$msg = $msg."--> Impossible to remove this option for the student $login_eleve because of the averages or appreciations already be typed for the group $nom_groupe for the period $j ! Start by removing these data!<br />";
					} else {
						if ($test != "0") {
...
//**************** EN-TETE **************************************
$titre_page = "Management of the classes | Management of the courses by student";
...
if(!isset($quitter_la_page)) {
	...
	if($_SESSION['statut']=="administrateur"){
		echo "<p class=bold><a href=\"#\" onclick=\"confirm_close (this, change, '$themessage');\">Close the page</a></p>\n";
	}
	else{
		echo "<p class=bold><a href=\"#\" onclick=\"self.close();\">Close the page</a></p>\n";
	}
...
if($_SESSION['statut']=="administrateur"){
	...
	echo "<p>To validate the modifications, click on the button.</p>\n";

	echo "<p align='center'><input type='submit' value='Save modifications' /></p>\n";
}
...
while ($j < $nb_periode) {
	...
	if($_SESSION['statut']=="administrateur"){
		...
		echo "<a href='javascript:modif_case($j,\"col\",true)'><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a>/\n";
		echo "<a href='javascript:modif_case($j,\"col\",false)'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
	}
	echo "</th>\n";
...
if($_SESSION['statut']=="administrateur"){
	echo "<th>\n";
	echo "<a href='javascript:$chaine_coche'><img src='../images/enabled.png' width='15' height='15' alt='Check all /></a>/\n";
	echo "<a href='javascript:$chaine_decoche'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
	echo "</th>\n";
}
...
while ($i < $nombre_ligne) {
		...
		if(mysql_num_rows($res_test_class_per)==0){
			if (mysql_num_rows($test) == "0") {
				...
				if($temoin!="") {
					...
					if($_SESSION['statut']=="administrateur") {
						$msg_erreur="This box is validated and should not be. Validate the form to correct.";
						echo "<a href='#' alt='$msg_erreur' title='$msg_erreur'><font color='red'>ERROR</font></a>";
					}
					else{
						$msg_erreur="This box is validated and should not be. Contact the administrator to correct.";
						echo "<a href='#' alt='$msg_erreur' title='$msg_erreur'><font color='red'>ERROR</font></a>";
					}
					$nb_erreurs++;
				}
				...
				if (!test_before_eleve_removal($login_eleve, $id_groupe, $j)) {
					echo "<img id='img_bull_non_vide_".$j."_".$i."' src='../images/icons/bulletin_16.png' width='16' height='16' title='Nonempty bulletin' alt='Nonempty bulletin' />";
				}
				...
				if($nb_notes_cn>0) {
					echo "<img id='img_cn_non_vide_".$j."_".$i."' src='../images/icons/cn_16.png' width='16' height='16' title='Nonempty report card: $nb_notes_cn notes' alt='Nonempty report card: $nb_notes_cn notes' />";
					//echo "$sql<br />";
				}
	...
	if($_SESSION['statut']=="administrateur"){
		...
		echo "<a href='javascript:modif_case($i,\"lig\",true)'><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a>/\n";
		echo "<a href='javascript:modif_case($i,\"lig\",false)'><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>\n";
		echo "</td>\n";
	}
	//=========================
	echo "</tr>\n";
...
if($nb_erreurs>0){
	echo "<p style='color:red;'>This student is affected in groups over periods for which it is not in the class.<br />";
	if($_SESSION['statut']=="administrateur"){
		echo "To remove the student of these groups, validate this form.";
	}
	else{
		echo "Contact the administrator for correct.";
	}
	echo "</p>\n";
}
//============================================
...
if($_SESSION['statut']=="administrateur"){
?>
	<p align='center'><input type='submit' value='Save modifications' /></p>
