...
if (isset($is_posted)) {
    ...
    while ($i < $nombre_lignes) {
        ...
        if (isset($_POST[$reg_matiere]) and ($_POST[$reg_matiere] == "yes")) {
            ...
            ");
		 updateOnline("DELETE FROM j_classes_matieres_professeurs
            WHERE
            (
            id_classe='$id_classe' and
            id_matiere='$reg_matiere'
            ...
            While ($m < $max) {
                if ((isset($prof[$m])) and ($prof[$m] != '')) {
                    ...
                    if ($nb == 0) {
						...
						updateOnline("INSERT INTO j_classes_matieres_professeurs SET id_professeur='$prof[$m]', priorite='$priority', id_classe='$id_classe', id_matiere='$reg_matiere', ordre_prof='$compteur', coef='$coef', recalcul_rang='y'");                       
					   $compteur++;
                    }
                    $prof[$m] = '';
                }
            ...
            if ($nb != 0) {
                ...
                if ($suppression == 'impossible') {
                    $msg = $msg."--> Impossible to remove the course ".$reg_matiere." because averages or appreciations has already be typed !<br />";
                    $message_enregistrement = "Impossible to remove one or more courses : read the message in red in top of the page.";
                    ...
                    while ($k < $nb_eleves) {
                        ...
                        ");
						updateOnline("DELETE FROM j_eleves_matieres
                        WHERE
                        (
                        login = '".$login_eleve[$k]."' and
                        matiere='$reg_matiere'
                    ...
                    while ($m < $nb_prof) {
                        ...
                        if (mysql_num_rows($test4) == 0)
							...
							updateOnline("delete from j_eleves_professeurs
                            where (
                            id_classe='".$id_classe."' and
                            professeur = '".$login_prof."'
                            )");
        ...
        if (isset($_POST['force_defaut'])) {
            ...
            ");
			updateOnline("UPDATE j_classes_matieres_professeurs SET priorite='".$priority_defaut."'
            WHERE (
            id_classe='".$id_classe."' AND
            id_matiere='".$reg_matiere."'
            )
    ...
    if ($mess_avertissement == 'yes') {$msg .="CAUTION : Certain courses are checked and do not have affected professors<br />The recording of a course is done if at least a professor was affected.<br>";}
    if ($mess_avertissement2 == 'yes') {$msg .="CAUTION : you tried to remove a course by removing all the associated professors whereas notes or appreciations attached to this course in this class already exist.<br>";}
}

//**************** EN-TETE **************************************
$titre_page = "Management of the classes | Management of the courses";
...
<?php
$themessage = "Changes have been done on this page and were not recorded. If you click on OK the changes will be lost.";
...
if ($test_prof_mat==0) {
    echo "<p class='grand'>No assignment professor<->course is available !</p>";
    echo "<p>You must initially define professors and affect courses to them !</p>";
    die();
}
...
if (($test1 != 0) and ($test2 != 0)) {
    ?>
    |<a href="init_options.php?id_classe=<?echo $id_classe?>">Initialization of the Options from Gep</a>
    <?php
}
echo "|<input type=\"submit\" onclick=\"return VerifChargement()\" name=\"Envoyer\" value=\"Save modifications\">";
echo "|</p>";

$call_nom_class = mysql_query("SELECT classe FROM classes WHERE id = '$id_classe'");
$classe = mysql_result($call_nom_class, 0, 'classe');
echo "<H3>Class : $classe</H3>";

echo "<b>For all the courses, force the values of priority of display to the default values :</b>
...
?>
<center><input type=submit value="Save modifications"></center></p>
