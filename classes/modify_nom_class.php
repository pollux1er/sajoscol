...
if (isset($is_posted) and ($is_posted == '1')) {
	...
	if (isset($id_classe)) {
		if ($reg_class_name) {
		 ...
		 updateOnline("UPDATE classes SET classe='$reg_class_name',
													nom_complet='$reg_nom_complet',
													suivi_par='$reg_suivi_par',
													formule= '".html_entity_decode($reg_formule)."',
													format_nom='$reg_format',
			...
			if (!$register_class) {
					$msg .= "An error occurred during the modification of the class.";
					} else {
					$msg .= "The class was indeed modified.";
			}
		...
		$msg .= "Please specify the name of the class !";
		}
	} else {
		if ($reg_class_name) {
		...
		$register_class = mysql_query("INSERT INTO classes SET classe = '$reg_class_name',
                                                    ...
                                                    ects_fonction_signataire_attestation='".$ects_fonction_signataire_attestation."'
												");
		 updateOnline("INSERT INTO classes SET classe = '$reg_class_name',
													nom_complet = '$reg_nom_complet',
													suivi_par = '$reg_suivi_par',
													formule = '$reg_formule',
													format_nom = '$reg_format',
												...
												");
		if (!$register_class) {
			$msg .= "An error occurred during the recording of the new class.";
		} else {
			$msg .= "The new class was indeed recorded.";
...
//**************** EN-TETE *******************************
$titre_page = "Management of the classes | modify the parameters ";
...
if(isset($id_classe)) {
	$titre="Navigation";
	$texte="";
	$texte.="<img src='../images/icons/date.png' alt='' /> <a href='periodes.php?id_classe=$id_classe' onclick=\"return confirm_abandon (this, change, '$themessage')\">Periods</a><br />";
...
if(getSettingValue('GepiAdminImprBulSettings')!='yes') {
	echo "<p><b>Remarque&nbsp;: </b>Connect with an account having the statute \"schooling\" to edit the bulletins and to have access to other parameters of
display.</p>\n";
}
...
if (isset($id_classe)) {
	...
	if(mysql_num_rows($call_nom_class)==0) {
		echo "<p>The identifier of class '$id_classe' is unknown.</p>\n";
...
?>
<p>Short name of the class&nbsp;: <input type=text size=30 name=reg_class_name value = "<?php echo $classe; ?>" onchange='changement()' /></p>
<p>Complete name of the class&nbsp;: <input type=text size=50 name=reg_nom_complet value = "<?php echo $nom_complet; ?>"  onchange='changement()' /></p>
<p>First name and name of the signatory of the bulletins<?php if ($gepiSettings['active_mod_ects'] == "y") echo " et des attestations ECTS" ?> (head of school or his representative)&nbsp;: <br /><input type=text size=30 name=reg_suivi_par value = "<?php echo $suivi_par; ?>"  onchange='changement()' /></p>
<?php
if ($gepiSettings['active_mod_ects'] == "y") {
    ?>
<p>Function of the above-named signatory (ex.: "Headmaster")&nbsp;: <br /><input type="text" size="40" name="ects_fonction_signataire_attestation" value="<?php echo $ects_fonction_signataire_attestation;?>" onchange='changement()' /></p>
<?php
}
    ?>
<p>Formulate to insert on the bulletins (this formula will be followed by the name and first name of the designated person above&nbsp;:<br /> <input type=text size=80 name=reg_formule value = "<?php echo $formule; ?>"  onchange='changement()' /></p>

<p><b>Formatting of the identity of the professors for the bulletins&nbsp;:</b>
<br /><br />
...
<label for='reg_format_np' style='cursor: pointer;'>Name First name (Durand Albert)</label>
<br />
...
<label for='reg_format_pn' style='cursor: pointer;'>First name Name (Albert Durand)</label>
<br />
...
<label for='reg_format_in' style='cursor: pointer;'>Initial-first name Name (A. Durand)</label>
<br />
...
<label for='reg_format_ni' style='cursor: pointer;'>Initial-first name Name (Durand A.)</label>
<br />
...
<label for='reg_format_cnp' style='cursor: pointer;'>Civility Name First name (M. Durand Albert)</label>
<br />
...
<label for='reg_format_cpn' style='cursor: pointer;'>Civility First name Name (M. Albert Durand)</label>
<br />
...
<label for='reg_format_cin' style='cursor: pointer;'>Civ. Initial-First name Name (M. A. Durand)</label>
<br />
...
<label for='reg_format_cni' style='cursor: pointer;'>Civ. Name initial-First name  (M. Durand A.)</label>
...
<tr>
    ...
    <label for='display_mat_cat' style='cursor: pointer;'>Display categories of courses on the bulletin (HTML), report booklets(HTML), and tools for visualization&nbsp;:</label>
    </td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">
	Parameter setting of categories of course for this class (only if box above is checked )
	</td>
	<td>
...
<tr>
    ...
    <label for='display_rang' style='cursor: pointer;'>Display on the bulletin the rank of each student&nbsp;:</label>
    </td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
    <label for='display_address' style='cursor: pointer;'>Display the block addresses of the responsable of the student&nbsp;:</label>
    </td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
    <label for='display_coef' style='cursor: pointer;'>Display coefficients of the courses (only if at least a coef is different from 0)&nbsp;:</label>
    </td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
    <label for='display_moy_gen' style='cursor: pointer;'>Display general averages on the bulletins (only if at least a coef different from 0)&nbsp;:</label>
    </td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
    <label for='display_nbdev' style='cursor: pointer;'>Display the number of exams on the bulletin&nbsp;:</label>
    </td>
...
<tr>
	<td colspan='3'>
	  <h2><b>Parameters PDF bulletin  &nbsp;: </b></h2>
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	...
	<td><?php
		...
		if(mysql_num_rows($requete_modele)==0) {
			...
			if($_SESSION['login']=='administrateur') {
				echo "You should carry out/to force an <a href='../utilitaires/maj.php'>update of the base</a> to correct.<br />Check that nobody other is connected.\n";
			}
			else {
				echo "Contact the administrator so that it carries out an update of the base.\n";
			}
			echo "</p>\n";
		}
		else {
			...
			if ($quel_modele == NULL) {
			echo "<option value=\"NULL\" selected=\"selected\" >No model selected</option>";
			}
...
<tr>
	...
	<td style="font-variant: small-caps; vertical-align: top;">
	   <?php echo ucfirst($gepi_denom_mention);?>s being able to appear in the opinion of the staff meeting on the bulletins&nbsp;:
	</td>
	<td><?php
		...
		if(mysql_num_rows($res)==0) {
			echo "<p>No $gepi_denom_mention is defined for this class.</p>\n";
		}
		else {
			echo "<ol>\n";
		...
		echo "<p><a href='../saisie/saisie_mentions.php' onclick=\"return confirm_abandon (this, change, '$themessage')\">Parameterize the ".$gepi_denom_mention."s</a></p>\n";
		?>
	</td>
</tr>
<!-- ========================================= -->
<tr>
	<td colspan='3'>
	  <h2><b>Parameters of the report booklet&nbsp;: </b></h2>
	</td>
</tr>
<!--
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
	<label for='rn_nomdev' style='cursor: pointer;'>Display the name of the exams&nbsp;:</label></td>
    <td><input type="checkbox" value="y" name="rn_nomdev" id="rn_nomdev"  <?php   if ($rn_nomdev=="y") echo " checked "; ?> onchange='changement()' /></td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
	<label for='rn_toutcoefdev' style='cursor: pointer;'>Display all coefficients of the exams&nbsp;:</label></td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
	<label for='rn_coefdev_si_diff' style='cursor: pointer;'>Display coefficients of the exams if different coefficients are present&nbsp;:</label></td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
	<label for='rn_datedev' style='cursor: pointer;'>Display dates of the exams&nbsp;:</label></td>
    <td><input type="checkbox" value="y" name="rn_datedev" id="rn_datedev"  <?php   if ($rn_datedev=="y") echo " checked "; ?> onchange='changement()' /></td>
</tr>

<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
	<td style="font-variant: small-caps;">Formula/Message to insert under the report booklet&nbsp;:</td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
	<label for='rn_sign_chefetab' style='cursor: pointer;'>Display a box for the signature of the head of school&nbsp;:</label></td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
	<label for='rn_sign_pp' style='cursor: pointer;'>Display a box for the signature of the principal teacher&nbsp;:</label></td>
    <td><input type="checkbox" value="y" name="rn_sign_pp" id="rn_sign_pp"  <?php   if ($rn_sign_pp=="y") echo " checked "; ?> onchange='changement()' /></td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">
	<label for='rn_sign_resp' style='cursor: pointer;'>Display a box for the signature of the parents/responsibles&nbsp;:</label></td>
...
<tr>
	<td colspan='3'>
	  <h2><b>Parameters of certificates ECTS&nbsp;: </b></h2>
	</td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
...
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">Complete name of the course of formation (ex: "BCPSE (Biology, Chemistry, Physics and Sciences of the Earth)")&nbsp;:</td>
    <td><input type="text" size="40" name="ects_parcours" value="<?php echo $ects_parcours;?>" onchange='changement()' /></td>
</tr>
<tr>
	<td>&nbsp;&nbsp;&nbsp;</td>
    <td style="font-variant: small-caps;">Short name of formation (ex: "BCPST")&nbsp;:</td>
...
<center><input type=submit value="Save" style="margin-top: 30px; margin-bottom: 100px;" /></center>
</form>
<?php
