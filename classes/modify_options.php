    ...
    while($i < $nombre_lignes) {
        ...
        while ($j < $nb_periode) {
            ...
            if ($nb_ligne != 0) {
                ...
                updateOnline("DELETE FROM j_eleves_matieres WHERE (matiere='$current_matiere' and login='$current_eleve_login' and periode = '$j') ");
			   } else {
                    ...
                    if ($nb_test == 0) {
                        ...
                        if (($nb_test1 != 0) or ($nb_test2 != 0)) {
                            $msg = $msg."--> Impossible to remove this option for the student $current_eleve_login because the averages or appreciations already be typed in $current_matiere for the period $j ! Start by removing these data!<br />";
                        } else {
                            $reg = mysql_query("INSERT INTO j_eleves_matieres SET matiere='$current_matiere' , login='$current_eleve_login', periode='$j'");
                       updateOnline("INSERT INTO j_eleves_matieres SET matiere='$current_matiere' , login='$current_eleve_login', periode='$j'");
					   }
                    }
                }
            }
...
//**************** EN-TETE *****************
$titre_page = "Management of the classes | Modification of the options ";
...
<p class=bold><a href="modify_class.php?id_classe=<?echo $id_classe?>"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return </a>|<a href="help.php"> Help </a>| <input type='submit' value='Save' />
</p>
<b><a href="javascript:CocheCase(true)">Check all</a> - <a href="javascript:CocheCase(false)">Uncheck all</a></b>

<?php
echo add_token_field();
echo "<p class='grand'>Class : $classe | Course : $current_matiere</p>";
?>

<p>
<table border=0 cellspacing=2 cellpadding=5>
<tr>
    <td><p><b>Name First name</b></p></td>
    <?php
    $i="1";
    while ($i < $nb_periode) {
...
$nombre_lignes = mysql_num_rows($appel_donnees_eleves);
    ...
    $k = 1;
        ...
        while ($k < $nb_periode) {
            ...
            echo "<a href=\"javascript:CochePeriode($elements[$k])\">Tout</a> <br/> <a href=\"javascript:DecochePeriode($elements[$k])\">None</a></td>";
...
while($i < $nombre_lignes) {
    ...
    echo "<td><a href=\"javascript:CochePeriode($elementlist)\">Tout</a> // <a href=\"javascript:DecochePeriode($elementlist)\">None</a></td>";
