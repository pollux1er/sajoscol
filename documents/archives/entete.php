...
if((isset($chaine_login_prof))&&($chaine_login_prof!='')) {
	...
	if(($_SESSION["statut"]=="professeur")&&(!in_array("'".strtolower($_SESSION["login"])."'",$tab_login))) {
		if(getSettingValue('debug_acces_archives_cdt')=='y') {
			echo "<p style='color:red'>The login ".$_SESSION["login"]." is not in the authorized list ".$chaine_login_prof."</p>\n";
		}
		else {
			header("Location: $pref_arbo_include/logout.php?auto=1");
		}
...
if(((isset($multisite))&&($multisite=='y'))||(getSettingValue('multisite')=='y')) {
	if(!preg_match('|documents/archives/'.$_COOKIE['RNE'].'/|',$_SERVER['SCRIPT_FILENAME'])) {
		...
		header("Location: $pref_arbo_include/logout.php?auto=1&msg=".rawurlencode("Attempt of unauthorized access to another school"));
		die();
	}
}
else {
...
function corriger_div_lien_retour() {
	...
	if(t.substring(0,16)=='CDT: Professeur ') {
		if(document.getElementById('div_lien_retour')) {
			document.getElementById('div_lien_retour').innerHTML='<a href=\'$gepiPath/documents/archives/index.php\'>Return</a>';
		}
	}
}
