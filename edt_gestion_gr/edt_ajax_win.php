...
if ($action == 'modifier') {
	...
	$query_maj = mysql_query($sql_maj)
										OR trigger_error('Impossible to modify the class', E_USER_ERROR);
	...
	echo '
		<select name="">
			<option value="rien">Type of subdivision</option>
			<option value="autre" onclick="classeEdtAjax(\''.$id_gr.'\', \'autre\', \'type_en\');">Several classes</option>
			<option value="classe" onclick="classeEdtAjax(\''.$id_gr.'\', \'classe\', \'type_en\');">A class (ent.)</option>
			<option value="demi" onclick="classeEdtAjax(\''.$id_gr.'\', \'demi\', \'type_en\');">Subdivision of a class</option>
		</select>';
	...
	if ($query_type) {
		...
		echo '
		<p onclick="classeEdtAjax(\'id2_'.$id2.'\', \''.$type.'\', \'type\');" title="modify the type">'.$type.'&nbsp;
			<span style="color: green; font-size:0.5em;">(ok !)</span></p>';
	}else{
		echo '
		<p title="'.$sql_type.'" style="color: red;">Error !</p>';
	}
	...
	if (is_numeric($id3)) {
		...
		if ($query_nom) {
			echo '
			<p onclick="classeEdtAjax(\'id00_'.$id3.'\', \''.$nom.'\', \'nom_gr\');" title="modify the name">'.$nom.'&nbsp;
				<span style="color: green; font-size:0.5em;">(ok !)</span></p>';
		}else{

			echo '
			<p title="'.$sql_nom.'" style="color: red;">Error !</p>';
	...
	if (is_numeric($id4)) {
		...
		if ($query_nom_lg) {
			echo '
		<p onclick="classeEdtAjax(\'id000_'.$id4.'\', \''.$nom_lg.'\', \'nom_gr2\');" title="modify the long name">'.$nom_lg.'&nbsp;
			<span style="color: green; font-size:0.5em;">(ok !)</span></p>';
		}else{
			echo '
			<p title="'.$sql_nom_lg.'" style="color: red;">Error !</p>';
		}
