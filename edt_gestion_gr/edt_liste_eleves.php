...
$titre_page = "manage the groups of EdT<br />Elèves";
...
$query_i = mysql_query("SELECT * FROM edt_gr_nom WHERE id = '".$id_gr."'") OR trigger_error('This group does not exist !', E_USER_ERROR);
...
if ($id_eleve != NULL AND $action != 'del_eleve_gr') {
	...
	if ($id_eleve == "tous") {
		...
		echo 'This functionality is not yet ready, sorry';
		...
		if (is_numeric($id_eleve)) {
			...
			$query_el = mysql_query($sql_el) OR trigger_error('Impossible to record this student.', E_USER_ERROR);
			//echo $sql_el;
...
elseif($action == "del_eleve_gr"){
	...
	if (is_numeric($id_eleve) AND is_numeric($id_gr)) {
		...
		$query_del = mysql_query($sql_del) OR trigger_error('Impossible to erase this student !', E_USER_ERROR);
		...
		$aff_classes_g .= "
			<tr>
				<td style=\"width: 196px;\">
					<a href=\"./edt_liste_eleves.php?id_gr=".$id_gr."&amp;cla=".$liste_classe[$a]["id"]."\">Students of ".$liste_classe[$a]["classe"]."</a>
				</td>
			</tr>\n";
	}
...
if ($classe_e AND is_numeric($classe_e) AND $rep_gr["subdivision_type"] != "classe") {
	...
	<table class=\"aid_tableau\" summary=\"List of students\">
	";
		...
		<tr class=\"".$aff_tr_css."\">
			<td>
				...
				<img src=\"../images/icons/add_user.png\" alt=\"Add\" title=\"Add\" /> Whole the class
				</a>
			</td>
		</tr>
		<tr>
	...
	for($b=0; $b<$nbre_ele_m; $b++) {
		$aff_ele_m[$b]["login"] = mysql_result($req_ele, $b, "login") OR DIE('Request error liste_eleves : '.mysql_error());
	...
	$aff_classes_m = '
		<p>You cannot modify the list of the students of this group because it corresponds to the manpower of the class</p>
		<p>If you want to modify this list, you must initially modify the type of the group in the previous page</p>';
}
...
if ($rep_gr["subdivision_type"] == 'classe') {
	...
	$sql_e = "SELECT DISTINCT e.nom, e.prenom, e.login, e.id_eleve, c.classe, c.id FROM eleves e, j_eleves_classes jec, classes c
									...
									ORDER BY nom, prenom";
	$req_ele_gr = mysql_query($sql_e) OR trigger_error('Impossible to recover the list of the students', E_USER_ERROR);
	...
	$req_ele_gr = mysql_query("SELECT DISTINCT ege.id_eleve, e.nom, e.prenom, e.login, c.classe, c.id
									...
									OR trigger_error('Impossible to recover the list of the students: '.mysql_error(), E_USER_ERROR);
	...
	while($rep_ele_gr = mysql_fetch_array($req_ele_gr)){
		...
		$aff_gr_d .= "<br />
			<a href='./edt_liste_eleves.php?id_eleve=".$rep_ele_gr["id_eleve"]."&amp;id_gr=".$id_gr."&amp;cla=".$rep_ele_gr["id"]."&amp;action=del_eleve_gr'>
				<img src=\"../images/icons/delete.png\" title=\"Delete this student\" alt=\"Delete\" />
...
?>
<p onclick="window.opener.location.href='./edt_win.php?var=<?php echo $id_gr; ?>&amp;var2=liste_e'; window.close();" style="cursor: pointer;">CLOSE</p>
...
<div id="edt_gr_g">
	<table>
		<thead><tr><th>List of classes</th></tr></thead>
