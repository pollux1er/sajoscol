...
$titre_page = "Manage the groups of EdT<br />Professors";
...
// ============================ Traitement des données ========================== //
	...
	$query_t = mysql_query($sql_t) OR trigger_error('Error during the treatment of this edt_gr.', E_USER_ERROR);
	$rep = mysql_fetch_array($query_t);

	$titre .= '<p>EDT : '.$rep["nom"].'&nbsp;('.$rep["nom_long"].')&nbsp;-&nbsp;List of professors.</p>';
...
$query_p = mysql_query("SELECT login, nom, prenom FROM utilisateurs WHERE statut = 'professeur' AND etat = 'actif' ORDER BY nom, prenom")
						OR trigger_error('Impossible to read the list of professors.', E_USER_ERROR);
	...
	if ($action == "ajouter") {
		...
		if ($test1) {
			...
			if ($test2 AND is_numeric($test2) AND $test2 >= 1) {
				...
				$msg .= '<p style="color: red;">This professor was not recorded because he is already member of this edt_gr.</p>';
				...
				if ($query_insert) {
					...
					$msg .= '<p style="color: green;">This professor was indeed recorded.</p>';
					...
					if ($query_insert) {
						...
						$msg .= '<p style="color: green;">This professor was not recorded : '.$sql_insert.'</p>';
			...
			$msg .= '<p style="color: red;">This professor was not recorded because he does not exist in the base.</p>';
	...
	if ($action == "effacer") {
		...
		$query_del = mysql_query($sql_del) OR trigger_error('Impossible to remove this professor : '.$sql_del, E_USER_WARNING);

		if ($query_del) {

			$msg .= '<p style="color: green;">This professor was erase of this edt_gr.</p>';

		}else{

			$msg .= '<p style="color: red;">Impossible to erase this professor of this edt_gr.</p>';
	...
	$sql_l = "SELECT login, nom, prenom FROM utilisateurs u, edt_gr_profs egp
										...
										ORDER BY nom, prenom";
	$query_l = mysql_query($sql_l) OR trigger_error('Impossible to list the professors of this group', E_USER_WARNING);
	...
	while($rep = mysql_fetch_array($query_l)){
		...
		<img src="../images/icons/delete.png" alt="Erase a professor : '.$rep["nom"].'" />'.$rep["nom"].' '.$rep["prenom"].'</a>'."\n";
...
?>
<p onclick="window.opener.location.href='./edt_win.php?var=<?php echo $id_gr; ?>&amp;var2=liste_p'; window.close();" style="cursor: pointer;">CLOSE</p>
