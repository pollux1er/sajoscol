...
if ($var2 == "changer_nom") {
	...
	if (isset($var) AND is_numeric($var)) {
		...
		<legend>&nbsp;Modify a group of students for EdT&nbsp;</legend>
		...
		<form name="ajout" action="edt_win.php" method="post">
			...
			<label for="nomGr" title="Such as it must appear in EdT">Name</label>
			...
			<input type="submit" name="enregistrer" value="Save the modifications" />
	...
	';
		...
		$aff_modif = '<p>Impossible to recover the data of this group.</p>';
	}
	...
	$aff_modif .= '<p style="text-align: right;"><a href="edt_liste_eleves.php?id_gr='.$var.'" target="_blank">Modify this list</a></p>';

	$query_e = mysql_query($sql_e) OR trigger_error('Impossible to recover the list of the students', E_USER_ERROR);
	...
	$sql_p = "SELECT login, nom, prenom FROM edt_gr_profs egp, utilisateurs u
										...
										AND id_gr_nom = '".$var."'
										ORDER BY nom, prenom";
	$query_p = mysql_query($sql_p) OR trigger_error("Impossible to recover the list of the professors of this group : ", E_USER_ERROR);

	$aff_modif .= '<p style="text-align: right;"><a href="./edt_liste_profs.php?id_gr='.$var.'" target="_blank">Modify this list</a></p>';
...
if ($action == "modifier_gr") {
	...
	if ($query_m) {
		// On ferme la fenêtre
		echo '<html><body><p>The modification was indeed recorded, you can close this window and refresh your navigator.</p></body></html>';
