...
// header
$titre_page = "Definition of the time crenels";
...
// On commence par Save le réglage sur le jour où les créneaux sont différents si c'est demandé
...
if ($demande_jour_semaine == "ok_diff") {
	...
	if ($jour_semaine != getSettingValue("creneau_different")) {
		...
		updateOnline("UPDATE setting SET value = '".$jour_semaine."' WHERE name = 'creneau_different'");
...
if ($cren == "diff") {
	$cherche_table = '';
	$texte_lien = ' See the crenels of all days';
}else{
	$cherche_table = '&amp;cren=diff';
	$texte_lien = ' See the crenels of the different day';
}
...
if ($action == "visualiser") {
	if ($cren == "diff") {
		...
		echo '<p>You must choose only one day when the crenels are different from the
other days (like Wednesday...)</p>';
...
if ($compter >= 1) {
	echo "<p class=\"red\">Attention, if you modify the crenels now, the courses of the timetable will be disturbed !</p>";
}
	/* div de centrage du tableau pour ie5 */
?>
<div style="text-align: center;">
<h2>Definition of the time crenels</h2>
	<p>
		<a href="admin_periodes_absences.php?action=ajouter<?php echo $aff_creneau_diff; ?>">
		<img src='../images/icons/add.png' alt='' class='back_link' /> Add a time crenel
		</a>
	</p><br />
	...
	<table cellpadding="0" cellspacing="1" class="tab_table" summary="Créneaux">
		<tr>
			...
			<th class="tab_th" style="width: 90px;">hour of beginning</th>
			<th class="tab_th" style="width: 90px;">hour of end</th>
...
<?php
	...
	?>
        <tr class="<?php echo $couleur_cellule; ?>">
          ...
          <td><a href="admin_periodes_absences.php?action=visualiser<?php echo $aff_creneau_diff; ?>&amp;action_sql=supprimer&amp;id_periode=<?php echo $data_periode['id_definie_periode']; ?>" onClick="return confirm('Are you sure you want to remove this crenel ?')"><img src="../templates/DefaultEDT/images/delete2.png" width="22" height="22" title="Delete" border="0" alt="Delete" /></a></td>
        </tr>
     <?php } ?>
    </table>
...
<div style="text-align:center">
  <?php if ($action == "ajouter") { ?>
	...
	<h2>Addition of time crenels</h2>
    ...
    <form name="form1" method="post" action="admin_periodes_absences.php?action=ajouter<?php echo $aff_creneau_diff; ?>">
      <table class="tab_table" summary="Créneaux">
        <tr>
          <th class="tab_th">Number of time crenels to add</th > </tr >
        <tr>
    ...
    <form action="admin_periodes_absences.php?action=visualiser<?php echo $aff_creneau_diff; ?>&amp;action_sql=<?php if($action=="ajouter") { ?>ajouter<?php } if($action=="modifier") { ?>modifier<?php } ?>" method="post" name="form2" id="form2">
      <table cellpadding="2" cellspacing="2" class="tab_table" summary="Créneaux">
        <tr>
          ...
          <th class="tab_th">Hour of beginning</th>
          <th class="tab_th">Hour of end</th>
          <th class="tab_th">Type</th>
          <th class="tab_th">Logical continuation</th>
        </tr>
        <?php
        $i = '1';
        $nb = 0;
        ...
        <?php if (isset($verification_erreur[$nb]) and $verification_erreur[$nb] != 1) { ?>
         <tr>
          ...
          <td colspan="3" class="erreur_rouge_jaune"><b>- Error -<br />
          <?php if ($verification_erreur[$nb] == 2) { ?>This time crenel already exists<?php } ?>
          <?php if ($verification_erreur[$nb] == 5) { ?>The hour of end is not defined<?php } ?>
          <?php if ($verification_erreur[$nb] == 4) { ?>The hour of beginning is not defined<?php } ?>
          <?php if ($verification_erreur[$nb] == 3) { ?>All the fields are not filled<?php } ?>
          <?php if ($verification_erreur[$nb] == 6) { ?>The hour of end cannot be smaller than the hour of beginning<?php } ?>
          </b><br /></td>
         </tr>
        <?php } ?>
        <tr class="<?php echo $couleur_cellule; ?>">
		...
		<select name="type_creneaux[<?php echo $nb; ?>]" size="3" class="input_sans_bord">
			<option value="cours" <?php if ( $action === 'modifier' ) { if ( $data_modif_periode['type_creneaux'] === 'cours' ) { ?>selected="selected"<?php } } elseif (isset($type_creneaux_erreur[$nb])) { if ( $type_creneaux_erreur[$nb] === 'cours' ) { ?>selected="selected"<?php } } ?>>course</option>
			<option value="pause" <?php if ( $action === 'modifier' ) { if ( $data_modif_periode['type_creneaux'] === 'pause' ) { ?>selected="selected"<?php } } elseif (isset($type_creneaux_erreur[$nb])) { if ( $type_creneaux_erreur[$nb] === 'pause' ) { ?>selected="selected"<?php } } ?>>pauses</option>
			<option value="repas" <?php if ( $action === 'modifier' ) { if ( $data_modif_periode['type_creneaux'] === 'repas' ) { ?>selected="selected"<?php } } elseif (isset($type_creneaux_erreur[$nb])) { if ( $type_creneaux_erreur[$nb] === 'repas' ) { ?>selected="selected"<?php } } ?>>meal</option>
		</select>
	  <td><input name="suivi_definie_periode[<?php echo $nb; ?>]" value="1" type="checkbox" <?php if ( ( $action === 'modifier' and $data_modif_periode['suivi_definie_periode'] === '1' ) or ( isset($suivi_definie_periode_erreur[$nb]) and $suivi_definie_periode_erreur[$nb] === '1') ) { ?>checked="checked"<?php } ?> title="logical continuation of the schedules" />
