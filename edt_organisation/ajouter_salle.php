...
<form name="ajouter_salle" action="ajouter_salle.php" method="post">
	...
	<tr>
		<td></td>
		<td><span class="legende">5 characters maximum</span></td>
		<td></td>
		<td><span class="legende">30 characters maximum</span></td>
	</tr>

	<tr>
    	<td>Number of the room : </td>
    	<td><input type="text" name="numerosalle" value="" size="5" maxlength="5" title="You can use the tabulator to change field" onfocus="this.className='focus';" onblur="this.className='normal';" />
			<img src="../images/icons/ico_ampoule.png" title="If you do not specify the name of the room, its number will be its
name." /></td>
...
if (isset($nom_salle) AND isset($numero_salle)) {
	...
	if ($numero_salle == "") {
		echo "<font color=\"red\">You must specify a number of room !</font>\n<br />\n";
	}
	if ($nom_salle == "") {
...
$verif_champs = 0;
	...
	if ($verif_long_num > 0) {
		if ($verif_long_num <= 5 OR $verif_long_nom <= 30) {
			...
			trigger_error(' An error of typing blocked the system, please restart. ', E_USER_ERROR);
		}
	}
...
updateOnline("INSERT INTO salle_cours
									(id_salle, numero_salle, nom_salle) VALUES
									('', '$add_new_numero', '$add_new_salle')");
			echo "<span class=\"accept\">The room number ".unslashes($add_new_numero)." called \"".unslashes($add_new_salle)."\" was indeed recorded !</span>";
		}
		else{
			echo "<span class=\"refus\">This room already exists ! Please change its number</span>";
		}
	}else{
		trigger_error('Impossible to enter new rooms. ', E_USER_WARNING);
	}
}
...
<fieldset id="modifier_nom">
	<legend>Modify the name of a room</legend>
...
?>
		...
		<option value="rien">Choice of the room</option>
<?php
...
<fieldset id="enlever">
	<legend>Erase a room of the data base</legend>
		<table border="0" cellpading="0" cellspacing="0">
			<tr><td>
<?php
if ($_SESSION["statut"] == "administrateur" AND isset($del_salle) AND $del_salle != NULL AND $del_salle != "rien") {
	...
	$rep_verif = mysql_num_rows($req_verif);
		if ($rep_verif != 1) {
			echo "Impossible to erase this room because it does not exist !";
			trigger_error("You try to erase a room which does not exist !", E_USER_ERROR);
		}
	$req_effacer = mysql_query("DELETE FROM salle_cours WHERE id_salle = '".$del_salle."'")
						OR trigger_error('This room could not be erase', E_USER_WARNING);
	...
	echo '
	<font color="green">The room '.$rep_nom["numero_salle"].$aff_nom_salle.' has been erase from the Gepi base.</font>';
}
?>
			</td><td>
...
?>
		...
		<input type="hidden" name="ajoutsalle" value="ok" />
		<br /><br />
		<input type="submit" name="Valider" value="Erase" />
		<br />
			</td></tr>
		</table>
</fieldset>
