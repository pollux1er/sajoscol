...
$message = "";
	...
	if ($modif_setting == "ok") {
		check_token();
		$modif = mysql_query($requete) OR DIE('The modification was not recorded : '.mysql_error());
		updateOnline($requete);
		//$message .= "<p class=\"red\">La modification a bien été enregistrée !</p>";
		$msg .= "The modification was indeed recorded !";
		$post_reussi=TRUE;
	}
  ...
  if (isset ($autorise_saisir_prof)){
    ...
    if (saveSetting("edt_remplir_prof", $autorise_saisir_prof)){
      $message .= "<p class=\"red\">The modification was recorded !</p>";;
	  $msg .= " The modification was recorded !";
    }
  }

	// Petite fonction pour déterminer le checked="checked"
	function eval_checked($Settings, $yn){
	$req_setting = mysql_fetch_array(mysql_query("SELECT value FROM setting WHERE name = '".$Settings."'")) OR DIE ('Request error eval_setting () : '.mysql_error());
