...
if (isset($data['calendrier']) AND isset($data['supprimer'])) {
	...
	$req_supp = mysql_query("DELETE FROM edt_calendrier WHERE id_calendrier = '".$data['supprimer']."'") or Die ('Impossible suppression !');
    if ($data['supprimer'] != 0) {
        $req_supp_cours = mysql_query("DELETE FROM edt_cours WHERE id_calendrier = '".$data['supprimer']."'") or Die ('Impossible suppression !');
    }
            ...
            if ($data['coller_edt'] != $_SESSION['copier_periode_edt']) {
                ...
                while ($rep_edt_periode = mysql_fetch_array($req_edt_periode)) {
                    $sql = "SELECT id_cours FROM edt_cours WHERE 
                            ...
                            ";
                    $verif_existence = mysql_query($sql) OR DIE('Error in the checking of the course : '.mysql_error());
                ...
                if ($i == 0) {
                    $data['message'] = "duplication was already carried out ";
                }
                else {
                    $data['message'] = "duplication carried out. ".$i." courses were copied successfully";
                }
            }
            else {
                $data['message'] = "you cannot duplicate a period on itself"; 
            } 
        }
        else {
            $data['message'] = "the target period does not exist";
        }
    }
    else {
        $data['message'] = "the period to be duplicated does not exist";
    }
}
			...
			if ($data['nom_periode'] != '' AND $heuredeb_ts != '' AND $heurefin_ts != '') {
				$req_insert = mysql_query("INSERT INTO edt_calendrier (`nom_calendrier`, `classe_concerne_calendrier`, `debut_calendrier_ts`, `fin_calendrier_ts`, `jourdebut_calendrier`, `heuredebut_calendrier`, `jourfin_calendrier`, `heurefin_calendrier`, `numero_periode`, `etabferme_calendrier`, `etabvacances_calendrier`)
								VALUES ('".$data["nom_periode"]."',
										...
										'".$data["etabferme"]."',
										'".$data["vacances"]."')")
								OR trigger_error('Failure in the request of creation of a new entry !', E_USER_WARNING);
			}

		}else{

			$data['message'] = "This name of period already exists";
		}
	}
	else {
		$data['message'] = "One of the dates does not have the awaited format.";
	}
}
...
if (isset($data['modif_ok']) AND isset($data['nom_periode'])) {
  ...
  updateOnline("UPDATE edt_calendrier
				SET nom_calendrier = '".traitement_magic_quotes($data['nom_periode'])."',
				classe_concerne_calendrier = '".$classes_concernees_insert."',
				debut_calendrier_ts = '".$deb_ts."',
				fin_calendrier_ts = '".$fin_ts."',
...
$data['req_affcalendar'] = mysql_query("SELECT * FROM edt_calendrier ORDER BY jourdebut_calendrier") OR die ('Impossible of display the calendar.');
...
for ($i=0; $i<$data['nbre_affcalendar']; $i++) {
	...
	if ($data['expl_aff'][$i] == "0" OR $data['rep_affcalendar'][$i]["classe_concerne_calendrier"] == "0") {
		$data['aff_classe_concerne'][$i] = "<span class=\"legende\">All</span>";
	}
	else {
		$data['contenu_infobulle'] = "<span style=\"color: brown;\">".(count($data['expl_aff'][$i]) - 1)." class(es).</span><br />";
		$contenu_infobulle = "";
