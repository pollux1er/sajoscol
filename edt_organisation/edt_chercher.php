...
if (strstr($ua, "MSIE 6.0")) {
	echo "<div class=\"cadreInformation\">Your navigator (Internet Explorer 6) is obsolete and behaves badly with respect to the display of the
timetables. Absolutely make an update towards versions 7 ou 8 or change navigator (FireFox, Chrome, Opera, Safari)</div>";
}
...
if ($cherch_salle == "ok") {
	if ($ch_heure != "rien") {
		...
		echo ("<div class=\"cadreInformation\">You must choose a crenel !</div>");

	if ($ch_jour_semaine != "rien") {
		$auto_aff_22 = 1;
	}
	else
		echo ("<div class=\"cadreInformation\">You must choose one day of the week !</div>");
...
echo "<option value='rien'>Day</option>\n";
...
echo "<option value='rien'>Week</option>\n";
	...
	for($d=0;$d<52;$d++) {
		...
		echo "<option value='".$tab_select_semaine[$d]["id_semaine"]."'>Week n° ".$tab_select_semaine[$d]["num_semaine"]." (".$tab_select_semaine[$d]["type_semaine"].") : ".$tab_select_semaine[$d]["lundis"]." - ".$tab_select_semaine[$d]["vendredis"]." </option>\n";
...
if ($auto_aff_2 === 1) {
	...
	echo "Free rooms the <font color=\"green\">".$ch_jour_semaine."</font> from <font color=\"green\">".$reponse_tab_creneaux["heuredebut_definie_periode"]." to ".$reponse_tab_creneaux["heurefin_definie_periode"]." ( ".$reponse_tab_creneaux["nom_definie_periode"]." )</font> are :\n";
	echo "<br />\n";
