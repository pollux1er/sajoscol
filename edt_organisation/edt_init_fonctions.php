...
function renvoiConcordances($chiffre, $etape){
	...
	if ($query) {
		...
		if ($test >= 1) {
			$reponse = mysql_fetch_array($query)
				OR trigger_error('Error in $reponse for the'.$chiffre.'<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-> on the request '.$sql, E_USER_WARNING);
		}else{
			$reponse["nom_gepi"] = '';
		}
...
function salleifexists($numero){
	...
	$query = mysql_query($sql)
				OR trigger_error('Impossible to check the existence of this room: the request '.$sql.' failed : '.mysql_error(), E_USER_WARNING);
...
function enregistreCoursCsv2($jour, $creneau, $classe, $matiere, $prof, $salle, $groupe, $regroupement, $effectif, $modalite, $frequence, $aire){
	...
	if ($jour_e == '' OR $creneau_e == 'erreur'
		...
		if (mysql_num_rows($ifexists) < 1) {
			...
			$envoi = mysql_query($sql) OR DIE('Error in the request '.$sql);
			if ($envoi) {
				// et on renvoie 'ok'
				$retour["reponse"] = 'ok';
			}else{
