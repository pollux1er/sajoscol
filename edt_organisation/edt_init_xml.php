...
*/
//**************** EN-TETE *****************
$titre_page = "Tool of initialization of the year : Importation of the timetable";
...
echo "Insertion of the room $salle in the base <br>";
	}
	}
...
echo "<h1>Treatment of the whole classes</h1>";
	foreach ($xml_emp_sts->xpath('//DIVISION') as $o_classe)
	{
	$s_nom_classe=(string)$o_classe["CODE"];
	echo "<h2>Treatment of the class".$s_nom_classe."</h2>";
...
{ echo "<font color=red> A professor was not found in the base, check its presence and restart the script<br>Nom:$s_nom_prof et Prénom:$s_prenom_prof</font>"; next;}
...
//		echo "<br>id_groupe :".$row_cours[0]."<br>";
		$id_groupe=$row_cours[0];
			...
			echo "<br> No groups corresponding with the following data : whole Class $s_nom_classe  Prof : ".$row_prof[0]." Matter ".$ar_matiere["$s_code_matiere"]." Treatment to make later on <br>";
		continue;
		}
...
{
echo "<br>The group of the class $s_nom_classe, course ".$ar_matiere["$s_code_matiere"].", prof $s_nom_prof,day ".$ar_horaires["jour_horaire_etab"][$s_day_cours].", hour ".$ar_periodes[$id_definie_periode]["nom_definie_periode"]." , room $s_salle already exist  <br>";
}
else
...
// le cours peut aussi finir en milieu de créneaux, on rajoute une demi-heure sur le créneau suivant
	...
	echo "<p><font color=green>Insertion of the course for the group of the class $s_nom_classe, course ".$ar_matiere["$s_code_matiere"].", prof $s_nom_prof,day ".$ar_horaires["jour_horaire_etab"][$s_day_cours].", hour ".$ar_periodes[$id_definie_periode]["nom_definie_periode"]." , room $s_salle for the period ".$ar_alternance[$code_repetition_cours]["LIBELLE_LONG"]."</font></p>";
}
unset($periodes);
}
}
...
echo "<h1>Treatment of the groups</h1>";
	...
	echo "This group has elements in the class ".$s_nom_classe."<br>";
...
$result_recherche_prof=mysql_query($query);
if (mysql_num_rows($result_recherche_prof)==0)
{ echo " <font color=red>A professor was not found in the base, check its presence and restart the script<br>Name:$s_nom_prof and First name:$s_prenom_prof</font>"; next;}
$row_prof=mysql_fetch_row($result_recherche_prof);
...
//		echo "<br>id_groupe : ".$row_cours[0]."<br>";
		...
		echo "<br> No groups corresponding with the following data : Group $s_nom_classe  Prof : ".$row_prof[0]." Course ".$ar_matiere["$s_code_matiere_groupe"]." Treatment to make later<br>";
		continue;
		}
...
if (mysql_num_rows($result_search_edt)>0)
{
echo "<br>The group of the class $s_nom_classe, course ".$ar_matiere["$s_code_matiere_groupe"].", prof $s_nom_prof,day ".$ar_horaires["jour_horaire_etab"][$s_day_cours].", hour ".$ar_periodes[$id_definie_periode]["nom_definie_periode"]." , room $s_salle already exist <br>";
}
else
//insertion du cours qui n'est pas encore défini
{
...
// le cours peut aussi finir en milieu de créneaux, on rajoute une demi-heure sur le créneau suivant
	...
	echo "<p><font color=green>Insertion of the course for the group of the class $s_nom_classe, course ".$ar_matiere["$s_code_matiere_groupe"].", prof $s_nom_prof,day ".$ar_horaires["jour_horaire_etab"][$s_day_cours].", hour ".$ar_periodes[$id_definie_periode]["nom_definie_periode"]." , room $s_salle for the period ".$ar_alternance[$code_repetition_cours]["LIBELLE_LONG"]."</font></p>";
}
unset($periodes);
}
}
