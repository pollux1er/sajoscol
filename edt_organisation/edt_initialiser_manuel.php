...
<p>For the simple entries, the manual typing is possible.
It will enable you to return hour per hour of information
by checking if two courses do not overlap.</p>
<p>We advise you that to type the timetables of the professors directly on their page.
 For that, click on [View], [Professor], then you choose the professor in the drop-down list.
  While clicking then on (-+-), a window appears where you can type the courses. A module of checking of the courses is present but cannot replace a
true software of manufacture of the timetables.</p>

	</div>

	<h4 style=\"color: red\">Caution! only courses
defined in Gepi can appear in the timetable. If you have courses without notes (support, ATP,...), you can use the IDA.</h4>
<br />";


	// Saisie manuelle de l'emploi du temps
echo '
	<span class="legend">You should use this menu only for rare occasions because it is not as
powerful as the method described above.</span>
		<form action="edt_initialiser_manuel.php" name="choix_prof" method="post">
	<fieldset id="init_edt1">
		<legend>Manual typing</legend>

		<select name="prof" onchange=\'document.choix_prof.submit();\'>
			<option value="rien">Choice of the professor</option>
	';
...
if (isset($choix_prof)) {
	...
	<fieldset id="init_edt2">
		<legend>Choice of the course</legend>
...
<table border="0" cellspacing="4" cellpadding="0">
	...
	echo "
				<option value=\"rien\">Choice of the course</option>\n";
	...
	<tr>
		<td>
			<select name="heure_debut">
				<option value="0">The course starts at the beginning of a crenel</option>
				<option value="0.5">The course starts in the medium of a crenel</option>
			</select>
		</td>';
	...
	if (isset($choix_prof) AND ($enseignement == "rien" OR $login_salle == "rien" OR $ch_heure == "rien" OR $ch_jour_semaine == "rien")) {
		echo '
	<p class="refus">You must fill all the fields!</p><br />';
	}else {
		if (isset($choix_prof) AND $enseignement != NULL) {
			...
			if ($nbre_verif_s != 0) {
				...
				if ($rep_present_s['id_aid'] != "") {
					...
					echo "<p class=\"refus\">This room is already occupied by a group of IDA( ".$aid." ).</p>";
				}else{
					$tab_present_s = get_group($rep_present_s["id_groupe"]);
					echo "<p class=\"refus\">This room is already occupied by ".$tab_present_s["classlist_string"]." in ".$tab_present_s["description"]."</p><br />";
				}
			...
			if ($nbre_verif_prof != 0) {
				...
				if ($verif_prof['id_aid'] != "") {
					...
					echo "<p class=\"refus\">This professor has already course with a group  of IDA ( ".$aid." ).</p>";
				}else{
					$tab_present_p = get_group($rep_verif_prof["id_groupe"]);
					echo "<p class=\"refus\">This professor has already course with ".$tab_present_p["classlist_string"]." en ".$tab_present_p["description"]."</p><br />";
				}
			}
			...
			if ($nbre_verif_prof === 0 AND $nbre_verif_s === 0) {
				...
				$titre_listeleve = "List of students";

				$classe_js = "<a href=\"#\" onmouseover=\"afficher_div('nouveau_cours','Y',10,10);return false;\">List</a>
					".creer_div_infobulle("nouveau_cours", $titre_listeleve, "#330033", $contenu, "#FFFFFF", 15,0,"n","n","y","n");
				echo "<p>This course is recorded :<font color=\"green\" size=\"1\">
					The ".$tab_infos["classlist_string"]." in ".$tab_infos["description"]." with ".$choix_prof." (".$classe_js.").</font></p>";
			}
		}
	}
}else {
