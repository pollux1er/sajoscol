				...
				if ($formatdatevalid) {
					...
					if ($PeriodeCalendaire->insertable()) {
						...
						if ($success) {
							...
							header('Content-type: text/html;charset=iso-8859-1;');
							$code = "error";
							$message = "impossible to update the period";
							$content = '[{
									"code": "'.$code.'",
									"message": "'.$message.'"
									}]';
						...
						header('Content-type: text/html;charset=iso-8859-1;');
						$code = "error";
						$message = "The overlapping of the periods is not possible";
						$content = '[{
								"code": "'.$code.'",
								"message": "'.$message.'"
								}]';					
					...
					$content = '[{
							"code": "error",
							"message": "The format of the dates is nonvalid" 
							}]';				
				}
			}
		}
	...
	public function deletePeriod(&$content, Request $request) {
		if ($request->getParam('periodid')) {
			...
			if ($success) {
				...
				$code = "success";
				$message = "suppression of the period done";
			}
			else {
				$code = "error";
				$message = "impossible to remove the period";
			}
			$content = '[{
					"code": "'.$code.'",
					"message": "'.$message.'" 
				...
				if ($request->getParam('lastday')) {
					...
					if ($PeriodeCalendaire->insertable()) {
						if (!$PeriodeCalendaire->save()) {
							$content = "error Impossible to record the period";
						}
						else {
							$content = "success";
						}
						...
						$content = "error The periods cannot overlap";					
					}
				}
			}
		}
		else {
			$content = "error Please enter a name of period";
		}	
	}
}
