		...
		if ($request->getParam('id_calendrier')) {
			...
			if ($calendrier->exists()) {
				...
				if (!$jointure->PeriodsCompatible()) {
					...
					$message = "Certain classes do not have the same periods of notes !  Thus you will not be able to associate a period of notes to a calendar
period.";
				}
				else {
				...
				$response->addVar('nom_calendrier', "Error - non-existent calendar");
				$response->addVar('Calendrier', "");			
			}
		}
		else {
			$response->addVar('nom_calendrier', "Error - no calendar requested");
			$response->addVar('Calendrier', "");
		}
