			...
			if ($request->getParam('operation')) {
				...
				if ($request->getParam('operation') == "delete") {
					if ($request->getParam('confirm_delete')) {
						if ($request->getParam('id_calendrier')) {
							...
							if (!$calendrier->delete()) {
								$message = "Impossible to remove the calendar";
							}
						}
					}
					else {
						if ($request->getParam('id_calendrier')) {
							$delete_confirmation = "<form action=\"index.php?action=calendriermanager\" method=\"post\">".
											...
											<p>The removal of a calendar involves the suppression of all the calendar periods which depend on it !</p>
											<input name=\"confirm_delete\" type=\"submit\" style=\"width:200px;\" value=\"Confirm the suppression\">
										</form>";
						}					
					...
					if ($request->getParam('nom_calendrier')) {
						...
						if (!$calendrier->save()) {
							$message = "Impossible to create the calendar";
						}
					}
				}
					...
					if ($request->getParam('new_name')) {
						...
						if (!$calendrier->update()) {
							$message = "Impossible to modify the name of the calendar";
						}
					}
					else {
						if ($request->getParam('id_calendrier')) {
							$new_name = "<form action=\"index.php?action=calendriermanager\" method=\"post\">".
											...
											<input name=\"bouton_valider_new_name\" type=\"submit\" style=\"width:200px;\" value=\"Modify the name of the calendar\">
										</form>";
						}
					}
				}
						...
						if ($request->getParam('classes_'.$id_calendrier)) {
							...
							foreach ($request->getParam('classes_'.$id_calendrier) as $id_classe) {
								...
								if (!$jointure->save_classe()) {
									$message .= "A class is already affected in another calendar<br/>";
								}
							}
