...
if (isset($_POST['is_posted']) and ($_POST['is_posted'] == "1")) {
	...
	if (($continue == 'yes') and (isset($reg_login))) {
		...
		if (my_ereg ("^[a-zA-Z_]{1}[a-zA-Z0-9_.]{0,".($longmax_login-1)."}$", $reg_login)) {
			if ($reg_no_gep != '') {
				...
				if ($count1 != "0") {
					...
					$msg .= "Error : a student having the same interns number  Sconet (elenoet) already exist.<br />";
					$ok = 'no';
				}
			}
				...
				if ($count == "0") {
					...
					*/
					$reg_data1 = mysql_query("INSERT INTO eleves SET
						...
						date_sortie = NULL
						");
						updateOnline("INSERT INTO eleves SET
						no_gep = '".$reg_no_nat."',
						nom='".$reg_nom."',
						prenom='".$reg_prenom."',
						email='".$reg_email ."',
						...
						if(mysql_num_rows($test_resp1)>0){
							...
							if(mysql_num_rows($test_resp1b)==1){
								...
								if(mysql_num_rows($test_resp1c)==1){
									...
									updateOnline($sql);
									if(!$res_update){
										$msg.="Error during the update of theresponsible $lig_autre_resp->pers_id to legal responsible n°2.<br />\n";
										$temoin_maj_resp="PB";
									}
								}
								...
								if($temoin_maj_resp==""){
									...
									updateOnline($sql);
									if(!$res_update){
										$msg.="Error during of the update of the responsible $reg_resp1 to legal responsible n°1.<br />\n";
									}
								}
							}
							...
							if(mysql_num_rows($test_resp1c)>0){
								...
								updateOnline($sql);
								if(!$res_menage){
									$msg.="Error during the suppression of association with the previous legal responsible n°2.<br />";
									$temoin_maj_resp="PB";
								}
								else{
									...
									updateOnline($sql);
									if(!$res_update){
										$msg.="Error during the update of the responsible $lig_autre_resp->pers_id to legal responsible n°2.<br />\n";
										$temoin_maj_resp="PB";
									}
								}
							}
							...
							if($temoin_maj_resp==""){
								...
								updateOnline($sql);
								if(!$reg_data2b){
									$msg.="Error during the update of the responsible $reg_resp1 to legal responsible n°1.<br />\n";
								}
							}
						}
					}
					...
					updateOnline("INSERT INTO j_eleves_regime SET login='$reg_login', doublant='-', regime='d/p';");
					if ($reg_no_gep != '') {
						...
						if ($count2 == "0") {
							if ($reg_etab != "(vide)") {
								...
								updateOnline("INSERT INTO j_eleves_etablissements VALUES ('$reg_no_gep','$reg_etab');");
							}
						} else {
							if ($reg_etab != "(vide)") {
							...
							updateOnline("UPDATE j_eleves_etablissements SET id_etablissement = '$reg_etab' WHERE id_eleve='$reg_no_gep';");
							} else {
							...
							updateOnline("DELETE FROM j_eleves_etablissements WHERE id_eleve='$reg_no_gep';");
							}
						}
					}
					if ((!$reg_data1) or (!$reg_data3)) {
						...
						$mess=rawurlencode("Student registered. You can type the next student.");
			...
			$msg="The selected identifier is made up to the maximum of 12 characters : letters, number, \"_\" ou \".\" and does not have to start with a number !";
		}
		...
		updateOnline("UPDATE eleves SET no_gep = '$reg_no_nat', nom='$reg_nom',prenom='$reg_prenom',email='$reg_email',sexe='$reg_sexe',naissance='".$reg_naissance."', ereno='".$reg_resp1."', elenoet = '".$reg_no_gep."' WHERE login='".$eleve_login."'");
		if (!$reg_data) {
			$msg = "Error during the recording of the data";
		} else {
			...
			if ($test_login > 0) {
				...
				updateOnline("UPDATE utilisateurs SET nom='".$reg_nom."', prenom='".$reg_prenom."', email='".$reg_email."' WHERE login = '".$eleve_login."'");
		...
		$res_ele_id_eleve=mysql_query($sql);
		if(mysql_num_rows($res_ele_id_eleve)==0){
			$msg.="Error: The field ele_id is not present. Your table 'eleves' seems not updated.<br />";
			$temoin_ele_id="PB";
		}
		else{
		...
		if($temoin_ele_id==""){
			...
			if(mysql_num_rows($test_resp1)>0){
				...
				if(mysql_num_rows($test_resp1c)>0){
					...
					// Y avait-il un autre responsable légal n°2?
					$sql="DELETE FROM responsables2 WHERE ele_id='$ele_id' AND resp_legal='2'";
					$res_menage=mysql_query($sql);updateOnline($sql);
					if(!$res_menage){
						$msg.="Error during the suppression of association with the previous legal responsible n°2.<br />";
						$temoin_maj_resp="PB";
					}
				...
				if($temoin_maj_resp==""){
					$sql="INSERT INTO responsables2 SET ele_id='$ele_id', pers_id='$reg_resp1', resp_legal='1', pers_contact='1'";
					$reg_data2b=mysql_query($sql);updateOnline($sql);
					if(!$reg_data2b){
						$msg.="Error during the update of the responsible $reg_resp1 to legal responsible n°1.<br />\n";
					}
				}
...
if ((isset($order_type)) and (isset($quelles_classes))) {
    echo "<p class=bold><a href=\"index.php?quelles_classes=$quelles_classes&amp;order_type=$order_type\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a></p>";
} else {
...
echo "<tr>\n";
    ...
    if (isset($eleve_login)) {
        echo "<td>GEPI Identifier  * : </td>
        <td>".$eleve_login."<input type=hidden name='eleve_login' size=20 ";
        if ($eleve_login) echo "value='$eleve_login'";
        echo " /></td>\n";
    } else {
        echo "<td>GEPI Identifier  * : </td>
        <td><input type=text name=reg_login size=20 value=\"\" maxlength='".$longmax_login."' /> (<i>max.$longmax_login characters</i>)</td>\n";
    }
    ?>
</tr>
<tr>
...
<?php
    ...
    echo "<tr><td>Sconet interns number   (<i>elenoet</i>) : </td><td><input type='text' name='reg_no_gep' size='20' ";
...
if (($reg_no_gep == '') and (isset($eleve_login))) {
   ...
   echo "<font color=red>CAUTION : This student does not have a Sconet interns number   (elenoet). You will not be able to import the absences from files GEP/Sconet for this students.</font>\n";
	...
	if(mysql_num_rows($test_sconet)>0){
		...
		if($lig_tmp->value=='1'){
			echo "<br />";
			echo "<font color=red>You will not be able to either carry out the updates of its information since Sconet<br />(<i>the'ELENOET and the 'ELE_ID will not correspond to the data of Sconet</i>).</font>\n";
		}
	}
}
...
*/
?>
<label for='reg_sexeM' style='cursor: pointer;'><input type=radio name=reg_sexe id='reg_sexeM' value=M <?php if ($eleve_sexe == "M") { echo "CHECKED" ;} ?> /> Masculine</label>
<label for='reg_sexeF' style='cursor: pointer;'><input type=radio name=reg_sexe id='reg_sexeF' value=F <?php if ($eleve_sexe == "F") { echo "CHECKED" ;} ?> /> Female</label>
</div></td><td><div class='norme'>
Date of birth (respect format 00/00/0000) : <br />
Day <input type=text name=birth_day size=2 value=<?php if (isset($eleve_naissance_jour)) echo $eleve_naissance_jour;?> />
Month<input type=text name=birth_month size=2 value=<?php if (isset($eleve_naissance_mois)) echo $eleve_naissance_mois;?> />
Year<input type=text name=birth_year size=4 value=<?php if (isset($eleve_naissance_annee)) echo $eleve_naissance_annee;?> />
</div></td></tr>
</table></center>

<p><b>Remarks</b> :
<br />- modification of the mode of the student (school luncher, internal, ...) is made in the module of management of the classes !
...
if ($nombreligne != 0) {
	...
	echo "<br /><hr /><h3>Sending of the bulletins by postal way</h3>\n";
	echo "<i>If you do not send the report cards by postal way, you can be unaware of this rubric.</i>";
	echo "<br /><br /><table><tr><td><b>Principal legal responsible : </b></td>\n";
		...
		if(mysql_num_rows($res_resp2)>0){
			...
			*/
			...
			if(substr($lig_resp1->adr1,0,strlen($lig_resp1->adr1)-1)==substr($lig_resp2->adr1, 0, strlen($lig_resp2->adr1)-1) and ($lig_resp1->cp==$lig_resp2->cp) and ($lig_resp1->commune==$lig_resp2->commune) and ($lig_resp1->pays==$lig_resp2->pays)) {
				$message = "<b>The addresses of the two legal responsibles are identical. Consequently, the bulletin will be sent only to the first address.</b>";
			} else {
				if($chaine_adr2!='') {
					$message =  "<b>The addresses of the two legal responsibles are not identical. Consequently, the bulletin will be sent to the two legal responsibles.</b>";
				} else {
					$message =  "<b>The bulletin will be sent to the legal responsible above.</b>";
				}
			}
...
*/
    ...
    echo "</table>\n";
    echo "<br />If the legal responsable does not appear in the list, you can add it to the base
    (after, if necessary, safeguardhaving safeguarded this card)
    <br />while returning in [Management of the bases-><a href='../responsables/index.php'>Management of the students responsibles </a>]";
...
echo "</form>\n";
echo "<p><b>Caution</b>: The recording of the school of origin is conditioned by the typing of an identifier SCONET/GEP (<i>Elenoet</i>)</p>\n";
