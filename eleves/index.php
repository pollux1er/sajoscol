...
if(($_SESSION['statut']=='administrateur')&&(isset($_GET['initialiser_regimes']))&&(in_array($_GET['initialiser_regimes'],$tab_regimes_autorises))) {
	...
	if($test_no_regime_effectif>0){
		...
		$msg="";
		if($nb_reg_regime>0) {
			$msg.="$nb_reg_regime regime were initialized.<br />";
		}
		if($nb_err_regime>0) {
			$msg.="$nb_err_regime error occurred at the time of the initialization of the regim.<br />";
		}
	}
	else {
		$msg="All the regim were already indicated.<br />";
	}
}
...
if($_SESSION['statut']=="professeur") {
	if(getSettingValue('GepiAccesGestElevesProfP')!='yes') {
		tentative_intrusion("2", "Attempt of access by a teacher to students cards , without having the authorization of it.");
		echo "You cannot access to this page because the access professor is not authorized !";
		...
		if (mysql_num_rows($test)==0) {
			tentative_intrusion("2", "Attempt of access by a teacher who is not $gepi_prof_suivi to students cards , without having the authorization .");
			echo "You cannot access in this page because you are not $gepi_prof_suivi !";
			require ("../lib/footer.inc.php");
			die();
		}
	}
...
if (isset($is_posted) and ($is_posted == '2')) {
	...
	if ($quelles_classes == 'certaines') {
		...
		updateOnline("DELETE FROM tempo WHERE num = '".SESSION_ID()."'");
		//
		...
		while ($nb < $nb_enr) {
			...
			if ($nb_en == 0) {
				...
				updateOnline("DELETE FROM tempo WHERE num = '$num'");
			}
			$nb++;
		}
		...
		while ($i < $nb) {
			...
			if ($temp == 'yes') {
				...
				updateOnline("insert into tempo Values('$id_classe','$nb_periode', '".SESSION_ID()."')");
...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {
	if (isset($is_posted) and ($is_posted == '1')) {
		...
		if(!is_array($delete_eleve)) {$delete_eleve=array();$msg="Error: The list of students to be removed should be a table.<br />";}
...
if (isset($action) and ($action == 'depot_photo') and $total_photo != 0)  {
	...
	while($cpt_photo < $total_photo)
	{
		if((isset($_FILES['photo']['type'][$cpt_photo]))&&($_FILES['photo']['type'][$cpt_photo] != ""))
		{
			...
			if (!isset($sav_photo['tmp_name'][$cpt_photo]) or ($sav_photo['tmp_name'][$cpt_photo] =='')) {
				$msg.="Error of download level 1 (<i>photo n°$cpt_photo</i>).<br />";
			} else if (!file_exists($sav_photo['tmp_name'][$cpt_photo])) {
				$msg.="Error of download level 2 (<i>photo n°$cpt_photo</i>).<br />";
			} else if (strtolower($sav_photo['type'][$cpt_photo])!="image/jpeg") {
				$msg.="Error: only files having the extension .jpg are authorized (<i>".$sav_photo['name'][$cpt_photo]."&nbsp;: ".$sav_photo['type'][$cpt_photo]."</i>)<br />";
			} else if (!preg_match('/jpg$/i',$sav_photo['name'][$cpt_photo])) {
				$msg.="Error: only files having the extension .jpg are authorized (<i>".$sav_photo['name'][$cpt_photo]."</i>)<br />";
			} else {
				$dest = $rep_photos;
				$n = 0;
				//$msg.="\$rep_photos=$rep_photos<br />";
				if (!deplacer_fichier_upload($sav_photo['tmp_name'][$cpt_photo], $rep_photos.$quiestce[$cpt_photo].".jpg")) {
					$msg.="Problem of transfer : the file n°$cpt_photo could not be transferred on the repertory photo/eleves/<br />";
				} else {
	...
	if(($msg=="")&&($cpt_photos_mises_en_place>0)) {$msg = "Successful download.";}
}
// fin de l'envoi des photos du trombinoscope

//**************** EN-TETE *****************
$titre_page = "Management of the students";
...
echo "<p class=bold><a href=\"".$retour."\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return </a>\n";
...
if(!getSettingValue('conv_new_resp_table')){
	...
	$test=mysql_query($sql);
	if(mysql_num_rows($test)>0){
		echo "<p>A conversion of the data students/responsibles is necessary.</p>\n";

		if($_SESSION['statut']=="administrateur"){
			echo "<p>Follow this link: <a href='../responsables/conversion.php'>CONVERT</a></p>\n";
		}
		else{
			echo "<p><a href=\"javascript:centrerpopup('../gestion/contacter_admin.php',600,480,'scrollbars=yes,statusbar=no,resizable=yes')\">Contact the administrator</a></p>\n";
		}
...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {
	echo " | <a href='add_eleve.php?mode=unique'>Add a student to the base (simple)</a>\n";
	echo " | <a href='add_eleve.php?mode=multiple'>Add students to the base (in batch)</a>\n";

	$droits = @sql_query1("SELECT ".$_SESSION['statut']." FROM droits WHERE id='/eleves/import_eleves_csv.php'");
	if ($droits == "V") {
		echo " | <a href=\"import_eleves_csv.php\" title=\"Download the file of the names, first names, identifiers GEPI and classes\">Download the file of the students in the csv format .</a>\n";

		if(getSettingValue("import_maj_xml_sconet")==1){
			echo " | <a href=\"../responsables/maj_import.php\">Update from Sconet</a>\n";
			echo " | <a href=\"import_communes.php\">Import the communes of birth of the students</a>\n";
		}
	}
}

if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {echo " | <a href='synchro_mail.php'>Synchronize the students addresses mall </a>\n";}
...
if(($_SESSION['statut']=="administrateur")&&(getSettingValue('exp_imp_chgt_etab')=='yes')) {
	...
	//echo " | ";
	echo "<br />";
	echo "Change of school: <a href='export_bull_eleve.php'>Export of the bulletins</a>\n";
	echo " et <a href='import_bull_eleve.php'>Importation of the bulletins</a>\n";
}
echo "</p>\n";

echo "<center><p class='grand'>Visualize \ modify a student card </p></center>\n";

$req = mysql_query("SELECT login FROM eleves");
$test = mysql_num_rows($req);
if ($test == '0') {
	echo "<p class='grand'>Caution : there is no student in base GEPI !</p>\n";
	if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
		echo "<p>You can add students to the base by clicking on one of the links above";
		if($_SESSION['statut']=="administrateur") {
			echo ", or directly <br /><a href='../initialisation/index.php'>import the students and the classes from files GEP</a></p>\n";
		}
		require("../lib/footer.inc.php");
		die();
	}
...
if (!isset($quelles_classes)) {
	...
	if($_SESSION['statut'] == 'professeur') {
		...
		if($nb_classes==0){
			echo "<p>You are not $gepi_prof_suivi</p>\n";
		...
		echo "<table cellpadding='5' width='100%' border='0' summary='Choice of the mode'>\n";
		...
		echo "<span class='norme'>Student whose name starts with: \n";
		...
		echo "<td>\n";
		echo "<label for='' style='cursor: pointer;'>\n";
		echo "<span class='norme'>Student whose first name starts with: \n";
		...
		if(mysql_num_rows($test_na)==0){
			...
			echo "<span class='norme'>All the students are affected in a class.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>Students nonaffected to a class (<i>".mysql_num_rows($test_na)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
...
// Eric Les élèves dont la date de sortie de l'établissement est renseignée      
		...
		if(mysql_num_rows($test_dse)==0){
			...
			echo "<span class='norme'>No student has the date of exit of the school indicated.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>The students whose date of exit of the school is indicated (<i>".mysql_num_rows($test_dse)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		if(mysql_num_rows($test_incomplet)==0){
			...
			echo "<span class='norme'>All the students have their Elenoet and their national Number (INE) indicated.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>The students whose Elenoet or the national Number (INE) is not indicated (<i>".mysql_num_rows($test_incomplet)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		if (getSettingValue("active_module_trombinoscopes")=='y') {
			...
			if(mysql_num_rows($test_elenoet_ok)!=0){
				...
				if($cpt_photo_manquante>0){
					...
					echo "<span class='norme'>Among the students whose Elenoet is indicated, $cpt_photo_manquante do not have their photo.</span><br />\n";
					echo "</label>\n";
					echo "</td>\n";
					echo "</tr>\n";
				}
					...
					echo "<span class='norme'>All the students, whose Elenoet is indicated, have their photo.</span><br />\n";
					echo "</td>\n";
					echo "</tr>\n";
				}
			}
				...
				echo "<label for='quelles_classes_photo' style='cursor: pointer;'>\n";
				echo "<span class='norme'>No student has its Elenoet indicated.<br />The display of the photographs is thus not functional.</span><br />\n";
				echo "</label>\n";
				echo "</td>\n";
				echo "</tr>\n";
			}
		...
		*/
		...
		if($test_no_cpe_effectif==0){
			...
			echo "<span class='norme'>All students (<i>affected in classes</i>) has an associated CPE.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>Students (<i>affected in classes</i>) without CPE (<i>".$test_no_cpe_effectif."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		if($test_no_regime_effectif==0){
			...
			echo "<span class='norme'>All students (<i>affected in classes</i>) have the regim indicated.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>Students (<i>affected in classes</i>) whose regim is not indicated (<i>".$test_no_regime_effectif."</i>).</span><br />\n";
			echo "<span style='color:red'>The nonindicated regim is blocking for a number of research in this page.</span><br />\n";
			if($_SESSION['statut']=='administrateur') {
				echo "<span style='color:red'>";
				echo "Initialize all the nonindicated regims to the value&nbsp;: ";
				echo "<a href='".$_SERVER['PHP_SELF']."?initialiser_regimes=d/p".add_token_in_url()."'>school luncher</a>, ";
				echo "<a href='".$_SERVER['PHP_SELF']."?initialiser_regimes=int.".add_token_in_url()."'>intern</a>, ";
				echo "<a href='".$_SERVER['PHP_SELF']."?initialiser_regimes=ext.".add_token_in_url()."'>external</a> ou ";
				echo "<a href='".$_SERVER['PHP_SELF']."?initialiser_regimes=i-e".add_token_in_url()."'>intern-externed</a>";
				echo "</span>";
				echo "<br />\n";
			}
			else {
				echo "<span style='color:red'>You can contact the administrator to initialize all the missing modes to the value of your choice among half-pensionnaire, intern, external, intern-externed.</span><br />\n";
			}
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		...
		if($test_no_pp_effectif==0){
			...
			echo "<span class='norme'>All the students have a ".getSettingValue('gepi_prof_suivi')." associated.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>Students without ".getSettingValue('gepi_prof_suivi')." (<i>".$test_no_pp_effectif."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		*/
		...
		if($test_no_resp_effectif==0){
			...
			echo "<span class='norme'>All the students have an associated responsible.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>Students without reponsibles but registered in a class (<i>".$test_no_resp_effectif."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		if(mysql_num_rows($test_no_etab)==0){
			...
			echo "<span class='norme'>All the students have their school of origin indicated.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>The students whose school of origin is not indicated (<i>".mysql_num_rows($test_no_etab)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		if(mysql_num_rows($test_incomplet)==0){
			...
			echo "<span class='norme'>All the students have their email indicated in the table 'eleves'.</span><br />\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		else{
			...
			echo "<span class='norme'>The students whose email is not indicated in the table 'eleves' (<i>".mysql_num_rows($test_incomplet)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		...
		echo "<span class='norme'>All students.</span><br />";
		echo "</label>\n";
		echo "</td>\n";
		echo "</tr>\n";
		...
		if ($nb !=0) {
			...
			echo "<span class = \"norme\">Only students of the classes selected below : </span>";
			echo "</label>\n";
			echo "<br />\n";
...
if(isset($quelles_classes)) {
	...
	echo "<p class='small'>Notice : the identifier mentioned here does not make it possible the students to be connected to Gepi, it is used simply as single identifier.";
	//if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	if($_SESSION['statut']=="administrateur") {
		echo " To allow the students to connect itself to Gepi, you must create accounts of access to them, while passing by the Management page of the bases -> Management of the accounts of access users -> <a href='../utilisateurs/edit_eleve.php'>Students</a>.";
	}
	elseif($_SESSION['statut']=="scolarite") {
		echo " To allow the students to connect itself to Gepi, you must connect yourselves in 'administrator' and to create accounts of access to them, while passing by the page Management of the bases -> Management of the accounts of access users -> Students.";
	}
	echo "</p>\n";
	...
	*/
	...
	if($_SESSION['statut'] == 'professeur') {
		...
		*/
		...
		echo "<p align='center'>List of students of the selected class.</p>\n";
	}
	else{
		if ($quelles_classes == 'certaines') {
			/*
			...
			*/
			...
			echo "<p align='center'>List of students of classes selected.</p>\n";
			...
			echo "<p align='center'> List of all the students.</p>\n";
			...
			*/
			...
			echo "<p align='center'>List of students nonaffected in a class.</p>\n";
			...
			*/
			...
			echo "<p align='center'>List of students whose Elenoet or the national Number (INE) is not indicated.</p>\n";
			...
			*/
			...
			echo "<p align='center'>List of students whose email is not indicated.</p>\n";
			...
			echo "<p align='center'>List of students without photo.</p>\n";
			...
			echo "<p align='center'>List of students without CPE.</p>\n";
			...
			echo "<p align='center'>List of students whose regim is not indicated.</p>\n";
			...
			echo "<p align='center'>List of students without ".getSettingValue('gepi_prof_suivi')."</p>\n";
			...
			*/
			...
			echo "<p align='center'>List of students whose first name starts with <b>$motif_rech</b></p>\n";
			...
			*/
			...
			echo "<p align='center'>List of students whose name starts with <b>$motif_rech</b></p>\n";
		}
			...
			echo "<p align='center'>List of students having a date of exit indicated.</p>\n";
			}
	...
	echo "<table border='1' cellpadding='2' class='boireaus'  summary='Table of the students of the class'>\n";
	echo "<tr>\n";
	echo "<th><p>Identifier</p></th>\n";
	echo "<th><p><a href='index.php?order_type=nom,prenom&amp;quelles_classes=$quelles_classes";
	if(isset($motif_rech)){echo "&amp;motif_rech=$motif_rech";}
	echo "'>Name First name</a></p></th>\n";
...
//    echo "<th><p>Classe</p></th>";
	echo "<th><p>Teacher<br />follow-ups</p></th>\n";
	echo "<th><p>".ucfirst(getSettingValue("gepi_prof_suivi"))."</p></th>\n";

	//if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	if($_SESSION['statut']=="administrateur") {
		echo "<th><p><input type='submit' value='Delete' onclick=\"return confirmlink(this, 'The suppression of a student is irreversible and involve the complete obliteration of all its data (notes, appreciations, ...). Are you sure you want to continue ?', 'Confirmation of the suppression')\" /></p></th>\n";
	}
	elseif($_SESSION['statut']=="scolarite") {
		echo "<th><p><span title=\"The suppression is possible only with an administrator account \">Delete</span></p></th>\n";
	}

	if (getSettingValue("active_module_trombinoscopes")=='y') {
		if($_SESSION['statut']=="professeur") {
			if (getSettingValue("GepiAccesGestPhotoElevesProfP")=='yes') {
				echo "<th><p><input type='submit' value='Download the photos' name='bouton1' /></th>\n";
			}
		}
		else{
			echo "<th><p><input type='submit' value='Download the photos' name='bouton1' /></th>\n";
...
*/
	...
	while ($i < $nombreligne){
		...
		if ($date_sortie_elv!=0) {
		     echo "<br/>";
		     echo "<span class=\"red\"><b>Exit the ".affiche_date_sortie($date_sortie_elv)."</b></span>";;
		}
		echo "</p></td>\n";
		...
		echo "<td><p><a href='../classes/eleve_options.php?login_eleve=".$eleve_login."&amp;id_classe=$eleve_id_classe&amp;quitter_la_page=y' target='_blank'><img src='../images/icons/chercher.png' width='16' height='16' alt='Followed courses' title='Followed courses' /></a></p></td>\n";
		...
		elseif($_SESSION['statut']=="scolarite") {
			echo "<td><p align='center'><span title=\"The suppression is possible only with an administrator account \">-</span></p></td>\n";
		}
		...
		if ((getSettingValue("active_module_trombinoscopes")=='y')&&
			...
			if($photo){
				...
				echo "' width='32' height='32'  align='middle' border='0' alt='photo present' title='photo present' />";
				echo "</a>";
			...
			if($nom_photo_test=="") {
				...
				echo "<span style='color:red'>Elenoet not indicated</span>";
			}
			else {
	...
	echo "</table>\n";
	echo "<p>Total : $nombreligne student";
...
?>
	...
	if(($max_file_uploads!="")&&(strlen(preg_replace("/[^0-9]/","",$max_file_uploads))==strlen($max_file_uploads))&&($max_file_uploads>0)) {
		echo "<p><i>Notes</i>&nbsp;:</p>\n";
		echo "<ul>\n";
		echo "<li><p>The upload of the photographs is limited to $max_file_uploads file(s) at the same time.</p></li>\n";
	...
	if($_SESSION['statut']=='administrateur') {
		...
		echo "<li><i>Note</i>&nbsp;: It is possible to upload a file <a href='../mod_trombinoscopes/trombinoscopes_admin.php#formEnvoi'>ZIP of a batch of photographs</a> rather than to upload one by one.<br />It is necessary that the photographs are named to the format ELENOET.JPG</p></li>\n";
		$temoin_notes_bas_de_page="y";
	}
	...
	echo "<div id='div_changer_regime' style='position: absolute; top: 220px; right: 20px; width: 200px; text-align:center; color: black; padding: 0px; border:1px solid black; display:none;'>\n";
		...
		echo add_token_field();
		echo "<input type='button' onclick='valider_changement_regime()' name='Valider' value='Validate' />\n";
		echo "</form>\n";
	...
	function afficher_changement_regime(login_eleve, regime_eleve) {
		...
		document.getElementById('titre_entete_changer_regime').innerHTML='Regim of '+login_eleve;
	...
	function valider_changement_regime() {
		if(document.getElementById('regime_login_eleve')) {
			...
			alert('document.getElementById(\'regime_login_eleve\') is not affected.')
		}
