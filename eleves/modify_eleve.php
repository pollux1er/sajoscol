...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {
	...
	if((isset($modif_adr_pers_id))&&(isset($adr_id))) {
		...
		updateOnline($sql);
		if(!$update){
			$msg="Failure of the modification of the address of the second responsible.";
		}
	}
	...
	*/
	...
	if((isset($eleve_login))&&(isset($definir_resp))&&(isset($_POST['valider_choix_resp']))) {
		...
		if($pers_id==""){
			...
			if(mysql_num_rows($res_ele)==0){
				$msg="Error: The student $eleve_login seems to be absent of the table 'eleves'.";
			}
			else{
				...
				if($suppr){
					$msg="Suppression of the association of the student with the responsible $definir_resp succeeded.";
				}
				else{
					$msg="Failure of the suppression the association of the student with the responsible $definir_resp.";
				}
			}
		}
		else{
			...
			if(mysql_num_rows($test)==0){
				$msg="Error: The identifier of responsible proposed does not exist.";
			}
			else{
				// Recherche de l'ele_id
				$sql="SELECT ele_id FROM eleves WHERE login='$eleve_login'";
				...
				if(mysql_num_rows($res_ele)==0){
					...
					if(mysql_num_rows($test)==0){
						...
						if($insert){
							$msg="Association of the student with the responsible $definir_resp succeeded.";
						}
						else{
							$msg="Failure of the association of the student with the responsible $definir_resp.";
						}
					}
					else{
		...
		if(mysql_num_rows($res_elenoet)>0) {
			...
			if($reg_no_gep!="") {
				if($reg_etab==""){
					...
					$suppr=mysql_query($sql);updateOnline($sql);
					if($suppr){
						$msg="Suppression of the association of the student with a school succeeded.";
					}
					else{
						$msg="Failure of the suppression the association of the student with a school.";
					}
				}
				else{
					...
					if(mysql_num_rows($test)==0){
						$msg="Error: Selected school (<i>$reg_etab</i>) do not exist in the table 'etablissement'.";
					}
					else{
						...
						if(mysql_num_rows($test)==0){
							...
							$insert=mysql_query($sql);updateOnline($sql);
							if($insert){
								$msg="Association of the student with the school $reg_etab succeeded.";
							}
							else{
								$msg="Failure of the association of the student with the school $reg_etab.";
							}
						}
						else{
	...
	if (isset($_POST['is_posted']) and ($_POST['is_posted'] == "1")) {
		...
		if (($reg_nom == '') or ($reg_prenom == '')) {
			$msg = "The fields name and first name are obligatory.";
			$continue = 'no';
		}
			...
			if (preg_match("/^[a-zA-Z_]{1}[a-zA-Z0-9_]{0,11}$/", $reg_login)) {
				if ($reg_no_gep != '') {
					...
					if ($count1 != "0") {
						...
						$msg .= "Error: a student having the same interns Sconet number (elenoet) already exists.<br />";
						$ok = 'no';
					}
				}
				...
				if ($reg_no_nat != '') {
					...
					if ($count2 != "0") {
						$msg .= "Error: a student having the same national number already exists.";
						$ok = 'no';
					}
				}
					...
					if ($count == "0") {
						...
						*/
						if ((!$reg_data1) or (!$reg_data3)) {
							$msg = "Error during the recording of the data";
						} elseif ($mode == "unique") {
							$mess=rawurlencode("Student registered!");
							header("Location: index.php?msg=$mess");
							die();
						} elseif ($mode == "multiple") {
							$mess=rawurlencode("Student recorded.You can type the next student.");
							header("Location: modify_eleve.php?mode=multiple&msg=$mess");
							die();
						}
					} else {
						$msg="A student having the same identifier already exists  !";
					}
				}
			} else {
				$msg="The selected identifier is made up to the maximum of 12 characters : letters, numbers or \"_\" and does not have to start with a number !";
			}
 ...
 */
								if($photo){
									if(unlink($photo)){
										$msg.="The photo ".$photo." was removed. ";
									}
									else{
										$msg.="Failure of the suppression of the photo ".$photo." ";
									}
								}
								else{
									$msg.="Failure of the removal of the photo correspondent to $reg_no_gep (<i>not found</i>) ";
								}
							}
						}
							...
							if(isset($_FILES['filephoto'])){
								...
								if($filephoto_tmp!=""){
									...
									if(is_uploaded_file($filephoto_tmp)){
										...
										if($res_copy){
											$msg.="Installation of the photograph done.";
										}
										else{
											$msg.="Error during the installation of the photo.";
										}
										...
										$msg.="Error during the upload of the photograph.";
									}
								}
							}
						}
						elseif($nb_elenoet==0){
								...
								$msg.="The interns Sconet number (elenoet) of the student is not recorded in the table 'eleves'.";
						}
						else{
							//$msg.="Le numéro GEP est commun à plusieurs élèves. C'est une anomalie.";
							$msg.="The interns Sconet number (elenoet) is common to several students. It is an anomaly.";
						}
					}
					else{
						//$msg.="Le numéro GEP proposé contient des caractères non numériques.";
						$msg.="The interns Sconet number  (elenoet) proposed contains nonnumerical characters.";
					}
				}
			}
			...
			$res_ele_id_eleve=mysql_query($sql);
			if(mysql_num_rows($res_ele_id_eleve)==0){
				$msg.="Error: The field ele_id is not present. Your table 'eleves' seem not updated.<br />";
				$temoin_ele_id="PB";
			}
			else{
 ...
 */
							if($photo){
								...
								$msg.="Failure of the suppression of the photo correspondent to $reg_no_gep (<i>not found</i>) ";
							}
						}
					}
						...
						if(isset($_FILES['filephoto'])){
							...
							if($filephoto_tmp!=""){
								...
								if(is_uploaded_file($filephoto_tmp)){
									...
									$res_copy=copy("$source_file" , "$dest_file");
									if($res_copy){
										$msg.="Installation of the photo done.";
									}
									else{
										$msg.="Error during the installation of the photo.";
									}
									...
									$msg.="Error during the upload of the photo.";
								}
							}
						}
					}
					...
					//$msg.="Le numéro GEP proposé contient des caractères non numériques.";
					$msg.="The interns Sconet number (elenoet) proposed contains nonnumericals characters .";
				}
			}
		}
	}
		...
		if(mysql_num_rows($res_email_utilisateur_ele)>0) {
			...
			if($tmp_lig_email->email!="") {
				if($tmp_lig_email->email!=$eleve_email) {
					...
					if(!$update) {
						if(!isset($msg)) {$msg="";}
						$msg.="Error during the update of the mall of the student according to its to use account <br />$eleve_email -&gt; $tmp_lig_email->email<br />";
					}
					else {
						if(!isset($msg)) {$msg="";}
						$msg.="Update of the email of $eleve_login in the table 'eleves' according to the email of its to use account <br />$eleve_email -&gt; $tmp_lig_email->email<br />";
					}
				}
...
//**************** EN-TETE *****************
$titre_page = "Management of the students | Add/Modify a student card ";
...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	...
	if(isset($definir_resp)){
		if(!isset($valider_choix_resp)){
			...
			echo "<p class=bold><a href=\"modify_eleve.php?eleve_login=$eleve_login\" onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a></p>\n";
			...
			echo "<p align='center'><input type='submit' name='filtrage' value='Display' /> les ";
			echo "<input type='text' name='nb_resp' value='$nb_resp' size='3' />\n";
			echo " responsibles of which the<b>nom</b> contient: ";
			echo "<input type='text' name='critere_recherche' value='$critere_recherche' />\n";
			echo " starting from the recording ";
			...
			if ($nombreligne != 0) {
				echo "<p align='center'><input type='submit' name='valider_choix_resp' value='Save' /></p>\n";
				echo "<table align='center' class='boireaus' summary='Responsible'>\n";
				echo "<tr>\n";
				echo "<td><input type='radio' name='reg_resp".$definir_resp."' value='' onchange='changement();' /></td>\n";
				echo "<td style='font-weight:bold; text-align:center; background-color:#96C8F0;'><b>Legal responsible $definir_resp</b></td>\n";
				echo "<td style='font-weight:bold; text-align:center; background-color:#AAE6AA;'><b>Adress</b></td>\n";
				echo "</tr>\n";
				...
				echo "<p>No responsible is defined, or no responsible corresponds to research.</p>\n";
			}

			echo "<p>If it legal responsible  do not appear in the list, you can add it to the base<br />\n";
			echo "(<i>after, if necessary, safeguardhaving safeguarded this card</i>)<br />\n";
			if($_SESSION['statut']=="scolarite") {
				echo "while returning to [<a href='../responsables/index.php'>Management of the cards students responsibles </a>]</p>\n";
			}
			else{
				echo "while returning to [Gestion des bases-><a href='../responsables/index.php'>Management of the students responsibles </a>]</p>\n";
			}
	...
	if(isset($definir_etab)){
		if(!isset($valider_choix_etab)){
			...
			*/
			...
			echo "<!-- Form of research/filtering among the schools -->\n";
			echo "<form enctype='multipart/form-data' name='form_rech' action='modify_eleve.php' method='post'>\n";
			echo add_token_field();

			echo "<input type='hidden' name='eleve_login' value='$eleve_login' />\n";
			echo "<input type='hidden' name='definir_etab' value='$definir_etab' />\n";
			echo "<table border='0' summary='Filtering'>\n";
			echo "<tr>\n";
			echo "<td valign='top'>\n";
			//echo "<p align='center'>";
			echo "<input type='submit' name='Filtering' value='Display' /> les ";
			...
			echo "/> <label for='champ_rech_nom' style='cursor: pointer;'>the <b>name</b></label><br />\n";

			echo "<input type='radio' name='champ_rech' id='champ_rech_rne' value='id' ";
			if($champ_rech=="id") {echo "checked ";}
			echo "/> <label for='champ_rech_rne' style='cursor: pointer;'>name <b>RNE</b></label><br />\n";

			echo "<input type='radio' name='champ_rech' id='champ_rech_cp' value='cp' ";
			if($champ_rech=="cp") {echo "checked ";}
			echo "/> <label for='champ_rech_cp' style='cursor: pointer;'>name <b>postal code</b></label><br />\n";

			echo "<input type='radio' name='champ_rech' id='champ_rech_ville' value='ville' ";
			if($champ_rech=="ville") {echo "checked ";}
			echo "/> <label for='champ_rech_ville' style='cursor: pointer;'>the <b>city</b></label>\n";
			...
			echo "<input type='submit' name='filtrage2' value='Display selection' /> ou ";

			echo "<input type='button' name='afficher_tous' value='Display all schools' onClick=\"document.getElementById('afficher_tous_les_etab').value='y'; document.form_rech.submit();\" /></p>\n";
			echo "</form>\n";
			echo "</div>\n";
			echo "</div>\n";
			//====================================================
			...
			echo "<!-- Form of choice of the school -->\n";
			echo "<form enctype='multipart/form-data' name='form_choix_etab' action='modify_eleve.php' method='post'>\n";
			echo add_token_field();

			echo "<p>Choice of the school of origin for <b>".casse_prenom($eleve_prenom)." ".strtoupper($eleve_nom)."</b></p>\n";
			...
			if ($nombreligne != 0) {
				echo "<p align='center'><input type='submit' name='valider_choix_etab' value='Validate' /></p>\n";
				echo "<table align='center' class='boireaus' border='1' summary='School'>\n";
				echo "<tr>\n";
				...
				echo "<p>No school is defined</p>\n";
			}

			echo "<p>If a school does not appear in the list, you can add it to the base<br />\n";
			echo "while returning to  [Management of the bases-><a href='../etablissements/index.php' onclick=\"return confirm_abandon (this, change, '$themessage')\">Management of schools</a>]</p>\n";
...
if ((isset($order_type)) and (isset($quelles_classes))) {
	...
	echo "&amp;order_type=$order_type\" onclick=\"return confirm_abandon (this, change, '$themessage')\">Return to your research</a>\n";
}
/*
else {
    echo "<p class=bold><a href=\"index.php\"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Retour</a></p>\n";
...
if(isset($eleve_login)) {
	...
	if(($compte_eleve_existe=="y")&&(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite"))) {
		...
		echo "<div style='float:right; width:; height:;'><a href='".$_SERVER['PHP_SELF']."?eleve_login=$eleve_login&amp;journal_connexions=y#connexion' title='Log of connections'><img src='../images/icons/document.png' width='16' height='16' alt='Log of connections' /></a></div>\n";
	}
}

//echo "<table border='1'>\n";
echo "<table summary='Student information >\n";
echo "<tr>\n";
echo "<td>\n";
...
if (isset($eleve_login)) {
	echo "<th style='text-align:left;'>Identifier GEPI * : </th>
...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	...
	if((isset($eleve_email))&&($eleve_email!='')) {
		...
		if($tmp_date['hours']>=18) {echo "Good evening";} else {echo "Hello";}
		echo ",%0d%0aCordialement.'>";
		echo "<img src='../images/imabulle/courrier.jpg' width='20' height='15' alt='Send a email' border='0' />";
		echo "</a>";
	}
	echo "</td>\n";
	echo "</tr>\n";
	...
	echo "<tr>\n";
	echo "<th style='text-align:left;'>National Identifier : </th>\n";
    ...
    echo "<tr><th style='text-align:left;'>Interns Sconet number  (<i>elenoet</i>) : </th><td><input type='text' name='reg_no_gep' size='20' ";
    if (isset($reg_no_gep)) echo "value=\"".$reg_no_gep."\"";
    echo " onchange='changement();' /></td>\n";
	echo "</tr>\n";
	
    echo "<tr><th style='text-align:left;'>Interns Sconet number (<i>ele_id</i>) : </th><td>";
    if (isset($reg_ele_id)) {echo $reg_ele_id;}
    echo "</td>\n";
	echo "</tr>\n";
	
	//Date de sortie de l'établissement
    echo "<tr><th style='text-align:left;'>Date of exit of the school : <br/>(Respect the format JJ/MM/AAAA)</th>";
	...
	<tr>
    <th style='text-align:left;'>National Identifier : </th>\n";
    echo "<td>";
    ...
    //echo "<tr><td>Numéro GEP : </td><td><input type=text name='reg_no_gep' size=20 ";
    echo "<tr><th style='text-align:left;'>Interns Sconet number(<i>elenoet</i>) : </th><td>";
    if (isset($reg_no_gep)) {
		echo "$reg_no_gep";
	...
	if ($eleve_date_de_sortie!=0) {
		//Date de sortie de l'établissement
	    echo "<tr><th style='text-align:left;'>Date of exit of the school : <br/></th>";
		echo "<td><div class='norme'>";	
...
if(isset($reg_no_gep)){
  ...
  if ((getSettingValue("active_module_trombinoscopes")=='y') and (($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")||(($_SESSION['statut']=="professeur")&&(getSettingValue("GepiAccesGestPhotoElevesProfP")=='yes')))){
		...
		if($temoin_photo=="oui"){
			...
			echo "Send<br />a file<br />photo</a>\n";
		}
		//echo "</span>\n";
		echo "</div>\n";
		...
		if("$photo"!=""){
			if(file_exists($photo)){
				echo "<br />\n";
				echo "<input type='checkbox' name='suppr_filephoto' id='suppr_filephoto' value='y' onchange='changement();' /><label for='suppr_filephoto' style='cursor:pointer;'> Remove the existing photo</label>\n";
			}
		}
		echo "</div>\n";
		echo "</div>\n";
...
if(isset($eleve_login)){
	...
	if($_SESSION['statut']=="professeur") {
		...
		echo "<tr><th style='text-align:left;'>Born the: </th><td>$eleve_naissance_jour/$eleve_naissance_mois/$eleve_naissance_annee</td></tr>\n";
		if ($eleve_sexe == "M") {
			echo "<tr><th style='text-align:left;'>Sex: </th><td>Masculine</td></tr>\n";
		}
		elseif($eleve_sexe == "F"){
			echo "<tr><th style='text-align:left;'>Sexe: </th><td>Féminin</td></tr>\n";
		}

		echo "<tr><th style='text-align:left;'>Regim: </th><td>";
		if ($reg_regime == 'i-e') {
			echo "Intern-externed";
		}
		...
		if ($reg_doublant == 'R') {echo "<tr><th style='text-align:left;'>Redoubling</th><td>Oui</td></tr>\n";}
		...
		//=========================
		// AJOUT: boireaus 20071107
		echo "<table style='border-collapse: collapse; border: 1px solid black;' align='center'  summary='Regim'>\n";
		echo "<tr>\n";
		echo "<th>Régime: </th>\n";
		...
		echo "<table style='border-collapse: collapse; border: 1px solid black;' align='center' summary='Redoubling'>\n";
		echo "<tr>\n";
		echo "<th>Redoubling: </th>\n";
		...
		if(mysql_num_rows($res_grp1)==0){
			echo "The student is still not associated to a class.";
		}
		else{
			while($lig_classe=mysql_fetch_object($res_grp1)){
				...
				echo "<a href='../classes/eleve_options.php?login_eleve=$eleve_login&amp;id_classe=$lig_classe->id_classe&amp;quitter_la_page=y' target='_blank'>Followed courses</a> en $lig_classe->classe\n";
				echo "<br />\n";
...
if (($reg_no_gep == '') and (isset($eleve_login))) {
   ...
   echo "<font color='red'>CAUTION : This student does not have a number interns Sconet (<i>elenoet</i>). You will not be able to import the absences from files GEP/Sconet for this student.<br />You will not be able to define the school of origin of the student.<br />This student will not be able to appear in the module trombinoscope.</font>\n";
	...
	if(mysql_num_rows($test_sconet)>0){
		...
		if($lig_tmp->value=='1'){
			echo "<br />";
			echo "<font color='red'>You will not be able to either carry out the updates of its information from Sconet<br />(<i>the ELENOET and the ELE_ID will not correspond to the data of Sconet</i>).</font>\n";
		}
	}
}
//echo "\$eleve_no_resp1=$eleve_no_resp1<br />\n";
...
<tr><td><div class='norme'><b>Sex :</b> <br />
<?php
if (!(isset($eleve_sexe))) {$eleve_sexe="M";}
?>
<label for='reg_sexeM' style='cursor: pointer;'><input type=radio name=reg_sexe id='reg_sexeM' value=M <?php if ($eleve_sexe == "M") { echo "CHECKED" ;} ?> onchange='changement();' /> Masculine</label>
<label for='reg_sexeF' style='cursor: pointer;'><input type=radio name=reg_sexe id='reg_sexeF' value=F <?php if ($eleve_sexe == "F") { echo "CHECKED" ;} ?> onchange='changement();' /> Female</label>
</div></td>

<td><div class='norme'>
<b>Date of birth (respect format 00/00/0000) :</b> <br />
Day <input type=text name=birth_day size=2 onchange='changement();' value=<?php if (isset($eleve_naissance_jour)) echo $eleve_naissance_jour;?> />
Month<input type=text name=birth_month size=2 onchange='changement();' value=<?php if (isset($eleve_naissance_mois)) echo $eleve_naissance_mois;?> />
Year<input type=text name=birth_year size=4 onchange='changement();' value=<?php if (isset($eleve_naissance_annee)) echo $eleve_naissance_annee;?> />
...
if(getSettingValue('ele_lieu_naissance')=='y') {
	...
	else {echo "<span style='color:red'>Nondefinite</span>";}
	echo "\n";
}
?>
</div></td>
...
<p><b>Notice</b> :
<br />- The fields * are obligatory.</p>
<?php
}
...
if(isset($eleve_login)){
	...
	if ($nombreligne != 0) {
		...
		echo "<br />\n";
		echo "<hr />\n";
		echo "<h3>Sending of the bulletins by postal way</h3>\n";

		//echo "\$eleve_no_resp1=$eleve_no_resp1<br />\n";

		echo "<i>If you do not send the report cards by postal way, you can be unaware of this rubric.</i>";
		echo "<br />\n<br />\n";
		...
		if("$eleve_no_resp1"=="0"){
			...
			echo "<p>The legal responsible 1 is not defined ";
			if($_SESSION['statut']=="professeur") {
				echo ".";
			}
			else{
				...
				if (isset($motif_rech)) {echo "&amp;motif_rech=$motif_rech";}
				echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Define the legal responsible 1</a>";
			}
			echo "</p>\n";
		}
		else{
			...
			if(mysql_num_rows($res_resp)==0){
				...
				echo "<table border='0' summary='Legal responsible 1'>\n";
				echo "<tr valign='top'>\n";
				echo "<td rowspan='2'>The legal responsible 1 is: </td>\n";
				echo "<td>";
				if($_SESSION['statut']=="professeur") {
				...
				if($_SESSION['statut']!="professeur") {
					...
					echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Change responsible</a></td>\n";
				}
				echo "</tr>\n";
				...
				if(mysql_num_rows($res_adr)==0){
					...
					echo "<td colspan='2'>\n";
					if($_SESSION['statut']=="professeur") {
						echo "The address of legal responsible  the 1 is not defined.\n";
					}
					else{
						...
						echo " The address of the legal responsible 1 is not defined: <a href='../responsables/modify_resp.php?pers_id=$eleve_no_resp1&amp;quitter_la_page=y#adresse' target='_blank' onclick=\"return confirm_abandon (this, change, '$themessage');\">Define the address of the legal responsible 1</a>\n";
					}
					echo "</td>\n";
					$adr_id_1er_resp="";
				}
					...
					if($_SESSION['statut']!="professeur") {
						...
						echo "<a href='../responsables/modify_resp.php?pers_id=$eleve_no_resp1&amp;quitter_la_page=y#adresse' onclick=\"return confirm_abandon (this, change, '$themessage');\" target='_blank'>Modify the address of the responsible</a>\n";
						echo "</td>\n";
					}
				}
				echo "</tr>\n";
		...
		if("$eleve_no_resp2"=="0"){
			...
			if($_SESSION['statut']=="professeur") {
				echo "<p>The legal responsible 2 is not defined: </p>\n";
 			}
			else{
				echo "<p>The legal responsible 2 is not defined: <a href='".$_SERVER['PHP_SELF']."?eleve_login=$eleve_login&amp;definir_resp=2";
				if (isset($order_type)) {echo "&amp;order_type=$order_type";}
				if (isset($quelles_classes)) {echo "&amp;quelles_classes=$quelles_classes";}
				if (isset($motif_rech)) {echo "&amp;motif_rech=$motif_rech";}
				echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Define the legal responsible 2</a></p>\n";
			}
		}
		else{
			...
			if(mysql_num_rows($res_resp)==0){
				...
				if($temoin_tableau!="oui"){
					echo "<table border='0' summary='Legal responsible 2'>\n";
					$temoin_tableau="oui";
				}
				echo "<tr valign='top'>\n";
				echo "<td rowspan='2'>The legal responsible 2 is: </td>\n";
				if($_SESSION['statut']=="professeur") {
				...
				if(mysql_num_rows($res_adr)==0){
					...
					echo "<td colspan='2'>\n";
					if($_SESSION['statut']=="professeur") {
						echo "The address of the legal responsible 2 is not defined.\n";
					}
					else{
						...
						echo "L'adresse du responsable légal 2 n'est pas définie: <a href='../responsables/modify_resp.php?pers_id=$eleve_no_resp2&amp;quitter_la_page=y#adresse' target='_blank' onclick=\"return confirm_abandon (this, change, '$themessage');\">Define the address of the legal responsible 2</a>\n";
					}
					echo "</td>\n";
				}
				else{
					...
					if(($lig_adr->adr_id!="")&&($lig_adr->adr_id!=$adr_id_1er_resp)){
						...
						if(casse_mot("$chaine_adr1",'min')==casse_mot("$chaine_adr2",'min')){
							echo "$chaine_adr2<br />\n<span style='color: red;'>The addresses are identical, but are recorded under two different identifiers (<i>$adr_id_1er_resp et $lig_adr->adr_id</i>); you should modify the address to point towards the same identifier of address.</span>";
						}
						else{
							echo "$chaine_adr2";
						}
					...
					if($_SESSION['statut']!="professeur") {
						...
						echo "<a href='../responsables/modify_resp.php?pers_id=$eleve_no_resp2&amp;quitter_la_page=y#adresse' onclick=\"return confirm_abandon (this, change, '$themessage');\" target='_blank'>Modify the address of the responsible</a>\n";
						if((isset($adr_id_1er_resp))&&(isset($adr_id_2eme_resp))){
							if("$adr_id_1er_resp"!="$adr_id_2eme_resp"){
								...
								echo "' onclick=\"return confirm_abandon (this, change, '$themessage');\">Take the address of the other responsible</a>";
							}
						}
						echo "</td>\n";
					}
	...
	function affiche_message_raffraichissement() {
		document.getElementById('message_target_blank').innerHTML=\"Remind to refresh the page after modification of the address responsible.<br />However, if you modified information in the present page, record them before reloading the page.\";
	}
</script>\n";
		...
		if("$chaine_adr2"!=""){
			if("$chaine_adr1"!=""){
				if("$chaine_adr1"!="$chaine_adr2"){
					echo "<p><b>The addresses of the two legal responsibles are not identical. Consequently, the bulletin will be sent to the two legal responsibles.</b></p>\n";
				}
				else{
					echo "<p><b>The addresses of the two legal responsibles are identical. Consequently, the bulletin will be sent only to the first address.</b>";
					echo "</p>\n";
				}
			}
			else{
				echo "<p><b>The bulletin will be sent only to the second responsible.</b></p>\n";
			}
		}
		else{
			if("$chaine_adr1"!=""){
				echo "<p><b>The bulletin will be sent only to the first responsible.</b></p>\n";
			}
			else{
				echo "<p><b>No address is indicated. The bulletin could not be sent.</b></p>\n";
			}
		}
		...
		if(("$eleve_no_resp1"=="0")||("$eleve_no_resp2"=="0")){
			if($_SESSION['statut']=="professeur") {
				echo "<p>If the legal responsible does not appear in the list, contact the administrator or a person having the statute 'schooling'.</p>\n";
			}
			else{
				echo "<p>If the legal responsible does not appear in the list, you can add it to the base<br />\n";
				echo "(<i>after, if necessary, safeguardhaving safeguarded this card</i>)<br />\n";
				...
				if($_SESSION['statut']=="scolarite") {
					...
					echo "while returning to [Management of the bases-><a href='../responsables/index.php'>Management of students responsibles </a>]</p>\n";
				}
			}
		}
	}
...
if((isset($eleve_login))&&(isset($reg_no_gep))&&($reg_no_gep!="")) {
	...
	$res_etab=mysql_query($sql);
	if(mysql_num_rows($res_etab)==0) {
		echo "<p>The school of origin of the student is not indicated.";
		if($_SESSION['statut']!="professeur") {
			echo "<br />\n";
			...
			if (isset($quelles_classes)) {echo "&amp;quelles_classes=$quelles_classes";}
			if (isset($motif_rech)) {echo "&amp;motif_rech=$motif_rech";}
			echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Indicate the school of origin</a>";
		}
		echo "</p>\n";
	}
	else{
		...
		if("$lig_etab->id_etablissement"==""){
			if($_SESSION['statut']=="professeur") {
				echo "<p>The school of origin of the student is not indicated.</p>\n";
			}
			else{
				...
				if (isset($quelles_classes)) {echo "&amp;quelles_classes=$quelles_classes";}
				if (isset($motif_rech)) {echo "&amp;motif_rech=$motif_rech";}
				echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Define the school of origin</a>";
				echo "</p>\n";
			}
		}
		else{
			...
			if(mysql_num_rows($res_etab2)==0) {
				echo "<p>Association with the identifier of school exists (<i>$lig_etab->id_etablissement</i>), but corresponding information does not exist in the table 'etablissement'.";
				if($_SESSION['statut']!="professeur") {
					echo "<br />\n";
					...
					echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Modify the school of origin</a>";
				}
				echo "</p>\n";
			}
			else{
				echo "<p>The school of origin of the student is&nbsp;:<br />\n";
...
if((isset($eleve_login))&&($compte_eleve_existe=="y")&&($journal_connexions=='n')&&(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite"))) {
	...
	//if(($compte_eleve_existe=="y")&&(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite"))) {
		...
		echo "<p><a href='".$_SERVER['PHP_SELF']."?eleve_login=$eleve_login&amp;journal_connexions=y#connexion' title='Journal des connexions'>Log of connections</a></p>\n";
	//}
}
