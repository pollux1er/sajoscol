...
if((!isset($ele_login))&&(!isset($Recherche_sans_js))) {
	echo "<div class='norme'><p class='bold'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>\n";
	echo "</p>\n";
	echo "</div>\n";
	...
	<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post' name='formulaire1'>
		<p>
		Afficher les ".$gepiSettings['denomination_eleves']." of which <b>name</b> contains&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type='text' name='rech_nom' value='' />
	...
	<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post' name='formulaire1'>
		<p>
		Display the ".$gepiSettings['denomination_eleves']." of which <b>first name</b> contains&nbsp;: <input type='text' name='rech_prenom' value='' />
		<input type='hidden' name='page' value='$page' />
		<input type='submit' name='Recherche_sans_js' value='Research' />
		</p>
	</form>
	...
	*/
	...
	echo "<p>\n";
	echo "Display the".$gepiSettings['denomination_eleves']." of which <b>name</b> contains&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type='text' name='rech_nom' id='rech_nom' value='' onchange=\"affichage_et_action('nom')\" />\n";
	echo "<input type='hidden' name='page' value='$page' />\n";
	echo "<input type='button' name='Recherche' id='Recherche_nom' value='Research' onclick=\"cherche_eleves('nom')\" />\n";
	echo "</p>\n";
	echo "</form>\n";
	...
	echo "<p>\n";
	echo "Display the ".$gepiSettings['denomination_eleves']." of which <b>first name</b> contains&nbsp;: <input type='text' name='rech_prenom' id='rech_prenom' value='' onchange=\"affichage_et_action('prenom')\" />\n";
	echo "<input type='hidden' name='page' value='$page' />\n";
	echo "<input type='button' name='Recherche' id='Recherche_prenom' value='Research' onclick=\"cherche_eleves('prenom')\" />\n";
	echo "</p>\n";
	echo "</form>\n";
...
if(document.getElementById('rech_nom')) {document.getElementById('rech_nom').focus();}
	...
	if(isset($id_classe)) {
		...
		if(mysql_num_rows($res_ele)>0) {
			echo "<p>".ucfirst($gepiSettings['denomination_eleves'])." of the class of ".get_class_from_id($id_classe).":</p>\n";
	...
	if(mysql_num_rows($res_clas)>0) {
		echo "<p>Or choose a ".$gepiSettings['denomination_eleve']." in a class:</p>\n";
...
elseif(isset($Recherche_sans_js)) {
	...
	echo "<div class='norme'><p class='bold'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>\n";
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Choose another ".$gepiSettings['denomination_eleve']."</a>\n";
	echo "</p>\n";
	echo "</div>\n";
	...
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Choose another ".$gepiSettings['denomination_eleve']."/class</a>\n";
	...
	if(isset($id_classe)) {
		...
		*/
		...
		if($ele_login_prec!=""){
			...
			echo ">".ucfirst($gepiSettings['denomination_eleve'])." previous</a>";
		}
		...
		if($ele_login_suiv!=""){
			...
			echo ">".ucfirst($gepiSettings['denomination_eleve'])." next</a>";
		}
...
<noscript>
Have patience during the retrieval of the data...thank you.
</noscript>
</div>\n";
	// Avec ça, le message ne disparait pas quand on a désactivé JavaScript...

	echo "<script type='text/javascript'>
	document.getElementById('patience').innerHTML=\"Have patience during the retrieval of the data... thank you.\";
	...
	if(mysql_num_rows($res_ele)==0){
		...
		echo "<p>The ".$gepiSettings['denomination_eleve']." whose login would be $ele_login is not in the table 'eleves'.</p>\n";
	}
	else{
		//================================
		unset($day);
		...
		elseif($_SESSION['statut']=='scolarite') {
			if (getSettingValue("GepiAccesTouteFicheEleveScolarite")!='yes') {
			    ...
			    if(mysql_num_rows($test)==0) {
				    echo "<p>You are not responsible of a ".$gepiSettings['denomination_eleve']." whose login would be $ele_login.</p>\n";
				    require_once("../lib/footer.inc.php");
				    die();
			    }
			}
		...
		elseif($_SESSION['statut']=='professeur') {
			...
			$GepiAccesReport cardSimplePP=getSettingValue('GepiAccesReport cardSimplePP');
			if(($GepiAccesReport cardSimplePP=="yes")&&($is_pp=="y")) {
				$acces_bulletins="y";
			}

			if($acces_bulletins=='n') {
				$GepiAccesReport cardSimpleProfToutesClasses=getSettingValue('GepiAccesReport cardSimpleProfToutesClasses');
				if($GepiAccesReport cardSimpleProfToutesClasses=='yes') {
					$acces_bulletins="y";
				}
				else {
					$GepiAccesReport cardSimpleProfTousEleves=getSettingValue('GepiAccesReport cardSimpleProfTousEleves');
					if($GepiAccesReport cardSimpleProfTousEleves=='yes') {
						if ($eleve_classe_prof=="y") {
							$acces_bulletins="y";
						}
						else {
					...
					if($acces_bulletins=='n') {
						$GepiAccesReport cardSimpleProf=getSettingValue('GepiAccesReport cardSimpleProf');
						if($GepiAccesReport cardSimpleProf=='yes') {
							if ($eleve_groupe_prof=="y") {
								$acces_bulletins="y";
							}
							else {
...
/*
	...
	.boireaus td {
		...
		*/
		...
		$releve_formule_bas=getSettingValue("releve_formule_bas") ? getSettingValue("releve_formule_bas") : "Report to be preserved preciously. No duplicate will be delivered. - GEPI : free solution of management and follow-up of the school results.";
	...
	document.getElementById('patience').style.display='none';
		...
		*/
		...
		if($acces_responsables=="y") {
			...
			echo "'>";
			echo "<a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=responsables' onclick=\"affiche_onglet('responsables');return false;\">Responsibles</a>";
			echo "</div>\n";
		}
		...
		if($acces_enseignements=="y") {
			...
			echo "'>";
			echo "<a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=enseignements' onclick=\"affiche_onglet('enseignements');return false;\">Courses</a>";
			echo "</div>\n";
		}
		...
		if($acces_bulletins=="y") {
			...
			echo "'>";
			echo "<a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=bulletins' onclick=\"affiche_onglet('bulletins');return false;\">Report cards</a>";
			echo "</div>\n";
		}
		...
		if($acces_releves=="y") {
			...
			echo "'>";
			echo "<a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=releves' onclick=\"affiche_onglet('releves');return false;\">Reports booklet</a>";
			echo "</div>\n";
		}
		...
		if($acces_cdt=="y") {
			...
			echo "'>";
			echo "<a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=cdt' onclick=\"affiche_onglet('cdt');return false;\">Textbook</a>";
			echo "</div>\n";
		}
		...
		if($acces_fp=="y") {
			...
			echo "'>";
			echo "<a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=fp' onclick=\"affiche_onglet('fp');return false;\">All projects</a>";
			echo "</div>\n";
		}
		...
		if($acces_anna=="y") {
			...
			echo "'>";
			echo "<a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=anna' onclick=\"affiche_onglet('anna');return false;\">Prev. years</a>";
			echo "</div>\n";
		}
		//=====================================================================================
		...
		echo "background-color: ".$tab_couleur['eleve']."; ";
		echo "'>";
		echo "<h2>Information on the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";
		//affichage de la date de sortie de l'élève de l'établissement
		if ($tab_ele['date_sortie']!=0) {
		   echo "<span class=\"red\">Date of exit of the school : the ".affiche_date_sortie($tab_ele['date_sortie'])."<br/><br/></span>";;
		}
		
		echo "<table border='0' summary='Student infos '>\n";
		echo "<tr>\n";
		echo "<td valign='top'>\n";

			echo "<table class='boireaus' summary='Student infos (1)'>\n";
			...
			echo "</td></tr>\n";
			echo "<tr><th style='text-align: left;'>First name&nbsp;:</th><td>".$tab_ele['prenom']."</td></tr>\n";
			echo "<tr><th style='text-align: left;'>Sex&nbsp;:</th><td>".$tab_ele['sexe']."</td></tr>\n";
			echo "<tr><th style='text-align: left;'>Born";
			if($tab_ele['sexe']=='F') {echo "e";}
			echo " le&nbsp;:</th><td>".$tab_ele['naissance']."</td></tr>\n";
			if(isset($tab_ele['lieu_naissance'])) {echo "<tr><th style='text-align: left;'>at&nbsp;:</th><td>".$tab_ele['lieu_naissance']."</td></tr>\n";}

			echo "<tr><th style='text-align: left;'>Regim&nbsp;:</th><td>";
			if ($tab_ele['regime'] == "d/p") {echo "";}
			if ($tab_ele['regime'] == "ext.") {echo "External";}
			if ($tab_ele['regime'] == "int.") {echo "Intern";}
			...
			echo "<tr><th style='text-align: left;'>Redoubling&nbsp;:</th><td>";
		...
		if($acces_responsables=="y") {
			...
			echo "background-color: ".$tab_couleur['responsables']."; ";
			echo "'>";
			echo "<h2>Responsibles of the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";

			if((!isset($tab_ele['resp']))||(count($tab_ele['resp'])==0)) {
				echo "<p>No responsible was recorded.</p>\n";
			}
			else {
				echo "<table border='0' summary='Responsibles infos >\n";
				echo "<tr>\n";
				$cpt_resp_legal0=0;
				for($i=0;$i<count($tab_ele['resp']);$i++) {
					if($tab_ele['resp'][$i]['resp_legal']!=0) {
						echo "<td valign='top'>\n";
						echo "<p>Legal responsible <b>".$tab_ele['resp'][$i]['resp_legal']."</b></p>\n";

						echo "<table class='boireaus' summary='Responsibles infos (1)'>\n";
						echo "<tr><th style='text-align: left;'>Name:</th><td>";
						...
						echo "</td></tr>\n";
						echo "<tr><th style='text-align: left;'>First name:</th><td>".$tab_ele['resp'][$i]['prenom']."</td></tr>\n";
						echo "<tr><th style='text-align: left;'>Civility:</th><td>".$tab_ele['resp'][$i]['civilite']."</td></tr>\n";
						if($tab_ele['resp'][$i]['tel_pers']!='') {echo "<tr><th style='text-align: left;'>Tel.pers:</th><td>".$tab_ele['resp'][$i]['tel_pers']."</td></tr>\n";}
						if($tab_ele['resp'][$i]['tel_port']!='') {echo "<tr><th style='text-align: left;'>Tel.port:</th><td>".$tab_ele['resp'][$i]['tel_port']."</td></tr>\n";}
						if($tab_ele['resp'][$i]['tel_prof']!='') {echo "<tr><th style='text-align: left;'>Tel.prof:</th><td>".$tab_ele['resp'][$i]['tel_prof']."</td></tr>\n";}
						...
						if(!isset($tab_ele['resp'][$i]['etat'])) {
							echo "<tr><th style='text-align: left;'>Have an account:</th><td>No</td></tr>\n";
						}
						else {
							echo "<tr><th style='text-align: left;'>Have an account:</th><td>Yes (";
						...
						if($tab_ele['resp'][$i]['adr1']!='') {echo "<tr><th style='text-align: left;'>Line 1 adresse:</th><td>".$tab_ele['resp'][$i]['adr1']."</td></tr>\n";}
						if($tab_ele['resp'][$i]['adr2']!='') {echo "<tr><th style='text-align: left;'>Line 2 adresse:</th><td>".$tab_ele['resp'][$i]['adr2']."</td></tr>\n";}
						if($tab_ele['resp'][$i]['adr3']!='') {echo "<tr><th style='text-align: left;'>Line 3 adresse:</th><td>".$tab_ele['resp'][$i]['adr3']."</td></tr>\n";}
						if($tab_ele['resp'][$i]['adr4']!='') {echo "<tr><th style='text-align: left;'>Line 4 adresse:</th><td>".$tab_ele['resp'][$i]['adr4']."</td></tr>\n";}
						if($tab_ele['resp'][$i]['cp']!='') {echo "<tr><th style='text-align: left;'>Postal code:</th><td>".$tab_ele['resp'][$i]['cp']."</td></tr>\n";}
						if($tab_ele['resp'][$i]['commune']!='') {echo "<tr><th style='text-align: left;'>Commune:</th><td>".$tab_ele['resp'][$i]['commune']."</td></tr>\n";}
						if($tab_ele['resp'][$i]['pays']!='') {echo "<tr><th style='text-align: left;'>Country:</th><td>".$tab_ele['resp'][$i]['pays']."</td></tr>\n";}
				...
				if($cpt_resp_legal0>0) {
					echo "<table border='0' summary='Responsibles infos (0)'>\n";
					echo "<tr>\n";
					for($i=0;$i<count($tab_ele['resp']);$i++) {

						if($tab_ele['resp'][$i]['resp_legal']==0) {
							echo "<td valign='top'>\n";
							echo "<p>Contact (<i>not legal responsible</i>)</p>\n";

							echo "<table class='boireaus' summary='Infos resp0'>\n";
							echo "<tr><th style='text-align: left;'>Name:</th><td>".$tab_ele['resp'][$i]['nom']."</td></tr>\n";
							echo "<tr><th style='text-align: left;'>First name:</th><td>".$tab_ele['resp'][$i]['prenom']."</td></tr>\n";
							echo "<tr><th style='text-align: left;'>Civility:</th><td>".$tab_ele['resp'][$i]['civilite']."</td></tr>\n";
							if($tab_ele['resp'][$i]['tel_pers']!='') {echo "<tr><th style='text-align: left;'>Tel.pers:</th><td>".$tab_ele['resp'][$i]['tel_pers']."</td></tr>\n";}
							if($tab_ele['resp'][$i]['tel_port']!='') {echo "<tr><th style='text-align: left;'>Tel.port:</th><td>".$tab_ele['resp'][$i]['tel_port']."</td></tr>\n";}
							if($tab_ele['resp'][$i]['tel_prof']!='') {echo "<tr><th style='text-align: left;'>Tel.prof:</th><td>".$tab_ele['resp'][$i]['tel_prof']."</td></tr>\n";}
							if($tab_ele['resp'][$i]['mel']!='') {echo "<tr><th style='text-align: left;'>Email:</th><td>".$tab_ele['resp'][$i]['mel']."</td></tr>\n";}
		...
		if($acces_enseignements=="y") {
			...
			echo "background-color: ".$tab_couleur['enseignements']."; ";
			echo "'>";
			echo "<h2>Courses followed by the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";

			if((!isset($tab_ele['periodes']))||(!isset($tab_ele['groupes']))) {
				echo "<p>No period or no course was found for this ".$gepiSettings['denomination_eleve'].".</p>\n";
			}
			else {
				echo "<table class='boireaus' summary='Enseignements'>\n";
				echo "<tr>\n";
				echo "<th>Course</th>\n";
				echo "<th>Professor(s)</th>\n";
				...
				echo "<p><b>CPE charged of the follow-up</b>: ";
				...
				if($tab_ele['equipe_liste_email']!="") {
					...
					echo "<p>Write an email to <a href='mailto:".$tab_ele['equipe_liste_email']."?subject=GEPI - [".remplace_accents($tab_ele['nom'],'all')." ".remplace_accents($tab_ele['prenom'],'all')."]&amp;body=";
					if($tmp_date['hours']>=18) {echo "Good evening";} else {echo "Hello";}
					if(preg_match("/,/",$tab_ele['equipe_liste_email'])) {echo " to all";}
					echo ",%0d%0aCordially.'>all teachers and to the CPE of the student</a>.</p>\n";
				}
			}
			echo "</div>\n";
		}
		...
		if($acces_bulletins=="y") {
			...
			echo "<h2>Report cards of the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";
		...
		if($acces_releves=="y") {
			...
			echo "<h2>Reports booklet of the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";
		...
		if($acces_cdt=="y") {
			...
			echo "background-color: ".$tab_couleur['cdt']."; ";
			echo "'>";
			echo "<h2>Textbook of the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";

			echo "<p align='center'>";
			echo "<a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=cdt&amp;day=$j_sem_prec&amp;month=$m_sem_prec&amp;year=$y_sem_prec'><img src='../images/icons/back.png' width='16' height='16' alt='Previous week' /></a> ";
			echo "From ".jour_en_fr(date("D",$date_ct1))." ".date("d/m/Y",$date_ct1)." to ".jour_en_fr(date("D",$date_ct2))." ".date("d/m/Y",$date_ct2);
			echo " <a href='".$_SERVER['PHP_SELF']."?ele_login=$ele_login&amp;onglet=cdt&amp;day=$j_sem_suiv&amp;month=$m_sem_suiv&amp;year=$y_sem_suiv'><img src='../images/icons/forward.png' width='16' height='16' alt='next week' /></a>";
			echo "</p>\n";
			...
			echo "<tr><th>Date</th><th>Work to do</th><th>Report of meeting</th></tr>\n";
      ...
      if ($nb_aid>0) {
  			echo "<h2>All projects of the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";
        ...
        echo "</table>";
  			...
  			echo "<h2>The ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']." is currently registered in no project.</h2>\n";
      }
      ...
      $nb_aid = mysql_num_rows($call_data);
      if ($nb_aid>0) {
        echo "<h2>Projects of the former years</h2>";
      ...
      if ($test_plugin=='y') {
		...
		if($acces_absences=="y") {
			...
			if(getSettingValue("active_module_absence")=='y' || getSettingValue("abs2_import_manuel_bulletin")=='y') {
			    echo "<h2>Absences and delays of the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";

			    if(count($tab_ele['absences'])==0) {
				    echo "<p>No assessment of absences is recorded.</p>\n";
			    }
			    else {
				    ...
				    echo "<th>Not justified absences</th>\n";
			    ...
			    if ($tab_ele['abs_quotidien']['autorisation'] == 'oui') {
				    // On affiche
				    echo '<br /><p class="bold">The detail of the recorded absences : </p>';

				    echo '
				    <table class="boireaus" style="margin-left: 4em;" summary="Detail of the absences">
					    <tr>
						    <th>R/A</th>
						    <th>Jour</th>
						    <th>Heure</th>
			    ...
			    echo "<h2>Absences and lateness of the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";
			    ...
			    echo "<tr>\n";
			    echo "<th>Period</th>\n";
			    echo "<th>Number of absences<br/>(1/2 days)</th>\n";
			    echo "<th>Not justified absences</th>\n";
			    echo "<th>Number of delays</th>\n";
			    echo "<th>Appreciation</th>\n";
			    echo "</tr>\n";
		...
		if(($gepiVersion=='trunk')||
			...
			if($acces_discipline=="y") {
				...
				echo "background-color: ".$tab_couleur['discipline']."; ";
				echo "'>";
				echo "<h2>Incidents \"concerning\" the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";
				...
				echo "<p>Extract the incidents between ";
				...
				echo "<input type='submit' name='restreindre_intervalle_dates' value='Validate' />\n";
		...
		if($acces_anna=="y") {
			...
			echo "background-color: ".$tab_couleur['anna']."; ";
			echo "'>";
			echo "<h2>Data of former years of the ".$gepiSettings['denomination_eleve']." ".$tab_ele['nom']." ".$tab_ele['prenom']."</h2>\n";
			...
			echo "<p>List of school years and periods for which data were preserved for this ".$gepiSettings['denomination_eleve']." :</p>\n";
			...
			if(mysql_num_rows($res_ant)==0){
				echo "<p>No former result was preserved for this ".$gepiSettings['denomination_eleve'].".</p>\n";
			}
			else{
				...
				//echo "<p>Report cards simplifiés:</p>\n";
				//echo "<table border='0'>\n";
				echo "<table class='boireaus' summary='Report cards'>\n";
				$alt=1;
				echo "<tr class='lig$alt'>\n";
				echo "<th rowspan='".$nb_annees."' valign='top'>Simplified bulletins:</th>";
				$cpt=0;
				while($lig_ant=mysql_fetch_object($res_ant)){
					...
					if(mysql_num_rows($res_ant2)==0){
						echo "<td>No former result was preserved for this ".$gepiSettings['denomination_eleve'].".</td>\n";
					}
					else{
				...
				//echo "<table border='0'>\n";
				echo "<table class='boireaus' summary='Opinion of the councils'>\n";
				$alt=1;
				echo "<tr class='lig$alt'>\n";
				echo "<th rowspan='".$nb_annees."' valign='top'>Opinion of the staff meetings:</th>";
				$cpt=0;
