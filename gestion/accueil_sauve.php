<?php
/*
 * $Id: accueil_sauve.php 8706 2011-12-05 06:57:45Z tbelliard $
 *
...
//echo "\$sav_file['tmp_name']=".$sav_file['tmp_name']."<br />";
    ...
    if (!isset($sav_file['tmp_name']) or ($sav_file['tmp_name'] =='')) {
        ...
        if (!deplacer_fichier_upload($sav_file['tmp_name'], "../backup/".$dirname."/".$nom_corrige)) {
            $msg = "Problem of transfer : the file could not be transferred on the repertory backup";
        } else {
...
if (isset($action) and ($action == 'sup'))  {
    ...
    if (isset($_GET['file']) && ($_GET['file']!='')) {
        if (@unlink("../backup/".$dirname."/".$_GET['file'])) {
            $msg = "The file <b>".$_GET['file']."</b> was removed.<br />\n";
        } else {
            $msg = "A problem occurred during attempt of removal of the file <b>".$_GET['file']."</b>.<br />
            They are perhaps a problem of rights on the repertory backup.<br />\n";
        }
    }
}
...
if (isset($action) and ($action == 'protect'))  {
    ...
    if(empty($_POST['pwd1_backup']) || empty($_POST['pwd2_backup'])) {
        $msg = "Problem: the two passwords are not identical or are empty.";
        $error = 1;
    } elseif ($_POST['pwd1_backup'] != $_POST['pwd2_backup']) {
        $msg = "Problem: the two passwords are not identical.";
        $error = 1;
    } elseif (empty($_POST['login_backup'])) {
        $msg = "Problem: the identifier is empty.";
        $error = 1;
    } else {
...
if (isset($action) and ($action == 'del_protect'))  {
   ...
   if ((@unlink("../backup/".$dirname."/.htaccess")) and (@unlink("../backup/".$dirname."/.htpasswd"))) {
       $msg = "Files .htaccess and .htpasswd were removed. The repertory /backup is not protected any more\n";
   }
}
...
function backupMySql($db,$dumpFile,$duree,$rowlimit) {
    ...
    if (mysql_error()) {
       echo "<hr />\n<font color='red'>ERROR during safeguard cause by a problem in the base.</font><br />".mysql_error()."<hr/>\n";
       return false;
       die();
    }
    ...
    for (;$offsettable<$numtab;$offsettable++){
        ...
        if ($offsetrow==-1){
            ...
            if (isset($debug)) echo "<b><br />Dump of the structure of the table ".$tables[$offsettable]."</b><br />\n";
            fwrite($fileHandle,$todump);
            $offsetrow++;
            $cpt++;
        }
        ...
        if ($duree>0 and $TPSCOUR>=$duree) //on atteint la fin du temps imparti
            return TRUE;
        if (isset($debug)) echo "<b><br />Dump of the data of the table ".$tables[$offsettable]."<br /></b>\n";
        $fin=0;
        while (!$fin){
            ...
            if ($rowtodump>0){
                ...
                current_time();
                if ($duree>0 and $TPSCOUR>=$duree) {//on atteint la fin du temps imparti
                    if (isset($debug)) echo "<br /><br /><b>Number of lines currently in the file: ".$cpt."</b><br />\n";
                    return TRUE;
                }
            } else {
                $fin=1;$offsetrow=-1;
        ...
        if (isset($debug)) echo "For this table, numbers of safeguarded lines: ".$offsetrow."<br />\n";
        if ($fin) $offsetrow=-1;
        current_time();
        if ($duree>0 and $TPSCOUR>=$duree) //on atteint la fin du temps imparti
            return TRUE;
    ...
    $offsettable=-1;
    $todump ="#\n";
    $todump.="# ******* End of the file - The safeguard finished normally ********\n";
    fwrite ($fileHandle,$todump);
    fclose($fileHandle);
    return TRUE;
}
...
function restoreMySqlDump($duree) {
	...
	if(mysql_num_rows($res)>0) {
		...
		echo "<p style='text-align:center;'>File $nb_tables_passees/$nb_tables</p>\n";

		echo "<p>Treatment of the table <span style='color:green;'>$nom_table</span><br />";
		...
		while(!gzeof($fileHandle)) {
			...
			if ($duree>0 and $TPSCOUR>=$duree) {  //on atteint la fin du temps imparti
				if ($old_offset == $offset) {
					echo "<p align=\"center\"><b><font color=\"#FF0000\">The procedure of restoration cannot continue.
					<br />A problem occurred during of the treatment of a request close to:.
				...
				if ($formattedQuery) {
					...
					if (mysql_query($sql)) {//réussie sinon continue à concaténer
						if(my_ereg("^DROP TABLE ",$sql)) {
							echo "Removal of the table <span style='color:green;'>$nom_table</span> if it exists.<br />";
						}
						elseif(my_ereg("^CREATE TABLE ",$sql)) {
							echo "Creation of the table <span style='color:green;'>$nom_table</span> according to the backup.<br />";
						}
						else {
							if($cpt_insert==0) {
								//echo "<div style='width:100%'>";
								echo "Restoration of the recordings of the table <span style='color:green;'>$nom_table</span> according to the backup: ";
							}
							else {
		...
		if($cpt_insert>0) {
			echo "<br />";
			echo "$cpt_insert recording(s) restored.";
		}

		if (mysql_error()) {
			echo "<hr />\nERROR from [$formattedQuery]<br />".mysql_error()."<hr />\n";
		}
		gzclose($fileHandle);
		...
		if($debug_restaure=='y') {
			if($nettoyage=mysql_query($sql)) {
				echo "Success of the suppression in a_tmp_setting.<br />\n";
			}
			else {
				echo "<p style='color:red;'>Error during suppression in 'a_tmp_setting'.</p>\n";
			}

			if(unlink($dumpFile)) {
				echo "Success of the suppression of $dumpFile.<br />";
			}
			else {
				echo "<p style='color:red;'>Error during suppression of $dumpFile.</p>\n";
			}
		}
		else {
			if(!$nettoyage=mysql_query($sql)) {
				...
				while($lig=mysql_fetch_object($res)) {
					...
					while(!gzeof($fileHandle)) {
						...
						if ($duree>0 and $TPSCOUR>=$duree) {  //on atteint la fin du temps imparti
							if ($old_offset == $offset) {
								echo "<p align=\"center\"><b><font color=\"#FF0000\">The procedure of restoration cannot continue.
								<br />A problem occurred during treatment of a request close to :.
								<br />".$debut_req."</font></b></p><hr />\n";
								return FALSE;
							}
							$old_offset = $offset;
							...
							if ($formattedQuery) {
								...
								if (mysql_query($sql)) {//réussie sinon continue à concaténer
									...
									elseif(my_ereg("^CREATE TABLE ",$sql)) {
										...
										if($cpt_insert==0) {
											//echo "<div style='width:100%'>";
											echo "Restoration of the record of the table <span style='color:green;'>$nom_table</span> according to the backup: ";
										}
										else {
											echo "<span style='font-size: xx-small;'>. </span>";
										}
					...
					if($cpt_insert>0) {
						echo "<br />";
						echo "$cpt_insert record(s) restored(s).";
						//echo "</div>\n";
					}
	
					if (mysql_error())
						echo "<hr />\nERROR starting from [$formattedQuery]<br />".mysql_error()."<hr />\n";
...
function extractMySqlDump($dumpFile,$duree) {
    ...
    if(!$fileHandle) {
        echo "Opening of $dumpFile impossible.<br />\n";
        return FALSE;
    }

    if ($offset!=0) {
        if (gzseek($fileHandle,$offset,SEEK_SET)!=0) { //erreur
            echo "Impossible to find the byte ".number_format($offset,0,""," ")."<br />\n";
            return FALSE;
        }
        flush();
    }
    ...
    while(!gzeof($fileHandle)) {
		...
		if(substr($buffer, 0, 1) != "#" AND substr($buffer, 0, 1) != "/" AND trim($buffer)!='') {
			...
			if(isset($fich)) {
				...
				echo "Not recorded: \$buffer=$buffer<br />";
			}
		}
	}
	if(isset($fich)) {fclose($fich);}
...
function restoreMySqlDump_old($dumpFile,$duree) {
    ...
    while(!gzeof($fileHandle)) {
        ...
        if ($duree>0 and $TPSCOUR>=$duree) {  //on atteint la fin du temps imparti
            if ($old_offset == $offset) {
                echo "<p align=\"center\"><b><font color=\"#FF0000\">The procedure of restoration cannot continue.
                <br />A problem occurred during the treatment of a request close to :.
                <br />".$debut_req."</font></b></p><hr />\n";
                return FALSE;
            }
            $old_offset = $offset;
...
function get_content($db, $table,$from,$limit) {
    ...
    if (!$resData) {
        $def .="Problem with the data of $table, possible corruption !\n";
    } else {
...
//**************** EN-TETE *****************
$titre_page = "Management tool | Backups/Restoration";
...
if ($test_write == 'no') {
    echo "<h3 class='gepi'>Problem of access rights :</h3>\n";
    echo "<p>The repertory \"/backup\" is not accessible in writing.</p>\n";
    echo "<p>You thus cannot access to the functions of backup/restoration of GEPI.
    Contact the technical administrator in order to solve this problem.</p>\n";
    require("../lib/footer.inc.php");
    die();
}

if (!function_exists("gzwrite")) {
    echo "<h3 class='gepi'>Problem of configuration :</h3>\n";
    echo "<p>Functions of compression 'zlib' are not activated. You must configure PHP so that it uses 'zlib'.</p>\n";
    echo "<p>You thus cannot access to the functions of backup/restoration of GEPI.
    Contact the technical administrator in order to solve this problem.</p>\n";
    require("../lib/footer.inc.php");
    die();
...
if (isset($action) and ($action == 'restaure_confirm'))  {
    ...
    echo "<h3>Confirmation of the restoration of the base</h3>\n";
    echo "File selected for the restoration : <b>".$_GET['file']."</b>\n";
    echo "<p><b>CAUTION :</b> The procedure of restoration of the base is <b>irreversible</b>. The file of restoration must be valid. According to the contents of this file, whole or part of the current structure of the base and the existing data can be removed and replaced by the structure
and the data present in the file.
    <br /><br />\n<b>WARNING :</b> This procedure can be very long  according to the quantity of data to be restored.</p>\n";
    ...
    echo "<td>\n";
		...
		echo "</td>\n";
		echo "<td align='left'>\n";
		echo "<input type=\"checkbox\" name=\"debug_restaure\" id=\"debug_restaure\" value=\"y\" /><label for='debug_restaure' style='cursor:pointer;'>Activate the debug mode</label><br />\n";

		echo "<input type=\"checkbox\" name=\"ne_pas_restaurer_log\" id=\"ne_pas_restaurer_log\" value=\"y\" /><label for='ne_pas_restaurer_log' style='cursor:pointer;'> Not restore the records of the table 'log'.</label><br />\n";

		echo "<input type=\"checkbox\" name=\"ne_pas_restaurer_tentatives_intrusion\" id=\"ne_pas_restaurer_tentatives_intrusion\" value=\"y\" /><label for='ne_pas_restaurer_tentatives_intrusion' style='cursor:pointer;'> Not restore the records of the table 'tentatives_intrusion'.</label><br />\n";

		echo "---<br />";
		echo "<input type=\"checkbox\" name=\"restauration_old_way\" id=\"restauration_old_way\" value=\"y\" /><label for='restauration_old_way' style='cursor:pointer;'> Restore the backup in block<br />(<i>useful for example to restore a file SQL not corresponding to a traditional backup</i>).</label><br />\n";
...
if (isset($action) and ($action == 'restaure'))  {
	...
	if($restauration_old_way=='y') {
		...
		if ($percent >= 0) {
			...
			echo "<div align='center'><table class='tab_cadre' width='400'><tr><td width='400' align='center'><b>Restoration in progress</b><br /><br />Progression ".$percent."%</td></tr><tr><td><table><tr><td bgcolor='red'  width='$percentwitdh' height='20'>&nbsp;</td></tr></table></td></tr></table></div>\n";
		}
		flush();
		if ($offset!=-1) {
			if (restoreMySqlDump_old($path.$file,$duree)) {
				if (isset($debug)) {
					echo "<br />\n<b>Click <a href=\"accueil_sauve.php?action=restaure&file=".$file."&duree=$duree&offset=$offset&cpt=$cpt&path=$path&restauration_old_way=$restauration_old_way".add_token_in_url()."\">here</a> to continue the restoration</b>\n";
				}

				if (!isset($debug)) {
					echo "<br />\n<b>Automatic redirection if not click <a href=\"accueil_sauve.php?action=restaure&file=".$file."&duree=$duree&offset=$offset&cpt=$cpt&path=$path&restauration_old_way=$restauration_old_way".add_token_in_url()."\">here</a></b>\n";
				}
			...
			echo "<div align='center'><p>Restoration finished .<br /><br />Your GEPI session is not valid any more, you must reconnect yourself<br /><a href = \"../login.php\">Connect</a></p></div>\n";
			require("../lib/footer.inc.php");
			die();
		}
		//===============================================
		...
		echo "<div align='center'><b>Restoration in progress</b></div>\n";
...
value VARCHAR(255) NOT NULL);";
		...
		if($succes_etape!="y") {
			...
			echo "<p style='color:red'>An error occurred!<br />";
			...
			if(mysql_num_rows($res)==0) {
				...
				//background-color: white;
					...
					echo "<p>Restoration finished .<br /><br />Your session GEPI is not valid any more, you should reconnect yourself<br /><a href=\"../login.php\">Connect</a></p>\n";
...
if (isset($action) and ($action == 'dump'))  {
    ...
    if (!isset($_GET["duree"])&&is_file($filename)){
        echo "<font color=\"#FF0000\"><center><b>The file already exists. Have patience a minute before retry the backup.</b></center></font>\n<hr />\n";
    } else {
        init_time(); //initialise le temps
        //début de fichier
        ...
        if ($percent >= 0) {
            ...
            <tr><td width='400' align='center'><b>Backup in progress</b><br/>
            <br/>At the end of the backup, Gepi will automatically propose you to download the file.
        ...
        if ($offsettable>=0){
            if (backupMySql($dbDb,$fichier,$duree,$rowlimit)) {
                if (isset($debug)) {
					echo "<br />\n<b>Click <a href=\"accueil_sauve.php?action=dump&amp;duree=$duree&amp;rowlimit=$rowlimit&amp;offsetrow=$offsetrow&amp;offsettable=$offsettable&amp;cpt=$cpt&amp;fichier=$fichier&amp;path=$path";
					if(isset($quitter_la_page)) {echo "&amp;quitter_la_page=y";}
					echo add_token_in_url();
					echo "\">here</a> to continue the backup.</b>\n";
				}
                if (!isset($debug)) {
					echo "<br />\n<b>Automatic redirection if not click <a href=\"accueil_sauve.php?action=dump&amp;duree=$duree&amp;rowlimit=$rowlimit&amp;offsetrow=$offsetrow&amp;offsettable=$offsettable&amp;cpt=$cpt&amp;fichier=$fichier&amp;path=$path";
					if(isset($quitter_la_page)) {echo "&amp;quitter_la_page=y";}
					echo add_token_in_url();
					echo "\">ici</a></b>\n";
				}
            ...
            echo "<div align='center'><p>Backup finished.<br />\n";
            ...
            echo "<br/><p class=grand><a href='savebackup.php?fileid=$fileid'>Download the file generated by the backup</a></p>\n";
            echo "<br/><br/><a href=\"accueil_sauve.php";
			if(isset($quitter_la_page)) {echo "?quitter_la_page=y";}
			echo "\">Return to the interface of backup/restoration</a><br /></div>\n";
			require("../lib/footer.inc.php");
            die();
        }
...
if (isset($action) and ($action == 'system_dump'))  {
	...
	if (filesize($filename) > 10000) {
		echo "<center><p style='color: red; font-weight: bold;'>The backup was carried out successfully.</p></center>\n";
	} else {
		echo "<center><p style='color: red; font-weight: bold;'>Error during the backup.</p></center>\n";
	}
}
...
if (isset($action) and ($action == 'zip'))  {
  ...
  if (isset($dossier_a_archiver)) {
        ...
        case "cdt":
			...
			if((isset($GLOBALS['multisite']) AND $GLOBALS['multisite'] == 'y')) {
				if((isset($_COOKIE['RNE']))&&($_COOKIE['RNE']!='')) {
					if(!preg_match('/^[A-Za-z0-9]*$/', $_COOKIE['RNE'])) {
						echo "<p style='color:red; text-align:center'>Invalid RNE &nbsp;: ".$_COOKIE['RNE']."</p>\n";
						$chemin_stockage="";
					}
					else {
					...
					echo "<p style='color:red; text-align:center'>Invalid RNE .</p>\n";
					$chemin_stockage="";
				}
			}
			else {
			...
			if ($chemin_stockage !='') {
				...
				if ($zip_error != 0) {
					...
					echo "<p style='color:red; text-align:center;'>Zip was created.</p>";
				}
		...
		default:
			$chemin_stockage = '';
			echo "<p style='color:red; text-align:center;'>The nature of archiving to be carried out is unknown.</p>";
		}
	}
}
...
if (!(file_exists("../backup/".$dirname."/.htaccess")) or !(file_exists("../backup/".$dirname."/.htpasswd"))) {
    echo "<h3 class='gepi'>Repertory backup not protected :</h3>\n";
    echo "<p><font color=\"#FF0000\"><b>The repertory \"/backup\" is currently not protected</b></font>.
    If you store files in this repertory, they will be accessible from outside using a simple navigator.</p>\n";
    echo "<form action=\"accueil_sauve.php\" name=\"protect\" method=\"post\">\n";
	echo add_token_field();
    echo "<table><tr><td>New identifier : </td><td><input type=\"text\" name=\"login_backup\" value=\"\" size=\"20\" /></td></tr>\n";
    echo "<tr><td>New password : </td><td><input type=\"password\" name=\"pwd1_backup\" value=\"\" size=\"20\" /></td></tr>\n";
    echo "<tr><td>Confirmation of the password : </td><td><input type=\"password\" name=\"pwd2_backup\" value=\"\" size=\"20\" /></td></tr></table>\n";

    echo "<p align=\"center\"><input type=\"submit\" Value=\"Send\" /></p>\n";
    echo "<input type=\"hidden\" name=\"action\" value=\"protect\" />\n";
    echo "</form>\n";
    echo "<hr />\n";
} else {
    echo " | <a href='#' onClick=\"clicMenu('2')\" style=\"cursor: hand\"><b>Protection of the repertory backup</b></a>\n";
    ...
    echo "
			<p>The repertory \"/backup\" is currently protected by an identifier and a password.
			For access the files stored in this repertory starting from a navigator Web, it is necessary to be authenticated.
			<br /><br />Click on the button below <b>to remove protection</b>
			or to define new <b>identifier and a password</b></p>\n";
    echo "
			<form action=\"accueil_sauve.php\" name=\"del_protect\" method=\"post\">\n";
	echo add_token_field();
    echo "
			<p align=\"center\"><input type=\"submit\" Value=\"Modify/Remove the protection of the repertory\" /></p>\n";
    echo "
...
<H3>Create a file of backup/restoration of the base <?php echo $dbDb; ?></H3>
<p>Two methods of backup are available: the use of the system command mysqldump or the integrated system into Gepi.<br/>
First method (mysqldump) is vigorously recommended because much less heavy in resources, but will function only on certain servers configurations.<br />
The second method is heavy in resources but will pass on all the configurations.</p>
...
<b>with mysqldump</b> (this option was forced)

<?php
} else if ($force_backup_mode == 'dump') {
?>
<input type='hidden' name='action' value='system_dump' />
<b>without mysqldump</b> (this option was forced)
<?php
} else {
?>
<select name='action' size='1'>
<option value='system_dump'<?php if (getSettingValue("mode_sauvegarde") == "mysqldump") echo " SELECTED";?>>with mysqldump</option>
<option value='dump'<?php if (getSettingValue("mode_sauvegarde") == "gepi") echo " SELECTED";?>>without mysqldump</option>
</select>
...
<span class='small'><b>Remarks</b> :</span>
<ul>
<li><span class='small'>Repertories "documents" (containing the documents joined to the textbooks) and "photos" (containing the photographs of the trombinoscope) will not be safeguarded.<br/>
A specific tool of backup is in bottom of <a href='#zip'>this page</a>.</span></li>
<li><span class='small'>Value of <b>duration of a portion</b> in seconds : <input type="text" name="duree" value="<?php echo $_SESSION['defaulttimeout']; ?>" size="5" />
<input type='checkbox' name='sauve_duree' value='yes' /> Memorize the duration of the portion for the next time
<br/><a href='#' onClick="clicMenu('1')" style="cursor: hand">Display/Hide the help</a>.</span></li>
</ul>
</form>
<div style="display:none" id="menu1">
<table border="1" cellpadding="5" bgcolor="#C0C0C0"><tr><td>The <b>value of the duration of a portion</b> must be lower than
<b>maximum value of execution of a script</b> on the server (max_execution_time).
<br />
<br />According to the size of the base and the configuration of the server,
the backup or the restoration can fail if time necessary to this operation is higher
than the maximum time authorized for the execution of a script (max_execution_time).
<br />
A message of the type "Maximum execution time exceeded" appears then, indicating to you that the process failed.
<br /><br />
For stage that, <b>this script backup and restores "by portions" of a duration fixed
by the user</b> by continue the process at the place where it stopped previously
until the operation of backup or restoration is finished.
</td></tr></table>
</div>
<hr />
...
if ($n > 0) {
    echo "<h3>Files of restoration</h3>\n";
    echo "<p>The table below indicates the list of the files of restoration currently stored in the repertory \"backup\" at the root of GEPI.</p>\n";
    echo "<center>\n<table class='boireaus' cellpadding=\"5\" cellspacing=\"1\">\n<tr><th><b>Name of the backup file</b></th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th></tr>\n";
    $m = 0;
	$alt=1;
    foreach($tab_file as $value) {
		...
		if ((my_ereg('^_photos',$value)&&my_ereg('.zip$',$value))||(my_ereg('^_cdt',$value)&&my_ereg('.zip$',$value))){
		   echo "<td> </td>\n";
            ...
            echo "<td><a href='accueil_sauve.php?action=restaure_confirm&amp;file=$value".add_token_in_url()."'>Restore</a></td>\n";
		}
        echo "<td><a href='savebackup.php?fileid=$m'>Download</a></td>\n";
        echo "<td><a href='../backup/".$dirname."/".$value."'>Direct Download.</a></td>\n";
        echo "</tr>\n";
        $m++;
    }
    clearstatcache();
...
echo "<h3>Uploader a file (of restauration) towards the repertory backup</h3>\n";
echo "<form enctype=\"multipart/form-data\" action=\"accueil_sauve.php\" method=\"post\" name=\"formulaire2\">\n";
echo add_token_field();
$sav_file="";
echo "The files of backup are safeguarded in a sub-directory of the repertory \"/backup\", whose name changes in a random way regularly.
If you wish it, you can upload a file of backup directly in this repertory.
Once that made, you will be able to select it in the list of the files of restoration,on this page.\n";

echo "<br />You can also directly download the file by ftp in the repertory \"/backup\".\n";

echo "<br /><br /><b>File to \"upload\" </b>: <input type=\"file\" name=\"sav_file\" />
<input type=\"hidden\" name=\"action\" value=\"upload\" />
<input type=\"submit\" value=\"Validate\" name=\"bouton1\" />
</form>
<br />\n";
...
echo "<p style='margin-left: 20px;'>According to the configuration of the server and the size of
the file, the operation of download towards the repertory \"/backup\" can fail
(<i>for example if the size of the file exceeds <b>maximum size authorized during the downloads</b></i>).
<br />Si c'est le cas, signalez le problème à l'administrateur du serveur.</p>\n";

//echo "<table border='1' align='center'>\n";
echo "<table class='boireaus' align='center'>\n";
echo "<tr><th style='font-weight: bold; text-align: center;'>Variable</th><th style='font-weight: bold; text-align: center;'>Value</th></tr>\n";
...
echo "<br /><hr />";
echo "<h3 id=\"zip\">Create an archive (Zip) of folders of Gepi</h3>\n";
echo "Once created, to download the file, go to the section \"Files of restoration\" of this page. <br />";
echo "<p style=\"color: red;\">CAUTION : take care to remove the file created once the file is downloaded.</p>";
echo "<form enctype=\"multipart/form-data\" action=\"accueil_sauve.php\" method=\"post\" name=\"formulaire3\">\n";
echo add_token_field();
echo "<br />Folder to be safeguarded :<br />";
echo "<input type=\"radio\" name=\"dossier\" id=\"dossier_photos\" value=\"photos\" checked/><label for='dossier_photos'> Photos Folder(_photos_le_DATE_a_HEURE.zip)</label><br />\n";
echo "<input type=\"radio\" name=\"dossier\" id=\"dossier_cdt\" value=\"cdt\" /><label for='dossier_cdt'> Folder files of textbook (_cdt_le_DATE_a_HEURE.zip)</label><br />\n";
echo "<br />\n";
echo "<input type=\"hidden\" name=\"action\" value=\"zip\" />\n
	  <input type=\"submit\" value=\"Create archive\" name=\"bouton3\" />\n
	  </form>\n";

echo "<br /><hr />";

echo "<h3 id=\"rw\">Control of folders having to be accessible in writing for the backup/restoration</h3>\n";

$tab_restriction=array("backup");
test_ecriture_dossier($tab_restriction);
echo "<br />\n";
echo "<p>Other files and folders having to be accessible in writing can be controlled in the page <a href='../mod_serveur/test_serveur.php'>Server configuration </a>";
echo "<br />\n";
echo "&nbsp;\n";
echo "</p>\n";
