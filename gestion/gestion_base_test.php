...
if (isset($action) and ($action == 'upload'))  {
    ...
    if (!isset($sav_file['tmp_name']) or ($sav_file['tmp_name'] =='')) {
        ...
        $msg = "Error : only the files having the extension sql or gz are authorized.";
    } else {
        $dest = "../backup/".$dirname."/";
        $n = 0;
...
function backupMySql($db,$dumpFile,$duree,$rowlimit) {
    ...
    if (mysql_error()) {
       echo "<hr />\n<font color='red'>ERROR during backup cause by a problem in the base.</font><br />".mysql_error()."<hr/>\n";
       return false;
       die();
    }
    ...
    for (;$offsettable<$numtab;$offsettable++){
        ...
        if (isset($debug)) echo "For this table, number of safeguarded lines : ".$offsetrow."<br />\n";
        if ($fin) $offsetrow=-1;
        current_time();
        if ($duree>0 and $TPSCOUR>=$duree) //on atteint la fin du temps imparti
            return TRUE;
    ...
    $offsettable=-1;
    $todump ="#\n";
    $todump.="# ******* End of the file - The backup finished normally ********\n";
    fwrite ($fileHandle,$todump);
    fclose($fileHandle);
    return true;
}
...
if (!function_exists("gzwrite")) {
    ...
    echo "<p>You thus cannot access with the functions of backup/restoration of GEPI.
...
if (isset($action) and ($action == 'restaure_confirm'))  {
    ...
    echo "<h3>Confirmation of loading of the data of test. Attention, not to make on a database in production</h3>\n";
    echo "File selected for the restoration : <b>".$_GET['file']."</b><br/>";
    echo "Attention, the data will be crushed, and there are  entries (tables of joints) who will be duplicated if the primary constraints of keys are not good.\n";
...
if (isset($action) and ($action == 'restaure'))  {
    ...
    $file = isset($_POST["file"]) ? $_POST["file"] : (isset($_GET["file"]) ? $_GET["file"] : NULL);
		...
		if(!isset($suite_restauration)) {
			// EXTRAIRE -> SCINDER
			echo "<p>Extraction of the archive...<br />";
...
//    } else {
        ...
        if ($offsettable>=0){
            if (backupMySql($dbDb,$fichier,$duree,$rowlimit)) {
                if (isset($debug)) {
					echo "<br />\n<b>Click <a href=\"gestion_base_test.php?action=dump&amp;duree=$duree&amp;rowlimit=$rowlimit&amp;offsetrow=$offsetrow&amp;offsettable=$offsettable&amp;cpt=$cpt&amp;fichier=$fichier&amp;path=$path";
					if(isset($quitter_la_page)) {echo "&amp;quitter_la_page=y";}
					echo add_token_in_url();
					echo "\">here</a> to continue the backup.</b>\n";
				}
                if (!isset($debug)) {
					echo "<br />\n<b>Automatic redirection if not click <a href=\"gestion_base_test.php?action=dump&amp;duree=$duree&amp;rowlimit=$rowlimit&amp;offsetrow=$offsetrow&amp;offsettable=$offsettable&amp;cpt=$cpt&amp;fichier=$fichier&amp;path=$path";
					if(isset($quitter_la_page)) {echo "&amp;quitter_la_page=y";}
					echo add_token_in_url();
					echo "\">ici</a></b>\n";
				}
	...
	if ($value == "doc.html") {
		...
		if ((my_ereg('^_photos',$value)&&my_ereg('.zip$',$value))||(my_ereg('^_cdt',$value)&&my_ereg('.zip$',$value))){
		   echo "<td> </td>\n";
            ...
            echo "<td><a href='gestion_base_test.php?action=restaure_confirm&amp;file=$value".add_token_in_url()."'>Load the data</a></td>\n";
		}
...
<H3>Create a file of backup/restoration of the base of test <?php echo $dbDb; ?></H3>
...
-->
To activate the backup of the data of tests, thank you for uncomment lines 737 to 746 of the file gestion/gestion_base_test.php

<?php
echo "<h3>Documentation of the base of test : </h3>\n";
