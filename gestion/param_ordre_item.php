<?php
/*
 * $Id: param_ordre_item.php 4896 2010-07-25 20:22:14Z regis $
 *
...
if ($initialiser){
  ...
  if (!mysql_query($sql)){
	$tbs_msg= "error during suppression of the table mn_ordre_accueil";
  }
  $sql="DROP TABLE IF EXISTS `mn_ordre_accueil_item`;";
  if (!mysql_query($sql)){
	$tbs_msg= "error during suppression of the table mn_ordre_accueil_item";
  }
...
nouveau_nom VARCHAR( 50 ) NOT NULL
  ...
  if (!mysql_query($sql)){
	$tbs_msg= "error during creation of the table mn_ordre_accueil";
  }else if (!mysql_query($sql2)){
	$tbs_msg= "error during creation of the table mn_ordre_accueil_item";
  } else {
// les tables existent
