...
//================================
$titre_page = "Typing of messages of connection";
...
echo "<p class='bold'><a href='gestion_connect.php#message_login'>Return</a>";
echo " | <a href='".$_SERVER['PHP_SELF']."?import_message=y'>Import a file of messages</a>";
...
if(isset($import_message)) {
	echo "<h3 class='gepi'>Importation of a file of messages&nbsp;:</h3>\n";

	echo "<p>The file must contain a message per line.</p>\n";

	echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post'>\n";
	echo add_token_field();
	echo "<p><input type=\"file\" size=\"80\" name=\"csv_file\" /></p>\n";
	echo "<p><input type='submit' name='valide_import_message' value='Validate' /></p>\n";
	echo "</form>\n";
...
elseif(isset($valide_import_message)) {
	...
	if (trim($csv_file['name'])=='') {
		...
		echo "<a href='".$_SERVER['PHP_SELF']."?import_message=y'>Click here</a> to restart!</p>\n";
	}
	else{
		...
		if(mime_content_type($csv_file['tmp_name'])!="text/plain") {
			...
			echo "You must provide a TXT file  (<i>type bloc-notes</i>), not a file text processing or anything of other.</p>\n";
		}
		else {
				...
				while(!feof($fp)){
					...
					if(trim($ligne)!="") {
						...
						if($insert) {
							...
							echo "<span style='color:red;'><b>Error during insertion of the message&nbsp;:</b> $ligne</span><br />\n";
							$temoin_erreur='y';
						}
					}
					if($nb_reg>=$nb_max_reg) {
						echo "<p style='color:red;'>One does not record more than $nb_max_reg messages during an importation.</p>";
						break;
					}
				}
				fclose($fp);

				if(($nb_reg>0)&&($temoin_erreur=='n')) {echo "<span style='color:red;'>Importation done.</span><br />";}
			}
		}
	}
}
...
if(mysql_num_rows($resultat_messages)!=0){
	echo "<p>Here the list of your messages:</p>\n";
	echo "<blockquote>\n";
...
echo "<p>Typing of a new message:</p>";
echo "<blockquote>\n";
...
echo "<center><input type='submit' name='ok' value='Validate' /></center>\n";
echo "</blockquote>\n";
