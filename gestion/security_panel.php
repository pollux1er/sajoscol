...
echo "<p class='bold'><a href='index.php#security_panel'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a> | <a href='security_policy.php'>Define the safety policy </a> | ";
$sql="SELECT 1=1 FROM tentatives_intrusion WHERE statut='';";
$test_arch=mysql_query($sql);
if(mysql_num_rows($test_arch)>0) {
	echo "<a href='security_panel_archives.php'>History of safety alarms</a>";
}
...
//	echo "Historique des alertes sécurité";
	...
	echo " Sort by <a href='".$_SERVER['PHP_SELF']."?tri=nom'>nom</a> / <a href='".$_SERVER['PHP_SELF']."?tri=score'>score</a>";
...
if(($afficher_les_alertes_d_un_compte=="y")&&($user_login!='')) {
	...
	if(mysql_num_rows($user_req)>0) {
		...
		echo "<p>Display of alarms concerning the account ";
		...
		if($user['email']!="") {
			$lien_mail="<a href=\"mailto:".$user['email']."?subject="."[Gepi]: Your account ";
			if($_SESSION['email']!='') {
		...
		echo "<p>Display of alarms concerning the account <b>$user_login</b> who does not exist or more.</p>\n";
	}
	...
	echo "<tr>\n";
	echo "<th colspan='5'>Recent alarms</th>\n";
	echo "</tr>\n";
	...
	while ($row = mysql_fetch_object($req)) {
		...
		*/
		...
		if (!empty($user)) {
			...
			if ($user['etat'] == "actif") {
				//echo " (compte actif)";
				echo " (<span style='color:green;'>active count </span>)";
			} else {
				//echo " (compte désactivé)";
				echo " (<span style='color:red;'>disabled count </span>)";
			}
			echo "<br/>Cumulated score : ".$user['niveau_alerte'];
		} else {
			echo "<b>External attack</b><br/>\n";
			echo "IP adresse : ".$row->adresse_ip."<br/>\n";
		}
		echo "</td>\n";
		echo "<td>".$row->date."</td>\n";
		...
		if (!empty($user)) {
			...
			if(isset($tri)) {echo "&amp;tri=$tri";}
			if(isset($order_by)) {echo "&amp;order_by=$order_by";}
			echo add_token_in_url()."'>Reinitialize cumul</a>";
			echo "</p>\n";
		} else {
...
//=======================================================================================================
// Toutes les alertes:
echo "<p>' recent' Alarms (not archived) are those whose level is taken into account on the home page (information 'Cumulated levels'). To re-initialize the meter of the page of home, it is enough for you to click on 'Archive'.</p>\n";
...
elseif(mysql_num_rows($req_observation)>0) {
	echo "<p style='color:red'><a href='#utilisateurs_en_observation'>".mysql_num_rows($req_observation)." user(s)</a> in <b>observation</b>.</p>\n";
}
...
elseif(mysql_num_rows($req_desactive)>0) {
	echo "<p style='color:red'><a href='#utilisateurs_desactives'>".mysql_num_rows($req_desactive)." user(s)</a> with alarm in this page have their  <b>count desactivated</b>.</p>\n";
}
...
echo "<table class='boireaus' style='width: 90%;'>\n";
echo "<tr>\n";
echo "<th colspan='5'>Recent alarms (<a href='security_panel.php?action=archiver".add_token_in_url()."'>archive</a>)</th>\n";
echo "</tr>\n";

echo "<tr>\n";
echo "<th style='width: 20%;'>\n";
echo "<a href='".$_SERVER['PHP_SELF']."?order_by=login' style='display:inline;'>User</a>";
echo "/";
...
while ($row = mysql_fetch_object($req)) {
	...
	if (!empty($user)) {
		...
		if ($user->etat == "actif") {
			//echo " (compte actif)";
			echo " (<span style='color:green;'>account activated</span>)";
		} else {
			//echo " (compte désactivé)";
			echo " (<span style='color:red;'>Account deactivated</span>)";
		}
		echo "<br/>Cumulated score : ".$user->niveau_alerte;
	} else {
		echo "<b>External attack</b><br/>\n";
...
if(mysql_num_rows($req_observation)==0) {
	echo "<p>No user is in observation.</p>\n";
}
else {
	echo "<p>".mysql_num_rows($req_observation)." user(s) in observation&nbsp;:</p>\n";
	//echo "<table class='menu'>\n";
	echo "<table class='boireaus'>\n";
	echo "<tr>\n";
	echo "<th colspan='3'>Users in observation</th>\n";
	echo "</tr>\n";
	
	echo "<tr>\n";
	echo "<th style='width: 200px;'>User</th>\n";
	echo "<th style='width: 50px;'>Current cumul</th>\n";
...
if(mysql_num_rows($req_desactive)==0) {
	echo "<p>No user with alarm in this page is deactivated.</p>\n";
}
else {
	echo "<p>".mysql_num_rows($req_desactive)." user(s) with alarm in this page are deactivated&nbsp;:</p>\n";
	//echo "<table class='menu'>\n";
	echo "<table class='boireaus'>\n";
	echo "<tr>\n";
	echo "<th colspan='3'>Deactivated users</th>\n";
	echo "</tr>\n";
