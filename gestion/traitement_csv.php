...
//**************** EN-TETE *****************
$titre_page = "Management tool | Importation Treatment";
...
for ($row=1; $row<$nb_row; $row++) {
    ...
    if ((!isset($reg_data1)) or (!isset($reg_data2)) or (!isset($reg_data3)) or (!isset($reg_data4)) or (!isset($reg_data5))) {
        if ($reg_statut == "nouveau") {
            echo "<p><font color=red>Error during recording of the user $reg_login !</font></p>";
        } else {
            echo "<p><font color=red>Error during modification of the user $reg_login !</font></p>";
        }
    } else {
        if ($reg_statut == "nouveau") {
            echo "<p>The user $reg_login was recorded successfully !</p>";
        } else {
            echo "<p>The user $reg_login was modified successfully !</p>";
        }
    }
}
