            ...
            if($id_matiere[$i]!=""){
                if(isset($checkmat[$i])){
                    if($checkmat[$i]=="nouveau_groupe"){
                        ...
                        if (!$create) {
                            //echo "<!-- erreur -->\n";
                            $msg .= "Error during the creation of the group $id_matiere[$i]";
                        }
                        else{
                            $id_groupe=$create;
                            ...
                            */
                            ...
                            if (!$create) {
                                $msg .= "Error during update of the group $id_matiere[$i]";
                            }
                            //else {
                    ...
                    if($id_groupe!=""){
                        ...
                        if(test_before_group_deletion($id_groupe)){
                            if(!delete_group($id_groupe)){
                                $msg="Error during suppression of the group.";
                            }
                        }
                        else{
                            $msg="Notes are typing for this group. The suppression is not possible.";
                        }
                    }
                }
            }
...
$titre_page = "Management of the courses 'simple' by batch";
...
<?php
echo "<h3>Management of the simple courses for the class :" . $classe["classe"]."</h3>\n";

echo "<p>Only the courses relating to only one class should be typed here (<i>not regroupings</i>) and only one professor by course.</p>\n";
...
if ($display == "current") {
	echo "<p><img src='../images/icons/add.png' alt='' class='back_link' /> <a href='edit_class_grp_lot.php?id_classe=".$id_classe."&amp;display=new' onclick=\"return confirm_abandon (this, change, '$themessage')\">Add new groups</a></p>";
} else {
	echo "<p><img src='../images/icons/configure.png' alt='' class='back_link' /> <a href='edit_class_grp_lot.php?id_classe=".$id_classe."&amp;display=current' onclick=\"return confirm_abandon (this, change, '$themessage')\">Edit the existing groups</a></p>";
}

if($tri_matiere=='alpha') {
	echo "<p style='font-size:x-small;'>The courses are sorted alphabetically.<br /><a href='".$_SERVER['PHP_SELF']."?id_classe=$id_classe&amp;display=$display&amp;tri_matiere=priorite'>Sort the courses by priority</a></p>\n";
}
else {
	echo "<p style='font-size:x-small;'>The courses are sorted by priority.<br /><a href='".$_SERVER['PHP_SELF']."?id_classe=$id_classe&amp;display=$display&amp;tri_matiere=alpha'>Sort the courses alphabetically</a></p>\n";
}
...
echo "<table border='0' class='boireaus' summary='Table of the courses'>\n";
echo "<tr valign='top'>";
echo "<th>&nbsp;</th>\n";
echo "<th style='font-weight: bold;'>Course</th>\n";
echo "<th style='font-weight: bold;'>Professor</th>\n";
echo "</tr>\n";
if($tri_matiere=='alpha') {
...
while($ligne_matiere=mysql_fetch_object($result_matiere)){
	...
	elseif(mysql_num_rows($result_grp)>1 and $display == "current") {
		...
		//echo "<td colspan='2' style='text-align:left;'>$ligne_matiere->matiere: groupe complexe (<i>plusieurs classes</i>), accessible par <a href='edit_class.php?id_classe=$id_classe' onclick=\"return confirm_abandon (this, change, '$themessage')\">Gérer les enseignements</a>.</td>\n";
		echo "<td colspan='2' style='text-align:left;'>$ligne_matiere->nom_complet: <a href='edit_class.php?id_classe=$id_classe' onclick=\"return confirm_abandon (this, change, '$themessage')\">managed here</a> (<i>other classes and several professors implied</i>)</td>\n";
