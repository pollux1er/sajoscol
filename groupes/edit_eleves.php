		...
		*/
		...
		if(count($reg_eleves)!=0){
			...
			if (!$create) {
				...
				$msg .= "The group was indeed updated.";
			}
		}
		else {
			...
			foreach($current_group["periodes"] as $period) {
				...
				for($i=0;$i<count($login_eleve);$i++) {
					if (test_before_eleve_removal($login_eleve[$i], $id_groupe, $period["num_periode"])) {
						...
						$msg .= "Error during suppression of the student having the login '" . $login_eleve[$i] . "', for the period '" . $period["num_periode"] . " (notes or appreciations exist).<br/>";
					}
				}
			}
		}
...
<?php
	echo "<h3>Manage the students of the course : ";
...
<?php
	...
	if(mysql_num_rows($res_grp_avec_eleves)>0) {
		...
		echo "</select>\n";
		echo "<br />\n";
		echo "<input type='button' name='Copie' value='Recopy of associated students' onclick=\"recopie_grp_ele(document.getElementById('choix_modele_copie').selectedIndex);changement();\" />\n";
		echo "<br />\n";
		echo "<input type='button' name='Copie' value='REVERSES Copy of associated students' onclick=\"recopie_inverse_grp_ele(document.getElementById('choix_modele_copie').selectedIndex);changement();\" />\n";
		echo "</p>\n";
...
echo "<p>Check the students which learn this course, for each period : </p>\n";

echo "<table border='1' class='boireaus' summary='Follow-up of this course by the students according to the periods'>\n";
echo "<tr>\n";
echo "<th><a href='edit_eleves.php?id_groupe=$id_groupe&amp;id_classe=$id_classe&amp;order_by=nom'>Name/First name</a></th>\n";
if ($multiclasses) {
	echo "<th><a href='edit_eleves.php?id_groupe=$id_groupe&amp;id_classe=$id_classe&amp;order_by=classe'>Class</a></th>\n";
}
foreach ($current_group["periodes"] as $period) {
	if($period["num_periode"]!=""){
...
if(count($total_eleves)>0) {
			...
			foreach ($current_group["periodes"] as $period) {
				if($period["num_periode"]!="") {
					...
					if(mysql_num_rows($res_test)>0){
						...
						if($nb_notes_cn>0) {
							echo "<img id='img_cn_non_vide_".$period["num_periode"]."_".$num_eleve."' src='../images/icons/cn_16.png' width='16' height='16' title='Nonempty report card: $nb_notes_cn notes' alt='Nonempty report card: $nb_notes_cn notes' />";
							//echo "$sql<br />";
						}
	...
	foreach ($current_group["periodes"] as $period) {
		if($period["num_periode"]!="") {
			echo "<th>";
			if(count($total_eleves)>0) {
				echo "<a href=\"javascript:DecocheColonne_si_bull_et_cn_vide(".$period["num_periode"].");changement();\"><img src='../images/icons/wizard.png' width='16' height='16' alt='Uncheck students without note/app on the bulletin and report card' title='Uncheck students without note/app on the bulletin and report card' /></a>";

				if((isset($tab_sig))&&(count($tab_sig)>0)) {
					echo "<span id='prise_en_compte_signalement_".$period["num_periode"]."'>&nbsp;&nbsp;<a href=\"javascript:prise_en_compte_signalement(".$period["num_periode"].");changement();\"><img src='../images/icons/flag2.gif' width='16' height='16' alt='Take into account all the descriptions of errors for the period ".$period["num_periode"]."' title='Take into account all the descriptions of errors for the period ".$period["num_periode"]."' /></a></span>";
				}
			}
			echo "</th>\n";
		}
...
";
	...
	echo "<p style='margin-left:3em;'>One can deregistered only of the students which do not have a note
nor of appreciation on the bulletins.<br />On the other hand, the presence of notes in the report card does not prevent the deregistration.</p>\n";
	...
	echo "<p style='color:red;'>The classes associated to the course do not comprise any student yet.</p>\n";
}
?>
</form>
