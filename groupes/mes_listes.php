...
if(((isset($id_groupe))||(isset($classe)))&&($refermer_onglet=='y')) {
	echo "<a href='javascript:self.close();'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Close</a>";
}
else {
	echo "<a href='$chemin_retour'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
}
if(check_droit_acces('/impression/impression_serie.php',$_SESSION['statut'])) {
	echo " | <a href='../impression/impression_serie.php'>Lists PDF</a>";
}
echo "</p>\n";


echo "<h3>My lists of ".$gepiSettings['denomination_eleves']."</h3>\n";

if($_SESSION['statut']=='professeur'){
	echo "<p>Select the course and the period for which you wish to download a file CSV of ".$gepiSettings['denomination_eleve']."s&nbsp;:</p>\n";
	...
	if(mysql_num_rows($res_grp)==0){
		echo "<p>You do not have apparently any course.</p>\n";
		echo "</body></html>\n";
		die();
	}
	else{
				...
				while($lig_class=mysql_fetch_object($res_class)){
					...
					if($cpt==0){
						...
						if(mysql_num_rows($res_per)==0){
							echo "<p>ERROR: No period is defined for the class $lig_class->classe</p>\n";
							echo "</body></html>\n";
							die();
						}
						else{
		...
		if(count($groups)>0) {
			...
			foreach($groups as $current_group) {
				...
				echo "<option value='".$current_group['id']."'>".$current_group['name']." (".$current_group['description'].") in ".$current_group['classlist_string']."</option>\n";
			}
			echo "</select>\n";
			...
			for($i=1;$i<$first_group['nb_periode'];$i++) {
				...
				echo "/><label for='periode_num_$i'> Period $i</label><br />\n";
			}
			echo "</div>\n";
...
elseif(isset($id_classe)) {
	...
	if($_SESSION['statut']=='cpe'){
		echo "<p>Select the class and the period for which you want to download a file CSV of ".$gepiSettings['denomination_eleves']."s&nbsp;:</p>\n";
	...
	elseif($_SESSION['statut']=='scol'){
		echo "<p>Select the class and the period for which you want to download a CSV file  of ".$gepiSettings['denomination_eleves']."s&nbsp;:</p>\n";
	...
	if(mysql_num_rows($result_classes)==0){
		echo "<p>It seems that no class was still created...<br />... or no class was allotted to you.<br />Contact the administrator so that it carries out the suitable parameter setting in the Management of the classes.</p>\n";
...
elseif(isset($classe)) {
	echo "<p class='bold'>List of $classe&nbsp;:</p>\n";
	echo "<input type='hidden' name='id_classe' value='$id_classe' />\n";
}
else {
	echo "<p class='bold'>Personalized lists&nbsp;:</p>\n";
...
echo "<p><b>Include</b>&nbsp;:<br />\n";
echo "<input type='checkbox' id='avec_classe' name='avec_classe' value='y' ";
if(((isset($_SESSION['mes_listes_avec_classe']))&&($_SESSION['mes_listes_avec_classe']=='y'))||
	(!isset($_SESSION['mes_listes_avec_classe']))) {echo "checked ";}
echo "/><label for='avec_classe'> the name of the class</label><br />\n";

echo "<input type='checkbox' id='avec_login' name='avec_login' value='y' ";
if(((isset($_SESSION['mes_listes_avec_login']))&&($_SESSION['mes_listes_avec_login']=='y'))||
	(!isset($_SESSION['mes_listes_avec_login']))) {echo "checked ";}
echo "/><label for='avec_login'> the login of the students</label><br />\n";

echo "<input type='checkbox' id='avec_nom' name='avec_nom' value='y' ";
if(((isset($_SESSION['mes_listes_avec_nom']))&&($_SESSION['mes_listes_avec_nom']=='y'))||
	(!isset($_SESSION['mes_listes_avec_nom']))) {echo "checked ";}
echo "/><label for='avec_nom'> the name</label><br />\n";

echo "<input type='checkbox' id='avec_prenom' name='avec_prenom' value='y' ";
if(((isset($_SESSION['mes_listes_avec_prenom']))&&($_SESSION['mes_listes_avec_prenom']=='y'))||
	(!isset($_SESSION['mes_listes_avec_prenom']))) {echo "checked ";}
echo "/><label for='avec_prenom'> the first name</label><br />\n";

echo "<input type='checkbox' id='avec_sexe' name='avec_sexe' value='y' ";
if(((isset($_SESSION['mes_listes_avec_sexe']))&&($_SESSION['mes_listes_avec_sexe']=='y'))||
	(!isset($_SESSION['mes_listes_avec_sexe']))) {echo "checked ";}
echo "/><label for='avec_sexe'> the sex of the students</label><br />\n";

echo "<input type='checkbox' id='avec_naiss' name='avec_naiss' value='y' ";
if(((isset($_SESSION['mes_listes_avec_naiss']))&&($_SESSION['mes_listes_avec_naiss']=='y'))||
	(!isset($_SESSION['mes_listes_avec_naiss']))) {echo "checked ";}
echo "/><label for='avec_naiss'> the date of birth</label>\n";
...
if(getSettingValue('ele_lieu_naissance')=='y') {
	...
	echo "/><label for='avec_lieu_naiss'> the birthplace</label><br />\n";
}

echo "<input type='checkbox' id='avec_email' name='avec_email' value='y' ";
if((isset($_SESSION['mes_listes_avec_email']))&&($_SESSION['mes_listes_avec_email']=='y')) {echo "checked ";}
echo "/><label for='avec_email'> the email</label><br />\n";
...
if(isset($current_group)) {
	...
	echo "/><label for='avec_prof'> professors information </label><br />\n";
	...
	echo "/><label for='avec_statut'> the statute (<i>student or professor</i>)</label><br />\n";
}

echo "<input type='checkbox' id='avec_no_gep' name='avec_no_gep' value='y' ";
if((isset($_SESSION['mes_listes_avec_no_gep']))&&($_SESSION['mes_listes_avec_no_gep']=='y')) {echo "checked ";}
echo "/><label for='avec_no_gep'> the national number of the students (INE)</label><br />\n";

echo "<input type='checkbox' id='avec_elenoet' name='avec_elenoet' value='y' ";
if((isset($_SESSION['mes_listes_avec_elenoet']))&&($_SESSION['mes_listes_avec_elenoet']=='y')) {echo "checked ";}
echo "/><label for='avec_elenoet'> the internal number (ELENOET)</label><br />\n";

echo "<input type='checkbox' id='avec_ele_id' name='avec_ele_id' value='y' ";
if((isset($_SESSION['mes_listes_avec_ele_id']))&&($_SESSION['mes_listes_avec_ele_id']=='y')) {echo "checked ";}
echo "/><label for='avec_ele_id'> the number ELE_ID</label><br />\n";
