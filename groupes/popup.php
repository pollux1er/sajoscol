<?php
/*
* $Id: popup.php 6528 2011-02-23 11:37:20Z crob $
*
...
if(isset($id_groupe)) {
	...
	if(isset($id_classe)){
		...
		elseif(mysql_num_rows($res_classe)>1){
			$msg.="ERROR: More than one class seems to correspond to the class n°$id_classe";
		}
		else{
			$msg.="ERROR: No class seems to correspond to the class n°$id_classe.";
		}
	}
}
else {
...
<?php
	...
	echo "'>";
	if($avec_details=='y') {echo "<img src='../images/icons/remove.png' width='16' height='16' alt='Without details' />";} else {echo "<img src='../images/icons/add.png' width='16' height='16' alt='With details' />";}
	echo "</a></div>";
	...
	if($id_groupe=="VIE_SCOLAIRE"){
		...
		echo "<p>Manpower of the class: $nb_eleves</p>\n";
		if($nb_eleves>0){
			//echo "<p>";
			echo "<table class='boireaus' border='1'>\n";
			$alt=1;
	...
	elseif(isset($id_classe)){
		...
		echo "<p>Manpower of the course: $nb_eleves/$nb_tous_eleves</p>\n";
		if($nb_eleves>0){
			//echo "<p>";
			echo "<table class='boireaus' border='1'>\n";
			$alt=1;
	...
	if(getSettingValue('envoi_mail_liste')=='y') {
		...
		if(count($tabmail)>0){
			...
			echo "<p>Send a <a href='mailto:$chaine_mail?".rawurlencode("subject=[GEPI]")."'>mall to all the students</a>.</p>\n";
		}
	}
?>
