<?php
/*
* $Id: repartition_ele_grp.php 8444 2011-10-07 05:25:51Z crob $
*
...
if(isset($_POST['enregistrer_repartition'])) {
	...
	for($j=0;$j<count($login_ele);$j++) {
			...
			if($grp_eleve[$j]==$id_groupe[$i]) {
				...
				if(in_array($login_ele[$j],$tab_eleve[$id_groupe[$i]])) {
					...
					if(test_before_eleve_grp_removal($login_ele[$j], $id_groupe[$i], $num_periode)) {
						...
						updateOnline($sql);
						if(!$suppr) {$msg.="Failure of the deregistration of $login_ele[$j] of the group $nom_groupe[$i] ($id_groupe[$i]) for the period $num_periode.<br />";} else {$nb_modif++;}
					}
					else {
						$msg.="Data block the deregistration of ".get_nom_prenom_eleve($login_ele[$j])." of the group ".$nom_groupe[$i]." ($id_groupe[$i])<br />";
		...
		if(($temoin_insertion_possible=="y")&&($requete_insertion!='')) {
			...
			updateOnline($requete_insertion);
			if(!$insert) {$msg.="Failure of the inscription of $login_ele[$j] in the group $nom_groupe[$i] ($id_groupe[$i]) for the period $num_periode.<br />";} else {$nb_modif++;}
			if($requete_insertion_assoc_grp_clas!='') {
				$insert=mysql_query($requete_insertion_assoc_grp_clas);
				updateOnline($requete_insertion_assoc_grp_clas);
				if(!$insert) {$msg.="Failure of the insertion of the association of the group $nom_groupe[$i] ($id_groupe[$i]) with the class of the student over the period $num_periode.<br />";}
			}
		}
	}

	if($nb_modif>0) {
		$msg.="$nb_modif inscription(s)/deregistration(s) recorded(s).<br />";
	}
}
...
if(isset($_POST['enregistrer_recopie'])) {
		...
		for($i=0;$i<count($id_groupe);$i++) {
			...
			for($m=1;$m<=$maxper;$m++) {
				...
				if(isset($grp_eleve[$i][$m][$j])) {
					...
					if(in_array($login_ele[$j],$tab_eleve[$id_groupe[$i]][$m])) {
						...
						if(test_before_eleve_grp_removal($login_ele[$j], $id_groupe[$i], $m)) {
							...
							$suppr=mysql_query($sql);
							updateOnline($sql);
							if(!$suppr) {$msg.="Failure of the deregistration of $login_ele[$j] du groupe $nom_groupe[$i] ($id_groupe[$i]) pour la période $m.<br />";}
							else {$nb_modif++;}
						}
						else {
							$msg.="Data block the deregistration of ".get_nom_prenom_eleve($login_ele[$j])." of the group ".$nom_groupe[$i]." ($id_groupe[$i]) for the period $m.<br />";
							$temoin_insertion_possible="n";
						}
					}
				}
				...
				if(($temoin_insertion_possible=="y")&&($requete_insertion!='')) {
					...
					$insert=mysql_query($requete_insertion);
					updateOnline($requete_insertion);
					if(!$insert) {$msg.="Failure of the inscription of $login_ele[$j] in the group $nom_groupe[$i] ($id_groupe[$i]) for the period $m.<br />";} else {$nb_modif++;}
					if($requete_insertion_assoc_grp_clas!='') {
						$insert=mysql_query($requete_insertion_assoc_grp_clas);
						updateOnline($requete_insertion_assoc_grp_clas);
						if(!$insert) {$msg.="Failure of the insertion of the association of the group $nom_groupe[$i] ($id_groupe[$i]) with the class $current_id_classe_ele[$m].<br />";}
					}
				}
	...
	if($nb_modif>0) {
		$msg.="$nb_modif modification(s) recorded(s).<br />";
	}
}

$themessage  = 'Information was modified. Do you really want to leave without recording ?';
//**************** EN-TETE **************************************
$titre_page = "Distribution of students in groups";
...
if(!isset($id_classe)) {
	echo "<p class='bold'\n>";
	echo "<a href=\"../classes/index.php\" onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
	echo "</p>\n";

	echo "<p><b>Choice of the classes&nbsp;:</b><br />\n";
...
if(!isset($id_groupe)) {
	...
	echo "<p class='bold'>Select the courses&nbsp;:</p>\n";

	// Liste des enseignements de la classe $id_classe
	echo "<table class='boireaus' summary='Choice of the courses'>\n";
	echo "<tr>\n";
	...
	for($i=0;$i<count($id_classe);$i++) {
		...
		if(count($groups)==0){
			echo "<p>No course was still created in this class.</p>\n";
		}
		else {
...
if(!isset($num_periode)) {
	...
	echo " | \n";
	echo "<input type='submit' name='Autre_groupe' value=\"Choose other groups\" />\n";
	for($i=0;$i<count($id_classe);$i++) {echo "<input type='hidden' name='id_classe[]' value='$id_classe[$i]' />\n";}
	echo "</p>\n";
	echo "</form>\n";

	echo "<p class='bold'>Choose the period&nbsp;</p>\n";
	...
	for($i=0;$i<count($id_classe);$i++) {
		...
		if(mysql_num_rows($res)>0) {
			...
			echo "The class ".get_class_from_id($id_classe[$i])." does not have period?<br />\n";
		}
	}
	...
	for($i=1;$i<=$maxper;$i++) {
		...
		if($i==1) {echo "checked ";}
		echo "/><label for='num_periode_$i'> Period $i</label>\n";
	}
...
if(!isset($_POST['recopie_select'])) {
	...
	echo "<noscript>";
	echo " <input type='submit' name='Autre_periode' value=\"Choose other periods\" />\n";
	echo "</noscript>";
	echo "</p>\n";
	echo "</form>\n";
	...
	echo "<input type='button' name='Passer_a_copie' id='Passer_a_copie' value=\"Recopy assignments\" onclick=\"confirm_Passer_a_copie(change, '$themessage');\" style='display:none' /> for other periods\n";
	echo "<noscript>";
	echo "<input type='submit' name='Passer_a_copie' value='Recopy assignments' /> for other periods\n";
	echo "</noscript>";
	...
	echo "<p class='bold'>Distribution of the students in the groups for the period $num_periode&nbsp;:</p>\n";
	...
	$tab_eleve=array();
	//$tmp_tab_eleve=array();
	echo "<table class='boireaus' summary='Distribution of the students'>\n";
	echo "<tr>\n";
	//echo "<th>Elève</th>\n";
	echo "<th><input type='submit' name='Valider_repartition2' value='Validate' />\n";
	echo "<br /><a href=\"javascript:document.getElementById('order_by_nom').checked=true;document.form3.submit();\">Student</a>";
	echo "</th>\n";
	echo "<th><a href=\"javascript:document.getElementById('order_by_classe').checked=true;document.form3.submit();\">Class</a></th>\n";
	for($i=0;$i<count($id_groupe);$i++) {
		echo "<th>\n";
	...
	*/
	...
	for($j=0;$j<count($tab_eleve);$j++) {
		...
		for($i=0;$i<count($id_groupe);$i++) {
			...
			*/
			...
			if(in_array($login_ele,$group[$i]["eleves"][$num_periode]["list"])) {
				$ligne_si_desinscription_impossible.="<img src='../images/enabled.png' width='20' height='20' alt='Affected in the group' title='Affected in the group' />\n";

				if(!test_before_eleve_grp_removal($login_ele, $id_groupe[$i], $num_periode)) {
					$ligne_si_desinscription_impossible.="<img src='../images/icons/securite.png' width='16' height='16' alt='Report card or nonempty report card' title=' Report card or nonempty report card' />\n";
	...
	echo "<th>\n";
	//echo "&nbsp;";
	echo "<input type='submit' name='Valider_repartition' value='Validate' />\n";
	echo "</th>\n";
	echo "<th>\n";
	echo "&nbsp;";
	echo "</th>\n";
	for($i=0;$i<count($id_groupe);$i++) {
		echo "<th>\n";
		echo "<a href=\"javascript:CocheColonne($i);changement();\"><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a>\n";
	...
	*/
	...
	echo "<input type='submit' name='Retour_affect' value='Return to assignments' />\n";
	echo "</form>\n";
	echo "</div>\n";
	//===============================
	...
	echo "<p class='bold'>Recopy of the distribution of the students in the groups&nbsp;:</p>\n";
	...
	$tab_eleve=array();
	//$tmp_tab_eleve=array();
	echo "<table class='boireaus' summary='Recopy of the distribution of the students'>\n";
	echo "<tr>\n";
	echo "<th rowspan='2'><a href=\"javascript:document.getElementById('order_by_nom').checked=true;document.form3.submit();\">Student</a><br />\n";
	echo "<br /><input type='submit' name='Valider_recopie2' value='Validate' />\n";
	echo "</th>\n";
	//echo "<th rowspan='2'>Classe</th>\n";
	echo "<th rowspan='2'><a href=\"javascript:document.getElementById('order_by_classe').checked=true;document.form3.submit();\">Class</a></th>\n";
	for($i=0;$i<count($id_groupe);$i++) {
	...
	*/
	...
	for($i=0;$i<count($id_groupe);$i++) {
		for($m=1;$m<=$maxper;$m++) {
			echo "<th>";
			echo "<a href='javascript:copier_selection($i,$m);changement();'><img src='../images/icons/coller_23x24.png' width='23' height='24' alt='Paste the selection' title='Paste the selection' /></a>\n";
			echo "</th>\n";
		}
	...
	for($j=0;$j<count($tab_eleve);$j++) {
				...
				for($m=1;$m<=$maxper;$m++) {
					...
					if(in_array($login_ele,$group[$i]["eleves"][$m]["list"])) {
						if(!test_before_eleve_grp_removal($login_ele, $id_groupe[$i], $m)) {
							echo "<img src='../images/enabled.png' width='20' height='20' alt='Affected in the group' title='Affected in the group' />\n";
		...
		/*
			...
			if(in_array($login_ele,$group[$i]["eleves"][$num_periode]["list"])) {
				...
				if(!test_before_eleve_grp_removal($login_ele, $id_groupe[$i], $num_periode)) {
					$ligne_si_desinscription_impossible.="<img src='../images/icons/securite.png' width='16' height='16' alt='Report card ou carnet de notes non vide' title='Report card ou carnet de notes non vide' />\n";
					$temoin_desinscription_impossible="y";
				}
			}
	...
	echo "<th>\n";
	//echo "&nbsp;\n";
	echo "<input type='submit' name='Valider_recopie' value='Validate' />\n";
	echo "</th>\n";
	...
	for($i=0;$i<count($id_groupe);$i++) {
		for($m=1;$m<=$maxper;$m++) {
			echo "<th>\n";
			echo "<a href=\"javascript:CocheColonne($i,$m)\"><img src='../images/enabled.png' width='15' height='15' alt='Check all' /></a> / <a href=\"javascript:DecocheColonne($i,$m)\"><img src='../images/disabled.png' width='15' height='15' alt='Uncheck all' /></a>";
			echo "</th>\n";
		}
	}
	echo "</tr>\n";
	...
	echo "<input type='hidden' name='num_periode' value='$num_periode' />\n";
	echo "<input type='hidden' name='recopie_select' value='y' />\n";
	echo "<p>Sort by<br /><input type='radio' name='order_by' id='order_by_nom' value='' ";
	if($order_by!='classe') {echo "checked ";}
	echo "/><label for='order_by_nom'> Name</label><br />\n";
	echo "<input type='radio' name='order_by' id='order_by_classe' value='classe' ";
	if($order_by=='classe') {echo "checked ";}
	echo "/><label for='order_by_classe'> Class</label><br />\n";
	echo "<input type='submit' value='Validate' />\n";
	echo "</form>\n";
	//===============================
