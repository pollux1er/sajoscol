...
if(isset($id_classe)){
	...
	if (!is_numeric($id_classe)){
		...
		echo "<p><b>ERROR</b>: The number of selected class is not valid.</p>\n";
		...
		if(($_SESSION['statut']=='professeur')&&(getSettingValue("GepiAccesVisuToutesEquipProf")!="yes")){
			...
			if($test_prof_classe==0){
				echo "<p>ERROR: You do not have access to this class.</p>\n";
				echo "</body></html>\n";
				die();
			}
		}
		...
		if((getSettingValue("GepiAccesVisuToutesEquipCpe") == "yes") AND $_SESSION['statut']=='cpe'){
			echo '<p style="font-size: 0.7em; color: green;">The administrator gave you the access to all the classes.</p>';
		...
		echo "<h3>Teaching staff of the class of ".$classe["classe"]."</h3>\n";
	...
	function ouvre_popup(id_groupe,id_classe){
		...
		if(count($tabmail)>0){
			...
			echo "<p>Send an <a href='mailto:$chaine_mail?".rawurlencode("subject=[GEPI] classe ".$classe['classe'])."'>mall to all the members of the team</a>.</p>\n";
		}
	}
}
else{
	...
	echo "<h3>Teaching staff of a class</h3>\n";
	//echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post'>\n";
	echo "<p>Choice of the class:</p>\n";
