...
if ($_SESSION['statut'] == "responsable") {
	...
	if ($login_eleve != null and (strtoupper($login_eleve) != strtoupper($_SESSION['login']))) {
		tentative_intrusion(2, "Attempt of a student to access to the teaching staff of another student.");
	}
	$login_eleve = $_SESSION['login'];
}

//**************** EN-TETE **************************************
$titre_page = "Teaching staff";
...
if (
	...
	tentative_intrusion(1, "Attempt to access to the teaching staff without being authorized there.");
...
if ($login_eleve != null and $_SESSION['statut'] == "responsable") {
	...
	if (mysql_result($test, 0) == 0) {
	    tentative_intrusion(2, "Attempt of a parent to access to the teaching staff of a student of which it is not legal responsible.");
	    echo "You can visualize only the report booklets of the students for which you are legal responsible.\n";
	    require("../lib/footer.inc.php");
		die();
	}
}
...
if ($login_eleve == null and $_SESSION['statut'] == "responsable") {
	...
	echo "<tr>\n";
	echo "<td valign='top'>\n";
	echo "<span class='bold'>Choose the student : </span>";
	echo "</td>\n";
	echo "<td valign='top'>\n";
	...
	echo "<h3>Teaching staff of the student : ".$prenom_eleve ." " . $nom_eleve;
	...
	if(mysql_num_rows($res_class)==0) {
		echo "</h3>\n";
		echo "<p>The student is not in any class???</p>\n";
		require "../lib/footer.inc.php";
		die();
	}
	$lig_clas=mysql_fetch_object($res_class);
	$id_classe=$lig_clas->id_classe;
	echo " of ".$lig_clas->nom_complet." (<i>".$lig_clas->classe."</i>)";
	/*
	...
	*/
    ...
    echo "<table border='0' summary='Staff'>\n";
