...
echo " | <a href='./impression_serie.php'>Multiple impressions</a>";
echo " | <a href='./parametres_impression_pdf.php'>set parameters of the pdf</a>";
echo "</p>\n";
...
echo "<h3>List of classes : </h3>\n";

// Pour tout le monde la possibilité d'imprimer la liste de toutes les classes par période.
echo "<p>Select the class and the period for which you wish to print a list of students in pdf format  :</p>\n";
...
// Module toutes les classes
	...
	if($_SESSION['statut']=='professeur'){
		echo "<p>Select the course and the period for which you want to print an alphabetical list of students in pdf format :</p>\n";
		...
		if(mysql_num_rows($res_grp)==0){
			echo "<p>It seem that you have no course.</p>\n";
			echo "</body></html>\n";
			die();
		}
		else{
			...
			while($lig_grp=mysql_fetch_object($res_grp)){
				...
				for($i=0;$i<count($tabnumper);$i++){
					if($i>0){echo "<td> - </td>\n";}
					echo "<td>\n";
					echo htmlentities($tabnomper[$i])." : Sorting <a href='liste_pdf.php?id_groupe=$lig_grp->id&amp;periode_num=$tabnumper[$i]' target='_blank'>Alpha</a> - <a href='liste_pdf.php?id_groupe=$lig_grp->id&amp;periode_num=$tabnumper[$i]&amp;tri=classes' target='_blank'>Class</a>\n";
					echo "</td>\n";
				}
				echo "</tr>\n";
			}
