...
//**************** EN-TETE **************************************
$titre_page = "Impression of lists in pdf format";
require_once("../lib/header.inc");
//**************** FIN EN-TETE **********************************
//debug_var();
echo "<p class='bold'>";
echo "<a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
echo " | <a href='./impression.php'>Fast impression to the unit</a>";
echo " | <a href='./parametres_impression_pdf.php'>Set the parameters of the pdf</a>";
if(check_droit_acces('/groupes/mes_listes.php',$_SESSION['statut'])) {
	echo " | <a href='../groupes/mes_listes.php'>CSV Lists </a>";
}
echo "</p>\n";

if ($id_choix_periode != 0) {
	$periode = "Period N°".$id_choix_periode;
	echo "<h3 align='center'>".$periode."</h3>\n";
	echo "<h3>List of classes : ";
	echo "</h3>\n";
} else {
	$periode="";
	echo "<h3>List of classes : ";
...
if ($id_choix_periode == 0) {
	...
	echo "<fieldset>\n";
	echo "<legend>Select the period for which you wish to print the lists.</legend>\n";
	...
	While ( $data_periode = mysql_fetch_array ($resultat_periode)) {
		echo "<label for='id_choix_periode".$data_periode['num_periode']."' style='cursor: pointer;'>Period ".$data_periode['num_periode']." : </label><input type='radio' name='id_choix_periode' id='id_choix_periode".$data_periode['num_periode']."' value='".$data_periode['num_periode']."' /> <br />\n";
	}
	echo "<br /><br /> <input value=\"Validate the period\" name=\"Valider\" type=\"submit\" />\n
			<br />\n";
	...
	if(!isset($_POST['passer_au_choix_des_groupes'])) {
		echo "<div style=\"text-align: center;\">\n";
		echo "<fieldset>\n";
		echo "<legend>Select classes for which you want to print the lists.</legend>\n";
			...
			*/
			if($nb_modeles>0) {
				...
				*/
				...
				echo "<br />\n";
				echo "Model of settings&nbsp;: <br /><select name='id_modele'>\n";
				echo "<option value=''>Default settings of Gepi</option>\n";
			...
			echo "<br /><br /> <input value=\"Validate the classes\" name=\"Valider\" type=\"submit\" />\n";
			echo "<br />\n";
		}
		echo "</form>\n";
		echo "</fieldset>\n";
		...
		if($_SESSION['statut']=='scolarite') {
			...
			echo "<h3 align='left'>List of courses : </h3>\n";

			echo "<fieldset>\n";
			echo "<legend>Select the classes in which to seek lists of groups.</legend>\n";
			//echo "<form method=\"post\" action=\"liste_pdf.php\" target='_blank' name=\"imprime_pdf\">\n";
			...
			if ($id_choix_periode != 0) {
				...
				echo "<br /><br /> <input value=\"Validate the courses\" name=\"Valider\" type=\"submit\" />\n";
				echo "<br />\n";
			}
			echo "</form>\n";
			echo "</fieldset>\n";
		...
		if(!isset($id_liste_classes)) {
			...
			for($i=0;$i<count($id_liste_classes);$i++) {
				...
				if(mysql_num_rows($res)==0) {
					echo "<p style='color:red'>No course in this class.</p>\n";
				}
				else {
			...
			echo "<br />\n";
			echo "Option of sorting&nbsp;:<br />\n";
			echo "<input type=\"radio\" name=\"tri\" id='tri_classe' value=\"classes\" /><label for='tri_classe''> By class then alphabeticaly </label><br />\n";
			echo "<input type=\"radio\" name=\"tri\" id='tri_alpha' value=\"alpha\" checked /><label for='tri_alpha''> Alphabetical</label><br />\n";
...
if ($id_choix_periode != 0) {
	...
	if($_SESSION['statut']=='professeur') {
		...
		*/
		...
		echo "<div style=\"text-align: center;\">\n";
		echo "   <fieldset>\n
		<legend>Select the course for which you want to print the lists.</legend>\n";
		...
		echo "		<optgroup label=\"-- The Courses --\">\n";
