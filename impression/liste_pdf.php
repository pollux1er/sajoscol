...
//echo $nb_pages;
	...
	for ($i_pdf=0; $i_pdf<$nb_pages ; $i_pdf++) {
		...
		if(count($donnees_eleves)==0) {
			...
			$pdf->SetXY(20,40);
			$pdf->SetFontSize(10);
			$pdf->Cell(150,7, "No student is affected in this class or course.",0,2,'');
		...
		//Entête du PDF
			...
			if (($affiche_pp==1)) {
			   if ($id_groupe == NULL) {
				   $pdf->CellFitScale($L_entete_classe,$H_entete_classe / 2,'Class of '.$current_classe,'LTR',2,'C');
			  ...
			  // On n'affiche pas le PP (il peut y en avoir plusieurs) ==> on affiche la période
				...
				if(mysql_num_rows($res_per)==0){
					die("Problem with the infos of the class $id_classe</body></html>");
				}
				else{
			...
			if ($id_groupe != NULL) {
				...
				$pdf->Cell($L_entete_classe,$H_entete_classe / 2,'School year '.getSettingValue('gepiYear'),'LRB',2,'C');
			} else {
