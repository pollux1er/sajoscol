...
if(isset($id_modele)) {
	if(($id_modele=='')||(!preg_match("/^[0-9]*$/",$id_modele))) {
		$msg="The model '$id_modele' does not have a correct value.<br />";
		unset($id_modele);
	}
}
...
if(isset($id_modele)) {
	...
	if(mysql_num_rows($res)==0) {
		$msg.="ERROR&nbsp;: The model '$id_modele' do not exist or is not associated to you.<br />\n";
		unset($id_modele);
	}
}
...
if(isset($nom_modele)) {
	...
	if($nom_modele=='') {
		$msg="The name of model '$nom_modele_initial' is not correct.<br />";
		unset($nom_modele);
	}
...
if((isset($_POST['enregistrer_parametres']))&&(isset($nom_modele))) {
	...
	if(isset($id_modele)) {
		...
		$res=mysql_query($sql);
		if(mysql_num_rows($res)==0) {
			$msg.="The model '$id_modele' do not exist or is not associated to you.<br />";
			unset($id_modele);
		}
	}
	...
	if(!isset($id_modele)) {
		...
		if(mysql_num_rows($test)>0) {
			$msg.="ERROR: A model of the same name already exists.<br />";
		}
	}
	...
	if($temoin_erreur=="n") {
		if(!isset($id_modele)) {
			...
			$res=mysql_query($sql);
			if(!$res) {
				$msg.="ERROR during creation of the model '$nom_modele'.<br />";
			}
			else {
			...
			$res=mysql_query($sql);
			if(!$res) {
				$msg.="ERROR during update of the name of model.<br />";
			}
		}
		...
		if(isset($id_modele)) {
			...
			for($loop=0;$loop<count($tab_champs);$loop++) {
				if(isset($_POST[$tab_champs[$loop]])) {
					...
					$res=mysql_query($sql);
					if(!$res) {
						$msg.="ERROR during recording of ".$tab_champs[$loop]." -&gt; ".$_POST[$tab_champs[$loop]]." for the model '$nom_modele'.<br />";
						$cpt_erreur++;
					}
					else {
			...
			if(($cpt_reg>0)&&($cpt_erreur==0)) {
				$msg.="Recording carried out for the model n°$id_modele.<br />";
			}
		}
	}
...
elseif(isset($choix_action_modele)) {
	...
	if(isset($id_modele)) {
		...
		if($choix_action_modele=='modele_par_defaut') {
			...
			$res=mysql_query($sql);
			if(!$res) {
				$msg.="ERROR during the cleaning of the default model.<br />\n";
			}
			else {
				...
				$res=mysql_query($sql);
				if(!$res) {
					$msg.="ERROR during definition of the default model to $id_modele.<br />\n";
				}
				else {
					$msg.="Model n°$id_modele defined by default.<br />\n";
				}
			}
		...
		elseif($choix_action_modele=='suppr_modele') {
			...
			$res=mysql_query($sql);
			if(!$res) {
				$msg.="ERROR during suppression of the values associated to the model n°$id_modele.<br />\n";
			}
			else {
				...
				$res=mysql_query($sql);
				if(!$res) {
					$msg.="ERROR during suppression of the model n°$id_modele.<br />\n";
				}
				else {
					$msg.="Model n°$id_modele removed.<br />\n";	
...
$titre_page = "Impression of lists in pdf format  <br />Choice of the parameters";
...
echo "<p class='bold'>";
echo "<a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
echo " | <a href='./impression.php'>Fast impression per unit</a>";
echo " | <a href='./impression_serie.php'>Serial printing</a>";
echo " | <a href='".$_SERVER['PHP_SELF']."'>Set the parameters of the pdf</a>";
echo "</p>\n";

if(!isset($enregistrer_parametres)) {
	if(!isset($choix_action_modele)) {
		echo "<fieldset>\n";
		echo "<legend>Models of pdf documents &nbsp;:</legend>\n";
		...
		echo "<p>Create a new model with the name&nbsp;: ";
		...
		echo " <input type='submit' name='nouveau_modele' value='Create' />\n";
		echo "</p>\n";
		echo "</form>\n";
		...
		if($nb_modeles>0) {
			...
			echo " <input type='submit' name='modele_par_defaut' value='Define as default model' />\n";
			echo "</p>\n";
			echo "</form>\n";
		
			echo "<form method=\"post\" action=\"parametres_impression_pdf.php\" name=\"form_modifier_modele\">\n";
			echo add_token_field();
			echo "<p>Modify the model&nbsp;: \n";
			echo "<select name='id_modele'>\n";
			for($loop=0;$loop<count($tab_modele);$loop++) {
				echo "<option value='".$tab_modele[$loop]['id_modele']."'>".$tab_modele[$loop]['nom_modele']."</option>\n";
			}
			...
			echo " <input type='submit' name='suppr_modele' value='Delete' />\n";
			echo "</p>\n";
			echo "</form>\n";
		}
		echo "</fieldset>\n";
...
elseif(isset($modif_modele)) {
	...
	if(mysql_num_rows($res)>0) {
		...
		if(mysql_num_rows($res)>0) {
			...
			echo "<p style='color:red;'>ERROR&nbsp;: The model '$id_modele' do not exist in the table 'modeles_grilles_pdf'.</p>\n";
			require("../lib/footer.inc.php");
			die();
		}
	}
	else {
		echo "<p style='color:red;'>ERROR&nbsp;: The model '$id_modele' do not exist or is not associated to you.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
}
else {
	echo "<p style='color:red;'>Invalid choice</p>\n";

	require("../lib/footer.inc.php");
	die();
}

echo "<h3>Choice of the parameters&nbsp;: </h3>\n";

echo "<div>\n
   <fieldset>\n";
		echo "<legend>Modify the appearance of pdf document &nbsp;:</legend>\n";

		//echo "<div style='float:right; '></div>";

		echo "<form method=\"post\" action=\"parametres_impression_pdf.php\" name=\"choix_parametres\">\n";
		echo add_token_field();
		echo "<input value=\"Validate parameters\" name=\"Valider\" type=\"submit\" /><br />\n";
		echo "<br />\n";
		...
		echo "<b>Definition of the margins of the document&nbsp;:</b></p>\n";
		echo "<table style='margin-left: 1em;' border='0'>\n";
		echo "<tr><td>margin on the left&nbsp;:</td><td><input type=\"text\" name=\"marge_gauche\" size=\"2\" maxlength=\"2\" value=\"$marge_gauche\" /></td></tr>\n";
		echo "<tr><td>margin on the right&nbsp;:</td><td><input type=\"text\" name=\"marge_droite\" size=\"2\" maxlength=\"2\" value=\"$marge_droite\" /></td></tr>\n";
		echo "<tr><td>margin of top&nbsp;:</td><td><input type=\"text\" name=\"marge_haut\" size=\"2\" maxlength=\"2\" value=\"$marge_haut\" /></td></tr>\n";
		echo "<tr><td>margin of bottom&nbsp;:</td><td><input type=\"text\" name=\"marge_bas\" size=\"2\" maxlength=\"2\" value=\"$marge_bas\" /></td></tr>\n";

		echo "<tr><td>Option margin binding ?</td><td><input type=\"radio\" name=\"marge_reliure\" id=\"marge_reliure_1\" value=\"1\" ";
		if($marge_reliure==1) {echo "checked ";}
		echo "/><label for='marge_reliure_1'> Yes</label> <input type=\"radio\" name=\"marge_reliure\" id=\"marge_reliure_0\" value=\"0\" ";
		if($marge_reliure!=1) {echo "checked ";}
		echo "/><label for='marge_reliure_0'> No</label></td></tr>\n";
		...
		echo "/><label for='avec_emplacement_trous_1'> Yes</label> <input type=\"radio\" name=\"avec_emplacement_trous\" id=\"avec_emplacement_trous_0\" value=\"0\" ";
		if($avec_emplacement_trous!=1) {echo "checked ";}
		echo "/><label for='avec_emplacement_trous_0'> No</label></td></tr>\n";
		echo "</table>\n";

		echo "<br />\n";

		echo "<b>Information to display on the document&nbsp;:</b><br />\n";
		echo "<table style='margin-left: 1em;' border='0'>\n";
		echo "<tr><td>Display the professor responsible of the class ?</td><td><input type=\"radio\" name=\"affiche_pp\" id=\"affiche_pp_1\" value=\"1\" ";
		if($affiche_pp==1) {echo "checked ";}
		echo "/><label for='affiche_pp_1'> Yes</label> <input type=\"radio\" name=\"affiche_pp\" id=\"affiche_pp_0\" value=\"0\" ";
		if($affiche_pp!=1) {echo "checked ";}
		echo "/><label for='affiche_pp_0'> No</label></td></tr>\n";
		echo "<tr><td valign='top'>Display a line of text before the table  ?</td><td><input type=\"radio\" name=\"avec_ligne_texte\" id=\"avec_ligne_texte_1\" value=\"1\" ";
		if($avec_ligne_texte==1) {echo "checked ";}
		echo "/><label for='avec_ligne_texte_1'> Yes</label> <input type=\"radio\" name=\"avec_ligne_texte\" id=\"avec_ligne_texte_0\" value=\"0\" ";
		if($avec_ligne_texte!=1) {echo "checked ";}
		echo "/><label for='avec_ligne_texte_0'> No</label><br />";
		echo "Texte&nbsp;: &nbsp;<input type=\"text\" name=\"ligne_texte\" size=\"50\" value=\"$ligne_texte\" /></td></tr>\n";
		echo "<tr><td>Display the manpower of the class ?</td><td><input type=\"radio\" name=\"afficher_effectif\" id=\"afficher_effectif_1\" value=\"1\" ";
		if($afficher_effectif==1) {echo "checked ";}
		echo "/><label for='afficher_effectif_1'> Yes</label> <input type=\"radio\" name=\"afficher_effectif\" id=\"afficher_effectif_0\" value=\"0\" ";
		if($afficher_effectif!=1) {echo "checked ";}
		echo "/><label for='afficher_effectif_0'> No</label></td></tr>\n";
		echo "</table>\n";

		echo "<br />\n";
		echo "<b>Styles of the table&nbsp;: </b><br />\n";
		echo "<table style='margin-left: 1em;' border='0'>\n";
		echo "<tr><td>All on only one page ?</td><td><input type=\"radio\" name=\"une_seule_page\" id=\"une_seule_page_1\" value=\"1\" ";
		if($une_seule_page==1) {echo "checked ";}
		echo "/><label for='une_seule_page_1'> Yes</label> <input type=\"radio\" name=\"une_seule_page\" id=\"une_seule_page_0\" value=\"0\" ";
		if($une_seule_page!=1) {echo "checked ";}
		echo "/><label for='une_seule_page_0'> No</label></td></tr>\n";
		echo "<tr><td>Height of a line&nbsp;:</td><td><input type=\"text\" name=\"h_ligne\" size=\"2\" maxlength=\"2\" value=\"$h_ligne\" /> </td></tr>\n";
		echo "<tr><td>Width of a column&nbsp;:</td><td><input type=\"text\" name=\"l_colonne\" size=\"2\" maxlength=\"2\" value=\"$l_colonne\" /> </td></tr>\n";
		echo "<tr><td>Width column Name / First name&nbsp;:</td><td><input type=\"text\" name=\"l_nomprenom\" size=\"2\" maxlength=\"2\" value=\"$l_nomprenom\" /> </td></tr>\n";
		echo "<tr><td>Numbers of line before&nbsp;:</td><td><input type=\"text\" name=\"nb_ligne_avant\" size=\"2\" maxlength=\"2\" value=\"$nb_ligne_avant\" /> \n";
		echo "<tr><td>Height of the first line before&nbsp;:</td><td><input type=\"text\" name=\"h_ligne1_avant\" size=\"2\" maxlength=\"$h_ligne1_avant\" value=\"25\" /> </td></tr>\n";
		echo "<tr><td>Numbers of Line after &nbsp;:</td><td><input type=\"text\" name=\"nb_ligne_apres\" size=\"2\" maxlength=\"2\" value=\"$nb_ligne_apres\" /> </td></tr>\n";
		echo "<tr><td>Total squaring of the cells ?</td><td><input type=\"radio\" name=\"encadrement_total_cellules\" id=\"encadrement_total_cellules_1\" value=\"1\" ";
		if($encadrement_total_cellules==1) {echo "checked ";}
		echo "/><label for='encadrement_total_cellules_1'> Yes</label> [ <input type=\"radio\" name=\"encadrement_total_cellules\" id=\"encadrement_total_cellules_0\" value=\"0\" ";
		if($encadrement_total_cellules!=1) {echo "checked ";}
		echo "/><label for='encadrement_total_cellules_0'> No</label> \n";
		echo "<tr><td>&nbsp;</td><td>Number of squared cells after the name&nbsp;: <input type=\"text\" name=\"nb_cellules_quadrillees\" size=\"2\" maxlength=\"2\" value=\"$nb_cellules_quadrillees\" /> ] </td></tr>\n";
		echo "</table>\n";
		echo "<br />\n";

		echo "<b>Information in bottom of the document&nbsp;: </b><br />\n";
		echo "&nbsp;&nbsp;Preserve an empty zone under the table ? <input type=\"radio\" name=\"zone_vide\" id=\"zone_vide_1\" value=\"1\" ";
		if($zone_vide==1) {echo "checked ";}
		echo "/><label for='zone_vide_1'> Oui</label> <input type=\"radio\" name=\"zone_vide\" id=\"zone_vide_0\" value=\"0\" ";
		if($zone_vide!=1) {echo "checked ";}
		echo "/><label for='zone_vide_0'> No</label><br />\n";
		echo "&nbsp;&nbsp;Height of the zone&nbsp;: <input type=\"text\" name=\"hauteur_zone_finale\" size=\"2\" maxlength=\"2\" value=\"$hauteur_zone_finale\" /> (0 all that remains)<br />\n";

		//echo "<input value=\"1\" name=\"ok\" type=\"hidden\" />\n";
		echo "<input type=\"hidden\" name=\"enregistrer_parametres\" value=\"1\" />\n";
		echo "<br />\n";
		echo "<input value=\"Validate the parameters\" name=\"Valider\" type=\"submit\" />\n";
		echo "<br />\n";
		echo "</form>\n";
   echo "</fieldset>\n
 </div>";
