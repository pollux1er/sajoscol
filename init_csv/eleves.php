...
//**************** EN-TETE *****************
$titre_page = "Initialization Tool of the year : Importation of the students - Stage 1";
...
echo "<center><h3 class='gepi'>First phase of initialization<br />Importation of the students</h3></center>\n";
...
if (!isset($_POST["action"])) {
	...
	echo "<p>You will carry out the first stage : it consists in importing the file <b>g_eleves.csv</b> containing the data students.</p>\n";
	echo "<p>The following fields must be present, in the order, and <b>separated by a semicolon</b> : </p>\n";
	echo "<ul><li>Name</li>\n" .
			"<li>First name</li>\n" .
			"<li>Date of birth to the format DD/MM/YYYY</li>\n" .
			"<li>n° identifier interns to the school (essential : it is this number which is used to make the connection during the other imports)</li>\n" .
			"<li>n° national identifier</li>\n" .
			"<li>Code previous school</li>\n" .
			"<li>Doubling (OUI or NON)</li>\n" .
			"<li>Regim (INTERN or EXTERN or IN.EX. or DP DAN)</li>\n" .
			"<li>Sex (F or M)</li>\n" .
			"</ul>\n";
	echo "<p>Please specify the complete name of the file <b>g_eleves.csv</b>.</p>\n";
	...
	echo "<p><label for='en_tete' style='cursor:pointer;'>If the file to be imported contains a first heading line (nonempty) to be ignored, <br />check the box opposite</label>&nbsp;<input type='checkbox' name='en_tete' id='en_tete' value='yes' checked /></p>\n";
	...
	if ($_POST['action'] == "save_data") {
		...
		if(mysql_num_rows($res_temp)==0) {
			echo "<p style='color:red'>ERROR&nbsp;: No student was found&nbsp;???</p>\n";
			echo "<p><br /></p>\n";
...
//die('yo');
		...
		if ($total > 0) echo "<p>" . $total . " students were recorded.</p>\n";
		...
		if(strtolower($csv_file['name']) == "g_eleves.csv") {
			...
			if(!$fp) {
				...
				echo "<p><a href='eleves.php'>Click here </a> to restart !</p>\n";
			} else {
				...
				//=========================
					...
					while (!feof($fp)) {
						...
						if(trim($ligne)!="") {
							...
							if ($tabligne[8] != "F" AND $tabligne[8] != "M") $tabligne[8] = "M";
				...
				echo "<table class='boireaus' border='1' summary='Table of the students'>\n";
				echo "<tr><th>Name</th><th>First name</th><th>Sexe</th><th>Date of birth</th><th>n° school.</th><th>n° nat.</th><th>Code school.</th><th>Double.</th><th>Regim</th></tr>\n";
				...
				echo "</table>\n";
				echo "$k students were detected in the file.<br />\n";
			...
			echo "<a href='eleves.php'>Click here </a> to restart !</p>\n";
