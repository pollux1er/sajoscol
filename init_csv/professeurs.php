...
if (!isset($_POST["action"])) {
	...
	echo "<p>You will carry out the fourth stage : it consists in importing the file <b>g_professeurs.csv</b> containing the data of the professors.</p>\n";
	...
	echo "<ul><li>Name</li>\n" .
			...
			"</ul>\n";
	echo "<p>Please specify the complete name of the file <b>g_professeurs.csv</b>.</p>\n";
	...
	echo "<br />\n";
	echo "<label for='en_tete' style='cursor:pointer;'>If the file to be imported comprises a first heading line (nonempty)
to ignore, <br />check the box opposite</label>&nbsp;<input type='checkbox' name='en_tete' id='en_tete' value='yes' checked />\n";


	echo "<br /><br /><p>Which formula to apply for the generation of the login ?<br />\n";
	echo "<input type='radio' name='login_mode' value='name' checked /> name";
	echo "<br />\n<input type='radio' name='login_mode' value='name8' /> name (truncated to 8 characters)";
	echo "<br />\n<input type='radio' name='login_mode' value='fname8' /> pname (truncated to 8 characters)";
	echo "<br />\n<input type='radio' name='login_mode' value='fname19' /> pname (truncated to 19 characters)";
	echo "<br />\n<input type='radio' name='login_mode' value='firstdotname' /> firstname.name";
	echo "<br />\n<input type='radio' name='login_mode' value='firstdotname19' /> firstname.name (truncated to 19 characters)";
	echo "<br />\n<input type='radio' name='login_mode' value='namef8' /> namep (truncated to 8 characters)";
	echo "<br />\n<input type='radio' name='login_mode' value='lcs' /> pname(façon LCS)";
	echo "<br />\n</p>\n<p>Which mode of authentification is used ?  (leave 'Gepi' if you do not know what it is)</p>\n";
	echo "<p>\n<input type='radio' name='sso' value='gepi' checked /> Gepi";
	echo "<br />\n<input type='radio' name='sso' value='sso' /> SSO (no password will be generated)";
	echo "<br />\n<input type='radio' name='sso' value='ldap' /> Ldap (no password will be generated)</p>\n";
	...
	if ($_POST['action'] == "save_data") {
		...
		if(mysql_num_rows($res_temp)==0) {
			echo "<p style='color:red'>ERROR&nbsp;: No professor was found&nbsp;???</p>\n";
			echo "<p><br /></p>\n";
		...
		if ($error > 0) {echo "<p><span style='color:red'>There was " . $error . " errors.</span></p>\n";}
		if ($total > 0) {echo "<p>" . $total . " professors were recorded.</p>\n";}
		if($total_deja_presents>0) {echo "<p>" . $total_deja_presents . " professors already present.</p>\n";}
		...
		if(strtolower($csv_file['name']) == "g_professeurs.csv") {
			...
			if(!$fp) {
				...
				echo "<p><a href='professeurs.php'>Click here </a> to restart !</p>\n";
			} else {
				...
				echo "<table border='1' class='boireaus' summary='Table of the professors'>\n";
				echo "<tr><th>Login</th><th>Name</th><th>First name</th><th>Civility</th><th>Email</th><th>Authentification</th></tr>\n";
