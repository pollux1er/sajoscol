...
if (!isset($is_posted)) {
   ...
   echo "<p> The useless data imported from GEP files during the various phases of initialization will be erase !</p>";
   echo "<form enctype='multipart/form-data' action='clean_tables.php' method='post'>";
   echo "<input type=hidden name='is_posted' value='yes' />";
   echo "<p><input type=submit value='Make a cleaning' />";
   echo "</form>";
} else {
   $j=0;
   $flag=0;
   ...
   if ($flag != 0){
       ...
       echo "The initialization of the data of the year is not finished, certain data concerning the students, classes, the groups, the professors or the courses are missing. The procedure cannot continue !</p></body></html>";
       die();
   }
   ...
   while ($i < $nb_prof) {
       ...
       if (mysql_num_rows($test)==0) {
           ...
           if (mysql_num_rows($test)==0) {
               $del = @mysql_query("delete from utilisateurs where login = '$login_prof'");
               echo "The professor $login_prof was removed from the bases.<br />";
               $sup = 'yes';
            }
       }
       $i++;
   ...
   if ($sup == 'no') {
	...
	if(mysql_num_rows($res_jee)==0) {
		echo "<p>No association student/school was not removed.</p>\n";
	}
	else {
		$cpt_suppr_jee=0;
		...
		echo "<p>$cpt_suppr_jee association(s) student/school were removed.<br />(<i>for students who are not any more in the school</i>).</p>\n";
	}
