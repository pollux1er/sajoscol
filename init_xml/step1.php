...
echo "<center><h3 class='gepi'>First phase of initialization<br />Importation of students, constitution of classes and assignment of the students in the classes</h3></center>\n";


if (!isset($is_posted)) {
	echo "<p>You will carry out the first stage : it consists in importing the file <b>ELEVES.CSV</b> (<i>generated starting from exports XML of Sconet</i>) containing all the data in a temporary table of the data base of <b>GEPI</b>.";
	echo "<p>Please specify the complete name of the file <b>ELEVES.CSV</b>.";
	...
	if(strtoupper($csv_file['name']) == "ELEVES.CSV"){
		...
		if(!$fp){
			...
			*/
				...
				if(!feof($fp)){
					...
					if(trim($ligne)!=""){
						...
						if ($enregistre == "yes") {
							...
							if (!$register) {
								echo "<p class=\"small\"><font color='red'>Analyze of the line $k : error during recording !</font></p>";
								$nb_reg_no++;
							} else {
								$nb_reg_ok++;
					//                        echo ".";
			...
			fclose($fp);
			if ($nb_reg_no != 0) {
				echo "<p>During recording of the data there was $nb_reg_no errors, you cannot proceed to the rest of initialization. Find the cause of the error and start again the procedure, after having emptied the temporary table.";
			}
			else {
				echo "<p>The $nblignes lines of the file ELEVES.CSV were analyzed.<br />$nb_reg_ok lines of data correspond to students of the year in progress were recorded in a temporary table.<br />There no were errors, you can proceed to the next stage.</p>";
				echo "<center><p><a href='step2.php?a=a".add_token_in_url()."'>Access to stage 2</a></p></center>";
			}
		}
	}
