...
if(isset($_POST['is_posted'])) {
	...
	if(($cat=='profs')&&($mode=='prof')) {
		if($login_prof=='') {
			$msg.="No login professor was proposed.<br />\n";
		}
		else {
	...
	foreach($id_groupe as $i => $value) {
		...
		for($j=0;$j<count($tab_login_prof);$j++) {
			if(!in_array($tab_login_prof[$j], $current_group['profs']['list'])) {
				...
				$update=mysql_query($sql);
				if(!$update) {
					$msg.="Error during association of ".$tab_login_prof[$j]." with the group n°".$id_groupe[$i]."<br />\n";
					$nb_err++;
				}
			}
		}
		...
		for($j=0;$j<count($current_group['profs']['list']);$j++) {
			if(!in_array($current_group['profs']['list'][$j], $tab_login_prof)) {
				...
				$update=mysql_query($sql);
				if(!$update) {
					$msg.="Error during deregistration of ".$current_group['profs']['list'][$j]." of the group n°".$id_groupe[$i]."<br />\n";
					$nb_err++;
				}
			}
		}
		...
		if($nb_err==0) {
			$msg.="Update of the group n°".$id_groupe[$i]." carried out.<br />\n";
		}
...
if(isset($_GET['suppr_groupe'])) {
	...
	if(!preg_match('/^[0-9]*$/', $_GET['suppr_groupe'])) {
		$msg.="The group n°".$_GET['suppr_groupe']." do not exist.<br />\n";
	}
	else {
		$delete=delete_group($_GET['suppr_groupe']);
		if($delete) {
			$msg.="The group n°".$_GET['suppr_groupe']." was removed.<br />\n";
		}
		else {
			$msg.="Error during suppression of the group n°".$_GET['suppr_groupe'].".<br />\n";
		}
	}
}
...
if(isset($_POST['add_groupes_classes'])) {
		...
		if(mysql_num_rows($res_per)>0) {
			...
			for($j=0;$j<count($matiere);$j++) {
				...
				if((!$creation)||(!preg_match('/^[0-9]*$/',$creation))) {
					$msg.="Error during creation of a group of $matiere[$j] en ".get_nom_classe($id_classe[$i]).".<br />\n";
				}
				else {
					...
					if(!$update) {
						$msg.="Error during the filling of the course of $matiere[$j] in ".get_nom_classe($id_classe[$i]).".<br />\n";
					}
					else {
						$nb_reg++;
					}
	...
	if(($msg=="")&&($nb_reg>0)) {
		$msg="Creation of the groups done.<br />\n";
	}
...
if(!isset($cat)) {
	echo "<a href='index.php'><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return home initialization</a>";
	echo "</p>\n";
	echo "</form>\n";

	echo "<center><h3 class='gepi'>Fourth phase of initialization<br />Alternative initialization</h3></center>\n";
	
	echo "<p>If the timetable did not go up yet towards STS, you have a file <b>sts_emp_RNE_ANNEE.xml</b> incomplete.<br />\n";
	echo "If you do not have either an export CSV of UnDeuxTemps to initialize the courses, you can carry out creation here.<br />It is advisable to follow the links in the order&nbsp;:</p>\n";
	
	echo "<ol>\n";
	echo "<li><p><a href='".$_SERVER['PHP_SELF']."?cat=profs'>Associate the courses to the professors</a></p></li>\n";
	echo "<li><p><a href='".$_SERVER['PHP_SELF']."?cat=classes'>Create courses in selections of classes</a></p></li>\n";
	// style='color:red'
	echo "<li><p>Once courses created, it is necessary to continue here to do the cleaning of the
assignments of students in the courses by taking account of the typing of options in Sconet.<br /><a href='init_options.php?a=a".add_token_in_url()."'>Taking into account of the options</a></p></li>\n";
	echo "</ol>\n";
}
//===================================================================================
elseif($cat=='profs') {
	...
	//echo " | ";
	echo "<a href='".$_SERVER['PHP_SELF']."'>Return to the choice of professors  or classes</a>";
	...
	if(!isset($mode)) {
		...
		echo "<center><h3 class='gepi'>Fourth phase of initialization<br />Alternative initialization</h3></center>\n";
		echo "<center><h4 class='gepi'>Professors</h4></center>\n";
		...
		echo "<p>a course, then professors to be associated to it&nbsp;: \n";
		...
		echo "<p>Activate an inactive professor&nbsp;: \n";
	...
	elseif($mode=='prof') {
		...
		echo " | <a href='".$_SERVER['PHP_SELF']."?cat=profs'>Return to the choice of mode of assignment of the courses to the professors</a>";
		...
		if($login_prof=='') {
			echo "<p style='color:red;'>No professor was selected.<br /><a href='".$_SERVER['PHP_SELF']."?cat=profs'>Return</a></p>\n";
		...
		if($civ_nom_prenom_prof=='') {
			echo "<p style='color:red;'>No professor corresponds to the login '$login_prof'.<br /><a href='".$_SERVER['PHP_SELF']."?cat=profs'>Return</a></p>\n";
		...
		echo "<p>Which courses associate to <b>".$civ_nom_prenom_prof."</b>&nbsp;: \n";
		...
		echo "<table width='100%' summary='Choice of courses'>\n";
...
function checkbox_change(cpt) {
	if(document.getElementById('matiere_'+cpt)) {
		...
		echo "<p style='text-align:center;'><input type='submit' name='confirm' value='Validate' /></p>\n";
		echo "</p>\n";
		echo "</form>\n";
		...
		echo "<center><h3 class='gepi'>Fourth phase of initialization<br />Alternative initialization</h3></center>\n";
		echo "<center><h4 class='gepi'>Courses</h4></center>\n";

		if($matiere=='') {
			echo "<p style='color:red;'>No course was selected.<br /><a href='".$_SERVER['PHP_SELF']."?cat=profs'>Return</a></p>\n";
		...
		if(mysql_num_rows($test)==0) {
			echo "<p style='color:red;'>The course <b>$matiere</b> do not exist.<br /><a href='".$_SERVER['PHP_SELF']."?cat=profs'>Return</a></p>\n";
...
elseif($cat=='classes') {
	//echo " | ";
	echo "<a href='".$_SERVER['PHP_SELF']."'>Return to the choice professors or classes</a>";
	...
	if((!isset($id_classe))||(count($id_classe)==0)) {
		...
		if($nb==0) {
			echo "<p style='color:red'>No class with students exists yet.</p>\n";
			require("../lib/footer.inc.php");
			die();
		}
		...
		echo "<p class='bold'>In which classes do you want to create courses&nbsp;?</p>\n";
		...
		while($lig=mysql_fetch_object($res)) {
			...
			echo "/><label for='id_classe_$i'><span id='texte_id_classe_$i'>Class : ".$lig->classe.".</span></label>";
...
*/
		...
		echo "<p class='bold'>Here associations already carried out.<br />You can carry out associations courses/professors there.</p>";
		for($i=0;$i<count($tab_classe);$i++) {
			...
			*/
			...
			foreach($groups as $current_group) {
				...
				echo "><img src='../images/delete16.png' width='16' height='16' alt='Remove this course' title='Remove this course' /></a> ";
				...
				echo " <a id='lien_profs_$cpt_groupe' style='display:none' href=\"javascript:affiche_choix_prof($cpt_groupe)\"><img src='../images/add_user.png' width='16' height='16' alt='Add one or more professors' title='Add one or more professors' /></a>\n";
...
function confirm_suppr_groupe(theLink, thechange, themessage) {
	...
	if (thechange != 'yes') {
		...
		var is_confirmed = confirm('Are you want to remove this group ?');
		return is_confirmed;
	}
	else {
...
function test_form_classe(id_classe) {
	...
	if((id_classe_form_prof_en_cours!='')&&(id_classe_form_prof_en_cours!=id_classe)) {
		alert('You modified the list of the professors for another class ('+eval('classe_'+id_classe_form_prof_en_cours)+'), without validating the modifications. You can edit list of professors of only one class at the same time.');
	}
...
<ul>
	<li>
		<p>It is recommended to save time to start by select several classes and create the traditional courses there (<i>MATHS, FRANC, HIGEO,...</i>) and add only after the courses existing only in certain classes.</p>
		<p>You can if you prefer to create all the courses of a selection of
class and remove the courses that do not exist in certain of these classes.</p>
	</li>
	<li>
		<p>In this device no course regrouping of several classes is created.<br />
		You will be able however to amalgamate thereafter the courses created in several classes.<br />
		For that, the procedure is <b>Management of the bases/Management of the classes/&lt;One_of_classes&gt;/Courses/&lt;The_name_of_course&gt;/ amalgamate the group with existing groups</b></p>
	</li>
		<p>At this stage, the courses created contain all the students of the
corresponding class.<br />The table 'temp_gep_import2' contains the options followed by students typed in Sconet.<br />Once courses created,
 it is necessary to continue here to do the cleaning of the
assignments of students in the courses by taking account of the typing of options in
Sconet.<br /><b><a href='init_options.php?a=a".add_token_in_url()."'>Considerations of the options</a></b></p>
	<li>
	</li>
</ul>\n";
	...
	echo " | <a href='".$_SERVER['PHP_SELF']."?cat=classes'>Choice of classes</a>";
	echo "</p>\n";
	echo "</form>\n";
	...
	echo "<p>Choice of the courses to be created in the classes of&nbsp;: <b>\n";
	for($i=0;$i<count($id_classe);$i++) {
		if($i>0) {echo ", ";}
	...
	echo "<p align='center'><input type='submit' value='Add teaching of these courses' /></p>\n";
...
function checkbox_change_matiere(cpt) {
	...
	echo "<p><em>NOTES</em>&nbsp;:</p>\n";
	echo "<ul>\n";
	echo "<li>If a course exists only in part of the selected classes, you can add it now, and remove the surplus groups thereafter.</li>\n";
	echo "<li>You will be able to amalgamate groups thereafter if they must be overlap on several classes, but so that the options typed for the students in Sconet are
taken into account, it is better to create now the groups on the various classes.</li>\n";
	echo "<li>The association of the professors will be made after the addition of the courses.</li>\n";
	echo "</ul>\n";
/*
