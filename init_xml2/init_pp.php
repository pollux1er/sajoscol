...
if (!isset($step1)) {
	...
	if(@file_exists("../temp/".$dirname."/matiere_principale.csv")) {
		...
		*/
		...
		$fich_mp=fopen("../temp/".$dirname."/matiere_principale.csv","r");
		if($fich_mp) {
			echo "<p class='bold'>Re-establishment of the principal course of each professor according to the records of the previous year.</p>\n";
					...
					if(in_array($matiere_prof,$tab_matiere_prof)) {
						...
						if($tab_matiere_prof[0]!=$matiere_prof) {
							...
							if($temoin_debut_mp==0) {
								echo "<p>Correction of the principal course for ";
							}
							else {
								echo ", ";
							}
			...
			if($temoin_debut_mp>0) {
				...
				echo "<p><i>Notice&nbsp;:</i> The professors for whom the principal course is already good do not appear in the corrections above.</p>\n";
			}
		}
	}
	...
	if ($flag != 0){
		...
		echo "Principal professors are currently defined in base GEPI (<i>table 'j_eleves_professeurs'</i>)<br /></p>\n";
		...
		echo "<p>If you continue the procedure these data will be removed and replaced by those of your importation XML.</p>\n";
	...
	echo "<center><h3 class='gepi'>Sixth phase<br />Importation of the principal professors</h3></center>\n";
}
...
if (!isset($is_posted)) {
	...
	if(!file_exists("../temp/$tempdir/f_div.csv")){
		echo "<p>The file <b>f_div.csv</b> is not present in your temporary folder.<br />Have you skip the stage of the importation of the professors???</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	echo "<p>Importation of associations class/principal professor.</p>\n";
	echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post'>\n";
	echo add_token_field();
	...
	//if(strtoupper($dbf_file['name']) == "F_DIV.CSV") {
		...
		if(!$fp) {
			echo "<p>Impossible to open CSV file normally generated during importation of the professors.</p>\n";
			...
			*/
			echo "<table border='1' class='boireaus' cellpadding='2' cellspacing='2' summary='Table of the principal professors'>\n";
			...
			if ($nb_reg_no != 0) {
				echo "<p>During recording of the data, there was $nb_reg_no errors.<br />Try to find the cause and restart the procedure before passing to the next stage.</p>\n";
			} else {
				if($nb_reg_ok>0){
				...
				echo "<p>Before carrying out a cleaning of the tables to remove the useless data, you should carry out one <a href='../gestion/accueil_sauve.php?action=$mode_sauvegarde&amp;quitter_la_page=y".add_token_in_url()."' target='_blank'>backup</a><br />\n";
				echo "After this backup, carry out cleaning while passing by 'General management/Initialization of the data from files DBF and XML/Proceed to the seventh phase'.<br />\n";
