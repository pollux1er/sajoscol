...
@set_time_limit(0);
/*
* $Id: prof_disc_classe_csv.php 8272 2011-09-19 14:58:27Z crob $
*
...
//**************** EN-TETE *****************
$titre_page = "Tool of initialization of the year : Importation of the relations professors/classes/courses";
...
echo "<center><h3 class='gepi'>Fifth phase of initialization" .
		"<br />Assignment of the courses to each professor," .
		"<br />Assignment of the professors in each class," .
		"<br />Importation of the options followed by the students" .
		"</h3></center>";

echo "<h3 class='gepi'>First stage : assignment of the courses to each professor and assignment of the professors in each class.</h3>";

if (!isset($step1)) {
	$test = mysql_result(mysql_query("SELECT count(*) FROM j_groupes_professeurs"),0);
	if ($test != 0) {
		echo "<p><b>CAUTION ...</b><br />";
		echo "Data concerning the assignment of professors in classes are currently present in base GEPI<br /></p>";
		echo "<p>If you continue the procedure these data will be erased.</p>";

		echo "<p>The emptied tables will be&nbsp;: 'j_groupes_professeurs' et 'j_professeurs_matieres'</p>\n";
...
if (!isset($suite)) {
	...
	*/
	...
	if(strtoupper($nom_racine)!='STS_EDT') {
		echo "<p style='color:red;'>ERROR: the provided XML file does not seem to be a file XML STS_EMP_&lt;RNE&gt;_&lt;ANNEE&gt;.<br />Its root should be 'STS_EDT'.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}

	echo "Analyze of the file to extract information from the section DIVISIONS...<br />\n";
	...
	if(mysql_num_rows($res_mat)==0){
		echo "<p>The table 'temp_matieres_import' is empty.<br />Have you skip stages???</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	$cpt=0;
	...
	echo "<p>At this stage, the students will be affected in all the groups.<br />It is only at the next stage that the options will be taken into account to prune the groups.</p>\n";
	// A REVOIR... à moins que cette page soit déjà longue en traitement...

	$temoin_div_sans_services=0;
	$temoin_service_sans_enseignant=0;
	echo "<h3>Creation of the groups whole class </h3>\n";
	for ($i=0;$i<count($divisions);$i++) {
		$classe=$divisions[$i]['code'];

		echo "<p class='bold'>Class of $classe</p>\n";
		echo "<blockquote>\n";
		...
		elseif($nb_clas==1) {
			...
			if(!isset($divisions[$i]['services'])) {
				echo "<p style='color:red;'>No service is defined for this class.<br />Has the timetable went up towards STS?</p>\n";
				...
				for($j=0;$j<count($divisions[$i]['services']);$j++) {
					...
					echo "<p>Creation of the group $descr_grp (<i>$nom_grp</i>) en $classe: ";
					if($id_groupe=create_group($nom_grp, $descr_grp, $mat, $tab_clas)) {
						...
						if(!isset($divisions[$i]['services'][$j]['enseignants'])) {
							echo "<p style='color:red;'>No teacher is associated this service.<br /> Has the timetable went up towards STS?</p>\n";
						...
						echo "Association of the students:<br />";
						echo "<blockquote>\n";
	...
	echo "<h3>Creation of groups</h3>\n";
	// Traiter les groupes ensuite
	if(!isset($groupes)) {
		echo "<p>No group is defined.</p>\n";		
	}
	else {
		for ($i=0;$i<count($groupes);$i++) {
				...
				if($nb_clas==1) {
					...
					if(mysql_num_rows($periode_query)>0) {
						...
						if(count($tab_test)==1) {
							...
							echo "<p style='color:red'>PROBLEM&nbsp;: Classes not having the same number of periods are associated in the same group.</p>\n";
						}
			...
			if(!isset($groupes[$i]['code'])) {
				echo "<p style='color:red'>The group n°$i does not have the standard form in the XML of STS .<br />It cannot be created in GEPI.</p>\n";
			}
			elseif(!isset($groupes[$i]['grp'])) {
				echo "<p style='color:red'>The group n°$i named in STS <b>".$groupes[$i]['code']."</b> is associated no course in STS???<br />\n";
				...
				echo "<p><span style='color:red'>Nevertheless the group is created.<br />You will have to re-examine association with a course.</span><br />Creation of the group $descr_grp (<i>$nom_grp</i>) in $list_classe";
				echo " (<i style='font-size:x-small;'>name sts: ".$code_groupe."</i>)";
				echo ": ";
				if($id_groupe=create_group($nom_grp, $descr_grp, $mat, $tab_clas)) {
					echo "<span style='color:green;'>$id_groupe</span>";
					//echo "<br />\n";
				...
				for($i_grp=0;$i_grp<count($groupes[$i]['grp']);$i_grp++) {
					...
					echo "<p>Creation of the group $descr_grp (<i>$nom_grp</i>) en $list_classe";
					echo " (<i style='font-size:x-small;'>name sts: ".$code_groupe."</i>)";
					echo ": ";
					if($id_groupe=create_group($nom_grp, $descr_grp, $mat, $tab_clas)) {
						echo "<span style='color:green;'>$id_groupe</span>";
						...
						if((!isset($groupes[$i]['grp'][$i_grp]['enseignant']))||(count($groupes[$i]['grp'][$i_grp]['enseignant'])==0)) {
							echo "<span style='color:red'>No professor is associated to this group</span>";
						}
						else {
	...
	*/
	...
	if($temoin_div_sans_services==0) {
		echo "<p>The importation of the relations professors/courses and professors/classes in base GEPI was carried out successfully !<br />You can proceed at the next stage of importation of the options followed by the students.</p>";
	}
	elseif($temoin_div_sans_services==1) {
		echo "<p style='color:red;'>$temoin_div_sans_services division does not have declared services.<br />Provided file STS is perhaps not complete.<br />That arrives in particular when the timetable did not go up towards STS.<br />You should control that before proceeding at the stage of importation of the options followed by the students.</p>\n";
	}
	else {
		echo "<p style='color:red;'>$temoin_div_sans_services divisions do not have declared services.<br />Provided file STS is perhaps not complete.<br />
		That arrives in particular when the timetable did not go up towards STS.<br />You should control that before proceeding at the stage of importation of the options followed by the students.</p>\n";
	}

	if($temoin_service_sans_enseignant==1) {
		echo "<p style='color:red;'>$temoin_service_sans_enseignant course (<i>service</i>) was declared without associated teacher.<br />The students practise self-tuition in autonomy or the STS is it badly indicated?</p>\n";
	}
	elseif($temoin_service_sans_enseignant>1) {
		echo "<p style='color:red;'>$temoin_service_sans_enseignant courses (<i>services</i>) were declared without associated teacher.<br />The students practise self-tuition in autonomy or the STS is it badly indicated?</p>\n";
	}

	if($nb_groupes_sans_matieres!=0) {
		echo "<p style='color:red;'>$nb_groupes_sans_matieres courses were declared without associated course.<br />You will have to correct the associated course as well as the
associated professor.</p>\n";
	}

	//}
	echo "<p align='center'><a href='init_options.php?a=a".add_token_in_url()."'>Import the options followed by the students</a></p>\n";
	echo "<p><br /></p>\n";
	...
	if (!$fp) {
		...
		*/
		...
		affiche_debug("=======================================================<br />\n");
		affiche_debug("One finished the cleaning of the file F_GPD<br />\n");
		affiche_debug("=======================================================<br />\n");
		unset($en_tete2);
		...
		for($k = 1; ($k < $nblignes+1); $k++){
			...
			if(!feof($fp)){
				...
				if(trim($ligne)!=""){
					...
					affiche_debug("One seeks if a teacher ensures the course corresponding to the group: select col1 from tempo2 where col2 = '$affiche[1]'<br />\n");
					...
					if ($login_prof != '') {
						...
						affiche_debug("One (at least) prof found: $login_prof<br />\n");
						...
						if($resverif == 0) {
							...
							echo "<p>Addition of the correspondence next prof/course : $login_prof/$affiche[0]<br />\n";
							if(!$req) $nb_reg_no++;
						}
						...
						for($i=0;$i<count($tabtmp);$i++){
							...
							if ($id_classe != '') {
								...
								if($resverif == 0) {
									...
									if ($resverif2 == 0) {
										affiche_debug("The group does not exist yet for the class \$id_classe=$id_classe<br />\n");
										...
										if($temoin_groupe_deja_cree=="non"){
											...
											echo "Creation of a group (n°$group_id) for the course $affiche[0], \n";
											...
											affiche_debug("Group already created with \$group_id=$group_id<br />");
										}
										...
										*/
										...
										if($nb_eleves>0){
											echo "Addition to this group of the following students: ";
											...
											echo "No student in this group???<br />\n";
										}
										...
										affiche_debug("The group already exists for the class \$id_classe=$id_classe, one add the professor $login_prof to the group:<br />\n");
										$group_id = mysql_result($verif2, 0);
										$res = mysql_query("insert into j_groupes_professeurs set id_groupe = '" . $group_id . "', login ='" . $login_prof . "'");
										affiche_debug("insert into j_groupes_professeurs set id_groupe = '" . $group_id . "', login ='" . $login_prof . "'<br />\n");
										echo "Addition of $login_prof to an existing group (<i>more than one professor for this group</i>).<br />\n";
		...
		*/
		...
		echo "<p>Control in the page if you do not have an error (<i>announced in red if necessary</i>), then you can proceed at the next stage of importation of the options followed by the students.</p>";

		echo "<center><p><a href='init_options.php?a=a".add_token_in_url()."'>Import the options followed by the students</a></p></center>";
		echo "<p><br /></p>\n";
	}
}
require("../lib/footer.inc.php");
