<?php
	...
	//**************** EN-TETE *****************
	$titre_page = "Tool of initialization of the year : Importation CSV UDT";
	...
	if(isset($_GET['nettoyage'])) {
		...
		echo "<p>If the CSV of UDT is present, it will be removed...</p>\n";
	...
	// =======================================================
		...
		echo "<center><h3 class='gepi'>First phase of the importation CSV UDT</h3></center>\n";
		...
		echo " | <a href='".$_SERVER['PHP_SELF']."?nettoyage=oui".add_token_in_url()."'>Suppression of an existing CSV</a>";
		echo "</p>\n";
		//echo "</div>\n";

		//if(!isset($_POST['is_posted'])) {
		if(!isset($step)) {
			echo "<p class='bold'>Upload of the file of export of UDT.</p>\n";

			echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post'>\n";
			echo "<p>Please provide the file of export CSV of UDT&nbsp;:<br />\n";
			...
			echo "<ul>\n";
			echo "<li>In UDT, the step is menu Research/Timetable, validate research, then click on Exporting.</li>\n";
			echo "<li>If generated export is in format TXT (<i>separator tabulation</i>), go in the menu Tools/Preferences to choose CSV rather than TXT for export.</li>\n";
			echo "<li>The fields of the CSV are&nbsp;:<br />Day;Hour;Div;Course;Professor;Room;Group;Regroup;Eff;Mo;Freq;Aire;</li>\n";
			...
			echo "<li>If you have an export <b>STS_EMP_&lt;RNE&gt;_&lt;ANNEE&gt;</b> after sending of the timetable towards STS, it is to better carry out the <a href='prof_disc_classe_csv.php?a=a".add_token_in_url()."'>initialization of the courses</a> starting from this file.</li>\n";
			echo "</ul>\n";
		}
		else {
				...
				if(!is_uploaded_file($csv_file['tmp_name'])) {
					...
					*/
					...
					if(!$res_copy) {
						...
						echo "<p>Please supplement the useful correspondences between UDT and GEPI&nbsp;:</p>\n";
						...
						if(!$fp) {
							echo "<p>ERROR during attempt of opening of the file $dest_file</p>\n";
							require("../lib/footer.inc.php");
							die();
						}
						...
						for($i=0;$i<count($ligne);$i++) {
							...
							if($ligne[$i]!='') {
								...
								if(mysql_num_rows($test)==0) {
									...
									//echo "$sql<br />\n";
									$insert=mysql_query($sql);
									if(!$insert) {echo "Error on $sql<br />\n";}
								}
						...
						if(mysql_num_rows($res)==0) {
							echo "<p>ERROR&nbsp;: There is not any class in the base.</p>\n";
							require("../lib/footer.inc.php");
							die();
						}
						...
						echo "<blockquote>\n";
						echo "<table class='boireaus' summary='Correspondences of classes'>\n";
						echo "<tr>\n";
						echo "<th>Class  in UDT</th>\n";
						echo "<th>Class in Gepi</th>\n";
						echo "</tr>\n";
						$alt=1;
						...
						$res=mysql_query($sql);
						if(mysql_num_rows($res)==0) {
							echo "<p>ERROR&nbsp;: There is not any course in the base.</p>\n";
							require("../lib/footer.inc.php");
							die();
						}
						...
						echo "<p><b>Courses</b>&nbsp;:</p>\n";
						echo "<blockquote>\n";
						echo "<table class='boireaus' summary='Course correspondences'>\n";
						echo "<tr>\n";
						echo "<th>Course in UDT</th>\n";
						echo "<th>Course in Gepi</th>\n";
						echo "<th>Info</th>\n";
						echo "</tr>\n";
						$alt=1;
						for($i=0;$i<count($udt_matiere);$i++) {
							...
							if($matiere_trouvee=="n") {
								...
								if(($res_info)&&(mysql_num_rows($res_info)>0)) {
									echo "<table align='center' class='boireaus' summary='Information on the course $udt_matiere[$i]'>\n";
									$info_alt=1;
						...
						$res=mysql_query($sql);
						if(mysql_num_rows($res)==0) {
							echo "<p>ERROR&nbsp;: There is not any professor in the base.</p>\n";
							require("../lib/footer.inc.php");
							die();
						}
						...
						echo "<blockquote>\n";
						echo "<table class='boireaus' summary='Correspondences of professors'>\n";
						echo "<tr>\n";
						echo "<th>Professor in UDT</th>\n";
						echo "<th>Professor in Gepi</th>\n";
						echo "<th>Info</th>\n";
						echo "</tr>\n";
						$alt=1;
						for($i=0;$i<count($udt_prof);$i++) {
							...
							if($prof_trouve=="n") {
								...
								$res_info=mysql_query($sql);
								if(($res_info)&&(mysql_num_rows($res_info)>0)) {
									echo "<table class='boireaus' summary='Information on the professor $udt_prof[$i]'>\n";
									$info_alt=1;
									while($lig_info=mysql_fetch_object($res_info)) {
										$info_alt=$info_alt*(-1);
										echo "<tr class='lig$info_alt'>\n";
						...
						echo "<input type='hidden' name='is_posted' value='yes' />\n";
						echo "<p style='text-align:center;'><input type='submit' value='Validate' /></p>\n";
						echo "</form>\n";

						//echo "<p style='color:red'>A FAIRE: Pouvoir n'afficher que les comptes actifs</p>\n";
						echo "<p style='color:red'>If you do not wish to create groups of LIFE OF CLASS, REMEDIATION, SUPPORT,... because no note must be typed in these 'courses', it is enough to not make correspond no GEPI course with UDT course .</p>\n";
					}
				}
			}
			elseif($step==1) {
				...
				if(!isset($_POST['is_posted'])) {
					echo "<p style='color:red'>ERROR&nbsp;: Part of the variables were not Posted.<br />You probably have a module PHP which limits the number of transmitted variables (<i>suhosin?</i>)</p>\n";
					require("../lib/footer.inc.php");
					die();
				}
				...
				echo "<p>Suppression of the courses, associations students/courses, classes/courses et professors/courses.</p>\n";
...
*/
				...
				*/
				...
				if(mysql_num_rows($res_classes)==0) {
					echo "<p style='color:red'>No class exists yet.</p>\n";
					require("../lib/footer.inc.php");
					die();
				}
				...
				if(mysql_num_rows($res_profs)==0) {
					echo "<p style='color:red'>No teacher exists yet.</p>\n";
					require("../lib/footer.inc.php");
					die();
				}
				...
				if(mysql_num_rows($res)>0) {
					while($lig=mysql_fetch_object($res)) {
						if(in_array($lig->id,$lignes_deja_traitees)) {
							...
							echo "mo=$lig->mo<br />\n";
							*/
							echo "<p>Association $lig->division/$lig->matiere/$lig->prof";
							if($lig->groupe!="") {echo " (<i>$lig->groupe</i>)";}
							if($lig->regroup!="") {echo " (<i>$lig->regroup</i>)";}
							echo " is already treated.";
							echo "<hr />\n";
						}
						else {
							/*
	...
	// Il faudrait alors les récupérer... et ne pas traiter à nouveau les lignes correspondantes de udt_lignes quand on va les atteindre...
							...
							if($lig->regroup!="") {
								...
								$res_clas=mysql_query($sql);
								if(mysql_num_rows($res_clas)==0) {
									echo "<p>No class is associated $lig->division in Gepi.</p>\n";
								}
								elseif(mysql_num_rows($res_clas)>1) {
									echo "<p>ANOMALY: More than one class is associated $lig->division in Gepi.</p>\n";
								}
								else {
							...
							$res_mat=mysql_query($sql);
							if(mysql_num_rows($res_mat)==0) {
								echo "<p>No course is associated in Gepi to the course $lig->matiere of UDT.</p>\n";
							}
							elseif(mysql_num_rows($res_mat)>1) {
								echo "<p>ANOMALY: More than one course is associated in Gepi to the course $lig->matiere of UDT.</p>\n";
							}
							else {
								$lig_mat=mysql_fetch_object($res_mat);
								$mat=$lig_mat->matiere;
								...
								if(count($tab_clas)==0) {
									echo "<p clas='color:red'>No class is associated the group of $mat.</p>\n";
								}
								else {
									//echo "<b>Récapitulatif avant création du groupe&nbsp;:</b>\n";
									echo "<p>Creation of the group $nom_grp ($descr_grp) associated to the course $mat ";
									$cpt_clas=0;
									echo "(<i>";
									...
									if($id_groupe=create_group($nom_grp, $descr_grp, $mat, $tab_clas)) {
										...
										echo "SUCCES&nbsp;: Group created with the id_groupe $id_groupe<br />\n";
	...
	mysql>
										...
										*/
										...
										$res_prof=mysql_query($sql);
										if(mysql_num_rows($res_prof)>0) {
											echo "Association of the professors to the group&nbsp;: ";
											$cpt_prof=0;
											while($lig_prof=mysql_fetch_object($res_prof)) {
												if($cpt_prof>0) {echo ", ";}
												...
												if(mysql_num_rows($test)==0) {
													...
													$insert=mysql_query($sql);
													if(!$insert) {echo "<br /><b>ERROR</b> during association of the professor $lig_prof->nom_gepi with the course $mat<br />\n";}
													else {echo " (<i>association of the professor with the course $mat</i>)";}
												}
												$cpt_prof++;
											}
										}
										else {
											echo "<span style='color:red'>No professor is associated the group.</span>\n";
										}
										echo "<br />\n";
										...
										if(!$update_group) {
											echo "<span style='color:red;'>ERROR during association of the professor(s) and student(s) with the group.</span><br />\n";
										}
									}
									else {
										echo "<span style='color:red;'>ERROR during creation of the group for...</span><br />\n";
									}
				...
				echo "<center><p><a href='init_pp.php?a=a".add_token_in_url()."'>Importation of the principal professors</a><br />It is probable that this information was not in the file of STS, the importation of the principal professors is likely to give nothing  ... but which does not try anything...</p></center>\n";
