...
@set_time_limit(0);
/*
 * $Id: prof_disc_classe.php 5937 2010-11-21 17:42:55Z crob $
...
if (!isset($is_posted)) {
    ...
    echo "<p>Importation of the files <b>F_men.dbf</b> and <b>F_gpd.dbf</b> containing the data of relations between professors, course and classes.";
    echo "<form enctype='multipart/form-data' action='prof_disc_classe.php' method=post>";
    echo "<p>Please specify the complete name of the file <b>F_men.dbf</b>.";
    echo "<p><input type='file' size='80' name='dbf_file' />";
    echo "<p>Please specify the complete name of the file <b>F_gpd.dbf</b>.";
    ...
    if ((strtoupper($dbf_file['name']) == "F_MEN.DBF") or (strtoupper($dbf_file2['name']) == "F_GPD.DBF")) {
        ...
        if (!$fp) {
            echo "<p>Impossible to open the file F_MEN.DBF !</p>";
            @dbase_close($fp2);
            echo "<a href='prof_disc_classe.php'>Click here </a> to restart !</center></p>";
        } else if (!$fp2) {
            echo "<p>Impossible to open the file F_GPD.DBF !</p>";
            @dbase_close($fp);
            echo "<a href='prof_disc_classe.php'>Click here </a> to restart !</center></p>";
        } else {
            ...
            if (@dbase_get_record_with_names($fp2,1)) {
                ...
                echo "<p>The file F_GPD.DBF selected is not valid !<br />";
                echo "<a href='prof_disc_classe.php'>Click here </a> to restart !</center></p>";
                die();
            }
            $nb = 0;
...
*/
			...
			if ($id_classe != '') {
				...
				if($resverif == 0) {
					...
					if ($resverif2 == 0) {
						...
						affiche_debug("The group already exists for the class \$id_classe=$id_classe, we add the professor  $login_prof to the group:<br />\n");
		...
		affiche_debug("===================================================<br />\n");
            ...
            if ($nb_reg_no != 0) {
                echo "<p>During recording of the data there was  $nb_reg_no errors. Test find the cause of the error and restart the procedure before passing at the following stage.";
            } else {
                echo "<p>The importation of the relations professors/courses and professors/classes in base GEPI was carried out successfully !<br />You can proceed to the next stage of importation of the options followed by the students.</p>";
        ...
        echo "<a href='prof_disc_classe.php'>Click here </a> to restart !</p>";
