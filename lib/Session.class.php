  ...
  private $cas_extra_attributes = false; # D'éventuels attributs chargés depuis la réponse CAS
	...
	public function authenticate($_login = null, $_password = null) {
	    ...
	    if (isset($GLOBALS['liste_noire_ip']) && in_array($_SERVER['REMOTE_ADDR'], $GLOBALS['liste_noire_ip'])) {
		  tentative_intrusion(1, "Attempt of connection from a IP on black list (login used : ".$_login.")");
	      return "3";
		  die();
		}
	...
	public function security_check() {
		...
		if ($this->auth_simpleSAML == 'yes') {
			...
			tentative_intrusion(1, "Access to a page without being identified (can come from a timeout of
session).");
			return "0";
			exit;
		}
			...
			# Un compte existe avec ce login
                        if ($db_salt == '') {
                            ...
                            if (mysql_num_rows(mysql_query("SHOW COLUMNS FROM utilisateurs LIKE 'salt';"))>0) {
                                ...
                                if ($update_query) {
                                    $this->debug_login_mdp($debug_test_mdp, $debug_test_mdp_file, 'Password ameliore in hmac'."\n");
                                } else {
                                    $this->debug_login_mdp($debug_test_mdp, $debug_test_mdp_file, 'Echec password ameliore in hmac'."\n");
                                }
                            }
                        } else {
                            //login deja en hmac sha256
                            if ($db_password == hash_hmac('sha256', $_password, $db_salt)) {
                                    $this->debug_login_mdp($debug_test_mdp, $debug_test_mdp_file, 'Authentification hmac OK without modification'."\n");
                            } else {
                                    if(getSettingValue('filtrage_html')=='htmlpurifier') {
                                            $tmp_mdp = array_flip (get_html_translation_table(HTML_ENTITIES));
                                            $_password_unhtmlentities = strtr ($_password, $tmp_mdp);
                                            if ($db_password == hash_hmac('sha256', $_password_unhtmlentities, $db_salt)) {
                                                    $this->debug_login_mdp($debug_test_mdp, $debug_test_mdp_file, 'Authentification hmac OK with unhtmlentities()'."\n");
                                            } else {
                                                    $this->debug_login_mdp($debug_test_mdp, $debug_test_mdp_file, 'Authentification hmac in failure with and without modification unhtmlentities'."\n");
                                                   return false;
                                            }
                                    } else {
                                            if ($db_password == hash_hmac('sha256', htmlentities($_password), $db_salt)) {
                                                    ...
                                                    $this->debug_login_mdp($debug_test_mdp, $debug_test_mdp_file, 'Authentification hmac in failure with and without modification htmlentities'."\n");
                                                    return false;
                                            }
                                    }
                            }
...
*/
    ...
    private function test_loginsso()
  {
	...
	public function logout_cas() {
		...
		if ($cas_use_logout) {
			...
			if ($cas_logout_url != '') {
				...
				echo '<html><head><title>GEPI</title></head><body><h2>You are disconnected.</h2></body></html>';
				exit();
			}
		...
		*/
		...
		if ($idpers) {
			...
			if (!$db_c || !mysql_select_db ($dbDb)) {
				echo "\n<p>Error : Failure of connection to the data base";
				exit;
			}
			$this->login = $login;
			$this->current_auth_mode = "sso";
	...
	public function record_failed_login($_login) {
		...
		if ($test_login != "0") {
			tentative_intrusion(1, "Attempt of connection with an incorrect password. It can be simply a typing error. This alarm is significant only in case of repetition. (login : ".$_login.")");
			# On a un vrai login.
			...
			tentative_intrusion(1, "Attempt of connection with an incorrect login (do not exist in the Gepi base). It can be simply a typing error. This alarm is significant only in the event of repetition. (login used : ".$_login.")");
	...
	private function lock_account($_login) {
       ...
       tentative_intrusion(2, "Locking of the account ".$_login." because of a too great number of unfruitful attempts at connection. It can be an attempt of attack rough-force.");
       return true;
	}
