<?php
/*
* $Id: bulletin_simple.inc.php 8693 2011-12-01 13:40:09Z crob $
*
...
if ($on_continue == 'yes') {
	...
	if ($current_eleve_sexe == "M") {
		echo ", born the $current_eleve_naissance";
		} else {
		echo ", born the $current_eleve_naissance";
	}
	if ($current_eleve_regime == "d/p") {echo ",&nbsp;school luncher";}
	if ($current_eleve_regime == "ext.") {echo ",&nbsp;external";}
	if ($current_eleve_regime == "int.") {echo ",&nbsp;intern";}
	if ($current_eleve_regime == "i-e")
		if ($current_eleve_sexe == "M") echo ",&nbsp;intern&nbsp;externed"; else echo ",&nbsp;intern&nbsp;externed";
	if ($current_eleve_doublant == 'R')
		if ($current_eleve_sexe == "M") echo ", <b>redoubling</b>"; else echo ", <b>redoubling</b>";
	echo "&nbsp;&nbsp;-&nbsp;&nbsp;Class of $current_eleve_classe, school year $gepiYear<br />\n";
	...
	echo "<table width=$larg_tab class='boireaus' cellspacing='1' cellpadding='1' summary='Courses/Notes/Appreciations'>\n";
	//=========================
	...
	echo "<td width=\"$larg_col3\" align=\"center\" class='bull_simpl'>student</td>\n";
	if ($affiche_rang=='y') {
		echo "<td width=$larg_col4 align=\"center\" class='bull_simpl'><i>Rank</i></td>\n";
	}
	echo "<td width=\"$larg_col5\" class='bull_simpl'>$bull_intitule_app</td>\n";
	if($afficher_signalement_faute=='y') {
		// A N'INSERER QUE POUR LES COMPTES DE PERSONNELS... de façon à éviter de donner les mails des profs à des élèves
		echo "<td class='bull_simpl noprint'>Signal</td>\n";
	}
	echo "</tr>\n";
...
*/
				...
				if($afficher_signalement_faute=='y') {
					...
					if($current_group["classe"]["ver_periode"][$id_classe][$nb]=='O') {
						...
						echo "<a href=\"mailto:$liste_email_profs_du_groupe?Subject=[Gepi]: Signal a problem/faute&body=Hello,I think that you made a typing error for $current_eleve_login in the course n°".$current_group['id'].".Cordialement.-- ".casse_mot($_SESSION['prenom'],'majf2')." ".$_SESSION['nom']."\"";
						...
						echo "><img src='../images/icons/mail.png' width='16' height='16' alt='Signal a problem/fault by mall' /></a>";
	...
	if($display_moy_gen=="y") {
		if ($test_coef != 0) {
			echo "<tr>\n<td";
			if ($nb_periodes > 1) echo " rowspan=".$nb_periodes;
			echo ">\n<p class='bull_simpl'><b>General average</b></p>\n</td>\n";
			//====================
			// Modif: boireaus 20070626
			if($affiche_coef=='y'){
				echo "<td";
	...
	echo "<span class='bull_simpl'><b>Absences and delays:</b></span>\n";
	//echo "<table width=$larg_tab border=1 cellspacing=1 cellpadding=1>\n";
	echo "<table width='$larg_tab' class='boireaus' cellspacing='1' cellpadding='1' summary='Absences and delays'>\n";
	$nb=$periode1;
	while ($nb < $periode2+1) {
	...
	if ($test_eleve_app != 0) {
		...
		if ($eleve_abs_j[$nb] == "1") {
			echo "Justified absences : one half-day";
		} else if ($eleve_abs_j[$nb] != "0") {
			echo "Justified absences : $eleve_abs_j[$nb] half-days";
		} else {
			echo "No justified absence";
		}
		echo "</td>\n";
		echo "<td valign=top class='bull_simpl' style='$style_bordure_cell'>\n";
		if ($eleve_abs_nj[$nb] == '1') {
			echo "Not justified absences : one half-day";
		} else if ($eleve_abs_nj[$nb] != '0') {
			echo "Not justified absences : $eleve_abs_nj[$nb] half-days";
		} else {
			echo "No not justified absence";
		}
		echo "</td>\n";
	...
	echo "<span class='bull_simpl'><b>Opinion of the staff meeting </b> ";
	...
	echo "<table width=\"$larg_tab\" class='boireaus' cellspacing='1' cellpadding='1' summary='Opinion of the staff meeting'>\n";
	$nb=$periode1;
	while ($nb < $periode2+1) {
...
<input type='button' onclick='valider_signalement_faute()' name='Envoyer' value='Send' />\n";
...
echo "</form>\n";
	...
	function signaler_une_faute(eleve_login, id_eleve, id_groupe, liste_profs_du_groupe, num_periode) {
		...
		document.getElementById('titre_entete_signaler_faute').innerHTML='Signal a problem/fault for '+info_eleve+' period '+num_periode;
		...
		message='Hello,\\n\\nThe appreciation of the student '+info_eleve+' on the course n°'+id_groupe+' ('+info_groupe+') in period n°'+num_periode+' present a problem or a fault:\\n';
		message=message+'================================\\n';
