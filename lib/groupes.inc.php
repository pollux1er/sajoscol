 ...
 * 
 * $Id: groupes.inc.php 8614 2011-11-09 13:48:49Z crob $
 * 
...
function update_group($_id_groupe, $_name, $_description, $_matiere, $_classes, $_professeurs, $_eleves) {
    ...
    foreach ($new_classes as $id_classe) {
        if (get_period_number($id_classe)!= $check_periods) {
            $per_error = true;
			$msg.="ERROR: get_period_number($id_classe)=".get_period_number($id_classe)." mais \$check_periods=$check_periods<br />";
        }
    }
    if (!$per_error) {
        ...
        foreach($former_groupe["periodes"] as $period) {
            ...
            foreach ($deleted_eleves as $e_login) {
                if (test_before_eleve_removal($e_login, $_id_groupe, $period["num_periode"])) {
                    ...
                    $msg .= "Error during suppression of the student having the login '" . $e_login . "', for the period '" . $period["num_periode"] . " (notes or appreciations exist).<br/>";
                }
            }
        }
    }
...
function delete_group($_id_groupe) {
    ...
    if (!$del1) $errors .= "Error during suppression of the link group-course.<br/>";
    $del2 = mysql_query("DELETE from j_groupes_professeurs WHERE id_groupe = '" . $_id_groupe . "'");
    if (!$del2) $errors .= "Error during suppression of the link group-professors.<br/>";
    $del3 = mysql_query("DELETE from j_eleves_groupes WHERE id_groupe = '" . $_id_groupe . "'");
    if (!$del3) $errors .= "Error during suppression of the link group-students.<br/>";
    $del4 = mysql_query("DELETE from j_groupes_classes WHERE id_groupe = '" . $_id_groupe . "'");
    if (!$del4) $errors .= "Error during suppression of the link group-classes.<br/>";
    $del5 = mysql_query("DELETE from groupes WHERE id = '" . $_id_groupe . "'");
    if (!$del5) $errors .= "Error during suppression of the group.<br/>";
    ...
    if ($text_ct > 0) $errors .= "Attention a textbook related to the removed group is now \"orphan\". Appointment in the module \"textbook\" to solve the problem.<br/>";
    $del7 = mysql_query("DELETE from j_signalement WHERE id = '" . $_id_groupe . "'");
    if (!$del7) $errors .= "Error during suppression of the group.<br/>";
    $del8 = mysql_query("DELETE from j_groupes_visibilite WHERE id = '" . $_id_groupe . "'");
    if (!$del8) $errors .= "Error during suppression of the group.<br/>";
