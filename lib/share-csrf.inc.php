...
function check_token($redirection=TRUE) {
	...
	if(getSettingValue('csrf_mode')=='strict') {
		if($csrf_alea!=$_SESSION['gepi_alea']) {
			...
			if($redirection) {
				header("Location: $pref_arbo/accueil.php?msg=Unauthorized operation");
			}
			else {
				echo "<p style='color:red'>Unauthorized operation</p>\n";
...
function action_alea_invalide($envoyer_mail=TRUE) {
	...
	$details="The person victim of the attack was ".$_SESSION['login'].".\n";
	$details.="The page targets was ".$_SERVER['PHP_SELF']." with the following variables:\n";
	...
	if($envoyer_mail) {
		...
		if($envoi_mail_actif!="n") {
			...
			if($destinataire!='') {
				$sujet="Attaque CSRF";
				$message="The variable csrf_alea not coincide with gepi_alea in SESSION.\n";
				$message.=$details;
