...
function make_area_list_html($link, $current_classe, $current_matiere, $year, $month, $day) {
  echo "<strong><em>Textbook of&nbsp;:</em></strong><br />";
...
function affiche_devoirs_conteneurs($id_conteneur,$periode_num, &$empty, $ver_periode) {
	...
	if(getSettingValue("gepi_denom_boite_genre")=='m'){
		$message_cont = "Are you sure you want to remove the ".getSettingValue("gepi_denom_boite")." below ?";
		$message_cont_non_vide = "The ".getSettingValue("gepi_denom_boite")." is nonempty. It cannot be removed.";
	}
	else{
		$message_cont = "Are you sure you want to remove ".getSettingValue("gepi_denom_boite")." below ?";
		$message_cont_non_vide = "The ".getSettingValue("gepi_denom_boite")." is nonempty. It cannot be removed.";
	}
	$message_dev = "Are you sure you want to remove the evaluation below and the notes which it contains ?";
			...
			if($nb_dev>0){
				...
				while ($j < $nb_dev) {
					...
					if(mysql_num_rows($test_anomalie)>0) {
						$titre_infobulle="Note for a phantom";
						$texte_infobulle="One or more notes exist for students which are not registered any more in this course&nbsp;:<br />";
						$cpt_ele_anomalie=0;
						...
						$texte_infobulle.="<br />";
						$texte_infobulle.="Click <a href='".$_SERVER['PHP_SELF']."?id_groupe=$id_groupe&amp;periode_num=$periode_num&amp;clean_anomalie_dev=$id_dev".add_token_in_url()."'>here</a> to remove the associated notes?";
					...
					if (getSettingValue("utiliser_sacoche") == 'yes') {
						echo " - <a href='#' onclick=\"document.getElementById('sacoche_form_".$j."').submit();\">Evaluate by competence</a>";
					}
					...
					if($display_parents==1) {echo "<img src='../images/icons/visible.png' width='19' height='16' title='Evaluation visible  on the report booklet' alt='Evaluation visible on the report booklet' />";}
					else {echo " <img src='../images/icons/invisible.png' width='19' height='16' title='Evaluation not visible on the report booklet' alt='Evaluation not visible on the report booklet' />\n";}
					echo "</i>)";
					echo " - <a href = 'index.php?id_racine=$id_racine&amp;del_dev=$id_dev".add_token_in_url()."' onclick=\"return confirmlink(this, 'suppression of ".traitement_magic_quotes($nom_dev)."', '".$message_dev."')\">Suppression</a>\n";
					echo "</li>\n";
					$j++;
				}
				echo "</ul>\n";
		...
		if($nb_cont>0) {
			...
			while ($i < $nb_cont) {
				...
				if ($id_cont != $id_parent) {
					echo "<li>\n";
					echo "$nom_conteneur - <a href='saisie_notes.php?id_conteneur=$id_cont'>Visualization</a>";
					...
					echo " (<i><span title='Coefficient $coef'>$coef</span> ";
					if($display_bulletin==1) {echo "<img src='../images/icons/visible.png' width='19' height='16' title='$gepi_denom_boite visible on the bulletin' alt='$gepi_denom_boite visible on the bulletin' />";}
					else {echo " <img src='../images/icons/invisible.png' width='19' height='16' title='$gepi_denom_boite non visible on the bulletin' alt='$gepi_denom_boite non visible on the bulletin' />\n";}
					echo "</i>)";
					...
					if(($nb_dev==0)&&($nb_sous_cont==0)) {
						echo " - <a href = 'index.php?id_racine=$id_racine&amp;del_cont=$id_cont".add_token_in_url()."' onclick=\"return confirmlink(this, 'suppression of ".traitement_magic_quotes($nom_conteneur)."', '".$message_cont."')\">Suppression</a>\n";
					}
					else {
					...
					if($nb_dev>0) {
						...
						while ($j < $nb_dev) {
							...
							$id_dev = mysql_result($appel_dev, $j, 'id');
							echo "<li>\n";
							echo "<font color='green'>$nom_dev</font> - <a href='saisie_notes.php?id_conteneur=$id_cont&amp;id_devoir=$id_dev'>Typing</a>";
							...
							$res_eff_dev=mysql_query($sql);
							$eff_dev=mysql_num_rows($res_eff_dev);
							echo " <span title=\"Manpower of the notes typed/total manpower of the course\" style='font-size:small;";
							if(isset($eff_groupe)) {if($eff_dev==$eff_groupe) {echo "color:green;";} else {echo "color:red;";}}
							echo "'>($eff_dev";
							if(isset($eff_groupe)) {echo "/$eff_groupe";}
							echo ")</span>";
							...
							if(mysql_num_rows($test_anomalie)>0) {
								$titre_infobulle="Note for a phantom";
								$texte_infobulle="One or more notes exist for one or students which are not registered any more in this course&nbsp;:<br />";
								$cpt_ele_anomalie=0;
								while($lig_anomalie=mysql_fetch_object($test_anomalie)) {
									if($cpt_ele_anomalie>0) {$texte_infobulle.=", ";}
									$texte_infobulle.=get_nom_prenom_eleve($lig_anomalie->login,'avec_classe')."&nbsp;(<i>";
...
function parametres_tableau($larg_tab, $bord) {
    echo "<table border='1' width='680' cellspacing='1' cellpadding='1' summary=\"Table of parameters\">\n";
...
function affiche_tableau($nombre_lignes, $nb_col, $ligne1, $col, $larg_tab, $bord, $col1_centre, $col_centre, $couleur_alterne) {
	...
	echo "<table border=\"$bord\" class='boireaus' cellspacing=\"0\" width=\"$larg_tab\" cellpadding=\"1\" summary=\"Table\">\n";
    echo "<tr>\n";
    $j = 1;
    while($j < $nb_col+1) {
...

{
  ...
  $out_html = "<form name=\"classe\" method=\"post\" action=\"".$_SERVER['PHP_SELF']."\"><strong><em>Class :</em></strong><br />
  " . $aff_input_rne . "
  <select name=\"classe\" onchange=\"classe_go()\">";

  $out_html .= "<option value=\"".$link."?year=".$year."&amp;month=".$month."&amp;day=".$day."&amp;id_classe=-1\">(Choose a class)";
...
function make_matiere_select_html($link, $id_ref, $current, $year, $month, $day, $special='')
{
	...
	<h2 class='h2_label'> \n<label for=\"enseignement\"><strong><em>Course :<br /></em></strong></label>\n</h2>\n<p>\n<select id=\"enseignement\" name=\"matiere\" onchange=\"matiere_go()\">\n ";
	
	if (is_numeric($id_ref)) {
		$out_html .= "<option value=\"".$link."?&amp;year=".$year."&amp;month=".$month."&amp;day=".$day."&amp;id_classe=$id_ref\">(Choose a course)</option>\n";
		...
		if($special!='') {
			...
			$out_html .= "<option $selected value=\"$link2\"$selected>All courses</option>\n";
		}
		...
		$out_html .= "<option value=\"".$link."?&amp;year=".$year."&amp;month=".$month."&amp;day=".$day."&amp;login_eleve=$id_ref\">(Choose a course)</option>\n";
...
function make_eleve_select_html($link, $login_resp, $current, $year, $month, $day)
{
	...
	if (mysql_num_rows($get_eleves) == 0) {
			// Aucun élève associé
		$out_html = "<p>You seem to be responsible of no student ! Contact the administrator to correct this error.</p>";
		...
		$out_html = "<p class='bold'>Student : ".$selected_eleve->prenom." ".$selected_eleve->nom."</p>";
	} else {
		// Plusieurs élèves : on affiche un formulaire pour choisir l'élève
	  $out_html = "<form id=\"eleve\" method=\"post\" action=\"".$_SERVER['PHP_SELF']."\">\n<h2 class='h2_label'>\n<label for=\"choix_eleve\"><strong><em>Student :</em></strong></label>\n</h2>\n<p>\n<select id=\"choix_eleve\" name=\"eleve\" onchange=\"eleve_go()\">\n";
	  $out_html .= "<option value=\"".$link."?year=".$year."&amp;month=".$month."&amp;day=".$day."\">(Choose a student)</option>\n";
...
function affiche_docs_joints($id_ct,$type_notice) {
  ...
  $res = sql_query($sql);
    if (($res) and (sql_count($res)!=0)) {
      $html .= "<span class='petit'>Joined documents:</span>";
...
function test_ecriture_dossier($tab_restriction=array()) {
    ...
    echo "<caption style='display:none;'>folders having to be accessible in writing<caption>\n";
	echo "<tr>\n";
	echo "<th>Folder</th>\n";
	echo "<th>Writing</th>\n";
	echo "</tr>\n";
	...
	for($i=0;$i<count($tab_dossiers_rw);$i++) {
		...
		echo "<td>";
		if($ok_rw=='yes') {
			echo "<img src='../images/enabled.png' height='20' width='20' alt=\"The folder is accessible in writing.\" />";
		}
		else {
			echo "<img src='../images/disabled.png' height='20' width='20' alt=\"The folder is not accessible in writing.\" />";
		}
		echo "</td>\n";
		echo "</tr>\n";
...
function journal_connexions($login,$duree,$page='mon_compte',$pers_id=NULL) {
	...
	if($page=='mon_compte') {
		echo "<h2>Log of your connections since <b>".$display_duree."</b>**</h2>\n";
	}
	else {
		echo "<h2>Log of connections of ".civ_nom_prenom($login)." depuis <b>".$display_duree."</b>**</h2>\n";
	}
	$requete = '';
	...
	echo "<ul>
<li>Lines in <span style='color:red; font-weight:bold;'>red</span> signal an attempt of connection with an <span style='color:red; font-weight:bold;'>erroneous password</span>.</li>
<li>Lines in <span style='color:orange; font-weight:bold;'>orange</span> signal a closed session for which you are not <span style='color:orange; font-weight:bold;'> disconnected correctly</span>.</li>
<li>Lines in <span style='color:black; font-weight:bold;'>black</span> signal a <span style='color:black; font-weight:bold;'>normally closed session</span>.</li>
<li>Lines in <span style='color:green; font-weight:bold;'>green</span>  indicate <span style='color:green; font-weight:bold;'>sessions in progress</span> (<em>that can correspond to a currently closed connection but for which you did not disconnect yourselves correctly</em>).</li>
</ul>
<table class='col' style='width: 90%; margin-left: auto; margin-right: auto; margin-bottom: 32px;' cellpadding='5' cellspacing='0' summary='Connections'>
	<tr>
		<th class='col'>Session beginning</th>
		<th class='col'>End of session</th>
		<th class='col'>Address IP and name of the machine</th>
...
";
	...
	if ($res) {
		for ($i = 0; ($row = sql_row($res, $i)); $i++)
		{
			...
			if ($row[4] == 2) {
				echo "<td class=\"col\">".$temp1."Attempt of connection<br />with erroneous password.".$temp2."</td>\n";
			}
			else {
	...
	echo "Display the Log of connections since : <select name=\"duree\" size=\"1\">\n";
	echo "<option ";
	if ($duree == 7) echo "selected";
	echo " value=7>One week</option>\n";
	echo "<option ";
	if ($duree == 15) echo "selected";
	echo " value=15 >Fifteen days</option>\n";
	echo "<option ";
	...
	echo " value='all'>The beginning</option>\n";
	echo "</select>\n";
	echo "<input type=\"submit\" name=\"Valider\" value=\"Validate\" />\n";

	echo "</form>\n";

	echo "<p class='small'>** The information above can enable you to check that a pirate connection was not carried out on your account.
	In the case of an unexplained connection, you must immediately inform the <a href=\"mailto:" . getSettingValue("gepiAdminAdress") . "\">administrator</a>.</p>\n";
}
		...
		while($lig=mysql_fetch_object($res)) {
			echo "<div id='info_action_$lig->id' style='border: 1px solid black; margin:2px;'>\n";
				...
				echo "<div id='info_action_corps_$lig->id' style='padding:3px;' class='infobulle_corps'>\n";
					...
					echo "<a href=\"".$_SERVER['PHP_SELF']."?del_id_info=$lig->id".add_token_in_url()."\" onclick=\"return confirmlink(this, '".traitement_magic_quotes($lig->titre)."', 'Are you sure you want to remove ".traitement_magic_quotes($lig->titre)."')\">Remove</span></a>";
					echo "</div>\n";
						...
						$retour.="<div id='info_acces_cdt_$lig->id' style='border: 1px solid black; margin:2px;'>\n";
							...
							$retour.="<div id='info_acces_cdt_corps_$lig->id' style='padding:3px;' class='infobulle_corps'>\n";
								if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')) {
									...
									$retour.="<a href=\"".$_SERVER['PHP_SELF']."?del_id_acces_cdt=$lig->id".add_token_in_url()."\" onclick=\"return confirmlink(this, '".traitement_magic_quotes($lig->description)."', 'Are you sure you want to remove this access')\">Remove the access</span></a>";
									$retour.="</div>\n";
								}
	
								$retour.="<p><b>The access was open for the following reason&nbsp;:</b><br />";
								$retour.=preg_replace("/\\\\r\\\\n/","<br />",$lig->description);
								$retour.="</p>\n";
								...
								if(mysql_num_rows($res3)>0) {
									...
									while($lig3=mysql_fetch_object($res3)) {
										...
										$chaine_enseignements.=$current_group['name']." (<i>".$current_group['description']."</i>) in ".$current_group['classlist_string']." (<i>".$chaine_profs."</i>)";
										$chaine_enseignements.="</li>\n";
									}
								}
								$chaine_enseignements.="</ul>";
								
								$retour.="<p>CDT accessible to the address <a href='$lig->chemin' target='_blank'>$lig->chemin</a> are&nbsp;:<br />".$chaine_enseignements."</p>";
							$retour.="</div>\n";
						$retour.="</div>\n";
						if($cpt_id>0) {$chaine_id.=", ";}
						$chaine_id.="'$lig->id'";
...
function affiche_actions_compte($login) {
	...
	if ($user['etat'] == "actif") {
		...
		$retour.=add_token_in_url()."'>Deactivate the account</a>";
	} else {
		...
		$retour.=add_token_in_url()."'>Reactivate the account</a>";
	}
	$retour.="<br />\n";
	if ($user['observation_securite'] == 0) {
		...
		$retour.=add_token_in_url()."'>Place in observation</a>";
	} else {
		...
		$retour.=add_token_in_url()."'>Remove the observation</a>";
	}
	if($user['niveau_alerte']>0) {
		$retour.="<br />\n";
		$retour.="Cumulated score&nbsp;: ".$user['niveau_alerte'];
		$retour.="<br />\n";
		...
		$retour.=add_token_in_url()."'>Reinitialize cumul</a>";
	}
	$retour.="</p>\n";
