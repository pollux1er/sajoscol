...
if ((isset($action)) and ($action == 'message') and (isset($_POST['message'])) and isset($_POST['ok'])) {
    ...
    if ($contenu_cor == '') {
        $msg = "CAUTION: The message is empty.<br />The recording cannot take place.";
        $record = 'no';
    }
    if (preg_match("#([0-9]{2})/([0-9]{2})/([0-9]{4})#", $_POST['display_date_debut'])) {
        ...
        $msg = "CAUTION: The date of beginning of posting is not valid.<br />The recording cannot take place.";
        $record = 'no';
    ...
    if (preg_match("#([0-9]{2})/([0-9]{2})/([0-9]{4})#", $_POST['display_date_fin'])) {
        ...
        $msg = "CAUTION: The completion date of posting is not valide.<br/> the
    ...
    if ($record == 'yes') {
		if (preg_match("#([0-9]{2})/([0-9]{2})/([0-9]{4})#", $_POST['display_date_decompte'])) {
			...
			if (preg_match("/([0-9]{1,2}):([0-9]{0,2})/", str_ireplace('h',':',$_POST['display_heure_decompte']))) {
				...
				$msg = "CAUTION: The hour of calculation is not valide.<br/> the recording
cannot take place.";
				$record = 'no';
			}

		} else {
			$msg = "CAUTION: The date of calculation is not valide.<br/> the recording
cannot take place.";
			$record = 'no';
		}
    ...
    if ($record == 'yes') {
      ...
      if ($req) {
          ...
          $msg = "Problem during the recording of the message.";
      }
    }
}
...
//**************** EN-TETE *****************
$titre_page = "Management of the messages";
...
echo "<span class='grand'>All messages</span><br />\n";
echo "<span class='small'>Classify by : ";
echo "<a href='index.php?order_by=date_debut'>date beginning</a> | <a href='index.php?order_by=date_fin'>date fin</a> | <a href='index.php?order_by=id'>date creation</a>\n";
echo "</span><br /><br />\n";
...
//  echo "<b><i>Message de </i></b>: ".$prenom_auteur." ".$nom_auteur.";
   echo "<b><i>Display </i></b>: du <b>".strftime("%a %d %b %Y", $date_debut1)."</b> to <b>".strftime("%a %d %b %Y", $date_fin1)."</b>\n";
   if(strstr($content,'_DECOMPTE_')) {
      //echo "<br />Avec décompte des jours jusqu'au ".formate_date_decompte($date_decompte1);
      echo "<br />With calculation of the days until ".strftime("%d/%m/%Y à %H:%M",$date_decompte1);
   }
   echo "<br /><b><i>Statutes recipient </i></b> : ";
	/*
   ...
   - <a href='index.php?id_del=$id_message&action=sup_entry".add_token_in_url()."' onclick=\"return confirmlink(this, 'Are you sure to want to remove this message ?', '".$message_suppression."')\">Remove</a>
...
//
if (isset($id_mess)) {
    $titre_mess = "Modification of a message";
...
echo "<p><i>The message will be posted :</i><br />date : ";
...
echo "<br />(<span style='font-size:small'>Respect the format jj/mm/aaaa</span>)</p></td></tr>\n";

//Date pour décompte
echo "<tr><td colspan=\"4\">\n";
echo "<p><i>Deduct days until :</i> ";
...
echo "<br />(<span style='font-size:small'>Respect the format jj/mm/aaaa</span>)<br />To seize chains <b>_DECOMPTE_</b> in the body of the message so that this date is taken into account.\n";

$titre_infobulle="DECOMPTE\n";
$texte_infobulle="In order to post a countdown, you must write a text of the style&nbsp;:<br />It remains you _DECOMPTE_ to seize your appreciations of the 1st quarter.<br />\n";
//$texte_infobulle.="\n";
...
//Destinataires
echo "<tr><td  colspan=\"4\"><i>Statutes recipients of the message :</i></td></tr>\n";
echo "<tr>\n";
echo "<td><input type=\"checkbox\" id=\"desti_p\" name=\"desti_p\" value=\"desti_p\"";
if (strpos($statuts_destinataires, "p")) echo "checked";
echo " /><label for='desti_p' style='cursor: pointer;'>Professors</label></td>\n";
...
?>
<br>
<i>Login of the recipient of the message :&nbsp;</i>
