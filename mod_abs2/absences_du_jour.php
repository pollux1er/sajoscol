...
if (!$groupe_col->isEmpty()) {
    ...
    echo '<button style="font-size:12px" dojoType="dijit.form.Button" type="submit">isplay the student</button>';
	echo "</p>\n";
    echo "</form>";
    echo "</td>";
}
...
if (!$classe_col->isEmpty()) {
    ...
    echo "<option value='-1'>choose a class</option>\n";
    ...
    echo "</select>&nbsp;";
    echo"<input type='hidden' name='date_absence_eleve' value='$date_absence_eleve'/>";
    echo '<button style="font-size:12px" dojoType="dijit.form.Button" type="submit">display the student</button>';
	echo "</p>\n";
    echo "</form>";
    echo "</td>";
} else {
    echo '<td>No class with affected pupil was found</td>';
}
...
if (!$aid_col->isEmpty()) {
    ...
    echo "<option value='-1'>choose a aid</option>\n";
...
echo"<input type='hidden' name='date_absence_eleve' value='$date_absence_eleve'/>";
echo '<button style="font-size:12px" dojoType="dijit.form.Button" type="submit">search</button>';
	echo "</p>\n";
echo '</form>';
echo '</td>';
...
echo "<form action=\"./absences_du_jour.php\" method=\"post\" style=\"width: 100%;\">\n";
	echo "<p>\n";
    echo ("Mode: <select dojoType=\"dijit.form.Select\" maxheight=\"-1\" style=\"width :12em;font-size:12px;\" name=\"filter_regime\" onchange='submit()' class=\"small\">");
    echo "<option value='-1'>choose a mode</option>\n";
    ...
    echo "</p>\n";
    echo "<p>\n";
    echo ("Display: <select dojoType=\"dijit.form.Select\" maxheight=\"-1\" style=\"font-size:12px;\" name=\"filter_manqement_obligation\" onchange='submit()' class=\"small\">");
    echo "<option value='y'>Failures with the obligation of presence</option>";
    	    echo "<option value='n'";
    ...
    echo"<input type='hidden' name='date_absence_eleve' value='$date_absence_eleve'/>";
    echo '<button style="font-size:12px" dojoType="dijit.form.Button" type="submit">filter</button>';
echo '</form>';
echo '</td>';
...
<div class="legende">
    ...
    <table class="legende">
        <tr>
            ...
            <td>
              Seizure and treatments  
            </td>
            <td>
              Notifications : Mail,phone,mail ou SMS    
            </td>
        </tr>
        <tr>
            <td>
                <font color="orange">&#9632;</font> Delay<br />
                <font color="red">&#9632;</font> Failure with the obligations of presence<br />
                <font color="blue">&#9632;</font> Not failure with the obligations of presence<br /> 
                <font color="green">&#9632;</font> Another seizure.<br />  
                <?php if (getSettingValue("abs2_alleger_abs_du_jour")!='y'): ?>
                <font color="purple">&#9632;</font> Saisie conflictuelle<br />
                <?php endif; ?>
            </td>
            <td>
                <img src="../images/icons/saisie.png" />  modify the seizure<br/>  
               <!-- <img src="../images/icons/flag_green.png" /> Saisie traitée<br/> -->
                <img src="../images/icons/ico_attention.png" />Untreated seizure.<br/>                
            </td>
            <td>
            <img src="../images/icons/courrier_envoi.png" /> Seizure in the course of notification (state initial or in progress).<br/>
            <img src="../images/icons/courrier_retour.png" /> Notified seizure (received or received with acknowledgement of delivery).<br/>
            </td>    
        </tr>
    </table>     
</div> <br />
...
?>
	<div style="text-align: center">
				...
				<input type="hidden" name="id_aid" value="<?php echo $id_aid?>"/>
					...
					return true;">Reinitialize filters</button>
			    </p>
			<?php
...
<?php if ($eleve_col->count() != 0) { ?>
			...
			<div dojoType="dijit.form.DropDownButton" style="display: inline">
			    <span>To add to the treatment</span>
			...
			if ($id_traitement != null && AbsenceEleveTraitementQuery::create()->findPk($id_traitement) != null) {
			    ...
			    echo '	    To add the seizures to the treatment n° '.$id_traitement.' ('.$traitement->getDescription().')'."\n";
			    echo '	</button>'."\n";
			}
			?>
			    </div>
			...
			<div dojoType="dijit.form.DropDownButton" style="display: inline">
			    <span>To add to the treatment (popup)</span>
			    <div dojoType="dijit.Menu" style="display: inline">				
			<?php
			$id_traitement = isset($_POST["id_traitement"]) ? $_POST["id_traitement"] :(isset($_GET["id_traitement"]) ? $_GET["id_traitement"] :(isset($_SESSION["id_traitement"]) ? $_SESSION["id_traitement"] : NULL));
			if ($id_traitement != null && AbsenceEleveTraitementQuery::create()->findPk($id_traitement) != null) {
			    ...
			    echo '	    To add the seizures to the treatment n° '.$id_traitement.' ('.$traitement->getDescription().') in a popup'."\n";
			    echo '	</button>'."\n";
			}
			?>
			    </div>
	    ...
	    <table class="tb_absences" summary="Liste des élèves pour l'appel. Colonne 1 : élèves, colonne 2 : absence, colonne3 : retard, colonnes suivantes : suivi de la journée par créneaux, dernière colonne : photos si actif">
		    ...
		    <tbody>
			    <tr class="titre_tableau_gestion" style="white-space: nowrap;">
				    <th style="text-align : center;" >Take care</th>
				    <th style="text-align : center;" abbr="élèves">List of student</th>
				    <th colspan="<?php echo (EdtCreneauPeer::retrieveAllEdtCreneauxOrderByTime()->count());?>" class="th_abs_suivi" abbr="Créneaux">Follow-up over the day</th>
			    </tr>
			    <tr>
				    <td></td>
				    <td></td>
...
<?php
			...
			for($i = 0; $i<$col_creneaux->count(); $i++){
					...
					foreach ($absences_du_creneau as $saisie) {
					    ...
					    echo '/>';                        
                        echo '<a style="font-size:88%;" href="#" onClick="javascript:showwindow(\'visu_saisie.php?id_saisie='.$saisie->getPrimaryKey().'&menu=false\',\'To modify,to treat or notify a seizure\');return false"><img src="../images/icons/saisie.png" title="See the seizure n°'.$saisie->getPrimaryKey().'"/>';
                        ...
                        if(!$saisie->getTraitee()){
                            echo '<img src="../images/icons/ico_attention.png" title="Untreated seizure" />';
                        }else{
			...
			foreach ($traitement_col as $traitement) {
			    ...
			    echo ' To add to the n° treatment '.$traitement->getId().' ('.$traitement->getDescription().')';
			    echo '</button>';
			}
			...
			echo '</div></div><br/>';
			echo '<div dojoType="dijit.form.DropDownButton"  style="white-space: nowrap; display: inline">
			    <span>To add (window)</span>
			    <div dojoType="dijit.Menu"  style="white-space: nowrap; display: inline">';				
			foreach ($traitement_col as $traitement) {
			    echo '<button dojoType="dijit.MenuItem" onClick="document.getElementById(\'id_traitement\').value = \''.$traitement->getId().'\'; document.getElementById(\'creation_traitement\').value = \'no\'; document.getElementById(\'ajout_traitement\').value = \'yes\'; postwindow(document.creer_traitement,\'To treat and notify seizures\');">';
			    echo ' To add to the n° treatment '.$traitement->getId().' ('.$traitement->getDescription().')';
			    echo '</button>';
			}
            echo'<button dojoType="dijit.MenuItem" onClick="document.getElementById(\'creation_traitement\').value = \'yes\'; document.getElementById(\'ajout_traitement\').value = \'no\'; postwindow(document.creer_traitement,\'To treat and notify seizures\');">
				    To create a new treatment (window)
				 </button>';
			echo '</div></div><br/>';
            echo '<div dojoType="dijit.form.DropDownButton"  style="white-space: nowrap; display: inline">
			    <span>To add (popup)</span>
			    <div dojoType="dijit.Menu"  style="white-space: nowrap; display: inline">';				
			foreach ($traitement_col as $traitement) {
            ...
            echo'<button dojoType="dijit.MenuItem" onClick="document.getElementById(\'creation_traitement\').value = \'yes\'; document.getElementById(\'ajout_traitement\').value = \'no\'; pop_it(document.creer_traitement);">
				    To create a new treatment (popup)
				 </button>';
			echo '</div></div>';
			echo '</td>';
			echo "</tr>";
    ...
    ?>
    <div dojoType="dijit.form.DropDownButton" style="display: inline">
	<span>To add the seizures notched to a treatment</span>
	<div dojoType="dijit.Menu" style="display: inline">
	    <button dojoType="dijit.MenuItem" onClick="document.getElementById('creation_traitement').value = 'yes'; document.getElementById('ajout_traitement').value = 'no'; document.creer_traitement.submit();">
		To create a new treatment
	    </button>
		...
		To create a new treatment in a popup
	    </button>
    <?php
    $id_traitement = isset($_POST["id_traitement"]) ? $_POST["id_traitement"] :(isset($_GET["id_traitement"]) ? $_GET["id_traitement"] :(isset($_SESSION["id_traitement"]) ? $_SESSION["id_traitement"] : NULL));
    if ($id_traitement != null && AbsenceEleveTraitementQuery::create()->findPk($id_traitement) != null) {
	$traitement = AbsenceEleveTraitementQuery::create()->findPk($id_traitement);
	echo '	<button dojoType="dijit.MenuItem" onClick="document.getElementById(\'creation_traitement\').value = \'no\'; document.getElementById(\'ajout_traitement\').value = \'yes\'; document.getElementById(\'id_traitement\').value = \''.$id_traitement.'\'; document.creer_traitement.submit();">'."\n";
	echo '	    To add the seizures to the n° treatment '.$id_traitement.' ('.$traitement->getDescription().')'."\n";
	echo '	</button>'."\n";
	echo '	<button dojoType="dijit.MenuItem" onClick="document.getElementById(\'creation_traitement\').value = \'no\'; document.getElementById(\'ajout_traitement\').value = \'yes\'; document.getElementById(\'id_traitement\').value = \''.$id_traitement.'\'; pop_it(document.creer_traitement);">'."\n";
	echo '	    To add the seizures to the n° treatment '.$id_traitement.' ('.$traitement->getDescription().') in a popup'."\n";
	echo '	</button>'."\n";
    }
    ?>
	</div>
...
$javascript_footer_texte_specifique = '<script type="text/javascript">
	...
	dojo.query("[add_select_shorcuts_button=true]").forEach(function(node, index, arr){
		...
		label: "without notification created",
		onClick: function() {
		    var eleve_id = dojo.attr(node,\'eleve_id\');
		    var query_string = \'input[type=checkbox][eleve_id=\'+eleve_id+\']\';
		    dojo.query(query_string).attr(\'checked\', true);
