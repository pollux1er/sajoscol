...
if ($action == 'supprimer') {
    ...
    if ($nom != '') {
		...
		$type->setRetardReport card($retard_bulletin);
...
//==========================================
// header
$titre_page = "Management of the types of absence";
...
<div style="text-align:center">
    <h2>Definition of the types of absence</h2>
    <p>
        <span style="font-weight: bold;">Caution :</span> To associate a type a seizure implies the creation of a treatment of this seizure.<br />
        To avoid a multiplicity of the treatments for a seizure, it can be interesting to limit the statutes being able to seize a type.<br />
        For example if it is wished that only the school life create the absence treatment or delay one will not assign these types to the statute professor.        
    </p>
<?php if ($action == "ajouter" OR $action == "modifier" OR $action == "supprimer_statut") { ?>
<div style="text-align:center">
    <?php
    	if($action=="ajouter") { 
	    echo "<h2>Addition of a type</h2>";
	} elseif ($action=="modifier") {
	    echo "<h2>Modify a type</h2>";
	}
	?>
...
?>
     <fieldset class="fieldset_efface">
      <table cellpadding="2" cellspacing="2" class="menu">
        <tr>
          <td>Name (obligatory)</td>
          <td>Comment (optional)</td>
	    <td>Exigible justification</td>
	    <td>The student is under the responsibility of the establishment</td>
	    <td>Failure obligations (appears on the bulletin)</td>
	    <td>Entered as delay on the bulletin (appears on the bulletin)</td>
	    <td>Type of seizure</td>
        <td>Place</td>
	    <td>Statute autorisé with the seizure</td>
       </tr>
        <tr>
          <td>
           <?php
	   ...
	   <?php } ?>
		...
		<option value='<?php echo AbsenceEleveType::SOUS_RESP_ETAB_VRAI?>' <?php  if ($type != null && $type->getSousResponsabiliteEtablissement() == AbsenceEleveType::SOUS_RESP_ETAB_VRAI) {echo "selected";} ?>>Yes</option>
		<option value='<?php echo AbsenceEleveType::SOUS_RESP_ETAB_FAUX?>' <?php  if ($type != null && $type->getSousResponsabiliteEtablissement() == AbsenceEleveType::SOUS_RESP_ETAB_FAUX) {echo "selected";} ?>>Not</option>
		<option value='<?php echo AbsenceEleveType::SOUS_RESP_ETAB_NON_PRECISE?>' <?php  if ($type != null && $type->getSousResponsabiliteEtablissement() == AbsenceEleveType::SOUS_RESP_ETAB_NON_PRECISE) {echo "selected";} ?>>not precized</option>
	     </select>
	   </td>
           <td>
	     <select name="manquement_obligation_presence" id="manquement_obligation_presence">
		<option value='<?php echo AbsenceEleveType::MANQU_OBLIG_PRESE_VRAI?>' <?php  if ($type != null && $type->getManquementObligationPresence() == AbsenceEleveType::MANQU_OBLIG_PRESE_VRAI) {echo "selected";} ?>>Yes</option>
		<option value='<?php echo AbsenceEleveType::MANQU_OBLIG_PRESE_FAUX?>' <?php  if ($type != null && $type->getManquementObligationPresence() == AbsenceEleveType::MANQU_OBLIG_PRESE_FAUX) {echo "selected";} ?>>Not</option>
		<option value='<?php echo AbsenceEleveType::MANQU_OBLIG_PRESE_NON_PRECISE?>' <?php  if ($type != null && $type->getManquementObligationPresence() == AbsenceEleveType::MANQU_OBLIG_PRESE_NON_PRECISE) {echo "selected";} ?>>not precized</option>
	     </select>
	   </td>
           <td>
	     <select name="retard_bulletin" id="retard_bulletin">
		<option value='<?php echo AbsenceEleveType::RETARD_BULLETIN_FAUX?>' <?php  if ($type != null && $type->getRetardReport card() == AbsenceEleveType::RETARD_BULLETIN_FAUX) {echo "selected";} ?>>Not</option>
		<option value='<?php echo AbsenceEleveType::RETARD_BULLETIN_VRAI?>' <?php  if ($type != null && $type->getRetardReport card() == AbsenceEleveType::RETARD_BULLETIN_VRAI) {echo "selected";} ?>>Yes</option>
	     </select>
	   </td>
           <td>
		...
		<!--<option value='DEBUT_ABS' <?php  if ($type != null && $type->getTypeSaisie() == 'DEBUT_ABS') {echo "selected";} ?>>Seize the moment of beginning of the absence </option>
		<option value='FIN_ABS' <?php  if ($type != null && $type->getTypeSaisie() == 'FIN_ABS') {echo "selected";} ?>>Seize the moment of end of the absence</option>
		<option value='DEBUT_ET_FIN_ABS' <?php  if ($type != null && $type->getTypeSaisie() == 'DEBUT_ET_FIN_ABS') {echo "selected";} ?>>Seize the moment of beginning and end</option>
		<option value='COMMENTAIRE_EXIGE' <?php  if ($type != null && $type->getTypeSaisie() == 'COMMENTAIRE_EXIGE') {echo "selected";} ?>>Seize a comment</option>
		--><option value='DISCIPLINE' <?php  if ($type != null && $type->getTypeSaisie() == 'DISCIPLINE') {echo "selected";} ?>>To seize a disciplinary incident</option>
	     </select>
	   </td>
        <td>
		...
		<option value=''>Addition of a statute</option>
...
<?php
	...
	<a href="admin_types_absences.php?action=ajouter"><img src='../../images/icons/add.png' alt='' class='back_link' /> Add a new type</a>
	<br/><br/>
	<a href="admin_types_absences.php?action=ajouterdefaut<?php echo add_token_in_url();?>"><img src='../../images/icons/add.png' alt='' class='back_link' /> Add the types by defect</a>
	<br/><br/>
	...
	<td>Delay</td>
        <td>Type of seizure</td>
        <td>Place</td>
	<td>Statute autorisé(s) with the seizure</td>
    ...
    foreach ($type_collection as $type) { ?>
	  ...
	  <td>
	    <?php if ($type->getManquementObligationPresence() == AbsenceEleveType::MANQU_OBLIG_PRESE_VRAI) { echo "<img src='../../images/enabled.png' width='20' height='20' title='yes' alt='oui' />"; }
		else if ($type->getManquementObligationPresence() == AbsenceEleveType::MANQU_OBLIG_PRESE_FAUX) { echo "<img src='../../images/disabled.png' width='20' height='20' title='not' alt='non' />"; }
		//si le ManquementObligationPresence est non precisé on affiche rien
	    ?>
	  </td>
	  <td>
	    <?php if ($type->getRetardReport card() == AbsenceEleveType::RETARD_BULLETIN_VRAI) { echo "<img src='../../images/enabled.png' width='20' height='20' title='oui' alt='oui' />"; }
		//else if ($type->getRetardReport card() == AbsenceEleveType::MANQU_OBLIG_PRESE_FAUX) { echo "<img src='../../images/disabled.png' width='20' height='20' title='oui' alt='non' />"; }
		else { echo "<img src='../../images/disabled.png' width='20' height='20' title='not' alt='non' />"; }
	    ?>
	  </td>
	  ...
	  ?></td>
          ...
          <td><a href="admin_types_absences.php?action=supprimer&amp;id=<?php echo $type->getId(); echo add_token_in_url();?>" onClick="return confirm('Are you sure to want to remove this type ?')"><img src="../../images/icons/delete.png" width="22" height="22" title="Remove" border="0" alt="" /></a></td>
