...
/**
 *
 * @version $Id: bilan_individuel.php 8189 2011-09-10 14:57:47Z dblanqui $
 *
...
if(($id_classe=='-1' && $affichage=='html' && $click_filtrage!="ok" && $raz!=="ok") && (is_null($id_eleve) || $id_eleve=='') && (is_null($nom_eleve) || strlen($nom_eleve)<2)){
    ...
    if($limite_temps && ($dt_date_absence_eleve_fin->format('U')-$dt_date_absence_eleve_debut->format('U'))>($limite_jours*24*3600) ){
        $message=' The interval of time chosen for all the classes must be lower than 7
days ';
...
?>
    <div id="contain_div" class="css-panes">
        ...
        <?php if($limite_temps && $utilisateur->getStatut() != "autre" && $utilisateur->getStatut() != "professeur" ) :?>
         <p>
             <strong>Research on all the classes is allowed only for one duration of <?php echo $limite_jours; ?> maximum days if no name returned.</strong>

        <p>
        <?php endif ;?>
            This page makes it possible to gather day per day the saises of the
same type (untreated or having the same treatment) and information of
the treatment.<br />
            For seizures having multiple treatments, the calculation of the
corresponding half-days thus apparaitre can several times. 
            LE total real of the half-days calculated by the module is posted under
the name of the student.
        </p>
        <p>
            Any modification must be validated to be taken into account.
        </p>
        <?php if ($inverse_date) :?>
        <h3 class="no">The dates of beginning and end were reversed.</h3>
        <?php endif; ?>
        <form dojoType="dijit.form.Form" id="bilan_individuel" name="bilan_individuel" action="bilan_individuel.php" method="post">
            <fieldset>
              <legend>Parameter setting of export (dates, classes, sorting?) and posting</legend>
            <h3>Individual assessment of	
    ...
    <input style="width : 8em;font-size:14px;" type="text" dojoType="dijit.form.DateTextBox" id="date_absence_eleve_fin" name="date_absence_eleve_fin" value="<?php echo $dt_date_absence_eleve_fin->format('Y-m-d')?>" />
          ...
          <?php
            ...
            if (!$classe_col->isEmpty()) {
                ...
                echo 'No class with affected student was found';
            }
            ?>
            Type :
            ...
            ?>
			>Data causing a failure with the obligations of presence</option>
                <option value='2' <?php
                        if ($type_extrait == '2') {
                            echo 'selected="selected"';
                        }
            ?>>List of all the data</option>
            </select><br />            
            ...
            ?>
			> Sorting of the data by failure to the obligations of presence, delay
then not failure.
            <br />
			...
			> To display only the seizures untreated or without type (nondefinite and
non covered by another treatment)
            <br />
            <?php endif; ?>
			...
			> Not to repeat total information of the pupil by line in export
spreadsheet (for totals by column)
            <br />
            <input dojoType="dijit.form.CheckBox" type="checkbox" name="sans_commentaire" value="no"  <?php
            if($sans_commentaire) {
                echo'checked';
            } ?>
			> Not to display the comments in export ods and odt
            <br />
            <input dojoType="dijit.form.CheckBox" type="checkbox" name="texte_conditionnel" value="ok"  <?php
            if($texte_conditionnel) {
                echo'checked';
            } ?>
			> To display the optional text in bottom of export odt
            <br />
            <?php endif; ?>            
            <button type="submit"  style="font-size:12px" dojoType="dijit.form.Button" name="affichage" value="html"> validate the modifications and to display with the screen</button>
        </fieldset>
		<br />
        <?php if($affichage_liens):?>
        <fieldset style="width:320px; float:left;">
            <legend>Choice of the mode of exit of the data</legend>            
            <button type="submit"  style="font-size:12px" dojoType="dijit.form.Button" name="affichage" value="ods" <?php
                 if($affichage==Null || $affichage=='') echo'disabled';?>> export in a spreadsheet (ods)</button>
            <button type="submit"  style="font-size:12px" dojoType="dijit.form.Button" name="affichage" value="odt" <?php
                 if($affichage==Null || $affichage=='') echo'disabled';?>> export in a text processing (odt)</button>
        </fieldset>
         <?php endif; ?>
    </form>
    <?php
    ...
    <div id="param-filtre">
     <form id="filtrage" method="POST" action="bilan_individuel.php" >
        <fieldset style="width:540px;">
            <legend>Filtering of the data</legend>
            <p style="color:<?php echo $color;?>">To display only the student of which the numbers of absences or delays observe the conditions below:<br />
            Choice of the condition if several conditions are seized for
filtering: 
            <select dojoType="dijit.form.Select" style="width :3em;font-size:12px;" name="type_filtrage"  <?php if($affichage==Null || $affichage=='') echo'disabled';?>>
                <option value="OU" <?php if($type_filtrage=="OU") echo 'selected="selected"';?>>OR</option>
                <option value="ET" <?php if($type_filtrage=="ET") echo 'selected="selected"';?>>AND</option>
            </select>
            <br />    
            Numbers total 1/2 days &ge;: <INPUT dojoType="dijit.form.NumberTextBox" style="width:3em;" constraints="{min:1}" type="text" <?php if($ndj!=Null)echo'value='.$ndj; else echo'value=""'; ?> name="ndj" size="3" maxlength="3"  <?php
             if($affichage==Null || $affichage=='') echo'disabled';?>/><br />
            (OR/AND) Numbers 1/2 days not justified &ge;: <INPUT dojoType="dijit.form.NumberTextBox" style="width:3em;" constraints="{min:1}" type="text" <?php if($ndjnj!=Null)echo'value='.$ndjnj; else echo'value=""'; ?> name="ndjnj" size="3" maxlength="3"  <?php
             if($affichage==Null || $affichage=='') echo'disabled';?>><br />
            (OU/ET) A number of delays &ge;: <INPUT dojoType="dijit.form.NumberTextBox" constraints="{min:1}" style="width:3em;" type="text" <?php if($nr!=Null)echo'value='.$nr; else echo'value=""'; ?> name="nr" size="3" maxlength="3"  <?php
             if($affichage==Null || $affichage=='') echo'disabled';?>><br />
            ...
            <button type="submit"  style="font-size:12px" dojoType="dijit.form.Button" name="raz" value="ok" <?php
             if($affichage==Null || $affichage=='') echo'disabled';?>>Reinitialize</button></p>
        </fieldset> 
    </form>
    </div>  
    <?php
...
if ($eleve_col->isEmpty()) {
    echo"<h2 class='no'>No student with the selected parameters was found.</h2>";
    die();
}
...
echo 'Information on the student';
echo '</td>';
echo '<td align="center">';
echo 'Seizures ';
echo '</td>';
            ...
            foreach ($journee as $key => $value) {                
                ...
                if ($precedent_eleve_id != $id) {                    
                    ...
                    if (strval($eleve['demi_journees']) == 0) {
                        ...
                        echo '<b>' . $eleve['demi_journees'] . '</b> half-day';
                        if (strval($eleve['demi_journees']) > 1)
                            echo's';

                        if (strval($eleve['demi_journees'] - $eleve['non_justifiees']) != 0) {
                            echo' <br /> ';
                            echo 'of which ' . strval($eleve['demi_journees'] - $eleve['non_justifiees']) . ' justified';
                        ...
                        if (strval($eleve['non_justifiees']) != 0) {
                            echo'<br />';
                            echo 'of which <b>' . $eleve['non_justifiees'] . ' not justified</b>';
                            if (strval($eleve['non_justifiees']) > 1)
                                echo's';
                        }
                    }
                    echo'<br /><br />';
                    echo '<u><i>Delays :</i></u><br />';
                    if (strval($eleve['retards']) == 0) {
                        echo 'No delay';
                    } else {
                        echo $eleve['retards'] . ' delay';
...
if ($id_classe != null && $id_classe != '' && $id_eleve == null) {
    ...
    if ($classe != null) {
        $titre .= ' for the class ' . $classe->getNom();
    }
}
if ($nom_eleve != null && $nom_eleve != '') {
    $titre .= ' for the student whose name or first name contains ' . $nom_eleve;
}
if ($id_eleve != null && $id_eleve != '') {
    $eleve_current=  EleveQuery::create()->filterByIdEleve($id_eleve)->findOne();
    $titre .= 'for ' . $eleve_current->getPrenom() . ' ' . $eleve_current->getNom();
}
