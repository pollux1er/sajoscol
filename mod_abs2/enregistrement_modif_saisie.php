...
if ($saisie == null) {
    $message_enregistrement .= 'Impossible modification : not found seizure.';
...
if ( isset($_POST["creation_traitement"])) {
	...
	$message_enregistrement = 'Modification of the type recorded';
    }
    include("visu_saisie.php");
    die();
} elseif (isset($_GET["version"])) {
	if ($utilisateur->getStatut() != 'cpe' && $utilisateur->getStatut() != 'scolarite') {
	    $message_enregistrement .= 'Unauthorized modification.';
	    include("visu_saisie.php");
	    die();
	}
        if ($saisie->getDeletedAt() != null) {
        $message_enregistrement .= 'This seizure is removed. You must restore it to modify it.';
        include("visu_saisie.php");
        die();
        }
...
if ($utilisateur->getStatut() == 'professeur') {
	...
	if ($date_debut->format('d/m/Y') != $jours_actuel) {
	    $message_enregistrement .= "Seizure of a date differente of the unauthorized current date.<br/>";
	    include("visu_saisie.php");
	    die();
	}
    }
    if (getSettingValue("abs2_saisie_prof_decale_journee") !='y' && getSettingValue("abs2_saisie_prof_decale") != 'y') {
       if ($saisie->getEdtCreneau() == null || $saisie->getEdtCreneau()->getHeuredebutDefiniePeriode('Hi') > $date_debut->format('Hi')) {
	    $message_enregistrement .= "Unauthorized seizure except creneau current.<br/>";
	    include("visu_saisie.php");
	    die();
       }
    }
...
if ($saisie->validate()) {
    $saisie->save();
    $message_enregistrement .= 'Recorded modification';
...
function modif_type ($saisie, $utilisateur) {
	...
	if ($traitement->count() != 1) {
	    $message_enregistrement .= "Problem with the id treatment : ".$_POST['id_traitement'][$i]."<br/>";
	    continue;
	}
	if (!$traitement->getFirst()->getModifiable()) {
	    $message_enregistrement .= "Treatment ".$_POST['id_traitement'][$i]." not modifiable<br/>";
	    continue;
	}
	...
	if ($type == null) {
	    $message_enregistrement .= "Impossible to remove a type.<br/>";
	    continue;
	}
	if (!$type->isStatutAutorise($utilisateur->getStatut())) {
	    $message_enregistrement .= "Unauthorized type of absence for this statute : ".$_POST['type_absence_eleve'][$i]."<br/>";
	    continue;
	}
	...
	if ($type != null) {
	    ...
	    $message_enregistrement .= "Problem with the id of the type of absence: ".$_POST['type_absence_eleve'][$i]."<br/>";
	}
    } else if ($ajout_type_absence == -1) {
	$message_enregistrement .= "A type should be specified<br/>";
    }
