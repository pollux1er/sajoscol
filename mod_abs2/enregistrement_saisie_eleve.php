...
if ($id_creneau != null && $id_creneau != -1) {
    ...
    if ($creneau == null) {
	$message_enregistrement .= "Problem with the parameter id_creneau<br/>";
	$id_creneau = null;
    }
} else {
    $creneau = null;
...
if ($id_cours != null && $id_cours != -1) {
	...
	$message_enregistrement .= "Problem with the parameter id_cours<br/>";
	$id_cours = null;
    }
} else {
   $current_cours = null;
...
if ($type_absence != null && $type_absence != -1) {
	...
	if (!$type->isStatutAutorise($utilisateur->getStatut())) {
	    $message_enregistrement .= "Unauthorized type of absence for this statute : ".$_POST['type_absence']."<br/>";
	    $type = null;
	}
    } else {
	$message_enregistrement .= "Problem with the id of the type of absence : ".$_POST['type_absence']."<br/>";
    }
}
...
if ($current_cours != null) {
	...
	if (isset ($_POST['semaine_'.$semaine->getPrimaryKey()])) {
		    ...
		    $message_enregistrement .= "Problem with the week : " .$semaine->getNumEdtSemaine()." ".$semaine->getTypeEdtSemaine();
	...
	$message_enregistrement .= "Bad format of date.<br/>";
    }
    try {
	...
	$message_enregistrement .= "The date of beginning of absence cannot be posterior at the completion date.<br/>";
    }
...
for($i=0; $i<$total_eleves; $i++) {
    ...
    if ($eleve == null) {
	$message_enregistrement .= "Problem with the id eleve : ".$_POST['id_eleve_absent'][$i]."<br/>";
	continue;
    }
	...
	if ($saisie->validate()) {
	    ...
	    $message_enregistrement .= "<a href='visu_saisie.php?id_saisie=".$saisie->getPrimaryKey()."'>Seizure recorded for the student : ".$eleve->getNom()."</a>";
		...
		$message_enregistrement .= " &nbsp;<a href='../mod_discipline/saisie_incident_abs2.php?id_absence_eleve_saisie=".
		    $saisie->getId()."&return_url=no_return".add_token_in_url()."'>To seize a disciplinary incident for the student: ".$eleve->getNom()."</a>";
	    }
