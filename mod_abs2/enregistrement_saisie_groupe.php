...
if ($type_selection == 'id_cours') {
	...
	$message_enregistrement .= "<span style='color :red'>Error: a course was specified but the module edt is decontaminated</span><br/>";
    }else {
	...
	if ($current_cours != null) {
	    ...
	    $message_enregistrement .= "<span style='color :red'>Erreur : Problem with the parameter id_cours</span><br/>";
	}
    }
} else {
    $current_creneau = EdtCreneauQuery::create()->findPk($id_creneau);
    if ($current_creneau == null) {
	$message_enregistrement .= "<span style='color :red'>Erreur : Problem with the parameter id_creneau</span><br/>";
    }
}
...
if ($type_selection == 'id_groupe') {
	...
	$message_enregistrement .= "<span style='color :red'>Erreur : Problem with the parameter id_groupe</span><br/>";
    }
} elseif ($type_selection == 'id_classe') {
    $current_classe = ClasseQuery::create()->findPk($id_classe);
    if ($current_classe == null) {
	$message_enregistrement .= "<span style='color :red'>Erreur : Problem with the parameter id_classe</span><br/>";
    }
} elseif ($type_selection == 'id_aid') {
    $current_aid = AidDetailsQuery::create()->findPk($id_aid);
    if ($current_aid == null) {
	$message_enregistrement .= "<span style='color :red'>Erreur : Problem with the parameter id_aid</span><br/>";
    }
}
...
if ($current_cours == null && $utilisateur->getStatut() == 'professeur' && getSettingValue("abs2_saisie_prof_hors_cours")!='y') {
	...
	$message_enregistrement .= "<span style='color :red'>Error: It is obligatorily necessary to seize a course of the timetable</span><br/>";
    }
}
...
if ($date_absence_eleve != null) {
	...
	$message_enregistrement .= "<span style='color :red'>Error: Bad format of date of absence.</span><br/>";
    }
} else {
    $message_enregistrement .= "<span style='color :red'>Error: The date of absence must be specified.</span><br/>";
}
...
if ($heure_debut_appel != null) {
	...
	$message_enregistrement .= "<span style='color :red'>Error: Bad format of hour of beginning of seizure.</span><br/>";
    }
} else {
    $message_enregistrement .= "<span style='color :red'>Error: hour of beginning of not precized seizure.</span><br/>";
}
if ($heure_fin_appel != null) {
	...
	$message_enregistrement .= "<span style='color :red'>Error : Bad format of hour of end of seizure.</span><br/>";
    }
} else {
    $message_enregistrement .= "<span style='color :red'>Error: hour of end of not precized seizure.</span><br/>";
}

if ($id_groupe == null && $id_classe == null && $id_aid == null) {
  	$message_enregistrement .= '<span style="color :red">Error: One needs at least a class, a aid, a group to make a call.</span><br/>';
}
if ($id_creneau == null && $id_cours == null) {
  	$message_enregistrement .= '<span style="color :red">Error: One needs at least a creneau or a course to make a call.</span><br/>';
}
...
if ($utilisateur->getStatut() == 'professeur' && getSettingValue("abs2_saisie_prof_decale_journee")!='y' && getSettingValue("abs2_saisie_prof_decale")!='y') {
	...
	$message_erreur .= "Call unauthorized and outside of the hours of course which one makes
the call.<br/>";
    }
}
...
if ($message_erreur != '') {
    $message_enregistrement .= '<span style="color :red">Error on the recording of the marker of call : '.$message_erreur.'</span>';
    //on arrete la saisie
    include("saisir_groupe.php");
    die();
} else if ($saisie->validate()) {
    $saisie->save();
    $message_enregistrement .= "<a href='visu_saisie.php?id_saisie=".$saisie->getPrimaryKey()."'>Marker of recorded call</a><br/>";
} else {
    //on arrete la saisie
    $message_enregistrement .= '<span style="color :red">Error on the recording of the marker of call : '.format_verif_failures($saisie).'</span>';
    include("saisir_groupe.php");
    die();
}
...
for($i=0; $i<$total_eleves; $i++) {
    ...
    if ($eleve == null) {
	$message_enregistrement .= "Problem with id student : ".$_POST['id_eleve_absent'][$i]."<br/>";
	continue;
    }
	...
	$message_erreur_eleve[$id_eleve] .= "Bad format of date.<br/>";
	continue;
    }
    try {
	$heure_debut = new DateTime($_POST['heure_debut_absence_eleve'][$i]);
    } catch (Exception $x) {
	$message_erreur_eleve[$id_eleve] .= "Bad format of hour.<br/>";
	continue;
    }
	...
	if ($type != null) {
		...
		$message_erreur_eleve[$id_eleve] .= "Unauthorized type of absence for this statute: ".$_POST['type_absence_eleve'][$i]."<br/>";
	    }
	} else {
	    $message_erreur_eleve[$id_eleve] .= "Problem with the id of the type of absence: ".$_POST['type_absence_eleve'][$i]."<br/>";
	}
    }
	...
	if ($saisie->validate()) {
	    ...
	    $message_enregistrement .= "<a href='visu_saisie.php?id_saisie=".$saisie->getPrimaryKey()."'>Seizure recorded for the student: ".$eleve->getNom().' '.$eleve->getPrenom()."</a>";
		...
		$message_enregistrement .= " &nbsp;<a href='../mod_discipline/saisie_incident_abs2.php?id_absence_eleve_saisie=".
		    $saisie->getId()."&return_url=no_return".add_token_in_url()."'>To seize a disciplinary incident for the student: ".$eleve->getNom().' '.$eleve->getPrenom()."</a>";
	    }
    ...
    if ($message_erreur_eleve[$id_eleve] != '') {
	$message_enregistrement .= '<span style="color :red">Error for the recording of the student '.$eleve->getNom().' '.$eleve->getPrenom().' : </span>'.$message_erreur_eleve[$id_eleve];
    }
}
...
function verif_debut_fin_saisie(DateTime $dt_date_debut_appel, DateTime $dt_date_fin_appel, UtilisateurProfessionnel $utilisateur, $edt_cours) {
	...
	$message_enregistrement .= "The beginning of seizure must be specified.<br/>";
	return $message_enregistrement;
    }
    if ($dt_date_fin_appel === null) {
	$message_enregistrement .= "The end of seizure must be specified.<br/>";
	return $message_enregistrement;
    }
    ////on verifie si l'utilisateur a le droit de saisir cela
    if ($utilisateur->getStatut() == 'professeur') {
	if ($dt_date_debut_appel->format('d/m/Y') != $dt_date_fin_appel->format('d/m/Y')) {
	    $message_enregistrement .= "Seizure over several days unauthorized.<br/>";
	}
	...
	if (getSettingValue("abs2_saisie_prof_decale")!='y') {
		...
		$message_enregistrement .= "Unauthorized seizure other that for the current day.<br/>";
	    }
	}
	if ($edt_cours != null && $utilisateur->getStatut() == 'professeur' && getSettingValue("abs2_saisie_prof_hors_cours")!='y') {
		...
		$message_enregistrement .= "The hour of beginning of seizure cannot be former to the course.<br/>";
	    }
	    if ($edt_cours->getHeureFin('Hi') < $dt_date_fin_appel->format('Hi')) {
		$message_enregistrement .= "The hour of end of seizure cannot be posterior with the course.<br/>";
	    }
	}
    }
    if ($dt_date_debut_appel->format('U') > $dt_date_fin_appel->format('U')) {
	$message_enregistrement .= "The hour of end of seizure cannot be former per hour of beginning.<br/>";
    }
    return $message_enregistrement;
}
