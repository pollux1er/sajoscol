...
if($dt_date_absence_eleve_debut->format("U")>$dt_date_absence_eleve_fin->format("U")){
    ...
    $message="The dates of beginning and end were reversed.";
    $inverse_date=true;
...
if ($affichage != 'ods') {// on affiche pas de html
    ...
    <p>
      <strong>Precision:</strong> A failure with the obligation of presence over one hour, entraine the calculation of the corresponding half-day for the student.
    </p>
    ...
    Name (optional) : <input dojoType="dijit.form.TextBox" type="text" style="width : 10em" name="nom_eleve" size="10" value="<?php echo $nom_eleve?>"/>
	...
	<p>
    <button type="submit"  style="font-size:12px" dojoType="dijit.form.Button" name="affichage" value="html">Display</button>
...
if ($affichage == 'html') {
    echo 'Total student : '.$eleve_col->count();
    ...
    echo '<td style="border:1px solid;">';
    echo 'a number of half-days of absence';
    echo '</td>';
    ...
    $titre = 'Extract of the half-days of absences of '.$dt_date_absence_eleve_debut->format('d/m/Y').' to '.$dt_date_absence_eleve_fin->format('d/m/Y');
    $classe = null;
