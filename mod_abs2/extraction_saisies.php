...
/**
 *
 * @version $Id: extraction_saisies.php 8192 2011-09-10 15:13:31Z dblanqui $
 *
...
if ($affichage != 'ods') {// on affiche pas de html
    ...
    if ($traitement_csv_en_cours != 'true') {
	    ...
	    <h2>To extract the seizures from 		
	    ...
	    <option value='1' <?php if ($type_extrait == '1') {echo 'selected="selected"';}?>>List seizures causing a failure with the obligations of presence</option>
	    <option value='2' <?php if ($type_extrait == '2') {echo 'selected="selected"';}?>>List of all the seizures</option>
	    </select>
	    <br />
	    <button style="font-size:12px" dojoType="dijit.form.Button" type="submit" name="affichage" value="html">ADisplay</button>
	    <button style="font-size:12px" dojoType="dijit.form.Button" type="submit" name="affichage" value="ods">To record with the format ods</button>
	    <?php } else {
	    	?><button style="font-size:12px" dojoType="dijit.form.Button" type="submit" name="affichage" value="csv">To record with the format csv</button>
	    	<?php
...
if ($affichage == 'html') {
    ...
    echo '</tr>';
    echo '</table>';
    echo '<h5>Made extraction it '.date("d/m/Y - H:i").'</h5>';
    ...
    $titre = 'Extract of the absences of '.$dt_date_absence_eleve_debut->format('d/m/Y').' to '.$dt_date_absence_eleve_fin->format('d/m/Y');
    $classe = null;
	...
	if ($traitement_csv_en_cours == 'false') {
		...
		echo '<br/><a href="'.'../backup/'.getSettingValue("backup_directory").'/absences/'.$_REQUEST['filename'].'">Download the result</a>';
		if ($page == 1) {
			//on s'est arreter à la page 0, il n'y a pas de résultat
			echo ' (empty extraction)';
		}
		echo '<br/>';
	} else {
		//print_r($page);die;
		...
		if ($page == 0) {
		    $output = '';
		    $date = new DateTime();
		    $output .= ('Extraction of the seizures of absence '.getSettingValue('gepiSchoolName').' '.getSettingValue('gepiYear')."\n");
		    $output .= 'Made extraction it '.date("d/m/Y - H:i")."\n";
		    $output .= ("Name, First name, Class, absence Beginning, end absence, Type, Failure has the obligation of presence, Under responsibility establishment\n");
			...
			echo '<br/>Please have patience, stage '.$page.' sur '.$saisie_col->getLastPage();
			if (ob_get_contents()) {
				ob_flush();
			}
			flush();
