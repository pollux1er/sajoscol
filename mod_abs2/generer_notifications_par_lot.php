...
if (isset($_GET['envoyer_courrier']) && $_GET['envoyer_courrier'] == 'true') {
	...
	if ($notif->getStatutEnvoi() == AbsenceEleveNotificationPeer::STATUT_ENVOI_ETAT_INITIAL
	    ...
	    $notif->setErreurMessageEnvoi('Failure of the impression by batch');
	    $notif->save();
...
if ($id_notif_col->isEmpty()) {
    echo 'No selected notification -> ';
    echo '<a href="liste_notifications.php">list notifications</a>';
    die;
}
...
if (isset($_GET['envoyer_email']) && $_GET['envoyer_email'] == 'true') {
    ...
    echo 'Mail sent : '.$nb_mail_envoyés.'<br/>';
}
//
...
if (!$notif_mail_fini_col->isEmpty()) {$notif = new AbsenceEleveNotification();
	...
	if ($notif->getStatutEnvoi() == AbsenceEleveNotificationPeer::STATUT_ENVOI_SUCCES
	    ...
	    echo '<div style="color : green;">successful sending</div>';
	} else {
	    echo '<div style="color : red;">Error : '.$notif->getErreurMessageEnvoi().'</div>';
	}
	echo '</td>';
	echo '<td>';
	echo ' <a href="generer_notifications_par_lot.php?retirer_id_notification='.$notif->getId().'">To withdraw batch</a>';
	echo '</td>';
	echo '<td>'.$notif->getEmail().'</td>';
	echo '<td>Status '.$notif->getStatutEnvoi().'</td>';
	echo '<td>'.$notif->getDateEnvoi('d/m/Y H:i').'</td>';
	echo '<td>Treatment '.$notif->getAbsenceEleveTraitement()->getDescription().'</td>';
	echo '</tr>';
    }
    echo '</table></br>';
    echo '<br/><br/>';
...
if (!$notif_mail_a_envoyer_col->isEmpty()) {$notif = new AbsenceEleveNotification();
    echo 'Notifications to be sent by mail';
    echo '<table id="table_liste_absents" style="border-spacing:0px;">';
    //en tete commentaire
    echo '</tr>';
    echo '<th>id</th>';
	...
	echo '<tr>';
	    echo '<td><a href="visu_notification.php?id_notification='.$notif->getId().'">'.$notif->getId().'</a></td>';
	    echo '<td><a href="generer_notifications_par_lot.php?retirer_id_notification='.$notif->getId().'"> withdraw batch</a></td>';
	    echo '<td>'.$notif->getEmail().'</td>';
	    echo '<td>Status '.$notif->getStatutEnvoi().'</td>';
	    echo '<td>Treatment '.$notif->getAbsenceEleveTraitement()->getDescription().'</td>';
	echo '</tr>';
    ...
    echo '</table></br>';
    echo '<a dojoType="dijit.form.Button" onclick="location.href=\'generer_notifications_par_lot.php?envoyer_email=true\';" href="generer_notifications_par_lot.php?envoyer_email=true">send the mail</a>';
    echo '<br/><br/>';
}
...
if (!$notif_courrier_fini->isEmpty()) {$notif = new AbsenceEleveNotification();
	...
	if ($notif->getStatutEnvoi() != AbsenceEleveNotificationPeer::STATUT_ENVOI_ECHEC) {
	    echo '<div style="color : green;">Successful impression</div>';
	} else {
	    echo '<div style="color : red;">Error</div>';
	}
	echo '</td>';
	echo '<td>';
	echo ' <a href="generer_notifications_par_lot.php?retirer_id_notification='.$notif->getId().'">withdraw batch</a>';
	echo '</td>';
	echo '<td>';
	foreach ($notif->getResponsableEleves() as $responsable) {
	...
	if ($notif->getAbsenceEleveTraitement() != null) {
	    echo 'Treatment '.$notif->getAbsenceEleveTraitement()->getDescription();
	}
	echo '</td>';
	echo '</tr>';
    }
...
if ($notif_courrier_a_envoyer_col->isEmpty() && !$notif_courrier_fini->isEmpty()) {
    echo '<a dojoType="dijit.form.Button" onclick="window.open(\'generer_notifications_par_lot.php?envoyer_courrier=true\'); var loc = \'window.location = \\\'generer_notifications_par_lot.php\\\'\'; setTimeout(loc, 3000);" href="generer_notifications_par_lot.php?envoyer_courrier=true">Print the mails duplicate</a>';
    echo '<br/><br/>';
} elseif (!$notif_courrier_a_envoyer_col->isEmpty() || !$notif_courrier_fini->isEmpty()) {
    echo '<a dojoType="dijit.form.Button" onclick="window.open(\'generer_notifications_par_lot.php?envoyer_courrier=true\'); var loc = \'window.location = \\\'generer_notifications_par_lot.php\\\'\'; setTimeout(loc, 3000);" href="generer_notifications_par_lot.php?envoyer_courrier=true">Print all the mails</a>';
    echo '<br/><br/>';
}
