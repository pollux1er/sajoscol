...
echo '<p>';
echo 'Name : <input type="hidden" name="type_selection" value="nom_eleve"/> ';
...
if (!$groupe_col->isEmpty()) {
	...
	echo ("Group : <select name=\"id_groupe\" class=\"small\">");
	...
	echo '<button type="submit">Display the student</button>';
	echo '</p>';
	echo "</form>";
	echo "</td>";
}
...
if (!$classe_col->isEmpty()) {
	...
	echo ("Class : <select name=\"id_classe\" style=\"width:160px\">");
    ...
    echo '<td>No class with affected student was found</td>';
}
...
<!-- Legende du tableau-->
	<?php echo ('<p>');
	    ...
	    echo '&nbsp;&nbsp;&nbsp;Comment : <input name="commentaire" type="text" maxlength="150" size="20"/>';
	    echo '</p> ';
	?>
<!-- Fin de la legende -->
	...
	<table class="tb_absences" style="width:750px;" summary="Liste des élèves pour l'appel. Colonne 1 : élèves, colonne 2 : absence, colonne3 : retard, colonnes suivantes : suivi de la journée par créneaux, dernière colonne : photos si actif">
		...
		<tbody>
			<tr class="titre_tableau_gestion" style="white-space: nowrap;">
				<th style="text-align : center;" abbr="élèves">List of student select :
				<a href="#" onclick="SetAllCheckBoxes('liste_absence_eleve', 'active_absence_eleve[]', '', true); return false;">All</a>
				<a href="#" onclick="SetAllCheckBoxes('liste_absence_eleve', 'active_absence_eleve[]', '', false); return false;">None</a>
				</th>
				<!--th></th>
				<th></th-->
			</tr>
...
<?php
			if (isset($message_erreur_eleve[$eleve->getIdEleve()]) && $message_erreur_eleve[$eleve->getIdEleve()] != '') {
			    echo "<br/>Error : ".$message_erreur_eleve[$eleve->getIdEleve()];
			}
...
echo "<td style='width:270px; vertical-align: top;'>";
	...
	echo '<p>';
    echo 'Beginning : <input size="9" id="date_absence_eleve_debut_saisir_eleve" name="date_absence_eleve_debut_saisir_eleve" value="'.$dt_date_absence_eleve_debut_saisir_eleve->format('d/m/Y').'" />&nbsp;';
   echo '</p>';
     echo '
...
echo '<br/>';
	echo '<p>';
echo 'End : <input size="9" id="date_absence_eleve_fin_saisir_eleve" name="date_absence_eleve_fin_saisir_eleve" value="'.$dt_date_absence_eleve_fin_saisir_eleve->format('d/m/Y').'" />&nbsp;';
...
echo '<p>';
echo 'Of <input name="heure_debut_absence_eleve" value="';
...
echo '	Create only one seizure <br/>';
echo '	<input type="radio" name="multisaisie" value="y"/>';
echo '	Create a seizure per day';
echo '</p></div>';
echo 'or ';
echo '<div style="border-width: 1px; border-style: solid; text-align: left; padding : 2px; margin : 4px;">';
    echo '<p>';
    ...
    echo "<option value='-1'>choose a crenel</option>\n";
...
if (!$cours_col->isEmpty()) {
	...
	echo "<option value='-1'>choose a course</option>\n";
    ...
    echo '<p>';
		...
		//$semaine = new EdtSemaine();
		    ...
		    echo "Week ".$semaine->getNumEdtSemaine()." ".$semaine->getTypeEdtSemaine();
	    ...
	    echo '<a href="./saisir_eleve.php">Not display every week</a>';
    echo '</p>';
	} else {
    echo '<p>';
	    //on va commencer la liste à la semaine 31 (milieu des vacances d'ete)
		...
		//$semaine = new EdtSemaine();
			...
			echo " of ".$semaine->getLundi('d/m').' au '.$semaine->getSamedi('d/m');
		    }
		    echo "<br/>\n";
	    }
	    echo '<a href="./saisir_eleve.php?affiche_toute_semaine=oui">Display every week</a>';
    echo '</p>';
	}
	echo '</div>';
