...
if ($utilisateur->getStatut() == 'professeur' && (getSettingValue("abs2_saisie_prof_decale")!='y')) {
	...
	echo "<span style='color :red'>Error : Bad format of date of absence.</span><br/>";
	$dt_date_absence_eleve = new DateTime('now');
    }
} else {
    $dt_date_absence_eleve = new DateTime('now');
...
//	    $edt_cours = new EdtEmplacementCours();
	...
	if (getSettingValue("abs2_saisie_prof_decale")=='y' || $utilisateur->getStatut() != "professeur") {
	    ...
	    echo "<option value='-1'>choose one week</option>\n";
		...
		//$semaine = new EdtSemaine();
		    ...
		    echo " Of ".$semaine->getLundi('d/m').' to '.$semaine->getSamedi('d/m');
		    echo "</option>\n";
	    }
	    echo "</select>&nbsp;";
	} else {
	    echo "Week&nbsp;".$current_semaine->getNumEdtSemaine()."&nbsp;".$current_semaine->getTypeEdtSemaine();
	...
	if ($current_cours != null && $current_cours->getTypeSemaine() != '' && $current_cours->getTypeSemaine() != '0' && $current_semaine != null && $current_cours->getTypeSemaine() != $current_semaine->getTypeEdtSemaine()) {
	    echo '<br>Error : the course does not correspond to the type of week.';
	    $current_cours = null;
	    $current_groupe = null;
	    $current_classe = null;
	    $current_aid = null;
...
if (!$classe_col->isEmpty()) {
    ...
    echo ("Class : <select name=\"id_classe\" class=\"small\">");
	...
	<p>
			...
			<p class="expli_page choix_fin">
				...
				<?php if (isset($current_groupe) && $current_groupe != null) {
				    echo 'the group '.$current_groupe->getNameAvecClasses();
				} else if (isset($current_aid) && $current_aid != null) {
				    echo 'l\'aid '.$current_aid->getNom();
				} else if (isset($current_classe) && $current_classe != null) {
				    echo 'the class '.$current_classe->getNom();
				}?>
				</strong>
				...
				<?php } ?>
				<br/>
				(the not notched student will be considered present)
			</p>
			<p class="choix_fin">
			...
			<?php if ($utilisateur->getStatut() == 'professeur' && getSettingValue("active_cahiers_texte")=='y') {
			    echo '
			    <p class="choix_fin">
				    <input value="To record and pass in the homework diary" name="cahier_texte" type="submit"/>
			    </p>';
			} ?>
...
<!-- <table style="text-align: left; width: 600px;" border="0" cellpadding="0" cellspacing="1"> -->
	<table class="tb_absences" summary="List student for the call. Column 1 : student, Column 2 : absence, Column 3 : delay, following columns : follow-up of the day by crenels, last column: photographs so active">
...
<?php
			...
			for($i = 0; $i<$col_creneaux->count(); $i++){
					...
					if ($nb_creneau_a_saisir > 0) {
						...
						echo "Error : ".$message_erreur_eleve[$eleve->getIdEleve()];
					    }
...
<?php				}
				...
				if ($current_creneau != null) {
				    echo '<td style="font-size:88%;">Comment : ';
