    ...
    if ($affichage != null && $affichage != '') {
	    ...
	    if (!$table_synchro_ok) {//la table n'est pas synchronisée. On va vérifier individuellement les élèves qui se sont pas synchronisés
			...
			if ($eleve_col->count()>150) {
				echo 'It seems that you to ask for statistics on too many student and your
table of statistics is not synchronized. Please make a request for less student or ask your administrator to fill the table with aggregation.';
				if (ob_get_contents()) {
					ob_flush();
				}
				flush();
    ...
    <?php endif; ?>
        <p>
            The formula used for the calculation of the absentee rate is the
percentage of the number of half-days of absences compared to the
number of wrought half-days.<br /><br />
        </p>
        <form dojoType="dijit.form.Form" name="abs_statistiques" id="abs_statistiques" action="statistiques.php" method="post">
            <fieldset>
                <legend>Parameter setting of export (dates, classes?) and Display</legend>  
                <h2>Absentee rate of  
                ...
                ?>
                Name (optional) : <input dojoType="dijit.form.TextBox" type="text" style="width : 10em" name="nom_eleve" size="10" value="<?php echo $nom_eleve ?>" onChange="document.bilan_individuel.id_eleve.value='';"/>
            ...
            ?> > Display the detail by reason (Longer treatment on all the classes). 
                <br />
                <button type="submit"  style="font-size:12px" dojoType="dijit.form.Button" name="affichage" value="html"> validate the modifications and to post with the screen</button>   
                <button type="submit" style="font-size:12px" dojoType="dijit.form.Button" name="affichage" value="ods" >Record with the format ods</button>     
            </fieldset>
            <br />       
            <?php
        }
        if ($affichage == 'html') {
            ...
            $titre = 'Extract of the Absentee rate of absences of ' . $dt_date_absence_eleve_debut->format('d/m/Y') . ' to ' . $dt_date_absence_eleve_fin->format('d/m/Y');
            $classe = null;
