...
/**
 *
 * @version $Id: visu_saisie.php 7601 2011-08-07 12:49:03Z dblanqui $
 *
...
if (!$modifiable) {
    echo "The seizure is not modifiable<br/>";
}
...
echo $saisie->getPrimaryKey();
    if ($saisie->getDeletedAt()!=null) {
    	echo ' <font color="red">(removed it ';
...
if ($modifiable) {   
    ...
    if ($saisie->getDeletedAt() == null) {
        ...
        echo '<button dojoType="dijit.form.Button" type="submit">Remove the seizure</button>';
        //echo '</a>';
    } else {
        ...
        if ($utilisateur->getStatut() == "cpe" || $utilisateur->getStatut() == "scolarite"
            ...
            echo '<button dojoType="dijit.form.Button" type="submit">Restore the seizure</button>';
            //echo '</a>';
        }
    }
    echo'</form>';
...
echo ' by '.  $saisie->getUtilisateurProfessionnel()->getCivilite().' '.$saisie->getUtilisateurProfessionnel()->getNom().' '.substr($saisie->getUtilisateurProfessionnel()->getPrenom(), 0, 1).'.';
echo '</TD></tr>';
...
if ($saisie->getCreatedAt('U') != $saisie->getVersionCreatedAt('U')) {
    ...
    if ($modifie_par_utilisateur != null) {
		echo ' by '.  $modifie_par_utilisateur->getCivilite().' '.$modifie_par_utilisateur->getNom().' '.substr($modifie_par_utilisateur->getPrenom(), 0, 1).'.';
    }
    echo '</TD></tr>';
}
...
if ($saisie->getIdSIncidents() !== null) {
	...
	$saisie->getId()."&return_url=no_return'>To seize a disciplinary incident</a>";
    echo '</TD></tr>';
}
$saisies_conflit_col = $saisie->getSaisiesContradictoiresManquementObligation();
if (!$saisies_conflit_col->isEmpty()) {
    echo '<tr><TD>';
    echo 'The seizure is in contradiction with : ';
    echo '</TD><TD colspan="2">';
...
if ($modifiable) {
    ...
    echo '>To record the modifications</button>';
    echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
    echo '</td></tr>';
}
