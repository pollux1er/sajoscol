...
if ($action_sql == "supprimer") {
      ...
      if ($test == "0")
      {
         ...
         // Execution de cette requete
            ...
            $msg = "The suppression was carried out successfully.";
      } else {
          $msg = "Can not be deleted because one or more absences were recorded with this type of pattern. Start by removing the absences involved";
      }
...
// header
$titre_page = "Management reasons for absence";
...
<div style="text-align:center">
    <h2>Definition of absence reasons</h2>
	<a href="admin_motifs_absences.php?action=ajouter"><img src='../../images/icons/add.png' alt='' class='back_link' /> Add one or more reason (s)</a>
	<br/><br/>
    <table cellpadding="0" cellspacing="1" class="tab_table">
      <tr>
        <td class="tab_th">Code</td>
        <td class="tab_th">Definition</td>
    ...
    while ( $data_motif = mysql_fetch_array( $execution_motif ) ) {
       if ($i === '1') { $couleur_cellule = 'couleur_ligne_1'; $i = '2'; } else { $couleur_cellule = 'couleur_ligne_2'; $i = '1'; } ?>
        <tr class="<?php echo $couleur_cellule; ?>">
          ...
          <td><?php if ( $data_motif['init_motif_absence'] != 'A' ) { ?><a href="admin_motifs_absences.php?action=visualiser&amp;action_sql=supprimer&amp;id_motif=<?php echo $data_motif['id_motif_absence']; ?>" onClick="return confirm('Are you sure you want to remove this pattern ?')"><img src="../images/x2.png" width="22" height="22" title="Supprimer" border="0" alt="" /></a><?php } ?></td>
        </tr>
     <?php } ?>
    </table>
    <br/><br/>
...
<div style="text-align:center">
    <?php
    	if($action=="ajouter") { 
    		echo "<h2> Adding one or more reason (s)</h2>";
		} elseif ($action=="modifier") {
			echo "<h2>Change a pattern</h2>";
		}
	?>
    ...
    <form method="post" action="admin_motifs_absences.php?action=ajouter" name="form1" id="form1">
     <fieldset class="fieldset_efface">
      <table cellpadding="2" cellspacing="2" class="tab_table">
        <tr>
          <th class="tab_th">Number of reasons to add</th>
        </tr>
     ...
     <fieldset class="fieldset_efface">
      <table cellpadding="2" cellspacing="2" class="tab_table">
        ...
        <td class="centre"><img src="../images/attention.png" width="28" height="28" alt="" /></td>
          <td colspan="2" class="erreur_rouge_jaune"><b>- Error -<br />
          <?php if ($verification_erreur[$nb] === 2) { ?>This initial pattern is already<?php } ?>
