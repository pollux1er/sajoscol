...
// header
$titre_page = "Definition of slots";
...
// On regarde la table utilisée avant d'afficher le lien
	if ($cren == "diff") {
		$cherche_table = '';
		$texte_lien = ' See slots daily';
	}else{
		$cherche_table = '&amp;cren=diff';
		$texte_lien = ' See the different niches of the day';
	}
...
if ($action == "visualiser") {
	if ($cren == "diff") {
		...
		echo '<p>You must choose a single day when the slots are different from other days (like Wednesday ...)</p>';
...
if ($compter >= 1) {
	echo "<p class=\"red\">Beware, if you change slots now, during the schedule will be disrupted!</p>";
}
	/* div de centrage du tableau pour ie5 */
?>
<div style="text-align: center;">
<h2>Definition of slots</h2>
	<p>
		<a href="admin_periodes_absences.php?action=ajouter<?php echo $aff_creneau_diff; ?>">
		<img src='../../images/icons/add.png' alt='' class='back_link' /> Add a time slot
		</a>
	</p><br />
	...
	<table cellpadding="0" cellspacing="1" class="tab_table" summary="Créneaux">
		<tr>
			...
			<th class="tab_th" style="width: 90px;">start time</th>
			<th class="tab_th" style="width: 90px;">end time</th>
...
<?php
	...
	?>
        <tr class="<?php echo $couleur_cellule; ?>">
          ...
          <td><a href="admin_periodes_absences.php?action=visualiser<?php echo $aff_creneau_diff; ?>&amp;action_sql=supprimer&amp;id_periode=<?php echo $data_periode['id_definie_periode']; ?>" onClick="return confirm('Are you sure you want to remove this niche?')"><img src="../images/x2.png" width="22" height="22" title="Supprimer" border="0" alt="Supprimer" /></a></td>
        </tr>
     <?php } ?>
    </table>
  ...
  <?php if ($action == "ajouter") { ?>
    ...
    <form name="form1" method="post" action="admin_periodes_absences.php?action=ajouter<?php echo $aff_creneau_diff; ?>">
      <table class="tab_table" summary="Créneaux">
        <tr>
          <th class="tab_th">Number of slots to add</th>
        </tr>
        <tr>
    ...
    <form action="admin_periodes_absences.php?action=visualiser<?php echo $aff_creneau_diff; ?>&amp;action_sql=<?php if($action=="ajouter") { ?>ajouter<?php } if($action=="modifier") { ?>modifier<?php } ?>" method="post" name="form2" id="form2">
      <table cellpadding="2" cellspacing="2" class="tab_table" summary="Créneaux">
        <tr>
          ...
          <th class="tab_th">Start time</th>
          <th class="tab_th">End time</th>
          <th class="tab_th">Type</th>
          <th class="tab_th">logical</th>
        </tr>
        <?php
        $i = '1';
        $nb = 0;
        ...
        <?php if (isset($verification_erreur[$nb]) and $verification_erreur[$nb] != 1) { ?>
         <tr>
          ...
          <?php if ($verification_erreur[$nb] == 2) { ?>This time slot is already<?php } ?>
          <?php if ($verification_erreur[$nb] == 5) { ?>The end time is not defined<?php } ?>
          <?php if ($verification_erreur[$nb] == 4) { ?>The start time is not defined<?php } ?>
          <?php if ($verification_erreur[$nb] == 3) { ?>All fields are not filled<?php } ?>
          <?php if ($verification_erreur[$nb] == 6) { ?>The end time can not be smaller than the start time<?php } ?>
          </b><br /></td>
         </tr>
        <?php } ?>
        <tr class="<?php echo $couleur_cellule; ?>">
		...
		<select name="type_creneaux[<?php echo $nb; ?>]" size="3" class="input_sans_bord">
			...
			<option value="pause" <?php if ( $action === 'modifier' ) { if ( $data_modif_periode['type_creneaux'] === 'pause' ) { ?>selected="selected"<?php } } elseif (isset($type_creneaux_erreur[$nb])) { if ( $type_creneaux_erreur[$nb] === 'pause' ) { ?>selected="selected"<?php } } ?>>break</option>
			<option value="repas" <?php if ( $action === 'modifier' ) { if ( $data_modif_periode['type_creneaux'] === 'repas' ) { ?>selected="selected"<?php } } elseif (isset($type_creneaux_erreur[$nb])) { if ( $type_creneaux_erreur[$nb] === 'repas' ) { ?>selected="selected"<?php } } ?>>meals</option>
		</select>
	  <td><input name="suivi_definie_periode[<?php echo $nb; ?>]" value="1" type="checkbox" <?php if ( ( $action === 'modifier' and $data_modif_periode['suivi_definie_periode'] === '1' ) or ( isset($suivi_definie_periode_erreur[$nb]) and $suivi_definie_periode_erreur[$nb] === '1') ) { ?>checked="checked"<?php } ?> title="logical scheduling" />
