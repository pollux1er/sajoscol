 ...
 */
$accessibilite="y";
$titre_page = "Gestion du module de l'absence";
$niveau_arbo = 2;
$gepiPathJava="./../..";
$post_reussi=FALSE;
$msg = '';
...
if (isset($_POST['is_posted']) and ($msg=='')) {
	...
	if (!((getSettingValue("active_module_absence")!='y')&& $_POST['activer']=="n")) {
		if (!saveSetting("active_module_absence", $_POST['activer'])) {
			$msg = "Error registering the activation/deactivation parameter !";
		}
	}
	}
	...
	if (isset($_POST['classement'])) {
		if (!saveSetting("absence_classement_top", $_POST['classement'])) {
			$msg = "Error when saving parameter classementdes absences (TOP 10) !";
		}
	}
...
if (!suivi_ariane($_SERVER['PHP_SELF'],"Gestion Absences"))
		echo "error when building the breadcrumb";
/****************************************************************
			FIN HAUT DE PAGE
****************************************************************/
...
if ($req_setting["value"] == 'y' OR $req_setting2["value"] == 'y') {
 // On initialise le $_SESSION["retour"] pour pouvoir revenir proprement
  ...
  $lien_sup[$a]['texte'] = "Define holiday periods and public holidays";
  $a++;
} else {
...
<div class="centre"><input type="submit" value="Save" style="font-variant: small-caps;"/></div>
</form>
