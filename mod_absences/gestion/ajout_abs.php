...
/*
*
*$Id: ajout_abs.php 8257 2011-09-16 21:45:48Z regis $
*
					...
					if ($d_date_absence_eleve_ins <= $a_date_absence_eleve_ins)
					{
						...
						if (date("w", mktime(0, 0, 0, $d_date_absence_eleve_verif[1], $d_date_absence_eleve_verif[2], $d_date_absence_eleve_verif[0])) != '0' OR $test_jour_dimanche == 'oui')
                        {
							...
							if (date("w", mktime(0, 0, 0, $a_date_absence_eleve_verif[1], $a_date_absence_eleve_verif[2], $a_date_absence_eleve_verif[0])) != '0' OR $test_jour_dimanche == 'oui')
							{
                                ...
                                if(( $d_heure_absence_eleve != "" and $a_heure_absence_eleve != "") or ( $dp_absence_eleve != '' or $ap_absence_eleve != '' ))
                                {
                                    ...
                                    if ($a_heure_absence_eleve_ins > $d_heure_absence_eleve_ins)
                                    {
                                        ...
                                        if ($d_date_absence_eleve_ins == $a_date_absence_eleve_ins)
                                        {
                                            $verification = '11';
											$erreur = '1';
											$texte_erreur="The start time can not be big than the end time";
									...
									{
										$verification = '2';
										$texte_erreur = "there is no schedule or time slots input ";
									}
									if(( $d_heure_absence_eleve != '' and $a_heure_absence_eleve != '' ) and ( $dp_absence_eleve != '' or $ap_absence_eleve != '' ))
									{
										$verification = '3';
										$texte_erreur = "you can not type a period and time";
									}
									$erreur = '1';
                                }
							} else {
								$verification = '7';
								$erreur = '1';
								$texte_erreur = "has end date falls on a Sunday";
							}
						} else {
							$verification = '6';
							$erreur = '1';
							$texte_erreur = "has start date falls on a Sunday";
						}
                    } else {
						$verification = '8';
						$erreur='1';
						$texte_erreur = "the start date to be small than the end date...";
					}
                } else {
					$verification = '5';
					$erreur = '1';
					$texte_erreur = "has end date not be correct";
				}
			} else {
				$verification = '4';
				$erreur = '1';
				$texte_erreur = "has start date not be correct";
			}
						...
						and $a_heure_absence_eleve_ins >=  $data['d_heure_absence_eleve']))
                	{
                    	...
                    	if (($d_heure_absence_eleve_ins >= $data['d_heure_absence_eleve'] and $d_heure_absence_eleve_ins <= $data['a_heure_absence_eleve']) and $a_heure_absence_eleve_ins >= $data['a_heure_absence_eleve'])
                        {
							...
							if ($d_heure_absence_eleve_ins >= $data['d_heure_absence_eleve'] and $a_heure_absence_eleve_ins <= $data['a_heure_absence_eleve'])
							{
                            	$erreur='1';
                                $verification = '10';
                                $texte_erreur="a absent is already record at this horary";
			...
			if($d_date_absence_eleve_ins != $a_date_absence_eleve_ins)
			{
				...
				while ($data = mysql_fetch_array($resultat_m))
				{
					...
					} elseif($d_date_absence_eleve_ins < $data['d_date_absence_eleve'] and $a_date_absence_eleve_ins > $data['a_date_absence_eleve'])
					{
                    	$erreur = '1';
                        $verification = '10';
                        $texte_erreur="you try to record a absent into an absent time interval  who are already attached ";
                        $id_abs = $data['id_absence_eleve'];
                        $erreur_aff_d_date_absence_eleve = date_fr($data['d_date_absence_eleve']);
                        $erreur_aff_a_date_absence_eleve = date_fr($data['a_date_absence_eleve']);
                        $erreur_aff_d_heure_absence_eleve = $data['d_heure_absence_eleve'];
					...
					} elseif($d_date_absence_eleve_ins >= $data['d_date_absence_eleve'] and $a_date_absence_eleve_ins <= $data['a_date_absence_eleve'])
                    {
						$erreur = '1';
						$verification = '10';
                        $texte_erreur=" a absent is already record in a date interval";
						$id_abs = $data['id_absence_eleve'];
						$erreur_aff_d_date_absence_eleve = date_fr($data['d_date_absence_eleve']);
						$erreur_aff_a_date_absence_eleve = date_fr($data['a_date_absence_eleve']);
						$erreur_aff_d_heure_absence_eleve = $data['d_heure_absence_eleve'];
...
?><div style="text-align:center"><?php
    //affichage des messages d'erreur
        ...
        <?php if($verification === '10' or $verification === '9') { ?>
                       <table style="margin: auto; width: 500px;" border="0" cellspacing="2" cellpadding="0">
                         <tr class="fond_rouge">
                           <td class="norme_absence_blanc_min"><strong>Of</strong></td>
                           <td class="norme_absence_blanc_min"><strong>At</strong></td>
                           <td class="norme_absence_blanc_min"><strong>Of</strong></td>
    ...
    <form method="post" action="ajout_abs.php?type=<?php echo $type; ?>&amp;id=<?php echo $id; ?>&amp;mode=<?php echo $mode; ?>" name="form1">
     <fieldset class="fieldset_efface">
      <table class="entete_tableau_absence" border="0" cellspacing="0" cellpadding="1">
        <tr>
          <td class="titre_tableau_absence" nowrap><strong>absent of student</strong></td>
          <td class="titre_tableau_absence_valider"><input type="submit" name="submit" value="Valider" /></td>
        </tr>
        <tr class="tr_tableau_absence_titre">
          <td class="centre">Identification of student</td>
          <td class="centre">Information on absent</td>
        </tr>
		...
		if ($mode==="eleve")
		{
			...
			{  ?>
				<table class="tableau_info_compt" border="0" cellspacing="0" cellpadding="2">
					<tr>
                        <td class="tableau_info_disp"><a href="javascript:centrerpopup('../lib/liste_absences.php?id_eleve=<?php echo $id_eleve; ?>&amp;type=D',260,320,'scrollbars=yes,statusbar=no,resizable=yes');" title="See exemption">exemption detect</a></td>
                    </tr>
                </table>
			<?php
			} ?>
                <table class="tableau_info_compt" border="0" cellspacing="0" cellpadding="2">
                    <tr>
                        <td class="tableau_info_compt"><a href="javascript:centrerpopup('../lib/liste_absences.php?id_eleve=<?php echo $id_eleve; ?>&amp;type=<?php echo $type; ?>',260,320,'scrollbars=yes,statusbar=no,resizable=yes');" title="See Absent">See Absent</a></td>
                    </tr>
                </table>
		<?php
		} ?>
		...
		?>
							<option value="">not choise</option>
		<?php
        ...
        <?php
                ...
                $resultat_pe = mysql_query($requete_pe) or die('Erreur SQL !'.$requete_pe.'<br />'.mysql_error()); ?>
							...
							<option value="">not choise</option><?php
                ...
                <tr>
					<td colspan="3">
						<span class="norme_absence_bleu">
							<strong>!</strong>if the date of and at are identical, inform only of 
						</span>
					</td>
                </tr>
        <?php } ?>
                <tr class="tr_tableau_absence_titre">
                  <td colspan="2"><strong>Motive</strong></td>
           ...
           <?php /* <strong>Action</strong><br />
                   ...
                   <td>
                    <select name="justify_absence_eleve[<?php echo $i; ?>]">
                      <option value="N" <?php if (($action === "modifier" and $justify_absence_eleve[$i] === "N") or ( isset($justify_absence_eleve_erreur[$i]) and $justify_absence_eleve_erreur[$i] === "N")) { ?>selected<?php } else { ?>selected<?php } ?>>None</option>
                      <option value="O" <?php if (($action === "modifier" and $justify_absence_eleve[$i] === "O") or ( isset($justify_absence_eleve_erreur[$i]) and $justify_absence_eleve_erreur[$i] === "O")) { ?>selected<?php } ?>>Yes</option>
                      <option value="T" <?php if (($action === "modifier" and $justify_absence_eleve[$i] === "T") or ( isset($justify_absence_eleve_erreur[$i]) and $justify_absence_eleve_erreur[$i] === "T")) { ?>selected<?php } ?>>By phone</option>
    ...
    ?>
                    ...
                    <div class="norme_absence_orange";><strong>Absent of student the day before</strong></div>
                    <table style="margin: auto; width: 500px;" border="0" cellspacing="2" cellpadding="0">
    ...
    ?>
                    ...
                    <div class="norme_absence_rouge"><strong>list of absent already record for this date </strong></div>
                    <table style="margin: auto; width: 500px;" border="0" cellspacing="2" cellpadding="0">
                        <tr class="fond_rouge">
                          <td class="norme_absence_blanc_min"><strong>Of</strong></td>
                          <td class="norme_absence_blanc_min"><strong>At</strong></td>
...
<?php $entete_absence_1 = 'Addition one or many absent'; ?>
  <form method="post" action="ajout_abs.php?type=<?php echo $type; ?>" name="form1">
   <fieldset class="fieldset_efface">
    <table class="entete_tableau_absence" border="0" cellspacing="0" cellpadding="1">
        ...
        <tr class="tr_tableau_absence_titre">
            <td class="centre">Identity</td>
            <td class="centre">Date</td>
            <td class="centre">Horary</td>
            <td class="centre">battlements</td>
            <td>Indication</td>
        </tr>
    <?php   $i = '0'; $ic = '1';
       ...
       ?>
            <?php if (isset($verification_erreur[$i]) and $verification_erreur[$i] != '1') { ?>
             <tr class="table_erreur">
              ...
              <?php if ($verification_erreur[$i] === '2') { ?>there is no timer setting<?php } ?>
			  
              <?php if ($verification_erreur[$i] === '3') { ?>you can not type a period and time<?php } ?>
              <?php if ($verification_erreur[$i] === '4') { ?>The start date is not correct<?php } ?>
              <?php if ($verification_erreur[$i] === '5') { ?>the end date is not correct<?php } ?>
              <?php if ($verification_erreur[$i] === '6') { ?>the start date falls on a Sunday<?php } ?>
              <?php if ($verification_erreur[$i] === '7') { ?>the end date falls on a Sunday<?php } ?>
              <?php if ($verification_erreur[$i] === '8') { ?>The start date must be smaller than the end date<?php } ?>
              <?php if ($verification_erreur[$i] === '9') { ?>an absence is already registered for this date in this zone set<?php } ?>
              <?php if ($verification_erreur[$i] === '10') { ?>an absence is already registered for this date in this zone set<?php } ?>
              <?php if ($verification_erreur[$i] === '11') { ?>The start time can not be greater than end<?php } ?>
              </strong></td>
             </tr>
            <?php } ?>
        <tr class="<?php echo $couleur_cellule; ?>">
			...
			if ($test_dispense != '0')
			{  ?>
				<div class="tableau_info_compt"><a href="javascript:centrerpopup('../lib/liste_absences.php?id_eleve=<?php echo $id_eleve; ?>&amp;type=D',260,320,'scrollbars=yes,statusbar=no,resizable=yes');" title="See exemption ">exemption detected</a></div>
                  
			<?php
			} ?>
                
             <div class="tableau_info_compt"><a href="javascript:centrerpopup('../lib/liste_absences.php?id_eleve=<?php echo $id_eleve; ?>&amp;type=<?php echo $type; ?>',260,320,'scrollbars=yes,statusbar=no,resizable=yes');" title="see absent">see absent</a></div>
              </td>     
            
            <td><?php if(($mode === "classe" and $i === '0') or $mode != "classe") { ?>
              of&nbsp;&nbsp;<a href="#calend" onClick="<?php echo $cal_a[$i]->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a><br /><input name="d_date_absence_eleve[<?php echo $i; ?>]" onfocus="javascript:this.select()" type="text" value="<?php if(empty($d_date_absence_eleve)) { echo $datejour; } else { if($action == "modifier" ) { echo $d_date_absence_eleve[$i]; } else { if(isset($d_date_absence_eleve_erreur[$i])) { echo $d_date_absence_eleve_erreur[$i]; } else { echo date_fr($d_date_absence_eleve); } } } ?>" size="10" maxlength="10" /><br />
              to&nbsp;&nbsp;<a href="#calend" onClick="<?php echo $cal_b[$i]->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a><br /><input name="a_date_absence_eleve[<?php echo $i; ?>]" onfocus="javascript:this.select()" type="text" value="<?php if(isset($a_date_absence_eleve_erreur[$i])) { echo $a_date_absence_eleve_erreur[$i]; } else { ?>JJ/MM/AAAA<?php } ?>" size="10" maxlength="10" />
           </td><?php } ?>
            <td><?php if(($mode === "classe" and $i === '0') or $mode != "classe") { ?>
            ...
            <td><?php if(($mode === "classe" and $i === '0') or $mode != "classe") { ?>
              ...
              <select name="justify_absence_eleve[<?php echo $i; ?>]">
                ...
                <option value="T" <?php if(isset($justify_absence_eleve[$i]) and $justify_absence_eleve[$i] === 'T') { ?>selected="selected"<?php } ?>>by phone</option>
                <option value="O" <?php if(isset($justify_absence_eleve[$i]) and $justify_absence_eleve[$i] === 'O') { ?>selected="selected"<?php } ?>>Yes</option>
              </select>
