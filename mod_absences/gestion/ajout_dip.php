                     ...
                     if ($d_date_absence_eleve <= $a_date_absence_eleve)
                      {
                            ...
                            if (date("w", mktime(0, 0, 0, $d_date_absence_eleve_verif[1], $d_date_absence_eleve_verif[2], $d_date_absence_eleve_verif[0])) != 0)
                            {
                                   ...
                                   if (date("w", mktime(0, 0, 0, $a_date_absence_eleve_verif[1], $a_date_absence_eleve_verif[2], $a_date_absence_eleve_verif[0])) != 0)
                                    {
                                        $verification = '1';
                                    } else { $verification = '7'; $erreur = '1'; $texte_erreur = "the end date falls on a Sunday."; }
                             } else { $verification = '6'; $erreur = '1'; $texte_erreur = "the start date falls on a Sunday."; }
                      } else { $verification = '8'; $erreur='1'; $texte_erreur = "The start date can not be greater than end."; }
               } else { $verification = '4'; $erreur = '1'; $texte_erreur = "The end date is not correct."; }
        } else { $verification = '5'; $erreur = '1'; $texte_erreur = "The start date is not correct."; }
          ...
          if ($verification === '1' and $erreur === '0')
                 {
                                   ...
                                   for($i =0;$i<strlen($info_absence_eleve);$i++)
                                    {
                                        if (strtolower($p) == strtolower(substr($info_absence_eleve,$i,2)))
                                         {
                                           $verification = '12'; $texte_erreur = "This period is not good!";
                                         }
                                    }
                               }
                        }
                      ...
                      // s'il y a une erreur dans la saisie des jours
                        if ($erreur_jour != '1')
                         ...
                         } else { $verification = '3'; $erreur='1'; $texte_erreur = "There is an error in the days of spécification."; }
                 }
...
<p class=bold>|<a href='gestion_absences.php?type=<?php echo $type; ?><?php if($fiche==='oui') { ?>&select_fiche_eleve=<?php echo $eleve_absent[0];?>&aff_fiche=abseleve#abseleve<?php } ?>'>Return</a>|
</p><?php
  ...
  <form method="post" action="ajout_dip.php?type=<?php echo $type; ?>" name="form1">
    <fieldset class="fieldset_efface">
      <table class="entete_tableau_absence" border="0" cellspacing="0" cellpadding="1">
        <tr>
          <td class="titre_tableau_absence" nowrap><strong>dispensation</strong></td>
        ...
        <tr class="tr_tableau_absence_titre">
          <td class="centre">Identity of the student</td>
          <td class="centre">Information on exemption</td>
        </tr>
                   ...
                   if ($test_dispense != 0)
                    {  ?>
                        <table class="tableau_info_compt" border="0" cellspacing="0" cellpadding="2">
                          <tr>
                            <td class="tableau_info_compt"><a href="javascript:centrerpopup('../lib/liste_absences.php?id_eleve=<?php echo $id_eleve; ?>&type=<?php echo $type; ?>', 260, 320, 'scrollbars=yes,statusbar=no,resizable=yes');">see these exemptions</a></td>
                          </tr>
                        </table>
              <?php } ?>
         </td>
...
?>
                 <div class="norme_absence_rouge"><strong>list of exemptions déjas recorded for this date</strong></div>
                 <?php /* div de centrage du tableau pour ie5 */ ?>
                 <div style="text-align:center">
                 <table style="margin: auto; width: 500px;" border="0" cellspacing="2" cellpadding="0">
                 <tr class="fond_rouge">
                      <td class="norme_absence_blanc"><strong>Of</strong></td>
                      <td class="norme_absence_blanc"><strong>to</strong></td>
                 </tr>
