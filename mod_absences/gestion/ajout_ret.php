           ...
           if(verif_date($d_date_absence_eleve_form) === 'pass')
            {
                if(my_eregi("[0-9]{2}:[0-9]{2}",$d_heure_absence_eleve_form))
                 {
                  } else { $erreur='1'; $erreur_valu='2'; $texte_erreur="The time format is not correct."; }
             } else { $erreur='1'; $erreur_valu='3'; $texte_erreur="The date is not correct."; }
       } else { $erreur='1'; $erreur_valu='4'; $texte_erreur="No time has been entered"; }
...
//**************** FIN EN-TETE *****************
?>
<p class="bold"><a href='gestion_absences.php?type=<?php echo $type; ?><?php if($fiche==='oui') { ?>&select_fiche_eleve=<?php echo $eleve_absent[0];?>&aff_fiche=abseleve#abseleve<?php }?>'><img src="../../images/icons/back.png" alt="Retour" title="Return" class="back_link" /> Return</a>
</p><?php
...
<form method="post" action="ajout_ret.php?type=<?php echo $type; ?>" name="form1">
 <fieldset class="fieldset_efface">
   <table class="entete_tableau_absence" border="0" cellspacing="0" cellpadding="1">
    <tr>
      <td colspan="3" class="titre_tableau_absence"><b><?php echo "Delays students"; ?></b></td>
    ...
    <tr class="tr_tableau_absence_titre">
      ...
      <td class="centre"></td>
      <td class="centre">Date/Horary</td>
	 ...
	 <?php
             $compte = mysql_result(mysql_query("SELECT COUNT(*) FROM ".$prefix_base."absences_eleves
                 ...
                 <table class="tableau_info_compt" border="0" cellspacing="0" cellpadding="2">
                   <tr>
                     <td>Student arrived <br /><strong><?php if ($compte != 0) { ?><a href="javascript:centrerpopup('../lib/liste_absences.php?id_eleve=<?php echo $id_eleve; ?>&amp;type=<?php echo $type; ?>',260,320,'scrollbars=yes,statusbar=no,resizable=yes');" title=""><?php } ?><?php echo $compte; ?>time<?php if ($compte != 0) { ?></a><?php } ?></strong><br />late</td>
                   </tr>
                 </table>
      </td>
      <td>the <input name="d_date_absence_eleve[<?php echo $i; ?>]" onfocus="javascript:this.select()" type="text" size="12" maxlength="12" value="<?php if(isset($d_date_absence_eleve[$i]) and !empty($d_date_absence_eleve[$i])) { echo $d_date_absence_eleve[$i]; } else { echo $datejour; } ?>" /><a href="#calend" onClick="<?php echo $cal_[$i]->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a><br /><br />to <input name="d_heure_absence_eleve[<?php echo $i; ?>]" onfocus="javascript:this.select()" type="text" size="5" maxlength="5" value="<?php if(isset($d_heure_absence_eleve[$i]) and !empty($d_heure_absence_eleve[$i])) { echo $d_heure_absence_eleve[$i]; } else { ?>00:00<?php } ?>" /></td>
      <td><?php echo "Motif :"; ?><br />
         ...
         <select name="justify_absence_eleve[<?php echo $i; ?>]">
                      <option value="N" <?php if (isset($justify_absence_eleve[$i]) and $justify_absence_eleve[$i] === "N") { ?>selected<?php } else { ?>selected<?php } ?>>no</option>
                      <option value="O" <?php if (isset($justify_absence_eleve[$i]) and $justify_absence_eleve[$i] === "O") { ?>selected<?php } ?>>Yes</option>
                      <option value="T" <?php if (isset($justify_absence_eleve[$i]) and $justify_absence_eleve[$i] === "T") { ?>selected<?php } ?>>by phone</option>
                    </select><br />
         <?php echo "more information" ?><br />
