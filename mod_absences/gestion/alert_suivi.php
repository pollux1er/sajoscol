...
/*
 *
 * $Id: alert_suivi.php 4878 2010-07-24 13:54:01Z regis $
 *
...
if ( $action_sql === 'supprimer_alert_groupe' and $valide_form === 'yes' ) {
	...
	// on vérifie s'il existe une alerte
		...
		} else { $message_erreur = 'this id not exist.'; }
...
if ( $action === 'modifier_alert_groupe' and $valide_form === 'yes' ) {
	...
	// on vérifie s'il n'existe pas une alerte de même nom
		...
		} else { $message_erreur = 'this id does not exist.'; }
}
...
if ( $action === 'modifier_alert_type' and $valide_form === 'yes' ) {
	...
	// on vérifie s'il n'existe pas une alerte de même nom
		...
		} else { $message_erreur = 'This id does not exit.'; }
}
...
<p class=bold><a href='gestion_absences.php?year=<?php echo $year; ?>&amp;month=<?php echo $month; ?>&amp;day=<?php echo $day; ?>'><img src="../../images/icons/back.png" alt="Retour" title="Retour" class="back_link" />&nbsp;Return</a> |
...
<div style="text-align: center; margin: auto; width: 95%;">
  ...
  <div style="border: 2px solid #BF0000; width: 25%; height: 150px; float: left; margin-top: 17px;">
	<div class="entete_alert"><b>Warning system</b></div>
	<div>
		<form name="form1" method="post" action="alert_suivi.php">
			Select group alert
				...
				{ ?>
        	          ...
        	          <?php } ?>
	               </select><br /><a href="alert_suivi.php?action_page=gestion_ag&amp;id_alert_groupe=<?php echo $id_alert_groupe; ?>&amp;action=editer_alert_groupe&amp;uid_post=<?php echo my_ereg_replace(' ','%20',$uid); ?>">manage groups Alert</a><br /><br />
...
<?php if ( $action_page === '' or $action_page === 'alert' ) { ?>
  <div style="margin-left: 25%; width: 74%;">
	<div class="entete_alert_message">ALERT IN PROGRESS</div>
	<div class="scroll" style="background-color: #EFEFEF; border-left: 4px solid #BF0000;">
		<table style="width: 98%; margin: 5px;" cellspacing="1" cellpadding="0">
		  <tr class="entete_alert_message">
			<td style="padding: 2px; text-align: center; font-weight: bold; color: #E8F1F4;">student</td>
			<td style="width: 50px; padding: 2px; text-align: center; font-weight: bold; color: #E8F1F4;">state</td>
		  </tr>
		  ...
		  <tr class="<?php echo $couleur_cellule; ?>">
			...
			<td style="width: 200px; padding: 2px;">
				<?php if ( $donnee_alert_eleve['etat_alert_eleve'] === '0' ) { ?>Uncategorized<?php } ?>
				<?php if ( $donnee_alert_eleve['etat_alert_eleve'] === '1' ) { ?>student convocation<?php } ?>
				<?php if ( $donnee_alert_eleve['etat_alert_eleve'] === '2' ) { ?>declassed<?php } ?>
			</td>
		  </tr>
		  <?php $i_cpt = $i_cpt + 1; } ?>
		</table>
		...
		<a name="ea"></a>
          ...
          ?><img src="<?php echo $photos; ?>" style="width: <?php echo $valeur[0]; ?>px; height: <?php echo $valeur[1]; ?>px; border: 0px" alt="" title="" /><?php
			...
			?>
				...
				<div style="border-bottom: 2px solid #1B2BDF; width: 70%"><?php echo 'Born at  '.$naissance_eleve.' - old (e) '.age($date_de_naissance).'Year<br />class : <strong>'.$classe_eleve.'</strong>'; ?></div><br />
				<div style="margin-left: 10px; padding-left: 10px;"><strong>officials :</strong><br />
					<?php
					$cpt_responsable = 0;
					while ( !empty($responsable_eleve[$cpt_responsable]) )
					{
						...
						$telephone = '';
							...
							if ( !empty($responsable_eleve[$cpt_responsable]['tel_port']) ) { $telephone = $telephone.'Port. '.$responsable_eleve[$cpt_responsable]['tel_port'].'<img src="../images/attention.png" alt="Warning premium rate number" title="Warning premium rate number" border="0" height="13" width="13" />'; }
						echo $telephone;
						$cpt_responsable = $cpt_responsable + 1;
					}
					?>
				</div><br />
				<div style="font-size: 150%; background: #555555; color: #FFFFFF;">filter</div>
				<div style="margin-left: 10px; padding-left: 10px;">
<?php

?>The alert that you selected contains the (s) filter (s) suiviant (s) :<br /><ul><?php
				$requete_alert_type = "SELECT * FROM ".$prefix_base."vs_alerts_groupes vsag, ".$prefix_base."vs_alerts_types vsat WHERE vsag.id_alert_groupe = '".$id_alert_groupe."' AND vsat.groupe_alert_type = vsag.id_alert_groupe";
	        	       	...
	        	       	while ( $donnee_alert_type = mysql_fetch_array ($resultat_alert_type))
				{
?><li>corresponds to a limit of <strong><?php echo $donnee_alert_type['nb_comptage_limit']; ?></strong> type <strong>
<?php
	$type_alert = '';
	...
	if ( $donnee_alert_type['type_alert_type'] === 'F' ) { $type_alert = 'Action on the student sheet'; }
	...
	if ( $donnee_alert_type['type_alert_type'] === 'P' )
	{
		...
		if ( $donnee_alert_type['specifisite_alert_type'] === 'R' ) { $spec_type_alert = 'late'; }
		if ( $donnee_alert_type['specifisite_alert_type'] === 'D' ) { $spec_type_alert = 'dispensation'; }
		if ( $donnee_alert_type['specifisite_alert_type'] === 'I' ) { $spec_type_alert = 'passage to the infirmary'; }
	}
...
<?php echo $type_alert.' ('.$spec_type_alert.')'; ?></strong> since at <strong><?php echo date_fr($donnee_alert_type['date_debut_comptage']); ?></strong>.</li><?php
				}
?></ul><br /><?php
...
*/ ?>
				...
				<div style="font-size: 150%; background: #555555; color: #FFFFFF;">State of Alert</div>
				<div style="margin-left: 10px; padding-left: 10px;">
					<div class="information_etat_ae">
						<?php if ( $etat_alert_eleve === '0' ) { ?>This alert is not selected for action now <?php } ?>
						<?php if ( $etat_alert_eleve === '1' ) { ?>The student was summoned by <?php echo qui($etatpar_alert_eleve); ?> for this alert<?php } ?>
						<?php if ( $etat_alert_eleve === '2' ) { echo who($etatpar_alert_eleve); ?> ignore this alert<?php } ?>
					</div>
					...
					<form name="form3" method="post" action="alert_suivi.php#ea">
				               	<input type="radio" name="etat_alert_eleve" id="eae1" value="1" onClick="javascript:document.form3.submit()" <?php  if ( $etat_alert_eleve === '1' ) { ?>checked="checked"<?php } ?> /><label for="eae1" style="cursor: pointer;">Convening of the student.</label><br />
				               	<input type="radio" name="etat_alert_eleve" id="eae2" value="2" onClick="javascript:document.form3.submit()" <?php  if ( $etat_alert_eleve === '2' ) { ?>checked="checked"<?php } ?> /><label for="eae2" style="cursor: pointer;">Ignore this alert.</label><br />
				               	<input type="radio" name="etat_alert_eleve" id="eae0" value="0" onClick="javascript:document.form3.submit()" <?php  if ( $etat_alert_eleve === '0' ) { ?>checked="checked"<?php } ?> /><label for="eae0" style="cursor: pointer;">No selection for now.</label><br />
...
<?php if ( $action_page === 'gestion_ag' ) { ?>
  <div style="margin-left: 25%; width: 74%;">
	<div class="entete_alert_message">Management groups Alert</div>
	<div class="scroll" style="background-color: #EFEFEF; border-left: 4px solid #BF0000;">
		<form name="form2" method="post" action="alert_suivi.php">
		<label for="nom_alert_groupe" style="cursor: pointer;">New</label> <input name="nom_alert_groupe" id="nom_alert_groupe" value="<?php if ( isset($nom_alert_groupe) and !empty($nom_alert_groupe) ) { echo $nom_alert_groupe; } ?>" style="border: 1px solid #B3BFB8;" />
		...
		<table style="width: 98%; margin: 5px;" cellspacing="1" cellpadding="0">
		  <tr class="entete_alert_message">
			<td style="padding: 2px; text-align: center; font-weight: bold; color: #E8F1F4;">Designation of the group alert</td>
	               	...
	               	while ( $donnee_alert_groupe = mysql_fetch_array ($resultat_alert_groupe))
			{
			  ...
			  <tr class="<?php echo $couleur_cellule; ?>">
				<td style="text-align: left; padding: 2px;"><a href="alert_suivi.php?action_page=<?php echo $action_page; ?>&amp;action=editer_alert_groupe&amp;id_alert_groupe=<?php echo $donnee_alert_groupe['id_alert_groupe']; ?>&amp;uid_post=<?php echo my_ereg_replace(' ','%20',$uid); ?>#eg" title="editer le groupe"><?php echo $donnee_alert_groupe['nom_alert_groupe']; ?></a></td>
			        ...
			        <td style="text-align: center;"><a href="alert_suivi.php?action_sql=supprimer_alert_groupe&amp;action_page=<?php echo $action_page; ?>&amp;id_alert_groupe=<?php echo $donnee_alert_groupe['id_alert_groupe']; ?>&amp;uid_post=<?php echo my_ereg_replace(' ','%20',$uid); ?>" onClick="return confirm('Warning this will delete all alerts for this student group...')"><img src="../images/x2.png" width="22" height="22" title="Supprimer" border="0" alt="" /></a></td>
			  </tr>
		<?php } ?>
		</table>
	</div>
	...
	<?php if ( $editer_ce_groupe != 'non' ) { ?>
		<form name="form3" method="post" action="alert_suivi.php#eg">
		       Type
	               <select name="type_alert_type" id="type_alert_type" tabindex="1" style="border : 1px solid #000000; margin-top: 5px;">
	                          <option value="P" <?php if ( !empty($type_alert_type) and $type_alert_type === 'P' ) { ?>selected="selected"<?php } ?> onclick="twAfficheCache('monForm-A',1);twAfficheCache('monForm-B',0);twAfficheCache('monForm-C',0);">Presence</option>
	                          <option value="C" <?php if ( !empty($type_alert_type) and $type_alert_type === 'C' ) { ?>selected="selected"<?php } ?> onclick="twAfficheCache('monForm-B',1);twAfficheCache('monForm-A',0);twAfficheCache('monForm-C',0);">mail</option>
	                          <option value="F" <?php if ( !empty($type_alert_type) and $type_alert_type === 'F' ) { ?>selected="selected"<?php } ?> onclick="twAfficheCache('monForm-B',0);twAfficheCache('monForm-A',0);twAfficheCache('monForm-C',1);">student sheet</option>
	               </select>
		       <span id="monForm-A" style="position:absolute;visibility:hidden;">
	               <select name="specifisite_alert_type_p" id="specifisite_alert_type_p" tabindex="1" style="border: 1px solid #000000; margin-top: 5px;">
	                          ...
	                          <option value="R" <?php if ( !empty($specifisite_alert_type_p) and $specifisite_alert_type_p === 'R' ) { ?>selected="selected"<?php } ?>>late</option>
	                          <option value="D" <?php if ( !empty($specifisite_alert_type_p) and $specifisite_alert_type_p === 'D' ) { ?>selected="selected"<?php } ?>>exemptions</option>
	                          <option value="I" <?php if ( !empty($specifisite_alert_type_p) and $specifisite_alert_type_p === 'I' ) { ?>selected="selected"<?php } ?>>infirmary</option>
	               </select></span>
		...
		<?php } else { echo '* Changing the group impossible. There are alerts to students identified.'; } ?>
		<table style="width: 98%; margin: 5px;" cellspacing="1" cellpadding="0">
		  <tr class="entete_alert_message">
			...
			<td style="padding: 2px; text-align: center; font-weight: bold; color: #E8F1F4;">since the</td>
			<td style="padding: 2px; text-align: center; font-weight: bold; color: #E8F1F4;">After</td>
		      <?php if ( $editer_ce_groupe != 'non' ) { ?>
			<td style="width: 25px; padding: 2px; text-align: center; font-weight: bold; color: #E8F1F4;"></td>
			<td style="width: 25px; padding: 2px; text-align: center; font-weight: bold; color: #E8F1F4;"></td>
		      <?php } ?>
		  ...
		  <?php
	               	...
	               	while ( $donnee_alert_type = mysql_fetch_array ($resultat_alert_type))
			{
			  ...
			  <tr class="<?php echo $couleur_cellule; ?>">
				...
				<td style="text-align: center; padding: 2px;"><?php echo $donnee_alert_type['nb_comptage_limit']; ?> time</td>
			      <?php if ( $editer_ce_groupe != 'non' ) { ?>
