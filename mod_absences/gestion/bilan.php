				...
				if ($test != "0")
 				{
					...
					if($nombre_de_responsable != 0)
					{
						...
						$civilite_responsable[$nb][0] = 'No charge selected';
...
$pdf->AddPage();
	...
	elseif ($Jour_semaine==6) {$jour='Sarturday';}
	...
	if ($aujourdhui[1]==8) { $aujourdhui[1]="August "; }
	...
	$pdf->SetFont('Arial','',12);
	$pdf->Text(20, 70,'Madam, Sir,');
$ident = "here is steady of student  ".$nom_eleve[$i]." ".$prenom_eleve[$i]." class ".$division[$i].",";
$pdf->Text(20, 80, traite_accents_utf8($ident));
$pdf->Text(20, 85,traite_accents_utf8('during period'.date_frl(date_sql($du))." at ".date_frl(date_sql($au))."."));
//tableau
$pdf->SetX(30);
$pdf->SetY(90);
        $pdf->SetFont('Arial','',9.5);
        $pdf->Cell(55, 5, 'The absent', 0, 1, '', '');
        $pdf->Cell(55, 5, 'of', 1, 0, '', '');
        $pdf->Cell(55, 5, 'at', 1, 0, '', '');
        $pdf->Cell(22, 5, 'reason', 1, 0, 'C', '');
        $pdf->Cell(54, 5, traite_accents_utf8('reason specify'), 1, 1, 'C', '');
		...
		while ( $data_1 = mysql_fetch_array($execution_1))
      	...
      	//tableau des absences
            $pdf->SetFont('Arial','',9.5);
            $debut = date_frc($data_1['d_date_absence_eleve'])." at ".heure($data_1['d_heure_absence_eleve']);
            $pdf->Cell(55, 5, traite_accents_utf8($debut), 0, 0, '', '');
            $fin = date_frc($data_1['a_date_absence_eleve'])." at ".heure($data_1['a_heure_absence_eleve']);
            $pdf->Cell(55, 5, traite_accents_utf8($fin), 0, 0, '', '');
            if ($data_1['type_absence_eleve'] == 'A') {$pour = "Absence"; }
            if ($data_1['type_absence_eleve'] == 'R') {$pour = "late"; }
        ...
        $pdf->Cell(54, 5, '', 0, 1, 'C', '');
        $pdf->Cell(55, 5, 'the delay', 0, 1, '', '');
        $pdf->Cell(55, 5, 'the', 1, 0, '', '');
        $pdf->Cell(131, 5, traite_accents_utf8('reason specify'), 1, 1, 'C', '');
		...
		while ( $data_2 = mysql_fetch_array($execution_2))
        {
       		//tableau des retards
            $pdf->SetFont('Arial','',9.5);
            $debut = date_frc($data_2['d_date_absence_eleve'])." at ".heure($data_2['d_heure_absence_eleve']);
            $pdf->Cell(55, 5, traite_accents_utf8($debut), 0, 0, '', '');
        ...
        $pdf->Cell(54, 5, '', 0, 1, 'C', '');
        $pdf->Cell(55, 5, 'the exemption', 0, 1, '', '');
        $pdf->Cell(55, 5, 'of', 1, 0, '', '');
        $pdf->Cell(55, 5, 'at', 1, 0, '', '');
        $pdf->Cell(76, 5, traite_accents_utf8('reason specify'), 1, 1, 'C', '');
...
$pdf->SetY(250);
if(substr($civilite_cpe[$i],0,1) == "M" OR substr($civilite_cpe[$i],0,1) == "" ) { $nomine = 'the principal education councillor'; }
if(substr($civilite_cpe[$i],0,2) == "Mrs") { $nomine = 'the principal education councillor'; }
if(substr($civilite_cpe[$i],0,2) == "Miss") { $nomine = 'the principal education councillor'; }
