	...
	/* DEBUT - construction du tableau des données */
		...
		while($donnee = mysql_fetch_array($execution))
		{
			...
			if ( !empty($donnee['d_heure_absence_eleve']) )
			{
				...
				$tableau[$cpt]['debut'] .= " at ".heure($donnee['d_heure_absence_eleve']);
        	...
        	else
        	{
            	...
            	if ( !empty($donnee['a_heure_absence_eleve']) )
            	{
            		...
            		$tableau[$cpt]['fin'] .= " at ".heure($donnee['a_heure_absence_eleve']);
...
/* DEBUT - Génération du PDF */
	...
	$pdf->SetTitle('statement of general absent');
	$pdf->SetSubject('statement of general absent');
...
while ( $page < $nb_page )
{
	...
	$pdf->Cell(0, 6, 'statement of absent', 0, 1, 'C', '');

	$pdf->SetFont('Arial','',10);
	if ( $absencenj === '1' ) { $pdf->SetX(65); $pdf->Cell(0, 4, 'with option absent without justification', 0, 1, 'C', ''); }
	if ( $retardnj === '1' ) { $pdf->SetX(65); $pdf->Cell(0, 4, 'with option late without justification', 0, 1, 'C', ''); }

	$pdf->SetFont('Arial','',14);
	$duu = "of ".date_frl(date_sql($du));
	$auu = "at ".date_frl(date_sql($au));
    ...
    $pdf->Cell(55, 5, traite_accents_utf8('full name'), 1, 0, 'C', '');
    $pdf->Cell(17, 5, 'Class', 1, 0, 'C', '');
    $pdf->Cell(42, 5, 'Reason', 1, 0, 'C', '');
    $pdf->Cell(38, 5, 'of', 1, 0, 'C', '');
    $pdf->Cell(38, 5, 'at', 1, 1, 'C', '');
