...
if ( $etiquette_action === 'originaux' ) {
	...
	while ( $donne_persone = mysql_fetch_array( $call_eleve ))
	{
		...
		if ($sexe_eleve[$i] == "M") {
			$date_naissance[$i] = 'born at '.date_fr($donne_persone['naissance']);
		} else {
			$date_naissance[$i] = 'born at '.date_fr($donne_persone['naissance']);
		}
...
function TextWithRotation($x,$y,$txt,$txt_angle,$font_angle=0)
{
    ...
    function Header()
    {
	    global $prefix_base;
			...
			$texte_filigrame='TEST DOCUMENT'; // texte du filigrame
			$affiche_logo_etab='1';
    ...
    function Footer()
    {
   	...
   	$this->SetXY(5,-10);
        ...
        $this->Cell(0,4.5, traite_accents_utf8(" summary slip of absent - GEPI : free solution management and monitoring of school performance."),0,0,'C');
    }
}
...
// bloc affichage information sur l'élèves
	if ( $active_bloc_eleve === '1' ) {
	 ...
	 if ( $affiche_effectif_classe === '1' ) {
	  if ( $info_bulletin[$ident_eleve_aff][$id_periode]['effectif']!="") {
		$pdf->Cell(45,4.5, traite_accents_utf8('Number : '.$info_bulletin[$ident_eleve_aff][$id_periode]['effectif'].' student'),0,0,''); }
	 }
	 if ( $affiche_numero_impression === '1' ) {
	  $num_ordre = $cpt_eleve;
	  $pdf->Cell(45,4, 'Printing N° '.$num_ordre,0,2,'');
	 }
	}
	...
	if($active_bloc_datation==='1') {
	 ...
	 $pdf->SetFont($caractere_utilse,'',10);
	 $pdf->Cell(90,5, traite_accents_utf8(" recapitulate slip of absent"),0,2,'C');
...
// bloc d'information divers
	...
	$pdf->Cell($l_divers, $h_divers, traite_accents_utf8('Number of letter consigned : ').$cpt_lettre_envoye, 0, 2, 'L', 0);
	// nombre de lettre resté sans réponse
	...
	$pdf->Cell($l_divers, $h_divers, traite_accents_utf8('Letter remain without answer : ').$cpt_lettre_envoye_sans_reponse, 0, 2, 'L', 0);
	// nombre d'avertissement
	...
	$pdf->Cell($l_divers, $h_divers, 'Number of warning : '.$cpt_lettre_avertissement, 0, 2, 'L', 0);
	// nombre d'exclusion
	...
	$pdf->Cell($l_divers, $h_divers, 'Number of exclusion : '.$cpt_lettre_exclusion, 0, 2, 'L', 0);
	// nombre de retenue
	...
	$pdf->Cell($l_divers, $h_divers, 'Number of detention : '.$cpt_lettre_retenue, 0, 2, 'L', 0);
...
$annuel = $mois;
	...
	$pdf->SetFont('arial','',9);
	$pdf->SetXY($x_annuel, $y_annuel);
	$pdf->Cell($l_intituler, $h_intituler, traite_accents_utf8('Absent not justify'), 1, 0, 'L', 0);
		$i = '0'; $total = '0';
		while ( !empty($annuel[$i]) ) {
				$texte = '';
	...
	$y_annuel = $y_annuel + $h_annuel;
	$pdf->SetXY($x_annuel, $y_annuel);
	$pdf->Cell($l_intituler, $h_intituler, traite_accents_utf8('Absent justify'), 1, 0, 'L', 0);
		$i = '0'; $total = '0';
		while ( !empty($annuel[$i]) ) {
				$texte = '';
				$annee_texte = $annuel[$i]['num_annee'];
	...
	$pdf->SetFont('arial','',9);
	$pdf->SetXY($x_annuel, $y_annuel);
	$pdf->Cell($l_intituler, $h_annuel, traite_accents_utf8('late not justify'), 1, 0, 'L', 0);
		$i = '0'; $total = '0';
		while ( !empty($annuel[$i]) ) {
				$texte = '';
				$annee_texte = $annuel[$i]['num_annee'];
	...
	$y_annuel = $y_annuel + $h_annuel;
	$pdf->SetXY($x_annuel, $y_annuel);
	$pdf->Cell($l_intituler, $h_annuel, traite_accents_utf8('late justify'), 1, 0, 'L', 0);
		$i = '0'; $total = '0';
		while ( !empty($annuel[$i]) ) {
				$texte = '';
				$annee_texte = $annuel[$i]['num_annee'];
...
//$total_absence_heure = my_eregi_replace("[-]{1}",'',$total_absence_heure);
	...
	$pdf->Cell(0, $h_annuel, 'Total of absent : '.$total_absence_nb.', Total of absent within hour : '.convert_minutes_heures($total_absence_heure).', Total des retards : '.$total_retard, 0, 0, 'C', 0);
// fin du tableau annuel des absences et retards

// tableau des nombre d'absences par jour et par heure (période)
$i = '0';
if ( isset($semaine_horaire['lundi']['ouverture']) ) { $semaine[$i]['jour'] = 'Monday'; $i = $i + 1; }
if ( isset($semaine_horaire['mardi']['ouverture']) ) { $semaine[$i]['jour'] = 'Tuesday'; $i = $i + 1; }
if ( isset($semaine_horaire['mercredi']['ouverture']) ) { $semaine[$i]['jour'] = 'Wednesday'; $i = $i + 1; }
if ( isset($semaine_horaire['jeudi']['ouverture']) ) { $semaine[$i]['jour'] = 'Thursday'; $i = $i + 1; }
if ( isset($semaine_horaire['vendredi']['ouverture']) ) { $semaine[$i]['jour'] = 'Friday'; $i = $i + 1; }
if ( isset($semaine_horaire['samedi']['ouverture']) ) { $semaine[$i]['jour'] = 'Sarturday'; $i = $i + 1; }
	...
	// case vide
	$pdf->Cell($l_horaire, $h_semaine, 'ABSENT', 'RB', 0, 'C', 0);
		// entête des jours
		$i = '0';
...
$i = '0';
	...
	$pdf->SetXY($x_semaine, $y_semaine);
	// case vide
	$pdf->Cell($l_horaire, $h_semaine, 'LATE', 'RB', 0, 'C', 0);
		// entête des jours
		$i = '0';
		while ( !empty($semaine[$i]) ) {
			$pdf->SetFont('arial','',9);
