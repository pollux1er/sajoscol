...
//**************** EN-TETE *****************
$titre_page = "management of absent";
...
$cal = new Calendrier("form1", "of");
$cal_3 = new Calendrier("form3", "of");
$cal_4 = new Calendrier("form3", "to");
$cal_5 = new Calendrier("form5", "of");
$cal_6 = new Calendrier("form5", "au");
$cal_7 = new Calendrier("form6", "of");
   ...
   if (empty($_GET['choix']) and empty($_POST['choix'])) {$choix="not justify";}
   ...
   if (empty($_GET['classe']) and empty($_POST['classe'])) {$classe="all";}
...
if(!empty($lettre_type_nouv) and $action_choix_lettre === 'editer' and $valide_form === 'yes')
 {
	...
	} else { $erreur = ' this type of letter is already exist'; }
 }
...
if($action_choix_lettre === 'supprimer' and $valide_form === 'yes')
 {
	if(!empty($lettre_type))
	{
                ...
                if ($test_existance === '0')
		{
	       		...
	       		$req_sql2 = mysql_query($req_delete);
		} else { $erreur = 'Impossible to delected because is already exist sending of this type'; }
	}
 }
	...
	if(!empty($id))
	{
                ...
                if ($test_existance === '0')
		{
			$requete="UPDATE ".$prefix_base."lettres_suivis SET envoye_date_lettre_suivi = '', envoye_heure_lettre_suivi = '', quienvoi_lettre_suivi = '' WHERE id_lettre_suivi = '".$id."'";
		        $resultat = mysql_query($requete) or die('Erreur SQL !'.$requete.'<br />'.mysql_error());
		} else { $erreur = ' an answer exist for this mail, then you cannot reinitialize the send '; }
	}
 }
...
<!-- ENTETE PRINCIPAL -->
<p class=bold><a href='gestion_absences.php?type=<?php echo $type; ?>&amp;year=<?php echo $year; ?>&amp;month=<?php echo $month; ?>&amp;day=<?php echo $day; ?>'><img src="../../images/icons/back.png" alt="Retour" title="Retour" class="back_link" />&nbsp;Return</a>|
<a href="impression_absences.php?year=<?php echo $year; ?>&amp;month=<?php echo $month; ?>&amp;day=<?php echo $day; ?>">printing</a> |
<a href="statistiques.php?year=<?php echo $year; ?>&amp;month=<?php echo $month; ?>&amp;day=<?php echo $day; ?>">statistical</a> |
<a href="gestion_absences.php?choix=lemessager&amp;year=<?php echo $year; ?>&amp;month=<?php echo $month; ?>&amp;day=<?php echo $day; ?>">Messenger</a> |
<a href="alert_suivi.php?choix=alert&amp;year=<?php echo $year; ?>&amp;month=<?php echo $month; ?>&amp;day=<?php echo $day; ?>"> alert systeme </a>
</p>
<!-- ENTETE PROPOSANT LES CHOIX -->
<div class="norme_absence centre">
	[ <a href="impression_absences.php?type_impr=laf">Letter to family</a> |
	 <a href="impression_absences.php?type_impr=bda">statement of absence</a> |
	 <a href="impression_absences.php?type_impr=bpc">statement advice</a> |
	 <a href="impression_absences.php?type_impr=bj">statement day</a> |
	 <a href="impression_absences.php?type_impr=fic"> récapitulative slip</a> |
	 <a href="impression_absences.php?type_impr=eti">label</a> |
	 <a href='impression_absences.php?type_impr=crea_lettre'>management of creation of type of letter </a> ]
	 </div><br />
...
<div style="text-align: center;">
	...
	if ( $form_pdf_lettre === 'affiche' and $_POST['Submit3']==='Sélectionner pour envoi' )
	{
		...
		<form method="post" action="lettre_pdf.php" name="imprime_pdf_ok" target="_blank">
	  		<fieldset style="width: 90%; margin: auto; border: 1px solid;"><legend>you selection</legend>
				...
				a click on this bouton print all letter select and pass their state at send
	  		 	<center>
				<input type="submit" id="valider_pdf" name="creer_pdf" value="generate the file  PDF" />
				</center>
			</fieldset>
		</form>
	...
	if ( $form_pdf_lettre === 'affiche' and $_POST['Submit3']==='remove the letter select' and $_POST['id_lettre_suivi'][0]!='')
	{
	$rq_efface_courriers='';
	...
	if($succes_efface_courriers==1){echo('<table class="table_erreur" border="0"><tr><td class="erreur">the letter are remove !</td></tr></table>');}
	}
	...
	if ( $form_pdf_lettre === 'affiche' and $_POST['Submit3']==='assemble the mails' and $_POST['id_lettre_suivi'][0]!='')
	{
		...
		echo('<table class="table_erreur" border="0"><tr><td class="erreur">the mail select are merge!</td></tr></table>');
	}
	...
	if ( $form_pdf_lettre === 'affiche' and $_POST['Submit3']==='Reinitialize send of mail' and $_POST['id_lettre_suivi'][0]!='')
	{
	...
	if($succes_repasse_courriers_attente==1){echo('<table class="table_erreur" border="0"><tr><td class="erreur">The letter are reinitialize !</td></tr></table>');}
	}
	...
	if ( $form_pdf_lettre === 'affiche' and $_POST['Submit3']==='consider the answer as received ' and $_POST['id_lettre_suivi'][0]!='')
	{
	$rq_reponse_recue='';
	...
	if($succes_reponse_recue==1){echo('<table class="table_erreur" border="0"><tr><td class="erreur">the letter select was received !</td></tr></table>');}
	}
  ...
  <form method="post" action="impression_absences.php?type_impr=<?php echo $type_impr; ?>" name="form1">
      <fieldset style="width: 460px; margin: auto; padding: 4px" class="couleur_ligne_3">
         <legend class="legend_texte">&nbsp;Selection&nbsp;</legend>
            <div class="titre_tableau_gestion">Letter of familly</div>
...
<option value="1" <?php if(empty($choix) or (!empty($choix) and $choix === '1')) { ?>selected="selected"<?php } ?>>all mail</option>
<option value="2" <?php if(!empty($choix) and $choix === '2') { ?>selected="selected"<?php } ?>>mail dispatch</option>
<option value="3" <?php if(!empty($choix) and $choix === '3') { ?>selected="selected"<?php } ?>>mail not dispatch</option>
<option value="4" <?php if(!empty($choix) and $choix === '4') { ?>selected="selected"<?php } ?>>mail without answer</option>
<option value="5" <?php if(!empty($choix) and $choix === '5') { ?>selected="selected"<?php } ?>>mail with answer</option>
</select><br />
<select name="action_lettre" style="width: 90px; border: 1px solid #000000; cursor: pointer;">
<option value="1" <?php if(empty($action_lettre) or (!empty($action_lettre) and $action_lettre === '1')) { ?>selected="selected"<?php } ?>>&eacute;put at</option>
<option value="2" <?php if(!empty($action_lettre) and $action_lettre === '2') { ?>selected="selected"<?php } ?>>poster at</option>
<option value="3" <?php if(!empty($action_lettre) and $action_lettre === '3') { ?>selected="selected"<?php } ?>>answer received at</option>
<option value="4" <?php if(!empty($action_lettre) and $action_lettre === '4') { ?>selected="selected"<?php } ?>>issued since</option>
<option value="5" <?php if(!empty($action_lettre) and $action_lettre === '5') { ?>selected="selected"<?php } ?>>not consider the date</option>
</select>
   ...
   <form method="post" action="impression_absences.php?type_impr=<?php echo $type_impr; ?>&amp;lettre_action=originaux" name="form2">
	...
	<table style="margin: auto; text-align: left; width: 750px; border: 2px solid #000000; background-color: #FFFFFF;" cellpadding="0" cellspacing="1">	  <tbody>
	    <tr class="fond_vert">
	      ...
	      <td align="center" nowrap="nowrap" valign="middle" class="norme_absence_blanc" style="width: 280px; font-weight:bold;">person concern</td>
	      <td align="center" nowrap="nowrap" valign="middle" class="norme_absence_blanc" style="width: 120px; font-weight:bold;">type of mail</td>
	      <td align="center" nowrap="nowrap" valign="middle" class="norme_absence_blanc" style="width: 90px; font-weight:bold;">issued</td>
	      <td align="center" nowrap="nowrap" valign="middle" class="norme_absence_blanc" style="width: 90px; font-weight:bold;">send</td>
	      <td align="center" nowrap="nowrap" valign="middle" class="norme_absence_blanc" style="width: 90px; font-weight:bold;">answer</td>
	...
	while ( $donner_liste_courrier = mysql_fetch_array ($resultat_liste_courrier))
	{
		...
		<tr id="tra<?php echo $i; ?>" class="<?php echo $couleur_cellule; ?>" onmouseover="document.getElementById('tr<?php echo $i; ?>').className='td_tableau_sel'; document.getElementById('tra<?php echo $i; ?>').className='td_tableau_sel';" onmouseout="document.getElementById('tr<?php echo $i; ?>').className='<?php echo $couleur_cellule; ?>'; document.getElementById('tra<?php echo $i; ?>').className='<?php echo $couleur_cellule; ?>';">
	      	...
	      	<td align="center" nowrap="nowrap" valign="middle" style="text-align: left;" colspan="7" >Status
				<select name="statu_lettre[0]" style="width: 130px; border: 1px solid #000000;">
					<option value="envoyer" <?php if($donner_liste_courrier['statu_lettre_suivi'] === 'envoyer') { ?>selected="selected"<?php } ?>>send</option>
					<option value="en attente" <?php if($donner_liste_courrier['statu_lettre_suivi'] === 'en attente') { ?>selected="selected"<?php } ?>>wait</option>
					<option value="recus" <?php if($donner_liste_courrier['statu_lettre_suivi'] === 'recus') { ?>selected="selected"<?php } ?>>answer received</option>
					<option value="annuler" <?php if($donner_liste_courrier['statu_lettre_suivi'] === 'annuler') { ?>selected="selected"<?php } ?>>mail cancel</option>
				</select>
				remark
		...
		<tr class="fond_vert">
			<td colspan="7" class="norme_absence_blanc"><?php if($nombre_d_entre!='' and $nombre_d_entre!='0') { ?>Number of letter post <strong><?php echo $nombre_d_entre.'</strong>'; } else { ?>not selection<?php } ?></td>
	    </tr>
        <tr>
            <td colspan="2" class="norme_absence">
				<?php $varcoche = $varcoche."'form2'"; ?>
				<a href="javascript:CocheCheckbox(<?php echo $varcoche; ?>)">tick</a> | <a href="javascript:DecocheCheckbox(<?php echo $varcoche; ?>)">shoot</a>
			</td>
		...
		<input type="submit" name="Submit3" value="Effacer les lettres selectionnées"  onClick="return confirm('are you sure to be delete mail tick ?')" />
                <br>
		<input type="submit" name="Submit3" value="Assembler les courriers" onClick="return confirm(' if many mail are waiting for a student, they were assemble at alone mail who ask justify for all absences. Are you sure to be proceed at this changement ?')" />
		<br>
		<input type="submit" name="Submit3" value="Réinitialiser les envois de courriers" onClick="return confirm('the status of mail tick must become in wait and can be redirect. are you sure to want proceed at this changement ?')"/>
		<br>
		<input type="submit" name="Submit3" onClick="return confirm('this are create a PDF file of mail  for parent. this mail change went the status and the date of send were record. are you sure to want send the mail ?')"  value="Sélectionner pour envoi" />
		<br>
		<input type="submit" name="Submit3" value="Consider the answer as received " onClick="return confirm('the mail who are send are pass at RECEIVE ANSWER. Are you sure to want proceed at this changement ?')"/>		
            </td>
        </tr>
	</tbody>
</table>
...
<div style="text-align: center;">
   <form method="post" action="impression_absences.php?type_impr=<?php echo $type_impr; ?>" name="form3">
      <fieldset style="width: 450px; margin: auto;" class="couleur_ligne_3">
         <legend class="legend_texte">&nbsp;Selection&nbsp;</legend>
            <div class="titre_tableau_gestion"> General statement absence</div>
            ...
            Class
                ...
                <br />
                of <input name="du" type="text" size="11" maxlength="11" value="<?php echo $du; ?>" /><a href="#calend" onClick="<?php  echo $cal_3->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a> to <input name="au" id="au" type="text" size="11" maxlength="11" value="<?php echo $au; ?>" onClick="getDate(au,'form3')" /><a href="#calend" onClick="<?php  echo $cal_4->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a>
		<br /><a href="#ao" onclick="affichercacher('div_1')" style="cursor: pointer;"><img style="border: 0px solid ; width: 13px; height: 13px; border: none; padding:2px; margin:2px; float: left;" name="img_1" alt="" title="Information" src="../../images/fleche_na.gif" align="middle" />other option</a>
		<div id="div_1" style="display: <?php if( $absencenj != '' or $retardnj != '' ) { ?>block<?php } else { ?>none<?php } ?>; border-top: solid 1px; border-bottom: solid 1px; padding: 10px; background-color: #E0EEEF"><a name="ao"></a>
		  <span style="font-family: Arial;">
			<input name="absencenj" id="absencenj" value="1" type="checkbox" onclick="activedesactive('retardnj','absencenj');" <?php if ( $absencenj === '1' ) { ?>checked="checked"<?php } ?> /><label for="absencenj" style="cursor: pointer;">List only absence not justify </label>
		  	<br /><input name="retardnj" id="retardnj" value="1" type="checkbox" onclick="activedesactive('absencenj','retardnj');" <?php if ( $retardnj === '1' ) { ?>checked="checked"<?php } ?> /><label for="retardnj" style="cursor: pointer;">List only  delay not justify </label>
		  </span>
		</div>
...
<div style="text-align: center;">
   <form method="post" action="impression_absences.php?type_impr=<?php echo $type_impr; ?>" name="form5">
      <fieldset style="width: 450px; margin: auto;" class="couleur_ligne_3">
        <legend style="clear: both" class="legend_texte">&nbsp;Selection&nbsp;</legend>
            <div class="titre_tableau_gestion">statement of absence for class advice</div>
            <div class="norme_absence" style="text-align: center;">of <input name="du" type="text" size="11" maxlength="11" value="<?php echo $du; ?>" /><a href="#calend" onClick="<?php  echo $cal_5->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a> ato <input name="au" id="au" type="text" size="11" maxlength="11" value="<?php echo $au; ?>" onClick="getDate(au,'form5')" /><a href="#calend" onClick="<?php  echo $cal_6->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a><input type="submit" name="Submit2" value="&gt;&gt;" /></div>
      </fieldset>
   </form>
   <br />
       ...
       <form method="post" action="bilan.php?type_impr=<?php echo $type_impr; ?>&amp;choix=<?php echo $choix; ?>" name="form6">
        <table style="width: 600px; margin: auto;" border="0" cellpadding="0" cellspacing="1">
          <tr class="fond_vert">
            <td class="norme_absence_blanc" style="width: 80px"><strong>to printing</strong></td>
            <td class="norme_absence_blanc"><strong>Class</strong></td>
            <td class="norme_absence_blanc"><strong>sender</strong></td>
          </tr>
        <?php
           $ic = '1';
           $i = '0';
...
<div style="text-align: center;">
   <form method="post" action="bilan_absences_quotidien_pdf.php" name="form6">
      <fieldset style="width: 450px; margin: auto;" class="couleur_ligne_3">
         <legend class="legend_texte">&nbsp;Selection&nbsp;</legend>
            <div class="titre_tableau_gestion"> General statement day </div>
            <div class="norme_absence" style="text-align: left;">
            <br />
            of <input name="du" type="text" size="11" maxlength="11" value="<?php echo $du; ?>" /><a href="#calend" onClick="<?php  echo $cal_7->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a>
			&nbsp;
...
<div style="text-align: center;">
   <form method="post" action="impression_absences.php?type_impr=<?php echo $type_impr; ?>" name="form3">
      <fieldset style="width: 450px; margin: auto;" class="couleur_ligne_3">
         <legend class="legend_texte">&nbsp;Selection&nbsp;</legend>
            <div class="titre_tableau_gestion">recapitulative file </div>
            <div class="norme_absence" style="text-align: center;">
		<table style="border: 0px; text-align: center; width: 100%;"><tr><td>
		  ...
		  <optgroup label="----- Listes des &eacute;l&egrave;ves -----">
			...
			?>
		     <?php if(empty($classe_multiple[0]) and empty($eleve_multiple[0])) { ?><option value="" disabled="disabled">empty</option><?php } ?>
		  </optgroup>
		  </select></td></tr></table>
                of <input name="du" type="text" size="11" maxlength="11" value="<?php echo $du; ?>" />
		<a href="#calend" onClick="<?php  echo $cal_3->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>">
		<img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a> to <input name="au" id="au" type="text" size="11" maxlength="11" value="<?php echo $au; ?>" onClick="getDate(au,'form3')" />
...
<div style="text-align: center;">
   <form method="post" action="etiquette_pdf.php?type_impr=<?php echo $type_impr; ?>&amp;choix=<?php echo $choix; ?>&amp;etiquette_action=originaux" name="form3">
      <fieldset style="width: 450px; margin: auto;" class="couleur_ligne_3">
         <legend class="legend_texte">&nbsp;Selection&nbsp;</legend>
            <div class="titre_tableau_gestion">printing label</div>
            <div class="norme_absence" style="text-align: left;">
            Class
                <select name="classe">
                    <option value="tous">All</option>
                ...
                <select name="trie_par">
                    <option value="1">Class, name, first name</option>
                    <option value="2">name, first name</option>
                </select><br />
		Type of label
                <select name="etiquette_type">
                    <option value="1">NAME first name, Class</option>
                    <option value="2">NAME first name, Class, Student number </option>
                    <option value="3">NAME first name, Student number</option>
                    <option value="4">Principal address </option>
                    <option value="5">Principal adress  + student</option>
                </select><br />
		<a href="impression_absences.php?type_impr=eti&amp;etiquette_aff=<?php if($etiquette_aff === 'bibliotheque') { ?><?php } else { ?>bibliotheque<?php } ?>#crea_etiquette">Format label </a>
	...
	<form method="post" action="impression_absences.php?type_impr=<?php echo $type_impr; ?>&amp;action_etiquette=bibliotheque" name="form10">
	    ...
	    <tr class="fond_vert">
	     <td class="titre_tableau_gestion" colspan="3">library format label</td>
	    </tr>
            <tr class="fond_vert">
	      <td style="width: 17px;"></td>
	      <td style="width: 17px;"></td>
              <td class="norme_absence_blanc"><strong>label</strong> - New <input name="nom_etiquette_format" value="" style="border: 1px solid #B3BFB8;" /><input type="hidden" name="type_impr" value="<?php echo $type_impr; ?>" /><input type="hidden" name="etiquette_aff" value="<?php echo $etiquette_aff; ?>" /><input type="hidden" name="uid_post" value="<?php echo my_ereg_replace(' ','%20',$uid); ?>" /><input type="submit" name="Submit10" value="Créer" /></td>
            </tr>
	  <?php
           $ic = '1';  $i = '0';
...
<div style="text-align: center;">
      ...
      <fieldset style="width: 450px; margin: auto;" class="couleur_ligne_3">
        <legend style="clear: both" class="legend_texte">&nbsp;Selection&nbsp;</legend>
            <div class="titre_tableau_gestion"> management of type letter </div>
            <div class="norme_absence" style="text-align: center;">
   ...
   <?php // afficher les informations sur la lettre
	if($type_impr === 'crea_lettre' and !empty($lettre_type) and $action_choix_lettre != 'supprimer' and $action_choix_lettre != 'modifier') { ?>
     <br />
[ <a href='../gestion/impression_absences.php?type_impr=crea_lettre&amp;lettre_type=<?php echo $lettre_type; ?>'>Contents of letter tick </a> | <a href='../gestion/impression_absences.php?type_impr=crea_lettre&amp;lettre_type=<?php echo $lettre_type; ?>&amp;sous_rubrique=gb'>management of frame library </a> ]
<br />
<br />
...
<div style="float: right; margin-right: 20px; border: 1px solid #000000; background-color: #C2CFC5; height: 150px; width: 210px; background-image: url('../images/1.png'); background-repeat: repeat-x;">
      <form method="post" action="impression_absences.php?type_impr=<?php echo $type_impr; ?>#crea_lettre" name="form7">
	    <strong style="color: #FFFFFF;">frame available</strong><br />
     ...
     <table style="width: 550px; margin: auto; border: 1px solid #000000;" cellpadding="0" cellspacing="0">
	  <tr class="fond_vert">
	   <td class="titre_tableau_gestion" colspan="8">contents of letter - <?php echo titre_lettre_type($lettre_type) ?></td>
	  </tr>
          <tr class="fond_vert">
	    <td style="width: 17px;"></td>
	    <td style="width: 17px;"></td>
            <td class="norme_absence_blanc"><strong>Name of frame</strong></td>
            <td class="norme_absence_blanc"><strong>Pos. X</strong></td>
            <td class="norme_absence_blanc"><strong>Pos. Y</strong></td>
            <td class="norme_absence_blanc"><strong>Width</strong></td>
            <td class="norme_absence_blanc"><strong>Height</strong></td>
            <td class="norme_absence_blanc"><strong>rule box</strong></td>
          </tr>
	  <?php
           $ic = '1';  $i = '0';
           ...
           while ( $data_1 = mysql_fetch_array($execution_1))
			...
			<input type="image" src="../../images/enabled.png" style="border: 0px; width: 15px; height: 15px;" title="Valider les informations" alt="Valider les informations" /><?php } else { if($data_1['encadre_lettre_tc'] === '1') { ?>yes<?php } else { ?>no<?php }; } ?></td>
                  </tr>
         <?php $i = $i + 1; } ?>
	</table>
	</form>
<a href="lettre_pdf.php?lettre_type=<?php echo $lettre_type; ?>&amp;mode=apercus">Start glance</a>
...
if($sous_rubrique === 'gb') { ?>
     ...
     <table style="width: 650px; margin: auto; border: 1px solid #000000;" cellpadding="0" cellspacing="0">
	  <tr class="fond_vert">
	   <td class="titre_tableau_gestion" colspan="3">library frame</td>
	  </tr>
          <tr class="fond_vert">
	    <td style="width: 17px;"></td>
	    <td style="width: 17px;"></td>
            <td class="norme_absence_blanc"><strong>Name of frame </strong> - New <input name="nom_lettre_cadre" value="" style="border: 1px solid #B3BFB8;" /><input type="hidden" name="type_impr" value="crea_lettre" /><input type="hidden" name="lettre_type" value="<?php echo $lettre_type; ?>" /><input type="hidden" name="uid_post" value="<?php echo my_ereg_replace(' ','%20',$uid); ?>" /><input type="submit" name="Submit6" value="Créer" /></td>
          </tr>
	  <?php
           $ic = '1';  $i = '0';
           ...
           while ( $data_1 = mysql_fetch_array($execution_1))
               { if ($ic === '1') { $ic = '2'; $couleur_cellule = 'td_tableau_absence_1'; } else { $couleur_cellule = 'td_tableau_absence_2'; $ic = '1'; }
                ?>
                  <tr class="<?php echo $couleur_cellule; ?>">
	            <td style="text-align: center;"><a name="crea_lettre<?php echo $data_1['id_lettre_cadre']; ?>"></a><a href="impression_absences.php?type_impr=crea_lettre&amp;lettre_type=<?php echo $lettre_type; ?>&amp;id=<?php echo $data_1['id_lettre_cadre']; ?>&amp;uid_post=<?php echo my_ereg_replace(' ','%20',$uid); ?>&amp;action_cadre=aff_modifier_cadre&amp;sous_rubrique=gb#crea_lettre<?php echo $data_1['id_lettre_cadre']; ?>"><img src="../../images/edit16.png" title="modify frame" border="0" alt="" /></a></td>
	            <td style="text-align: center;"><a href="impression_absences.php?type_impr=crea_lettre&amp;lettre_type=<?php echo $lettre_type; ?>&amp;id=<?php echo $data_1['id_lettre_cadre']; ?>&amp;uid_post=<?php echo my_ereg_replace(' ','%20',$uid); ?>&amp;action_cadre=supprimer_cadre&amp;sous_rubrique=gb#crea_lettre" onClick="return confirm(' are you sure to want delete...')"><img src="../../images/delete16.png" title=" delete frame" border="0" alt="" /></a></td>
			...
			Encadrement <select name="encadre_lettre_cadre" style="border: 1px solid #B3BFB8;"><option value="0" <?php if($data_1['encadre_lettre_cadre'] === '0') { ?>selected="selected"<?php } ?>>no</option><option value="1" <?php if($data_1['encadre_lettre_cadre'] === '1') { ?>selected="selected"<?php } ?>>yes</option></select>
			colour of  back (
			...
			<label for="texte_lettre_cadre"><strong>Text of frame</strong></label><br />
	...
	<div style="text-align: center; float: right; margin-right: 20px; border: 1px solid #000000; background-color: #C2CFC5; height: 167px; width: 210px; background-image: url('../images/1.png'); background-repeat: repeat-x;">
	    <strong style="color: #FFFFFF;">flied available</strong><br />
		...
		<optgroup label="Style">
		   <option value="[g][/g]">Fat</option><?php echo "\n"; ?>
		   <option value="[i][/i]">Italics</option><?php echo "\n"; ?>
		   <option value="[u][/u]">Underline</option><?php echo "\n"; ?>
		   <option value="[t3][/t3]">Title</option><?php echo "\n"; ?>
		</optgroup>
		<optgroup label="Date">
		   <option value="[date_court]">short date (00/00/0000)</option><?php echo "\n"; ?>
		   <option value="[date_long]"> length date(Vendredi 00 0000)</option><?php echo "\n"; ?>
		</optgroup>
		<optgroup label="Courrier">
		   <option value="[courrier_demande_par]">mail ask by </option><?php echo "\n"; ?>
		   <option value="[remarque_eleve]">reason of mail </option><?php echo "\n"; ?>
		   <option value="[courrier_signe_par_fonction]">mail signed by fonct.</option><?php echo "\n"; ?>
		   <option value="[courrier_signe_par]">mail signed by</option><?php echo "\n"; ?>
		</optgroup>
		<optgroup label="Elève">
		   <option value="[nom_eleve]">Name</option><?php echo "\n"; ?>
		   <option value="[prenom_eleve]">first name</option><?php echo "\n"; ?>
		   <option value="[classe_eleve]">Class of</option><?php echo "\n"; ?>
		   <option value="[sujet_eleve]">subject of</option><?php echo "\n"; ?>
		   <option value="[remarque_eleve]">remark of</option><?php echo "\n"; ?>
		   <option value="[liste]">List abs. and delay.</option><?php echo "\n"; ?>
		   <option value="[liste_abs]">List of absence</option><?php echo "\n"; ?>
		   <option value="[liste_ret]">List of delay</option><?php echo "\n"; ?>
		</optgroup>
		<optgroup label="Responsable">
		   <option value="[civilite_court_responsable]">civility short</option><?php echo "\n"; ?>
		   <option value="[civilite_long_responsable]">civility length</option><?php echo "\n"; ?>
		   <option value="[nom_responsable]">Name</option><?php echo "\n"; ?>
		   <option value="[prenom_responsable]">first name</option><?php echo "\n"; ?>
		   <option value="[adresse_responsable]">Adress</option><?php echo "\n"; ?>
		   <option value="[adressecomp_responsable]">Adress line 2</option><?php echo "\n"; ?>
		   <option value="[adressecomp2_responsable]">Adress line 3</option><?php echo "\n"; ?>
		   <option value="[adressecomp3_responsable]">Adress line 4</option><?php echo "\n"; ?>
		   <option value="[cp_responsable]"> postal Code </option><?php echo "\n"; ?>
		   <option value="[ville_responsable]">town</option><?php echo "\n"; ?>
		</optgroup>
		<optgroup label="CPE en charge de l'élève">
		   <option value="[civilite_court_cpe]">civility short</option><?php echo "\n"; ?>
		   <option value="[civilite_long_cpe]">civility length</option><?php echo "\n"; ?>
		   <option value="[nom_cpe]">Name</option><?php echo "\n"; ?>
		   <option value="[prenom_cpe]">first name</option><?php echo "\n"; ?>
		</optgroup>
 	    </select><br />
