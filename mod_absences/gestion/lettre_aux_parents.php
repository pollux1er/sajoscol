              ...
              while($eleve_data = mysql_fetch_array($eleve_sql))
                {
                   ...
                   $civilite[$nb] = "";
                   if ($eleve_data['sexe']=="M") { $civilite[$nb]="M."; } elseif ($eleve_data['sexe']=="F") { $civilite[$nb]="Miss"; }
               ...
               if (isset($a_imprimer[$t_1]))
                {
                    ...
                    if ($test_responsable != 0)
                    {
                          ...
                          while($eleve_data = mysql_fetch_array($eleve_sql))
                            {
                               $civilite_responsable[$nb_1] = "Sir. and Madam";
                         ...
                         {
                               $civilite_responsable[$nb_1] = "no responsable exist ";
...
// champs facultatifs
$pdf->SetAuthor('');
$pdf->SetCreator('create with Fpdf');
$pdf->SetTitle('Title');
...
// date
$Jour_semaine=date("w");
if ($Jour_semaine==0) {$jour='sunday';}
elseif ($Jour_semaine==1) {$jour='Monday';}
elseif ($Jour_semaine==2) {$jour='Tuesday';}
elseif ($Jour_semaine==3) {$jour='Wednesday';}
elseif ($Jour_semaine==4) {$jour='Thursday';}
elseif ($Jour_semaine==5) {$jour='Friday';}
elseif ($Jour_semaine==6) {$jour='Saturday';}
$aujourdhui = date("d/m/Y");
$aujourdhui = explode('/', $aujourdhui);
if ($aujourdhui[1]==1) { $aujourdhui[1]="January"; }
if ($aujourdhui[1]==2) { $aujourdhui[1]="February"; }
if ($aujourdhui[1]==3) { $aujourdhui[1]="March"; }
if ($aujourdhui[1]==4) { $aujourdhui[1]="April"; }
if ($aujourdhui[1]==5) { $aujourdhui[1]="May"; }
if ($aujourdhui[1]==6) { $aujourdhui[1]="June"; }
if ($aujourdhui[1]==7) { $aujourdhui[1]="July"; }
if ($aujourdhui[1]==8) { $aujourdhui[1]="Ausgust"; }
if ($aujourdhui[1]==9) { $aujourdhui[1]="September"; }
if ($aujourdhui[1]==10) { $aujourdhui[1]="October"; }
if ($aujourdhui[1]==11) { $aujourdhui[1]="November"; }
if ($aujourdhui[1]==12) { $aujourdhui[1]="December"; }
$aujourdhui = $ville_etab.', the '.$jour.' '.$aujourdhui[0].' '.$aujourdhui[1].' '.$aujourdhui[2];
...
if ($choix=="rappel")
  {
    ...
    $pdf->Text(65, 50,' REMINDER ');
    $pdf->SetTextColor(0, 0, 0);
  }
$pdf->SetFont('Arial','B',18);
$pdf->Text(39, 75,'INFORMATION LETTER OF FAMILY );
$pdf->SetLineWidth(0,2);
$pdf->SetDrawColor(0, 0, 0);
$pdf->Rect(30, 67.5, 145, 10, 'D');
$pdf->SetFont('Arial','',12);
$pdf->Text(20, 90,'Madam, Sir,');
$ident = "our knowledge, student ".$nom_eleve[$i]." ".$prenom_eleve[$i]." class of ".$division[$i].",";
$pdf->Text(20, 100, traite_accents_utf8($ident));
$pdf->Text(20, 110,traite_accents_utf8(' not receiving follower course '));

//tableau
$pdf->SetX(30);
$pdf->SetY(120);
            $pdf->SetFont('Arial','',9.5);
            $pdf->Cell(55, 5, 'of', 1, 0, '', '');
            $pdf->Cell(55, 5, 'to', 1, 0, '', '');
            $pdf->Cell(22, 5, 'Type', 1, 0, 'C', '');
            $pdf->Cell(54, 5, 'thank you to indicate the reasons', 1, 1, 'C', '');
...
while ( $data_1 = mysql_fetch_array($execution_1))
      ...
      //tableau des absences
            ...
            if ($data_1['type_absence_eleve'] == 'R') {$pour = "Delay"; }
            if ($data_1['type_absence_eleve'] == 'D') {$pour = "exemption"; }
            if ($data_1['type_absence_eleve'] == 'I') {$pour = "Infirmary"; }
...
$pdf->SetX(30);
$pdf->SetY(210);
$pdf->Write( 5, traite_accents_utf8('I thank you for agreeing to make known the reason for his absence in best the delay in order to regularize his situation.If you already provided a document in proof, please not take account of this mail.'));
$pdf->SetY(230);
if(substr($civilite_cpe[$i],0,1) == "M" OR substr($civilite_cpe[$i],0,1) == "" ) { $nomine = '
The principal councillor of education '; }
if(substr($civilite_cpe[$i],0,2) == "Mm") { $nomine = 'The principal councillor of education'; }
if(substr($civilite_cpe[$i],0,2) == "Ml") { $nomine = 'The principal councillor of education'; }
...
$pdf->Cell(50, 5, 'SIGNATURE OF PRINCIPALE :', 0, 1, '', '');
$pdf->SetY(275);
//$pdf->Cell(0, 5, 'Number of reference : 154214', 0, 1, 'R', '');
$pdf->SetLineWidth(0,2);
$pdf->SetDrawColor(0, 0, 0);
