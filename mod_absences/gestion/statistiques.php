			...
			if( $doublon_journee != '')
			   {
				...
				{
				   $onprend = 'yes';
				   $eleve_passe_tab[$date_passe][$eleve_passe] = 'yes';
				} else { $onprend = 'not'; }
			   }
...
// Gestion du titre du graphique
	if(!empty($type[0])) {
		...
		while ( !empty($type[$i]) )
		{
			if( $type[$i] === 'A' ) { $type_selectionne_titre = $type_selectionne_titre."absences"; }
			if( $type[$i] === 'R' ) { $type_selectionne_titre = $type_selectionne_titre."delays"; }
			if( $type[$i] === 'D' ) { $type_selectionne_titre = $type_selectionne_titre."exemption"; }
			if( $type[$i] === 'I' ) { $type_selectionne_titre = $type_selectionne_titre."passage to the infirmary"; }
				if(!empty($type[$i+1]) and !empty($type[$i+2])) { $type_selectionne_titre = $type_selectionne_titre.", "; }
				if(!empty($type[$i+1]) and empty($type[$i+2])) { $type_selectionne_titre = $type_selectionne_titre." and "; }
		$i = $i + 1;
		}
	}
	...
	if ( $type_graphique === 'ligne' ) {
		if ( $echelle_y === 'D' ) {
			if($echelle_x === 'M') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." per month"; }
			if($echelle_x === 'J') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." by day"; }
			if($echelle_x === 'P') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." for hour (period)"; }
			if($echelle_x === 'C') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." by class"; }
			if($echelle_x === 'E') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." by student"; }
		}
		if ( $echelle_y === 'E' ) {
			if($echelle_x === 'M') { $donnee_titre[0] = "Total ".$type_selectionne_titre." by month"; }
			if($echelle_x === 'J') { $donnee_titre[0] = "Total ".$type_selectionne_titre." by day"; }
			if($echelle_x === 'P') { $donnee_titre[0] = "Total ".$type_selectionne_titre." for hour (period)"; }
			if($echelle_x === 'C') { $donnee_titre[0] = "Total ".$type_selectionne_titre." by class"; }
			if($echelle_x === 'E') { $donnee_titre[0] = "Total ".$type_selectionne_titre." by student"; }
		}
		if ( $echelle_y === 'H' ) {
			if($echelle_x === 'M') { $donnee_titre[0] = "Hour ".$type_selectionne_titre." by month"; }
			if($echelle_x === 'J') { $donnee_titre[0] = "Hour ".$type_selectionne_titre." by day"; }
			if($echelle_x === 'P') { $donnee_titre[0] = "Hour ".$type_selectionne_titre." for hour (period)"; }
			if($echelle_x === 'C') { $donnee_titre[0] = "Hour ".$type_selectionne_titre." by class"; }
			if($echelle_x === 'E') { $donnee_titre[0] = "Hour ".$type_selectionne_titre." by student"; }
		}
	}
	if ( $type_graphique === 'camembert' ) {
		if ( $echelle_y === 'D' ) {
			if($echelle_x === 'M') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." by month"; }
			if($echelle_x === 'J') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." by day"; }
			if($echelle_x === 'P') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." for hour (period)"; }
			if($echelle_x === 'C') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." by class"; }
			if($echelle_x === 'E') { $donnee_titre[0] = "Half-day ".$type_selectionne_titre." by student"; }
		...
		if ( $echelle_y === 'E' ) {
			if($echelle_x === 'M') { $donnee_titre[0] = "Percentage of the total ".$type_selectionne_titre." by month"; }
			if($echelle_x === 'J') { $donnee_titre[0] = "Percentage of the total ".$type_selectionne_titre." by day"; }
			if($echelle_x === 'P') { $donnee_titre[0] = "Percentage of the total ".$type_selectionne_titre." for hour (period)"; }
			if($echelle_x === 'C') { $donnee_titre[0] = "Percentage of the total ".$type_selectionne_titre." by class"; }
			if($echelle_x === 'E') { $donnee_titre[0] = "Percentage of the total ".$type_selectionne_titre." by student"; }
		}
		if ( $echelle_y === 'H' ) {
			if($echelle_x === 'M') { $donnee_titre[0] = "Percentage of hour ".$type_selectionne_titre." by month"; }
			if($echelle_x === 'J') { $donnee_titre[0] = "Percentage of hour ".$type_selectionne_titre." by day"; }
			if($echelle_x === 'P') { $donnee_titre[0] = "Percentage of hour ".$type_selectionne_titre." for hour (period)"; }
			if($echelle_x === 'C') { $donnee_titre[0] = "Percentage of hour ".$type_selectionne_titre." by class"; }
			if($echelle_x === 'E') { $donnee_titre[0] = "Percentage of hour ".$type_selectionne_titre." by student"; }
		}
	}
...
?>
<p class=bold><a href='gestion_absences.php?year=<?php echo $year; ?>&amp;month=<?php echo $month; ?>&amp;day=<?php echo $day; ?>'><img src="../../images/icons/back.png" alt="Retour" title="Return" class="back_link" />&nbsp;Return</a> |
...
<a href="alert_suivi.php?choix=alert&amp;year=<?php echo $year; ?>&amp;month=<?php echo $month; ?>&amp;day=<?php echo $day; ?>">Alarm system</a>
</p>
...
<?php /* DIV contenant le formulaire de recherche */ ?>
  <div style="border: 2px solid #D9EF1D; width: 210px; height: 360px; float: left; margin-top: 17px; text-align: center;">
	<div class="entete_stats"><b>Alarm system</b></div>
	<div>
		<form name="form1" method="post" action="statistiques.php">
	               <select name="type[]" id="type" multiple="multiple" size="3" tabindex="1" style="width: 200px; border : 1px solid #000000; margin-top: 5px;">
      			    <optgroup label="Les types">
		               <option value="A" <?php if(!empty($type) and in_array('A', $type)) { ?>selected="selected"<?php } ?>>Absences</option>
        		       <option value="R" <?php if(!empty($type) and in_array('R', $type)) { ?>selected="selected"<?php } ?>> delays</option>
                	       <option value="D" <?php if(!empty($type) and in_array('D', $type)) { ?>selected="selected"<?php } ?>>Exemptions</option>
	                       <option value="I" <?php if(!empty($type) and in_array('I', $type)) { ?>selected="selected"<?php } ?>>Passages to the infirmary</option>
			    </optgroup>
	               </select><br />

			<input name="justification" id="jus1" value="T" tabindex="8" type="radio" <?php if(!empty($justification) and $justification === 'T') { ?>checked="checked"<?php } ?> /><label for="jus1" title="Justified and not justified" style="cursor: pointer;">All</label>
			<input name="justification" id="jus2" value="O" tabindex="8" type="radio" <?php if(!empty($justification) and $justification === 'O') { ?>checked="checked"<?php } ?> /><label for="jus2" title="Justified" style="cursor: pointer;">Justi.</label>
			<input name="justification" id="jus3" value="N" tabindex="9" type="radio" <?php if(!empty($justification) and $justification === 'N') { ?>checked="checked"<?php } ?> /><label for="jus3" title="Not justified" style="cursor: pointer;">Not justi.</label><br />
			    ...
			    <optgroup label="Les classes">
          		        <option value="" <?php if($classe[0] === '') { ?>selected="selected"<?php } ?>>all</option>
                 		<?php
			...
			<input name="long_absence" id="lon1" value="1" tabindex="4" type="checkbox" <?php if(!empty($long_absence) and $long_absence === '1') { ?>checked="checked"<?php } ?> /><label for="lon1" title="To enter the long absences" style="cursor: pointer;">Long absences</label><br />
			<input name="doublon_journee" id="dou1" value="1" tabindex="5" type="checkbox" <?php if(!empty($doublon_journee) and $doublon_journee === '1') { ?>checked="checked"<?php } ?> /><label for="dou1" title="Comptabiliser un élève q'une fois par jour" style="cursor: pointer;">Count once</label><br />
			...
			to&nbsp;<input name="au" onfocus="javascript:this.select()" type="text" tabindex="5" value="<?php if (isset($au) and !empty($au)) { echo $au; } else { ?>JJ/MM/AAAA<?php } ?>" size="10" maxlength="10" style="border: 1px solid #000000;" /><a href="#calend" onClick="<?php echo $cal_2->get_strPopup('../../lib/calendrier/pop.calendrier.php', 350, 170); ?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a><br />
	               ...
	               <select name="echelle_x" id="echelle_x" size="1" tabindex="6" style="width: 200px; border : 1px solid #000000; margin-top: 5px;">
      			    <optgroup label="Les échelles (X)">
		               <option value="M" <?php if(!empty($echelle_x) and $echelle_x === 'M') { ?>selected="selected"<?php } ?>>By month</option>
        		       <option value="J" <?php if(!empty($echelle_x) and $echelle_x === 'J') { ?>selected="selected"<?php } ?>>By day</option>
                	       <?php /* <option value="P" <?php if(!empty($echelle_x) and $echelle_x === 'P') { ?>selected="selected"<?php } ?>>Par heure (période)</option> */ ?>
                	       <option value="C" <?php if(!empty($echelle_x) and $echelle_x === 'C') { ?>selected="selected"<?php } ?>>By class</option>
                	       <option value="E" <?php if(!empty($echelle_x) and $echelle_x === 'E') { ?>selected="selected"<?php } ?>>By student</option>
			    </optgroup>
	               </select><br />
	               ...
	               <select name="echelle_y" id="echelle_y" size="1" tabindex="7" style="width: 200px; border : 1px solid #000000; margin-top: 5px;">
      			    <optgroup label="Les échelles (Y)">
		               <option value="D" <?php if(!empty($echelle_y) and $echelle_y === 'D') { ?>selected="selected"<?php } ?>>Half-day</option>
		               <option value="E" <?php if(!empty($echelle_y) and $echelle_y === 'E') { ?>selected="selected"<?php } ?>>Number of recordings</option>
		               <option value="H" <?php if(!empty($echelle_y) and $echelle_y === 'H') { ?>selected="selected"<?php } ?>>Number of hours</option>
			    </optgroup>
	               </select><br />

			<input name="type_graphique" id="gra1" value="ligne" tabindex="8" type="radio" <?php if(!empty($type_graphique) and $type_graphique === 'ligne') { ?>checked="checked"<?php } ?> /><label for="gra1" style="cursor: pointer;">Line</label>
			<input name="type_graphique" id="gra2" value="camembert" tabindex="9" type="radio" <?php if(!empty($type_graphique) and $type_graphique === 'camembert') { ?>checked="checked"<?php } ?> /><label for="gra2" style="cursor: pointer;">Camembert cheese</label><br />
			<br />
  ...
  <div style="margin-left: 210px; width: 555px;">
	<div class="entete_stats_message">Graph of the statistics</div>
	...
	<?php } else { ?>No data corresponding to your seeking was found<?php } ?>
		</div>
