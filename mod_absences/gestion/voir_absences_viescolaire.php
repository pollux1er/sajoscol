...
//**************** EN-TETE *****************
$titre_page = "the absent of today.";
...
//debug_var();
	...
	if ($_SESSION["statut"] == "cpe") {
		if (isset($vers_absence)) {
			...
			$query_lettre = mysql_query($sql_lettre) OR DIE('Impossible to destroy the letter who is already  issued <br /> -->' . $sql_lettre . '<br /><p style="color: red;">' . mysql_error() . '</p>');
			...
			if ($test === 1) {
				...
				$message_erreur_lettre_a_effacer = 'we have the letter who correspond to this delay but not at all is destroy.';
			}
		}
	}
...
if (isset($choix_creneau)) {
	...
	$req = mysql_query($sql) OR trigger_error('Impossible to list absent.', E_USER_ERROR);
...
$periode_active = mysql_query($sql) OR DIE('Impossible to recover the number of active periode' . $sql . '<br />--> ' . mysql_error());
...
for($i = 0; $i < $nbre_rep; $i++) {
	...
	if ($rep_absences[$i]["eleve_id"] != "appel") {
		...
		if ($verif_aid[0] == "AID") {
			...
			// On construit alors l'affichage de cette info qui doit permettre à la vie scolaire de savoir
			// quand un prof a fait l'appel alors qu'il est avec un aid
			$aff_aid_absences .= "".$rep_prof["nom"]." ".$rep_prof["prenom"]." are make a call with group ".$rep_aid["nom"]."<br />";
	...
	if ($rep_absences[$i]["eleve_id"] == "appel") {
		...
		$etat = "<span style=\"color: brown; font-style: bold;\" onmouseover=\"changementDisplay('".$nom_prof.$i."', '');\" onmouseout=\"changementDisplay('".$nom_prof.$i."', '');\">
					call has made good in  ".$rep_matiere["description"].".</span>
					<div id=\"".$nom_prof.$i."\" class=\"abs_appear\" style=\"display: none;\">by ".$rep_prof["nom"]." ".$rep_prof["prenom"]."</div>";
		$modif = "";
		$modif_f = "";
	} else if ($rep_absences[$i]["retard_absence"] == "R") {
		$etat = " (retard)";
		$modif = "<a href=\"./voir_absences_viescolaire.php?vers_absence=".$rep_absences[$i]["id_abs"]."&amp;choix_creneau=".$choix_creneau."\" title=\"at delay\" style=\"color: green;\">";
		$modif_f = "</a>";
	} else {
		$etat = "";
		$modif = "<a href=\"./voir_absences_viescolaire.php?vers_retard=".$rep_absences[$i]["id_abs"]."&amp;choix_creneau=".$choix_creneau."\" title=\"Absent, by ".$rep_prof["nom"]." ".$rep_prof["prenom"]."\"><b>";
		$modif_f = "</b></a>";
	}
	...
	for($c = 0; $c < $nbre_classe; $c++){
          ...
          for ($b = 0 ; $b < $verif_groupes ; ++$b){
            if (isset($rep_classe1) AND array_key_exists($b, $rep_classe1) AND $rep_classe1[$b] == $aff_classe[$c]) {
              $td_classe1[$c] = '<span>Class group with '.implode(', ', $rep_classe1).'</span><br />';
            }
          }
...
// On annonce le nombre total d'absents sur le créneau
	$aff_nbre_abs = "we have ".$nbre_abs_cren." absent on the crenel";

?>
<h2><a href="../../accueil.php"><img src="../../images/icons/back.png" alt="Retour" title="Retour" class="back_link" />&nbsp;Return</a> -
	the absent of <?php echo $date_jour; ?> orderly by class and by  alphabetical order  - <a href="./bilan_absences_quotidien.php">statement of day</a></h2>

<form name="choix_du_creneau" action="voir_absences_viescolaire.php" method="post">
	<p>you must choise a crenel to see the absents 
	<select name="choix_creneau" onchange='document.choix_du_creneau.submit();'>
		<option value="rien">choise of crenel</option>
<?php
...
?>
	...
	&nbsp;-&nbsp;<span style="cursor: pointer; color: blue;" onclick="changementDisplay('id4_aide', '');">Help summary</span>
	</p>
		<div id="id4_aide" class="abs_appear" style="display: none; margin-left: 400px;">
	if you want to see the name of professor who is made the call when we not have absence, please past the mouse on the right text.<br />
	if you want to see the timetable of a class, click on the name of class .</div>
</form>
...
<table class="tab_edt" summary="List of absent distributed by class">
...
<h2>a click on a student change her state (absent to delay or inversely).</h2>
<p>care, this action is only available since a account CPE or school life.</p>
