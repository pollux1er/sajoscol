...
if ($var == "0000-00-00") {} else {
  ...
  $tab_mois = array('1'=>"January", '2'=>"February", '3'=>"March", '4'=>"April", '5'=>"May", '6'=>"June", '7'=>"July", '8'=>"August", '9'=>"September", '01'=>"January", '02'=>"February", '03'=>"March", '04'=>"April", '05'=>"May", '06'=>"June", '07'=>"July", '08'=>"August", '09'=>"September", '10'=>"October", '11'=>"November", '12'=>"December");
  $tab_jour = array("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday");
  $tab_date = explode(',', $date);
  $jour = date("w", mktime(0, 0, 0, $tab_date[1], $tab_date[0], $tab_date[2]));
...
function date_frc($var)
  ...
  $tab_jour = array("Sun.", "Mon.", "Tues.", "Wed.", "THur.", "Frid.", "Satu.");
  $tab_date = explode(',', $date);
  $jour = date("w", mktime(0, 0, 0, $tab_date[1], $tab_date[0], $tab_date[2]));
  if ($tab_date[0]=='01') {$tab_date[0]='1er';}
  if ($tab_date[0]=='02') {$tab_date[0]='2';}
...
function motab($motab)
{
   switch ($motab)
   {
      ...
      case "H": $motab = "Hospitalisé(e)"; break;
    ...
    if(my_eregi("[0-9]{4}-[0-9]{2}-[0-9]{2}",$date_a_verif))
     {
       //explotion de la date en jour, mois, année
        ...
        if( $resultats == true ) { $valide_date = "pass"; } else { $valide_date = "error"; }
     } else { $valide_date = "error"; }
    return($valide_date);
 }
				...
				while ( $boucle === 'oui' )
			 	{
					...
					if ( !isset($tableau_base[$date_selection]['matin']) ) { $tableau_base[$date_selection]['matin'] = 'not'; }
					if ( !isset($tableau_base[$date_selection]['apresmidi']) ) { $tableau_base[$date_selection]['apresmidi'] = 'not'; }
