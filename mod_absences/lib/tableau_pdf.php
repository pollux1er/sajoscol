...
if ($type=='A') {$typetableau='absences';}
if ($type=='R') {$typetableau='delays';}
if ($type=='D') {$typetableau='des dispenses';}
if ($type=='I') {$typetableau='Infirmary';}
...
/* début de la génération du fichier PDF */
	...
	$pdf->SetKeywords('');
	// sujet du document
	$pdf->SetSubject('Daily assessment of the absences');
	// titre du document
	$pdf->SetTitle('Daily assessment of the absences');
...
/* ENTETE TITRE - DEBUT */
		...
		$pdf->Cell(120, 6, 'Printing of the Table '.$typetableau, 0, 1, 'C');

		$pdf->SetFont('Arial','',12);

		$pdf->SetX(85);
		$pdf->Cell(80, 6, 'of '.$du, 0, 1, 'C');

		$pdf->SetX(85);
		$pdf->Cell(80, 6, 'at '.$au, 0, 1, 'C');

		$pdf->SetX(85);
		$pdf->Cell(80, 4, 'year', 0, 1, 'C');
...
if ($type=='A') {
	...
	// Cellule identité
	$pdf->Cell($lar_col_eleve, $hau_entete, 'Name First name', 1, 0, 'C');

	// Cellule classe ajout

	$pdf->Cell($lar_col_classe, $hau_entete, 'Class', 1, 0, 'C');

	// Cellule date du
    $pdf->Cell($lar_col_date_du, $hau_entete, 'Absence of', 1, 0, 'C');

	// Cellule date au
    $pdf->Cell($lar_col_date_au, $hau_entete, 'to', 1, 0, 'C');

	// Cellule Heure debut
    $pdf->Cell($lar_col_heure_debut, $hau_entete, 'of', 1, 0, 'C');

		// Cellule Heure fin
    $pdf->Cell($lar_col_heure_debut, $hau_entete, 'at', 1, 0, 'C');
	// Cellule motif
    $pdf->Cell($lar_col_motif, $hau_entete, 'Reasons', 1, 0, 'C');
	...
	// Cellule date du
    $pdf->Cell($lar_col_date_le, $hau_entete, 'The', 1, 0, 'C');

	// Cellule heure
    $pdf->Cell($lar_col_heure_a, $hau_entete, 'at', 1, 0, 'C');

	// Cellule cause
    $pdf->Cell($lar_col_cause, $hau_entete, 'reasons', 1, 0, 'C');
	...
	// Cellule date du
    $pdf->Cell($lar_col_date_du, $hau_entete, 'of', 1, 0, 'C');

	// Cellule date au
    $pdf->Cell($lar_col_date_au, $hau_entete, 'at', 1, 0, 'C');

	// Cellule Heure debut
    $pdf->Cell($lar_col_horaire, $hau_entete, 'schedule', 1, 0, 'C');
