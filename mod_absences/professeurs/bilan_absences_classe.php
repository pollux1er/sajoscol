...
$titre_page = "Assessment of the absences";
...
// ======================== Traitement des données ======================== //
	...
	if ($id_classe == NULL) {
		trigger_error("Impossible to display information requested because the class is not
specified.", E_USER_ERROR);
	...
	$query_c = mysql_query($sql_c) OR trigger_error('Impossible to display the class.', E_USER_WARNING);
	...
	$sql_a = "SELECT id, nom, prenom, retard_absence, debut_ts, fin_ts, eleve_id
				...
				ORDER BY nom, prenom";
	$query_a = mysql_query($sql_a) OR trigger_error('Impossible to list the absent ones.', E_USER_ERROR);
	$nbre_rep = mysql_num_rows($query_a);

	$aff = get_eleves_classe($id_classe);

		$aff_tab = '<tr><td>Name</td>';
...
<h3 class="gepi"><a href="./bilan_absences_professeur.php"><img src="../../images/icons/back.png" alt="To retrogress" />&nbsp;RETURN</a>
Absences of the class of <span style="font-weight: bold;"><?php echo $aff_nom_classe; ?>&nbsp;</span>
of&nbsp;
			<input type="text" name="debut" value="<?php echo $debut; ?>" title="Click on the small calendar and choose a date." />
		...
		<img src="../../lib/calendrier/petit_calendrier.gif" alt="choice goes back to beginning" border="0" title="Click and choose a date." /></a>
&nbsp;&nbsp;to&nbsp;
		...
		<img src="../../lib/calendrier/petit_calendrier.gif" alt="choix date de fin" border="0" title="Click and choose a date." /></a>


		<input type="hidden" name="id_classe" value="<?php echo $id_classe; ?>" />
		<input type="submit" name="Valider" value="Valider" title="Click to validate your choice." />
</h3>
	</form>
...
<div id="aff_tab_abs">
<table summary="List the absent ones over time requested" id="aff_abs">

	<thead><tr><th><?php echo $aff_nom_classe; ?></th><th colspan="<?php echo $nbre_de_jours; ?>">Summary of absences recorded each day by teachers.</th></tr></thead>

	<?php echo $aff_tab; ?>

	<?php echo $aff_liste_abs; ?>

	<tfoot><tr><th colspan="2">To display it <?php echo date("d/m/Y - h:i"); ?></th><th colspan="<?php echo $nbre_de_jours - 1; ?>">Summary of absences recorded each day by teachers.</th></tr></tfoot>
