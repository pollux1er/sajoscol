...
if ($etape == 2 AND $classe != "toutes" AND $classe != "" AND $action_sql == "ajouter") {
	...
	for($a=0; $a<count($eleve_absent); $a++) {
		...
		if (isset($active_absence_eleve[$a])) {
			if ($active_absence_eleve[$a] == 1) {
				...
				if ($nbr_cherche == 0) {
					...
					$echo .= '<p class="enregistre_bon"> absence of '.$noms["prenom"].' '.$noms["nom"].' is well recorded !</p>';
				}else {
					$echo .='<p class="enregistre_deja"> absence of '.$noms["prenom"].' '.$noms["nom"].' was already seized ! </p>';
				}
			}
		}
		if (isset($active_retard_eleve[$a])) {
			if ($active_retard_eleve[$a] == 1){
				...
				if ($nbr_cherche == 0) {
					...
					$insere_abs = mysql_query($saisie_sql) OR DIE ('Erreur SQL !'.$saisie_sql.'<br />'.mysql_error());//('Impossible d\'enregistrer l\'absence de '.$eleve_absent[$a]);
					$echo .= '<p class="enregistre_bon">Le retard de '.$noms["prenom"].' '.$noms["nom"].' is well recorded !</p>';
				}else {
					...
					$update = mysql_query("UPDATE absences_rb SET retard_absence = 'R'
															WHERE id = '".$id_abs."'");
					$echo .='<p class="enregistre_modifie"> absence of '.$noms["prenom"].' '.$noms["nom"].' was modified late ! </p>';
		...
		if (isset($active_repas_eleve[$a])) {
			if ($active_repas_eleve[$a] == 1){
				...
				//echo $saisie_sql;
				$insere_abs = mysql_query($saisie_sql) OR DIE ('Erreur SQL !'.$saisie_sql.'<br />'.mysql_error());//('Impossible d\'enregistrer l\'absence de '.$eleve_absent[$a]);
				$echo .= '<p class="enregistre_bon"> meal for '.$noms["prenom"].' '.$noms["nom"].' is well recorded !</p>';
			}
		}
		//Fin Ajout Eric
	...
	if ($echo == "") {
		...
		if ($nbre_verif == 0) {
			$requete_sql = "INSERT INTO absences_rb (eleve_id, groupe_id, edt_id, jour_semaine, creneau_id, debut_ts, fin_ts, date_saisie, login_saisie) VALUES ('appel', '".$classe."', '0', '".$jour_semaine[0]."', '".$rep_creneau["id_definie_periode"]."', '".$ts_debut."', '".$ts_fin."', '".$ts_actu."', '".$_SESSION["login"]."')" OR DIE ('Impossible to enter seizure');
			$saisie_appel = mysql_query($requete_sql);
			$echo .= '<p class="enregistre_bon">No absent - The call was indeed carried out.</p>';
		}
		else {
			$echo .= '<p class="enregistre_deja">No absent but this call was already recorded.</p>';
		}
	}
...
if ( $etape == '2' AND $menuBar != "ok") {
	if ( $d_heure_absence_eleve>=$a_heure_absence_eleve) { $msg_erreur = 'Attention the schedule of beginning must precede the schedule by end'; $etape = ''; }
	if ( $a_heure_absence_eleve === '' ) { $msg_erreur = 'Attention it is necessary to seize a schedule of end'; $etape = ''; }
	if ( $d_heure_absence_eleve === '' ) { $msg_erreur = 'Attention it is necessary to seize a schedule of beginning'; $etape = ''; }
	if ( $d_date_absence_eleve === '' ) { $msg_erreur = 'Attention it is necessary to seize a date'; $etape = ''; }
}
...
if (getSettingValue("liste_absents") == "y") {
	echo "
	 |<a href=\"../lib/tableau.php?type=A&amp;pagedarriver=prof_ajout_abs\"> To visualize the absences</a>\n";
}
echo "</p>";
...
?>
		<p>
		<label for="d_heure_absence_eleve">From</label>
...
?>
			<option value=""<?php echo $selected; ?>>no selection</option>
<?php
...
?>
		...
		<label for="a_heure_absence_eleve">&nbsp;to&nbsp;</label>
...
<p class="choix_fin">
	<label for="classe">Subject - class</label>
...
<?php
	if ( $etape == '2' and $classe == '' and $eleve_initial == '' ) {
		echo '
			<p class="erreur_rouge_jaune">
				<img src="../../images/icons/ico_attention.png" alt="ATTENTION" title="ATTENTION" />&nbsp;
				Erreur de selection, do not forget to select a class or a student
			</p>
	   ';
	}
	...
	if (getSettingValue("active_module_trombinoscopes")=='y') {
		...
		<label for="affPhoto">With picture</label>'."\n";
	}
	...
	if (getSettingValue("memorisation") == "y") {
		...
		<label for="edtEnregistrement">To memorize this selection</label>
		';
	}
	echo '</p>'."\n";
?>
	...
	<p class="choix_fin">
		...
		<input value="Class list" name="Valider" type="submit" onclick="this.form.submit();this.disabled=true;this.value='En cours'" />
	</p>
	<p>
...
?>
		it is currently : <?php echo date('G:i')  ?>
	</p>
	<p class="voir_tout"><a href="./bilan_absences_professeur.php">Display all absences</a></p>
	</form>
</div>
<?php
...
if ( $etape === '2' AND $classe != 'toutes' AND ( $classe != '' OR $eleve_initial != '' ) AND $msg_erreur === '') {
	...
	//echo "$sql<br />";
    $periode_active = mysql_query($sql) OR DIE('Impossible to recover the number of the active period' . $sql . '<br />--> ' . mysql_error());
	...
	<div class="centre_tout_moyen">
		<form method="post" action="prof_ajout_abs.php" id="liste_absence_eleve">
			<p class="expli_page choix_fin">
				Seizure of the absences<br/>
				of <strong><?php echo date_frl(date_sql($d_date_absence_eleve)); ?></strong>
				od <strong><?php echo heure_court($d_heure_absence_eleve); ?></strong>
				at <strong><?php echo heure_court($a_heure_absence_eleve); ?></strong>
				<br/>
<?php
	echo "
	...
	if($passage_auto === 'oui' and $passage_form === '') {
		echo '
			<p class="choix_fin">
				<a href="prof_ajout_abs.php?passage_form=manuel">This is not the good polling list ?</a>
			</p>
		';
	}
	?>
...
<!-- Legende du tableau-->
	...
	<table class="tb_code_couleur" summary="Code des couleurs">
		<tr>
			<td class="td_Retard">&nbsp;R&nbsp;</td><td>&nbsp;Delay</td>
...
<!-- <table style="text-align: left; width: 600px;" border="0" cellpadding="0" cellspacing="1"> -->
	<table class="tb_absences" summary="Liste des élèves pour l'appel. Colonne 1 : élèves, colonne 2 : absence, colonne3 : retard, colonnes suivantes : suivi de la journée par créneaux, dernière colonne : photos si actif">
		...
		<tbody>
			<tr class="titre_tableau_gestion" style="white-space: nowrap;">
				<th class="td_abs_eleves" style="width: 10%;">&nbsp;Yesterday&nbsp;</th>
				<th class="td_abs_eleves" abbr="élèves">List student</th>
	...
	$req_noms = mysql_query($sql) OR DIE ('No crenels available.');
	$nbre_noms = mysql_num_rows($req_noms) OR die ('Impossible to count the crenels.');

	echo '
				<th colspan="'.$nbre_noms.'" class="th_abs_suivi" abbr="Créneaux">Follow-up over the day</th>'."\n";
?>
			</tr>
			<tr>
				<td></td>
...
?>
	...
	if ($test[0] == "AID") {
		...
		$requete_liste_eleve = "SELECT * FROM eleves, groupes, j_eleves_groupes 
                            ...
                            ORDER BY nom, prenom";
/* ajouté le 17/09/2012 pour résoudre un bug par patient. La liste des élèves pour la saisie des absences semble vide a caude de la connexion*/var_dump($requete_liste_eleve);
...
//======================== début de la saisie des retards ==================================================
		...
		if (getSettingValue("renseigner_retard") == "y") {
			...
			if ($pass === '0') {
?>
				<label for="active_retard_eleve<?php echo $cpt_eleve; ?>" class="invisible no_print">Delay</label>
				...
				<label for="heure_retard_eleve<?php echo $cpt_eleve; ?>" class="invisible no_print">hour delay</label>
...
?>
				Late<input id="active_retard_eleve<?php echo $cpt_eleve; ?>" name="active_retard_eleve[<?php echo $cpt_eleve; ?>]" value="0" type="hidden" />
<?php
			}
		echo"\n</td>\n";
		} // if (getSettingValue("renseigner_retard") == "y")
...
//======================== début de la saisie des Repas ==================================================
		...
		if (getSettingValue("renseigner_Repas") == "y") {
			...
			if ($pass === '0') {
?>
				<label for="active_repas_eleve<?php echo $cpt_eleve; ?>" class="invisible no_print">Meal</label>
