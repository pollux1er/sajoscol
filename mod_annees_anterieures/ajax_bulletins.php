	...
	// On a passé les barrières, on passe au traitement
		...
		*/
		...
		if(mysql_num_rows($res_ele)==0) {
			...
			echo "<p>The student whose login would be '$logineleve' is not in the table 'student'.</p>\n";
		}
		else {
			...
			if(!isset($annee_scolaire)) {
				...
				if(mysql_num_rows($res_annee)==0) {
					echo "<p>No year filed for this student.</p>\n";
					die();
				}
			...
			if(mysql_num_rows($res_per)==0) {
				$nom_periode="period $num_periode";
				$classe_ant="???";
			}
			else {
			...
			echo "<h2 style='color:black;'>Antecedents of $ele_prenom $ele_nom: millésime $annee_scolaire</h2>\n";
	
			//echo "<p>Report card simplifié de $prenom $nom pour la période $num_periode de l'année scolaire $annee_scolaire</p>";
			echo "<p>Report card simplified of $ele_prenom $ele_nom: $nom_periode de l'année scolaire $annee_scolaire in <strong>$classe_ant</strong> <em style='font-size: x-small;'>(currently in $classe)</em></p>\n";
			...
			echo "<th rowspan='2'>Appreciation/Councils</th>\n";
			echo "</tr>\n";
			...
			elseif(mysql_num_rows($res_abs)>1) {
				echo "<p>Odd: There is more than one recording for this student, this period and
this year.</p>\n";
			}
			else {
				...
				if ($nb_absences=='0') {
					...
					if ($non_justifie != '0') {
						echo " (of which <b>$non_justifie</b> not justified"; if ($non_justifie != '1') {echo "s";}
						echo ")";
					}
					echo ".";
				}
