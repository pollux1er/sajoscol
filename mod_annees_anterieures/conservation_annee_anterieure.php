...
$themessage  = 'Are you sure to want to remove all the data concerant this year ?';

//**************** EN-TETE *****************
$titre_page = "Conservation of the former data (others that AID)";
...
if(!isset($annee_scolaire)){
	...
	if(mysql_num_rows($res_annee)==0){
		echo "<p>Concerning the data other than AIDs, no year is still safeguarded.</p>\n";
	}
	else{
		...
		while($lig_annee=mysql_fetch_object($res_annee)){
			...
			echo "<li><b>Year $annee_scolaire (<a href='".$_SERVER['PHP_SELF']."?action=supp_annee&amp;annee_supp=".$annee_scolaire.add_token_in_url()."'   onclick=\"return confirm_abandon (this, 'yes', '$themessage')\">Remove all the data filed for this year</a>) :<br /></b> ";
	...
	if(mysql_num_rows($res_test)>0){
		if(!isset($confirmer)){
			...
			if(mysql_num_rows($res_test)>1){
				...
				echo "<p>Data were already safeguarded for the year $annee_scolaire (<i>classes of $chaine_classes</i>).<br />If you confirm, these data will be crushed with the new data (<i>if you do not notch the same classes, the data will be only
supplemented</i>).</p>\n";
			}
			else{
				echo "<p>Data were already safeguarded for the year $annee_scolaire (<i>classify $chaine_classes</i>).<br />If you confirm, these data will be crushed with the new data (<i>if you do not notch the same classes, the data will be only
supplemented</i>).</p>\n";
			}
	...
	if(!isset($id_classe)) {
		...
		echo "<h2>Choice of the classes</h2>\n";

		echo "<p>Conservation of the data for the school year: $annee_scolaire</p>\n";

		echo "<p>Choose the classes whose you wish to file the results,
appreciations...</p>";
		...
		if(count($id_classe)==0){
			echo "<p>ERROR: You did not tick of class.</p>\n";
			echo "</form>\n";
			require("../lib/footer.inc.php");
			die();
		}
		...
		*/
		...
		if($temoin_encore_des_classes>0) {
			...
			echo "<p>Treatment of the last selected class: <span id='annonce_fin_traitement' style='font-weight:bold; font-size:2em; color: green;'></span></p>\n";
		}
		//===================================
		...
		if(mysql_num_rows($res_periode)==0){
			echo "<p>No period seems defined for the class $classe</p>\n";
		}
		else{
			unset($tab_periode);
			$tab_periode=array();
			...
			if(mysql_num_rows($res_ele)==0){
				echo "<p>No student in the class $classe???</p>\n";
			}
			else{
				unset($tab_eleve);
				$tab_eleve=array();
				...
				echo "<table class='boireaus' border='1' summary='Table of the student'>\n";
				...
				for($i=1;$i<=count($tab_periode);$i++){
					...
					if(!$res_nettoyage){
						echo "<p style='color:red'><b>ERROR</b> during cleaning</p>\n";
						echo "</form>\n";
						require("../lib/footer.inc.php");
						die();
					}
	...
	document.getElementById('td_".$i."_".$j."').style.backgroundColor='red';
						...
						*/
						...
						if(mysql_num_rows($res_grp)==0){
							...
							echo "<!-- In period $num_periode, l'élève $login_eleve is associated no teaching -->\n";
						}
						else{
	...
	document.getElementById('td_".$i."_".$j."').style.backgroundColor='red';
							...
							echo "<!-- Before appropriations ECTS of the student $login_eleve -->\n";
							...
							echo "<!-- After appropriations ECTS of the student $login_eleve -->\n";
...
//==================================================
	...
	setTimeout('document.formulaire.submit();', 5000);
			...
			echo "<p style='text-align:center; font-weight:bold; font-size:2em; color: green;'>Finished treatment.</p>\n";
			echo "<script type='text/javascript'>
