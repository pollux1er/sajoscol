...
if(getSettingValue('active_annees_anterieures')!="y"){
	...
	tentative_intrusion(1, "Attempt at one ".$_SESSION["statut"]." (".$_SESSION["login"].") to reach the module former Years which is not activated.");
		...
		if(isset($logineleve)){
			...
			*/
			...
			if(mysql_num_rows($test)==0){
				...
				// A VOIR: Comment enregistrer une tentative d'accès illicite?
				//echo "$sql<br />";
				tentative_intrusion(1, "Attempt at one ".$_SESSION["statut"]." (".$_SESSION["login"].") to reach the data of former Years of $logineleve who is not raises of one of his classes.");
				header("Location: ../logout.php?auto=1");
				die();
			}
		}
	...
	elseif($AAProfGroupes=="yes"){
		...
		if(isset($logineleve)){
			...
			if(mysql_num_rows($test)==0){
				...
				// A VOIR: Comment enregistrer une tentative d'accès illicite?
				//echo "$sql<br />";
				tentative_intrusion(1, "Attempt at one ".$_SESSION["statut"]." (".$_SESSION["login"].") to reach the data of former Years $$logineleve>which is not raises of
one of its lesson.");
				header("Location: ../logout.php?auto=1");
				die();
			}
	...
	elseif($AAProfPrinc=="yes"){
		...
		if(isset($logineleve)){
			...
			if(mysql_num_rows($test)==0){
				...
				// A VOIR: Comment enregistrer une tentative d'accès illicite?
				//echo "$sql<br />";
				tentative_intrusion(1, "Attempt at one ".$_SESSION["statut"]." (".$_SESSION["login"].") to reach the data of former Years of $$logineleve>which it is not ".getSettingValue("gepi_prof_suivi").".");
				header("Location: ../logout.php?auto=1");
				die();
			}
		}
		...
		if(mysql_num_rows($test)>0){
			...
			if(isset($logineleve)){
				...
				if(mysql_num_rows($test)==0){
					// A DEGAGER
					// A VOIR: Comment enregistrer une tentative d'accès illicite?
					tentative_intrusion(1, "Tentative d'un ".$_SESSION["statut"]." (".$_SESSION["login"].") to reach the data of former Years of $$logineleve>which it is not
responsible.");
					header("Location: ../logout.php?auto=1");
					die();
				}
		...
		if(mysql_num_rows($test)>0){
			...
			if(isset($logineleve)){
				...
				if(mysql_num_rows($test)==0){
					...
					// A VOIR: Comment enregistrer une tentative d'accès illicite?
					tentative_intrusion(1, "Attempt at one ".$_SESSION["statut"]." (".$_SESSION["login"].") to reach the data of former Years $$logineleve>which is not raises of
one of the classes whose CPE is responsible.");
					header("Location: ../logout.php?auto=1");
					die();
				}
			}
		...
		if(mysql_num_rows($test)>0){
			...
			if(isset($logineleve)){
				...
				if(mysql_num_rows($test)==0){
					// A DEGAGER
					// A VOIR: Comment enregistrer une tentative d'accès illicite?
					tentative_intrusion(1, "Attempt at one ".$_SESSION["statut"]." (".$_SESSION["login"].") to reach the data of former Years of $$logineleve>which it is not
responsible.");
					header("Location: ../logout.php?auto=1");
					die();
				}
...
if($acces!="y"){
	...
	tentative_intrusion(1, "Illicit attempt at one ".$_SESSION["statut"]." (".$_SESSION["login"].") to reach data of former Years.");
...
//**************** EN-TETE *****************
$titre_page = "Consultation of the former data";
...
if(($_SESSION['statut']=='administrateur')&&(isset($_GET['ine']))) {
	...
	echo " | <a href='nettoyer_annee_anterieure.php'>Cleaning</a>";
	if(($mode!='bull_simp')&&($mode!='avis_conseil')) {
		echo "</p>\n";
		echo "</div>\n";

		echo "<h2 style='text-align: center;'>Choice of former information</h2>\n";
		//tab_choix_anterieure($logineleve);
		tab_choix_anterieure('','',$_GET['ine']);
	}
	else {
		echo " | <a href='".$_SERVER['PHP_SELF']."?ine=".$_GET['ine']."'>Choice of information</a>\n";
		echo "</p>\n";
		echo "</div>\n";

		//if($mode=='bull_simp'){
			echo "<h2 style='text-align: center;'>One year simplified bulletin former</h2>\n";
			if(!isset($annee_scolaire)){
				echo "<p><strong>ERREUR:</strong> The former school year does not seem to have been selected.</p>\n";
			}
			elseif(!isset($num_periode)){
				echo "<p><strong>ERROR:</strong> The period does not seem to have been selected.</p>\n";
			}
			/*
...
if(!isset($id_classe)){
	...
	echo "<h2>Choice of the class</h2>\n";

	echo "<p>Choose the class in which currently one is ".$gepiSettings['denomination_eleve']." for which you wish to consult the data of former years.</p>";
	...
	if(!isset($sql_classes)){
		echo "<p>ERROR: It seems that the request of choice of the class was not initialized.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	if($nb_classes==0){
		echo "<p>ERROR: It seems that no class is yet defined.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	if((!isset($logineleve))&&(!isset($aff_classe))) {
		...
		if(!isset($sql_ele)){
			echo "<p>ERROR: It seems that the request of choice of L'".$gepiSettings['denomination_eleve']." was not initialized.</p>\n";
			require("../lib/footer.inc.php");
			die();
		}
		...
		if(mysql_num_rows($res_ele)==0){
			echo "<p>ERROR: It seems that there is none ".$gepiSettings['denomination_eleve']." in this class.</p>\n";
			require("../lib/footer.inc.php");
			die();
		}
		else{
			echo "<p>Choose one ".$gepiSettings['denomination_eleve']." for which you wish to consult former information.</p>\n";
			...
			echo "<table width='100%' summary=\"Choice of the student\">\n";
			echo "<tr valign='top' style='text-align: center;'>\n";
		...
		($_SESSION['statut']=='professeur')) {
			...
			echo "<p>Or to post information for all the class over the selected period:</p>\n";
			echo "<blockquote>\n";
			...
			if(mysql_num_rows($res_ant)==0){
				...
				echo "<p>No former result was preserved for this class.</p>\n";
			}
			else{
				...
				$alt=1;
				echo "<table class='boireaus table_annee_anterieure' summary='Report cards'>\n";
				...
				for($i=0;$i<count($tab_annees);$i++){
					...
					echo "Year-school <a href='".$_SERVER['PHP_SELF']."?aff_classe=y&amp;annee_scolaire=".$tab_annees[$i]."&amp;mode=avis_conseil";
	...
	elseif((isset($aff_classe))&&(isset($sql_ele))&&(
		...
		//if($_SESSION['statut']!='eleve'){
			echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_classe'>Choose another period or ".$gepiSettings['denomination_eleve']."</a></p>\n";
			echo "</form>\n";
			echo "</div>\n";
		//}
		...
		if(mysql_num_rows($res_liste_ele)==0) {
			echo "<p>Aucun ".$gepiSettings['denomination_eleve']." A does not seem it found.</p>\n";
		}
		else {
			require("fonctions_annees_anterieures.inc.php");
		...
		if($_SESSION['statut']!='eleve'){
			echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_classe'>Choose another ".$gepiSettings['denomination_eleve']."</a>\n";
		...
		if((!isset($logineleve))||(($mode!='bull_simp')&&($mode!='avis_conseil'))) {
			...
			echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_classe&amp;logineleve=$logineleve'>Choice of information</a>\n";
			echo "</p></form>\n";
			echo "</div>\n";
			...
			if($mode=='bull_simp'){
				echo "<h2 style='text-align: center;'>One year simplified bulletin to form</h2>\n";
				if(!isset($annee_scolaire)){
					echo "<p><strong>ERROR:</strong> The former school year does not seem to have been selected.</p>\n";
				}
				elseif(!isset($num_periode)){
					echo "<p><strong>ERROR:</strong> The period does not seem to have been selected.</p>\n";
				}
				elseif(!isset($id_classe)){
					echo "<p><strong>ERREUR:</strong> The identifier of the current class of this ".$gepiSettings['denomination_eleve']." do not seem to be provided.</p>\n";
				}
				else{
					/*
					if(!isset($num_periode)){
			...
			elseif($mode=='avis_conseil'){
				echo "<h2 style='text-align: center;'>Opinion of the Staff meetings one year former</h2>\n";
				if(!isset($annee_scolaire)){
					echo "<p><strong>ERREUR:</strong> The former school year does not seem to have been selected.</p>\n";
				}
				else{
