	...
	*/
	...
	if($logineleve!="") {
		...
		if(mysql_num_rows($res_ele)==0) {
			...
			echo "<p>The student whose login would be $logineleve is not in the table 'eleves'.</p>\n";
			$poursuivre="n";
		}
	}
	elseif($ine=="") {
		echo "<p>No login nor INE was proposed.</p>\n";
		$poursuivre="n";
	}
	else {
		...
		if(mysql_num_rows($res_ele)==0) {
			// On ne devrait pas arriver là.
			echo "<p>The pupil whose INE would be $ine is not in the table 'archivage_eleves'.</p>\n";
			$poursuivre="n";
		}
	}
	...
	if($poursuivre=="y") {
		...
		*/
		...
		echo "<h2>Antecedents of $ele_prenom $ele_nom: millésime $annee_scolaire</h2>\n";

		//echo "<p>Report card simplifié de $prenom $nom pour la période $num_periode de l'année scolaire $annee_scolaire</p>";
		echo "<p>Report card simplified of $ele_prenom $ele_nom: $nom_periode school year $annee_scolaire in <strong>$classe_ant</strong> <em style='font-size: x-small;'>(currently in $classe)</em></p>\n";
		...
		echo "<th colspan='3'>Class</th>\n";
		echo "<th rowspan='2'>Student</th>\n";
		echo "<th rowspan='2'>Appreciations/Councils</th>\n";
		echo "</tr>\n";
		...
		if(mysql_num_rows($res_mat)==0){
			// On ne devrait pas arriver là.
			echo "<tr><td colspan='6'>No recorded result???</td></tr>\n";
		}
		else{
	...
	ORDER BY type.nom, aid.nom";
		...
		*/
		...
		if(mysql_num_rows($res_abs)==0){
			echo "<p>No information on the absences/delays.</p>\n";
		}
		elseif(mysql_num_rows($res_abs)>1){
			echo "<p>Odd: There is more than one recording for this student, this period and this year.</p>\n";
		}
		else{
			...
			if ($nb_absences=='0') {
				...
				if ($non_justifie=='0') {echo "justified ";}
				echo ": </i><b>$nb_absences</b>";
				if ($non_justifie != '0') {
					echo " (dont <b>$non_justifie</b> not justified"; if ($non_justifie != '1') {echo "s";}
					echo ")";
				}
				echo ".";
			}
			if ($nb_retards!='0') {
				echo "<i> Number of delays : </i><b>$nb_retards</b>";
			}
			echo "  (C.P.E. charged )";
			echo " follow-up : ".$lig_abs->prof.")";
		...
		echo "<table class='table_annee_anterieure' width='100%' summary='Opinion of the council'>\n";
		echo "<tr>\n";
		echo "<td align='left'>\n";
		echo "<p><i>Opinion of the Staff meeting : </i><br />\n";

		$prof_suivi="";
		if(mysql_num_rows($res_avis)==0){
			echo "No information on the opinion of the staff meeting.</p>\n";
		}
		elseif(mysql_num_rows($res_avis)>1){
			echo "Odd: There is more than one recording for this pupil, this period and
this year.</p>\n";
			$prof_suivi="?";
		}
		else{
		...
		echo "<p>Classify followed by : <b>$prof_suivi</b></p>\n";
		echo "</td>\n";
		echo "</tr>\n";
		echo "</table>\n";
...
function avis_conseils_de_classes_annee_anterieure($logineleve,$annee_scolaire){
	...
	if(mysql_num_rows($res_ele)==0){
		...
		if($annee_precedente!=""){
			...
			echo "'><img src='../images/icons/back_.png' width='16' height='14' alt='Previous year' /></a> \n";
		}
		echo "<b>$annee_scolaire</b>\n";
		if($annee_suivante!=""){
			echo " <a href='".$_SERVER['PHP_SELF']."?logineleve=$logineleve&amp;annee_scolaire=$annee_suivante&amp;mode=avis_conseil";
			if(isset($id_classe)){echo "&amp;id_classe=$id_classe";}
			echo "'><img src='../images/icons/forward_.png' width='16' height='14' alt='Following year' /></a>\n";
		}
		echo "</li>\n";
		echo "</ul>\n";
		...
		echo "<p>Opinion of the staff meetings for $ele_prenom $ele_nom at the time of the school year $annee_scolaire</p>\n";
		...
		if(mysql_num_rows($res_avis)==0){
			...
			echo "<table class='table_annee_anterieure' width='100%' summary='Opinion of the council'>\n";
			echo "<tr>\n";
			echo "<th>Year-school</th>\n";
			echo "<th>Opinion of the staff meeting</th>\n";
			echo "<th>Classify followed Par</th>\n";
			echo "</tr>\n";
...
function tab_choix_anterieure($logineleve,$id_classe=NULL,$ine=''){
	...
	if(mysql_num_rows($res_ele)==0){
		...
		elseif($ine!="") {
			echo "<p>The student whose INE would be $ine is not in the table 'archivage_eleves'.</p>\n";
		}
	}
	else{
		$lig_ele=mysql_fetch_object($res_ele);
		...
		echo "<p>List school years and periods for which data concerning $ele_prenom $ele_nom ";
		if((isset($id_classe))&&($id_classe!='')) {
			$classe=get_nom_classe($id_classe);
			echo "(<i>$classe</i>) ";
		}
		echo "were preserved:</p>\n";
		...
		if(mysql_num_rows($res_ant)==0){
			echo "<p>No former result was preserved for this pupil.</p>\n";
		}
		else{
			...
			while($lig_ant=mysql_fetch_object($res_ant)){
				...
				if(mysql_num_rows($res_ant2)==0){
					echo "<td>No former result was preserved for this student.</td>\n";
				}
				else{
					$cpt=0;
			...
			//echo "<table border='0'>\n";
			$alt=1;
			echo "<table class='boireaus table_annee_anterieure' summary='Opinion of the councils'>\n";
			echo "<tr class='lig$alt'>\n";
...
function insert_eleve($login,$ine,$annee,$param) {
	...
	if ($test == 0) {
		...
		$res_ele=mysql_query($sql);
		if(mysql_num_rows($res_ele)==0) {
			return "<tr><td colspan='4'>No data available for the pupil whose identifier is ".$login."</td></tr>";
			die();
		} else {
			while($lig_ele=mysql_fetch_object($res_ele)){
				...
				$res_insert=mysql_query($sql);
					if(!$res_insert){
					return "<tr><td colspan='4'><font color='red'>Error of recording of the data for the student whose identifier is ".$login."</font></td></tr>";
					exit();
				} else {
					if ($param != 'y') {
	...
	echo "<div id='div_annees_anterieures' class='infobulle_corps' style='position: absolute; top: 220px; right: 20px; width: 700px; text-align:center; color: black; padding: 0px; border:1px solid black; display:none;'>\n";
		...
		echo "<div id='corps_annees_anterieures' class='infobulle_corps' style='color: #ffffff; cursor: auto; font-weight: bold; padding: 0px; height: 15em; width: 700px; overflow: auto;'>";
			...
			if(count($tab_periodes)==0) {
				echo "<p>No data filed for ".get_nom_prenom_eleve($logineleve).".</p>\n";
			}
			else {
				$t_annee_prec="";
				for($i=0;$i<count($tab_periodes);$i++) {
