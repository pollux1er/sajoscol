...
$accessibilite="y";
$titre_page = "Management of export APB";
$niveau_arbo = 1;
$gepiPathJava="./..";
$post_reussi=FALSE;
$msg = '';
...
if (isset($_POST['activer'])) {
    ...
    if (!saveSetting("active_mod_apb", $_POST['activer'])) {
      ...
      $msg = "The statute of activation of the module was indeed recorded.";
    }
}
...
if (isset($_POST['posted_selection'])) {
	...
	while ($classe = mysql_fetch_object($req_classes)) {
		...
		if ($classe->apb_niveau != $new_value) {
			...
			if (!$rec_classe) {
				$errors = true;
				$msg .= "Error during the recording of the new value for the class ".$classe->classe.".";
			}
		}
	}
	if (!$errors) {
		$msg .= "The data were recorded successfully.";
		$post_reussi=TRUE;
	}
}
