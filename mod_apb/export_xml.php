 ...
 * Etablissement/Eleves/Annees-scolaires/Report cards/Notes
 * 
 */
...
if (!isset($_POST)) {
  echo "You cannot reach this script directly.";
  die();
}
...
if (isset($_POST['num_export'])) {
  ...
  if (!is_numeric($numero_export)) {
    echo "The number of export is not correct.";
    die();
  }
} else {
  echo "Impossible to determine the number of export.";
  die();
}
...
while ($classe = mysql_fetch_object($req_classes)) {
  ...
  if (!array_key_exists($classe->periodes,$limites_periodes)) {
    if (!isset($_POST[$classe->periodes.'per'])) {
      echo "Impossible to find the limit of periods.";
      die();
    } else if (!is_numeric($_POST[$classe->periodes.'per'])) {
      echo "Impossible to determine the limit of periods.";
      die();
    }
