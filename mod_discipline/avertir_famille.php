...
if(strtolower(substr(getSettingValue('active_mod_discipline'),0,1))!='y') {
	$mess=rawurlencode("You try D to reach the Discipline module which is decontaminated !");
...
//**************** EN-TETE *****************
$titre_page = "Discipline: To inform the family";
...
echo "<p class='bold'><a href='index.php' onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Index</a>\n";

echo " | <a href='saisie_incident.php?id_incident=$id_incident' onclick=\"return confirm_abandon (this, change, '$themessage')\">Incidental return n°$id_incident</a>\n";
...
echo "<tr class='lig1'>\n";
echo "<td style='font-weight:bold; text-align:left; vertical-align: top;'>Plain&nbsp;:</td>\n";
...
if($nb_adr==0) {
	echo "<tr class='lig-1'>\n";
	echo "<td style='font-weight:bold; text-align:left; vertical-align: top;'>Recipients&nbsp;:</td>\n";
	echo "<td colspan='4'>No responsible was found in the table 'resp_pers'.</td>\n";
	echo "</tr>\n";
}
elseif($nb_adr==1) {
	echo "<tr class='lig-1'>\n";
	echo "<td style='font-weight:bold; text-align:left; vertical-align: top;' rowspan='".$nb_resp_legaux."'>Recipients&nbsp;:</td>\n";
	...
	for($i=0;$i<count($tab_resp);$i++) {
		...
		echo "<td style='text-align:left;'>\n";
		if(!isset($tab_resp[$i]['adr1'])) {
			echo "<span style='color:red;'>No  address</span>";
		}
		else {
...
echo "<tr class='lig1'>\n";
echo "<td style='font-weight:bold; text-align:left; vertical-align: top;'>Mail&nbsp;:</td>\n";
echo "<td style='text-align:left;' colspan='4'>\n";
...
if(!isset($id_communication)) {
	...
	if(mysql_num_rows($res_incident)==0) {
		echo "??? The incident n°$id_incident do not exist ???";
	}
	else {
...
echo "<p style='color:red;'><b>TO MAKE:</b> Also display the phone numbers.<br />
Propose ' mail' only if the addresses mall of the resp are
indicated.<br />
To be able to record the fact that the parents were informed.<br />
How to also preserve a trace of the mails sent? and capacity to erase
the tests.</p>\n";
