...
if(mysql_num_rows($test_table)==0) {
	...
	if($creation) {
		// Mesures prises
		$tab_mesure=array("Additional work","Word in the notebook of connection");
		...
		// Mesures demandées
		$tab_mesure=array("Reserve","Exclusion");
		for($loop=0;$loop<count($tab_mesure);$loop++) {
			$sql="SELECT 1=1 FROM s_mesures WHERE mesure='".$tab_mesure[$loop]."';";
			//echo "$sql<br />";
			$test=mysql_query($sql);
...
if(mysql_num_rows($test_table)==0) {
	...
	$creation=mysql_query($sql);updateOnline($sql);
	if($creation) {
...
/* ===== Titre du menu ===== */
	...
	$menuTitre[$a]->texte='Configuration of the module';
...
*/
	...
	{
		$nouveauItem->titre="Definition of the places" ;
		$nouveauItem->expli="Define the list of the places of the incidents." ;
...
*/
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Definition of the roles" ;
		$nouveauItem->expli="Define the list of the roles of the protagonists." ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
*/
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Definition of measurements" ;
		$nouveauItem->expli="Define the list of the measurements taken in reference to an
incident." ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
...
*/
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Definition of the types of sanctions" ;
		$nouveauItem->expli="Define the list of the sanctions being able to be taken in
reference to an incident." ;
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Definition of natures of incidents" ;
		$nouveauItem->expli="Define natures of incidents (<em>indicative list or imposed list</em>)." ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Definition of the categories of incidents" ;
		$nouveauItem->expli="Define the categories of incidents (<em>at ends of statistics</em>)." ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
*/
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Definition of the recipients of alarms" ;
		$nouveauItem->expli="Allows to define the list of the users receiving a mall during the
seizure/modification of an incident." ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Management of the temporary delegations of exclusions" ;
		$nouveauItem->expli="Allows to manage the list of the délégataires for the generation of
the mails of temporary exclusion" ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
/* ===== Titre du menu ===== */
  ...
  $menuTitre[$a]->icone['titre']='';
  $menuTitre[$a]->icone['alt']="";
  $menuTitre[$a]->texte=' Disciplinary / health records';
  $menuTitre[$a]->indexMenu=$a;
...
*/
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Disciplinary / Health situation" ;
		$nouveauItem->expli="Record the disciplinary / health situation (<em>nature, date, time, place,...</em>).<br />Indicate measures/actions taken or requested" ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
($_SESSION['statut']=='autre')) {
	...
	if(mysql_num_rows($test)>0) {
	  ...
	  if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	  {
		  $nouveauItem->titre="Incidents without protagonists" ;
		  $nouveauItem->expli="List incidents announced without protagonists." ;
		  $nouveauItem->indexMenu=$a;
		  $menuPage[]=$nouveauItem;
	  }
	  unset($nouveauItem);
...
if($nb_incidents_incomplets>0) {
  ...
  $nouveauItem->chemin="/mod_discipline/traiter_incident.php";
	  if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	  {
		...
		$nouveauItem->expli="You announced ".$aff_incident." without specifying the nature of the incident.<br />To facilitate the management of the incidents, it would be necessary
to supplement." ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	  }
...
/* ===== Titre du menu ===== */
  ...
  $menuTitre[$a]->icone['titre']='';
  $menuTitre[$a]->icone['alt']="";
  $menuTitre[$a]->texte='To treat an incident';
  $menuTitre[$a]->indexMenu=$a;
  ...
  */
   ...
   */
	...
	if ($temoin) $ajout_titre= "(<em>avec protagonistes</em>)";
		  $nouveauItem->titre="Treat the continuations of an incident ".$ajout_titre;
		  $nouveauItem->expli="Treat the continuations of an incident: to define a punishment or a
sanction" ;
		  $nouveauItem->indexMenu=$a;
		  $menuPage[]=$nouveauItem;
	  }
...
/* ===== Titre du menu ===== */
  ...
  $menuTitre[$a]->icone['titre']='';
  $menuTitre[$a]->icone['alt']="";
  $menuTitre[$a]->texte='Consult the incidents';
  $menuTitre[$a]->indexMenu=$a;
...
/*	echo "<tr>\n";
	...
	*/
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="List sanctions of the day" ;
		$nouveauItem->expli="Visualize the list of the sanctions of the day (<em>Exclusion, reserve,...</em>)" ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
/*
	...
	*/
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Accede with the statistics" ;
		$nouveauItem->expli="Select the period of treatment, data to be treated (<em>establishment, classes, student,...</em>) while applying (<em>or not</em>) filters in order to obtain more or less detailed assessments. <br />Visualize the evolutions in the shape of graphs. To publish Signal
10, ..." ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
...
/*
	...
	*/
	...
	if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	{
		$nouveauItem->titre="Carry out research/various statistics" ;
		$nouveauItem->expli="To be able to list the incidents having had such student for
protagonist (<em>by specifying or not the role in the incident</em>), the number of work, reserves, exclusions,... between such and such date,..." ;
		$nouveauItem->indexMenu=$a;
		$menuPage[]=$nouveauItem;
	}
	unset($nouveauItem);
...
elseif (($_SESSION['statut']=='professeur') || ($_SESSION['statut']=='autre')) {
	...
	if((mysql_num_rows($test)>0)) { //on a bien un prof ou statut autre comme déclarant ou un protagoniste
	  ...
	  if ($nouveauItem->acces($nouveauItem->chemin,$_SESSION['statut']))
	  {
		  $nouveauItem->titre="Consult the continuations of the incidents" ;
		  $nouveauItem->expli="Visualize the list of the declared incidents and their treatments." ;
		  $nouveauItem->indexMenu=$a;
		  $menuPage[]=$nouveauItem;
	  }
	unset($nouveauItem);
		...
		*/
		...
		//unset($nouveauItem);
		   ...
		   */
			...
			$nouveauItem->titre="Consult the continuations of the incidents" ;
			$nouveauItem->expli="No incident (<em>with protagonist</em>) you concerning is not still declared." ;
			$nouveauItem->indexMenu=$a;
			$menuPage[]=$nouveauItem;
...
echo "<ul>\n";
echo "<li><p>Once the incident is closed, il ne peut plus être modifié et aucune sanction liée ne peut être ajoutée/modifiée/supprimée.</p></li>\n";
