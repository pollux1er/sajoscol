<?php
/*
 * $Id: param_courrier_html.php 6727 2011-03-29 15:14:30Z crob $
 *
...
$titre_page = "Parameters of configuration of report cards HTML";
...
if (isset($_POST['disc_textsize'])) {
    ...
    if (!saveSetting("disc_textsize", $_POST['disc_textsize'])) {
        $msg .= "Error during the recording de disc_textsize !";
        $reg_ok = 'no';
    }
}
...
if (isset($_POST['disc_p_html_margin'])) {
    ...
    if (!saveSetting("disc_p_html_margin", $_POST['disc_p_html_margin'])) {
        $msg .= "Error during the recording of disc_p_html_margin !";
        $reg_ok = 'no';
    }
}
...
if (isset($_POST['disc_body_marginleft'])) {
    ...
    if (!saveSetting("disc_body_marginleft", $_POST['disc_body_marginleft'])) {
        $msg .= "Error during the recording of disc_body_marginleft !";
        $reg_ok = 'no';
    }
}
...
if (isset($_POST['disc_logo'])) {
    if (!saveSetting("disc_logo", $_POST['disc_logo'])) {
        $msg .= "Error during the recording of disc_logo !";
        $reg_ok = 'no';
    }
}
...
if(isset($_POST['disc_affiche_tel'])) {
    if (!saveSetting("disc_affiche_tel", $_POST['disc_affiche_tel'])) {
        $msg .= "Error during the recording of disc_affiche_tel !";
        $reg_ok = 'no';
    }
}
...
if(isset($_POST['disc_affiche_fax'])) {
    if (!saveSetting("disc_affiche_fax", $_POST['disc_affiche_fax'])) {
        $msg .= "Error during the recording of disc_affiche_fax !";
        $reg_ok = 'no';
    }
}
...
if (isset($_POST['disc_addressblock_padding_right'])) {
    ...
    if (!saveSetting("disc_addressblock_padding_right", $_POST['disc_addressblock_padding_right'])) {
        $msg .= "Error during the recording of disc_addressblock_padding_right !";
        $reg_ok = 'no';
    }
}
...
if (isset($_POST['disc_addressblock_padding_top'])) {
    ...
    if (!saveSetting("disc_addressblock_padding_top", $_POST['disc_addressblock_padding_top'])) {
        $msg .= "Error during the recording of disc_addressblock_padding_top !";
        $reg_ok = 'no';
    }
}
...
if (isset($_POST['disc_addressblock_padding_text'])) {
    ...
    if (!saveSetting("disc_addressblock_padding_text", $_POST['disc_addressblock_padding_text'])) {
        $msg .= "Error during the recording of disc_addressblock_padding_text !";
        $reg_ok = 'no';
    }
}
...
if (isset($_POST['disc_addressblock_length'])) {
    ...
    if (!saveSetting("disc_addressblock_length", $_POST['disc_addressblock_length'])) {
        $msg .= "Error during the recording of disc_addressblock_length !";
        $reg_ok = 'no';
    }
}
...
if (isset($_POST['disc_affich_nom_etab'])) {
	...
	if (!saveSetting("disc_affich_nom_etab", $disc_affich_nom_etab)) {
		$msg .= "Error during the recording of disc_affich_nom_etab !";
		$reg_ok = 'no';
	}
}
...
if (isset($_POST['disc_affich_adr_etab'])) {
	...
	if (!saveSetting("disc_affich_adr_etab", $disc_affich_adr_etab)) {
		$msg .= "Error during the recording of disc_affich_adr_etab !";
		$reg_ok = 'no';
	}
}
...
// End standart header
//**************** EN-TETE *****************
$titre_page = "Discipline: Parameters mails";
require_once("../lib/header.inc");
//**************** FIN EN-TETE *****************

echo "<p class='bold'><a href='index.php'><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return </a></p>\n";
...
echo "<h3>Page-setting of the mails</h3>\n";
echo "<table cellpadding='8' cellspacing='0' width='100%' border='0' summary='Page-setting'>\n";
...
echo "<label for='bull_body_marginleft' style='cursor: pointer;'>Stroke left of the page (en pixels) :</label>\n";
echo "</td>\n";
...
echo ">\n";
echo "<td style='font-variant: small-caps;'>\n";
echo "<label for='disc_textsize' style='cursor: pointer;'>size in points of the text (en pixels) :</label>\n";
echo "</td>\n";
...
echo ">\n";
echo "<td style='font-variant: small-caps;'>\n";
echo "<label for='disc_p_html_margin' style='cursor: pointer;'>High and low margins of the paragraphs in points of the text :</label>\n";
echo "</td>\n";
...
echo ">\n";
echo "<td style='font-variant: small-caps;'>\n";
echo "display the logo of the establishment :\n";
echo "<br /><span style='color:red;'>Test if a logo is defined</span>";
echo "</td>\n";
...
echo ">\n";
echo "<td style='font-variant: small-caps;'>\n";
echo "Make apparaitre the name of the establishment on the bulletin :<br />(<i>some establishments have the name in the Logo</i>) :\n";
echo "</td>\n";
...
echo ">\n";
echo "<td style='font-variant: small-caps;'>\n";
echo "Make apparaitre the address of the establishment on the bulletin :<br />(<i>some establishments have the address in the Logo</i>) :\n";
echo "</td>\n";
...
echo ">\n";
echo "<td style='font-variant: small-caps;'>\n";
echo "Display the phone number of the establishment :\n";
echo "</td>\n";
...
echo ">\n";
echo "<td style='font-variant: small-caps;'>\n";
echo "display the number of fax of the establishment :\n";
echo "</td>\n";
...
echo "<hr />\n";
//============================================
echo "<h3>Block addresses responsible</h3>\n";

echo "<center><table border='1' cellpadding='10' width='90%' summary='Bloc adresse'><tr><td>
These options control the positioning of the block addresses
responsible pupil directly on the mails.
</td></tr></table></center>\n";
...
?>
    ...
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor; ?>>
        ...
        <label for='disc_addressblock_padding_right' style='cursor: pointer;'>Space in mm between the right margin of the sheet and the block "address" :</label>
        </td>
    ...
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor;$nb_ligne++; ?>>
        <td colspan="2"><i>Take account of the right margin of impression to calculate space
between the flat rim of the sheet and the block addresses</i></td>
     </tr>
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor;?>>
        <td style="font-variant: small-caps;">
        <label for='disc_addressblock_padding_top' style='cursor: pointer;'>Space in mm between the high margin of the sheet and the block "address" :</label>
        </td>
    ...
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor;$nb_ligne++; ?>>
        <td colspan="2"><i>Take account of the high margin of impression to calculate space
between the high edge of the sheet and the block addresses</i></td>
    </tr>
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor;$nb_ligne++; ?>>
        <td style="font-variant: small-caps;">
        <label for='disc_addressblock_padding_text' style='cursor: pointer;'>Vertical space in mm between the block "addresses" and the block of
the results :</label>
        </td>
    ...
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor;$nb_ligne++; ?>>
        <td style="font-variant: small-caps;">
        <label for='disc_addressblock_length' style='cursor: pointer;'>Length in mm of the block "adress" :</label>
        </td>
    ...
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor;$nb_ligne++; ?>>
        <td style="font-variant: small-caps;">
        <label for='disc_addressblock_font_size' style='cursor: pointer;'>Cut in points of the texts of the block "adress" :</label>
        </td>
	<?php
    ...
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor;$nb_ligne++; ?>>
        <td style="font-variant: small-caps;">
        <label for='disc_addressblock_logo_etab_prop' style='cursor: pointer;'>Proportion (in % of the width of page) allocated with the logo and the
address of the establishment :</label>
        </td>
	<?php
    ...
    <tr <?php if ($nb_ligne % 2) echo "bgcolor=".$bgcolor;$nb_ligne++; ?>>
        <td style="font-variant: small-caps;">
        <label for='disc_addressblock_classe_annee' style='cursor: pointer;'>Proportion (in % of the width of page) allocated with the block
"Classifies, year, period" :</label>
        </td>
	<?php
