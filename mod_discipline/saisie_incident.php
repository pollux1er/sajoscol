...
/*
 * $Id: saisie_incident.php 8743 2012-01-15 17:28:17Z crob $
 *
...
function choix_heure($champ_heure,$div_choix_heure) {
	...
	if(mysql_num_rows($res_abs_cren)>0) {
		echo " <a href='#' onclick=\"afficher_div('$div_choix_heure','y',10,-40); return false;\">Choice</a>";

		$texte="<table class='boireaus' style='margin: auto;' border='1' summary=\"One hour choice\">\n";
...
function recherche_ele($rech_nom,$page) {
	...
	if($nb_ele==0){
		...
		echo "<p>No name of student contains the chains $rech_nom.</p>\n";
	}
	else{
		//echo "<p>La recherche a retourné <b>$nb_ele</b> réponse(s):</p>\n";
		echo "<p>Research turned over <b>$nb_ele</b> r&eacute;ponse";
		if($nb_ele>1) {echo "s";}
		echo ":</p>\n";
...
function recherche_utilisateur($rech_nom,$page) {
	...
	if($nb_utilisateur==0){
		// On ne devrait pas arriver là.
		echo "<p>No name of user contains the chains $rech_nom.</p>\n";
	}
	else{
		echo "<p>Research turned over <b>$nb_utilisateur</b> answer";
	...
	if((isset($_POST['suppr_ele_incident']))&&(isset($id_incident))) {
		...
		for($i=0;$i<count($suppr_ele_incident);$i++) {
			...
			if(mysql_num_rows($test_sanction)>0) {
				$msg.="ERROR: It is not possible to remove ".$suppr_ele_incident[$i]." for the incident $$id_incident>because one or sanctions are taken. You must initially remove the associated sanctions.<br />\n";
			}
			else {
				...
				updateOnline($sql);
				if(!$menage) {
					$msg.="ERROR during the suppression of the treatments associated with ".$suppr_ele_incident[$i]." for the incident $id_incident. Required measurements or catches pose a problem.<br />\n";
				}
				else {
					...
					if(!$res) {
						$msg.="ERROR during the suppression of ".$suppr_ele_incident[$i]." for the incident $id_incident<br />\n";
					}
				}
			}
		}
			...
			for($i=0;$i<$nb_protagonistes;$i++) {
				if(isset($ele_login[$i])) {
					...
					if(mysql_num_rows($res)==0) {
						...
						$res=mysql_query($sql);updateOnline($sql);
						if(!$res) {
							$msg.="ERROR during the recording of ".$ele_login[$i]."<br />\n";
						}
					}
					else {
					...
					if(isset($avertie[$i])) {
						...
						if(!$update) {
							echo "Failure of the recording for the family of ".$ele_login[$i].".";
						}
					}
				}
			}
			...
			for($i=0;$i<$nb_protagonistes;$i++) {
				if(isset($u_login[$i])) {
					...
					if(mysql_num_rows($res)==0) {
						...
						$res=mysql_query($sql);updateOnline($sql);
						if(!$res) {
							$msg.="ERROR during the recording of ".$u_login[$i]."<br />\n";
						}
					}
					else {
		...
		if((!isset($_POST['recherche_eleve']))&&(!isset($_POST['recherche_utilisateur']))) {
			...
			if(!isset($id_incident)) {
				...
				if(!checkdate($mois,$jour,$annee)) {
					...
					$msg.="The date suggested was not valid. It was replaced by the current
current date.";
				}
				...
				if(!$res) {
					...
					$msg.="Recording of the incident n°".$id_incident." carried out.<br />\n";
				}
	
				$texte_mail="Seizure by ".civ_nom_prenom($_SESSION['login'])." of an incident (n°$id_incident) survenu le $jour/$mois/$annee à $display_heure:\n";
...
current date.";
				...
				if(isset($nature)) {
					...
					if(mysql_num_rows($res2)>0) {
						...
						if (count($tab_res)>1) {$msg.="There are several affected categories with this nature. First is
retained for this incident. You should update your categories of incidents.<br />";}
			...
			//	}
				...
				if($temoin_modif=="y") {
					...
					$res=mysql_query($sql);
					if(!$res) {
						$msg.="ERROR at the time of the update of the incident ".$id_incident."<br />\n";
					}
					else {
						$msg.="Update of the incident n°".$id_incident." effectuée.<br />\n";
					}
				}
		        ...
		        if(mysql_num_rows($res_declarant)>0) {
			        ...
			        $texte_mail= "Initial declaration of the incident by ".u_p_nom($lig_decclarant->declarant)."\n";
		        }
	
				$texte_mail.="Update by ".civ_nom_prenom($_SESSION['login'])." of an incident (n°$id_incident)";
				if(isset($display_heure)) {
					$texte_mail.=" occurred it $jour/$mois/$annee à/en $display_heure:\n";
				}
				if(isset($nature)) {
					$texte_mail.="\nNature: $nature\nDescription: $description\n";
				}
			...
			if(isset($id_incident)) {
				...
				for($i=0;$i<count($u_login);$i++) {
					...
					if(mysql_num_rows($res)==0) {
						...
						if(mysql_num_rows($res_statut)>0) {
							...
							$msg.="ERROR during the recording of ".$u_login[$i].": unknown statute???<br />\n";
						}
					}
				}
					...
					for($i=0;$i<count($mesure_ele_login);$i++) {
						...
						for($loop=0;$loop<count($suppr_doc_joint);$loop++) {
							if((preg_match("/\.\./",$suppr_doc_joint[$loop]))||(preg_match("#/#",$suppr_doc_joint[$loop]))) {
								$msg.="File name ".$suppr_doc_joint[$loop]." invalid<br />";
							}
							else {
								...
								if(!unlink($fichier_courant)) {
									$msg.="Error during the suppression of $fichier_courant<br />";
								}
							}
						}
						...
						if((isset($document_joint['tmp_name']))&&($document_joint['tmp_name']!="")) {
							...
							*/
							...
							if(!is_uploaded_file($document_joint['tmp_name'])) {
								$msg.="The upload of the file failed.<br />\n";
							}
							else{
								if(!file_exists($document_joint['tmp_name'])) {
									if($document_joint['name']!="") {
										...
										if(!in_array($extension, $AllowedFilesExtensions)) {
											$msg.="You proposed : ".$document_joint['name']."<br />Extension $extension is not authorized.<br />\n";
										}
										else {
											$msg.="The file would have been uploadé... but would not be present/preserved.<br />\n";
										}
									}
									else {
										$msg.="The file would have been uploadé... mboard would not be present/preserved.<br />\n";
										$msg.="It may be that the extension of the file proposed is not authorized.<br />\n";
										$msg.="The authorized types are ".array_to_chaine($AllowedFilesExtensions)."<br />";
									}
								}
								else {
									...
									if(!$res_copy) {$msg.="Failure of the installation of the file ".$document_joint['name']."<br />";}
								}
							}
						}
								...
								*/
								if(in_array($lig_mes->id,$tab_mes_enregistree)) {
									if((!in_array($lig_mes->id,$mesure_prise))&&
										...
										if(!$suppr) {
											$msg.="ERROR during the suppression of measurement ".$lig_mes->mesure." pour ".$mesure_ele_login[$i]."<br />\n";
										}
									}
								}
								else {
									...
									if((in_array($lig_mes->id,$mesure_prise))||
										...
										$insert=mysql_query($sql);
										if(!$insert) {
											$msg.="ERROR during the measurement recording ".$lig_mes->mesure." for ".$mesure_ele_login[$i]."<br />\n";
										}
									}
				...
				if(isset($clore_incident)) {
					...
					$update=mysql_query($sql);
					if(!$update) {
						$msg.="ERROR at the time of the incident n° closure $id_incident.<br />\n";
					}
					else {
						$msg.="Enclose incident n° $id_incident.<br />\n";
					}
				}
...
//**************** EN-TETE *****************
$titre_page = "Discipline: Announce an incident";
...
($_SESSION['statut']=='scolarite')) {
	...
	$test=mysql_query($sql);
	if(mysql_num_rows($test)>0) {
		echo " | <a href='incidents_sans_protagonistes.php' onclick=\"return confirm_abandon (this, change, '$themessage')\">Incidents without protagonists</a>\n";
		echo " | <a href='traiter_incident.php' onclick=\"return confirm_abandon (this, change, '$themessage')\">List incidents</a> (<i>with protagonists</i>)\n";
	}
	else {
		echo " | <a href='traiter_incident.php' onclick=\"return confirm_abandon (this, change, '$themessage')\">List incidents</a>\n";
	}
}
elseif (($_SESSION['statut']=='professeur')||
...
if ($step==2) {   //Eric Ajout génération du modèle Ooo pour imprimer le rapport d'incident.
    echo " | <a href='../mod_ooo/rapport_incident.php?mode=module_discipline&amp;id_incident=$id_incident".add_token_in_url()."' onclick=\"return confirm_abandon (this, change, '$themessage')\">Print the incident report</a>\n";
}
...
if(isset($id_incident)) {
	...
	if(mysql_num_rows($res)>0) {
		...
		echo "<p class='bold'>Protagonists of the incident n°$id_incident&nbsp;:</p>\n";

		echo "<blockquote>\n";

		echo "<table class='boireaus' border='1' summary='Protagonists'>\n";
		echo "<tr>\n";
		echo "<th>Individual</th>\n";
		echo "<th>Statute</th>\n";
		//echo "<th>Qualité dans l'incident</th>\n";
		echo "<th>Role in the incident</th>\n";
			...
			*/
			...
			((($_SESSION['statut']=='professeur')&&(getSettingValue('imprDiscProfRetenueOOo')=='yes'))||($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($_SESSION['statut']=='cpe'))) {
				...
				if ($lig->qualite=='Responsable') { //un retenue seulement pour un responsable !
		            echo "<a href='../mod_ooo/retenue.php?mode=module_discipline&amp;id_incident=$id_incident&amp;ele_login=$lig->login".add_token_in_url()."' title='Print reserve'><img src='../images/icons/print.png' width='16' height='16' alt='Print Reserve' /></a>\n";
				}
                echo "</td>";
		    }
				...
				if($lig->statut=='eleve') {
					...
					*/
					...
					if($etat_incident!='clos') {
						...
						echo "<a href='avertir_famille.php?id_incident=$id_incident&amp;ele_login=$lig->login' title='Avertir par courrier'><img src='../images/icons/saisie.png' width='16' height='16' alt='Inform' /></a>\n";
					}
					else {
	...
	setTimeout('check_protagonistes_sans_qualite()',10000);
		...
		if($etat_incident!='clos') {
			...
			echo "<p><input type='submit' name='enregistrer_qualite' value='Record' /></p>\n";
		...
		if($step!=2) {
			...
			if(mysql_num_rows($test)==0) {
				...
				echo "'";
				echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
				echo ">préciser l'incident</a> after addition of the protagonists.</p>\n";
			}
		}
		echo "</blockquote>\n";
	...
	//]]>
		...
		echo "<p style='color:red;'>No protagonist has (<i>encore</i>) summer specified for this incident .</p>\n";
	}
}
else {
	echo "<p class='bold'>Protagonists of the incident &nbsp;:</p>\n";
	echo "<blockquote>\n";
	echo "<p style='color:red;'>No protagonist has (<i>still</i>) summer specified for this incident.</p>\n";
	echo "</blockquote>\n";
}
...
if($step==0) {
	...
	<p>
	To post the student of which it <b>nom</b> contient: <input type='text' name='rech_nom' value='' />
	...
	if(isset($_POST['recherche_eleve'])) {
		...
		echo "<p><input type='submit' name='Ajouter' value='Add' /></p>\n";
	...
	elseif(isset($id_classe)) {
		...
		if(mysql_num_rows($res_ele)>0) {
			echo "<p>student of the class of ".get_class_from_id($id_classe)."&nbsp;:</p>\n";
			...
			echo "<table width='100%' summary=\"Table of choice of the student\">\n";
	...
	if ($_SESSION['statut']!='autre') { //statut autre : ajout Eric de la condition 
		$res_clas=mysql_query($sql);
		if(mysql_num_rows($res_clas)>0) {
			echo "<p>Or to choose a student in a class:</p>\n";
...
elseif($step==1) {
	...
	echo "<p class='bold'>Add personnel&nbsp;:</p>\n";
	...
	if(mysql_num_rows($res)==0) {
		...
		echo "<p style='color:red;'>The table 'utilisateurs' do not comprise any account???</p>\n";
		echo "<p><br /></p>\n";
	...
	elseif(isset($categ_u)) {
		...
		echo "<table width='100%' summary=\"Table of choice of $categ_u\">\n";
		echo "<tr valign='top' align='center'>\n";
		echo "<td align='left'>\n";
	...
	echo "<p class='bold'>Choose a category of personnel&nbsp;:</p>\n";
	echo "<blockquote>\n";
	while($lig=mysql_fetch_object($res)) {
...
elseif($step==2) {
	...
	echo "<p class='bold'>Details of the incident";
	if(isset($id_incident)) {
		echo " n°$id_incident";
		...
		if(mysql_num_rows($res_dec)>0) {
			...
			echo " (<span style='font-size:x-small; font-style:italic; color:red;'>announced by ".u_p_nom($lig_dec->declarant)."</span>)";
		}
	}
	echo "&nbsp;:</p>\n";
	...
	if($etat_incident!='clos') {
		...
		echo "<noscript><input type='submit' name='enregistrer' value='Really record' /></noscript>\n";
		echo "</td>\n";
	}
	else {
		echo "<td style='text-align:left;'>\n";
	...
	if($etat_incident!='clos') {
		$texte="CAUTION&nbsp;: Should not be spent an empty hour.<br />In the event of uncertainty immediately, it is to better put a
question mark in own way of hour.<br />If not, if there is no uncertainty, the time crenel M1, S2,... is preferable.";
		$tabdiv_infobulle[]=creer_div_infobulle("div_infobulle_avertissement_heure","Not empty hour","",$texte,"",30,0,'y','y','n','n',2);
	...
	*/
	...
	echo "<tr class='lig$alt'>\n";
	echo "<td style='font-weight:bold;vertical-align:top;text-align:left;'>\n";
	echo "Nature of the incident <span style='color:red;'>(*)</span>&nbsp;:";
	echo "</td>\n";
	echo "<td style='text-align:left;'";
	if($etat_incident!='clos') {echo " colspan='2'";}
	echo ">\n";
...
new Ajax.Autocompleter (
	...
	{method: 'post', paramName: 'nature'}
			...
			if(mysql_num_rows($res_nat)>0) {
				...
				$texte="Click to choose a nature existante.<br/> Or if no nature is already
defined, seize the nature of incident of your choice.";
...
<img src='../images/icons/close16.png' width='16' height='16' alt='Fermer' />
	...
	echo "<tr class='lig$alt'>\n";
	echo "<td style='font-weight:bold;vertical-align:top;text-align:left;'>\n";
	echo "Description of the incident&nbsp;:&nbsp;";
    ...
    echo "</div>";
	$texte="Detailed account of the facts (marked words, gestes, reactions, ...)<br />";
...
setTimeout('comptage_caracteres_textarea()', 1000);
	...
	if ($autorise_commentaires_mod_disc=="yes") {
		...
		$texte="This zone of text is available to dialogue with the school life of the
particular methods of treatment of the incident or to follow the
continuations of this one.<br /> Timetable and place of a reserve requested, request for convocation of
the student by the CPE, ...";
		$tabdiv_infobulle[]=creer_div_infobulle('div_explication_commentaires',"Zone of text : dialogue","",$texte,"",18,0,'y','y','n','n');
		...
		if($etat_incident!='clos') {
			...
			if((count($tab_mes_prise)>0)||(count($tab_mes_demandee)>0)) {
				...
				if(count($tab_mes_demandee)>0) {
					...
					$texte="Required measurements are it by professors.<br />";
					$texte.="An account cpe or schooling can then seize the corresponding sanction
if it considers the request suitable.<br />";
					$texte.="<br />";
					$texte.="There is no for a CPE to notch one of these boxes.<br />";
					$texte.="It is to better pass to the seizure while following the
Traitement/sanction bond in top on the right.<br />";
					$tabdiv_infobulle[]=creer_div_infobulle("div_mesures_demandees","Measurements requested","",$texte,"",30,0,'y','y','n','n');
					...
					echo "Work and/or document joint with a measurement requested";
					echo "</th>\n";
				}
				echo "</tr>\n";
				...
				echo "<p>No type of measurement is defined.</p>\n";
			}
	...
	if($etat_incident!='clos') {
		...
		echo "<label for='clore_incident' style='cursor:pointer;'>&nbsp;Close the incident.</label>\n";
		//echo "<br />";
		echo "<em style='font-size:x-small;'>(subject to not <strong>Ask</strong> of measurement)</em>\n";
		echo "</p>\n";
		...
		echo "<p style='text-align:center;'><input type='button' name='enregistrer2' value='Record' onclick='verif_details_incident();' /></p>\n";
		echo "<noscript><input type='submit' name='enregistrer2' value='Really record' /></noscript>\n";

		//echo "<p style='text-align:center;'><input type='submit' name='enregistrer2' value='Enregistrer' onsubmit='verif_details_incident();' /></p>\n";


		echo "<p><em>NOTE&nbsp;</em> <span style='color:red;'>(*)</span> It is imperative to seize a Nature of incident for questions of
facility of treatment thereafter.</p>\n";
	}
	echo "</blockquote>\n";
...
";
		...
		echo "
			alert(\"The nature of the incident must be specified.\");
			return false;
		}
		else {
			if(document.getElementById('display_heure').value=='') {
				alert(\"The hour of the incident (not empty) must be specified. \\nIn case of doubt immediately, Put one '?'.\");
				return false;
			}
			else {
