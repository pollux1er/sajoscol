...
/*
 * $Id: saisie_sanction.php 7994 2011-08-26 10:03:39Z crob $
 *
		...
		if(isset($id_sanction)) {
			...
			if ($report_demande=="OK") { // c'est un report
				...
				if(mysql_num_rows($res)==0) {
					$msg.="Reserve n°$id_sanction does not exist in 's_retenues'.<br />It cannot be deferred.<br />";
				}
				else {
				...
				if(!$res) {
					$msg.="Error during the insertion of information of carryforward in 's_reports'.<br />";
				}
			}
			...
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				$msg.="The sanction n°$id_sanction do not exist in 's_sanctions'.<br />It cannot be updated.<br />";
			}
			else {
				...
				$res=mysql_query($sql);
				if(mysql_num_rows($res)==0) {
					$msg.="Reserve n°$id_sanction n'existe pas dans 's_retenues'.<br />It cannot be updated.<br />";
				}
				else {
					//Eric
					...
					if(!$update) {
						$msg.="Error at the time of the update of exclusion n°$id_sanction.<br />";
					}
				}
			}
		}
			...
			$res=mysql_query($sql);updateOnline($sql);
			if(!$res) {
				$msg.="Error during the insertion of the sanction in 's_sanctions'.<br />";
			}
			else {
			...
			if(count($autre_protagoniste_meme_sanction)>0) {
				for($loop=0;$loop<count($autre_protagoniste_meme_sanction);$loop++) {
					...
					$res=mysql_query($sql);updateOnline($sql);
					if(!$res) {
						$msg.="Error during the insertion of the sanction in 's_sanctions' for $autre_protagoniste_meme_sanction[$loop].<br />";
					}
					else {
...
current date.";
		...
		if($tmp_timestamp_debut>$tmp_timestamp_fin) {
			...
			$msg.="The completion date was former to the date of beginning of the
exclusion.<br/> the dates were inverted.<br />";
		}
		...
		if(isset($id_sanction)) {
			...
			if(mysql_num_rows($res)==0) {
				...
				$res=mysql_query($sql);
				if(mysql_num_rows($res)==0) {
					$msg.="The sanction n°$id_sanction do not exist in 's_exclusions'.<br />It cannot be updated.<br />";
				}
				else {
...
current date.";
		...
		if(isset($id_sanction)) {
			...
			if(mysql_num_rows($res)==0) {
				...
				$res=mysql_query($sql);
				if(mysql_num_rows($res)==0) {
					$msg.="Work n°$id_sanction do not exist in 's_travail'.<br />It cannot be updated.<br />";
				}
				else {
		...
		if(mysql_num_rows($res)>0) {
			...
			if(isset($id_sanction)) {
				...
				if(mysql_num_rows($res)==0) {
					...
					$res=mysql_query($sql);
					if(mysql_num_rows($res)==0) {
						$msg.="The sanction n°$id_sanction does not exist in 's_autres_sanctions'.<br />It cannot be updated.<br />";
					}
					else {
						...
						$update=mysql_query($sql);
						if(!$update) {
							$msg.="Error at the time of the update of the sanction '$type_sanction' n°$id_sanction.<br />";
						}
					}
				}
			}
				...
				if(!$res) {
					...
					$res=mysql_query($sql);
					if(!$res) {
						$msg.="Error during the recording of the sanction '$type_sanction' n°$id_sanction.<br />";
					}
				}
				...
				if(count($autre_protagoniste_meme_sanction)>0) {
					for($loop=0;$loop<count($autre_protagoniste_meme_sanction);$loop++) {
						...
						if(!$res) {
							...
							$res=mysql_query($sql);
							if(!$res) {
								$msg.="Error during the recording of the sanction '$type_sanction' n°$tmp_id_sanction.<br />";
							}
						}
					}
				}
		...
		if((isset($document_joint['tmp_name']))&&($document_joint['tmp_name']!="")) {
			...
			if(!is_uploaded_file($document_joint['tmp_name'])) {
				...
				if(!file_exists($document_joint['tmp_name'])){
					$msg.="The file would have been uploadé... but would not be present/conserved.<br />\n";
				}
				else {
...
$chaine_date = "of $date_debut";
$journee = "the day";
} else {
$chaine_date = "of $date_debut to $date_fin inclus";
$journee = "days";
}
...
echo "<script type='text/javascript'>
	// The function is brought back of saisie_sanction.inc.php dans saisie_sanction.php
	// because them javascript defined in the page saisie_sanction.inc.php called via ajax are not taken into account.
...
echo ">List incidents</a>";
echo "</li>\n";

echo "<li>\n";
echo "<a href='liste_sanctions_jour.php'";
echo " onclick='return confirm_abandon (this, change, \"$themessage\")'";
echo ">List sanctions of the day</a>";
echo "</li>\n";
echo "</ul>\n";
echo "</div>\n";
//=====================================================
...
echo " onclick='return confirm_abandon (this, change, \"$themessage\")'";
echo "><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return index</a>\n";

if(!isset($id_incident)) {
	echo "</p>\n";

	echo "<p><strong>Error&nbsp;:</strong> It is necessary to start by selecting the incident.</p>\n";
...
//
					...
					while($lig_sanction=mysql_fetch_object($res_sanction)) {
						...
						//echo "<td>Retenue</td>\n";
						echo "<td><a href='".$_SERVER['PHP_SELF']."?mode=modif&amp;valeur=retenue&amp;id_sanction=$lig_sanction->id_sanction&amp;id_incident=$id_incident&amp;ele_login=$lig->login'>Reserve</a></td>\n";
						...
						if($tmp_doc_joints!="") {
							if($texte!="") {$texte.="<br />";}
							$texte.="<b>Joined documents</b>&nbsp;:<br />";
							$texte.=$tmp_doc_joints;
						}

						$tabdiv_infobulle[]=creer_div_infobulle("div_travail_sanction_$lig_sanction->id_sanction","Work (sanction n°$lig_sanction->id_sanction)","",$texte,"",20,0,'y','y','n','n');
...
//Eric
						...
						if ($gepiSettings['active_mod_ooo'] == 'y') { //impression avec mod_ooo
							echo "<a href='../mod_ooo/retenue.php?mode=module_retenue&amp;id_incident=$id_incident&amp;id_sanction=$lig_sanction->id_sanction&amp;ele_login=$lig->login".add_token_in_url()."' title='Print reserve'><img src='../images/icons/print.png' width='16' height='16' alt='Print Reserve' /></a>\n";
						}
						else {
							echo "-";
						}
						echo "</td>\n";
//
						echo "<td><a href='".$_SERVER['PHP_SELF']."?mode=suppr_sanction&amp;id_sanction=$lig_sanction->id_sanction&amp;id_incident=$id_incident".add_token_in_url()."' title='Remove the sanction n°$lig_sanction->id_sanction'><img src='../images/icons/delete.png' width='16' height='16' alt='Remove the sanction n°$lig_sanction->id_sanction' /></a></td>\n";
						echo "</tr>\n";
					}
					echo "</table>\n";
				}
...
//Eric						
						echo "<td>";
						if ($gepiSettings['active_mod_ooo'] == 'y') { //impression avec mod_ooo
							echo "<a href='".$_SERVER['PHP_SELF']."?odt=exclusion&amp;id_sanction=$lig_sanction->id_sanction&amp;id_incident=$id_incident&amp;ele_login=$lig->login".add_token_in_url()."' title='Imprimer la sanction n°$lig_sanction->id_sanction'><img src='../images/icons/print.png' width='16' height='16' alt=\"Print the document\" /></a>\n";
						}
						else {
							echo "-";
						}
						...
						echo "<td><a href='".$_SERVER['PHP_SELF']."?mode=suppr_sanction&amp;id_sanction=$lig_sanction->id_sanction&amp;id_incident=$id_incident".add_token_in_url()."' title='Supprimer la sanction n°$lig_sanction->id_sanction'><img src='../images/icons/delete.png' width='16' height='16' alt='Remove the sanction n°$lig_sanction->id_sanction' /></a></td>\n";
						echo "</tr>\n";
					}
					echo "</table>\n";
				}
				...
				echo "<a href='".$_SERVER['PHP_SELF']."?id_incident=$id_incident&amp;ele_login=$lig->login&amp;mode=ajout' title='Add a sanction'><img src='../images/icons/add.png' width='16' height='16' alt='Add a sanction' /></a>";
			}
			else {
		...
		echo "<p>No protagonist has (<i>encore</i>) summer specified for this incident.</p>\n";
	}
...
elseif($mode=='ajout') {
	...
	function edt_eleve() {
		// With the call Ajax at the time of an Addition of sanction, one does not manage to recover the value of ele_login
	...
	echo "<p class='bold'>Addition of a sanction for ".p_nom($ele_login);
	...
	echo "<p class='bold'>Nature of the sanction&nbsp;:<br />\n";
	...
	*/
	...
	echo "<script type='text/javascript'>
	// With this function, one does nothing but add fields in the form (no
recording before validation of the form)
...
elseif($mode=='modif') {
	...
	echo "<p class='bold'>Sanction (<em>$traitement</em>) n°$id_sanction concerning ".p_nom($ele_login);
	echo infobulle_photo($ele_login);
	echo "&nbsp;:</p>\n";
	echo "<blockquote>\n";
	...
	echo "<p>Euh... it is not expected that one arrives there (for the moment;o).</p>\n";
}
