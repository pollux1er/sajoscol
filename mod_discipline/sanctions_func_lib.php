<?php
/*
$Id: sanctions_func_lib.php 8699 2011-12-03 11:17:04Z crob $
*/
...
function rappel_incident($id_incident) {
	...
	if(isset($id_incident)) {
		...
		if(mysql_num_rows($res_dec)>0) {
			...
			echo " (<span style='font-size:x-small; font-style:italic;'>announced by ".u_p_nom($lig_dec->declarant)."</span>)";
		}
	}
	echo "&nbsp;:</p>\n";
	echo "<blockquote>\n";
	...
	if(mysql_num_rows($res_incident)>0) {
		...
		echo "<tr class='lig-1'><td style='font-weight:bold;vertical-align:top;text-align:left;'>Hour: </td><td style='text-align:left;'>$lig_incident->heure</td></tr>\n";

		echo "<tr class='lig1'><td style='font-weight:bold;vertical-align:top;text-align:left;'>Place: </td><td style='text-align:left;'>";
		/*
		...
		*/
		...
		echo "<p>The incident n°$id_incident do not seem not recorded???</p>\n";
	}
	echo "</blockquote>\n";
}
...
function tab_lignes_adresse($ele_login) {
	...
	if(mysql_num_rows($res)==0) {
		return "No address of responsible for this pupil.";
	}
	else {
		$tab_resp=array();
		...
		if (!isset($tab_resp[0])) {
			$tab_adr_ligne1[0]="<font color='red'><b>MISSING ADDRESS</b></font>";
			$tab_adr_ligne2[0]="";
			$tab_adr_ligne3[0]="";
		}
		else {
...
function tab_mod_discipline($ele_login,$mode,$date_debut,$date_fin) {
	...
	$res=mysql_query($sql);
	if(mysql_num_rows($res)>0) {
		$retour="<p>Table of the incidents concerning ".p_nom($ele_login)."</p>\n";
		$retour.="<table class='boireaus' border='1' summary='Table of the incidents concerning $ele_login'>\n";
		$retour.="<tr>\n";
		$retour.="<th>Num</th>\n";
		$retour.="<th>Date</th>\n";
		$retour.="<th>Quality</th>\n";
		$retour.="<th>Description</th>\n";
		$retour.="<th>Follow-up</th>\n";
		$retour.="</tr>\n";
		$alt_1=1;
		...
		while($lig=mysql_fetch_object($res)) {
			...
			*/
			...
			$res_prot=mysql_query($sql);
			if(mysql_num_rows($res_prot)>0) {
				$retour.="<table class='boireaus' border='1' summary='Protagonists of the incident n°$lig->id_incident'>\n";
				...
				while($lig_prot=mysql_fetch_object($res_prot)) {
					...
					if(mysql_num_rows($res_suivi)>0) {
						...
						$retour.="<table class='boireaus' border='1' summary='Table of measurements for the protagonist $lig_prot->login de l incident n°$lig->id_incident'>\n";

						$retour.="<tr>\n";
						$retour.="<th>Nature</th>\n";
						$retour.="<th>Measure</th>\n";
						$retour.="</tr>\n";
					...
					if(mysql_num_rows($res_suivi)>0) {
						...
						$retour.="<table class='boireaus' border='1' summary='Table of the sanctions for the protagonist $lig_prot->login incident n°$lig->id_incident'>\n";

						$retour.="<tr>\n";
						$retour.="<th>Nature</th>\n";
						$retour.="<th>Date</th>\n";
						$retour.="<th>Description</th>\n";
						$retour.="<th>Carried out</th>\n";
						$retour.="</tr>\n";
						...
						while($lig_suivi=mysql_fetch_object($res_suivi)) {
							...
							elseif($lig_suivi->nature=='exclusion') {
								...
								if(mysql_num_rows($res_exclusion)>0) {
									...
									$retour.="dof ".formate_date($lig_exclusion->date_debut)." (<i>$lig_exclusion->heure_debut</i>) to ".formate_date($lig_exclusion->date_fin)." (<i>$lig_exclusion->heure_fin</i>)<br />$lig_exclusion->lieu";
								}
								else {
									$retour.="X";
								}
				...
				$retour .=  "<p style='text-align:left;'><b>Comments on the incident&nbsp;:&nbsp;</b></br></br>$zone_de_commentaire</p>";	
				}
			}
		...
		// Totaux
		$retour.="<p style='font-weight: bold;'>Totals of the incidents/mesures/sanctions as Responsible.</p>\n";

		$retour.="<div style='float:left; width:33%;'>\n";
		$retour.="<p style='font-weight: bold;'>Incidents</p>\n";
		if(count($tab_incident)>0) {
			$retour.="<table class='boireaus' border='1' summary='Incidental totals'>\n";
			...
			$retour.="<p>No incident raised in quality of responsible.</p>\n";
		}
		$retour.="</div>\n";
		...
		if(count($tab_mesure)>0) {
			...
			$retour.="<table class='boireaus' border='1' summary='Totals measurements taken'>\n";
			$retour.="<tr><th>Measure</th><th>Total</th></tr>\n";
			$alt=1;
			...
			$retour.="<p>No measurement taken in the capacity as responsible.</p>\n";
		}
		$retour.="</div>\n";
...
function afficher_tableau_des_reports($id_sanction) {
    ...
    $sql="SELECT * FROM s_reports WHERE id_sanction=$id_sanction ORDER BY id_report";
		...
		while($lig=mysql_fetch_object($res)) {
		  ...
		  echo "<td><a href='".$_SERVER['PHP_SELF']."?mode=suppr_report&amp;id_report=$lig->id_report&amp;id_sanction=$lig->id_sanction&amp;id_incident=$id_incident&amp;".add_token_in_url()."' title='Supprimer le report n°$lig->id_report'><img src='../images/icons/delete.png' width='16' height='16' alt='Remove the carryforward n°$lig->id_report' /></a></td>\n";
		...
		echo "</table>\n";
		  ...
		  $retour = "No carryforward currently for this sanction.";
		}	
	return $retour;
}
...
function sanction_documents_joints($id_incident, $ele_login) {
	...
	echo "<p>Join a file&nbsp;: <input type=\"file\" size=\"15\" name=\"document_joint\" id=\"document_joint\" /><br />\n";
	...
	if(count($tab_doc_joints2)>0) {
		...
		if($temoin_deja_tous_joints=="n") {
			...
			echo "<b>Files proposed during the seizure of measurements requested&nbsp;:</b>";
			echo "<table class='boireaus' width='100%'>\n";
			echo "<tr>\n";
			echo "<th>Joindre</th>\n";
			echo "<th>Fichier</th>\n";
			...
			if(count($tab_doc_joints)>0) {
				...
				for($loop=0;$loop<count($tab_doc_joints);$loop++) {
					if(!unlink($chemin."/".$tab_doc_joints[$loop])) {
						$retour.="Error during the suppression of $chemin/$tab_doc_joints[$loop]<br />";
						$temoin_erreur="y";
					}
				}
				if($temoin_erreur=="n") {
...
function lien_envoi_mail_rappel($id_sanction, $num, $id_incident="") {
	...
	if(($id_sanction!="")||($id_incident!="")) {
		...
		if($id_sanction=="") {
			...
			$trame_message.="The sanction (See the N° incident$num_incident) de $chaine_protagonistes est planifiée.\n";
		}
		else {
			...
			if(mysql_num_rows($res)>0) {
				...
				if($lig_sanction->nature="retenue") {
					...
					$trame_message.="Reserve (to see the N° incident%num_incident%) of %prenom_nom% (%classe%) it is planned %jour% in/at %heure% for one duration of %duree%H \n";
				...
				elseif($lig_sanction->nature="exclusion") {
					$trame_message.="Exclusion (to see the N° incident%num_incident%)of %prenom_nom% (%classe%) is planned %jour_debut% to %jour_fin% \n";
				...
				elseif($lig_sanction->nature="travail") {
					$trame_message.="Work (See the N° incident%num_incident%) de %prenom_nom% (%classe%) is planned for a date of return to %jour_retour% at %heure_retour% \n";
					...
					$trame_message.="The sanction '$lig_sanction->nature' (See the incident N°%num_incident%) of %prenom_nom% (%classe%) is planned.\n";
				}
			}
		...
		$trame_message.="Thank you to bring work envisaged to the school life.\n\n-- \n School life";
		...
		if(($_SESSION['statut']=='administrateur') || ($_SESSION['statut']=='cpe') || ($_SESSION['statut']=='scolarite')) {
			...
			if((!isset($lig_sanction))||($lig_sanction->effectuee!="O")) {
				$retour.="<span id='mail_envoye_$num'><a href='#' onclick=\"envoi_mail_rappel_sanction($num);return false;\"><img src='../images/icons/icone_mail.png' width='25' height='25' alt='To send a mall to ask work to the informant' title='To send a mall to ask work to the informant' /></a></span>";
			}
		}
	}
	return $retour;
