...
class SelectCtrl extends Controleur {
  ...
  private function test_edt_active() {
    if(! $this->objet_periodes->is_EDT_active())
      echo"<script type='text/javascript'>alert('Activate module EDT for the administrators then inform the periods of the calendar in admin');
  ...
  private function test_db_periodes_calendrier() {
    if(! $this->objet_periodes->is_periodes_renseignees())
      echo"<script type='text/javascript'>alert('Inform the periods of the calendar in admin');
  ...
  private function verif_date_selected() {
    ...
    if(!Gepi_Date::isValid_fr($this->du)) {
      $this->messages[]=Array('class'=>'mess_orange','message'=>'The date of beginning is not with format JJ/MM/AAAA ; The date of beginning per defect is selected');
      return($this->du=Gepi_Date::get_date_begin_yearschool());
    }
    else if(!Gepi_Date::isValid_fr($this->au)) {
      $this->messages[]=Array('class'=>'mess_orange','message'=>'The completion date is not with format JJ/MM/AAAA; The completion date per defect is selected');
      return($this->au=date('d/m/Y'));
    }
    else if (Gepi_Date::compare_date($this->du,$this->au)) {
      $this->messages[]=Array('class'=>'mess_orange','message'=>'The completion date must be posterior with that of the beginning ; The dates by defect are selected');
  ...
  public function test_data_selected() {
    ...
    else echo"<script type='text/javascript'>alert('You do not have select individuals or classes');</script>";
  }
