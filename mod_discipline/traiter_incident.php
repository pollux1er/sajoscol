...
function liste_sanctions($id_incident,$ele_login) {
	...
	if(mysql_num_rows($res)==0) {
		$retour="<p style='color:red;'>The incident n°$id_incident do not exist???</p>\n";
	}
	else {
		...
		if(mysql_num_rows($res_sanction)>0) {
			...
			if ($nombre_de_report <> 0) {
			   $retour.="<th>Carryforward numbers</th>\n";
			}
			...
			while($lig_sanction=mysql_fetch_object($res_sanction)) {
				...
				$tabdiv_infobulle[]=creer_div_infobulle("div_travail_sanction_$lig_sanction->id_sanction","Work (sanction n°$lig_sanction->id_sanction)","",$texte,"",20,0,'y','y','n','n',2);
		...
		if(mysql_num_rows($res_sanction)>0) {
			...
			$retour.="<tr>\n";
			$retour.="<th>Nature</th>\n";
			$retour.="<th>Date beginning</th>\n";
			$retour.="<th>Hour beginning</th>\n";
			$retour.="<th>Date end</th>\n";
			$retour.="<th>Hour end</th>\n";
			$retour.="<th>Place</th>\n";
			$retour.="<th>Work</th>\n";
			if(($etat_incident!='clos')&&($_SESSION['statut']!='professeur')) {
				$retour.="<th>Suppr</th>\n";
		...
		if(mysql_num_rows($res_sanction)>0) {
			...
			while($lig_sanction=mysql_fetch_object($res_sanction)) {
				...
				if(($etat_incident!='clos')&&($_SESSION['statut']!='professeur')) {
					//$retour.="<td><a href='".$_SERVER['PHP_SELF']."?mode=suppr_sanction&amp;id_sanction=$lig_sanction->id_sanction&amp;id_incident=$id_incident' title='Supprimer la sanction n°$lig_sanction->id_sanction'><img src='../images/icons/delete.png' width='16' height='16' alt='Supprimer la sanction n°$lig_sanction->id_sanction' /></a></td>\n";
					$retour.="<td><a href='saisie_sanction.php?mode=suppr_sanction&amp;id_sanction=$lig_sanction->id_sanction&amp;id_incident=$id_incident".add_token_in_url()."' title='Remove the sanction n°$lig_sanction->id_sanction'><img src='../images/icons/delete.png' width='16' height='16' alt='Supprimer la sanction n°$lig_sanction->id_sanction' /></a></td>\n";
				}
				$retour.="</tr>\n";
			}
			$retour.="</table>\n";
	...
	if(isset($form_id_incident)) {
		...
		for($i=0;$i<count($form_id_incident);$i++) {
			...
			if($acces_modif_etat=="y") {
				...
				if(!$res) {
					$msg.="ERROR at the time of the update of the state of the incident n°".$form_id_incident[$i].".<br />\n";
				}
			}
		}
	}
...
if((isset($_POST['suppr_incident']))&&(($_SESSION['statut']!='professeur')||($_SESSION['statut']=='autre'))) {
	...
	for($i=0;$i<count($suppr_incident);$i++) {
		...
		if($temoin_erreur=="n") {
			...
			$res=mysql_query($sql);updateOnline($sql);
			if(!$res) {
				$msg.="ERROR during the removal of the protagonists of the incident ".$suppr_incident[$i].".<br />\n";
				$temoin_erreur="y";
			}
		}
		...
		if($temoin_erreur=="n") {
			...
			if(mysql_num_rows($res_sanction)>0) {
				while($lig=mysql_fetch_object($res_sanction)) {
					...
					$res=mysql_query($sql);updateOnline($sql);
					if(!$res) {
						$msg.="ERROR during the suppression of reserves attached to the incident ".$suppr_incident[$i].".<br />\n";
						$temoin_erreur="y";
					}
					...
					$res=mysql_query($sql);updateOnline($sql);
					if(!$res) {
						$msg.="ERROR during the suppression of excluded attached to the incident ".$suppr_incident[$i].".<br />\n";
						$temoin_erreur="y";
					}
					...
					$res=mysql_query($sql);updateOnline($sql);
					if(!$res) {
						$msg.="ERROR during the suppression of work attached to the incident ".$suppr_incident[$i].".<br />\n";
						$temoin_erreur="y";
					}
					...
					$res=mysql_query($sql);updateOnline($sql);
					if(!$res) {
						$msg.="ERROR during the suppression of other sanctions attached to the
incident ".$suppr_incident[$i].".<br />\n";
						$temoin_erreur="y";
					}
				}
				...
				if($temoin_erreur=="n") {
					...
					$res=mysql_query($sql);updateOnline($sql);
					if(!$res) {
						$msg.="ERROR during the suppression of the sanction associated with the
incident ".$suppr_incident[$i].".<br />\n";
						$temoin_erreur="y";
					}
				}
			...
			if($temoin_erreur=="n") {
				...
				$res=mysql_query($sql);updateOnline($sql);
				if(!$res) {
					$msg.="ERROR during the suppression of the treatments of incident
(measurements) of the incident ".$suppr_incident[$i].".<br />\n";
					$temoin_erreur="y";
				}
				...
				if($temoin_erreur=="n") {
					...
					$res=mysql_query($sql);updateOnline($sql);
					if(!$res) {
						$msg.="ERROR during the suppression of the work suggested for a measurement
requested of the incident ".$suppr_incident[$i].".<br />\n";
						$temoin_erreur="y";
					}
				}
				...
				if($temoin_erreur=="n") {
					...
					$res=mysql_query($sql);updateOnline($sql);
					if(!$res) {
						$msg.="ERROR during the suppression of the incident ".$suppr_incident[$i].".<br />\n";
					}
				}
			}
		}
...
if (($_SESSION['statut']=='professeur')||($_SESSION['statut']=='autre'))  {
	$titre_page = "Discipline: Consult an incident";
}
else {
	$titre_page = "Discipline: Treat an incident";
}
...
echo "<p class='bold'><a href='index.php' onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to the index</a>\n";
//echo "</p>\n";
...
if(!isset($id_incident)) {
	...
	$res=mysql_query($sql);
	if(mysql_num_rows($res)==0) {
		echo " | <a href='".$_SERVER['PHP_SELF']."' onclick='history.go(-1);return false;'> Return to the preceding page</a>\n";
		echo "</p>\n";

		if($incidents_clos=="y") {
			echo "<p>No incident is still declared";
			if(($date_incident!="")||
			($heure_incident!="")||
			($nature_incident!="---")||
			...
			//($declarant_incident!="")||
			($id_classe_incident!="")) {echo " with the selected criteria";}
			echo ".</p>\n";
		}
		else {
			...
			if(mysql_num_rows($res)==0) {
				...
				echo "<p>No incident (<i>non clos</i>) is not declared";
				if(($date_incident!="")||
				($heure_incident!="")||
				($nature_incident!="---")||
				(($declarant_incident!="---")&&($declarant_incident!=""))||
				...
				echo "<p><a href='".$_SERVER['PHP_SELF']."?incidents_clos=y$chaine_criteres'>Display the incidents closed with the same criteria</a>.</p>\n";
			}
		}
		echo "<p><br /></p>\n";
	...
	*/
	...
	echo "<p class='bold'>Choose the incident to be treated/Consult &nbsp;:</p>\n";
	echo "<blockquote>\n";
	...
	echo " /><label for='declarant_incident2' style='cursor:pointer;'> See only my declarations of incidents</label>\n";
	echo "</p>\n";

	echo "<div style='float: right; border: 1px solid black;'>";
	echo mysql_num_rows($res)." incidents";
	if($chaine_criteres!="") {echo " with the criteria selected";}
	echo "</div>\n";
		...
		while($lig_declarant=mysql_fetch_object($res_declarant)) {
			...
			if($lig_declarant->declarant!='') {
				...
				if(mysql_num_rows($res1_declarant)>0) {
					...
					echo "ERROR: Login $lig1_declarant->declarant unknown";
				}
	...
	if(($_SESSION['statut']!='professeur')&&($_SESSION['statut']!='autre')) {
	    echo "<th>Print the incident report</th>\n";
		echo "<th>Suppr</th>\n";
	}
	//echo "<th></th>\n";
	echo "</tr>\n";
		...
		if($affiche_ligne_incident=='y') {
            ...
            if (!(($_SESSION['statut']=='professeur')||($_SESSION['statut']=='autre'))) {			
				...
				if(mysql_num_rows($res_declarant)>0) {
					...
					echo "ERROR: Login $lig->declarant inconnu";
				}
				echo "</td>\n";
			}
			//=================================================
				...
				while($lig2=mysql_fetch_object($res2)) {
					...
					if($lig2->statut=='eleve') {
						...
						if(mysql_num_rows($res3)>0) {
							...
							echo "ERROR: Login $lig2->login inconnu";
						}
				...
				if($lig->nature=='') {
					...
					if(mysql_num_rows($res_mail)>0) {
						...
						*/
						...
						$texte.="&amp;body=Bonjour%20".$lig_mail->civilite."%20".$lig_mail->nom."%20".substr(ucfirst($lig_mail->prenom),0,1).".,%0A%0a"."You declared an incident (number $lig->id_incident) without specifying nature, the details of them."."%0A%0a"."The incident took place it ".formate_date($lig->date)." en $lig->heure with for protagonists: $liste_protagonistes"."%0A%0a"."Could you specify?"."%0A%0a"."Merci."."\">";
			...
			if($lieu_incident=="") {
				...
				$lieu_incident="not specified";
			}
			$texte.="<br /><span style='font-size:x-small;'>Lieu&nbsp;: ".$lieu_incident."</span>";

			if($lig->heure!="") {
				$texte.="<span style='font-size:x-small;'> per hour $lig->heure</span>";
			}

			$texte.="<br /><span style='font-size:x-small;'>Incident announced by ".u_p_nom($lig->declarant)."</span>";
			...
			*/
			...
			if($texte!="") {
				...
				$tabdiv_infobulle[]=creer_div_infobulle("sanctions_incident_".$lig->id_incident,"Sanctions incident n°$lig->id_incident","","No sanction is still seized","",20,0,'y','y','n','n');
			...
			if(($_SESSION['statut']!='professeur')&&($_SESSION['statut']!='autre')) {
				echo "<td>\n";
				echo "<a href='../mod_ooo/rapport_incident.php?mode=module_discipline&id_incident=$lig->id_incident".add_token_in_url()."' title='Print the incident report'><img src='../images/icons/print.png' width='16' height='16' alt='Print the Incident report'></a>\n";
				echo "</td>\n";
			}
			//===============================
	...
	if(mysql_num_rows($res)>0) {
		...
		echo "<p>Normally, one does not arrive here...</p>\n";
	}
}
...
echo "<p><em>NOTE&nbsp;:</em></p>\n";
echo "<blockquote>\n";
echo "<p>When an incident is closed, one cannot modify the incident any more,
nor Seize/Modify sanction.</p>\n";
echo "</blockquote>\n";
echo "<p><br /></p>\n";
