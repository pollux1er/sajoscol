    ...
    if (($periode_num < $nb_periode) and ($periode_num > 0) and ($ver_periode[$periode_num] == "N" OR $ver_periode[$periode_num] == "P"))  {
        ...
        if (!$presaisie and !$acces_scol) {
             ...
             if ($test_prof_suivi == '-1') {
                 $msg = "You are not a professor of follow-up of this student.";
                 $reg = 'no';
             }
         }
         if ($reg == 'yes') {
             ...
             if (isset($_POST['delete'])) {
                 if (!$presaisie) { // Pas de reset des données en mode 'présaisie' !
                   if (isset($_POST['delete_all']) and $_POST['delete_all'] == 'yes') {
                      ...
                      $msg = "The data of this pupil have been just removed.";
                   } else {
                       $msg = 'To remove data, you must notch the box above the button of validation of suppression.';
                   }
                 }
...
$themessage = 'Values were modified. Do you want to really leave without recording?';
//**************** EN-TETE *****************
$titre_page = "Seizure of the ECTS";
...
if (isset($id_classe) and (!isset($periode_num))) {
    ...
    echo "<p class=bold><a href=\"index_saisie.php\"><img src='../images/icons/back.png' alt='Return' class='back_link' /> My classes</a></p>\n";
    echo "<p><b>".$classe_suivi.", choose the period : </b></p>\n";
    include "../lib/periodes.inc.php";
    $i="1";
    echo "<ul>\n";
    while ($i < $nb_periode) {
        if ($ver_periode[$i] != "O") {
            ...
            echo "<li>".ucfirst($nom_periode[$i])." (".$gepiClosedPeriodLabel.", impossible seizure)</li>\n";
        }
    $i++;
    }
    echo "</ul>\n";
...
if (isset($id_classe) and (isset($periode_num)) and (!isset($fiche))) {
    ...
    <p class=bold><a href="saisie_ects.php?id_classe=<?php echo $id_classe; ?>"><img src='../images/icons/back.png' alt='Retour' class='back_link' /> To choose another period</a>
...
if (isset($id_class_prec) && $id_class_prec!=0) {
	echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_class_prec&amp;periode_num=$periode_num' onclick=\"return confirm_abandon (this, change, '$themessage')\">Classify preceding</a>";
}
...
if(isset($id_class_suiv)){
	if($id_class_suiv!=0){echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_class_suiv&amp;periode_num=$periode_num' onclick=\"return confirm_abandon (this, change, '$themessage')\">Following class</a>";}
}
...
echo "</form>\n";
    ...
    <p class='grand'>Class : <?php echo $classe_suivi; ?></p>

    <?php
    if ($ver_periode[$periode_num] != "N" AND $ver_periode[$periode_num] != "P") {
        echo "This period is closed for this class !";
    } else {
        ?>

        <p>Click on the name of the pupil for whom you want to enter or modify
appropriations ECTS.</p>
        <?php
        if ($acces_scol) {
...
<?php
	...
	echo "<div class='norme'><p class='bold'><a href='saisie_ects.php?mode=$mode_saisie&id_classe=$id_classe&periode_num=$periode_num'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>\n";
    ...
    if($ele_login_prec!=""){
        ...
        echo "'>".ucfirst($gepiSettings['denomination_eleve'])." precedent</a>";
    }
    ...
    echo "<p><b>Principal fields of studies</b> : ".$Classe->getEctsDomainesEtude()."</p>";
    ...
    if ($presaisie && $dossier_valide_conseil) {
      ...
      echo $nb_cols+1;
      echo "'>";
      echo "<p style='color: red;'>File validated as a staff meeting</p>";
      echo "</td></tr>";
    }
    ...
    if (!$presaisie) {
      ...
      if (!$donnees_enregistrees) {
          echo "<p style='color: red;'>Dossier non-examiné<br/><span style='font-size: small;'>The default values are pre-seizures, but are not recorded yet in data
base.</span></p>";
      } else {
          ...
          if ($total_valeur[$periode_num] < 30) {
              echo "<p style='color: red;'>Not validated<br/><span style='font-size: small;'>Appropriations are recorded in data base, but the total is lower than
30.</span></p>";
          } elseif ($total_valeur[$periode_num] == 30) {
              echo "<p style='color: blue;'>Validated<br/><span style='font-size: small;'>30 appropriations ECTS are recorded in data base for this period.</span></p>";
          } else {
              echo "<p style='color: red;'>Excess of crédit<br/><span style='font-size: small;'>More than 30 appropriations are recorded in data base for this period.</span></p>";
          }
      }
      echo "</td></tr>";
    }    
    ...
    if (!$presaisie || ($presaisie && !$dossier_valide_conseil)) {
      ...
      if($ele_login_suiv!=""){
          echo "<input type='submit' NAME='ok1' value=\"Record and pass to the following pupil\" />";
      }
      ?>
      <input type="submit" NAME="ok2" value="Record and return to the list" />
      </td></tr>
<?php
...
?>
    ...
    <p style='font-size: small;'>If you wish to remove all data ECTS of this pupil for this period,<br/>tick the box below then validate with the button ' To remove the
data' <br/>This irreversible operation is.</p>
    <input id="delete_all" type="checkbox" name="delete_all" value="yes" /><label for="delete_all" style='font-size: small;color: red;'>Remove all the data.</label><br/>
    <br/>
