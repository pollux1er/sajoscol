...
secours='F',
autre='F',
description='White test: To allot the copies to the professors',
statut='';";
	...
	if(mysql_num_rows($res)==0) {
		...
		if($etat!='clos') {
			...
			for($i=0;$i<count($login_ele);$i++) {
				...
				if(!$update) {$msg.="Error during the attribution of the copy of '$login_ele[$i]' at '$login_prof[$i]'.<br />";}
			}
...
//**************** EN-TETE *****************
$titre_page = "White test: Attribution of the copies";
...
$res=mysql_query($sql);
if(mysql_num_rows($res)==0) {
	echo "<p>No professor is yet selected.</p>\n";
	require("../lib/footer.inc.php");
	die();
}
...
echo "<p class='bold'>Sort the student by&nbsp;:</p>\n";
echo "<ul>\n";
if($pas_de_salle=="y") {
	echo "<li>No room is yet selected</li>\n";
}
else {
...
if($tri=='groupe') {
	...
	while($lig=mysql_fetch_object($res)) {
		...
		if($etat!='clos') {
			...
			for($i=0;$i<count($info_prof);$i++) {
				echo "<th>\n";
				echo "<span id='eff_prof_".$lig->id."_$i'>Manpower</span>";
...
elseif($tri=='salle') {
	...
	$res=mysql_query($sql);
	if(mysql_num_rows($res)==0) {
		echo "<p>No room is still associated the test.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	while($lig=mysql_fetch_object($res)) {
		...
		//echo "\$cpt=$cpt<br />";
		echo "<table class='boireaus' summary='Student of the room $lig->id'>\n";
		echo "<tr>\n";
		echo "<th>Student</th>\n";
		echo "<th>Classes</th>\n";
		for($i=0;$i<count($info_prof);$i++) {
		...
		while($lig2=mysql_fetch_object($res2)) {
			...
			for($i=0;$i<count($info_prof);$i++) {
				...
				if((isset($tab_ele_prof_habituel[$login_ele]))&&($tab_ele_prof_habituel[$login_ele]==$login_prof[$i])) {
					echo "<div style='float:right; width:17px;'><img src='../images/icons/flag.png' width='17' height='18' title='Usual professor of this student' alt='Usual professor of this student' /></div>\n";
				}
	...
	if(mysql_num_rows($res2)==0) {
		echo "<p>All the student are affected in rooms.</p>\n";
	}
	else {
		echo "<p>Student <b>not affected</b> in a room &nbsp;:</p>\n";
		echo "<blockquote>\n";
	
		//echo "\$cpt=$cpt<br />";
		echo "<table class='boireaus' summary='Not affected student'>\n";
		echo "<tr>\n";
		echo "<th>Student</th>\n";
		echo "<th>Classes</th>\n";
		for($i=0;$i<count($info_prof);$i++) {
		...
		if($etat!='clos') {
			...
			for($i=0;$i<count($info_prof);$i++) {
				echo "<th>\n";
				echo "<span id='eff_prof_na_$i'>Manpower</span>";
				echo "</th>\n";
			}
			echo "<th>\n";
			//$i++;
...
echo "<p style='color:red;'>TO MAKE: To allow not to allot the copies? to be able to seize while
picking in the list.</p>\n";
