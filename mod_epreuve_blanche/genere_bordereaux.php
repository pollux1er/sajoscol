<?php
/* $Id: genere_bordereaux.php 8061 2011-08-30 22:01:10Z jjacquard $ */
/*
...
secours='F',
autre='F',
description='White test: Generation of the forms professors',
statut='';";
...
if(isset($imprime)) {
	...
	if(mysql_num_rows($res)==0) {
		...
		$cpt=0;
		$profs[$cpt]['login']="";
		$profs[$cpt]['civ_n_p']="Copy not allotted ";
		$cpt++;
...
if(!isset($imprime)) {
	...
	if(mysql_num_rows($test1)!=mysql_num_rows($test2)) {
		echo "<p style='color:red;'>The numbers anonymities are not single on the test (<i>that should not arrive</i>).</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	if($nb_tmp==1) {
		echo "<p style='color:red;'>$nb_tmp student is not affected in a room.</p>\n";
	}
	elseif($nb_tmp>1) {
		echo "<p style='color:red;'>$nb_tmp student were not affected in rooms.</p>\n";
	}
	//========================================================

	echo "<form method=\"post\" action=\"".$_SERVER['PHP_SELF']."\" name='form1'>\n";

	echo "<p>Choose the type of forms to be printed&nbsp;:<br />\n";
	...
	echo "<p>Information to be included&nbsp;:<br />";
	echo "<input type='checkbox' name='avec_num_anonymat' id='avec_num_anonymat' value='y' checked /><label for='avec_num_anonymat'>With the number anonymity</label><br />";
	...
	echo "<input type='checkbox' name='avec_colonne_vide_1' id='avec_colonne_vide_1' value='y' checked /><label for='avec_colonne_vide_1'> With a column empties whose heading is </label><input type='text' name='titre_colonne_vide_1' value='Note sur $note_sur' /><br />";
	echo "<input type='checkbox' name='avec_colonne_vide_2' id='avec_colonne_vide_2' value='y' /><label for='avec_colonne_vide_2'> With a second column empties whose heading is </label><input type='text' name='titre_colonne_vide_2' value='Pointage départ' /><br />";
	echo "<input type='checkbox' name='avec_colonne_vide_3' id='avec_colonne_vide_3' value='y' /><label for='avec_colonne_vide_3'> With a third column empties whose heading is </label><input type='text' name='titre_colonne_vide_3' value='Pointage retour' /><br />";
	echo "Normally, the following fields would not owe apparaitre on forms
professors, perhaps but you will imagine uses of which the developers
had not thought.<br />";
	echo "<input type='checkbox' name='avec_nom_prenom' id='avec_nom_prenom' value='y' /><label for='avec_nom_prenom'>With the name/prénom of the student</label><br />";
	echo "<input type='checkbox' name='avec_naissance' id='avec_naissance' value='y' /><label for='avec_naissance'>With the date of birth of the student</label><br />";
	echo "<input type='checkbox' name='avec_classe' id='avec_classe' value='y' /><label for='avec_classe'>With the class of the student</label><br />";
	echo "<input type='checkbox' name='avec_salle' id='avec_salle' value='y' /><label for='avec_salle'>With the name of room in which the student passed the test</label><br />";
