...
secours='F',
autre='F',
description='White test: Annotating generation',
statut='';";
...
if(isset($imprime)) {
	...
	if(mysql_num_rows($res)==0) {
		$msg="The test n°$id_epreuve do not exist.";
	}
	else {
...
//**************** EN-TETE *****************
$titre_page = "White test: Annotating";
...
echo "<p class='bold'><a href='index.php?id_epreuve=$id_epreuve&amp;mode=modif_epreuve'>Return</a>";
//echo "</p>\n";
//echo "</div>\n";
...
if(!isset($imprime)) {
	...
	if(mysql_num_rows($test1)!=mysql_num_rows($test2)) {
		echo "<p style='color:red;'>The numbers anonymities are not single on the test (<i>cela should not arrive</i>).</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	echo "<p>Choose the type of list to be printed&nbsp;:</p>\n";
	echo "<ul>\n";
	echo "<li><b>CSV</b>&nbsp;:\n";
	 	echo "<ul>\n";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;imprime=sans_num_anonymat&amp;mode=csv".add_token_in_url()."'>With the columns 'NAME;FIRST NAME;SIGNATURE'</a></li>\n";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;imprime=avec_num_anonymat&amp;mode=csv".add_token_in_url()."'>With the columns 'NAME;FIRST NAME;NUM_ANONYMAT;SIGNATURE'</a></li>\n";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;imprime=tout&amp;mode=csv".add_token_in_url()."'>With the columns 'NAME;FIRST NAME;CLASS;DATE_DE_NAISSANCE;NUM_ANONYMAT;SIGNATURE'</a></li>\n";
		echo "</ul>\n";
	echo "</li>\n";
	echo "<li><b>PDF</b>&nbsp;:\n";
	 	echo "<ul>\n";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;imprime=sans_num_anonymat&amp;mode=pdf".add_token_in_url()."'>With the columns 'NOM_PRENOM;SIGNATURE'</a></li>\n";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;imprime=avec_num_anonymat&amp;mode=pdf".add_token_in_url()."'>With the columns 'NOM_PRENOM;NUM_ANONYMAT;SIGNATURE'</a></li>\n";
		echo "</ul>\n";
	echo "</li>\n";
	echo "</ul>\n";
}
