<?php
/* $Id: genere_etiquettes.php 8061 2011-08-30 22:01:10Z jjacquard $ */
/*
...
secours='F',
autre='F',
description='White test: Labels generation',
statut='';";
...
'dy');
	...
	if($cpt>0) {$msg="$cpt recordings carried out.";}
}
			...
			if(mysql_num_rows($res)>0) {
				...
				*/
				...
				while($lig=mysql_fetch_object($res)) {
					...
					$texte.="Birth: ".formate_date($lig->naissance)."\n";
					$texte.="Number: ".$lig->n_anonymat;
					//cell_ajustee($texte,$x,$y,$largeur_dispo,$h_cell,$hauteur_max_font,$hauteur_min_font,$bordure,$v_align='C',$align='L',$increment=0.3,$r_interligne=0.3) {
					cell_ajustee($texte,$x,$y,$largeur_dispo,$h_cell,$hauteur_max_font,$hauteur_min_font,$bordure,$v_align,$align);
...
if((!isset($mode))||($mode!='parametrer')) {
	...
	echo "<p class='bold'>Labels&nbsp;:</p>\n";
	echo "<ul>\n";
	echo "<li><a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;mode=parametrer".add_token_in_url()."'>Parameterize</a> labels</li>\n";
	echo "<li><a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;mode=imprime".add_token_in_url()."'>Imprimer</a> labels</li>\n";
	echo "</ul>\n";
}
else {
	// Paramétrage des étiquettes
	echo " | <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve'>Etiquettes</a>";
	echo " | <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;mode=imprime".add_token_in_url()."'>Print</a>";
	echo "</p>\n";

	echo "<form method=\"post\" action=\"".$_SERVER['PHP_SELF']."\" name='form1'>\n";
	echo add_token_field();

	echo "<p>The impression is envisaged with the A4 format with two columns of
labels.<br />You can adjust some parameters concerning the labels.</p>\n";
	...
	*/
	...
	echo "<table class='boireaus' summary='Table of the parameters'>\n";
	...
	echo "<tr class='lig$alt'>\n";
	echo "<td style='text-align:left;'>\n";
	echo "Vertical space between two labels&nbsp;:\n";
	echo "</td>\n";
	echo "<td>\n";
