...
secours='F',
autre='F',
description='White test: Reception',
statut='';";
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')) {
	...
	if((isset($_POST['creer_epreuve']))||(isset($_POST['modif_epreuve']))) {
		...
		if(!preg_match('/^[0-9]*$/',$note_sur)) {
			$note_sur=20;
			$msg.="Value of note_sur invalide<br />";
		}
		...
		if(!isset($id_epreuve)) {
			...
			if($insert=mysql_query($sql)) {
				...
				$msg.="Test n°$id_epreuve : '$intitule' created.<br />";
			}
			else {
				$msg.="ERROR during the creation of the test '$intitule'.<br />";
				//$msg.="<br />$sql";
			}
		}
		else {
			...
			if($update=mysql_query($sql)) {
				$msg.="Test n°$id_epreuve : '$intitule' setting with jour.";
					...
					while($lig=mysql_fetch_object($res)) {
						...
						if($type_anonymat=='alea') {
							...
							if(in_array($n_anonymat,$tab_n_anonymat_affectes)) {
								$msg.="Error: The number '$n_anonymat' de $lig->login is already affected to another pupil.<br />";
								$temoin_erreur="y";
				...
				$msg.="ERROR during the modification of the test '$intitule'.";
				//$msg.="<br />$sql";
			}
		}
		$mode="modif_epreuve";
	...
	elseif((isset($id_epreuve))&&($mode=='suppr_epreuve')) {
		...
		for($i=0;$i<count($tab_tables);$i++) {
			...
			if(!$suppr) {
				$msg="ERROR during the suppression of the test $id_epreuve";
				//for($j=0;$j<$i;$j++) {$msg.=""}
				unset($id_epreuve);
				unset($mode);
				break;
		...
		if($msg=='') {
			...
			if(!$suppr) {
				...
				$msg="Suppression of the test $id_epreuve carried out.";
			}
		}
		unset($id_epreuve);
		unset($mode);
	...
	elseif((isset($id_epreuve))&&($mode=='ajout_groupes')) {
		...
		$suppr=mysql_query($sql);
		if(!$suppr) {
			$msg="ERROR during the rebootstrapping of the registered groups.";
		}
		else {
			$sql="SELECT * FROM eb_epreuves WHERE id='$id_epreuve';";
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				$msg="The test n°$id_epreuve do not exist.<br />";
			}
			else {
				$lig=mysql_fetch_object($res);
				...
				for($i=0;$i<count($id_groupe);$i++) {
					...
					$insert=mysql_query($sql);
					if(!$insert) {
						$msg.="Error at the time of the addition of the group n°$id_groupe[$i]<br />";
					}
					...
					while($lig=mysql_fetch_object($res)) {
						...
						if(mysql_num_rows($test)==0) {
							...
							$cpt_ano += 1;		
								...
								$n_anonymat=$lig->$type_anonymat;
								if(in_array($n_anonymat,$tab_n_anonymat_affectes)) {$msg.="Error: The number '$n_anonymat' de $lig->login is already affected to another student.<br />";}
								$tab_n_anonymat_affectes[]=$n_anonymat;
							}
							...
							if(!$insert) {
								$msg.="Error at the time of the addition of the student $lig->login<br />";
							}
							else {
								$temoin_n_anonymat='y';
							}
				...
				if($msg=='') {$msg="Addition de(s) group carried out.<br />";}
			}
		}
		$mode='modif_epreuve';
	}
	elseif((isset($id_epreuve))&&($mode=='suppr_groupe')) {
		...
		if(isset($id_groupe)) {
			...
			if(mysql_num_rows($test)==1) {
				$msg="A note was already seized for a copy associated with the group.";
			}
			elseif(mysql_num_rows($test)>1) {
				$msg=mysql_num_rows($test)." notes were already seized for copies associated with the group.";
			}
			else {
				...
				//if(!$suppr) {
				if($nb_err_suppr>0) {
					$msg="ERROR during the removal of the copies associated with the group n°$id_groupe.";
				}
				else {
					...
					$suppr=mysql_query($sql);
					if(!$suppr) {
						$msg="ERROR during the suppression of the n° group $id_groupe.";
					}
					else {
						$msg="Suppression of the n° group $id_groupe carried out.";
					}
				}
			}
		}
	...
	elseif((isset($id_epreuve))&&($mode=='ajout_profs')) {
		...
		$suppr=mysql_query($sql);
		if(!$suppr) {
			$msg="ERROR during the rebootstrapping of the registered professors.";
		}
		else {
			$sql="SELECT * FROM eb_epreuves WHERE id='$id_epreuve';";
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				$msg="The test n°$id_epreuve do not exist Pas.<br />";
			}
			else {
				...
				for($i=0;$i<count($login_prof);$i++) {
					...
					if(!in_array($login_prof[$i],$tab_profs_inscrits)) {
						...
						$insert=mysql_query($sql);
						if(!$insert) {
							$msg.="Error at the time of the addition of the professor $login_prof[$i]<br />";
						}
					}
				}
				if(($msg=='')&&(count($login_prof)>0)) {$msg="Addition of professor carried out.";}
				...
				if(mysql_num_rows($res)>0) {
					...
					while($lig=mysql_fetch_object($res)) {
						...
						if(!in_array($lig->login_prof,$tab_profs_inscrits)) {
							...
							$update=mysql_query($sql);
							$msg.="Removal of professor who was associated copies.<br />";
						}
					}
				}
			}
	...
	elseif((isset($id_epreuve))&&($mode=='clore')) {
		...
		if(!$cloture) {
			$msg="ERROR at the time of cloture of the test $id_epreuve";
			unset($id_epreuve);
			unset($mode);
			break;
		}
		else {$msg="Cloture of the test n°$id_epreuve carried out.";}
		unset($id_epreuve);
		unset($mode);
	}
	elseif((isset($id_epreuve))&&($mode=='declore')) {
		...
		$cloture=mysql_query($sql);
		if(!$cloture) {
			$msg="ERROR at the time of the reopening of the test $id_epreuve";
			unset($id_epreuve);
			unset($mode);
			break;
		}
		else {
			$msg="Reopening of the test n°$id_epreuve carried out.";
			$mode='modif_epreuve';
		}
	}
			...
			if(isset($copie_affect_ele_salle)) {
				for($loop=0;$loop<count($copie_affect_ele_salle);$loop++) {
					if(!in_array($copie_affect_ele_salle[$loop],$id_salle)) {
						$msg.="It is not possible to copy the assignments student/room if the room
is not copied.<br />";
					}
					else {
						...
						if(mysql_num_rows($res)>0) {
							while($lig=mysql_fetch_object($res)) {
								...
								if(mysql_num_rows($test)>0) {
									...
									$insert=mysql_query($sql);
									if(!$insert) {
										$msg.="Error at the time of the assignment of $lig->login_ele in ".$tab_salle[$tab_corresp_id_salle[$copie_affect_ele_salle[$loop]]]."<br />";
									}
								}
	...
	if($temoin_erreur_n_anonymat=='y') {
		if(!isset($msg)) {$msg="";}
		$msg.="<br />One or of the errors occurred on anonymity.<br />You should control the numbers anonymity.";
	}
	elseif($temoin_n_anonymat=='y') {
		if(!isset($msg)) {$msg="";}
		$msg.="<br />Numbers anonymity were modified. Regénérez so necessary label/lists of annotating.";
	}
}
...
//**************** EN-TETE *****************
$titre_page = "White test: Reception";
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')) {
	if(!isset($id_epreuve)) {
		...
		if(!isset($mode)) {
			...
			echo "<li>\n";
			echo "<p><a href='".$_SERVER['PHP_SELF']."?mode=creer_epreuve'>To create a new test</a></p>\n";
			echo "</li>\n";
			...
			if(mysql_num_rows($res)>0) {
				echo "<li>\n";
				echo "<p><b>Tests in progress&nbsp;:</b><br />\n";
				while($lig=mysql_fetch_object($res)) {
					//echo "Modifier <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$lig->id&amp;modif_epreuve=y'";
					echo "Modify <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$lig->id&amp;mode=modif_epreuve'";
					if($lig->description!='') {
					...
					echo " - <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$lig->id&amp;mode=suppr_epreuve".add_token_in_url()."' onclick=\"return confirm('Are you sure to want to remove L test?')\">Remove</a><br />\n";
				}
				echo "</li>\n";
			}
			...
			if(mysql_num_rows($res)>0) {
				echo "<li>\n";
				echo "<p><b>Closed tests&nbsp;:</b><br />\n";
				while($lig=mysql_fetch_object($res)) {
					echo "Epreuve $lig->intitule(<i>".formate_date($lig->date)."</i>)\n";

					echo " - <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$lig->id&amp;mode=modif_epreuve'>Consult</a>\n";

					echo " - <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$lig->id&amp;mode=declore".add_token_in_url()."' onclick=\"return confirm('Are you sure  to want to reopen L test?')\">Reopen</a><br />\n";
				}
			...
			echo "<p style='color:red'>TO STILL MAKE&nbsp;: A bond to empty all the tables of white tests.<br />Is it necessary to empty these tables at the time of initialization?<br />If so, perhaps to add a conservation in the tables filings (former years).</p>\n";
		}
		//===========================================================================
		// Création d'une épreuve
		elseif($mode=='creer_epreuve') {
			echo " | <a href='".$_SERVER['PHP_SELF']."'>Small white tests</a>\n";
			echo "</p>\n";

			echo "<p class='bold'>Creation of a white test&nbsp;:</p>\n";
			...
			echo "<tr>\n";
			echo "<td>Intitule&nbsp;:</td>\n";
			echo "<td><input type='text' name='intitule' value='White test' /></td>\n";
			echo "</tr>\n";
			...
			echo "<tr>\n";
			echo "<td>Anonymity mode&nbsp;:</td>\n";
			echo "<td>\n";
			echo "<select name='type_anonymat'>\n";
			echo "<option value='elenoet'>Identifiant Elenoet</option>\n";
			echo "<option value='ele_id'>Identifier Ele_id</option>\n";
			echo "<option value='no_gep'>Number INE</option>\n";
			echo "<option value='alea'>Chains random</option>\n";
			echo "<option value='chrono'>Chronological number MC00nnn</option>\n";
			echo "</select>\n";
			echo "</td>\n";
			echo "</tr>\n";
			...
			echo "<ul>";
			echo "<li><p style='color:red'>The type_anonymat should be fixed once that one controlled if l'ELENOET, l'INE is well informed for the selected student.</p></li>\n";
			echo "</ul>";
		}
	}
	//===========================================================================
	...
	elseif($mode=='modif_epreuve') {
		...
		// VERIFIER: Il faut avoir saisi un intitulé,...
			...
			if(mysql_num_rows($test)>0) {
				...
				echo ">To copy parameter settings since another white test</a>\n";
			}
		}
		...
		if(!isset($aff)) {
			...
			echo "<p><b>Modification of a white test&nbsp;:</b> Test n°$id_epreuve</p>\n";

			$sql="SELECT * FROM eb_epreuves WHERE id='$id_epreuve';";
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				echo "<p style='color:red;'>ERROR&nbsp;: The test $id_epreuve do not exist.</p>\n";
			...
			if(mysql_num_rows($res_groupes)>0) {
				...
				echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
				if($etat!='clos') {
					echo ">Define the rooms</a><br />\n";
				}
				else {
					echo ">Consult the rooms</a><br />\n";
				}
				...
				if(mysql_num_rows($test_affect_salle)>0) {
					...
					echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
					echo ">Generate the payrolls</a><br />\n";
					// Fiches avec:
					...
					$sql="SELECT * FROM eb_copies WHERE id_epreuve='$id_epreuve' AND id_salle='-1';";
					$test_affect_salle=mysql_query($sql);
					if(mysql_num_rows($test_affect_salle)>0) {echo "<span style='color:red'>".mysql_num_rows($test_affect_salle)." student not affected in a room.</span>";}
					echo "</li>\n";

					echo "<li>\n";
					echo "<a href='genere_liste_affichage.php?id_epreuve=$id_epreuve'";
					echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
					echo ">Generate the lists of display</a><br />\n";
					// Proposer d'enregistrer des paramètres
					// Choisir les champs supplémentaires à afficher (date et lieu de naissance, INE, classe,...)
					// Permettre d'organiser les élèves en salles
					// Générer CSV, PDF
				...
				if($etat!='clos') {
					if(mysql_num_rows($res_profs)>0) {
						...
						echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
						echo ">To allot the copies to the professors</a><br />\n";
						echo "</li>\n";
					}
					else {
						echo "<li>\n";
						echo "No corrector is yet selected\n";
						echo "</li>\n";
					}
				}
				...
				echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
				echo ">Generate the forms professors</a><br />\n";
				echo "</li>\n";
				...
				//echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
				//echo ">&nbsp;</a>";
				echo "Pass the test in phase starting_block<br />\n";
				echo "</li>\n";
				...
				echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
				if($etat!='clos') {
					echo ">Seizure of the notes</a><br />\n";
				}
				else {
					echo ">Consult the notes</a><br />\n";
				}
				echo "</li>\n";
				...
				if($etat!='clos') {
					...
					if(mysql_num_rows($test_affectation_salle)==0) {
						$info_affectation_salle="<span style='color:red'>No student is yet affected in a room.</span><br />\n";
					}
					...
					if(($info_affectation_salle=='')&&(mysql_num_rows($test)==0)) {
						...
						echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
						echo ">Transfer to report cards</a><br />\n";
						echo "</li>\n";

						echo "<li>\n";
						echo "<a href='bilan.php?id_epreuve=$id_epreuve'";
						echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
						echo ">Assessment of the test</a><br />\n";
						echo "</li>\n";

						echo "<li>\n";
						echo "<a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;mode=clore".add_token_in_url()."'";
						echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
						echo ">To close the test</a><br />\n";
						echo "</li>\n";
					}
					else {
						echo "<li>\n";
						echo $info_affectation_salle;
						echo mysql_num_rows($test)." note not yet seized.<br />\n";
						echo "The following choices are thus not yet accessible&nbsp;:";
						echo "<ul>\n";
						echo "<li>Transfer to report cards</li>\n";
						echo "<li>Assessment of the test</li>\n";
						echo "<li>Close the test</li>\n";
						echo "</ul>\n";
					}
				}
				echo "</ol>\n";
			...
			*/
			...
			echo "<tr>\n";
			echo "<td style='font-weight:bold;'>Date from the test &nbsp;:</td>\n";
			echo "<td>\n";
			...
			echo "<tr>\n";
			echo "<td style='font-weight:bold;'>Anonymity mode &nbsp;:</td>\n";
			echo "<td>\n";
			if($etat!='clos') {
			...
			elseif($lig->type_anonymat=='no_gep') {
				...
				echo "Identifier ".strtoupper($lig->type_anonymat);
			}
			echo "</td>\n";
			echo "</tr>\n";
			...
			*/
			...
			//$res_groupes=mysql_query($sql);
			if(mysql_num_rows($res_groupes)>0) {
				echo "<p><b>List groups registered with the test &nbsp;:</b></p>\n";
				echo "<blockquote>\n";
				while($lig=mysql_fetch_object($res_groupes)) {
					...
					if($etat!='clos') {
						echo " - <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;id_groupe=$lig->id&amp;mode=suppr_groupe".add_token_in_url()."' onclick=\"return confirm('Are you sure to want to remove the group of test?')\">Remove</a>\n";
					}
					echo "<br />\n";
				...
				echo "<p><b>No group is still registered with the test &nbsp;:</b></p>\n";
				echo "<blockquote>\n";
			}
			if($etat!='clos') {
				echo "<p><a href='".$_SERVER['PHP_SELF']."?mode=modif_epreuve&amp;id_epreuve=$id_epreuve&amp;aff=classes'>Add groups</a></p>\n";
			}
			echo "</blockquote>\n";
			...
			//$res_profs=mysql_query($sql);
			if(mysql_num_rows($res_profs)>0) {
				echo "<p><b>List correct happy professors appointed for the test &nbsp;:</b></p>\n";
				echo "<blockquote>\n";
				while($lig=mysql_fetch_object($res_profs)) {
					...
					if($compte_total==0) {
						echo " (<span style='font-style:italic;color:red;'>no allotted copy</span>)";
					}
					else {
				...
				if($etat!='clos') {
					echo "<p><a href='".$_SERVER['PHP_SELF']."?mode=modif_epreuve&amp;id_epreuve=$id_epreuve&amp;aff=profs'>Add/Remove of the correct professors</a></p>\n";
				}
			}
			else {
				echo "<p><b>Aucun correct is not still indicated for the test &nbsp;:</b></p>\n";
				echo "<blockquote>\n";

				if($etat!='clos') {
					echo "<p><a href='".$_SERVER['PHP_SELF']."?mode=modif_epreuve&amp;id_epreuve=$id_epreuve&amp;aff=profs'>To add correct professors</a></p>\n";
				}
			}
			...
			if(mysql_num_rows($res_groupes)>0) {
				...
				if(mysql_num_rows($res_salles)>0) {
					echo "<p><b>List rooms chosen for the test &nbsp;:</b></p>\n";
					echo "<blockquote>\n";
					while($lig=mysql_fetch_object($res_salles)) {
						...
						echo "Room '<b>$lig->salle</b>' (<i>eff.".mysql_num_rows($res_eff)."</i>)";
						echo "<br />\n";
					}
					echo "</blockquote>\n";
				}
				else {
					echo "<p><b>No room is yet selected for the test.</b></p>\n";
				}
			}
			echo "<p><br /></p>";
			...
			echo "<p style='color:red'>NOTES&nbsp;:</p>";
			echo "<ul>";
			echo "<li><p style='color:red'>TO MAKE: To be able to prohibit (or alert) the modification of type_anonymat once them listings_émargement/label generated.<br />Put for example eb_epreuves.etat='starting_block' and to prohibit the modifs then anonymity.</p></li>\n";
		...
		elseif($aff=='classes') {
			...
			if($etat=='clos') {
				echo "<p class='bold'>The test $id_epreuve is closed.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}

			echo "<p class='bold'>Choice of the classes for the test $id_epreuve&nbsp;:</p>\n";
			...
			if ($nb==0) {
				echo "<p>No class seems defined.</p>\n";
			}
			else {
...
function checkbox_change(cpt) {
	if(document.getElementById('id_classe_'+cpt)) {
		...
		elseif($aff=='groupes') {
			...
			echo " | <a href='".$_SERVER['PHP_SELF']."?mode=modif_epreuve&amp;id_epreuve=$id_epreuve'>Epreuve $id_epreuve</a>\n";
			echo " | <a href='".$_SERVER['PHP_SELF']."?mode=modif_epreuve&amp;id_epreuve=$id_epreuve&amp;aff=classes'>Choice of the classes</a>\n";
			echo "</p>\n";
			...
			if(!isset($id_classe)) {
				echo "<p style='color:red'>ERROR&nbsp;: No class was selected.</p>\n";
			}

			echo "<form method=\"post\" action=\"".$_SERVER['PHP_SELF']."\" name='form1'>\n";
			echo add_token_field();

			echo "<p class='bold'>Choice of the groups for the test $id_epreuve&nbsp;:</p>\n";
			...
			if($lig->type_anonymat=='alea') {
				...
				echo "<p>The type of selected anonymity is a number ' random'.</p>\n";
				echo "<p><b>Attention</b>&nbsp;: During the validation of this form, the numbers of anonymity are generated/regénérés.<br />You this form once should not validate that labels were stuck or of the anonymées copies.</p>\n";
				echo "</div>\n";
			}
			// Ajout Eric Mode chrono
			if($lig->type_anonymat=='chrono') {
				echo "<div style='float:right; width:20em; border: 1px solid black;'>\n";
				echo "<p>The type of selected anonymity is a Chronologique number.</p>\n";
				echo "<p><b>Caution</b>&nbsp;: During the validation of this form, the numbers of anonymity are generated/regénérés.<br />You this form once should not validate that labels were stuck or of
the anonymées copies.</p>\n";
				echo "</div>\n";
			}
			...
			echo "<table class='boireaus' summary='Groups sorted by classes'>\n";
			echo "<tr>\n";
	...
	if(document.getElementById('id_groupe_'+cpt)) {
		...
		*/
		...
		elseif($aff=='profs') {
			...
			echo " | <a href='".$_SERVER['PHP_SELF']."?mode=modif_epreuve&amp;id_epreuve=$id_epreuve'>Test $id_epreuve</a>\n";
			echo "</p>\n";
			...
			if(mysql_num_rows($res_profs_groupes)>0) {
				echo "<p>The professors called to correct are probably the teachers of the
selected groups.</p>\n";
				//$cpt=0;
			...
			$res_profs=mysql_query($sql);
			if(mysql_num_rows($res_profs)>0) {
				echo "<p>To select professors without concern of groups&nbsp;:</p>\n";

				$nb=mysql_num_rows($res_profs);
				$nb_prof_par_colonne=round($nb/3);
				echo "<table width='100%' summary='Choice of the professors'>\n";
				echo "<tr valign='top' align='center'>\n";
...
*/
		...
		if(mysql_num_rows($res)==0) {
			...
			echo "<p style='color:red'>The test n°$id_epreuve do not exist.</p>\n";
			require("../lib/footer.inc.php");
			die();
		}
		else {
		...
		if(!isset($id_epreuve_modele)) {
			...
			echo "<p>Choice of a model for the test n°$id_epreuve</p>\n";

			echo "<p class='bold'>List other tests&nbsp;:</p>\n";
			$sql="SELECT * FROM eb_epreuves WHERE id!='$id_epreuve';";
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				echo "<p style='color:red'>No other test was found.</p>\n";
			}
			else {
				echo "<ul>\n";
				while($lig=mysql_fetch_object($res)) {
					echo "<li><p><a href='".$_SERVER['PHP_SELF']."?id_epreuve=$id_epreuve&amp;mode=copier_choix&amp;id_epreuve_modele=$lig->id".add_token_in_url()."'>Test n°$lig->id</a>&nbsp;: $lig->intitule<br />".nl2br($lig->description)."</p></li>\n";
				}
				echo "</ul>\n";
			}
		}
		else {
			echo " | <a href='".$_SERVER['PHP_SELF']."?mode=copier_choix&amp;id_epreuve=$id_epreuve'>Choice of the model test</a>\n";
			echo "</p>\n";

			echo "<h3>Copy parameters for the test n°$id_epreuve: $intitule_epreuve (".formate_date($date_epreuve).")</h3>\n";

			$sql="SELECT * FROM eb_epreuves WHERE id='$id_epreuve_modele';";
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				echo "<p style='color:red'>The test n°$id_epreuve_modele was not found.</p>\n";
			}
			else {
				$lig=mysql_fetch_object($res);
				...
				echo "<p><b>Model&nbsp;:</b> Test n°$lig->id&nbsp;:</p>\n";
				...
				$res=mysql_query($sql);
				if(mysql_num_rows($res)==0) {
					echo "<p>The test n°$id_epreuve_modele n'est associée à aucun groupe/teaching.</p>\n";
				}
				else {
					echo "<p>List lesson of the model test&nbsp;:</p>\n";
				...
				$res=mysql_query($sql);
				if(mysql_num_rows($res)==0) {
					echo "<p>The test n°$id_epreuve_modele is associated no room.</p>\n";
				}
				else {
					echo "<p>List rooms&nbsp;:</p>\n";
					echo "<p style='margin-left: 3em;'>";
					...
					if(mysql_num_rows($res_ele_salle)==0) {
						echo "<p>No student is affected in a room for the test n°$id_epreuve_modele.</p>\n";
					}
					else {
						$current_id_salle="";
						$cpt=0;
						while($lig=mysql_fetch_object($res_ele_salle)) {
							...
							if($lig->id_salle!='-1') {
								...
								if($lig->id_salle!=$current_id_salle) {
									...
									//if($lig->id_salle!='-1') {
										echo "To copy the assignments of student in ".$tab_salle[$lig->id_salle]."</label>";
									/*
									}
									else {
				...
				$res=mysql_query($sql);
				if(mysql_num_rows($res)==0) {
					echo "<p>The test n°$id_epreuve_modele is associated to no corrector.</p>\n";
				}
				else {
					echo "<p>List correctors &nbsp;:</p>\n";
					echo "<p style='margin-left: 3em;'>";
					while($lig=mysql_fetch_object($res)) {
					...
					if(mysql_num_rows($res_ele_prof)==0) {
						echo "<p>No copy student is assigned to a corrector for the test n°$id_epreuve_modele.</p>\n";
					}
					else {
						$current_login_prof="";
						$cpt=0;
						while($lig=mysql_fetch_object($res_ele_prof)) {
							...
							if($lig->login_prof!=$current_login_prof) {
								...
								echo "<label for='copie_affect_copie_prof_".$lig->login_prof."'>To copy the assignments copies student with the corrector ".$tab_prof[$lig->login_prof]."</label>";
								//echo "<table class='boireaus' summary=\"Liste des élèves affectés en Salle $tab_salle[$lig->id]\">\n";
								echo "<br />\n";
								echo "<span class='infobulle_css'>\n";
								$cpt=0;
				...
				echo "<input type='submit' name='copier_les_parametres' value='To copy the selected parameters' />\n";
				echo "</form>\n";
	...
	if(mysql_num_rows($res)>0) {
		...
		while($lig=mysql_fetch_object($res)) {
			...
			if(mysql_num_rows($test1)!=mysql_num_rows($test2)) {
				echo "<span style='color:red;'>The numbers anonymities are not single on the test (<i>that should not arrive</i>).<br />The seizure is not possible on the test </span>";
				if($lig->description!='') {
					echo "<a href='#'";
					echo " onmouseover=\"delais_afficher_div('div_epreuve_".$lig->id."','y',-100,20,1000,20,20)\" onmouseout=\"cacher_div('div_epreuve_".$lig->id."')\"";
				...
				//echo "Modifier <a href='".$_SERVER['PHP_SELF']."?id_epreuve=$lig->id&amp;modif_epreuve=y'";
				echo "To seize the notes for <a href='saisie_notes.php?id_epreuve=$lig->id'";
				if($lig->description!='') {
					echo " onmouseover=\"delais_afficher_div('div_epreuve_".$lig->id."','y',-100,20,1000,20,20)\" onmouseout=\"cacher_div('div_epreuve_".$lig->id."')\"";
