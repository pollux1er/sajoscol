...
secours='F',
autre='F',
description='Practice test: Report cards',
statut='';";
					...
					for($j=0;$j<$nb_matieres;$j++) {
						...
						while($lig_dev=mysql_fetch_object($res_dev)) {
							...
							for($i=0;$i<$nb_classes;$i++) {
								...
								$tab_note["$lig_dev->login"][$tab_id_classe[$i]]["$tab_matiere[$j]"]["info_dev"]="Note except lesson of the year.";
/*
								if($lig_dev->statut=='') {
...
//**************** EN-TETE *****************
$titre_page = "Practice test: Report cards";
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($acces_mod_exb_prof=='y')) {
	...
	*/
	...
	echo "<form method=\"post\" action=\"".$_SERVER['PHP_SELF']."\" name='form1'>\n";
	echo "<p class='bold'>Choose the classes for which you wish to publish bulletins&nbsp;:</p>\n";
	for($i=0;$i<$nb_classes;$i++) {
	...
	echo "<input type='submit' name='Imprimer' value='Printing' />\n";
	echo "<input type='hidden' name='id_exam' value='$id_exam' />\n";
	echo "<input type='hidden' name='mode' value='Printing' />\n";
	echo "</form>\n";

	echo "<p><a href='#' onClick='ModifCase(true)'>tickr</a> / <a href='#' onClick='ModifCase(false)'>décocher</a> all classes</p>\n";
...
function ModifCase(mode) {
	...
	echo "<p><br /></p>\n";
	echo "<p style='color:red;'><i>PROBLEM&nbsp;:</i> The student who change class are badly managed.<br />It would be necessary to choose one reference period for the
memberships of the student for a class.</p>\n";
