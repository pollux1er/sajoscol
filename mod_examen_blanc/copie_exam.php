...
secours='F',
autre='F',
description='Practice test: Copy',
statut='';";
...
if(!preg_match('/^[0-9]+$/', $id_exam)) {
	header("Location: index.php?msg=".rawurlencode("None id_exam was not selected."));
	die();
}
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($acces_mod_exb_prof=='y')) {
	...
	if(isset($id_exam_modele)) {
		...
		if(mysql_num_rows($res)==0) {
			$msg="The examination models selected (<i>$id_exam_modele</i>) do not exist.\n";
		}
		else {
			...
			if($copier_classes=="y") {
				if($vider_param_anterieurs=="y") {
					...
					$del=mysql_query($sql);
					if(!$del) {
						$msg.="Error during the cleaning of the classes.<br />\n";
						$nb_err++;
					}
				}
				...
				if(mysql_num_rows($res_modele)>0) {
					while($lig_modele=mysql_fetch_object($res_modele)) {
						...
						if(mysql_num_rows($test)==0) {
							...
							$insert=mysql_query($sql);
							if(!$insert) {
								$msg.="Error during association with the class ".get_nom_classe($lig_modele->id_classe).".<br />\n";
								$nb_err++;
							}
							else {
								$nb_reg++;
			...
			if($copier_matieres=="y") {
				if($vider_param_anterieurs=="y") {
					...
					$del=mysql_query($sql);
					if(!$del) {
						$msg.="Error during the cleaning of the matters.<br />\n";
						$nb_err++;
					}
				}
				...
				if(mysql_num_rows($res_modele)>0) {
					while($lig_modele=mysql_fetch_object($res_modele)) {
						...
						if(mysql_num_rows($test)==0) {
							...
							$insert=mysql_query($sql);
							if(!$insert) {
								$msg.="Error during association with the matter ".$lig_modele->matiere."<br />\n";
								$nb_err++;
							}
							else {
								$nb_reg++;
				...
				if(mysql_num_rows($res_modele)>0) {
					while($lig_modele=mysql_fetch_object($res_modele)) {
						...
						if(mysql_num_rows($test)>0) {
							...
							if(!$update) {
								$msg.="Error at the time of the update of the coefficients and no-claims bonus of the matter ".$lig_modele->matiere."<br />\n";
								$nb_err++;
							}
							else {
								$nb_reg++;
			...
			if($copier_groupes=="y") {
				if($vider_param_anterieurs=="y") {
					...
					$del=mysql_query($sql);
					if(!$del) {
						$msg.="Error during the cleaning of the groups.<br />\n";
						$nb_err++;
					}
				}
				...
				if(mysql_num_rows($res_modele)>0) {
					while($lig_modele=mysql_fetch_object($res_modele)) {
						...
						if(mysql_num_rows($test)==0) {
							...
							$insert=mysql_query($sql);
							if(!$insert) {
								$msg.="Error during association with the group n°".$lig_modele->id_groupe."<br />\n";
								$nb_err++;
							}
							else {
								$nb_reg++;
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($acces_mod_exb_prof=="y")) {
	...
	if(mysql_num_rows($res_autres_exam)==0) {
		echo "<p style='color:red'>No other practice test is recorded.</p>\n";
	...
	if(count($tab_exam)==0) {
		echo "<p style='color:red'>You do not have any other recorded practice test.</p>\n";
	...
	echo "<form method=\"post\" action=\"".$_SERVER['PHP_SELF']."\" name='form1'>\n";
	echo "<p class='bold'>To copy the parameters of the examination&nbsp;:</p>\n";
	echo "<table class='boireaus' summary='Table of the practice tests'>\n";
	echo "<tr>\n";
	echo "<th>Choice</th>\n";
	echo "<th>Id</th>\n";
	echo "<th>Intitule</th>\n";
	...
	echo "<p class='bold'>Which parameters to copy&nbsp;?</p>\n";
	echo "<p>\n";
	...
	echo "<input type='checkbox' name='copier_matieres' id='copier_matieres' value='y' onchange=\"checkbox_change('copier_matieres');changement()\" /><label for='copier_matieres' id='texte_copier_matieres'>Subject</label><br />\n";
	echo "<input type='checkbox' name='copier_groupes' id='copier_groupes' value='y' onchange=\"checkbox_change('copier_groupes');changement()\" /><label for='copier_groupes' id='texte_copier_groupes'>Lesson (<i>groups</i>)</label><br />\n";
	echo "<input type='checkbox' name='copier_coef' id='copier_coef' value='y' onchange=\"checkbox_change('copier_coef');changement()\" /><label for='copier_coef' id='texte_copier_coef'>Coefficients and no-claims bonus</label><br />\n";
	//echo "<input type='checkbox' name='' id='' value='y' /><label for=''></label><br />\n";
	echo "</p>\n";

	echo "<p><input type='checkbox' name='vider_param_anterieurs' id='vider_param_anterieurs' value='y' onchange=\"checkbox_change('vider_param_anterieurs');changement()\" /><label for='vider_param_anterieurs' id='texte_vider_param_anterieurs'>To empty the possible former selections of classes, groups, matters... of the practice test  n°$id_exam</label></p>\n";
