<?php
/* $Id: index.php 7691 2011-08-10 19:25:29Z crob $ */
/*
...
secours='F',
autre='F',
description='Practice test: Reception',
statut='';";
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($acces_mod_exb_prof=='y')) {
	...
	if(isset($id_exam)) {
		...
		if(mysql_num_rows($res)==0) {
			$msg="The selected examination (<i>$id_exam</i>) do not exist.\n";
		}
		else {
	...
	if((isset($_POST['creer_exam']))||(isset($_POST['modif_exam']))) {
		...
		if($intitule=="") {$intitule="Practice test";}
		...
		if(!isset($id_exam)) {
			...
			if($insert=mysql_query($sql)) {
				...
				$msg="Examination n°$id_exam : '$intitule' created.<br />";
			}
			else {
				$msg="ERROR during the creation of the examination '$intitule'.<br />";
				//$msg.="<br />$sql";
			}
		}
		else {
			...
			if($update=mysql_query($sql)) {
				$msg="Examination n°$id_exam: '$intitule' update.";
			}
			else {
				$msg="ERROR during the modification of the examination '$intitule'.";
				//$msg.="<br />$sql";
			}
		}
		$mode="modif_exam";
	...
	elseif((isset($id_exam))&&($mode=='modif_coef')) {
		...
		for($i=0;$i<count($tab_matiere);$i++) {
			if(isset($coef[$i])) {
				...
				if(preg_match("/[^0-9\.]/",$coef[$i])) {
					$msg.="$coef[$i] contains nonnumerical characters.<br />\n";
					$enregistrer='n';
				}
				elseif(strlen(preg_replace("/[^\.]/","",$coef[$i]))>1) {
					$msg.="There are several POINTS in $coef[$i]<br />\n";
					$enregistrer='n';
				}
		...
		if($nb_reg>0) {$msg.="Coefficients and updated no-claims bonus.<br />";}
	...
	elseif((isset($id_exam))&&($mode=='suppr_exam')) {
		...
		for($i=0;$i<count($tab_tables);$i++) {
			...
			if(!$suppr) {
				$msg="ERROR during the suppression of the examination $id_exam";
				//for($j=0;$j<$i;$j++) {$msg.=""}
				unset($id_exam);
				unset($mode);
				break;
		...
		if($msg=='') {
			...
			if(!$suppr) {
				...
				$msg="Suppression of the examination $id_exam carried out.";
			}
		}
		unset($id_exam);
		unset($mode);
		...
		if($_SESSION['statut']=='professeur') {
			for($i=0;$i<count($id_classe);$i++) {
				if(!is_pp($_SESSION['login'], $id_classe[$i])) {
					header("Location: ".$_SERVER['PHP_SELF']."?id_exam=$id_exam&msg=".rawurlencode("You are not ".getSettingValue('gepi_prof_suivi')." in the class of ".get_class_from_id($id_classe[$i])));
					die();
				}
			}
		}
	...
	elseif((isset($id_exam))&&($mode=='ajout_groupes')) {
		...
		if(mysql_num_rows($res)==0) {
			...
			if($nb_ajout>0) {$msg.="Addition of $nb_ajout groupe(s) carried out.<br />";}
			...
			if($groupe_hors_enseignement=='y') {
				...
				if(mysql_num_rows($test)==0) {
					...
					$insert=mysql_query($sql);
					if($insert) {$msg.="Creation of a group except lesson.<br />";}
				}
			}
		}
		$mode='modif_exam';
...
*/
	elseif((isset($id_exam))&&($mode=='modif_choix_dev')) {
		...
		$res=mysql_query($sql);
		if(mysql_num_rows($res)==0) {
			$msg="The examination n°$id_exam do not exist.<br />";
		}
		else {
			$nb_enr=0;
			for($i=0;$i<count($id_groupe);$i++) {
				...
				if($id_dev=='Plusieurs_periodes') {
					...
					if(count($id_dev_liste_periode)==0) {
						$msg.="ERROR: No period was selected for the group ".$id_groupe[$i].".<br />";
					}
					else {
						...
						if(mysql_num_rows($res_id_ex_grp)==0) {
							$msg.="Identifier of the group in ex_groupe not found for the examination $id_exam and groups it $id_groupe[$i].<br />";
						}
						else {
...
*/
			...
			if($nb_enr>0) {$msg.="Update of the list of the duties carried out.<br />";}
		}
		$mode='modif_exam';
	}
...
//**************** EN-TETE *****************
$titre_page = "Practice test: Reception";
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($acces_mod_exb_prof=="y")) {
	if(!isset($id_exam)) {
		...
		if(!isset($mode)) {
			...
			echo "<li>\n";
			echo "<p><a href='".$_SERVER['PHP_SELF']."?mode=creer_exam'>To create a new examination</a></p>\n";
			echo "</li>\n";
			...
			if(mysql_num_rows($res)>0) {
				...
				while($lig=mysql_fetch_object($res)) {
					...
					if($afficher_cet_examen_blanc=="y") {
						...
						echo " - <a href='".$_SERVER['PHP_SELF']."?id_exam=$lig->id&amp;mode=suppr_exam".add_token_in_url()."' onclick=\"return confirm('Are you sure to want to remove examination?')\">Delecte</a><br />\n";
					}
				}
				echo "</li>\n";
			}
			...
			echo "<p style='color:red'>TO STILL MAKE&nbsp;: A bond to empty all the tables of practice tests.<br />Is it necessary to empty these tables at the time of initialization?<br />If so, perhaps to add a conservation in the tables filings (previous years).</p>\n";
		}
		//===========================================================================
		...
		elseif($mode=='creer_exam') {
			...
			echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
			echo ">Small practice tests</a>\n";
			echo "</p>\n";

			echo "<p class='bold'>Creation of a practice test&nbsp;:</p>\n";

			echo "<blockquote>\n";
			echo "<form method=\"post\" action=\"".$_SERVER['PHP_SELF']."\" name='form1'>\n";
			echo add_token_field();

			echo "<table summary='Parameters'>\n";
			echo "<tr>\n";
			echo "<td>Intitule&nbsp;:</td>\n";
			...
			echo "<tr>\n";
			echo "<td>Date from the examination &nbsp;:</td>\n";
			echo "<td>\n";
	...
	elseif($mode=='modif_exam') {
		...
		if(!isset($aff)) {
			...
			echo "<p><b>Modification of a practice test&nbsp;:</b> Examination n°$id_exam</p>\n";

			$sql="SELECT * FROM ex_examens WHERE id='$id_exam';";
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				echo "<p style='color:red;'>ERROR&nbsp;: The examination $id_exam do not exist.</p>\n";
			...
			//if(mysql_num_rows($res_groupes)>0) {
				...
				if(mysql_num_rows($res_autres_exam)>0) {
					...
					if($acces_copie_exam=="y") {
						...
						echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
						echo ">Copy the parameters of another practice test</a></p>\n";
						echo "</li>\n";
					}
				}
				...
				if($nb_classes>0) {
					...
					echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
					echo ">Modify the list of the classes</a></p>\n";
				}
				else {
					echo "<p><a href='".$_SERVER['PHP_SELF']."?id_exam=$id_exam&amp;mode=modif_exam&amp;aff=classes'";
					echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
					echo ">Add classes</a></p>\n";
				}
				echo "</li>\n";
				echo "<li>\n";
				...
				if($nb_matieres>0) {
					...
					echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
					echo ">Modify the list of the matters</a></p>\n";
				}
				else {
					echo "<p><a href='".$_SERVER['PHP_SELF']."?id_exam=$id_exam&amp;mode=modif_exam&amp;aff=matieres'";
					echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
					echo ">Add matters</a></p>\n";
				}
				echo "</li>\n";
				echo "<li>\n";
				echo "<p><a href='releve.php?id_exam=$id_exam'";
				echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
				echo ">Publish statements of notes and averages</a></p>\n";
				echo "</li>\n";

				echo "<li>\n";
				echo "<p><a href='bull_exb.php?id_exam=$id_exam'";
				echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
				echo ">Publish bulletins</a></p>\n";
				echo "</li>\n";
				echo "</ul>\n";
			...
			*/
			...
			echo "<tr>\n";
			echo "<td style='font-weight:bold;'>Date from the examination &nbsp;:</td>\n";
			echo "<td>\n";
			...
			//=================================
			echo "<table border='0' summary='Working'>\n";
			echo "<tr>\n";
			echo "<td valign='top'>\n";
			//$sql="SELECT c.classe, ec.id_classe FROM ex_classes ec, classes c WHERE ec.id_exam='$id_exam' AND c.id=ec.id_classe ORDER BY c.classe;";
			//echo "$sql<br />";
			...
			//$nb_classes=mysql_num_rows($res_classes);
			if($nb_classes>0) {
				echo "<p class='bold'>List classes&nbsp;:</p>\n";
				echo "<ul>\n";
			...
			*/
			...
			if((count($tab_matiere)>0)&&(count($tab_classe)>0)) {
				...
				echo "<p class='bold'>Choice of the groups and duties&nbsp;:</p>\n";
				...
				echo "<th>Groups<br />Devoirs</th>\n";
				echo "<th>Coef</th>\n";
				echo "<th>No-claims bonus</th>\n";
				for($i=0;$i<count($tab_classe);$i++) {
					echo "<th>$tab_classe[$i]</th>\n";
				}
				if($grp_hors_enseignement=='y') {echo "<th>Except lesson</th>\n";}
				echo "</tr>\n";
				...
				for($j=0;$j<count($tab_matiere);$j++) {
					...
					echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
					echo ">Choice of the groups</a><br />\n";
							...
							while($lig=mysql_fetch_object($res_groupes)) {
								...
								if($lig->id_dev>0) {
									...
									if(mysql_num_rows($res_dev)==0) {
										echo "To have n°$lig->id_dev\n";
										echo "<span style='color:red;'>ERREUR&nbsp;: homework unknown???</span>";
									}
									else {
										...
										if(!in_array($lig->id_dev,$tab_dev)) {
											...
											$titre="homework n°$lig->id_dev (<i>$lig_per->nom_periode</i>)";
											...
											if($eff_tot>0) {
												...
												$texte.="<span style='color:red;'>No seized note</span>";
											}
								...
								elseif($lig->type=='moy_bull') {
									...
									if(!in_array("moy_bull_".$lig->id."_".$lig->valeur,$tab_bull)) {
										...
										$titre="Average bulletins (<i>$lig_per->nom_periode</i>)";
										$texte="<p><b>Averages of the pupils on the bulletins for the period $lig_per->nom_periode</b>";
										...
										if($eff_notes_bull>0) {
											...
											if($eff_non_note>0) {$texte.="$eff_non_note not note<br />\n";}
										}
										else {
											$texte.="<span style='color:red;'>No seized average</span>";
										}
								...
								elseif($lig->type=='moy_plusieurs_periodes') {
									...
									if(!in_array($chaine_mpp,$tab_moy_plusieurs_periodes)) {
										...
										$titre="Averages of the periods $lig->valeur";
										$texte="<p><b>Average of the averages of the pupils on the bulletins for the
periods $lig->valeur</b>";
										...
										$res_id_ex_grp=mysql_query($sql);
										if(mysql_num_rows($res_id_ex_grp)==0) {
											$texte.="<span style='color:red'>Identifier of the group in ex_groupe not found for the examination $id_exam and groups it $id_groupe[$i].</span><br />";
										}
										else {
					...
					if($grp_hors_enseignement=='y') {
						...
						if(mysql_num_rows($test)>0) {
							...
							if($eff_tot>0) {
								...
								echo "<span style='color:red;'>No seized note</span>";
							}
						}
						else {
							echo "&nbsp;";
				...
				echo "<p align='center'><input type='submit' name='reg_coef' value='To record the coefficients and no-claims bonus' /></p>\n";
				echo "<input type='hidden' name='id_exam' value='$id_exam' />\n";
			...
			echo "<p><i>A FAIRE&nbsp;:</i> Able to regulate the order of the subject (*), Publish bulletins.<br />Present also the table above and that of selection of the duties by
section of 5 or 6 classes at more.<br />Put the coef only on the first table.<br />(*) The current order is alphabetical while making pass at the end the
subject to no-claims bonus.</p>\n";
		...
		elseif($aff=='classes') {
			...
			*/
			...
			echo "<p class='bold'>Choice of the classes for the examination $id_exam&nbsp;:</p>\n";
			...
			if ($nb==0) {
				...
				while ($i < $nb) {
					...
					echo "/><label for='id_classe_$i'><span id='texte_id_classe_$i'$temp_style>Class : ".$classe.".</span></label><br />\n";
					$i++;
				}
				echo "</td>\n";
				echo "</tr>\n";
	...
	if(document.getElementById('id_classe_'+cpt)) {
		...
		elseif($aff=='matieres') {
			...
			*/
			...
			echo "<p class='bold'>Choice of the matters for the examination $id_exam&nbsp;:</p>\n";
			...
			if($nb==0) {
				echo "<p>No subject seems defined.</p>\n";
			}
			else {
				...
				echo "<table width='100%' summary='Choice of the subject'>\n";
				echo "<tr valign='top' align='center'>\n";
				...
				while($i<$nb) {
					...
					echo "/><label for='matiere_$i'><span id='texte_matiere_$i'$temp_style>Subject : ".$matiere."</span></label><br />\n";
					$i++;
				}
				echo "</td>\n";
				echo "</tr>\n";
	...
	if(document.getElementById('matiere_'+cpt)) {
		...
		elseif($aff=='groupes') {
			...
			*/
			...
			if(!isset($matiere)) {
				echo "<p style='color:red'>ERROR&nbsp;: No subject was selected.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
			...
			echo "<p class='bold'>Choice of the groups for the examination $id_exam&nbsp;: Class ".get_class_from_id($id_classe)." andsubject $matiere</p>\n";
			...
			echo "<table class='boireaus' summary='List groups'>\n";
...
*/
			...
			echo "/>\n";
			echo "<label for='groupe_hors_enseignement' style='cursor: pointer;'>Create a group except teaching</label><br />\n";
			echo "(<i>An phone 'groupe' allows for example to seize into final notes obtained with the French baccalaureat in first</i>).</p>\n";
			...
			echo "<p><a href='javascript:cocher_decocher(true)'>tick</a> / <a href='javascript:cocher_decocher(false)'>shoot</a> all groups.</p>\n";
...
function cocher_decocher(mode) {
	for (var k=0;k<$cpt;k++) {
		if(document.getElementById('id_groupe_'+k)){
			...
			echo "<p><br /></p>\n";
			echo "<p><i>Notice in connection with the notes except teaching&nbsp;:</i> One normally is created 'groupe' except teaching for a note having to be taken into account whereas it
does not correspond to a teaching exempted over the year (<i>exemple: note of French for BAC</i>).<br />One should not then have odd situation like that proposed/commented on
below&nbsp;:<br />If a student is registered for the same matter at the same time with a
duty (<i>or an average of bulletin</i>) and with a note except teaching, it is the note except teaching which is taken into account.</p>\n";
			echo "<p><br /></p>\n";
		...
		elseif($aff=='choix_dev') {
			...
			if($nb_classes==0) {
				echo "<p style='color:red'>No class was selected.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
			...
			echo "<p class='bold'>Choice of the groups for the examination $id_exam&nbsp;: Class";
			if($nb_classes>1) {
				echo "s";
			}
			echo " ";
					...
					while($lig=mysql_fetch_object($res)) {
						...
						if(mysql_num_rows($res2)>0) {
							...
							while($lig2=mysql_fetch_object($res2)) {
								if($lig2->periode!=$periode_precedente) {
									...
									echo "<label for='id_dev_".$cpt_grp."_$cpt' style='cursor: pointer;' alt='Average of the bulletin for the period' title='Average of the bulletin for the period'>";
									...
									if(isset($lig3)) {
										if($lig3->verouiller=='N') {echo "<img src='../images/icons/flag.png' width='17' height='18' alt='CAUTION: Not closed period' title='CAUTION: Not closed period' />\n";}
									}
									echo "<br />\n";
						...
						if(mysql_num_rows($res2)>0) {
							while($lig2=mysql_fetch_object($res2)) {
								if((!isset($tab_periodes[$cpt_grp]))||(!in_array($lig2->periode, $tab_periodes[$cpt_grp]))) {
									...
									if($lig2->verouiller=='N') {echo "<img src='../images/icons/flag.png' width='17' height='18' alt='CAUTION: Not closed period' title='CAUTION: Not closed period' />\n";}
									echo "<br />\n";
						...
						*/
						...
						if(isset($tab_periodes[$cpt_grp])) {
							...
							echo "/><label for='id_dev_".$cpt_grp."_$cpt' style='cursor: pointer;'><span id='texte_id_dev_".$cpt_grp."_$cpt' $temp_style>Average of several periods &nbsp;:</span></label><br />\n";
							...
							for($j=0;$j<count($tab_periodes[$cpt_grp]);$j++) {
								...
								echo "/><label for='id_dev_".$cpt_grp."_periodes_$j'>Period ".$tab_periodes[$cpt_grp][$j]."</label></span><br />\n";
							}
							$cpt++;
						}
