<?php
/* $Id: releve.php 8061 2011-08-30 22:01:10Z jjacquard $ */
/*
...
secours='F',
autre='F',
description='Practice test: Statement',
statut='';";
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($acces_mod_exb_prof=='y')) {
	...
	if(isset($id_exam)) {
		...
		if(mysql_num_rows($res_test_id_exam)==0) {
			...
			if($nb_classes==0) {
				$msg="<p>No class is associated the examination???</p>\n";
			}
			else {
				$tab_id_classe=array();
				$tab_classe=array();
				...
				if($nb_matieres==0) {
					$msg="<p>No subject is associated the examination???</p>\n";
				}
				else {
					$tab_matiere=array();
					$tab_coef=array();
									...
									if($lig_groupe->type=='moy_bull') {
										...
										if(mysql_num_rows($res_bull)>0) {
											...
											if(!in_array('bull_'.$lig_groupe->id_groupe.'_'.$lig_groupe->valeur,$tab_bull)) {
												...
												$titre="Average of the bulletin (<i>$lig_per->nom_periode</i>)";
												$texte="<p><b>Average of the bulletin over the period $lig_per->nom_periode</b>";
												$texte.="<br />";
									...
									elseif($lig_groupe->type=='') {
										...
										if(mysql_num_rows($res_dev)>0) {
											...
											if(!in_array($lig_groupe->id_dev,$tab_dev)) {
												...
												$titre="Homework n°$lig_groupe->id_dev (<i>$lig_per->nom_periode</i>)";
...
//echo "count(\$reserve_header_tabdiv_infobulle)=".count($reserve_header_tabdiv_infobulle)." pour div_dev_$lig_groupe->id_dev<br />";
									...
									elseif($lig_groupe->type='moy_plusieurs_periodes') {
										...
										if(!in_array($chaine_mpp,$tab_moy_plusieurs_periodes)) {
											...
											$titre="Averages of the periods $lig_groupe->valeur";
											$texte="<p><b>Averages of the averages of bulletins for the periods $lig_groupe->valeur</b>";
											$texte.="<br />";
					...
					if($mode=='pdf') {
						...
						$pdf->SetKeywords('');
						$pdf->SetSubject('Practice test '.$id_exam);
						$pdf->SetTitle('Practice test '.$id_exam);
						...
						for($i=0;$i<$nb_classes;$i++) {
							...
							// Titre
							$pdf->SetXY($x0, $y0);
							$texte="Statement of notes of the practice test n° $id_exam - Classe de ".$tab_classe[$i];
...
//**************** EN-TETE *****************
$titre_page = "Practice test: Statement";
...
if(!isset($id_exam)) {
	...
	echo "<p>Error&nbsp;: No examination was selected.</p>\n";
...
if(mysql_num_rows($res)==0) {
	...
	echo "<p>The selected examination (<i>$id_exam</i>) do not exist.</p>\n";
	require("../lib/footer.inc.php");
	die();
}
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($acces_mod_exb_prof=='y')) {
	...
	*/
	...
	if($nb_matieres==0) {
		...
		echo "<p>No subject is associated the examination???</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	//$csv="";
	for($i=0;$i<$nb_classes;$i++) {
		echo "<p class='bold'>Class $tab_classe[$i]</p>\n";
		...
		echo "<tr>\n";
		echo "<th>Coefficients and no-claims bonus /<br />Student</th>\n";
		for($j=0;$j<count($tab_matiere);$j++) {
			echo "<th>$tab_coef[$j]";
			if($tab_bonus[$j]=='y') {
				echo "<br />";
				echo "No-claims bonus";
			}
			echo "</th>\n";
		}
		//echo "<th></th>\n";
...
echo "<p style='color:red;'><i>NOTES&nbsp;:</i> The averages currently suppose that the reference frame of the homework is 20.<br />It will be necessary to modify to take into account notes on another thing that 20.<br />The ' bonus' consist in counting only the points higher than 10.<br />Ex.: for 12 (coef 3), 14 (coef 1) et 13 (coef 2 and no-claims bonus), calculation is (12*3+14*1+(13-10)*2)/(3+1)</p>\n";
echo "<p><br /></p>\n";
require("../lib/footer.inc.php");
?>
