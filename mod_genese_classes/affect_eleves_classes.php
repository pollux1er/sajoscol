...
secours='F',
autre='F',
description='Genesis of the classes: Assignment of the student',
statut='';";
...
if(isset($_POST['is_posted'])) {
	...
	if($nb_err==0) {
		$msg="$nb_reg recordings carried out.";
	}
	else {
		$msg="ERREUR: $nb_err errors during the recording of the future classes,...";
	}
	$msg.=$complement_msg;
}
...
//**************** EN-TETE *****************
$titre_page = "Genesis classifies: assignment of the student";
...
if(!isset($choix_affich)) {
	...
	if($nb_clas_act==0) {
		echo "<p>No current class is yet selected for this project.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
...
function change_display(id) {
	if(document.getElementById(id)) {
		...
		echo "<p class='bold'>Lists of definite postings</p>\n";
		while($lig_req_aff=mysql_fetch_object($res_req_aff)) {
			echo "<p><a href='#' onclick=\"change_display('id_aff_$lig_req_aff->id_aff')\">Display n°$lig_req_aff->id_aff</a>";
			echo "</p>\n";
			...
			if(mysql_num_rows($res)>0) {
				...
				while($lig=mysql_fetch_object($res)) {
					...
					if($lig->nom_requete!="") {
						...
						$txt_requete.="Request n°$lig->id_req";
					}
					$txt_requete.="</a></b>";
					...
					$txt_requete.=" - <a href='#' onclick=\"afficher_div('div_id_aff_".$lig_req_aff->id_aff."_id_req_".$lig->id_req."','y',100,100);\"><img src='../images/vert.png' width='16' height='16' title='Display the student of the request n°$id_req en infobulle' /></a>";
					$txt_requete.="<br />";
	...
	echo "<input type='checkbox' name='clas_fut[]' id='clas_fut_$cpt' value='' /><label for='clas_fut_$cpt'>Not yet affected</label><br />\n";
	$cpt++;
	echo "</td>\n";
	...
	echo "<p><i>NOTES&nbsp;:</i></p>\n";
	echo "<ul>\n";
	echo "<li>By selecting all the future classes and the Nonaffected ones, one
obtains a list with useful manpower in the options.<br />
	In top of table, one has the total staff complements and in bottom of
table, one has manpower of the selection.</li>";
...
according to the mode OR<br />
	...
	It will be for example '<i>With AGL1 AND ESP2 AND LATIN AND Without DECP3</i>'</li>\n";
	echo "<li>The lines of the column with profile are treated according to the mode
OR.<br />
	The lines of the column without profile are treated according to the
mode AND.<br />
	It will be for example '<i>With profile RAS OR profile B</i>'
	</li>\n";
	echo "</ul>\n";
}
else {
	...
	*/
	...
	echo "<p>Name of the request&nbsp;: <input type='text' name='nom_requete' value=\"$nom_requete\" ></p>\n";
	...
	if($chaine_classes_actuelles!="") {echo "Current classes $chaine_classes_actuelles<br />\n";}
	if($chaine_classes_futures!="") {echo "Future classes $chaine_classes_futures<br />\n";}
	if($chaine_avec_opt!="") {echo "With $chaine_avec_opt<br />\n";}
	if($chaine_sans_opt!="") {echo "Without $chaine_sans_opt<br />\n";}
	if($chaine_avec_profil!="") {echo "With profil $chaine_avec_profil<br />\n";}
	if($chaine_sans_profil!="") {echo "Without profil $chaine_sans_profil<br />\n";}
	echo "</a>\n";
	echo "&nbsp;</p>\n";
...
*/
	...
	echo "<th rowspan='2'>Class<br />actuelle</th>\n";
	echo "<th rowspan='2'>Profile</th>\n";
	echo "<th rowspan='2'>Level</th>\n";
	echo "<th rowspan='2'>Not justified absences Delays</th>\n";

	//if(count($classe_fut)>0) {echo "<th colspan='".(count($classe_fut)+2)."'>Classes futures</th>\n";}
	if(count($classe_fut)>0) {echo "<th colspan='".count($classe_fut)."'>Future classes</th>\n";}
	...
	if(count($autre_opt)>0) {echo "<th colspan='".count($autre_opt)."'>Other options</th>\n";}
	echo "</tr>\n";
	//==========================================
	echo "<tr>\n";
	...
	for($i=0;$i<count($classe_fut);$i++) {
		echo "<th>\n";
		echo "<a href=\"javascript:modif_colonne('classe_fut_$i',true);changement();\"><img src='../images/enabled.png' width='15' height='15' alt='All tick' /></a>";
	...
	echo "<th>student</th>\n";
	echo "<th>Sexe</th>\n";
	echo "<th>Class<br />actuelle</th>\n";
	echo "<th>Profile</th>\n";
	echo "<th>Level</th>\n";
	echo "<th>Not justified absences Delays</th>\n";
	...
	$titre="<span id='entete_div_photo_eleve'>student</span>";
	...
	*/
	...
	echo "<div id='div_test_aff_classe2' class='infobulle_corps' style='position:absolute; border:1px solid black;'>Future classes</div>\n";
...
echo "
	...
	echo "<ul>\n";
	//echo "<li></li>\n";
	echo "<li>Redoubling (<i>Red</i>) and Leaving (<i>Dep</i>) are excluded from the selection since already affected.</li>\n";
	echo "</ul>\n";
