...
secours='F',
autre='F',
description='Genesis of the classes: Posting of lists',
statut='';";
...
if((isset($projet))&&(isset($_POST['chgt_classe']))&&(isset($_POST['login_ele']))&&(isset($_POST['classe_fut']))) {
	...
	if($temoin=="y") {
		...
		$res=mysql_query($sql);
		if(!$res) {
			$msg="ERROR during the change of class of ".get_nom_prenom_eleve($_POST['login_ele'])."<br />\n";
		}
		else {
			$msg="Change of class of <b>".get_nom_prenom_eleve($_POST['login_ele'])."</b> towards <b>".$_POST['classe_fut']."</b> carried out.<br />\n";
		}
	}
}

//**************** EN-TETE *****************
$titre_page = "Genesis classifies: posting of lists";
...
if(!isset($afficher_listes)) {
	...
	//((isset($_POST['modifier_requete']))&&($_POST['modifier_requete']=='y'))) {
		...
		if((isset($_POST['modifier_requete']))&&($_POST['modifier_requete']=='y')) {
			...
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				echo "<p style='color:red'>The request n°$id_req is not associated posting n°$id_aff on the project $projet.</p>\n";
			...
			if(!$menage) {
				echo "<p style='color:red'>ERROR at the time of the preliminary household of the request n° $id_req posting n°$id_aff on the project $projet.</p>\n";
	...
	if($nb_clas_fut==0) {
		echo "<p>No future class is yet defined for this project.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	...
	if(mysql_num_rows($res)>0) {
		...
		if(isset($id_aff)) {
			...
			echo "<p align='center'><b>Affichage n°$id_aff&nbsp;:</b> <input type='submit' name='afficher_listes' value='Display the lists' /></p>\n";
			echo "</form>\n";
			echo "</div>\n";
		}
	...
	if(isset($id_aff)) {
		echo "<p class='bold'>List requests for posting n°$id_aff</p>\n";
	}
	...
	if((isset($_GET['editer_requete']))&&(isset($_GET['id_req'])&&($_GET['id_req']!="")&&(strlen(my_ereg_replace("[0-9]","",$_GET['id_req']))==0))) {
		$id_req=$_GET['id_req'];
		echo "<p class='bold'>Modification of the request n°$id_req</p>\n";
	...
	echo "<table class='boireaus' border='1' summary='Choice of the parameters'>\n";
	echo "<tr>\n";
	echo "<th>Current class</th>\n";
	echo "<th>Future class</th>\n";
	if($nb_lv1>0) {echo "<th>LV1</th>\n";}
	if($nb_lv2>0) {echo "<th>LV2</th>\n";}
	if($nb_lv3>0) {echo "<th>LV3</th>\n";}
	if($nb_autre>0) {echo "<th>Another option</th>\n";}
	echo "<th>Profile</th>\n";
	echo "</tr>\n";
	...
	echo "/><label for='id_clas_act_$cpt'>Redoubling</label><br />\n";
	$cpt++;
	...
	echo "/><label for='id_clas_act_$cpt'>Newcomers</label><br />\n";
	$cpt++;
	echo "</td>\n";
	...
	echo "/><label for='clas_fut_$cpt'>Not yet affected</label><br />\n";
	$cpt++;
	...
	*/
	...
	if(isset($_GET['editer_requete'])) {
		echo "<p align='center'><input type='submit' name='ajouter' value='Modify the request' /></p>\n";
	}
	else {
		echo "<p align='center'><input type='submit' name='ajouter' value='Add' /></p>\n";
	}
	//================================
	...
	if(isset($id_aff)) {
		...
		if(mysql_num_rows($res)>0) {
			while($lig=mysql_fetch_object($res)) {
				...
				$txt_requete.="<b><label for='suppr_$lig->id_req'>Request n°$lig->id_req</label> <a href='".$_SERVER['PHP_SELF']."?editer_requete=y&amp;id_aff=$id_aff&amp;id_req=$lig->id_req&amp;projet=$projet'><img src ='../images/edit16.png'
width='16' height='16' alt='Publish the parameters of the request' /></a></b>";
				...
				$res2=mysql_query($sql);
				if(mysql_num_rows($res2)>0) {
					$txt_requete.="With the options (<span style='color:green;'>";
					$cpt=0;
					while($lig2=mysql_fetch_object($res2)) {
						if($cpt>0) {$txt_requete.=", ";}
						$txt_requete.=$lig2->valeur;
				...
				$res2=mysql_query($sql);
				if(mysql_num_rows($res2)>0) {
					$txt_requete.="Without the options (<span style='color:red;'>";
					$cpt=0;
					while($lig2=mysql_fetch_object($res2)) {
						if($cpt>0) {$txt_requete.=", ";}
						$txt_requete.=$lig2->valeur;
			...
			echo "<p>Notch the requests to be removed.</p>\n";
	...
	echo "<p><i>NOTES&nbsp;:</i></p>\n";
	echo "<ul>\n";
	echo "<li>The columns Classifies current and Classifies future are treated
according to the mode OR<br />
	If you notch two classes, the pupils taken into account will be '<i>member of Class 1 OR member of Class 2</i>'</li>\n";
	echo "<li>The columns of options are treated according to the mode AND.<br />
	It will be for example '<i>With AGL1 AND With ESP2 AND With LATIN AND Without DECP3</i>'</li>\n";
	//echo "<li></li>\n";
	echo "</ul>\n";
	...
	if(!isset($id_aff)) {
		echo "<p>ERROR: The variable 'id_aff' is not affected.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}

	echo " | <a href='".$_SERVER['PHP_SELF']."?projet=$projet&amp;id_aff=$id_aff'>Modify the list of requests for posting n°$id_aff</a>";
	echo "</p>\n";
	...
	if(mysql_num_rows($res)>0) {
		...
		echo "<a href='#' onclick=\"afficher_div('recap_eff','y',-100,20);return false;\">Manpower of the requests</a>";
		echo "</div>\n";
...
//============================================================
	...
	$res=mysql_query($sql);
	if(mysql_num_rows($res)==0) {
		echo "<p>No current class is still selected for this project.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	else {
	...
	echo "<script type='text/javascript'>
	// Table of lists to ensure the colour application of the lines
...
//============================================================
	...
	// Colorisation
	echo "<p>Colour application &nbsp;: ";
	echo "<select name='colorisation' onchange='lance_colorisation()'>
	<option value='classe_fut' selected>Future class</option>
...
//============================================================
	...
	for($loop=0;$loop<count($tab_id_req);$loop++) {
		...
		$fin_lien_affect="' target='_blank' alt='Modify the request n°$id_req' title='Modify the request n°$id_req'><b>Request n°$id_req</b></a>";
		$fin_lien_affect.=" - <a href='#' onclick=\"afficher_div('div_requete_$id_req','y',100,100); return false;\"><img src='../images/vert.png' width='16' height='16' title='To post the request n°$id_req in infobulle' /></a>";
		$fin_lien_affect.="<br />";
		...
		while($lig=mysql_fetch_object($res)) {
			switch($lig->type) {
				...
				case 'avec_autre':
					...
					if(!isset($tab_requete[2])) {$tab_requete[2]="With the options (<span style='color:green;'>";$tab_requete_csv[2]="Avec les options (";} else {$tab_requete[2].=", ";$tab_requete_csv[2].=", ";}
					$tab_requete[2].=$lig->valeur;$tab_requete_csv[2].=$lig->valeur;
				...
				case 'avec_profil':
					...
					if(!isset($tab_requete[1])) {$tab_requete[1]="With profile (<span style='color:black;'>";$tab_requete_csv[1]="With profile (";} else {$tab_requete[1].=", ";$tab_requete_csv[1].=", ";}
					$tab_requete[1].=$lig->valeur;$tab_requete_csv[1].=$lig->valeur;
				...
				case 'sans_lv1':
					$sans_lv1[]=$lig->valeur;
					if(!isset($tab_requete[3])) {$tab_requete[3]="Without the options (<span style='color:red;'>";$tab_requete_csv[3]="Without the options (";} else {$tab_requete[3].=", ";$tab_requete_csv[3].=", ";}
				...
				case 'sans_lv2':
					$sans_lv2[]=$lig->valeur;
					if(!isset($tab_requete[3])) {$tab_requete[3]="Without the options (<span style='color:red;'>";$tab_requete_csv[3]="Sans les options (";} else {$tab_requete[3].=", ";$tab_requete_csv[3].=", ";}
					$tab_requete[3].=$lig->valeur;$tab_requete_csv[3].=$lig->valeur;
				...
				case 'sans_profil':
					...
					if(!isset($tab_requete[1])) {$tab_requete[1]="Without profile (<span style='color:black;'>";$tab_requete_csv[1]="Sans profil (";} else {$tab_requete[1].=", ";$tab_requete_csv[1].=", ";}
					$tab_requete[1].=$lig->valeur;$tab_requete_csv[1].=$lig->valeur;
		...
		$lignes_requetes.="<a href='#requete_$id_req'>Request n°$id_req</a> (<i>\n";
		...
		$contenu_affichage_requete_courante.="<th>Student</th>\n";

		$contenu_affichage_requete_courante.="<th>Profile</th>\n";
		$contenu_affichage_requete_courante.="<th>Level</th>\n";
		$contenu_affichage_requete_courante.="<th>Absences</th>\n";
		$contenu_affichage_requete_courante.="<th>Class<br />current</th>\n";
						...
						if(($fut_classe!='Red')&&($fut_classe!='Dep')&&($fut_classe!='')) {
							for($i=0;$i<count($tab_ele_opt);$i++) {
								if(in_array($tab_ele_opt[$i],$tab_opt_exclue["$fut_classe"])) {
									$contenu_affichage_requete_courante.="<span style='color:red;'>ERROR: The option $tab_ele_opt[$i] is excluded in $fut_classe</span>";
								}
							}
						}

						$titre_chgt_classe="Change classifies ".strtoupper($lig->nom)." ".ucfirst(strtolower($lig->prenom));
		...
		if(count($tab_moy_eleves)>0) {
			...
			$contenu_affichage_requete_courante.="<table class='boireaus' summary='Table of the average, median, min et max'>\n";
			$contenu_affichage_requete_courante.="<tr class='lig1'>\n";
			$contenu_affichage_requete_courante.="<th>Average</th>\n";
			$contenu_affichage_requete_courante.="<td>".moyenne($tab_moy_eleves)."</td>\n";
			$contenu_affichage_requete_courante.="</tr>\n";
	
			$contenu_affichage_requete_courante.="<tr class='lig-1'>\n";
			$contenu_affichage_requete_courante.="<th>Median</th>\n";
	...
	echo "<p>Manpower of these selections &nbsp;: ".count($tab_ele_toutes_requetes)."</p>\n";

	if(count($tab_exclus)>0) {
		echo "<p><a href='#' onclick=\"modif_div_exclus();return false;\">List student except selection</a></p>\n";
		echo "<div id='div_exclus'>\n";
		echo "<p>Manpower out of the selections&nbsp;: ".count($tab_exclus)."</p>\n";
		echo "<table class='boireaus' border='1' summary='List student except selection'>\n";
		echo "<tr><th>student</th><th>Options</th></tr>\n";
		$alt=1;
	...
	if($user_temp_directory) {
		...
		echo "Files <a href='../temp/".$user_temp_directory."/listes_".$projet."_".$date_fichier.".csv'>CSV</a>\n";
		echo " - ";
	...
	document.getElementById('div_ods').innerHTML=\"Fichiers <a href='../temp/".$user_temp_directory."/listes_".$projet."_".$date_fichier.".csv'>CSV</a> <a href='genere_ods.php?fichier_csv=listes_".$projet."_".$date_fichier.".csv&amp;projet=$projet&amp;detail=non' target='_blank'>ODS</a> <a href='genere_ods.php?fichier_csv=listes_".$projet."_".$date_fichier.".csv&amp;projet=$projet&amp;detail=oui' target='_blank'>ODS_detaillé</a>\";
		...
		echo "<p style='color:red'>The temporary file of the user does not exist.<br />One does not generate a file CSV.</p>\n";

		echo "<script type='text/javascript'>
	document.getElementById('div_ods').innerHTML=\"<p style='color:red'>The temporary file of the user does not exist.<br />One does not generate a file CSV.</p>\";
</script>\n";
	}
	//=================================
...
echo "
	...
	echo "<p style='bold'>Summary of manpower &nbsp;:</p>\n";
	echo $lignes_requetes;

	$titre="Summary of manpower";
