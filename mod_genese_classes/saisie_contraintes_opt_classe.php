...
secours='F',
autre='F',
description='Genesis of the classes: Seizure of the constraints options/classes',
statut='';";
...
if((isset($is_posted))&&(isset($projet))) {
	...
	if(isset($_POST['ajouter'])) {
		...
		if($nb_err>0) {$msg="$nb_err errors at the time of the addition.";}
		elseif($nb_reg>0) {$msg="$nb_reg recordings.";}
	}
	elseif(isset($_POST['suppr'])) {
		...
		if($nb_err>0) {$msg="$nb_err errors during the suppression.";}
		elseif($nb_suppr>0) {$msg="$nb_suppr suppressions.";}
	}
}

//**************** EN-TETE *****************
$titre_page = "Genesis classifies: Constraints options/classes";
...
if($nb_opt==0) {
	echo "<p>No option is yet defined for this project.</p>\n";
	require("../lib/footer.inc.php");
	die();
}
...
echo "<p>You can seize here the options which one should not find on some
classes.</p>\n";
...
echo "<table class='boireaus' border='1' summary='Choice of the constraints'>\n";
echo "<tr>\n";
echo "<th rowspan='2'>Future class</th>\n";
echo "<th colspan='$cpt'>Unauthorized options</th>\n";
echo "</tr>\n";
...
if(mysql_num_rows($res)>0) {
	...
	echo "<p>If you wish to relieve beforehand definite stresses, notch and validate &nbsp;:</p>\n";
	$cpt=0;
	$classe_prec="";
	$alt=1;
	echo "<table class='boireaus' border='1' summary='Seized constraints'>\n";
	while($lig=mysql_fetch_object($res)) {
		...
		echo "<input type='checkbox' name='suppr[]' id='suppr_$cpt' value='$lig->id' /><label for='suppr_$cpt'> <b>$lig->classe_future</b>&nbsp;: Not $lig->opt_exclue</label><br />\n";
	...
	echo "<input type='hidden' name='is_posted' value='y' />\n";
	echo "<input type='hidden' name='projet' value='$projet' />\n";
	echo "<p align='center'><input type='submit' name='supprimer' value='Remove' /></p>\n";
	echo "</form>\n";
}
