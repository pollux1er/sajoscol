...
if (isset($_POST['is_posted_notes'])) {
  ...
  if (!saveSetting("active_inscription_utilisateurs", $_POST['activer']))
    ...
    if (!saveSetting("mod_inscription_explication", $imp)) $msg .= "Error during the recording of the parameter mod_inscription_explication !";
  }
  if (!saveSetting("mod_inscription_titre", $_POST['mod_inscription_titre']))
    $msg .= "Error during the recording of mod_inscription_titre !";
}
...
//**************** EN-TETE *****************
$titre_page = "Configuration of the module Inscription";
...
if (isset($_GET['action']) and ($_GET['action'] == "ajout")) {
    echo "<p class=bold><a href=\"./inscription_config.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a> | <a href=\"./inscription_config.php?action=ajout\">To add one item</a> | <a href=\"javascript:centrerpopup('help.php',800,500,'scrollbars=yes,statusbar=no,resizable=yes')\">Help</a></p>\n";
    ...
    echo "<H2>Addition of one item</H2>\n";
    echo "<p>one item corresponds to an entity (training course, intervention in the establishments, meeting...) to which the users can be registered.</p>\n";
    echo "<table cellpadding=\"6\">\n";
    echo "<tr><td>Date (au format AAAA/MM/JJ) : </td><td><input type=\"text\" name=\"date\" value=\"$date\" size=\"20\" /></td></tr>\n";
    echo "<tr><td>Hour : </td><td><input type=\"text\" name=\"heure\" value=\"$heure\" size=\"20\" /></td></tr>\n";
    echo "<tr><td>Description (place, ...) : </td><td><input type=\"text\" name=\"description\" value=\"$description\" size=\"50\" /></td></tr>\n";
...
echo "<p class=bold><a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return</a> | <a href=\"./inscription_config.php?action=ajout\">Add one item</a> | <a href=\"javascript:centrerpopup('help.php',800,500,'scrollbars=yes,statusbar=no,resizable=yes')\">Help</a></p>\n";
echo "<form name=\"formulaire\" method=\"post\"  action=\"inscription_config.php\">\n";
echo add_token_field();
echo "<H2>Activation  / Desactivation</H2>\n";
$active_prof = getSettingValue("active_inscription_utilisateurs");
if ($active_prof == "y") {
  echo "Currently, the page authorizing the inscriptions is activated : users (professors, cpe, schooling) can thus be registered.";
} else {
  echo "Currently, the page authorizing the inscriptions is not activated : the users (professors, cpe, schooling) cannot be registered.";
}
...
&nbsp;Activate access with the inscriptions</label><br />
...
&nbsp;Deactivate the access to the inscriptions</label>
<?php
echo "<H2>Liste des items</H2>\n";
if ($nombre_lignes != 0) {
  echo "<p>Each item below corresponds to an entity (training course, intervention in the establishments, meeting...) to which the users can be registered.</p>\n";
  ...
  echo "<td><p class='bold'>Hour</p></td>\n";
  echo "<td><p class='bold'><a href='inscription_config.php?order_by_=description'>Description (place...)</a></p></td>\n";
  echo "<td><p class='bold'>Currently registered people</p></td>\n";
  ...
  while ($i < $nombre_lignes){
    ...
    echo "<td><a href=\"./inscription_config.php?id_inter=$id&amp;action=supprimer\" onclick=\"javascript:return confirm('Are you sure to want to remove this item ?')\">Delected</a></td>\n";
    echo "</tr>\n";
    $i++;
  }
  echo "</table>\n";
} else {
  echo "<p>Currently no item is present in the base.";
}
$contenu = getSettingValue("mod_inscription_explication");
echo "<H2>Title of module</H2>\n";
echo "<input type=\"text\" name=\"mod_inscription_titre\" size=\"40\" value=\"".getSettingValue("mod_inscription_titre")."\" />\n";
echo "<H2>Explanatory text</H2>\n";
echo "<p>The text below will be visible by the people reaching the module of inscription/désincription.</p>\n";
