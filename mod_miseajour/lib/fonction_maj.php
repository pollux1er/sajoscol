...
function envoi_ftp($tableau, $source, $destination)
 {
	...
	if($conn_id===FALSE)
         {
	       ...
	       if($conn_id===FALSE) { $message_ftp['connection']='Impossible to connect itself to waiter ftp: '.$ftp_server; exit(); } else { $message_ftp['connection']='Connected to waiter ftp in not protected transfer'; }
	 } else { $message_ftp['connection']='Connected to waiter ftp in protected transfer'; }
	...
	if($login_result===FALSE)
	 {
		...
		$message_ftp['authentification']='The name of user or the password is erroneous for the user: '.$ftp_user_name;
		exit();
	 }
	...
	if($ftp_mode===FALSE) 
		...
		if($ftp_mode===FALSE) 
			...
			$message_ftp['mode']='An error intervened in the choice of the mode';
			exit();
		 } else { $message_ftp['mode']='not passive mode'; }
	 } else { $message_ftp['mode']='passive mode'; }
...
//		    	echo "Le dossier courant est maintenant : " . ftp_pwd($conn_id) . "\n<br />";
				  ...
				  if (@ftp_mkdir($conn_id, $dossier_du_fichier[$i_d])) {
					...
					@ftp_chdir($conn_id, $dossier_du_fichier[$i_d]);
						...
						$message_ftp['dossier'] = 'impossible to create the file';
					  }
				}
			$i_d++;
		}
