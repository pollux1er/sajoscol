...
if(url_exists($site_de_miseajour."version.msj")) {
	...
	if (!$fp = fopen($site_de_miseajour."version.msj","r")) {
		...
		$_SESSION['message_erreur'] = "Impossible to open the file of information of update";
...
// version stable
$nouvelle_stable = 'non';
$texte_stable='no the currently available stable version';
if($version_stable_serveur[1]>$version_stable_client[1]) {
	$texte_stable = 'a new stable version is available';
	$nouvelle_stable = 'oui';
} elseif($version_stable_serveur[1]===$version_stable_client[1]) {
	$texte_stable = 'your version is up to date';
} elseif($version_stable_serveur[1]<$version_stable_client[1]) {
	$texte_stable = 'you have a version superior with that available';
}
...
{
	$nouvelle_rc = 'non';
	$texte_rc='no currently available version RC';
	...
	if($rc_version[0]>$version_stable_client[1] or $rc_version[0]===$version_stable_client[1])
	{
		if($rc_version[1]>$version_rc_client[1] and $rc_version[1]!='0') {
			$texte_rc = 'a new version RC is available';
			$nouvelle_rc = 'oui';
		} elseif($rc_version[1]===$version_rc_client[1]) {
			$texte_rc = 'your version RC is up to date';
		} elseif($rc_version[1]<$version_rc_client[1]) {
			$texte_rc = 'you have a version superior with that available';
		}
	} else { $texte_rc = 'no version RC available actuellement'; }
}
...
{
$nouvelle_beta = 'non';
$texte_beta='no the currently available version BETA';
...
if($beta_version[0]>$version_stable_client[1] or $beta_version[0]===$version_stable_client[1])
 {
	if($beta_version[1]>$version_beta_client[1] and $beta_version[1]!='0') {
		$texte_beta = 'a new version BETA is available';
		$nouvelle_beta = 'oui';
	} elseif($beta_version[1]===$version_beta_client[1]) {
		$texte_beta = 'your version up to date BETA';
	} elseif($beta_version[1]<$version_beta_client[1]) {
		$texte_beta = 'you have a version superior with that available';
	}
 } else { $texte_beta = 'no version currently available BETA'; }
...
if($maj_logiciel==='oui' and $valide_form==='yes')
{
	...
	if(!url_exists($file))
	   ...
	   $_SESSION['message_erreur'] = "The file $file is non-existent";
           header("Location: fenetre.php?maj_type=".$maj_type);
	 } else {
		  ...
		  if (!copy($file, $newfile))
		    ...
		    $_SESSION['message_erreur'] = "The copy of the file $file did not succeed...";
	            header("Location: fenetre.php?maj_type=".$maj_type);
		  } else {
			    umask($old);
			    ...
			    if($md5_de_verif!=$md5_du_fichier_telecharge)
			    {
			      ...
			      $_SESSION['message_erreur'] = "the downloaded file is corrompu, updated impossible";
			      header("Location: fenetre.php?maj_type=".$maj_type);
			     } else {
					...
					$_SESSION['message_erreur'] = "it misses the library of decompression of the files";
			      	        header("Location: fenetre.php?maj_type=".$maj_type);
				      } else {
...
//echo '<pre>';
						   ...
						   $_SESSION['message_traitement'] = 'finished update ! Think you of disconnecting and you reconnecter as an administrator
after this update';
...
<center>
<h2>System of update of the software</h2>
<div style="border-style:solid; border-width:1px; border-color: #6F6968; background-color: #5A7ACF;  padding: 2px; margin-left: 2px; margin-right: 2px; margin-top: 2px; margin-bottom: 0px;  text-align: left;">
   <div style="border-style:solid; border-width:0px; border-color: #6F6968; background-color: #5A7ACF;  padding: 2px; margin: 2px; font-family: Helvetica,Arial,sans-serif; font-weight: bold; color: #FFFFFF;">STABLE VERSION</div>
   <div style="border-style:solid; border-width:1px; border-color: #6F6968; background-color: #FFFFFF;  padding: 6px; margin: 2px;">
	...
	<div style="font-family: Helvetica,Arial,sans-serif; color: rgb(0, 0, 0); margin-left: 20px; text-align: justify;">description: <a href="<?php echo $changelog[1]; ?>" target="_blank">to see the changelog</a></div>
...
<div style="border-style:solid; border-width:0px; border-color: #6F6968; background-color: #5A7ACF;  padding: 2px; margin-left: 20px; margin-right: 2px; margin-top: 0px; margin-bottom: 2px; text-align: left;">
   <div style="border-style:solid; border-width:0px; border-color: #6F6968; background-color: #5A7ACF;  padding: 0px; margin: 0px; font-family: Helvetica,Arial,sans-serif; font-weight: bold; color: #FFFFFF;">File being able to be updated for this version.</div>
   <?php
...
while(!empty($donne_fichier['nom_fichier'][$nb_a])) { ?>
	   ...
	   if(!empty($info_fichier_base[$identitification]['date']))
	   {
	       if($info_fichier_base[$identitification]['date']===$donne_fichier['date_fichier'][$nb_a])
	        {
	  	    if($info_fichier_base[$identitification]['heure']===$donne_fichier['heure_fichier'][$nb_a]) { $amettreajour='non'; $info_etat='version of the up to date file'; }
  		    if($info_fichier_base[$identitification]['heure'] < $donne_fichier['heure_fichier'][$nb_a]) { $amettreajour='oui'; $info_etat='an update of the file is available'; }
		    if($info_fichier_base[$identitification]['heure'] > $donne_fichier['heure_fichier'][$nb_a]) { $amettreajour='non'; $info_etat='version of the up to date file'; }
 	        }
	       if($info_fichier_base[$identitification]['date'] < $donne_fichier['date_fichier'][$nb_a]) { $amettreajour='oui'; $info_etat='an update of the file is available'; }
	       if($info_fichier_base[$identitification]['date'] > $donne_fichier['date_fichier'][$nb_a]) { $amettreajour='non'; $info_etat='version of the up to date file'; }
	   }
	   if(empty($info_fichier_base[$identitification]['date']))
	   {
	     $amettreajour='oui';
		 $info_etat='an update of the file is available';
	   }
	?>
	...
	<span style=" font-family: Helvetica,Arial,sans-serif; color: #000000;">File <strong><?php echo $donne_fichier['nom_fichier'][$nb_a]; ?></strong> update of: <strong><?php echo $donne_fichier['date_fichier'][$nb_a].' '.$donne_fichier['heure_fichier'][$nb_a]; ?></strong></span><br />
	<span style="font-family: Helvetica,Arial,sans-serif; color: rgb(255, 0, 0); margin-left: 20px;"><?php echo $info_etat; ?></span><br />
	<div style="font-family: Helvetica,Arial,sans-serif; color: rgb(0, 0, 0); margin-left: 20px; text-align: justify;">description: <?php echo $donne_fichier['descriptif_fichier'][$nb_a]; ?></div>
   </div>
...
<div style="border-style:solid; border-width:1px; border-color: #6F6968; background-color: rgb(255, 0, 0);  padding: 2px; margin-left: 2px; margin-right: 2px; margin-top: 2px; margin-bottom: 0px;  text-align: left;">
   ...
   <div style="border-style:solid; border-width:1px; border-color: #6F6968; background-color: #FFFFFF;  padding: 6px; margin: 2px;">
	...
	<span style=" font-family: Helvetica,Arial,sans-serif; font-weight: bold; color: #000000;">Version RC of GEPI has to install: <?php if($version_rc_client[1]!='') { echo 'RC '.$version_rc_client[1]; } else { echo 'aucune RC installé'; } ?></span><br />
	<span style="font-family: Helvetica,Arial,sans-serif; color: rgb(255, 0, 0); margin-left: 20px;"><?php echo $texte_rc; ?></span><br />
	<div style="font-family: Helvetica,Arial,sans-serif; color: rgb(0, 0, 0); margin-left: 20px; text-align: justify;">description: <a href="<?php echo $changelog[1]; ?>" target="_blank">See the changelog</a></div>
...
<br />
	...
	<span style=" font-family: Helvetica,Arial,sans-serif; font-weight: bold; color: #000000;">Version BETA of GEPI has to install: <?php if($version_beta_client[1]!='') { echo 'BETA '.$version_beta_client[1]; } else { echo 'none'; } ?></span><br />
...
<?php } ?>
<br /><br /><a href="javascript:window.close();">Close the window</a>
</center>
</body>
</html>
<?php }
	...
	echo '
			...
			<br /><br /><h3 class="center">Impossible update</h3>
			<p class="center">the waiter of update is not available</p>
		';
}
mysql_close();
