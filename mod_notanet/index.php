...
if(!$query) {
	$msg.="error during modification of flied type 'id_mat' of table 'notanet'.<br />
	
	this risk to put a problem if you must impound note of regional alive language .<br />";
}

$query = mysql_query("ALTER TABLE notanet_corresp CHANGE id_mat id_mat INT( 4 ) NOT NULL;");
if(!$query) {
	$msg.="Error during modification of flied type 'id_mat' of table 'notanet_corresp'.<br />this risk to put a problem if you must impound note of regional alive language.<br />";
}

$query = mysql_query("ALTER TABLE notanet_app CHANGE id_mat id_mat INT( 4 ) NOT NULL;");
if(!$query) {
	$msg.="Error during modification of flied type 'id_mat' of table 'notanet_app'.<br />this risk to put a problem if you must impound note of regional alive language.<br />";
}
//===========================================================
...
echo "<div class='noprint'>\n";
echo "<p class='bold'><a href='../accueil.php'>Welcome</a>";
echo "</p>\n";
echo "</div>\n";
...
if($_SESSION['statut']=="administrateur") {
	...
	if($truncate_tables=='y') {
		check_token();
		echo "<p>cleaning tables of Notanet</p>\n";
...
if($_SESSION['statut']=="administrateur") {
	echo "<ol>\n";
	echo "<li><a href='saisie_param.php'>Impound academique parameter , Session,...</a>.</li>\n";
	echo "<li><a href='select_eleves.php'>make student association / certificate type </a></li>\n";

	echo "<li><a href='select_matieres.php'>make type of certificate association/subject</a>  (<i>to specify status: impose and option</i>)</li>\n";
	...
	echo "<li><a href='saisie_lvr.php'>Impound 'notes' of regional alive language </a> (<i> if someone instruction is evaluate at institution</i>)</li>\n";

	echo "<li><a href='extract_moy.php'>make an extraction of average, affichage et traitement des cas particuliers</a></li>\n";
