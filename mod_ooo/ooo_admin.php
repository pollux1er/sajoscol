...
if ((isset($_POST['is_posted']))&&(isset($_POST['activer']))) {
	check_token();
    if (!saveSetting("active_mod_ooo", $_POST['activer'])) $msg.= "Error during the recording of the parameter activation/desactivation !";

	if (isset($_POST['fb_dezip_ooo'])) {
		if (!saveSetting("fb_dezip_ooo", $_POST['fb_dezip_ooo'])) {
			$msg .= "Error during the recording of fb_dezip_ooo !";
		}
	}
}
...
if (!($resultat_mkdir)) {
  $droitRepertoire[]="CAUTION: Rights of writing on the file
  /mod_ooo/$nom_dossier_modeles_ooo_mes_modeles are incorrect. Gepi must have the rights of creation files and files in this file to ensure the correct operation of the
module";
}
else {
	@rmdir($dossier_test);
...
$resultat_mkdir = @mkdir($dossier_test);
if (!($resultat_mkdir)) {
  $droitRepertoire[]="CAUTION : Rights of writing on the file /mod_ooo/tmp/ are incorrect.
	Gepi must have the rights of creation of files and files in this file
to ensure the correct operation of the module";
}
else {
	@rmdir($dossier_test);
}
