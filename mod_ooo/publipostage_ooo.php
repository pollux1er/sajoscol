<?php
/*
 * $Id: publipostage_ooo.php 8039 2011-08-29 18:04:24Z crob $
 *
...
elseif(isset($_GET['suppr_fich'])) {
	...
	if(!preg_match('/^[0-9]$/',$_GET['suppr_fich'])) {
		$msg="Invalid number of file : ".$_GET['suppr_fich']."<br />\n";
	}
	else {
		$tab_file=get_tab_file($path);

		if(!file_exists($path."/".$tab_file[$_GET['suppr_fich']])) {
			$msg="The file ".$_GET['suppr_fich']." do not exist.<br />\n";
		}
		else {
			$menage=unlink($path."/".$tab_file[$_GET['suppr_fich']]);
			if($menage) {$msg="Fichier ".$tab_file[$_GET['suppr_fich']]." supprimé.<br />";}
			else {$msg="Error during the removal of the file ".$tab_file[$_GET['suppr_fich']]."<br />";}
		}
	}
}

//**************** EN-TETE *****************
$titre_page = "Open Model Office - Publipostage";
...
if(!isset($num_fich)) {
	...
	if(isset($_FILES['monfichier'])) {
		...
		if($upload_modele_ooo_autorise!='y') {
			echo "<p style='color:red'>Unauthorized action&nbsp;: Upload of a personalized model.</p>\n";
			tentative_intrusion(1, "Unauthorized attempt at upload of a model OOo (".$monfichiername.")");
		}
		else {

			if ($monfichiername=="") {
				echo "<p style='color:red'>No the file indicated ! It is necessary to start again...</p>\n";
			}
			else {
				...
				if($_SESSION['statut']=='administrateur') {
					...
					for($i=0;$i<count($login_user);$i++) {
						...
						if(!file_exists($path_user)) {
							...
							if(!$creation) {
								echo "<p style='color:red;'>ERROR during the creation of the file of model OpenOffice.org for ".$login_user[$i]."</p>\n";
								$temoin_erreur="y";
							}
						}
						...
						if($temoin_erreur=="n") {
							if(!file_exists($path_user."/index.html")) {
								if(!creation_index_redir_login($path_user,1)) {
									echo "<p style='color:red;'>ERROR during the creation of an index in your file of model OpenOffice.org for ".$login_user[$i]."</p>\n";
								}
							}
				...
				*/
				...
				for($i=0;$i<count($cible);$i++) {
					...
					if(!$res_copy) {echo "<p style='color:red'>Failure of the installation of the file ".$cible[$i]."</p>";}
					else {
						$nb_copies++;
					}
				}
				if($nb_copies>0) {
					echo "<p style='color:red'>File set up for $nb_copies user.</p>\n";
				}
			}
		}
	}
	...
	if(file_exists($path)) {
		...
		if(count($tab_file)==0) {
			$upload_modele_ooo_autorise="n";
			echo "<p style='color:red;'>You do not have any model.";
			...
			if($upload_modele_ooo_autorise!="y") {
				echo "<br />\n";
				echo "And you do not have the authorization of uploader your models.";
			}
			...
			echo "<p>Use the model&nbsp;:<br />";
			for($i=0;$i<count($tab_file);$i++) {
				echo "<a href='".$_SERVER['PHP_SELF']."?num_fich=$i'>".$tab_file[$i]."</a> - <a href='".$_SERVER['PHP_SELF']."?suppr_fich=$i".add_token_in_url()."'><img src='../images/delete16.png' width='16' height='16' title=\"Remove the file ".$tab_file[$i]."\" /></a><br />";
			}
			echo "</p>\n";
		}
	}
		...
		$creation=mkdir($path);
		if(!$creation) {
			echo "<p style='color:red;'>ERROR during the creation of your file of model OpenOffice.org</p>\n";
	...
	if(!file_exists($path."/index.html")) {
		if(!creation_index_redir_login($path,1)) {
			echo "<p style='color:red;'>ERROR during the creation of an index in your file of model OpenOffice.org</p>\n";
		}
	}
	...
	if($upload_modele_ooo_autorise=='y') {
		...
		echo "<p>To set up a new model&nbsp;:</p>\n";
		echo "<INPUT TYPE=\"hidden\" name=\"MAX_FILE_SIZE\" value=\"512000\">";
	
		if($_SESSION['statut']=='administrateur') {
			echo "<p>For quel(s) user wish you to set up the model&nbsp;? ";
			echo "<a href='javascript:cocher_decocher(true)'>all tick</a> / <a href='javascript:cocher_decocher(false)'>All shoot</a>\n";
			echo "</p>\n";
			...
			if(mysql_num_rows($res)>0) {
				...
				echo "<table width='100%' summary=\"Table of choice of the users to which to distribute the model\">\n";
		...
		echo "<p>Model file&nbsp;:&nbsp;<input type='file' name='monfichier' value='it clicked the guy'><br />\n";
		echo "<input type='submit' name='btn' Align='middle' value='Send' /></p>\n";
		echo "</form>\n";
...
function cocher_decocher(mode) {
	for (var k=0;k<$cpt;k++) {
		...
		echo "<ul>\n";
			echo "<li>\n";
				echo "<p style='margin-left:3em;'>The provided file can use the following fields&nbsp;:</p>\n";
			...
			echo "</li>\n";
			echo "<li>\n";
				echo "<p>Examples of models are available&nbsp;: <a href='http://www.sylogix.org/projects/gepi/wiki/Publipostage_ooo'>http://www.sylogix.org/projects/gepi/wiki/Publipostage_ooo</a></p>\n";
			echo "</li>\n";
		echo "</ul>\n";

		if($_SESSION['statut']=='administrateur') {
			echo "<p style='color:red'>TO MAKE : To allow the administrator to do the housework in the model files of the other users.<br />To allow to limit the fields to which access the users have according to their statute.</p>\n";
		}
	}
}
else {
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Choose another model</a>";
	if((!isset($id_classe))&&(!isset($id_groupe))) {
		...
		if(mysql_num_rows($res)>0) {
			echo "<form method='post' ENCTYPE='multipart/form-data' action='".$_SERVER['PHP_SELF']."'>\n";
			echo "<p>For which classes wish you to print the document <b>".$tab_file[$num_fich]."</b>&nbsp;?";
			echo " <a href=\"javascript:cocher_decocher('id_classe_', true)\">tick</a> / <a href=\"javascript:cocher_decocher('id_classe_', false)\">shoot</a> all classes\n";
			echo "</p>\n";
			echo add_token_field();
			...
			while($lig=mysql_fetch_object($res)) {
				...
				echo "/><label for='id_classe_$cpt'><span id='texte_id_classe_$cpt'>$lig->class</span></label><br />\n";
		...
		if($_SESSION['statut']=='professeur') {
			...
			if(count($groups)>0) {
				echo "<form method='post' ENCTYPE='multipart/form-data' action='".$_SERVER['PHP_SELF']."'>\n";
				echo "<p>For which teaching wish you to print the document ".$tab_file[$num_fich]."&nbsp;?";
				echo " <a href=\"javascript:cocher_decocher('id_groupe_', true)\">tick</a> / <a href=\"javascript:cocher_decocher('id_groupe_', false)\">décocher</a> all lesson\n";
				echo "</p>\n";
				echo add_token_field();
				echo "<input type='hidden' name='num_fich' value='$num_fich' />\n";
...
function cocher_decocher(prefixe_id, mode) {
	for (var k=0;k<$cpt;k++) {
		...
		echo " | <a href='".$_SERVER['PHP_SELF']."?num_fich=$num_fich'>Choose another class or teaching</a>";
		echo "</p>\n";
