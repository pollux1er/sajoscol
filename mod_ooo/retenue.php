...
if (($mode=='module_discipline')||($mode=='module_retenue')) {
	...
	if(mysql_num_rows($res_incident)>0) {
		...
		$travail ='Given on the spot'; // texte par défaut, c'est un enseignant qui rédige l'incident, il n'y a pas de possibilité de saisir le travail.
		...
		if(mysql_num_rows($res)>0) {
			...
			echo "ERROR: Login $lig_incident->declarant";
		}
	...
	if ($mode=='module_retenue') {	
	    ...
	    if(mysql_num_rows($res_sanction)>0) {
			...
			return "RESERVE EAST UNKNOWN FACTOR";
		}     
	} // mode = module retenue	
