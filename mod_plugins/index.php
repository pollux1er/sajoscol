      ...
      if ($testXML->getReponse() === true){
        ...
        if ($traitement_requetes->getReponse() === true){
		  ...
		  if (function_exists($fonction_post))
			...
			if ($retour!="") 
				{
				$_msg="Error post_installation : ".$retour;
				$_erreur=10;
				}
			}
 
        }else{
          $_msg = '<p class="red">ERROR(r) : ' . $traitement_requetes->getErreur() . '</p>';
        }
      }else{

        $_msg = '<p class="red">ERROR(x) : ' . $testXML->getErreur() . '</p>';
...
# On traite des plugin déjà installés
	...
	if (function_exists($fonction_ante))
	  ...
	  if ($traitement_requetes->getReponse() === true){
		  ...
		  if (function_exists($fonction_post))
			...
			if ($retour!="") 
				{
				$_msg="Error post_desinstallation : ".$retour;
				$_erreur=10;
				}
			}
		  break;
  ...
  default:
    $_msg = "<p>The action required does not exist !</p>";
    break;
  }
}
...
switch ($_erreur) {
  case "1":
    $_msg = "<p class=\"red\">It misses the file plugin.xml with this plugin, impossible to install it !</p>";
    break;
  case "2":
    $_msg = "<p class=\"red\">Le fichier plugin.xml do not respect struture requested ! see <a href=\"https://www.sylogix.org/wiki/gepi/plugin\">collaborative documentation (wiki)</a></p>";
    break;
  case "10":
...
// ================= HEADER ========================//
$titre_page = "Parameterize the plugins";
...
<h3 class="Gepi">List plugins installed</h3>
<p>For more information concerning the plugins of Gepi, see
  <a onclick="window.open(this.href, '_blank'); return false;" href="https://www.sylogix.org/projects/gepi/wiki/GuideAdministrateur#Syst%C3%A8me-de-plugins">collaborative documentation (wiki)</a>
</p>
 <table class="table">
  <tr>
    <th>Plugin</th>
...
foreach($liste_plugins as $plugin){
  if (!is_object($plugin)){
    ...
    if ($plugin->getOuvert() == 'y'){
      $aff_ouvert = '<a href="index.php?plugin_id='.$plugin->getId().'&amp;action=fermer'.add_token_in_url().'" title=" Do you want to close it ?">YES</a>';
    }else{
      $aff_ouvert = '<a href="index.php?plugin_id='.$plugin->getId().'&amp;action=ouvrir'.add_token_in_url().'" title=" Do you want to open it ?">NON</a>';
    }
    echo '
    <tr>
      ...
      <td><a href="index.php?plugin_id='.$plugin->getId().'&amp;action=desinstaller'.add_token_in_url().'" title=" Do you want to desinstalle it ?" onclick="return confirm('."'The desinstallation of a plugin the suppression of the possibly associated tables and the
data involves which they contain. Are you sure  to want desinstaller this plugin ?'".');">yes</a></td>
      <td>'.$aff_ouvert.'</td>
    </tr>';
  }
}
