...
class gepiPlugIn {
  ...
  public function  __construct($plugin) {
    ...
    if ($this->_plugin->getOuvert() != "y"){
      tentative_intrusion("2", "Attempt at reading of a file of the plugin ".$infos_plugin->getNom()." who is not opened with the public.");
      $this->_logout();
    }
  }
  ...
  private function affErreur($number){
    if(is_numeric($number)){
      switch ($number) {
        case "1":
          Die('It is necessary to specify the name of the plugin in the creation of
its authority : $user_auth = new gepiPlugIn("nom_plugin");');
          break;
        case "2":
          Die('The plugin object is not instancie; or does not exist !');
          break;
        case "3":
          tentative_intrusion(2, "Attempt at opening of a page of the plugin ".$this->_plugin->getNom()." without having the sufficient rights !");
          Die('You do not have the sufficient rights to post this page !');
          break;

      default:
          Die("Not indexed error !");
        break;
  }
    }else{
      Die(" The information passed to the method " . __METHOD__ . " is not valid !");
    }
  }
