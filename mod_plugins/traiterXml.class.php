...
class traiterXml {
  ...
  private function retourneErreur($_e, $_m){
    switch ($_e) {
      case 1:
        $message = 'It misses the node ' . $_m . ' in the file plugin.xml';
        break;
      case 2:
        $message = 'The file ' . $_m . ' is declared in plugin.xml but is missing in the plugin.';
        break;
      case 3:
        $message = 'The file ' . $_m . ' is present in the plugin but n\' does not have its rights in plugin.xml.';
        break;

      default:
        $message = "no the error message";
      break;
    }
    $this->erreur = $message;
  }
