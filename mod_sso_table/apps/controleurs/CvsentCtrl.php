    ...
    function result() {
        try {
            ...
            if (file_exists($this->csv)) {
                ...
                unlink($this->csv);
                if (file_exists($this->csv)){
                    throw new Exception('Impossible to remove the file csv in your repertoire temp. It is
advised to do it manually.');
                }
    ...
    private function traite_file($file) {
        ...
        if (is_null($this->erreurs_lignes)) {
            ...
            if (mysql_num_rows($this->res) == 1) {
                ...
                echo "il y a " . mysql_num_rows($this->res) . " denominations for the statute raises ";
                die ();
            }
            ...
            if (mysql_num_rows($this->res) == 1) {
                ...
                echo "il y a " . mysql_num_rows($this->res) . " denominations for the responsible statute ";
                die ();
            }
            ...
            if (mysql_num_rows($this->res) > 0) {
                $class = "message_red";
                $message = "This account poses problem";
                while ($this->row2 = mysql_fetch_array($this->res)) {
            ...
            if (mysql_num_rows($this->res) != 0) {
                while ($this->ligne = mysql_fetch_array($this->res)) {
                    ...
                    if (mysql_num_rows($this->res2) > 0) {
                        $this->message = 'There are several people in Gépi having the same names and first
names';
            ...
            if (mysql_num_rows($this->res) != 0) {
                while ($this->ligne = mysql_fetch_array($this->res)) {
                    ...
                    if (mysql_num_rows($this->res2) == 0) {
                        ...
                        if ($probable) {
                            $this->message = "The user east can be " . $probable['nom'] . " " . $probable['prenom'] . "( " . $probable['statut'] . ")";
                            $this->class = "message_purple";
                            $nomPrenom = $this->ligne['nom'] . " " . $this->ligne['prenom'];
                            $this->table[] = array('login_gepi' => $nomPrenom, 'login_sso' => $this->ligne['uid'], 'couleur' => $this->class, 'message' => $this->message);
                        } else {
                            $this->message = "The user probably does not exist in gépi.";
                            $this->class = "message_red";
                            $nomPrenom = $this->ligne['nom'] . " " . $this->ligne['prenom'];
                            $this->table[] = array('login_gepi' => $nomPrenom, 'login_sso' => $this->ligne['uid'], 'couleur' => $this->class, 'message' => $this->message);
                        }
        ...
        switch ($code) {
            case 0:
                ...
                if ($row[0] == $this->ligne['uid']) {
                    ...
                    $this->message = 'An identical entry already exists in the table for this login gépi';
                } else {
                    $this->class = "message_red";
                    $this->message = 'A different entry already exists in the table for this login gépi';
                }
                break;
            case 1:
                $this->class = "message_red";
            ...
            case 3:
                ...
                $this->message = 'The user exists but its account is not parameterized for the sso. It is necessary to correct absolutely so that the correspondence
functions.';
                break;
            case 5:
                $this->class = "message_red";
