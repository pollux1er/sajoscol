...
class MajCtrl extends Controleur {
  ...
  function search () {
    try {
      ...
      if(!$this->search_result)throw new exception("No user corresponding to your search criteria exists in Gépi with his account parameterized in sso");
  ...
  function updated() {
    try {
      ...
      if($this->login_sso=="")throw new Exception ('You must enter a value for the correspondence');
      $data=new ImportModele();
      if($data->verif_exist_login_sso($this->login_sso)) throw new Exception ('An entry exists déja with this login sso; impossible update');
