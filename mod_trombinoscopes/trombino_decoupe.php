...
secours='F',
autre='F',
description='Generation of a grid pdf for the trombinoscopes,...',
statut='';";
...
if(isset($parametrer_pdf)) {
	...
	if(($trombino_pdf_nb_lig>0)&&($trombino_pdf_nb_col>0)) {
		if (!saveSetting("trombino_pdf_nb_col", $trombino_pdf_nb_col)) {
			$msg .= "error during registration of trombino_pdf_nb_col !";
		}

		if (!saveSetting("trombino_pdf_nb_lig", $trombino_pdf_nb_lig)) {
			$msg .= "error during registration of trombino_pdf_nb_lig !";
		}

		if($msg=="") {$msg="registration of  made parameter .";}
	}
}
//=================================================
if(isset($_POST['suppr_grille'])) {
	...
	for($i=0;$i<count($suppr_grille);$i++) {
		...
		$del=mysql_query($sql);
		if(!$del) {$msg.="error during delection number $suppr_grille[$i]<br />";}
		else {
			$sql="DELETE FROM trombino_decoupe_param WHERE id_grille='$suppr_grille[$i]';";
			$del=mysql_query($sql);
			if(!$del) {$msg.="error during delection of parameter of railings number $suppr_grille[$i]<br />";}
		}
	}
	if($msg=="") {$msg="delection made.<br />";}
}
//=================================================
...
if(isset($_POST['upload_scan'])) {
	...
	if(!isset($_POST['fin_form_upload_scan'])) {
		$msg="form cannot  poster entirely.<br />you have perhaps too greedy with number and volume of picture propose.<br />";
		// Ca ne fonctionne pas... si on est trop gourmand, on se retrouve avec $_POST vide.
	}

	$tempdir=get_user_temp_directory();
	if(!$tempdir){
		$msg="it seem that the temporary file of user  ".$_SESSION['login']." is not define!?<br />\n";
		// Il ne faut pas aller plus loin...
	}
	elseif(!isset($id_grille)) {
		$msg="no id_grille has choisen.<br />\n";
	}
	else {
		//echo "2";
		...
		if(mysql_num_rows($res)==0) {
			$msg="id_grille n° $id_grille cannot correspond at writing in table'trombino_decoupe'.<br />\n";
		}
		else {
			//echo "3";
			...
			if(mysql_num_rows($res_param)==0) {
				$msg="no parameter is find for id_grille n° $id_grille in table 'trombino_decoupe_param'.<br />\n";
			}
			else {
				...
				*/
				...
				$msg.="processing of cut-out with a railings of $trombino_pdf_nb_col colums on $trombino_pdf_nb_lig lignes (id_grille n°$id_grille).<br />";
				...
				for($i=0;$i<=$lig->page_global;$i++) {
					if($_FILES["image_".$i]['type']!='') {
						...
						if(!is_uploaded_file($image['tmp_name'])) {
							$msg.="downloading of file n° $i is fail.<br />\n";
							$msg.="the variable of php.ini can be explaint the problem :<br />\n";
							$msg.="post_max_size=$post_max_size<br />\n";
							$msg.="upload_max_filesize=$upload_max_filesize<br />\n";
						}
						else{
							if(!file_exists($image['tmp_name'])){
								$msg.="file n°$i will be downloading ... but doesnt be present/preserved .<br />\n";
								$msg.="the variable of php.ini can be explain the problem :<br />\n";
								$msg.="post_max_size=$post_max_size<br />\n";
								$msg.="upload_max_filesize=$upload_max_filesize<br />\n";
								$msg.="and volume of ".$image['name']." was<br />\n";
								$msg.="\$image['size']=".volume_human($image['size'])."<br />\n";
							}
		
							//echo "<p>Le fichier ".$image['name']." sous ".$image['tmp_name']." a été uploadé.</p>\n";
							if($image['name']=="") {$msg.="there is a problem during downloading/processing.<br />file processing downloading was type JPEG? (type find &nbsp;: ".$image['type'].")<br />";}

							$source_file=$image['tmp_name'];
							$dest_file="../temp/".$tempdir."/image_$i.jpg";
							$res_copy=copy("$source_file" , "$dest_file");
							if(!$res_copy) {
								$msg.="Error during transfer of ".$image['name']." verse temporary file of user.<br />\n";
							}
							else {
								$num_page=$i+1;
								//$msg.="Traitement de la page n°$i<br />\n";
								$msg.="processing of page n°$num_page<br />\n";
...
if(isset($generer_pdf)) {
	check_token();
	if(!isset($id_classe)) {
		$msg="ERREUR&nbsp;: no class was  selected.<br />\n";
		unset($mode);
	}
	else {
		...
		define('WidthPage',$largeur_page);
		define('HeightPage',$hauteur_page);
...
if(!isset($mode)) {
	//**************** EN-TETE *****************
	$titre_page = "railings PDF for the trombinoscopes";
	require_once("../lib/header.inc");
	//**************** FIN EN-TETE *****************
	//debug_var();

	echo "<p class=bold><a href='trombinoscopes.php'> <img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
	echo "</p>\n";

	echo "<p>Choice &nbsp;:</p>\n";
	echo "<ul>\n";
	echo "<li><a href='".$_SERVER['PHP_SELF']."?mode=parametrer'>parametrize railings</a></li>\n";
	echo "<li><a href='".$_SERVER['PHP_SELF']."?mode=generer_grille'>Generate railings</a></li>\n";
	$sql="SELECT 1=1 FROM trombino_decoupe;";
	$test=mysql_query($sql);
	if(mysql_num_rows($test)>0) {
		echo "<li><a href='".$_SERVER['PHP_SELF']."?mode=uploader'>downloading scanner railings  after sticking picture</a></li>\n";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?mode=suppr_grille'>delected railings</a></li>\n";
	}
	echo "</ul>\n";

	echo "<p><i>NOTES</i>&nbsp;:</p>\n";
	echo "<ul>\n";
	echo "<li>directions for use this page is following &nbsp;:<br />administrator choise the number of column and  line.<br />it generate and print the railings PDF.<br />picture are  paste with  tidiness on railings with a pipe of glue, all we are classic.<br />
	
	railings are scanner in watch that the border of each picture scanner has coincident with border of page.<br />It uploade then scannées images.<br />The device is given the responsability to cut out the grids to place
the famous individual photographs in ../photos/eleves/ELENOET.jpg</li>\n";
	...
	echo "<li><span style='color:red;'>TO RE-EXAMINE&nbsp;:</span> Other parameters should be proposed and recorded in the base.... and
been dependent on an edition of particular grid.</li>\n";
	//echo "<li></li>\n";
	echo "</ul>\n";
...
elseif($mode=='parametrer') {
	//**************** EN-TETE *****************
	$titre_page = "railings PDF for trombinoscopes";
	require_once("../lib/header.inc");
	//**************** FIN EN-TETE *****************
	//debug_var();

	echo "<p class=bold><a href='trombinoscopes.php'> <img src='../images/icons/back.png' alt='Return' class='back_link'/> Welcome trombinoscopes</a>";
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Welcome cut-out</a>\n";
	echo "</p>\n";
	...
	echo "<fieldset>\n";
	echo "<p>parametrizing &nbsp;:</p>\n";
	echo "<table style='margin-left:2em;' class='boireaus' summary='Parameter of trombinoscope'>\n";
	echo "<tr class='lig1'>\n";
	echo "<td>\n";
...
elseif($mode=='generer_grille') {
	...
	echo "<p class=bold><a href='trombinoscopes.php'> <img src='../images/icons/back.png' alt='Retour' class='back_link'/> Welcome trombinoscopes</a>";
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Welcome cut-out </a>\n";
	echo "</p>\n";
	...
	if($nb_classes==0) {
		echo "<p style='color:red'>ERREUR&nbsp;: there exist no class.</p>\n";
	...
	echo add_token_field();
	echo "<fieldset>\n";
	echo "<p>Generate railings PDF for &nbsp;:</p>\n";

	// Affichage sur 3 colonnes
	$nb_classes_par_colonne=round($nb_classes/3);

	echo "<table width='100%' summary='Choise of class'>\n";
	...
	echo "<p><a href='#' onClick='ModifCase(true)'>all tick</a> / <a href='#' onClick='ModifCase(false)'>All shoot</a></p>\n";
...
elseif($mode=='uploader') {
	...
	echo "<p>the picture downloading must be at type JPEG.</p>\n";

	echo "<p> this parameter can be influence number of picture that you are downloading suddenly &nbsp;:<br />\n";
	$post_max_size=ini_get('post_max_size');
	$upload_max_filesize=ini_get('upload_max_filesize');
	...
	if(!isset($id_grille)) {
		...
		$test=mysql_query($sql);
		if(mysql_num_rows($test)==0) {
			echo "<p style='color:red'>ERROR&nbsp;: not yet railings was generate.</p>\n";
			require("../lib/footer.inc.php");
			die();
		}
		elseif(mysql_num_rows($test)==1) {
			...
			echo "<p> for which railings wish you downloading picture?</p>\n";
			echo "<ul>\n";
			while($lig=mysql_fetch_object($test)) {
				echo "<li>\n";
				echo "<a href='".$_SERVER['PHP_SELF']."?mode=uploader&amp;id_grille=$lig->id_grille".add_token_in_url()."'>Railings n°$lig->id_grille</a>\n";
	...
	echo "<p class='bold'>Railings n°$id_grille</p>\n";

	$sql="SELECT DISTINCT classe, page, page_global FROM trombino_decoupe WHERE id_grille='$id_grille' ORDER BY page_global, page;";
	$test=mysql_query($sql);
	if(mysql_num_rows($test)==0) {
		echo "<p style='color:red'>ERREUR&nbsp;: no class is associate at railings n°$id_grille.</p>\n";
	}
	else {
		...
		echo "<table style='margin-left:2em;' class='boireaus' summary='downloading page of trombinoscope'>\n";
		echo "<tr>\n";
		echo "<th>Page of railings</th>\n";
		echo "<th>Class</th>\n";
		echo "<th>Page of railings</th>\n";
		echo "<th>File scanner</th>\n";
		echo "</tr>\n";
		$alt=1;
		while($lig=mysql_fetch_object($test)) {
			$alt=$alt*(-1);
		...
		//echo "<input type='hidden' name='id_grille' value='$id_grille' />\n";
		//echo "<input type='hidden' name='upload_scan' value='yes' />\n";
		echo "<p> It happens that there is a vertical shift developing line after line
on cuttings.<br />By defect, one does not shift&nbsp;: 
<input type='text' id='correctif_vertical' name='correctif_vertical' value='1' size='3' onkeydown=\"clavier_3(this.id,event,0.1,1.5,0.01);\" /><br />If your cuttings are shifted a little downwards (<i>it misses the top of craniums</i>), try to correct with 0.97<br />No corrective measure is proposed for the width.<br />Take care that your scannées images have the edges cut with the width
of the page.</p>\n";
		...
		if(($max_file_uploads!="")&&(strlen(my_ereg_replace("[^0-9]","",$max_file_uploads))==strlen($max_file_uploads))&&($max_file_uploads>0)) {
			echo "<p><i>Note</i>&nbsp;: downloading of picture is limited at $max_file_uploads simultaneous file.</p>\n";
		}
	}
...
elseif($mode=='suppr_grille') {
	...
	$test=mysql_query($sql);
	if(mysql_num_rows($test)==0) {
		echo "<p style='color:red'>ERROR&nbsp;: no railings was generate.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	else {
		...
		echo add_token_field();
		echo "<fieldset>\n";
		echo "<p>with railings wish you delected ?</p>\n";
		while($lig=mysql_fetch_object($test)) {
			echo "<input type='checkbox' name='suppr_grille[]' id='suppr_$lig->id_grille' value='$lig->id_grille' /><label for='suppr_$lig->id_grille'> railings n°$lig->id_grille";
		...
		echo "<p><input type='submit' value='delected' /></p>\n";
		echo "</fieldset>\n";
		echo "</form>\n";
