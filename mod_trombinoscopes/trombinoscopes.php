...
function classe_de($id_classe_eleve)
		...
		include("../secure/connect.inc.php");
			...
			$execution_classe_eleve = mysql_query($requete_classe_eleve) or die('Error SQL !'.$requete_classe_eleve.'<br />'.mysql_error());
...
//**************** EN-TETE *********************
$titre_page = "Visualization of the trombinoscopes";
...
<?php
	...
	echo " | <a href='trombinoscopes.php'>Carry out another selection</a>";

	if($_SESSION['statut']=='administrateur') {
		echo " | <a href='trombino_decoupe.php'>Cut out trombinoscope</a>";
	}

	if(($_SESSION['statut']=='professeur')&&(file_exists("./plan_de_classe.php"))) {
		echo " | <a href='plan_de_classe.php'>Plan of class</a>";
	}
	...
	if(($id_classe!='')&&(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($_SESSION['statut']=='cpe')||($_SESSION['statut']=='professeur'))) {
		...
		if($chaine_options_classes!="") {
			...
			echo " | Class : <select name='classe' onchange=\"document.forms['form1'].submit();\">\n";
	...
	if ( ( $classe === 'toutes' or $groupe === 'toutes' or $equipepeda === 'toutes' or $discipline === 'toutes' ) or ( $classe === '' and $groupe === '' and $equipepeda === '' and $discipline === '' and $statusgepi === '' ) ) {
		...
		if((getSettingValue('active_module_trombinoscopes')=='y')&&($acces=="y")) {
			...
			//=================================================================
			// CLASSES
			echo "<span style='margin-left: 15px;'>By class</span><br />\n";
			...
			$resultat_classe_prof = mysql_query($requete_classe_prof) or die('Error SQL !'.$requete_classe_prof.'<br />'.mysql_error());

			echo "<option value='' ";
			if ( empty($classe) ) { echo "selected='selected'"; }
			echo " onclick=\"reactiver('equipepeda,groupe,discipline,statusgepi,affdiscipline');\">no selection</option>\n";

			if (( $classe != 'toutes' )&&($_SESSION['statut']!='eleve')) {
				echo "<option value='toutes'>See all the classes</option>\n";
			}
			...
			if ( $classe == 'toutes' and ($_SESSION['statut'] == 'professeur')||($_SESSION['statut'] == 'eleve')) {
				echo "<option value=''>See my classes</option>\n";
			}

			echo "<optgroup label='-- Classes --'>\n";
			...
			echo "<span style='margin-left: 15px;'>By group</span><br />\n";
			...
			echo "onclick=\"reactiver('classe,equipepeda,discipline,statusgepi,affdiscipline');\">no selection</option>\n";

			if (( $groupe != 'toutes' )&&($_SESSION['statut']!='eleve')) {
				echo "<option value='toutes'>See all the groups</option>\n";
			}
			...
			if ( $groupe == 'toutes' and ($_SESSION['statut'] == 'professeur')||($_SESSION['statut'] == 'eleve')) {
				echo "<option value=''>See my groups</option>\n";
			}
			...
			echo "<br />\n";
			echo "<span style='margin-left: 15px;'><input type='radio' order_by_alpha' name='order_by' value='alpha' checked /><label for='order_by_alpha'> Alphabetical sorting</span></label><br />\n";
			echo "<span style='margin-left: 15px;'><input type='radio' id='order_by_classe' name='order_by' value='classe' /><label for='order_by_classe'> Sorting by class</span></label><br />\n";
			echo "<br />";
		...
		if((getSettingValue('active_module_trombino_pers')=='y')&&($acces=="y")) {
			...
			echo "<div style='font: normal small-caps normal 14pt Verdana; border-collapse: separate; border-spacing: 0px; border: none; border-bottom: 1px solid lightgrey;'>Personnel</div>\n";

			//==========================================================
			// EQUIPES PEDAGOGIQUES
			echo "<span style='margin-left: 15px;'>By teaching staff</span><br />\n";
			...
			$resultat_equipe_pedagogique = mysql_query($requete_equipe_pedagogique) or die('Error SQL !'.$requete_equipe_pedagogique.'<br />'.mysql_error());

			echo "<option value=''";
			if ( empty($equipepeda) ) {
				echo " selected='selected'";
			}
			echo " onclick=\"reactiver('classe,groupe,discipline,statusgepi');\">no selection</option>\n";


			//if ( $equipepeda != 'toutes' ) {
			if (( $equipepeda != 'toutes' )&&($_SESSION['statut']!='eleve')) {
				echo "<option value='toutes'>See all the teams pedagogic</option>\n";
			}
			...
			if ( $equipepeda == 'toutes' and ($_SESSION['statut'] == 'professeur')||($_SESSION['statut'] == 'eleve')) {
				echo "<option value=''>See my equipes pedas</option>\n";
			}
			...
			if((($_SESSION['statut']!='eleve')||(($_SESSION['statut']=='eleve')&&($GepiAccesEleTrombiPersonnels=='yes')))&&
				...
				echo "<br />&nbsp;&nbsp;&nbsp;<input type='checkbox' name='affdiscipline' id='affdiscipline' value='oui' />&nbsp;<label for='affdiscipline' style='cursor: pointer; cursor: hand;'>Diplay the disciplines</label>\n";
				echo "<br /><br />\n";

				echo "<span style='margin-left: 15px;'>By discipline</span><br />\n";
				...
				*/
				...
				$resultat_discipline = mysql_query($requete_discipline) or die('Error SQL !'.$requete_discipline.'<br />'.mysql_error());
				...
				echo " onclick=\"reactiver('classe,groupe,equipepeda,statusgepi,affdiscipline');\">no selection</option>\n";

				if ( $discipline != 'toutes' ) {
					echo "<option value='toutes'>See all the disciplines</option>\n";
				}
				if ( $discipline == 'toutes' and $_SESSION['statut'] == 'professeur' ) {
					echo "<option value=''>See my disciplines</option>\n";
				}

				echo "<optgroup label='-- Disciplines --'>\n";
			...
			$resultat_statusgepi = mysql_query($requete_statusgepi) or die('Error SQL !'.$requete_statusgepi.'<br />'.mysql_error());
			...
			echo " onclick=\"reactiver('classe,groupe,equipepeda,discipline,affdiscipline');\">no selection</option>\n";
...
if ( $etape === '2' and $classe != 'toutes' and $groupe != 'toutes' and $discipline != 'toutes' and $equipepeda != 'toutes' and ( $classe != '' or $groupe != '' or $equipepeda != '' or $discipline != '' or $statusgepi != '') ) {
	...
	if ( $action_affiche === 'classe' ) {
		if($_SESSION['statut']=='eleve') {
			...
			elseif($GepiAccesEleTrombiElevesClasse=='yes') {
				...
				echo "<p>You do not have access to the trombinoscopes classes.</p>\n";
	...
	if ( $action_affiche === 'groupe' ) {
		if($_SESSION['statut']=='eleve') {
			...
			elseif($GepiAccesEleTrombiElevesClasse=='yes') {
				...
				echo "<p>You do not have access to the trombinoscopes groups.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
		}
	...
	if ( $action_affiche === 'equipepeda' ) {
		if($_SESSION['statut']=='eleve') {
			...
			elseif($GepiAccesEleTrombiProfsClasse=='yes') {
				...
				echo "<p>You do not have access to the trombinoscopes teaching staffs.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
		}
	...
	if ( $action_affiche === 'discipline' ) {
		if($_SESSION['statut']=='eleve') {
			if($GepiAccesEleTrombiPersonnels=='yes') {
				...
				echo "<p>You do not have access to the trombinoscopes by disciplines.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
		}
	...
	if ( $action_affiche === 'statusgepi' ) {
		if($_SESSION['statut']=='eleve') {
			if(($GepiAccesEleTrombiPersonnels!='yes')&&($GepiAccesEleTrombiProfsClasse=='yes')) {
				...
				echo "<p>You do not have access to the trombinoscopes by statute.</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
		}
	...
	$execute_qui = mysql_query($requete_qui) or die('Error SQL !'.$requete_qui.'<br />'.mysql_error());
	if(mysql_num_rows($execute_qui)==0) {
		...
		echo "<p>The request did not turn over any recording.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
	$donnees_qui = mysql_fetch_array($execute_qui) or die('Error SQL !'.$execute_qui.'<br />'.mysql_error());
	...
	if ( $action_affiche === 'equipepeda' ) {
		echo "Teaching staff : ".$donnees_qui['nom_complet']." (<i>".$donnees_qui['classe']."</i>)";
