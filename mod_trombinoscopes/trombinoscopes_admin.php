...
* $Id: trombinoscopes_admin.php 8600 2011-11-05 20:11:36Z jjacquard $
*
...
$accessibilite="y";
$titre_page = "Management of the module trombinoscope";
$niveau_arbo = 1;
$gepiPathJava="./..";
$post_reussi=FALSE;
		...
		while ($fichier = readdir($dossier)) {
			if ($fichier!="." && $fichier!="..") {
				...
				if (is_dir($chemin_fichier)) aplanir_tree($chemin_fichier,$destination);
						...
						{
						if (!@copy($chemin_fichier,$destination."/".$fichier)) $erreurs.="Impossible to copy file ".$chemin_fichier." vers ".$destination.".<br/>";
						if ($chemin_fichier!=$destination.$fichier)
							if (!@unlink($chemin_fichier)) $erreurs.="Impossible to remove file ".$chemin_fichier.".<br/>";
						}
			}
		}
		closedir($dossier);
		...
		while ($fichier = readdir($dossier)) {
			if ($fichier != "." && $fichier != "..") {
				...
				if (is_dir($chemin_fichier)) del_tree($chemin_fichier);
					else if (!@unlink($chemin_fichier)) $erreurs.="Impossible to remove file ".$chemin_fichier.".<br/>";
			}
		}
		closedir($dossier);
		if (!@rmdir($chemin)) $erreurs.="Impossible to remove file ".$chemin.".<br/>";
    }
	else if (!@unlink($chemin)) $erreurs.="Impossible to remove file".$chemin.".<br/>";
	return $erreurs;
}
...
// $type_a_traiter :  : 'élève' ou 'personnel'
{
	...
	if($test_folder && !file_exists($folder)) {
		$avertissement.="you ZIP doesnt contain arborescence / picture/ ".$dossier_a_traiter." :</b><br/><span style='font-variant:normal; font-size: smaller;'>if you want to restore the picture of  ".$type_a_traiter."s, you must have <br/>in you  ZIP picture yhe ".$type_a_traiter."s into a sub-file picture/".$dossier_a_traiter."/</span><br/>\n";
	}
	else {
		$nb_photos=0;
		...
		if($nb_photos>0) {$msg_nb_trts.=$nb_photos." picture ".$type_a_traiter."(s) transfer(s).<br/>\n";}
		closedir($dossier);
	}
}
	...
	if (isset($_POST['num_aid_trombinoscopes'])) {
		if ($_POST['num_aid_trombinoscopes']!='') {
			if (!saveSetting("num_aid_trombinoscopes", $_POST['num_aid_trombinoscopes']))
					$msg = "Error during registration of parameter num_aid_trombinoscopes !";
		} else {
...
// Affichage des élèves sans photo
	...
	if (!recherche_eleves_sans_photo()){
		$msg .= "error during selection  of student without";
	}else{
		$eleves_sans_photo=recherche_eleves_sans_photo();
		$msg.="list of student whitout picture at the bottom ";
		$post_reussi=TRUE;
		}
	}
		...
		if (cree_zip_archive("photos")==TRUE)
			...
			if (isset($GLOBALS['multisite']) AND $GLOBALS['multisite']=='y')
				...
				if (!$repertoire_photos=$_COOKIE['RNE'])
					...
					$msg_multisite.="Multisite : error during the recuperation of file picture of establishment .<br/>";


					if ($msg_multisite=="")
				{
				if ($repertoire_photos!="") $repertoire_photos.="/";
				$repertoire_photos="../picture/".$repertoire_photos;
				...
				if ($nb_photos_supp>0)
					if ($nb_photos_supp>1) $msg=$nb_photos_supp." picture has delected.<br/>";
						else $msg="one picture has delected.<br/>";
					else $msg="not picture has delected.<br/>";
				$post_reussi=TRUE;
				if ($nb_erreurs>0)
					{
					if ($nb_erreurs>1) $msg.=$nb_erreurs." picture cannot be delecte.<br/>";
						else $msg.="one picture cannot be delecte.<br/>";
					$post_reussi=FALSE;
					}
				} else $msg=$msg_multisite.$msg;
			}
		else $msg.="error during creation of safeguard.<br/>";
		}
...
if (isset($_POST['action']) and ($_POST['action']=='upload_photos_eleves'))  {
	...
	if ($sav_file) {
		...
		if (is_file($dir_temp) && !@unlink($dir_temp)) $msg.="Impossible to delecte ".$dir_temp.".<br/>\n";
			else if (!file_exists($dir_temp)) 
				if (!@mkdir($dir_temp,0700,true)) $msg.="Impossible to create ".$dir_temp."..<br/>\n";
		if ($msg=="") {
			...
			if (!file_exists($dir_temp_photos_eleves)) 
				if (!@mkdir($dir_temp_photos_eleves,0700,true)) $msg.="Impossible to create ".$dir_temp_photos_eleves.".<br/>\n"; ;
			if ($msg=="") {
				...
				if ($reponse!="ok") {
					...
					if ($reponse!="ok") {
						...
						if (!@unlink ($dir_temp_photos_eleves."/".$_FILES["nom_du_fichier"]['name'])) {
							$msg .= "error during the removal of ".$dir_temp."/".$_FILES["nom_du_fichier"]."<br/>\n";
						}
					...
					if ($msg_multisite=="")
						...
						if ($msg_nb_trts=="") $msg_nb_trts="no picture was send .<br/>\n";
						if ($msg==""){
							$msg= $msg_nb_trts;
							$post_reussi=TRUE;
...
if (isset($_POST['action']) and ($_POST['action'] == 'upload'))  {
	...
	if ($sav_file) {
		...
		$dir_temp ="../temp/trombinoscopes";
		if ($multisite=='y' && isset($_COOKIE['RNE'])) $dir_temp."_".$_COOKIE['RNE'];
		if (is_file($dir_temp) && !@unlink($dir_temp)) $msg.="Impossible to remove ".$dir_temp."<br/>\n";
			else if (!file_exists($dir_temp)) 
				if (!@mkdir($dir_temp,0700,true)) $msg.="Impossible to create ".$dir_temp."<br/>\n";
		if ($msg=="") {
			//  copie du fichier ZIP dans $dir_temp
			...
			if ($reponse!="ok") {
				...
				if ($reponse!="ok") {
					...
					if (!@unlink ($dir_temp."/".$_FILES["nom_du_fichier"]['name'])) {
						$msg .= "error during the delection of ".$dir_temp."/".$_FILES["nom_du_fichier"]."<br/>\n";
					}
...
if (!suivi_ariane($_SERVER['PHP_SELF'],$titre_page))
		echo "error during creation of ariane wire ";
/****************************************************************
			FIN HAUT DE PAGE
****************************************************************/
//debug_var();
