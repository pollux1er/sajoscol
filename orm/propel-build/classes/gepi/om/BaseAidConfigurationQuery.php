 ...
 * @method     AidConfigurationQuery orderByDisplayReport card($order = Criteria::ASC) Order by the display_bulletin column
 ...
 * @method     AidConfigurationQuery groupByDisplayReport card() Group by the display_bulletin column
 ...
 * @method     AidConfiguration findOneByDisplayReport card(string $display_bulletin) Return the first AidConfiguration filtered by the display_bulletin column
 ...
 * @method     array findByDisplayReport card(string $display_bulletin) Return AidConfiguration objects filtered by the display_bulletin column
...
abstract class BaseAidConfigurationQuery extends ModelCriteria
{
	 ...
	 * Example usage:
	 * <code>
	 * $query->filterByDisplayReport card('fooValue');   // WHERE display_bulletin = 'fooValue'
	 * $query->filterByDisplayReport card('%fooValue%'); // WHERE display_bulletin LIKE '%fooValue%'
	 * </code>
	 *
	 * @param     string $displayReport card The value to use as filter.
	 *              Accepts wildcards (* and % trigger a LIKE)
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    AidConfigurationQuery The current query, for fluid interface
	 */
	public function filterByDisplayReport card($displayReport card = null, $comparison = null)
	{
		if (null === $comparison) {
			if (is_array($displayReport card)) {
				$comparison = Criteria::IN;
			} elseif (preg_match('/[\%\*]/', $displayReport card)) {
				$displayReport card = str_replace('*', '%', $displayReport card);
				$comparison = Criteria::LIKE;
			}
		}
		return $this->addUsingAlias(AidConfigurationPeer::DISPLAY_BULLETIN, $displayReport card, $comparison);
	}
