 ...
 * @method     ClasseQuery orderByModeleReport cardPdf($order = Criteria::ASC) Order by the modele_bulletin_pdf column
 ...
 * @method     ClasseQuery groupByModeleReport cardPdf() Group by the modele_bulletin_pdf column
 ...
 * @method     Classe findOneByModeleReport cardPdf(string $modele_bulletin_pdf) Return the first Classe filtered by the modele_bulletin_pdf column
 ...
 * @method     array findByModeleReport cardPdf(string $modele_bulletin_pdf) Return Classe objects filtered by the modele_bulletin_pdf column
...
abstract class BaseClasseQuery extends ModelCriteria
{
	 ...
	 * Example usage:
	 * <code>
	 * $query->filterByModeleReport cardPdf('fooValue');   // WHERE modele_bulletin_pdf = 'fooValue'
	 * $query->filterByModeleReport cardPdf('%fooValue%'); // WHERE modele_bulletin_pdf LIKE '%fooValue%'
	 * </code>
	 *
	 * @param     string $modeleReport cardPdf The value to use as filter.
	 *              Accepts wildcards (* and % trigger a LIKE)
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    ClasseQuery The current query, for fluid interface
	 */
	public function filterByModeleReport cardPdf($modeleReport cardPdf = null, $comparison = null)
	{
		if (null === $comparison) {
			if (is_array($modeleReport cardPdf)) {
				$comparison = Criteria::IN;
			} elseif (preg_match('/[\%\*]/', $modeleReport cardPdf)) {
				$modeleReport cardPdf = str_replace('*', '%', $modeleReport cardPdf);
				$comparison = Criteria::LIKE;
			}
		}
		return $this->addUsingAlias(ClassePeer::MODELE_BULLETIN_PDF, $modeleReport cardPdf, $comparison);
	}
