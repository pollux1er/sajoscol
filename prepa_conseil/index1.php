...
//**************** EN-TETE *****************
if ($en_tete == "yes") $titre_page = "Visualization of the averages and appreciations";
...
if (!$current_group) {
	...
	*/
    ...
    echo "<p class='bold'>\n";
	echo "<a href=\"".$retour."\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
	...
	if(($_SESSION['statut']=='professeur')||($_SESSION['statut']=='secours')) {
		...
		$tab_groups = get_groups_for_prof($login_prof_groupe_courant,"classify then matter");
    ...
    if ((!(check_prof_groupe($_SESSION['login'],$id_groupe))) and ($_SESSION['statut']!='scolarite') and ($_SESSION['statut']!='secours') and ($test_acces_pp=="n")) {
        echo "<p>You are not in this class the professor of the selected matter !</p>\n";
        echo "<p><a href='index1.php'>Return to home page</a></p>\n";
        die();
    }
    ...
    echo "<p>Choose the data to be printed (<i>you can notch several boxes</i>) : </p>\n";
    $i="1";
	$cpt=0;
    while ($i < $nb_periode) {
		$name = "visu_note_".$i;
		...
		echo "/><label for='$name' style='cursor: pointer;'><span id='champ_numero_$cpt'$temp_style>".ucfirst($nom_periode[$i])." - extract the averages</span></label></p>\n";
		$i++;
		$cpt++;
    }
    $i="1";
    while ($i < $nb_periode) {
		...
		if((isset($_SESSION[$name]))&&($_SESSION[$name]=='yes')) {echo "checked "; $temp_style=" style='font-weight:bold;'";} else {$temp_style="";}
		echo "onchange=\"checkbox_change(this.id, $cpt)\" ";
		echo "/><label for='$name' style='cursor: pointer;'><span id='champ_numero_$cpt'$temp_style>".ucfirst($nom_periode[$i])." - extract the appreciations</span></label></p>\n";
	    $i++;
		$cpt++;
    }
	...
	if($aff_rang=="y") {
		...
		if((isset($_SESSION[$name]))&&($_SESSION[$name]=='yes')) {echo "checked "; $temp_style=" style='font-weight:bold;'";} else {$temp_style="";}
		echo "onchange=\"checkbox_change(this.id, $cpt)\" ";
		echo "/><label for='afficher_rang' style='cursor: pointer;'><span id='champ_numero_$cpt'$temp_style>Display the row of the student.</span></label></p>\n";
		$cpt++;
	}
	//==========================================
	...
	if((isset($_SESSION[$name]))&&($_SESSION[$name]=='yes')) {echo "checked "; $temp_style=" style='font-weight:bold;'";} else {$temp_style="";}
	echo "onchange=\"checkbox_change(this.id, $cpt)\" ";
	echo "/><label for='afficher_mediane' style='cursor: pointer;'><span id='champ_numero_$cpt'$temp_style>Display the median, 1st and 3rd quartiles for each column of note.</span></label></p>\n";
	$cpt++;
	//==========================================
	...
	if((isset($_SESSION[$name]))&&($_SESSION[$name]=='yes')) {echo "checked "; $temp_style=" style='font-weight:bold;'";} else {$temp_style="";}
	echo "onchange=\"checkbox_change(this.id, $cpt)\" ";
	echo "/><label for='stat' style='cursor: pointer;'><span id='champ_numero_$cpt'$temp_style>Display the statistics on the extracted averages (<i>general average, percentages,...</i>)</span></label></p>\n";
	$cpt++;

    if ($multiclasses) {
        echo "<p><input type='radio' name='order_by' id='order_by_nom' value='nom' checked /><label for='order_by_nom' style='cursor: pointer;'> Classify the student alphabetically </label>";
        echo "<br/><input type='radio' name='order_by' id='order_by_classe' value='classe' /><label for='order_by_classe' style='cursor: pointer;'> Classify the student by class</label></p>";
    }
	...
	echo "onchange=\"checkbox_change(this.id, $cpt)\" ";
	echo "/><label for='couleur_alterne' style='cursor: pointer;'><span id='champ_numero_$cpt'$temp_style>Colour the lines (<i>alternate the prime coats</i>).</span></label></p>\n";
	$cpt++;
...
function checkbox_change(champ, cpt) {
    ...
    //
    $ligne1[1] = "Name First name";
    $ligne1_csv[1] = "Name First name";
    if ($multiclasses) {
		$ligne1[2] = "Classe";
		$ligne1_csv[2] = "Classe";
    }
...
//            $only_stats = 'no';
    ...
    if ($en_tete == "yes") {
		//echo " target=\"_blank\">\n";
		echo "<form enctype=\"multipart/form-data\" name= \"form1\" action=\"".$_SERVER['PHP_SELF']."\" method=\"get\">\n";
		echo "<p class='bold'><a href=\"index1.php?id_groupe=$id_groupe\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
		...
		echo "\n<!-- Form for display without heading -->\n";
		echo "<form enctype=\"multipart/form-data\" action=\"index1.php\" method=\"post\" name=\"formulaire1\"  target=\"_blank\">\n";
	...
	// MODIF: boireaus 20080421
	// Pour permettre de trier autrement...
	echo "\n<!-- Form for display with sorting on the clicked column -->\n";
	...
	if((isset($col_csv))&&(count($col_csv)>0)) {
		...
		if ($en_tete == "yes") {
			...
			echo "<br />\n";
			echo "Row height &nbsp;: <input type='text' name='h_cell' id='h_cell' value='10' size='2' onKeyDown=\"clavier_2(this.id,event,5,50);\" AutoComplete=\"off\" />mm\n";
		}
		echo "</form>\n";
...
//    $nombre_eleves = mysql_num_rows($appel_donnees_eleves);
    ...
    if ($test == 1 and  $stat == "yes") {
        ...
        <p><b>General average of the class : ".$moy_gen."</b>
        <br /><br /><b>Percentage of the pupils having a general average : </b>\n";
		echo "<table style='margin-left: 3em;' border='0'>\n";
		echo "<tr>\n";
        echo "<td>lower strictly than 8 : </td><td class='bold'>".$pourcent_i8."</td>\n";
		echo "</tr>\n";
		echo "<tr>\n";
        echo "<td>between 8 et 12 : </td><td class='bold'>".$pourcent_se8_ie12."</td>\n";
		echo "</tr>\n";
		echo "<tr>\n";
        echo "<td>higher or equalizes to 12 : </td><td class='bold'>".$pourcent_se12."</td>\n";
		echo "</tr>\n";
		echo "</table>\n";
		echo "</div>\n";
    }
