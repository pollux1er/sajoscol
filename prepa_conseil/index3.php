...
if ($_SESSION['statut'] == "responsable") {
	...
	if ($login_eleve != null and (strtoupper($login_eleve) != strtoupper($_SESSION['login']))) {
		tentative_intrusion(2, "Attempt at one ".$gepiSettings['denomination_eleve']." to visualize the bulletin simplified of another ".$gepiSettings['denomination_eleve'].".");
	}
...
if (isset($id_classe)) {
	...
	if (!is_numeric($id_classe)) {
		tentative_intrusion("2", "Change of the value of id_classe for a not numerical type.");
		echo "Error.";
	...
	//echo "\getSettingValue(\"GepiAccesReport cardSimpleProfToutesClasses\")=".getSettingValue("GepiAccesReport cardSimpleProfToutesClasses")."<br />";
	...
	if ($_SESSION['statut'] == "professeur" AND getSettingValue("GepiAccesReport cardSimpleProfToutesClasses") != "yes") {
		...
		if ((getSettingValue("GepiAccesReport cardSimpleProf") == "yes")||(getSettingValue("GepiAccesReport cardSimpleProfTousEleves") == "yes")) {
			...
			if ($test == "0") {
				tentative_intrusion("2", "Attempt at access by a teacher to a class in which it does not teach,
without having the authorization of it.");
...
there !";
			...
			if ($test == "0") {
				tentative_intrusion("2", "Attempt at access by a teacher to a class in which it is not	 $gepi_prof_suivi, without having the authorization of it.");
				echo "You cannot reach this class because do not be you there $gepi_prof_suivi!";
				require ("../lib/footer.inc.php");
				die();
			}
		}
...
//**************** EN-TETE *******************************
$titre_page = "Simplified edition of the bulletins";
...
if ($_SESSION['statut'] == "responsable" and $error_login == true) {
	echo "<p class=\"bold\"><a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return reception</a>";

	echo "<p>It seems that you are associated none ".$gepiSettings['denomination_eleve'].". Contact the administrator to solve this error.</p>";
	require "../lib/footer.inc.php";
	die();
}
...
if (
	...
	tentative_intrusion(1, "Attempt at access to the bulletins simplified without authorization.");
...
if (!isset($id_classe) and $_SESSION['statut'] != "responsable" AND $_SESSION['statut'] != "eleve") {
	...
	echo "<p class=\"bold\"><a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return reception</a>";
	...
	elseif($_SESSION['statut'] == 'professeur' and getSettingValue("GepiAccesReport cardSimpleProfToutesClasses") != "yes"){
	...
	if($nombreligne==0){
		echo "<p>No class is allotted to you.<br />Contactez the administrator so that it carries out the suitable parameter
setting in the Management of the classes.</p>\n";
	}
	else{
		echo "<p>Click on the class for which you wish to extract the bulletins</p>\n";
		//echo "<table border=0>\n";
		$nb_class_par_colonne=round($nombreligne/3);
			//echo "<table width='100%' border='1'>\n";
			echo "<table width='100%' summary='Choice of the class'>\n";
	...
	echo "<p>Cliquez sur le nom d'un ".$gepiSettings['denomination_eleve']." to visualize its simplified bulletin :</p>";
	...
	if ($_SESSION['statut'] != "responsable" and $_SESSION['statut'] != "eleve") {
		...
		*/
		...
		if(isset($id_class_prec)){
			if($id_class_prec!=0){echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_class_prec'>Classify preceding</a>";}
		}
		...
		if(isset($id_class_suiv)){
			if($id_class_suiv!=0){echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_class_suiv'>Following class</a></p>";}
		}
		...
		echo "<table summary='Choice of the student'>\n";
		echo "<tr>\n";
		...
		echo "/></td>\n";
		echo "<td><label for='choix_edit_1' style='cursor: pointer;'>Report cards simplified of all them ".$gepiSettings['denomination_eleves']." class";
		if ($_SESSION['statut'] == "professeur" AND getSettingValue("GepiAccesReport cardSimpleProfTousEleves") != "yes" AND getSettingValue("GepiAccesReport cardSimpleProfToutesClasses") != "yes") {
			echo " (only them ".$gepiSettings['denomination_eleves']." that I have in progress)";
		}
		echo "</label></td></tr>\n";
		...
		if ($nb_lignes > 1) {
			...
			echo "/></td>\n";
			echo "<td><label for='choix_edit_3' style='cursor: pointer;'>Only bulletins simplified of ".$gepiSettings['denomination_eleves']." of which it ".getSettingValue("gepi_prof_suivi")." is :</label>\n";
		...
		echo "/></td>\n";
		echo "<td><label for='choix_edit_2' style='cursor: pointer;'>Only the bulletin simplified of ".$gepiSettings['denomination_eleve']." selected opposite : </label>\n";
		...
		echo "/></td>\n";
		echo "<td><label for='choix_edit_4' style='cursor: pointer;'>The bulletin simplified of the appreciations on groupe-classe";
		echo "</label></td></tr>\n";
	...
	echo "Period : <select onchange=\"change_periode()\" size=1 name=\"periode1\">\n";
	$i = "1" ;
	while ($i < $nb_periode) {
	echo "<option value='$i'";
	...
	echo "&nbsp;at the period : <select size=1 name=\"periode2\">\n";
	$i = "1" ;
	while ($i < $nb_periode) {
	echo "<option value='$i'";
	...
	echo "<br /><br /><center><input type=submit value=Validate /></center>\n";
	echo "</form>\n";
}
require("../lib/footer.inc.php");
?>