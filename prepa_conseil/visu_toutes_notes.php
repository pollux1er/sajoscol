<?php
/*
* $Id: visu_toutes_notes.php 8673 2011-11-30 13:17:40Z crob $
*
...
if (isset($id_classe)) {
	...
	if (!is_numeric($id_classe)) {
		tentative_intrusion("3", "Change of the value of id_classe for a not numerical type, by changing the value of a field 'hidden' of a form.");
		echo "Erreur.";
	...
	if ($_SESSION['statut'] == "professeur" AND getSettingValue("GepiAccesMoyennesProfToutesClasses") != "yes") {
		...
		if ($test == "0") {
			tentative_intrusion("3", "Attempt at access by a teacher to a class in which it does not teach,
without having the authorization of it. Advanced attempt: change of the values of fields of the type 'hidden' form.");
...
//=========================
if ($aff_date_naiss){
	$ligne1[] = "<img src=\"../lib/create_im_mat.php?texte=".rawurlencode("Date of birth")."&amp;width=22\" width=\"22\" border=\"0\" alt=\"date of birth\" />";
	$ligne1_csv[] = "Date de naissance";
}
//=========================
if ($aff_reg){
	$ligne1[] = "<img src=\"../lib/create_im_mat.php?texte=".rawurlencode("Mode")."&amp;width=22\" width=\"22\" border=\"0\" alt=\"mode\" />";
	$ligne1_csv[]="Régime";
}
if($aff_doub){
	$ligne1[] = "<img src=\"../lib/create_im_mat.php?texte=Redoublant&amp;width=22\" width=\"22\" border=\"0\" alt=\"doubling\" />";
	$ligne1_csv[]="Redoublant";
}
if ($aff_abs){
	$ligne1[] = "<a href='#' onclick=\"document.getElementById('col_tri').value='".(count($ligne1)+1)."';".
				"document.forms['formulaire_tri'].submit();\">".
				"<img src=\"../lib/create_im_mat.php?texte=".rawurlencode("1/2 days of absence")."&amp;width=22\" width=\"22\" border=\"0\" alt=\"1/2 days of absence\" />".
				"</a>";
...
if (($aff_rang) and ($referent=="une_periode")){
	$ligne1[] = "<a href='#' onclick=\"document.getElementById('col_tri').value='".(count($ligne1)+1)."';".
				...
				"document.forms['formulaire_tri'].submit();\">".
				"<img src=\"../lib/create_im_mat.php?texte=".rawurlencode("Row of the student")."&amp;width=22\" width=\"22\" border=\"0\" alt=\"Row of the student\" />".
				"</a>";
		...
		if ($referent == "une_periode") {
			...
			while ($p < $nb_periode) {
				...
				if (!in_array($current_eleve_login[$j], $current_group["eleves"][$p]["list"])) {
					...
					if($current_eleve_login[$j]==$ele_login_debug) {
						$lignes_debug.="Period $p: Not followed<br />";
					}
				}
				else {
					// On récupère l'indice de l'élève dans $tab_moy pour la période $num_periode
					...
					if($current_eleve_login[$j]==$ele_login_debug) {
						...
						$lignes_debug.="Total for the matter: $moy (for $coef_moy note(s))<br />";
					}
			...
			if ($non_suivi == (pow(2,$nb_periode))) {
				...
				if($current_eleve_login[$j]==$ele_login_debug) {
					$lignes_debug.="Teaching not followed by the year.<br />";
				}
			}
			else if ($coef_moy != 0) {
				...
				if($current_eleve_login[$j]==$ele_login_debug) {
					$lignes_debug.="Annual average: $moy<br />";
				}
				...
				if ($current_coef > 0) {
					...
					if($current_eleve_login[$j]==$ele_login_debug) {
						$lignes_debug.="<b>Total of the coef:</b> ".$total_coef_eleve[$j+$ligne_supl]."<br />";
					}
					...
					if($current_mode_moy!="bonus") {
						...
						if($current_eleve_login[$j]==$ele_login_debug) {
							$lignes_debug.="One increases the total of the points of $coef_eleve*$moy<br />";
						}
					...
					elseif($moy>10) { // Cas d'une note à bonus:
						...
						if($current_eleve_login[$j]==$ele_login_debug) {
							$lignes_debug.="One increases the total of the points of $coef_eleve*($moy-10)<br />";
						}
					}

					if($current_eleve_login[$j]==$ele_login_debug) {
						$lignes_debug.="<b>Total of the points:</b> ".$total_points_eleve[$j+$ligne_supl]."<br />";
					}
					...
					if ($affiche_categories) {
						...
						if($current_eleve_login[$j]==$ele_login_debug) {
							$lignes_debug.="<p>One increases the total of the coef of the category $prev_cat_id de $coef_eleve_reserve<br />";
							$lignes_debug.="\$total_cat_coef_eleve[$j+$ligne_supl][$prev_cat_id]=".$total_cat_coef_eleve[$j+$ligne_supl][$prev_cat_id]."<br />";

							$lignes_debug.="One increases the total of the points of the category $prev_cat_id de $coef_eleve_reserve*$moy<br />";
...
if ($ligne_supl >= 1) {
	...
	if((isset($num_p1))&&(isset($num_p2))) {
		for($loop=$num_p1;$loop<$num_p2;$loop++) {
			...
			$ligne1[$nb_col].="<img src=\"../lib/create_im_mat.php?texte=".rawurlencode("General average P$loop")."&amp;width=22\" width=\"22\" border=\"0\" alt=\"General average P$loop\" />";
			$ligne1[$nb_col].="</a>";
			$ligne1_csv[$nb_col] = "General average P$loop";
			$j = '0';
			while($j < $nb_lignes_tableau) {
...
*/
	...
	if($referent!='une_periode') {
		...
		if(preg_match("/^Rang/i",$ligne1[$nb_col])) {$ligne1[$nb_col].="document.getElementById('sens_tri').value='inverse';";}
		$ligne1[$nb_col].="document.forms['formulaire_tri'].submit();\">";
		$ligne1[$nb_col].="<img src=\"../lib/create_im_mat.php?texte=".rawurlencode("General average")."&amp;width=22\" width=\"22\" border=\"0\" alt=\"General average\" />";
		$ligne1[$nb_col].="</a>";
		$ligne1_csv[$nb_col] = "General average";
		$j = '0';
		...
		while($j < $nb_lignes_tableau) {
			...
			if ($total_coef_eleve[$j+$ligne_supl] > 0) {
				...
				if($current_eleve_login[$j]==$ele_login_debug) {
					$lignes_debug.="<b>General average=</b>".$total_points_eleve[$j+$ligne_supl]."/".$total_coef_eleve[$j+$ligne_supl]."=".$col[$nb_col][$j+$ligne_supl]."<br />";
				}
		...
		if (($aff_rang) and ($referent!="une_periode")) {
			...
			*/
			...
			$ligne1_csv[$nb_col] = "Row of the student";
...
if($vtn_coloriser_resultats=='y') {
	...
	echo "<p class='bold' style='text-align:center;'>Caption colour application</p>\n";
	...
	$legende_colorisation.="<thead>\n";
		...
		$legende_colorisation.="<th>Limit<br />higher</th>\n";
		$legende_colorisation.="<th>Color text</th>\n";
		$legende_colorisation.="<th>Color cell</th>\n";
		$legende_colorisation.="</tr>\n";
	$legende_colorisation.="</thead>\n";
...
if ($referent == "une_periode") {
	echo "<p class=bold>Classe : $classe - Results : $nom_periode[$num_periode] - School year : ".getSettingValue("gepiYear")."</p>";
} else {
	echo "<p class=bold>Classe : $classe - Results : Annual averages - School year : ".getSettingValue("gepiYear")."</p>";
}
...
if($temoin_note_sup10=='y') {
	...
	if($nb_note_sup_10==1) {
		echo "<p>A matter is counted only for the notes higher than 10.</p>\n";
	}
	else {
		//echo "<p>".count($note_sup_10)." matières ne sont comptées que pour les notes supérieures à 10.</p>\n";
		echo "<p>".$nb_note_sup_10." matters are counted only for the notes higher than 10.</p>\n";
	}
}

if($temoin_note_bonus=='y') {
	if($temoin_note_bonus==1) {
		echo "<p>There is a matter with no-claims bonus&nbsp;: ";
	}
	else {
		echo "<p>There is ".$temoin_note_bonus." matters with no-claims bonus &nbsp;: ";
	}

	echo "only the points above 10/20 count (<em>possibly balanced</em>), but their coefficient is not integrated in the total of the
coefficients. (<em>regulate applied to the options of the Baccalaureat, by ex.</em>)'</p>\n";
}

if($vtn_coloriser_resultats=='y') {
	echo "<p class='bold'>Caption colour application&nbsp;:</p>\n";
	echo $legende_colorisation;
}
echo "<p><br /></p>\n";
...
echo "\n<!-- Form for posting with sorting on the clicked column -->\n";
...
*/
	$titre="Information";
	$texte="<p>This page posts the averages of the pupils of the class of ".$classe.".</p>";
	$texte.="<ul>";
	$texte.="<li>You can sort this table with the request&nbsp;: each heading of column is a key of sorting.</li>";
	$texte.="<li>You can also export these averages with format CSV (<i>readable by a spreadsheet</i>).</li>";
	$texte.="</ul>";
	//$texte.="";
	...
	echo "<script type='text/javascript'>
	// I seized not why the capture of the movements does not function
correctly here???
	// In fact, there was a problem of initialization of xMousePos and
yMousePos (corrected in position.js)
...
echo "<br />\n";
echo "You can also consult the averages of the report cards at one moment T
before the end of period via <a href='../cahier_notes/index2.php?id_classe=$id_classe'>Visualization of the averages of the report cards</a> while knowing that before the end of period, all the notes are not yet
necessarily seized... and that consequently information obtained can be called into question
by the results seized thereafter.";
echo "</p>\n";
//===========================================================
echo "</div>\n";
