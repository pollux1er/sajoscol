...
//**************** EN-TETE *****************
$titre_page = "Homework diary";
$page_accueil = "index.php?id_classe=-1";
require_once("lib/header.inc");
//**************** FIN EN-TETE *************
//On vérifie si le module est activé
if (getSettingValue("active_cahiers_texte")!='y') {
    echo("<center><p class='grand'>Homework diary is not accessible for the moment.</p></center>");
    require ("../lib/footer.inc.php");
    die();
}
if (getSettingValue("cahier_texte_acces_public")!='yes') {
    echo("<center><p class='grand'>The homework diary is not in public access.</p></center>");
    require ("../lib/footer.inc.php");
    die();
}
...
if ($test_login > 0) {
	echo "<p>If you have of an identifier and a password, < has href='../login.php'
>connectez you !</a></p>";
}
echo "</td>\n";
echo "<td style=\"text-align:center;\">\n";
echo "<p><span class='grand'>Homework diary";
...
if (($nb_test == 0) and ($id_classe != -1) and ($delai != 0)) {
    if ($delai == "") die("Error: Time of visualization of nondefinite personal work. Contact the administrator of GEPI of your establishment.");
    $nb_dev = 0;
    for ($i = 0; $i <= $delai; $i++) {
        ...
        if ($nb_devoirs_cahier_texte != 0) {
            ...
            if ($nb_dev == '1') {
                ...
                echo "<br /><center><b><font style='font-variant: small-caps;'>Personal work of $delai days according to ".strftime("%d %B %Y", $today)." for the class of $classe_nom</font></b></center><br />\n";
            ...
            echo "<div style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."; background-color: ".$color_fond_notices["f"].";\"><font color='".$color_police_travaux."' style='font-variant: small-caps;'><b>Personal work for ".strftime("%a %d %b", $jour)."</b></font>\n";
    ...
    echo "<center><h3 class='gepi'>".getSettingValue("gepiSchoolName"). " - school year " . getSettingValue("gepiYear")."</H3>\n";
    echo "<h3 class='gepi'><font color='red'>Choose a class and a matter.</font></h3>\n";
    ...
    echo "<br /><br /><p>Ce cahier de textes est géré sous <a href='http://gepi.mutualibre.org/'>GEPI</a>.\n<br />GEPI is, follow-up, and a graphic visualization management tool of the
school results (schools, colleges, colleges).\n<br />GEPI is a free and free software diffused under licence LPG.</p><br />\n";
...
if ($delai == "") die("Error: Nondefinite time of visualization of the homework. Contact the
administrator of GEPI of your establishment.");
    ...
    for ($i = 0; $i <= $delai; $i++) {
        ...
        if ($nb_devoirs_cahier_texte != 0) {
            ...
            if ($nb_dev == '1') {
                ...
                echo "<p><strong><font color='blue' style='font-variant: small-caps;'>Week of ".strftime("%d %B", $debutsemaine)." to ".strftime("%d %B %Y", $finsemaine)."</font></strong></p>\n";
                echo "<b>Personal work of $delai next days</b>\n";
            ...
            echo "<div style=\"border-style:solid; border-width:1px; border-color: ".$couleur_bord_tableau_notice."; background-color: ".$color_fond_notices["f"].";\"><div style='color: ".$color_police_travaux."; font-variant: small-caps; text-align: center; font-weight: bold;'>Personal work<br />pour le ".strftime("%a %d %b", $jour)."</div>\n";
            // Affichage des devoirs dans chaque matière
            while ($ind < $nb_devoirs_cahier_texte) {
                $content = mysql_result($appel_devoirs_cahier_texte, $ind, 'contenu');
                // Mise en forme du texte
...
$tm = date("m",$i);
$td = date("d",$i);
echo "</td><td><a title=\"Go at the previous day\" href=\"index.php?year=$yy&amp;month=$ym&amp;day=$yd&amp;id_classe=$id_classe&amp;id_groupe=$id_groupe\">&lt;&lt;</a></td><td align=center><a href=\"index.php?id_classe=$id_classe&amp;id_groupe=$id_groupe\">Today</a></td><td align=right><a title=\"Go at the next day\" href=\"index.php?year=$ty&amp;month=$tm&amp;day=$td&amp;id_classe=$id_classe&amp;id_groupe=$id_groupe\">&gt;&gt;</a></td></tr>\n";
// affichage du texte
echo "<tr><td colspan=\"4\">\n";
echo "<center><b>ten last meetings until ".strftime("%A %d %B %Y", $today)." :</b></center></td></tr>\n";
...
while (true) {
    ...
    $titre = "";
    if ($not_dev->type == "t") {
        $titre .= "<strong>To make for : </strong>\n";
    }
    ...
    if ($not_dev->type == "c") {
      if ($date_ct_old == $not_dev->date_ct) {
        $num_notice++;
        $titre .= " <b><i>(note N° ".$num_notice.")</i></b>";
      } else {
