...
if (!isset($annee_scolaire))  {
    ...
    if ($nb_annee >= 1) {
        ...
        echo "Consultation of the cards project</h1>";
        echo "<h2>Choose the year :</h2>\n";
...
if (($indice_aid =='') and ($annee_scolaire!=''))  {
  ...
  if ($nb_projet!=0) {
        ...
        if ($k!=0) {
            echo "Consultation of the cards project</h1></center>";
            echo "<ul>\n";
            $i = 0;
            while ($i < $nb_projet) {
              if ($annee_courante)
            ...
            echo "No card project is currently available.</h1></center>";
    } else {
        echo "No card project is currently available.</h1></center>";
    }
    include "../lib/footer.inc.php";
...
if ($annee_courante) {
  if (!VerifAidIsAcive($indice_aid,"",""))  {
    echo "<p>You try to reach tools which are not activated want to contact the
...
if ($nb_fiches_publiques ==0) {
     echo "<br />No card project is currently available.</h3></center>";
     include "../lib/footer.inc.php";
     die();

}
echo "<br />List projects ".$nom_projet."</H3></center>";


$_login = "";
$message_avertissement = "Warning: the school work which you will reach is in phase of
construction, which explains the possible defects.";
$non_defini = "-";
if (!$annee_courante)
