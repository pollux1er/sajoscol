...
//**************** EN-TETE *****************
if ($current_imprime=='n') $titre_page = "Homework diary - Overall picture";
$page_accueil = "index.php?id_classe=-1";
require_once("lib/header.inc");
//**************** FIN EN-TETE *************
//On vérifie si le module est activé
if (getSettingValue("active_cahiers_texte")!='y') {
    echo("<center><p class='grand'>The homework diary is not accessible for the moment.</p></center>");
...
if ($current_group) {
    if ($current_imprime=='n')
    echo "<a href=see_all.php?id_classe=$id_classe&amp;id_groupe=$id_groupe&amp;ordre=$ordre&amp;imprime=$current_imprime>Sort in the opposite order</a> - ";
    echo "<a href=see_all.php?id_classe=$id_classe&amp;id_groupe=$id_groupe&amp;ordre=$current_ordre&amp;imprime=$imprime>$text_imprime</a>";
    if ($current_imprime=='n')
    echo " - <a href=index.php?id_classe=$id_classe&amp;id_groupe=$id_groupe>Return</a> - ";
}
echo "<hr />";
...
if ($nb_test == 0) {
    echo "<center><h3 class='gepi'>Choose a class and a matter.</h3></center>";
    echo "</body></html>";
    die();
}
...
if ($html != '') {
    echo "<div  style=\"border-bottom-style: solid; border-width:2px; border-color: ".$couleur_bord_tableau_notice."; \"><b>GENERAL INFORMATION</b></div>";
...
echo "<div  style=\"border-bottom-style: solid; border-width:2px; border-color: ".$couleur_bord_tableau_notice."; \"><b>HOMEWORK DIARY: compte-returned meeting</b></div><br />";
  ...
  if ($current_ordre == "DESC") {
    ...
    if ($not_dev->type == "c") {
      if ($date_ct_old == $not_dev->date_ct) {
        $num_notice++;
        echo " <b><i>(note N° ".$num_notice.")</i></b>";
      } else {
