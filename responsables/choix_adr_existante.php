	...
	if($is_posted=="choix_adr_existante"){
		if($adr_id_existant==''){
			...
			if(mysql_num_rows($test)>0){
				...
				if(!$res_update){
					$msg.="Error during the insertion of association nobody/adress. ";
				}
				else{
					if(!isset($quitter_la_page)) {
						header("Location: modify_resp.php?pers_id=$pers_id");
...
//**************** EN-TETE *******************************
$titre_page = "Choose a responsible address";
...
if(!getSettingValue('conv_new_resp_table')){
	...
	if(mysql_num_rows($test)==0){
		...
		$test=mysql_query($sql);
		if(mysql_num_rows($test)>0){
			echo "<p>A conversion of the datastudent/responsible is necessary.</p>\n";
			echo "<p>Suivez ce lien: <a href='conversion.php'>CONVERT</a></p>\n";
			require("../lib/footer.inc.php");
			die();
		}
	}
...
if(!isset($quitter_la_page)){
	...
	echo "><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return</a></p>\n";
}
else {
	echo "<p class=bold><a href=\"#\"";
	echo " onclick=\"return confirm_close (this, change, '$themessage')\"";
	echo ">Close again the page</a></p>\n";
}
...
if($nb_adr==0){
	echo "<p>No address is yet defined.<p>\n";
}
else{
	...
	echo "<p>Choice of an address for $lig_pers->nom $lig_pers->prenom (<i>$pers_id</i>)</p>\n";
	...
	echo "Display <select name='limit'>\n";
	...
	echo "or <a href='#' onClick=\"document.getElementById('div_rech').style.display='';return false;\">filter:</a>\n";
	echo "<div id='div_rech' style='display:none;' align='center'>\n";
	echo "<table border='0'>\n";
	echo "<tr>\n";
	echo "<td>the addresses of which \n";
	echo "</td>\n";
	echo "<td>\n";
	echo "<label for='crit_rech_adr' style='cursor: pointer;'>\n";
	echo "<input type='radio' name='crit_rech' id='crit_rech_adr' value='adr' checked /> a line 'adrX'\n";
	echo "</label>\n";
	echo "<br />\n";
	echo "<label for='crit_rech_cp' style='cursor: pointer;'>\n";
	echo "<input type='radio' name='crit_rech' id='crit_rech_cp' value='cp' /> the postal code\n";
	echo "</label>\n";
	echo "<br />\n";
	echo "<label for='crit_rech_commune' style='cursor: pointer;'>\n";
	echo "<input type='radio' name='crit_rech' id='crit_rech_commune' value='commune' /> the name of commune\n";
	echo "</label>\n";
	echo "</td>\n";
	echo "<td>\n";
	...
	echo "<input type='radio' name='mode_rech' id='mode_rech_commence' value='start with' /> start with \n";
	echo "</label>\n";
	echo "<br />\n";
	echo "<label for='mode_rech_termine' style='cursor: pointer;'>\n";
	echo "<input type='radio' name='mode_rech' id='mode_rech_termine' value='ends in' /> ends in \n";
	echo "</label>\n";
	/*
	...
	*/
	...
	echo "</tr>\n";
	echo "</table>\n";
	echo "<center><input type='submit' value='Display' /></center>\n";
	...
	echo "<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Identifier</td>\n";
	echo "<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Lines of the address</td>\n";
	echo "<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Postal code</td>\n";
	echo "<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>";
	echo "Commune";
	echo "</td>\n";
	echo "<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Country</td>\n";
	echo "<td style='text-align:center; font-weight:bold; background-color:#96C8F0;'>Responsible associated</td>\n";
	...
	if(!isset($adr_id_actuel)){
		...
		echo "/></td>\n";
		echo "<td style='text-align:center; background-color:#FAFABE;' colspan='7'>Not to use an existing address (<i>not to modify</i>)</td>\n";
		echo "</tr>\n";
	}
