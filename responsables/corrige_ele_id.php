...
//**************** EN-TETE *****************
$titre_page = "Correction of ELE_ID according to Sconet";
...
if(!isset($step)) {
	...
	echo "<h2>File ElevesSansAdresses.xml</h2>\n";

	echo "<p>This page is intended to correct the fields ELE_ID table 'eleves' and of the table 'responsables2' according to the file ElevesSansAdresses.xml of Sconet.<br />The correction is not possible that if them ELENOET are correct.</p>\n";

	echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post'>\n";
	echo add_token_field();
	echo "<input type=hidden name='step' value='0' />\n";
	echo "<p>Select the file <b>ElevesAvecAdresses.xml</b> (<i>ou ElevesSansAdresses.xml</i>):<br />\n";
	...
	echo "<input type='checkbox' name='stop' id='id_form_stop' value='y' /><label for='id_form_stop' style='cursor: pointer;'> Decontaminate the automatic mode.</label></p>\n";
	//==============================
	...
	echo "<p><br /></p>\n";
	echo "<p><i>NOTE:</i> After a phase of analysis, the corrections will be proposed.</p>\n";
}
else{
	/*
	if($step>0){
	...
	*/
	...
	switch($step){
		case 0:
			...
			echo "<h2>Analyze XML student</h2>\n";
...
file temp/$tempdir</p>\n";
					...
					if($fp){
						...
						echo "<p>\n";
						echo "Analyze file to extract associations ELENOET/ELE_ID student affected in a class...<br />\n";
						...
						while(!feof($fp)){
							...
							if(strstr($ligne,"<STRUCTURES>")){
								echo "Beginning of the section STRUCTURES with the line <span style='color: blue;'>$cpt</span><br />\n";
								$temoin_structures++;
							}
							//if(strstr($ligne[$cpt],"</STRUCTURES>")){
							if(strstr($ligne,"</STRUCTURES>")){
								echo "End of the section STRUCTURES to the line <span style='color: blue;'>$cpt</span><br />\n";
								$temoin_structures++;
								break;
							}
	...
	setTimeout(\"test_stop('1')\",3000);
						...
						echo "<p align='center'><a href='".$_SERVER['PHP_SELF']."?step=1&amp;stop=y".add_token_in_url()."'>Continuation</a></p>\n";

						require("../lib/footer.inc.php");
						die();
					}
					else{
						echo "<p>ERROR: It was not possible to open the input file...</p>\n";
		...
		case 1:
			echo "<h2>Continuation of the analysis of XML</h2>\n";

			$dest_file="../temp/".$tempdir."/eleves.xml";
			$fp=fopen($dest_file,"r");
			if(!$fp){
				echo "<p>The XML student does not have the air present in the temporary file.<br />You would have jumped a stage???</p>\n";
				require("../lib/footer.inc.php");
				die();
			}
			else{
				...
				*/
				...
				echo "<p>";
				echo "Analyze file to extract information from the section TUDENT...<br />\n";
				//echo "<blockquote>\n";
				...
				while(!feof($fp)){
					...
					if(strstr($ligne,"<ELEVES>")){
						echo "Beginning of the section STUDENT with the line <span style='color: blue;'>$cpt</span><br />\n";
						flush();
						$temoin_eleves++;
					}
					//if(strstr($ligne[$cpt],"</ELEVES>")){
					if(strstr($ligne,"</ELEVES>")){
						echo "End of the section STUDENT with the line <span style='color: blue;'>$cpt</span><br />\n";
						flush();
						$temoin_eleves++;
						break;
					}
	...
	setTimeout(\"test_stop('2')\",3000);
			...
			echo "<p>$stat recording were updated in the table 'temp_gep_import2'.</p>\n";

			echo "<p align='center'><a href='".$_SERVER['PHP_SELF']."?step=2&amp;stop=y".add_token_in_url()."'>Continuation</a></p>\n";
			...
			break;
		case 2:
			echo "<h2>Seek correspondences ELENOET</h2>\n";
					...
					for($i=0;$i<$nb_ele_id;$i++){
						...
						if($elenoet[$i]!='') {
							if((isset($ele_id[$i]))&&($ele_id[$i]!='')) {
								...
								fclose($fsql);
								*/
								$correction1=mysql_query($sql);updateOnline($sql);

								if($old_ele_id[$i]!='') {
									$sql="UPDATE responsables2 SET ele_id='$ele_id[$i]' WHERE ele_id='$old_ele_id[$i]';";
									affiche_debug("$sql<br />\n");
									//echo "$sql<br />\n";
									$correction2=mysql_query($sql);updateOnline($sql);
								}
								$cpt++;
							}
						}
					}
					echo "<p>$cpt correction carried out.</p>\n";
				}
			}
			...
			if(!$res=mysql_query($sql)) {
				echo "<p>Une <span style='color:red;'>error</span> occurred on the request &nbsp;:<br /><span style='color:green;'>".$sql."</span><br />\n";
				//Illegal mix of collations
				if(my_eregi("Illegal mix of collations",mysql_error())) {
					//echo "<span style='color:red'>".mysql_error()."</span>\n";
					echo "It seems that there is a problem of 'collation' between the fields 'eleves.ele_id' et 'temp_gep_import2.ele_id'&nbsp;:<br />\n";
					echo "<span style='color:red'>".mysql_error()."</span><br />\n";
					echo "It would be necessary to remove the table ' temp_gep_import2 ', to
inform the value of 'mysql_collate' in the table 'setting' by putting the same collation as for your field 'eleves.ele_id'.<br />\n";
				...
				if(mysql_num_rows($res)==0){
					if(isset($_POST['is_posted'])) {
						echo "<p>There is no more d'ELENOET for which to carry out a correction.</p>\n";
						echo "<p>Finished.</p>\n";
					}
					else{
						echo "<p>None ELENOET found to carry out a correction.</p>\n";
					}
				}
				else{
					echo "<p>To notch the lines for which to correct l'ELE_ID in the table 'eleves' (<i>and possibly in the table 'responsables2'</i>) according to the proposal in last column.</p>\n";
