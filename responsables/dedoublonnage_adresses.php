...
//**************** EN-TETE *****************
$titre_page = "duplicate of the responsible addresses";
...
if(!isset($step)) {
	...
	echo "<h2>Duplicate addresses responsible</h2>\n";

	echo "<p>This page is intended to carry out the duplicate addresses
considered wrongly by Sconet as of the different addresses whereas
they are identical.</p>\n";
	...
	echo "<input type='checkbox' name='stop' id='id_form_stop' value='y' /><label for='id_form_stop' style='cursor: pointer;'> Determine the automatic mode.</label></p>\n";
	//==============================
	...
	echo "<h2>Duplicate of the addresses the responsible ones</h2>\n";

	if(!isset($parcours_diff)){

		echo "<p>Initialization of the process.</p>\n";
		...
		if($nb_resp==0){
			echo "<p>The table 'tempo2' is empty???<br />No responsible would be yet defined?</p>\n";
			require("../lib/footer.inc.php");
			die();
		}

		echo "<p>Les ".$nb_resp." responsible will be traversed by sections of 20 in the search of
differences.</p>\n";

		$nb_parcours=ceil($nb_resp/20);

		$parcours_diff=0;
		echo "<p>Course of the section <b>$parcours_diff</b>.</p>\n";
	}
	else{
					...
					while($lig3=mysql_fetch_object($res3)) {
						...
						if($temoin=="y") {
							...
							*/
							...
							if($cpt==0) {echo "<p><b>duplicate for:</b> ";} else {echo " - ";}
