...
if(isset($suppr_ad)) {
	...
	for($i=0;$i<count($suppr_ad);$i++){
		...
		if(mysql_num_rows($test)==0){
			...
			if(!$res_suppr){
				$msg.="Error during the removal of the address n∞$suppr_ad[$i]. ";
				$temoin_suppr++;
			}
		}
		else{
			$msg.="Impossible removal of the address n∞$suppr_ad[$i] associÈe ";
			$temoin_suppr++;
			if(mysql_num_rows($test)==1){
				$lig_resp=mysql_fetch_object($test);
				$msg.="with responsible the n∞ <a href='modify_resp.php?pers_id=".$lig_resp->pers_id."'>".$lig_resp->pers_id."</a>. ";
			}
			else{
				$msg.="with responsible the n∞";
				$lig_resp=mysql_fetch_object($test);
...
//**************** EN-TETE *******************************
$titre_page = "Management of the addresses the responsible ones";
...
?>
<p class='bold'><a href="index.php"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return</a> | <a href="dedoublonnage_adresses.php">Duplicate adresses</a></p>
...
<?php
	...
	*/
	...
	$critere_recherche=preg_replace("/[^a-zA-Z¿ƒ¬…» ÀŒœ‘÷Ÿ€‹Ωº«Á‡‰‚ÈËÍÎÓÔÙˆ˘˚¸0-9_ -]/", "", $critere_recherche);
...
*/
	...
	echo "<input type='submit' name='filtrage' value='display' /> les ";
	echo "<input type='text' name='nb_adr' id='nb_adr' value='$nb_adr' size='3' />\n";
	echo " first addresses ";
	...
	echo "/><label for='champ_rech_commune' style='cursor:pointer;'> the field <b>commune</b></label><br />\n";
	echo "<input type='radio' name='champ_rech' id='champ_rech_cp' value='cp' ";
	if($champ_rech=="cp") {echo "checked ";}
	echo "/><label for='champ_rech_cp' style='cursor:pointer;'> the field <b>code postal</b></label><br />\n";
	echo "<input type='radio' name='champ_rech' id='champ_rech_adrX' value='adrX' ";
	if($champ_rech=="adrX") {echo "checked ";}
	echo "/><label for='champ_rech_adrX' style='cursor:pointer;'> one of the fields <b>adrX</b></label>\n";
	echo "</td>\n";
	...
	echo "/><label for='champ_rech_nonAssoc' style='cursor:pointer;'> not associated.</label>";
	echo "</td>\n";
	echo "</tr>\n";
...
if(mysql_num_rows($res_adr)>0){
	...
	echo "<p align='center'><input type='submit' value='Record' /></p>\n";

	echo "<p align='center'><a href=\"javascript:modif_suppr_adr_id_non_assoc('coche')\">Tick</a>\n";
	echo " / ";
	echo "<a href=\"javascript:modif_suppr_adr_id_non_assoc('decoche')\">shoot</a>\n";
	echo " not associated addresses.</p>";


	// Ajouter un lien pour cocher les adresses non associÈes.
	...
	$ligne_titre.="<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Identifier</td>\n";
	$ligne_titre.="<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Lines of the address</td>\n";
	$ligne_titre.="<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Postal code</td>\n";
	$ligne_titre.="<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Commune</td>\n";
	$ligne_titre.="<td style='text-align:center; font-weight:bold; background-color:#AAE6AA;'>Country</td>\n";
	$ligne_titre.="<td style='text-align:center; font-weight:bold; background-color:#96C8F0;'>Responsible associated</td>\n";
	$ligne_titre.="<td style='text-align:center; font-weight:bold; background-color:red;'>Remove address <br />\n";
	...
	function reporter_valeur(){
		for (i=0;i<$cpt;i++) {
			if (document.forms.choix_adr.adr_id_existant[i].checked==true) {
				//alert('C est le choix '+Number(i+1)+' qui est sÈlectionnÈ');
		...
		// Je ne parviens pas ‡ modifier l'adr_id affichÈ dans la page modify_resp.php
...
*/
	...
	echo "<p>No address in the table 'resp_adr'";
	if($critere_recherche!="") {echo " for research";}
	echo ".</p>\n";
}
