	...
	if(isset($suppr_resp1)){
		...
		for($i=0;$i<count($suppr_resp);$i++){
			...
			if($res0){
					...
					if(mysql_num_rows($res1)>0){
						...
						if(mysql_num_rows($res3)>0){
							...
							if($lig3->statut=='responsable') {
								...
								//echo "$sql<br />\n";
								$res4=mysql_query($sql);updateOnline($sql);
								if(!$res4){
									$msg.="Error during the suppression of the account of user $lig1->login.<br />\n";
								}
								else {
									$msg.="Count of user $lig1->login delecte.<br />\n";
								}
							}
							else {
								$msg.="ANOMALY: An account of user exists associated with the login $lig1->login, but not of responsible statute &nbsp;: $lig3->statut<br />\n";
							}
						}
						...
						if(!$res2){
							$msg.="Error during the suppression of the responsible one $suppr_resp[$i] table 'resp_pers'.<br />\n";
						}
						else{
							...
							if(mysql_num_rows($res3)==0){
								...
								$res4=mysql_query($sql);updateOnline($sql);
								if(!$res4){
									$msg.="Error during the removal of the address $lig1->adr_id table 'resp_adr'.<br />\n";
								}
							}
						}
					}
				...
				$msg.="Error during the suppression of the responsible one $suppr_resp[$i] table 'responsables2'.<br />";
			}
		}
	}
...
//**************** EN-TETE *****************
$titre_page = "Management of ".$gepiSettings['denomination_responsables'];
...
if(!getSettingValue('conv_new_resp_table')){
	...
	if(mysql_num_rows($test)>0){
		echo "<p>A conversion of the data ".$gepiSettings['denomination_responsables']." is necessary.</p>\n";
		echo "<p>Follow this jump: <a href='conversion.php'>CONVERT</a></p>\n";
	...
	$test=mysql_query($sql);
	if(mysql_num_rows($test)==0){
		echo "<p>A conversion of the data ".$gepiSettings['denomination_eleves']."/".$gepiSettings['denomination_responsables']." is necessary.</p>\n";
		...
		if(mysql_num_rows($test)>0){
			if ($_SESSION['statut'] == 'administrateur'){
				echo "<p class='bold'><a href=\"../accueil_admin.php\"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return</a></p>\n";
			}
			else{
...
echo "<p class='bold'>";
if ($_SESSION['statut'] == 'administrateur') {
	echo "<a href=\"../accueil_admin.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
	echo " | <a href=\"modify_resp.php\">Add one ".$gepiSettings['denomination_responsable']."</a>\n";
	if(getSettingValue("import_maj_xml_sconet")==1){
		echo " | <a href=\"maj_import.php\">Update since Sconet</a>\n";
	}

	if($num_resp!=0){
		echo " | <a href=\"index.php?num_resp=0&amp;order_by=nom,prenom\">Nonassociated people</a>\n";
	}
	else{
		echo " | <a href=\"index.php?num_resp=1&amp;order_by=nom,prenom\">Associated people</a>\n";
	}

	echo " | <a href=\"gerer_adr.php\">Manage adresses</a>\n";

	$sql="SELECT 1=1 FROM utilisateurs WHERE statut='responsable';";
	$test_resp=mysql_query($sql);
	if(mysql_num_rows($test_resp)>0) {
		echo " | <a href=\"synchro_mail.php\">Synchronize the addresses mall responsibles</a>\n";
	}
}
else{
...
if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($_SESSION['statut']=='cpe')) {
	echo " | <a href='infos_parents.php'>Information student/parents, phone, mail and addresses</a>";
}
echo "</p>\n";
	...
	if($val_rech!=""){
		...
		switch($champ_rech){
			case "resp0":
					...
					$chaine_info_recherche.="the $crit_rech $mode_rech $val_rech";
				break;
			case "resp1":
					$chaine_recherche="rp.$crit_rech LIKE '$valeur_cherchee'";
					$num_resp=1;

					$chaine_info_recherche.="lthe $crit_rech the responsible legal one 1 $mode_rech $val_rech";
				break;
			case "resp2":
					$chaine_recherche="rp.$crit_rech LIKE '$valeur_cherchee'";
					$num_resp=2;

					$chaine_info_recherche.="the $crit_rech the responsible legal one 2 $mode_rech $val_rech";
				break;
			case "eleves":
					$chaine_recherche="e.$crit_rech LIKE '$valeur_cherchee'";
					$num_resp="ele";

					$chaine_info_recherche.="the $crit_rech of student $mode_rech $val_rech";
				break;
		}
	}
}
...
//else{
	...
	if(($order_by=="nom,prenom")&&("$num_resp"=="0")){
		...
		*/
		...
		$test=mysql_query($sql);
		if(mysql_num_rows($test)>0){
			echo "<p>Suppression of responsibilities without ".$gepiSettings['denomination_eleve'].".<br />Here the list of the identifiers of ".$gepiSettings['denomination_responsables']." who were associated ".$gepiSettings['denomination_eleves']." non-existent: \n";
			$cpt_nett=0;
	...
	if($cpt==0){
		echo "<p>None ".$gepiSettings['denomination_responsable']." find.</p>\n";
		//if($chaine_recherche!="") {
		if((isset($chaine_recherche))&&($chaine_recherche!="")) {
			echo "<p><a href='".$_SERVER['PHP_SELF']."'>Return to the index ".$gepiSettings['denomination_responsables']."</a></p>\n";
		}
		require("../lib/footer.inc.php");
		die();
	}
...
if("$num_resp"=="0"){
	echo ucfirst($gepiSettings['denomination_responsables'])." without ".$gepiSettings['denomination_eleve']." associé";
}
elseif(($order_by=="nom,prenom")&&("$num_resp"=="1")) {
	echo $gepiSettings['denomination_responsables']." sorted by name of ".$gepiSettings['denomination_responsable']." 1";
}
elseif(($order_by=="nom,prenom")&&("$num_resp"=="2")) {
	echo ucfirst($gepiSettings['denomination_responsables'])." sorted by name of ".$gepiSettings['denomination_responsable']." 2";
}
elseif(($order_by=="nom,prenom")&&("$num_resp"=="ele")) {
	echo ucfirst($gepiSettings['denomination_responsables'])." sorted by name of pupil";
}

if($chaine_info_recherche!=""){
	echo "<br />dont ".$chaine_info_recherche;
}
echo "(<span style='font-weight:normal; font-style:italic;'>manpower: $cpt</span>)";
echo ".</p>\n";
...
echo "<input type='button' value='Display' onClick='decoche_suppr_et_valide();' />\n";
...
echo "</select> recordings starting from the recording n°\n";
...
echo "<script type='text/javascript'>
	...
	function suivant(){
		...
		if(limit=='TOUS'){
			...
			// It would be necessary to recover the number of lines of the table...\n";
...
if($num_resp==0){
	...
	echo "<a href='#' onClick=\"document.getElementById('div_rech').style.display='';return false;\">search</a>\n";
	echo "<div id='div_rech' style='display:none;' align='center'>\n";
	echo "<table border='0' summary='Recherche'><tr><td>the personnnes of which it \n";
	...
	echo "<input type='radio' name='mode_rech' id='mode_rech_commence' value='start with'";
	if($val_rech!=""){
	...
	echo "<input type='radio' name='mode_rech' id='mode_rech_termine' value='ends in'";
	if($val_rech!=""){
	...
	echo "<a href='#' onClick=\"document.getElementById('div_rech').style.display='';return false;\">search</a>\n";
	echo "<div id='div_rech' style='display:none;' align='center'>\n";
	echo "<table border='0' summary='search'><tr><td>among </td>\n";
	echo "<td>\n";
	echo "<label for='champ_rech_resp1' style='cursor: pointer;'>\n";
	echo "<input type='radio' name='champ_rech' id='champ_rech_resp1' value='resp1' checked /> responsible (<i>legal 1</i>)\n";
	echo "</label>\n";
	echo "<br />\n";
	echo "<label for='champ_rech_resp2' style='cursor: pointer;'>\n";
	echo "<input type='radio' name='champ_rech' id='champ_rech_resp2' value='resp2' /> responsible (<i>legal 2</i>)\n";
	echo "</label>\n";
	echo "<br />\n";
	echo "<label for='champ_rech_eleves' style='cursor: pointer;'>\n";
	echo "<input type='radio' name='champ_rech' id='champ_rech_eleves' value='eleves' /> student\n";
	echo "</label>\n";
	echo "</td>\n";
	echo "<td>\n";
	echo " those of which it \n";
	echo "</td>\n";
/*
	echo "<td>\n";
	echo "<label for='crit_rech_nom' style='cursor: pointer;'>\n";
...
if("$num_resp"=="0"){
	...
	*/
	...
	if(mysql_num_rows($res1)>0){
		...
		$ligne_titre="";
		$ligne_titre.="<tr>\n";
		$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#AAE6AA;'>Name first name</td>\n";
		$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#AAE6AA;'>Address</td>\n";
		$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#AAE6AA;'>Remove";
		$ligne_titre.="<br />\n";

		$ligne_titre.="<a href=\"javascript:modifcase('coche')\">\n";
		$ligne_titre.="<img src='../images/enabled.png' width='15' height='15' alt='All tick' /></a>\n";
		$ligne_titre.=" / ";
		$ligne_titre.="<a href=\"javascript:modifcase('decoche')\">";
		$ligne_titre.="<img src='../images/disabled.png' width='15' height='15' alt='ALL shoot' /></a>\n";
	...
	function modifcase(mode){
		...
		echo "<table class='boireaus' align='center' summary='Responsible'>\n";
...
/*
		...
		echo "<p>None ". $gepiSettings['denomination_responsable']." was not found in the table 'resp_pers'.</p>\n";
	}
	...
	$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#AAE6AA;' colspan='3'>Legal responsible 1</td>\n";
	//$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#FAFABE;' rowspan='2'><a href='index.php?order_by=nom,prenom&amp;tri=ele'>Student(s)</a></td>\n";
	...
	$ligne_titre.="'>Student(s)</a></td>\n";
	//$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#96C8F0;' colspan='2'>Responsable légal 2</td>\n";
	$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#96C8F0;' colspan='3'>Legal responsible 2</td>\n";
	$ligne_titre.="</tr>\n";
	$ligne_titre.="<tr>\n";
	...
	$ligne_titre.="'>Name first name</a></td>\n";
	$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#AAE6AA;'>Adress</td>\n";
	$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#AAE6AA;'>Rmove</td>\n";
	...
	$ligne_titre.="'>Name first name</a></td>\n";
	$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#96C8F0;'>Adress</td>\n";
	$ligne_titre.="<td style='font-weight:bold; text-align:center; background-color:#96C8F0;'>Remove</td>\n";
	$ligne_titre.="</tr>\n";
...
if($cpt>0){
	...
	echo "<p align='center'>None ". $gepiSettings['denomination_responsable']." was not found.</p>\n";
}
echo "<p><br /></p>\n";
