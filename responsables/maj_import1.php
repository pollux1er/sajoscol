<?php
/*
 * $Id: maj_import1.php 6578 2011-03-02 16:45:10Z crob $
 *
...
Sconet.</p>\n";
	...
	echo "<p>You will import the files <b>CSV</b> (<i><a href='../init_xml/lecture_xml_sconet.php?ad_retour=".$_SERVER['PHP_SELF']."'>generated</a> starting from exports XML of Sconet</i>).<br />\nThe necessary files during the procedure are initially ELEVES.CSV, then two files PERSONNES.CSV and ADRESSES.CSV, and finally the file RESPONSABLES.CSV</p>\n";
...
to the importation responsible information.<br />\n";
	echo "<a href='".$_SERVER['PHP_SELF']."?is_posted=y&amp;step=3".add_token_in_url()."'>Pass nevertheless in the page of importation the responsible ones</a></p>";
}
else{
	check_token();
	...
	switch($step){
		...
		echo "<h2>Importation/update of the student</h2>\n";
			...
			echo "<p>In the table, the classes are mentioned only on a purely informative
basis.<br />The assignment in the classes is not (<i>still</i>) ensured since this page.</p>\n";
			...
			if(strtoupper($csv_file['name']) == "ELEVES.CSV"){
				...
				if(!$fp){
					...
					echo "<p><a href='".$_SERVER['PHP_SELF']."'>Cliquer ici </a> Start again !</center></p>\n";
				}
				else{
					...
					*/
					...
					echo "<td style='text-align: center; font-weight: bold;'>Record<br />\n";
					...
					echo "</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>Statute</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>elenoet</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>ele_id</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>Name</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>First name</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>Sexe</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>Birth</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>Doubly</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>N°NAT</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>Mode</td>\n";
					echo "<td style='text-align: center; font-weight: bold;'>Class</td>\n";
					echo "</tr>\n";
					$cpt_modif=0;
					$cpt_new=0;
					$alt=1;
		...
		*/
					...
					echo "</table>\n";
					echo "<p>One counts $cpt_modif modified fields and $cpt_new nouveaux student.</p>\n";
					fclose($fp);
				}

			}
			else{
				echo "<p style='color:red;'>The name of the file proposed does not coincide with what is awaited: ELEVES.CSV</p>\n";
				echo "<p><a href='".$_SERVER['PHP_SELF']."'>Return to the choice of the file ELEVES.CSV</a></p>\n";
				echo "</form>\n";
				echo "</div>\n";
				echo "</body></html>\n";
				die();
		...
		case 2:
			...
			if(isset($modif)){
				...
				for($i=0;$i<count($modif);$i++){
					...
					$res1=mysql_query($sql);updateOnline($sql);
					//echo "<p>$sql</p>\n";
					if(!$res1){
						//echo " (<font color='red'>erreur</font>)";
						echo "<br />\n<font color='red'>Error:</font> $sql<br />\n";
						$erreur++;
					}
					...
					if(!$res1){
						...
						if(mysql_num_rows($res2)==0){
							$sql="INSERT INTO j_eleves_regime SET doublant='$doublant',
										...
										login='$login_eleve'";
							//echo "$sql<br />\n";
							$res3=mysql_query($sql);updateOnline($sql);
							if(!$res3){
								//echo " (<font color='red'>erreur</font>)";
								echo "<br />\n<font color='red'>Error:</font> $sql<br />\n";
								$erreur++;
			...
			switch($erreur){
				case 0:
					echo "<p>Pass at the stage of <a href='".$_SERVER['PHP_SELF']."?step=3".add_token_in_url()."'>importation/update of the people (<i>responsible</i>) and addresses</a>.</p>\n";
					break;

				case 1:
					echo "<p><font color='red'>An error occurred.</font><br />\nYou should seek the cause of it before passing at the stage of <a href='".$_SERVER['PHP_SELF']."?step=3".add_token_in_url()."'>importation/update of the people (<i>responsables</i>) and addresses</a>.</p>\n";
					break;

				default:
					echo "<p><font color='red'>$erreur errors occurred.</font><br />\nYou should seek the cause of it before passing at the stage of <a href='".$_SERVER['PHP_SELF']."?step=3".add_token_in_url()."'>importation/update of the people (<i>responsible</i>) and addresses</a>.</p>\n";
					break;
			}

			break;
		case 3:
			echo "<h2>Importation/update of the people (<i>responsible</i>) and addresses</h2>\n";
			...
			echo "<p>You will import the files <b>CSV</b> (<i><a href='../init_xml/lecture_xml_sconet.php?ad_retour=".$_SERVER['PHP_SELF']."'>generated</a> starting from exports XML of Sconet</i>).<br />\nThe files required here are the two files PERSONNES.CSV and ADRESSES.CSV</p>\n";
			...
			echo "<p>It is recommended to import information student and to pass only then
to the importation responsible associations/student.<br />\n";
			echo "<a href='".$_SERVER['PHP_SELF']."?is_posted=y&amp;step=6".add_token_in_url()."'>To pass nevertheless in the page of importation responsible
associations/student </a></p>";
			break;
		case 4:
			...
			if(strtoupper($csv_file['name']) == "ADRESSES.CSV") {
				...
				echo "<p>The selected files are not valid !<br />\n";
				echo "Control that the file names coincide with what is claimed.<br />\n";
				//echo "<a href='disciplines.php'>Cliquer ici </a> pour recommencer !</center></p>";
				echo "<a href='".$_SERVER['PHP_SELF']."'>Click here </a> to start again !</center></p>\n";
				echo "</body>\n</html>\n";
				die();
			...
			if(strtoupper($csv_file['name']) == "PERSONNES.CSV") {
				...
				if(!$fp){
					...
					echo "<p><a href='".$_SERVER['PHP_SELF']."'>To click here </a> to start again !</center></p>\n";
				}
				else{
				...
				//echo "<a href='disciplines.php'>Cliquer ici </a> pour recommencer !</center></p>";
				echo "<a href='".$_SERVER['PHP_SELF']."'>To click here </a> to start again !</center></p>\n";
				echo "</body>\n</html>\n";
				die();
			...
			*/
			...
			echo "<td style='text-align: center; font-weight: bold;' rowspan='2'>Record<br />\n";
			echo "<a href=\"javascript:modifcase('coche')\">";
			echo "<img src='../images/enabled.png' width='15' height='15' alt='All tick' /></a>";
			echo " / ";
			echo "<a href=\"javascript:modifcase('decoche')\">";
			...
			echo "<td style='text-align:center; font-weight:bold; background-color:#FAFABE;' rowspan='2'>Adress</td>\n";
			...
			echo "<td style='text-align:center; font-weight:bold; background-color: rgb(150, 200, 240);'>Civility</td>\n";
	...
	function modifcase(mode){
		...
		case 5:
			...
			if(isset($new)){
				echo "<p>Insertion of information for ";
				for($i=0;$i<count($new);$i++){
					$cpt=$new[$i];
			...
			switch($erreur){
				case 0:
					echo "<p>Pass at the stage of <a href='".$_SERVER['PHP_SELF']."?step=6".add_token_in_url()."'>importation/update of the responsible ones</a>.</p>\n";
					break;

				case 1:
					echo "<p><font color='red'>An error occurred.</font><br />\nYou should seek the cause of it before passing at the stage of <a href='".$_SERVER['PHP_SELF']."?step=6".add_token_in_url()."'>importation/update of the responsible ones</a>.</p>\n";
					break;
								...
								*/
								...
								if(mysql_num_rows($res1)==0){
									...
									if(mysql_num_rows($res2)==0){
										...
										echo "<td colspan='7'>No associated person???</td>\n";
									}
									else{
										$lig2=mysql_fetch_object($res2);
										echo "<td style='text-align:center;'>\n";
										...
										echo "<td style='text-align:center; background-color: rgb(150, 200, 240);'>New</td>\n";
										...
										if(mysql_num_rows($res4)==0){
											...
											echo "No student for ele_id=$ele_id ???";
											echo "</td>\n";
										}
										else{
									...
									(stripslashes($lig1->pers_contact)!=stripslashes($affiche[3]))){
										...
										*/
										...
										if(mysql_num_rows($res2)==0){
											...
											echo "<td style='background-color:red;'>&nbsp;</td>\n";
											echo "<td colspan='5'>No associated person???</td>\n";
										}
										else{
											$lig2=mysql_fetch_object($res2);
											echo "<td style='text-align:center;'>\n";
	...
	function modifcase(mode){
		...
		case 8:
			...
			switch($erreur){
				case 0:
					echo "<p>The importation correctly occurred.</p>\n";
					break;

				case 1:
					echo "<p><font color='red'>An error occurred.</font><br />\nYou should seek the cause of it.</p>\n";
					break;
...
echo "<ul>\n";
echo "<li>\n";
echo "<p>The file names provided in the fields of forms must coincide with the
name indicated ELEVES.CSV, ADRESSES.CSV,...\n";
echo "</p>\n";
echo "</li>\n";
echo "<li>";
echo "<p>It also remains to ensure the importation of the establishment of
origin with the files etablissements.csv et eleves_etablissements.csv<br />\n";
echo "In addition, the inscription of the pupils in such or such class, with such and such option is not ensured yet by this page of
importation/update.<br />\n";
echo "(<i>it is thus necessary thereafter to assign the new pupils in the
classes and to register them in the groups/options/subject</i>)<br />\n";
echo "It is also necessary, for the new registered student (<i>s'il y en a</i>), to do association with the responsible CPE and it ".getSettingValue('gepi_prof_suivi').".";
echo "</p>\n";
echo "</li>\n";
echo "</ul>\n";
