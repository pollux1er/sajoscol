...
if(isset($_SESSION['retour_apres_maj_sconet'])) {
	echo "<a href=\"".$_SESSION['retour_apres_maj_sconet']."\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
}
else {
...
if(!isset($step)) {
	...
	echo "<p>This page is intended to carry out the importation of the student and
...
operation of Gepi , particularly in the pages comprising of many fields of form.<br />That can prevent the correct operation of the Update according to
Sconet.</p>\n";
	}
	...
	if ($gepiSettings['unzipped_max_filesize']>=0) {
		echo "<p style=\"font-size:small; color: red;\"><i>REMARQUE&nbsp;:</i> You can provide to Gepi the compressed file resulting directly from SCONET. (Ex : ElevesAvecAdresses.zip)</p>";
	}
	...
	if(mysql_num_rows($test_comptes_ele)==0) {
		...
		echo "/>\n";
		echo "<label for='alert_diff_mail_ele_y' style='cursor: pointer;'> Announce";
		...
		echo "/>\n";
		echo "<label for='alert_diff_mail_ele_n' style='cursor: pointer;'> not to announce";
	...
	echo "<input type='checkbox' name='stop' id='id_form_stop' value='y' /><label for='id_form_stop' style='cursor: pointer;'> disable the automatic mode.</label>\n";
	//==============================
...
to the importation responsible information.<br />\n";
	...
	echo "<a href='".$destination_formulaire."?is_posted=y&amp;step=9'>Pass nevertheless in the page of importation the responsible ones</a></p>\n";
...
modifications.</li>
		<li>To proceed to the update two files will be requis&nbsp;:<br />
		Files '<b>ElevesSansAdresses.xml</b>' and '<b>ResponsablesAvecAdresses.xml</b>' must being recovered since the application Web Sconet.<br />
		Nicely ask your secretary to go in 'Sconet/Access Bases student mode normal/Exploitation/Standard exports/Generic Exports XML' to recover the files '<b>ElevesSansAdresses.xml</b>' and '<b>ResponsablesAvecAdresses.xml</b>'.</li>
	</ul>\n";
	...
	if($step>0){
		echo " | <a href=\"".$_SERVER['PHP_SELF']."\">Update Sconet</a>";
	}
	echo "</p>\n";
			...
			if(!is_uploaded_file($xml_file['tmp_name'])) {
				...
				*/
				...
				if(file_exists($dest_file)) {
					...
					else {echo "<span style='color:red'>FAILURE</span>";}
					echo "</p>\n";
				}
...
file <b>temp/$tempdir</b></p>\n";
	...
	setTimeout(\"test_stop('0b')\",3000);
					...
					echo "<p align='center'><a href='".$_SERVER['PHP_SELF']."?step=0b&amp;stop=y' onClick=\"test_stop_suite('0b'); return false;\">Continuation</a></p>\n";
	...
	setTimeout(\"test_stop('0b')\",3000);
		...
		case "0b":
			...
			echo "<p>\n";
			echo "Analyze section STRUCTURES to preserve only the identifiers of pupils
affected in a class...<br />\n";
	...
	setTimeout(\"test_stop('1')\",3000);
			...
			echo "<p>$stat associations identifying student/class were inserted in the table 'temp_gep_import2'.</p>\n";
	...
	setTimeout(\"test_stop('$suite')\",1000);
		...
		case "2":
			...
			*/
				...
				$nom_racine=$ele_xml->getName();
				if(strtoupper($nom_racine)!='BEE_ELEVES') {
					echo "<p style='color:red;'>ERROR: The file XML provided does not have the air to be a file XML student.<br />Its root should be 'BEE_ELEVES'.</p>\n";
					require("../lib/footer.inc.php");
					die();
				}
	...
	setTimeout(\"test_stop('$suite')\",3000);
		...
		case "2b":
			...
			if(mysql_num_rows($res)==0) {
				echo "<p>No student left the establishment.</p>\n";

				echo "<input type='hidden' name='step' value='3' />\n";
				echo "<p><input type='submit' value='Pass to the continuation' /></p>\n";
				echo "</form>\n";
			}
			else {
...
désinscrits classes and lesson over the future periods. One search below the periods over which the student do not have a note nor anything on the bulletin.</p>\n";
	
				echo "<p>tick the periods for which you wish unsubcrible  the student who left
the establishment and validate at the foot of the page to pass to the
continuation.</p>\n";
	
				echo "<p>";
				echo "<a href=\"javascript:modifcase('coche')\">";
				echo "Tshoot all the student who it is possible of unsubcrible</a>";
				echo " / ";
	...
	function modifcase(mode){
		for(i=0;i<$cpt;i++){
			if(document.getElementById('desinscription_'+i)){
				...
				echo "<blockquote>\n";
				echo "<p>The student noted in Sconet as having left the establishment can be
unsubcrible classes and lesson over the future periods.<br />One search above the periods over which the student do not have a note
nor anything on the bulletin.</p>\n";
		...
		case "2c":
			...
			if(!isset($_POST['desinscription'])) {
				echo "<p>No unsubcrible was validated.</p>\n";
			}
			else {
				...
				for($i=0;$i<count($desinscription);$i++) {
					...
					echo "unsubcrible classes and lesson of ".strtoupper($lig_ele->nom)." ".ucfirst(strtolower($lig_ele->prenom))." for the period $periode: ";
					...
					if(!mysql_query($sql)) {
						...
						info_debug($sql);
						if(!mysql_query($sql)) {
							echo "<span style='color:red;'>ERROR at the time of the unsubcrible of the class</span>";
						}
						else {
		...
		case "3":
			...
			if(file_exists("../temp/".$tempdir."/eleves.xml")) {
				...
				if(unlink("../temp/".$tempdir."/eleves.xml")){
					...
					echo "<font color='red'>Echec!</font> Check the rights of writing on the waiter.<br />\n";
				}
			...
			if(!isset($parcours_diff)){
				...
				if($nb_eleves==0){
					echo "<p>The table 'temp_gep_import2' is empty???</p>\n";
					require("../lib/footer.inc.php");
					die();
				}

				echo "<p>Les ".$nb_eleves." student will be traversed by sections of 20 in the search of differences.</p>\n";
			...
			*/
			...
			if(!isset($parcours_diff)){
				...
				elseif($cpt>1){
					echo "<p>the ELE_ID of $cpt new student were found: $chaine_nouveaux</p>\n";
				}
	...
	*/
	...
	setTimeout(\"test_stop2()\",3000);
		...
		case "4":
			...
			if(!isset($tab_ele_id_diff)){
				echo "<p>No difference was found.</p>\n";
				...
				*/
				...
				$texte_infobulle="The address mall will not be modified, because your parameter setting of the addresses student is &nbsp;: <b>".getSettingValue('mode_email_ele')."</b>";
								...
								if($alert_diff_mail_ele=="y") {
									...
									if(isset($tmp_email_utilisateur_eleve)) {
										...
										if(($tmp_email_utilisateur_eleve!=$affiche[12])&&($alert_diff_mail_ele=='y')) {
											...
											$info_action_titre="Address mail not synchro for ".remplace_accents(stripslashes($lig_ele->nom)."_".stripslashes($lig_ele->prenom));
...
// RENSEIGNER UNE TABLE AVEC L'INDICATION QU'IL Y AURA UNE MODIF A FAIRE...
									...
									$info_action_texte="To carry out it <a href='classes/classes_const.php?id_classe=$lig_clas1->id&amp;msg=".rawurlencode("Change of class of ".remplace_accents(stripslashes($lig_ele->nom)."_".stripslashes($lig_ele->prenom))." was announced at the time of the update Sconet of $lig_clas1->classe vers $affiche[9].")."'>change of class</a> of $lig_clas1->classe vers $affiche[9]";
									$info_action_destinataire="administrator";
				...
				if($cpt_chgt_classe>0) {
					echo "<p><span style='font-weight:bold; color:red;'>Caution</span>&nbsp;: A change of class at least was located.<br />To carry out the change of class, click on the corresponding red cell.</p>\n";
				}
		...
		case "5":
							...
							if($auth_sso=='lcs') {
								...
								if ($result) {
									...
									if($info[0]["uid"]["count"]==0) {
										echo "<span style='color:red;'>No recording was found in the LDAP for the student ".$lig->ELENOM." ".$lig->ELEPRE."</span><br />\n";
										$erreur++;
									}
									if($info[0]["uid"]["count"]>1) {
										echo "<span style='color:red;'>Several recordings were found in the LDAP for the student ".$lig->ELENOM." ".$lig->ELEPRE." with the employee number '$lig->ELENOET'.<br />It is an anomaly.</span><br />\n";
										$erreur++;
									}
									...
									echo "<p>Failure of research in the LDAP of l'ELENOET for $lig->ELENOET ($lig->ELENOM $lig->ELEPRE).</p>";
								}
							}
							else {
			...
			if($cpt==0){
				...
				switch($erreur){
					...
					default:
						echo "<p><font color='red'>$erreur errors occurred.</font><br />\nYou should seek the cause of it before passing at the stage d'<a href='".$_SERVER['PHP_SELF']."?step=9&amp;stop=$stop'>importation/update of the people (<i>responsables</i>) and addresses</a>.</p>\n";
						break;
				}
			}
			else{
				switch($erreur){
					case 0:
						echo "<p>To pass at the stage of<a href='".$_SERVER['PHP_SELF']."?step=6&amp;stop=$stop'>assignment of the new student in their classes</a>.</p>\n";
						break;

					case 1:
						echo "<p><font color='red'>An error occurred.</font><br />\nYou should seek the cause of it before passing at the stage d'<a href='".$_SERVER['PHP_SELF']."?step=6&amp;stop=$stop'>assignment of the new student in their classes</a>.</p>\n";
						break;
...
group $nom_groupe for the period $j ! Start by removing these data !<br />";
							} else {
			...
			if(mysql_num_rows($res_classe)==0){
				...
				if(mysql_num_rows($res_per)==0){
					echo "<p>The student $prenom_eleve $ele_nom_eleve would be in a class without period???</p>\n";
	...
	function modif_case(rang,type,statut){
		...
		case "9":
			...
			//echo "<input type='hidden' name='is_posted' value='yes' />\n";
			if ($gepiSettings['unzipped_max_filesize']>=0) {
				echo "<p style=\"font-size:small; color: red;\"><i>NOTICE&nbsp;:</i> You can provide to Gepi the compressed file resulting directly from SCONET. (Ex : ResponsablesAvecAdresses.zip)</p>";
			}
			...
			echo "<label for='ne_pas_proposer_resp_sans_eleve' style='cursor: pointer;'> Not to propose to add responsible the not associated ones with student.</label><br />(<i>such entries can remain in very great number in Sconet</i>)<br />\n";
			...
			if(mysql_num_rows($test_comptes_resp)==0) {
				...
				echo "/>\n";
				echo "<label for='alert_diff_mail_resp_n' style='cursor: pointer;'> not to announce";
				echo " differences of address Mail between Sconet and the account of user.</label><br />\n";
			}
...
passing at the following stage.</p>\n";
							...
							echo "<p>The importation of the people (responsible) in base GEPI was carried out successfully (<em>".$stat." recordings on the whole</em>).</p>\n";
			...
			*/
				...
				*/
				...
				foreach ($objet_resp->children() as $responsable_eleve) {
					...
					if($debug_import=='y') {
						echo "<pre style='color:green;'><b>Table \$responsables[$i]&nbsp;:</b>";
...
passing at the following stage .</p>\n";
					...
					echo "<p>The importation of the relations student/responsible in base GEPI was
	...
	setTimeout(\"test_stop('12')\",3000);
		...
		case "12":
			...
			*/
				...
				$nom_racine=$resp_xml->getName();
				if(strtoupper($nom_racine)!='BEE_RESPONSABLES') {
					echo "<p style='color:red;'>ERROR: The file XML provided does not have the air to be a file XML responsible.<br />Its root should be 'BEE_RESPONSABLES'.</p>\n";
					require("../lib/footer.inc.php");
					die();
				}
				...
				*/
				...
				if ($nb_err != 0) {
					echo "<p>During the recording of the data ADDRESSES of responsible, there was $nb_err errors. Test find the cause of the error and start again the procedure before
passing at the following stage.</p>\n";
				} else {
		...
		case "13":
			...
			if(file_exists("../temp/".$tempdir."/responsables.xml")) {
				...
				if(unlink("../temp/".$tempdir."/responsables.xml")){
					...
					echo "<font color='red'>Failure!</font> Check the rights of writing on the waiter.</p>\n";
				}
			}
			...
			if(!isset($parcours_diff)){
				...
				echo "<p>Les ".$nb_pers." responsible people will be traversed by sections of 20 in the search of
differences.</p>\n";
	...
	*/
	setTimeout(\"test_stop2()\",3000);
				...
				echo "<p>Search differences on the traversed section: ";
		...
		case "14":
			...
			if(mysql_num_rows($res1)==0) {
					...
					while($lig2=mysql_fetch_object($res2)) {
						...
						if(mysql_num_rows($test)>0){
							while($lig3=mysql_fetch_object($test)){
								...
								info_debug($sql);
								$insert=mysql_query($sql);
								info_debug("Modif addresses $lig2->adr_id for resp_pers.pers_id=$lig3->pers_id");
							}
						}
						else{
				...
				if($ne_pas_proposer_resp_sans_eleve=="si"){
					...
					echo "<p><input type='submit' value='Display the differences' /></p>\n";
				}
				else{
	...
	setTimeout(\"test_stop2()\",3000);
				...
				while($lig=mysql_fetch_object($res1)){
					...
					if(mysql_num_rows($test1)==0){
						...
						info_debug("Modif test adr_id=$lig->adr_id (durée: $diff_debug_time)");
						if(mysql_num_rows($test)>0){
							if($cpt>0){
								echo ", ";
							}
							...
							info_debug($sql);
							$update=mysql_query($sql);
							info_debug("Adress adr_id=$lig->adr_id unchanged.");
						}
					}
					flush();
				}
...
you want to remove them your base.</p>\n";
			...
			echo "<table class='boireaus' summary='Table of responsible disappeared from Sconet'>\n";
	...
	function modifcase(mode){
		...
		case "14c":
			...
			if(is_array($valid_pers_id)) {
				...
				for($i=0;$i<count($valid_pers_id);$i++) {
					...
					$res=mysql_query($sql);
					if(mysql_num_rows($res)==0) {
						echo "<p style='color:red;'>Responsible the n°".$valid_pers_id[$i]." do not exist.</p>\n";
					}
					else {
						...
						if(mysql_num_rows($test_utilisateur)>0) {
							...
							if($lig_u->statut=='responsable') {
								echo "Delection of the account of user associated with the person: ";
								...
								echo "<span style='color:red;'>ANOMALY</span>&nbsp;: Responsible the n°".$valid_pers_id[$i]." was associated the account of user '$lig_u->login' whose statute is '$lig_u->statut'.<br />You should seek how that could occur.";
							}
						}
						echo "<br />\n";
		...
		case "15":
			...
			echo "<p>Les ".mysql_num_rows($test)." people will be controlled to make sure that they are well associated
to student.</p>\n";
		...
		case "16":
			...
			if(!isset($parcours_diff)) {
				...
				elseif($nb_tmp_modif==1){
					echo "<p>Person/address modified requires your attention.</p>\n";
				}
				else{
					echo "<p>$nb_tmp_modif people/modified addresses require your attention.</p>\n";
				}
				...
				info_debug("========================");
				info_debug("Recording of the validations/refusal of the section...");
			...
			$titre_infobulle="Address mall not updated";
			$texte_infobulle="The address mall will not be modified, because your parameter setting of the responsible addresses is &nbsp;: <b>".getSettingValue('mode_email_resp')."</b>";
			$tabdiv_infobulle[]=creer_div_infobulle('chgt_email_non_pris_en_compte',$titre_infobulle,"",$texte_infobulle,"",18,0,'y','y','n','n');
			...
			if(mysql_num_rows($res1)>0){
				...
				*/
				...
				$ligne_entete_tableau.="<td style='text-align:center; font-weight:bold; background-color: rgb(150, 200, 240);'>First name</td>\n";
				$ligne_entete_tableau.="<td style='text-align:center; font-weight:bold; background-color: rgb(150, 200, 240);'>Civility</td>\n";
				$ligne_entete_tableau.="<td style='text-align:center; font-weight:bold; background-color: rgb(150, 200, 240);'>phone / mel</td>\n";
				...
				// - si oui on indique le changement en piochant la nouvelle adresse dans temp_resp_adr_import2
				// - sinon on indique 'Identifiant d adresse inchangé'
				$ligne_entete_tableau.="<td style='text-align:center; font-weight:bold; background-color: #FAFABE;'>Adress</td>\n";
				$ligne_entete_tableau.="</tr>\n";
											...
											if($login_resp1!='') {
												...
												if(mysql_num_rows($res_email_resp)>0) {
													...
													if($lig_email_resp->email=='') {
														...
														$info_action_texte="You should update Sconet for <a href='responsables/modify_resp.php?pers_id=$lig_pers2->pers_id'>".remplace_accents(stripslashes($lig_pers2->nom)."_".stripslashes($lig_pers2->prenom))."</a><br />Address email informed by the person via 'Manage my account' is empty contrary to the address recorded in Sconet ($lig_pers2->mel).<br />You can also carry out <a href='responsables/synchro_mail.php'>synchronization overall</a>.";
														$info_action_destinataire=array("administrateur","scolarite");
														$info_action_mode="statut";
														enregistre_infos_actions($info_action_titre,$info_action_texte,$info_action_destinataire,$info_action_mode);
													}
														...
														if($lig_email_resp->email!=$lig_pers2->mel) {
															...
															$info_action_texte="You should update Sconet for <a href='responsables/modify_resp.php?pers_id=$lig_pers2->pers_id'>".remplace_accents(stripslashes($lig_pers2->nom)."_".stripslashes($lig_pers2->prenom),'all')."</a><br />Address email informed by the person via 'Manage my account' ($lig_email_resp->email) differ from the address recorded in Sconet ($lig_pers2->mel).<br />You can also carry out <a href='responsables/synchro_mail.php'>synchronization overall</a>.";
															$info_action_destinataire=array("administrateur","scolarite");
															$info_action_mode="statut";
															enregistre_infos_actions($info_action_titre,$info_action_texte,$info_action_destinataire,$info_action_mode);
														}
															...
															$info_action_titre="Address mall not synchro for ".remplace_accents(stripslashes($lig_pers2->nom)."_".stripslashes($lig_pers2->prenom));
															$info_action_texte="You should update Sconet for <a href='responsables/modify_resp.php?pers_id=$lig_pers2->pers_id'>".remplace_accents(stripslashes($lig_pers2->nom)."_".stripslashes($lig_pers2->prenom))."</a><br />Address email informed by the person via 'Manage my compte' ($lig_email_resp->email) differ from the address recorded in Sconet ($lig_pers2->mel).<br />You can also carry out <a href='responsables/synchro_mail.php'>synchronization overall</a>.";
															$info_action_destinataire=array("administrateur","scolarite");
															$info_action_mode="statut";
															enregistre_infos_actions($info_action_titre,$info_action_texte,$info_action_destinataire,$info_action_mode);
														}
...
<tr>
	...
	<td>$infos_adresse</td>
						...
						if(($adr1_2!="")||($adr2_2!="")||($adr3_2!="")||($adr4_2!="")||($cp2!="")||($commune2!="")||($pays2!="")){
							...
							$ligne_parent.="<span color='red'>Address vacuum</span>\n";
						}
					}
					$ligne_parent.="</td>\n";
				...
				if($liste_resp_sans_eleve!=""){
					echo "<p>One or of the people seem news, but are associated to no student (<i>nor in the current table 'responsables2', nor in the temporary table 'temp_responsables2_import'</i>).<br />To tick only responsible the really associated ones with student, click here: <a href=\"javascript:modifcase2()\"><img src='../images/enabled.png' width='15' height='15' alt='Any coachman intelligently' /></a></p>\n";
				}

				if($nb_chgt_adresse_inapproprie_non_affiche==1) {
					echo "<p>$nb_chgt_adresse_inapproprie_non_affiche nobody with this stage was proposed for a Re-doublonnage of address.</p>\n";
				}
				elseif($nb_chgt_adresse_inapproprie_non_affiche>1) {
					echo "<p>$nb_chgt_adresse_inapproprie_non_affiche people with this stage were not proposed for a Re-doublonnage of
address.</p>\n";
				}
...
";
		...
		case "17":
			...
			if(mysql_num_rows($res1)==0){
						...
						if(mysql_num_rows($test)==0){
							...
							if((getSettingValue('mode_email_resp')=='')||(getSettingValue('mode_email_resp')=='sconet')) {
								...
								if(mysql_num_rows($res_email_resp)>0) {
									...
									info_debug("There is an email in the table users: $lig_email_resp->email and the mall in XML est $lig->mel (nothing is done)");
								}
								// Si le responsable n'a pas de compte
								else {
									// Alors on fait la mise à jour
									$sql.="						mel='".addslashes($lig->mel)."',";
									info_debug("There is no email in the table users; one updates according to XML: $lig->mel");
								}
							}
				...
				echo "<p><b>Indication:</b> in <span style='color:blue;'>blue</span>, people added and in <span style='color:darkgreen;'>green</span> updated people/adresses.<br />Les <span style='color:red;'>(*)</span> possibly present announce a concern concerning the address.</p>\n";
...
phase.";
						echo "<br />\n";
						echo "Here is couples ELE_ID/PERS_ID: ";
					}
					else{
						echo ", ";
					}
...
phase.";
							echo "<br />\n";
							echo "Here are the couples ELE_ID/PERS_ID: ";
						}
						else{
							echo ", ";
						}
	...
	*/
	setTimeout(\"test_stop2()\",3000);
				...
				echo "<p>The course of the differences concerning associations student/responsible is finished.<br />You will be able to control the differences.</p>\n";
				...
				echo "<p>To traverse the differences by sections of <input type='text' name='eff_tranche' id='eff_tranche' value='".min(20,$nb_associations_a_consulter)."' size='3' onkeydown=\"clavier_2(this.id,event,0,200);\" autocomplete='off' /> on a total of $nb_associations_a_consulter.<br />\n";

				echo "Not to propose to remove responsible the not associated ones with
		...
		case "19":
			...
			if((!isset($parcours_diff))||(!isset($_POST['temoin_phase_19']))) {
				...
				elseif($nb_associations_a_consulter==1){
					echo "<p>".$nb_associations_a_consulter." association ELE_ID/PERS_ID your attention requires.</p>\n";
				}
				else{
			...
			if(mysql_num_rows($res0)>0){
				...
				echo "<td style='text-align:center; font-weight:bold; background-color: red;' rowspan='2'>Suppression<br />the responsible one</td>\n";
				//=========================
					...
					if(mysql_num_rows($res1)==0){
						...
						*/
						...
						if(mysql_num_rows($res2)==0){
							...
							$ligne_courante.="<td class='nouveau'>New</td>\n";
	...
	function modifcase(mode){
		for(i=0;i<$cpt;i++){
			if(document.getElementById('check_'+i)){
				...
				echo "<p><i>NOTES:</i></p>\n";
				echo "<ul>\n";
				echo "<li>box of suppression of responsible has is not proposed that if it is
associated to no student indeed present in your table 'eleves'.</li>\n";
				echo "<li>The message 'No student for ele_id=...' mean that the importation refers to an identifier of student who is not
any more in the establishment or which was proposed with the
importation of the student and that you did not shoot.<br />That does not mean that the responsible one is not associated other
raises which is quite present to him in your table 'eleves'.<br />Nothing is inserted in the table 'responsables2' for these lines.</li>\n";
				echo "</ul>\n";
	...
	function modifcase(mode){
		...
		case "21":
			...
			if(isset($suppr_resp_ele)) {
				...
				echo "<p>$nb_suppr suppression of responsibility.<br />$nb_err erreur(s).</p>\n";
			}
