...
if($_SESSION['statut']=='professeur') {
	...
	$verif_eleve = mysql_query($sql)
			or die('Error of verif_var1 : '.mysql_error());
	...
	if($mode!="verif") {
		...
		if (mysql_num_rows($verif_prof) >= 1) {
			...
			log_ajax_app("You cannot seize appreciations for this student ");
			die('You cannot seize appreciations for this student');
		}
	}
	else {
		$temoin_prof=1;
...
if (($_SESSION['statut']=='scolarite') || ($_SESSION['statut']=='cpe') || (($temoin_eleve !== 0 AND $temoin_prof !== 0))) {
	if($mode=='verif') {
		...
		if(!$create_table) {
			echo "<span style='color:red'>Error during the creation of the table ' vocabulaire'.</span>";
		}
		else {
				...
				*/
				...
				for($loop=0;$loop<count($tab_voc);$loop++) {
					if(preg_match("/ ".$tab_voc[$loop]." /i",$appreciation_test)) {
						if($chaine_retour=="") {$chaine_retour.="<span style='font-weight:bold'>Suspicion of typing error &nbsp;: </span>";}
