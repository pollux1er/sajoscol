...
function get_classe_from_id($id){
	...
	if(mysql_num_rows($resultat_classe)!=1){
		...
		echo "<p>ERROR! The class of identifier '$id' could not be identified.</p>";
	}
	else{
...
<p class="bold"><a href="../accueil.php">Return</a>
 | <a href="commentaires_types.php">Seize comments</a>
 | <a href="commentaires_types.php?recopie=oui">Recopy comments</a>
</p>
...
<?php
	...
	if($recopie!="oui"){
		...
		if(!isset($_POST['id_classe'])){
			...
			echo "<p>For which class wish you to define/to modify the comment-type?</p>\n";
			echo "<blockquote>\n";
			...
			*/
			...
			if(!isset($_POST['num_periode'])){
				...
				echo "<h2>Saisy/Modification of the comment-types for the class of ".get_classe_from_id($id_classe)."</h2>\n";
				...
				if(mysql_num_rows($resultat_num_periode)==0){
					echo "No period is yet defined for this class...<br />\n";
					echo "</body>\n</html>\n";
					exit();
				}
				else{
					echo "<p>Here the comment-type currently seized for this class:</p>\n";
					echo "<ul>\n";
					...
					//for($i=0;$i<count($num_periode);$i++){
						...
						if(mysql_num_rows($resultat_commentaires)>0){
							...
							echo "<p style='color:red;'>No comment-type is seized for this class over this period.</p>\n";
						}
						echo "</li>\n";
					}
					echo "</ul>\n";
				...
				echo "<p>For which periods wish you to define/to modify the comment-type?</p>\n";
				//echo "<p>\n";
				...
				if(mysql_num_rows($resultat_classe)==0){
					echo "<p>The identifier of the class seems erroneous.</p>\n</form>\n</body>\n</html>\n";
					exit();
				}
				$ligne_classe=mysql_fetch_object($resultat_classe);
				...
				*/
				...
				echo "<h2>Classify $classe_courante</h2>\n";
				...
				$cpt=1;
				if(mysql_num_rows($resultat_commentaires)!=0){
					echo "<p>Here the list of the comment-type existing for the class and the periods chosen :</p>\n";
					echo "<blockquote>\n";
	...
	document.getElementById('no_anti_inject_commentaire_".$cpt."').focus();
		...
		if(!isset($_POST['id_classe'])){
			...
			echo "<p>Of which class wish you to recopy the comment-type?</p>\n";
			echo "<blockquote>\n";
			$sql="select distinct id,classe from classes order by classe";
			$resultat_classes=mysql_query($sql);
			if(mysql_num_rows($resultat_classes)==0){
			...
			echo "<h2>Recopy comment-types</h2>\n";
...
*/
			...
			echo "<p><b>Classify model:</b> $classe_source</p>\n";
			...
			if(!isset($_POST['num_periode'])){
				...
				if(mysql_num_rows($resultat_num_periode)==0){
					...
					echo "<p>Here the comment-type seized for this class:</p>\n";
					echo "<ul>\n";
					while($ligne_periode=mysql_fetch_object($resultat_num_periode)){
					//for($i=0;$i<count($num_periode);$i++){
						echo "<li>\n";
				...
				echo "<p>For which periods wish you to recopy the comment-type?</p>\n";
				//echo "<p>\n";
				//echo "<input type='hidden' name='id_classe' value='$id_classe'>\n";
				...
				$resultat_num_periode=mysql_query($sql);
				if(mysql_num_rows($resultat_num_periode)==0){
					echo "<p>No period is yet defined for this class...</p>\n";
				}
				else{
					/*
					$ligne_num_periode=mysql_fetch_object($resultat_num_periode);
...
*/
				...
				if(!isset($_POST['id_dest_classe'])){
					...
					echo "<p>Here the comment-type seized for this class and selected periods:</p>\n";
					echo "<ul>\n";
					for($i=0;$i<count($num_periode);$i++){
						echo "<li>\n";
						$sql="select nom_periode from periodes where num_periode='$num_periode[$i]'";
					...
					echo "<p>For which classes wish you to remove the existing comment-type and to
replace them by those of $classe_source?</p>\n";
...
*/
					...
					for($i=0;$i<count($num_periode);$i++){
						...
						$resultat_commentaires_source=mysql_query($sql);
						if(mysql_num_rows($resultat_commentaires_source)==0){
							echo "<p>It is malignant... there is not comment-type for the model class and the selected period.<br />\n Moreover, the existing comments for the classes destination were
removed...</p>\n";
						}
						else{
							while($ligne_commentaires_source=mysql_fetch_object($resultat_commentaires_source)){
