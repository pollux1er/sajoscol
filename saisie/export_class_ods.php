...
if(getSettingValue("export_cn_ods")!='y') {
    die("Export ODS is not activated.");
}
...
if(isset($nettoyage)){
	if(!my_ereg(".ods$",$nettoyage)){
		$msg="The file is not extension ODS.";
	}
	elseif(!my_ereg("^".$_SESSION['login'],$nettoyage)){
		...
		if(strlen(my_ereg_replace("[a-zA-Z0-9_.]","",strtr($nettoyage,"-","_")))!=0){
			$msg="The file proposed is not valid: '".my_ereg_replace("[a-zA-Z0-9_.]","",strtr($nettoyage,"-","_"))."'";
		}
		else{
...
if((strlen(my_ereg_replace("[0-9]","",$id_groupe))!=0)||(strlen(my_ereg_replace("[0-9]","",$periode_num))!=0)){
	$msg="At least of the values id_groupe or periode_num est invalide.";
    header("Location: index.php?msg=$msg");
    die();
}
...
if($_SESSION['statut']!='secours') {
	...
	if (mysql_num_rows($res_test)==0) {
		$mess=rawurlencode("You try to reach data which do not belong to you !");
...
//**************** EN-TETE *****************
$titre_page = "Export of the notes/appreciations";
...
echo "<a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return reception </a>|\n";
...
echo "<p>To download: <a href='$chemin_temp/$nom_fic'>$nom_fic</a></p>\n";
...
echo "<p>Unnecessarily not to encumber the waiter and by preoccupations with a
confidentiality, it is recommended to remove the file of the waiter
after recovery of the file above.<br />\n";
echo "<a href='".$_SERVER['PHP_SELF']."?nettoyage=$nom_fic'>Remove the file</a>.";
