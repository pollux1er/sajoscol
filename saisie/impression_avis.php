...
//**************** EN-TETE *****************
$titre_page = "Impression of the opinions of the staff meeting";
...
echo " | <a href='../impression/parametres_impression_pdf_avis.php'>Regulate the parameters of the pdf</a>";
echo "</p>\n";
...
if ($_SESSION['statut'] == 'scolarite') { // Scolarite
	...
	if (($id_liste_periodes)!=0) {	
	   ...
	   $periode = "Periods N° ".$chaine_periodes;
		
		echo "<h3>".$periode;
		echo "</h3>\n";
	} else {
		$periode="";
		echo "<h3>Choice of the period : ";
		echo "</h3>\n";
	}
	...
	if ($id_liste_periodes == 0) {
		echo "<legend>Initially, select the periods for which you wish to print the opinions</legend>\n";
		...
		While ($data_periode = mysql_fetch_array ($resultat_periode)) {
			...
			echo "\">";
			echo "Period N°".$data_periode['num_periode'];
			echo "</option>\n";
		}
		echo "		</optgroup>\n";
		echo "	</select>\n";	
		echo "<br />Note: if you select several periods, the data will be sorted by pupil
and period for each class";		
		echo "<br /><br /> <input value=\"Validate the period\" name=\"Valider\" type=\"submit\" />\n<br />\n";			
		echo "</form>\n";
	} else {
		echo "<legend>Then, for the periods chosen, séléctionnez to it (or them) class for which
you wish to print the opinions.</legend>\n";
	...
	echo "<h3>List classes : </h3>\n"; //modele imprime PDF
	
	// Pourle compte scolarite, possibilité d'imprimer les avis en synthèse (pour ses classes)
	echo "<p>Select the class and the period for which you wish to print
the opinions :</p>\n";
	...
	if(mysql_num_rows($result_classes)==0){
		echo "<p>It seems that no class was still created.</p>\n";
	}
	else {
		$nb_classes=mysql_num_rows($result_classes);
...
// Module toutes les classes scolarité
    ...
    if ($nombre_classe == "0") {
        echo "You are not ".getSettingValue("gepi_prof_suivi")." ! It thus does not return to you to print the opinions of staff meeting.";
    } else {
        $j = "0";
