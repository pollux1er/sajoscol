...
//**************** EN-TETE *****************
$titre_page = "Seizure of the averages and appreciations";
require_once("../lib/header.inc");
//**************** FIN EN-TETE *****************
?>

<p class='bold'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Home</a>
<?php

if($_SESSION['statut']=='secours') {
	echo " | <a href='saisie_secours_eleve.php'>Choice of a student</a>";
}
...
if ($current_group) {
    ...
    echo " | <a href='index.php'>My lesson</a></p>\n";
    //echo "<p class='grand'> Groupe : " . $current_group["description"] . " ($classes) | Matière : $matiere_nom</p>";
    echo "<p class='grand'> Group : " . htmlentities($current_group["description"]) . " ($classes) | Matière : ".htmlentities($matiere_nom)."</p>\n";
	...
	*/
    ...
    echo "<p class='bold'>Manual seizure :</p>\n";
	echo "<blockquote>\n";
	//echo "<ul>\n";
    //echo "<li>\n";
		$i=1;
		...
		while ($i < $nb_periode) {
			...
			if (($current_group["classe"]["ver_periode"]["all"][$i] >= 2)||
				...
				$tabdiv_infobulle[]=creer_div_infobulle("info_periode_$i","","","<center>Seize the averages</center>","",10,0,"n","n","y","n");
				...
				$tabdiv_infobulle[]=creer_div_infobulle("info_periode_$i","","","<center>Consult the averages</center>","",12,0,"n","n","y","n");

				echo "<a href='saisie_notes.php?id_groupe=$id_groupe&amp;periode_cn=$i' onmouseover=\"afficher_div('info_periode_$i','y',10,10)\" onmouseout=\"cacher_div('info_periode_$i')\"><img src='../images/icons/chercher.png' width='32' height='32' alt='Consultation of the averages' ";
		...
		$tabdiv_infobulle[]=creer_div_infobulle("info_visu","","","<center>Visualization of the averages and appreciations</center>","",12,0,"n","n","y","n");

		echo "<td rowspan='2'><a href='../prepa_conseil/index1.php?id_groupe=$id_groupe' onmouseover=\"afficher_div('info_visu','y',10,10)\" onmouseout=\"cacher_div('info_visu')\"><img src='../images/icons/print.png' width='32' height='32' alt='Visualization of the averages and appreciations' /></a></td>\n";
		echo "</tr>\n";
		...
		*/
		...
		while ($i < $nb_periode) {
			...
			if (($current_group["classe"]["ver_periode"]["all"][$i] >= 2)||
				...
				$tabdiv_infobulle[]=creer_div_infobulle("info_app_periode_$i","","","<center>Seize the appreciations</center>","",12,0,"n","n","y","n");
				...
				echo "<img src='../images/disabled.png' width='20' height='20' alt='Period close' ";
		...
		echo "<tr class='lig-1'>\n";
		echo "<th>Importation of a file CSV</th>\n";
		$i=1;
		while ($i < $nb_periode) {
			echo "<td>\n";
			//if ($current_group["classe"]["ver_periode"]["all"][$i] >= 2) {
			if (($current_group["classe"]["ver_periode"]["all"][$i] >= 2)||
				((($current_group["classe"]["ver_periode"]["all"][$i]!=0))&&($_SESSION['statut']=='secours'))) {
				$tabdiv_infobulle[]=creer_div_infobulle("info_import_csv_periode_$i","","","<center>Import CSV<br />(<i>the empty fields are not imported</i>)</center>","",15,0,"n","n","y","n");
	...
	*/
	...
	echo "<p class='bold'>Preparation of the importation of a file of averages/appreciations :</p>\n";
	echo "<blockquote>\n";

	echo "<table class='boireaus' border='1' summary='Export'>\n";
	echo "<tr>\n";
	echo "<th>Export/Period</th>\n";
	$i=1;
	while ($i < $nb_periode) {
		//echo "<th>Période $i</th>\n";
		echo "<th>".$current_group["periodes"][$i]["nom_periode"]."</th>\n";
	...
	while ($i < $nb_periode) {
		...
		if (($current_group["classe"]["ver_periode"]["all"][$i] >= 2)||
			((($current_group["classe"]["ver_periode"]["all"][$i]!=0))&&($_SESSION['statut']=='secours'))) {
			$tabdiv_infobulle[]=creer_div_infobulle("info_export_csv_periode_$i","","","<center>Export CSV of identifiers GEPI, with the Average columns and Appreciations of this class, with line of heading.</center>","",15,0,"n","n","y","n");
		...
		if($temoin_ods=="y"){
			...
			while ($i < $nb_periode) {
				...
				if (($current_group["classe"]["ver_periode"]["all"][$i] >= 2)||
					...
					$tabdiv_infobulle[]=creer_div_infobulle("info_export_ods_periode_$i","","","<center>Export spreadsheet OpenOffice.org (<i>ODS</i>) identifiers GEPI, with the Average columns and Appreciations of this class, with line of heading.</center>","",15,0,"n","n","y","n");
			...
			echo "<td colspan='$nb_periode'><font color='red'>Export spreadsheet ODS is not possible.</font></td>\n";
		}
		echo "</tr>\n";
	}
	echo "</table>\n";
...
/*
	...
	*/
    ...
    if ($_SESSION["statut"] == "professeur") {
        echo "<p>Seize the averages or appreciations by class :</p>\n";
        ...
        while($i < $lignes){
			...
			echo "<a href='recopie_moyennes.php?id_classe=$id_classe&amp;retour=saisie_index'><b>Recopy averages</b></a> - ";
