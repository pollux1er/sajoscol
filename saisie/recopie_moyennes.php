...
if(isset($_POST['recopier'])) {
	...
	if(mysql_num_rows($test)>0) {
		$msg="The period n°$num_periode is closed.";
	}
	else {
		$login=$_POST['login'];
		...
		if($nberr>0){
			$msg="$nberr errors took place.<br />";
		}
		elseif($nbsucces>0){
			$msg="$nbsucces recordings carried out.<br />";
		}
	}
}
...
//**************** EN-TETE *****************
$titre_page = "Report card - Recopy averages";
...
if(!isset($id_classe)){
	...
	echo "<div class='norme'><p class=bold><a href='../accueil.php'>Home</a>";
	...
	if(isset($_SESSION['retour'])){
		echo " | <a href='".$_SESSION['retour']."'>Return</a>";
	}

	echo "</p></div>\n";

	//echo "<form enctype=\"multipart/form-data\" name= \"formulaire\" action=\"".$_SERVER['PHP_SELF']."\" method='post'>\n";
	echo "<p>This page is intended to carry out the recopy of the averages of the
report cards towards the bulletins.<br />Only the averages other than will be recopied 'abs', 'disp' and '-'.</p>\n";
	$sql="SELECT DISTINCT id,classe FROM classes ORDER BY classe";
	$res_classe=mysql_query($sql);

	if(mysql_num_rows($res_classe)==0){
		echo "<p>It seems that no class is defined.</p>\n";
		echo "<p><br /></p>\n";
		...
		*/
		...
		echo "<p><br /></p>\n";
		echo "<p class='bold'>Choose the class:</p>\n";
	...
	if($id_class_prec!=0) {
		...
		echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Classify preceding</a>";
	}
	...
	if($id_class_suiv!=0) {
		...
		echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Following class</a>";
	}
	...
	if(!isset($num_periode)){
		//echo "<form enctype=\"multipart/form-data\" name= \"formulaire\" action=\"".$_SERVER['PHP_SELF']."\" method='post'>\n";
		echo "<p>This page is intended to carry out the recopy of the averages of the
report cards towards the bulletins.</p>\n";
		...
		if(mysql_num_rows($res_classe)==0){
			echo "<p>It seems that the selected class does not exist.</p>\n";
			echo "<p><br /></p>\n";
			require("../lib/footer.inc.php");
			die();
		}
			...
			echo "<table summary='Choice of the period'><tr><td valign='top'><p class='bold'>Choose one period for the class of $tmp_classe[0]: </p></td><td><p>\n";
			...
			*/
			while($lig_per=mysql_fetch_object($res_per)){
				...
				if($lig_per->verouiller!='O'){
					...
					echo "$lig_per->nom_periode (<i>closed period</i>)<br />\n";
				}
			}
			echo "</p></td></tr></table>\n";
		...
		if(mysql_num_rows($res_per)==0){
			echo "<p>It seems that the selected period does not exist.</p>\n";
			echo "<p><br /></p>\n";
			require("../lib/footer.inc.php");
			die();
		}
		...
		if(mysql_num_rows($resultat)==0){
			echo "<p>It seems that no report card is yet defined.</p>\n";
			die("</body></html>");
		}
		else{
			echo "<p>Recopy averages of the report cards towards the bulletins.</p>\n";
			$nb_carnets_notes=mysql_num_rows($resultat);
			if($nb_carnets_notes==1){
				echo "<p>".$nb_carnets_notes." report card only is currently indicated.<br />It is only proposed in the recopy below.</p>\n";
			}
			else{
				echo "<p>".$nb_carnets_notes." report cards are currently indicated.<br />It is only proposed in the recopy below.</p>\n";
			}
			...
			echo "<p align='center'><input type=\"submit\" name='recopier' value=\"Validate the recopy\" /></p>\n";
			...
			echo "<p>Recopy averages for the class <b>$tmp_classe[0]</b> of over the period <b>$tmp_per[0]</b>:</p>\n";

			//echo "<table border='1'>\n";
			echo "<table class='boireaus' width='100%' summary='Comparaisons'>\n";
			echo "<tr>\n";
			echo "<th style='text-align:center; font-weight:bold;'>Class</th>\n";
			echo "<th style='text-align:center; font-weight:bold;'>Group</th>\n";
			echo "<th style='text-align:center; font-weight:bold;'>student</th>\n";
			echo "<th style='text-align:center; font-weight:bold;'>Average<br />notebook<br />notes</th>\n";
			echo "<th style='text-align:center; font-weight:bold;'>Average<br />on<br />bulletin</th>\n";
			echo "<th style='text-align:center; font-weight:bold;'>Recopy</th>\n";
			echo "<th style='text-align:center; font-weight:bold;'>\n";
			echo "<a href='javascript:modif_toutes_cases(true)'><img src='../images/enabled.png' width='15' height='15' alt='All tick' title='all tick' /></a>/\n";
			echo "<a href='javascript:modif_toutes_cases(false)'><img src='../images/disabled.png' width='15' height='15' alt='All shoot' title='All shoot' /></a>\n";
			echo "";
			echo "</th>\n";
			echo "</tr>\n";
			...
			while($ligne=mysql_fetch_object($resultat)){
				...
				*/
				...
				if(mysql_num_rows($res_ele)==0){
					echo "<td colspan='3'>No student in this class and this group!</td>\n";
					echo "</tr>\n";
					echo "</table>\n";
					...
					while($lig_ele=mysql_fetch_object($res_ele)){
						...
						*/
						...
						if($temoin_grp==0){
							echo "<td>";
							//Coche/décoche
							echo "<a href='javascript:modif_case($i,true)'><img src='../images/enabled.png' width='15' height='15' alt='tick all the groupe' title=' tick all the group' /></a>/\n";
							echo "<a href='javascript:modif_case($i,false)'><img src='../images/disabled.png' width='15' height='15' alt='shoot all the group' title='shoot all the group' /></a>\n";
							echo "</td>\n";
						}
						else{
							echo "<td>&nbsp;</td>\n";
			...
			echo "<p><i>NOTE:</i> The boxes in red highlight the not recopied averages.</p>\n";
