				...
				if (($k >= $display_begin) and ($k <= $display_end)) {
					...
					if ($ver_periode[$k] == "N"){
						...
						if($num_eleve!=-1){
							...
							if (!$register) {$msg = "Error during the recording of the data of the period $k ";} else {$msg = "The modifications were recorded !";$affiche_message = 'yes';}
						}
					}
				}
				$k++;
...
$themessage  = 'Notes or appreciations were modified. Want you to really leave without
recording ?';
//**************** EN-TETE *****************
$titre_page = "Seizure of the appreciations ".$nom_aid;
...
if (!isset($aid_id)) {
	...
	if ($_SESSION['statut'] != "secours") {
		...
		if ($nombre_aid == "0") {
			echo "<p>$nom_aid : You are not a responsible professor. You do not have thus to enter of appreciations.</p></html></body>\n";
			die();
		} else {
			$i = "0";
			...
			while ($i < $nombre_aid) {
				...
				echo "<br /><span class='bold'>".$aff_numero_aid.$aid_display."</span>
				 --- <a href='saisie_aid.php?aid_id=".$aid_id."&amp;indice_aid=".$indice_aid."'>To seize the appreciations for this heading</a>\n";
				$i++;
			}
			echo "</p>\n";
		}
		...
		$nombre_aid = mysql_num_rows($call_prof_aid);
		if ($nombre_aid == "0") {
			echo "<p>$nom_aid : There are no entries !</p>\n";
		} else {
			$i = "0";
			echo "<p><b>".$nom_aid." - Seizure of the appreciations :</b><br />\n";
			while ($i < $nombre_aid) {
				$aid_display = mysql_result($call_prof_aid, $i, "nom");
				$aid_id = mysql_result($call_prof_aid, $i, "id");
				$aid_numero = mysql_result($call_prof_aid, $i, "numero")." : ";
	...
	echo " | <a href='saisie_aid.php?indice_aid=$indice_aid' onclick=\"return confirm_abandon (this, change, '$themessage')\">Choice $nom_aid</a></p>\n";
	...
	echo "<p class='grand'>Appreciations $nom_aid : $aid_nom</p>\n";
	...
	while ($num < $nb_periode_max + 1) {
		...
		if ($nombre_lignes != '0') {
			echo "<tr>\n";
			echo "<th><b>Name First name</b></th>\n";
