<?php
/*
* $Id: saisie_appreciations.php 8415 2011-10-04 12:28:08Z crob $
*
...
if (is_numeric($id_groupe) && $id_groupe > 0) {
	...
	$mess=rawurlencode("Anomaly: You arrive on this page without teaching being selected ! If you had selected a teaching well, it may be that you have a module php type 'suhosin' who limits the number of variables which can be posted in a form.");
...
if ($_SESSION['statut'] != "secours") {
	if (!(check_prof_groupe($_SESSION['login'],$current_group["id"]))) {
		$mess=rawurlencode("You are not a professor of this teaching !");
		header("Location: index.php?msg=$mess");
		die();
	}
}
		...
		if(isset($_POST['app_grp_'.$k])){
			...
			if(($current_group["classe"]["ver_periode"]['all'][$k]>=2)||
				...
				if ($test != "0") {
					...
					if (!$register) {$msg = $msg."Error during the recording of the data of the period $$k>for the
group/class<br />";}
...
group/class<br />";}
				...
				$msg.="Anomaly: Attempt at seizure of an appreciation of class whereas the period is
not open in seizure.<br />";
			}
		}
		//=================================================
					...
					if (in_array($reg_eleve_login, $current_group["eleves"][$k]["list"])) {
						...
						if(($current_group["classe"]["ver_periode"][$eleve_id_classe][$k]=="N")||
							...
							if ($test != "0") {
								...
								if (!$register) {$msg = $msg."Error during the recording of the data of the period $k for the student $reg_eleve_login<br />";}
	...
	if($msg=='') {
		...
		OR die('Error in the obliteration of the temporary table (1) :'.mysql_error());
	}
...
elseif((isset($_POST['correction_login_eleve']))&&(isset($_POST['correction_periode']))&&(isset($_POST['no_anti_inject_correction_app_eleve']))) {
	...
	if($ver_periode[$correction_periode]=='P') {
		...
		if($saisie_valide!='y') {
			$msg.="ERROR: The seizure is not authorized.<br />";
		}
		else {
			...
			if (isset($NON_PROTECT["correction_app_eleve"])) {
				...
				(strlen(preg_replace('/[0-9]/','',$correction_periode))!=0)) {
					$msg.="Invalid characters are proposed for the login student $correction_nom_prenom_eleve or for the period $correction_periode.<br />";
				}
				else {
					...
					if(mysql_num_rows($test)==0) {
						$msg.="student $correction_nom_prenom_eleve is not associated the group n°$id_groupe for the period $correction_periode.<br />";
					}
					else {
						...
						if ($test!="0") {
							if ($app!="") {
								...
								if (!$register) {$msg = $msg."Error during the recording of the corrections for $correction_nom_prenom_eleve over the period $correction_periode.<br />";} 
								else {
									$msg.="Recording of the proposal for a correction for $correction_nom_prenom_eleve over the period $correction_periode carried out.<br />";
									$texte_mail.="A correction suggested was updated by ".casse_mot($_SESSION['prenom'],'majf2')." ".casse_mot($_SESSION['nom'],'maj')."\r\npour l'élève ".$correction_nom_prenom_eleve." over the period $correction_periode\r\nen ".$current_group['name']." (".$current_group["description"]." en ".$current_group["classlist_string"].").\n";
								}
							} else {
								...
								$register=mysql_query($sql);
								if (!$register) {$msg = $msg."Error during the suppression of the proposal for a correction for $correction_nom_prenom_eleve over the period $correction_periode.<br />";} 
								else {
									$msg.="Suppression of the proposal for a correction for $correction_nom_prenom_eleve over the period  $correction_periode carried out.<br />";
									$texte_mail.="Suppression of the proposal for a correction for the student $correction_nom_prenom_eleve\r\nsur the period $correction_periode en ".$current_group['name']." (".$current_group["description"]." en ".$current_group["classlist_string"].")\r\npar ".casse_mot($_SESSION['prenom'],'majf2')." ".casse_mot($_SESSION['nom'],'maj').".\n";
								}
							}
							...
							if ($app != "") {
								...
								$register=mysql_query($sql);
								if (!$register) {$msg = $msg."Error during the recording of the proposal for a correction for $correction_nom_prenom_eleve over the period $correction_periode.<br />";}
								else {
									$msg.="Recording of the proposal for a correction for $correction_nom_prenom_eleve over the period $correction_periode carried out.<br />";
									$texte_mail.="A correction was proposed by ".casse_mot($_SESSION['prenom'],'majf2')." ".casse_mot($_SESSION['nom'],'maj')."\r\nfor the student $correction_nom_prenom_eleve over the period $correction_periode\r\nen ".$current_group['name']." (".$current_group["description"]." in ".$current_group["classlist_string"].").\n";
								}
							}
						}
						...
						if($texte_mail!="") {
							...
							if($envoi_mail_actif=='y') {
								...
								if(mysql_num_rows($req)>0) {
									...
									$sujet_mail="[GEPI] Ask for validation of correction of appreciation";
									...
									$salutation=(date("H")>=18 OR date("H")<=5) ? "Good evening" : "Hello";
...
$themessage = 'Appreciations were modified. Do you want  to really leave without recording ?';
$message_enregistrement = "The modifications were recorded !";
$utilisation_prototype = "ok";
$javascript_specifique = "saisie/scripts/js_saisie";
//**************** EN-TETE *****************
$titre_page = "Seizure of the appreciations";
...
if (($periode_cn != 0)&&($_SESSION['statut']!='secours')) {
	...
	echo "<a href=\"../cahier_notes/index.php?id_groupe=$id_groupe&amp;periode_num=$periode_cn\" onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>\n";
} else {
	echo "<a href=\"index.php\" onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return reception seized</a>\n";
}
...
echo " | <a href='saisie_notes.php?id_groupe=$id_groupe&amp;periode_cn=$periode_cn' onclick=\"return confirm_abandon (this, change, '$themessage')\">Seize the averages</a>";
...
echo " | <a href='../prepa_conseil/index1.php?id_groupe=$id_groupe'>Print</a>\n";

//=========================
// AJOUT: boireaus 20071108
echo " | <a href='index.php?id_groupe=" . $current_group["id"] . "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Importation/Export notes and appreciations</a>";
//=========================
...
if(($_SESSION['statut']=='professeur')||($_SESSION['statut']=='secours')) {
	...
	if($login_prof_groupe_courant!='') {
		...
		$tab_groups = get_groups_for_prof($login_prof_groupe_courant,"classify then subject");
	...
	if(!empty($tab_groups)) {
		...
		if(isset($id_grp_prec)){
			if($id_grp_prec!=0){
				...
				echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Preceding teaching</a>";
			}
		}
	...
	function confirm_changement_classe(thechange, themessage)
	{
		...
		if(isset($id_grp_suiv)){
			if($id_grp_suiv!=0){
				...
				echo "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Following teaching</a>";
				}
		}
	}
	// =================================
...
if ($insert_mass_appreciation_type=="y") {
	...
	if($droit_insert_mass_appreciation_type=="y") {
		...
		echo "Insert the appreciation-type following for all the empty
appreciations: ";
...
echo "<h2 class='gepi'>Report card - Seizure of the appreciations</h2>\n";

echo "<p>You can reveal in your appreciation the list of the notes of the student
for the period while inserting the character strings <b>@@Notes</b><br />(<i>the notes will appear then during visualization/impression of the bulletin</i>)</p>\n";
...
if ($multiclasses) {
	echo "<p>Display :";
	echo "<br/>-> <a href='saisie_appreciations.php?id_groupe=$id_groupe&amp;order_by=classe'>To gather the student by class</a>";
	echo "<br/>-> <a href='saisie_appreciations.php?id_groupe=$id_groupe&amp;order_by=nom'>Display the list alphabetically</a>";
	echo "</p>\n";
}
...
function focus_suivant(num){
	temoin='';
	// The variable ' dernier' can exceed manpower of the class... but that is not dramatic
	dernier=num+".count($liste_eleves)."
	// One traverses the fields from that of the pupil in progress until
meeting an existing field
	// (to succeed in passing a pupil who would not be any more during the
time)
	// After validation, it is this field which will obtain the x-ray one if
one were not at the end of the list.
...
// ====================== Modif pour la sauvegarde en ajax =================
	...
	if ($test !== 0 AND $restauration == NULL) {
		...
		echo "
		<p class=\"red\">some appreciations were not recorded during your last seizure.<br />
			They are indicated below in red. Do you want to restore them ?
		</p>
		<p class=\"red\">
		<a href=\"./saisie_appreciations.php?id_groupe=".$current_group["id"]."&amp;restauration=oui".add_token_in_url()."\">YES</a>
		(they will replace the preceding seizure then)
			-
		<a href=\"./saisie_appreciations.php?id_groupe=".$current_group["id"]."&amp;restauration=non".add_token_in_url()."\">NOT</a>
		(they then will be definitively lost)
		</p>
		";
	}
...
echo "<table width=\"750\" class='boireaus' cellspacing=\"2\" cellpadding=\"5\" summary=\"Appreciation on the group/class\">\n";
echo "<tr>\n";
echo "<th width=\"200\"><div align=\"center\">&nbsp;</div></th>\n";
echo "<th width=\"30\"><div align=\"center\"><b>Moy.</b></div></th>\n";
echo "<th>\n";
echo "<div style='float:right; width:16;'><a href='javascript:affichage_div_photo();'><img src='../images/icons/wizard.png' width='16' height='16' alt='Display the quartiles and possibly the picture of student' title='Display the picture of student during the seizure' /></a></div>\n";
echo "<div align=\"center\"><b>Appreciation on the group/class</b>\n";
echo "</div></th>\n";
echo "</tr>\n";
//=================================================
...
((getSettingValue("GepiAccesReport cardSimpleProf")=="yes")||
(getSettingValue("GepiAccesReport cardSimpleProfToutesClasses")=="yes")||
(getSettingValue("GepiAccesReport cardSimpleProfTousEleves")=="yes")
)) {
	$acces_bull_simp='y';
}
/*
	...
	while ($k < $nb_periode) {
		...
		if (in_array($eleve_login, $current_group["eleves"][$k]["list"])) {
			...
			if ($restauration != "oui" AND $restauration != "non") {
				...
				if ($verif_t != 0) {
					$eleve_app_t = "\n".'<p>Not recorded appreciation : <span style="color: red;">'.@mysql_result($app_t_query, 0, "appreciation").'</span></p>';
				} else {
					$eleve_app_t = '';
				}
			} else {
			...
			if ((($current_group["classe"]["ver_periode"][$eleve_id_classe][$k]!="N")&&($_SESSION['statut']!='secours'))||
				...
				if(($_SESSION['statut']=='professeur')&&($current_group["classe"]["ver_periode"][$eleve_id_classe][$k]=="P")) {
					...
					($tab_autorisation_exceptionnelle_de_saisie[$k]=='y')) {
						...
						$mess[$k].="<div style='float:right; width:2em; height:1em;'><a href='#' onclick=\"affiche_div_correction('$eleve_login','$k','$cpt_correction');return false;\" alt='Propose a correction' title='Propose a correction'><img src='../images/edit16.png' width='16' height='16' alt='Propose a correction' title='Propose a correction' /></a>";
				...
				if(mysql_num_rows($correct_app_query)>0) {
					...
					$mess[$k].="<div style='color:darkgreen; border: 1px solid red;'><b>Proposal for a correction on standby &nbsp;:</b><br />".nl2br($lig_correct_app->appreciation)."</div>\n";
				}
				...
				if ($current_eleve_nbct ==0) {
					$liste_notes='No the note in the notebook for this period.';
				}

				$mess[$k]="<td>".$note."</td>\n";
				//$mess[$k].="<td>".$notes_conteneurs."Contenu du carnet de notes : ";
				$mess[$k].="<td>Contents of the report card : ";
				...
				if($liste_notes_detaillees!='') {
					...
					$titre="Notes of $eleve_nom $eleve_prenom over the period $k";
					$texte="";
					//$texte.="<div align='center'>\n";
			...
			//
			$suit_option[$k] = 'no';
			$mess[$k] = "<td>&nbsp;</td><td><p class='small'>not followed</p></td>\n";
		}
		$k++;
	}
	...
	if ($display_eleve=='yes') {
		...
		*/
		...
		if(($id_premiere_classe!='')&&($acces_bull_simp=='y')) {
			...
			echo "<img src='../images/icons/bulletin_simp.png' width='17' height='17' alt='Simple bulletin all periods in infobulle' title='Simple bulletin all periods in infobulle' />";
			echo "</a>";
			//echo "</div>\n";
		}
			...
			*/
			...
			if ($current_group["classe"]["ver_periode"]["all"][$k] == 0) {
				...
				if((isset($current_id_classe[$k]))&&($acces_bull_simp=='y')) {
					...
					echo " <a href=\"#\" onclick=\"afficher_div('div_bull_simp','y',-100,20); affiche_bull_simp('$eleve_login','$current_id_classe[$k]','$k','$k');return false;\" alt='Simple bulletin in infobulle' title='Simple bulletin in infobulle'>";
					//echo $nom_periode[$k];
					echo "<img src='../images/icons/bulletin_simp.png' width='17' height='17' alt='Simple bulletin of the period in infobulle' title='Simple bulletin of the period in infobulle' />";
					echo "</a>";
				}
				//else {
				//	echo $nom_periode[$k];
				...
				if((isset($current_id_classe[$k]))&&($acces_bull_simp=='y')) {
					...
					echo "<a href=\"#\" onclick=\"afficher_div('div_bull_simp','y',-100,40); affiche_bull_simp('$eleve_login','$current_id_classe[$k]','$k','$k');return false;\" alt='Simple bulletin in infobulle' title='Simple bulletin in infobulle'>";
...
<?php
	...
	echo "<img src='../images/icons/wizard.png' width='16' height='16' alt='Insert the notes of the homework' title='Insert the notes of the homework' />";
	echo "</a>\n";
...
((substr(getSettingValue('autoriser_correction_bulletin'),0,1)=='y')||($une_autorisation_exceptionnelle_de_saisie_au_moins=='y'))) {
	$titre="Correction of an appreciation";
	...
	$texte.="You can propose a correction for <span id='span_correction_login_eleve' class='bold'>...</span> over the period <span id='span_correction_periode' class='bold'>...</span>&nbsp;: ";
	...
	$texte.="<input type='submit' name='soumettre_correction' value='Subject the correction' />\n";
	$texte.="</form>\n";
...
function affiche_div_correction(eleve_login,num_periode,num_eleve) {
	...
	if(change!='no') {
		alert(\"Modifications were not recorded. If you validate the proposal for a correction without initially
recording, the modifications will be lost.\")
	}
}

/*
// Ca does not function correctly.
...
if ($restauration == "oui" OR $restauration == "non") {
	$effacer = mysql_query("DELETE FROM matieres_appreciations_tempo WHERE id_groupe = '".$id_groupe."'")
	OR DIE('Error in the obliteration of the temporary table (2) : '.mysql_error());
}
