...
//**************** EN-TETE *****************
$titre_page = "Seizure of the opinions of the staff meeting";
...
if (($_SESSION['statut'] == 'scolarite') or ($_SESSION['statut'] == 'secours')) {
	...
	if($nombre_classe==0){
		...
		echo "<th rowspan='2' onMouseover=\"afficher_div('info_classe','y',10,10)\" onmouseout=\"cacher_div('info_classe')\">Opinion alone</th>\n";
		echo "<th rowspan='2'>Individual with<br />appréciations</th>\n";
		echo "<th colspan='$nb_max_periode'>Import CSV</th>\n";
		echo "</tr>\n";

		$tabdiv_infobulle[]=creer_div_infobulle("info_classe","","","<center>type the opinions, for all the class, with recall of the opinions
of the other periods.</center>","",10,0,"n","n","y","n");
		...
		$tabdiv_infobulle[]=creer_div_infobulle("saisie_avis1","","","<center>type the opinions, for all the class, with recall of the opinions of the other periods.</center>","",15,0,"n","n","y","n");
		$tabdiv_infobulle[]=creer_div_infobulle("saisie_avis2","","","<center>type the opinions, raises by student, with visualization of the results of the student.</center>","",15,0,"n","n","y","n");
		$tabdiv_infobulle[]=creer_div_infobulle("import_avis","","","<center>Import a file of appreciations (format csv)</center>","",15,0,"n","n","y","n");		$tabdiv_infobulle[]=creer_div_infobulle("periode_close","","","<center>".$gepiClosedPeriodLabel."</center>","",8,0,"n","n","y","n");
    ...
    if ($nombre_classe == "0") {
        echo "You are not ".getSettingValue("gepi_prof_suivi")." ! It thus does not return to you to type the opinions of staff meeting.";
    } else {
        $j = "0";
        ...
        while ($j < $nombre_classe) {
            ...
            echo "<br />$classe_suivi --- <a href='saisie_avis1.php?id_classe=$id_classe'>type the opinions for my group.</a>";
            echo "<br />$classe_suivi --- <a href='saisie_avis2.php?id_classe=$id_classe'>To type the opinions for my group, with visualization of the results.</a>";
            include "../lib/periodes.inc.php";
            $k="1";
            while ($k < $nb_periode) {
               if ($ver_periode[$k] != "O") {
                   echo "<br />$classe_suivi --- ".ucfirst($nom_periode[$k]);
                   echo " --- <a href='import_app_cons.php?id_classe=$id_classe&amp;periode_num=$k'>Import a file of appreciations (format csv)</a>";
               }
               $k++;
            }
            echo "<br />";
