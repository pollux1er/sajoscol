	...
	if(isset($_POST['enregistrer_ajout_a_textarea_vide'])) {
		if (isset($NON_PROTECT["ajout_a_textarea_vide"])) {
			...
			if(!saveSetting('default_mass_appreciation', $ajout_a_textarea_vide)) {
				$msg.="Error during the recording of 'default_mass_appreciation'<br />\n";
			}
		}
	}
		...
		if ($ver_periode[$i] != "O"){
			if (isset($NON_PROTECT["synthese_".$i])){
				...
				if(mysql_num_rows($test)==0) {
					...
					if(!$insert) {$msg.="Error during the recording of the synthesis.";}
					...
					if(!$update) {$msg.="Error At the time of the update of the synthesis.";}
			...
			if ($ver_periode[$i] != "O"){
				...
				if ($result_test != 0) {
					...
					if($num_eleve!=-1){
						...
						if (!$register) {
							$msg.="Error during the recording of the data of the period $i for $reg_eleve_login<br />\n";
							$pb_record = 'yes';
						}
					}
				}
...
//**************** EN-TETE *****************
$titre_page = "Seizure of the opinions | Seizure";
...
<p class='bold'><a href="saisie_avis.php" onclick="return confirm_abandon(this, change, '<?php echo $themessage; ?>')"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> My classes</a>
...
if(isset($id_class_prec)){
	if($id_class_prec!=0){echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_class_prec' onclick=\"return confirm_abandon (this, change, '$themessage')\">Preceding Class</a>";}
}
...
if ($id_classe) {
	...
	?>
	<p class= 'grand'>Opinion of the staff meeting. Class : <?php echo $classe; ?></p>
	<?php
...
if ($insert_mass_appreciation_type=="y") {
	...
	if($droit_insert_mass_appreciation_type=="y") {
		...
		echo "Insert the opinion-type following for all the empty opinions&nbsp;: ";
		...
		echo "<input type='checkbox' name='enregistrer_ajout_a_textarea_vide' id='enregistrer_ajout_a_textarea_vide' value='y' /><label for='enregistrer_ajout_a_textarea_vide'>Record this opinion-type like opinion-type by defect</label><br />\n";

		echo "<input type='button' name='ajouter_a_textarea_vide' value='Ajouter' onclick='ajoute_a_textarea_vide(); changement()' /><br />\n";

		echo "<input type='button' name='button_vider_tous_les_avis' value='Empty all the opinions' onclick='vider_tous_les_avis(); changement()' /><br />\n";
		echo "</div>\n";
	...
	function vider_tous_les_avis() {
		var is_confirmed = confirm('CAUTION: You asked to empty all the opinions seized for this class ! Are ypu sure to want to empty these opinions ?');
		if(is_confirmed){
			champs_textarea=document.getElementsByTagName('textarea');
			for(i=0;i<champs_textarea.length;i++){
				if(champs_textarea[i].name!='no_anti_inject_ajout_a_textarea_vide') {
...
//=========================
	...
	echo "<table width=\"750\" class='boireaus' border='1' cellspacing='2' cellpadding='5' summary=\"Synthesis of class\">\n";
	echo "<tr>\n";
	echo "<th width=\"200\"><div align=\"center\"><b>&nbsp;</b></div></th>\n";
	echo "<th><div align=\"center\"><b>Synthesis of class</b>\n";
	echo "</div></th>\n";
	echo "</tr>\n";
	//========================
				...
				if ($result_test != 0) {
					...
					if((file_exists('saisie_commentaires_types.php'))
						...
						if($commentaires_type_classe_periode[$k]=="y"){
							echo "<a href='#' onClick=\"document.getElementById('textarea_courant').value='n".$k.$num_id."';afficher_div('commentaire_type','y',30,-50);return false;\">Add a comment-type</a>\n";
						}
					}
	...
	if ($test_periode_ouverte == 'yes') {
		...
		<center><div id="fixe"><input type='submit' value='Record' />
