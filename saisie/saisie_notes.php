<?php
/*
* $Id: saisie_notes.php 8415 2011-10-04 12:28:08Z crob $
*
...
$themessage  = 'Notes were modified. Do you want to really leave without recording ?';
$message_enregistrement = "The modifications were recorded !";
//**************** EN-TETE *****************
$titre_page = "Seizure of the averages";
...
echo "<p class='bold'>\n";
if (isset($retour_cn)) {
	echo "<a href=\"../cahier_notes/index.php?id_groupe=" . $current_group["id"] . "&amp;periode_num=$periode_cn\" onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return towards my evaluations</a>";
} else {
...
echo " | <a href='saisie_appreciations.php?id_groupe=" . $current_group["id"] . "&amp;periode_cn=$periode_cn' onclick=\"return confirm_abandon (this, change, '$themessage')\">To seize the appreciations </a>";
...
echo " | <a href='index.php?id_groupe=" . $current_group["id"] . "' onclick=\"return confirm_abandon (this, change, '$themessage')\">Import/export notes and appreciations</a>";
//=========================
...
echo "<h2 class='gepi'>Report card - Seizure of the averages </h2>\n";
...
if ($affiche_bascule == 'yes') {
	...
	if ($id_racine == '') {echo "<font color=\"#FF0000\">Currently, you do not use the book of notes. There is thus no note to import.</font>\n";}

	echo "<form enctype=\"multipart/form-data\" action=\"saisie_notes.php\" method=\"post\">\n";
	echo add_token_field();
	if ($is_posted != 'bascule') {
		//echo "<tr><td><input type=\"submit\" value=\"Recopier\"></td><td> : Just copy column \"carnet de notes\" in the column colonne \"Report Card\"</td></tr>\n";
		echo "<input type=\"submit\" value=\"Recopier\" /> : Recopy the column \"Carnet Notes\" in the column \"Report Card\"\n";
		...
		echo "<input type=\"submit\" value=\"Cancel recopy\" /> : Post in the column \"bulletin\" currently recorded averages\n";
	}
...
<?php
	...
	if (($current_group["classe"]["ver_periode"]["all"][$periode_cn]>=2)||
		(($current_group["classe"]["ver_periode"]["all"][$periode_cn]!=0)&&($_SESSION['statut']=='secours'))) {
		echo "<p><input type='submit' value='Enregistrer' /> : Record the averages in the bulletin</p>\n";

		echo "<p><i>Type a note from 0 to 20 for each student, or failing this the code 'a' for 'absent', the code 'd' for 'dispensé', the code 'n' or '-' for absence of note.</i></p>\n";
	}

	echo "<p><b>Averages (on 20) of : ".htmlentities($current_group["description"])." (" . $current_group["classlist_string"] . ")</b></p>\n";
	...
	echo "<tr>\n";
	echo "<td><b><a href='saisie_notes.php?id_groupe=$id_groupe&amp;periode_cn=$periode_cn&amp;order_by=nom'>Name First name</a></b></td>\n";

	if ($multiclasses) {
		echo "<td><b><a href='saisie_notes.php?id_groupe=$id_groupe&amp;periode_cn=$periode_cn&amp;order_by=classe'>Class</a></b></td>";
	}
	$i = 1;
	while ($i < $nb_periode) {
		/*
	...
	while ($i < $nb_periode) {
		...
		if (($current_group["classe"]["ver_periode"]["all"][$i]>=2)||
			...
			if ($periode_cn == $i) {
				echo "<td bgcolor=\"$couleur_moy_cn\" style='text-align:center;'>Carnet<br />notes</td><td bgcolor=\"$couleur_fond\"  style='text-align:center;'>Report Card</td>\n";
			} else {
				//echo "<td>&nbsp;</td>\n";
				echo "<td style='text-align:center;'>Carnet<br />notes</td><td style='text-align:center;'>Report Card</td>\n";
			}
		} else {
...
if(count($tab_recopie_vide)>0) {
	$chaine_js="<p style='text-align:center'>No the average recopied for:<br />";
	for($i=0;$i<count($tab_recopie_vide);$i++) {
		$chaine_js.="<b>".$tab_recopie_vide[$i]."</b><br />";
	}
	$chaine_js.="It will be necessary to seize Absent manually (<b>a</b>), Exempted (<b>d</b>) or Not noted (<b>-</b>).</p>";
...
?>
	<script type="text/javascript" language="javascript">
	<!--
	alert("Caution, the imported notes are not recorded yet in base GEPI. You must confirm the importation (bouton \"Record\") !");
	//-->
	</script>
<?php
}
