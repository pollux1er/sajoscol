...
if((isset($id_classe))&&(isset($num_periode))) {
	...
	if($_SESSION['statut']=='professeur') {
		...
		if(mysql_num_rows($test)==0) {
			...
			header('Location: ../accueil.php&msg='.rawurlencode("You are not authorized to seize the synthesis for this class."));
	...
	if (isset($NON_PROTECT["synthese"])) {
		...
		if(mysql_num_rows($test)==0) {
			...
			else {$msg="The synthesis was recorded.";}
		}
		else {
			...
			else {$msg="The synthesis was updated.";}
		}
	}
	...
	$titre_page="Synthesis classifies";
	...
	echo "<p class=\"bold\">";
	if(isset($url_retour)) {
		echo "<a href=\"$url_retour\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to the seizure of the opinions</a>";
	}
	else {
		echo "<a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return reception</a>";
	}
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Autre classe</a>\n";
	echo " | <a href='".$_SERVER['PHP_SELF']."?id_classe=$id_classe'>Another period</a>\n";
	echo "</p>\n";
	...
	*/
	...
	echo "<p><b>Seizure of the synthesis for the group classifies &nbsp;:</b><br />\n";
...
if(!isset($id_classe)) {
	...
	if($_SESSION['statut']=='professeur') {
		...
		if(mysql_num_rows($res_classe)==0) {
			header('Location: ../accueil.php&msg='.rawurlencode("You are not authorized to seize the synthesis of a class."));
			die();
		}
	}
	else {
		...
		$res_classe=mysql_query($sql);
		if(mysql_num_rows($res_classe)==0) {
			header('Location: ../accueil.php&msg='.rawurlencode("You are not authorized to seize the synthesis for a class."));
			die();
		}
	}

	echo "<p class=\"bold\"><a href=\"../accueil.php\"><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return reception</a>";
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Another class</a>\n";
	echo "</p>\n";

	$nombreligne = mysql_num_rows($res_classe);
	echo "<p>Click on the class for which you wish to seize the synthesis of the
appreciations groups class.</p>\n";
	//echo "<table border=0>\n";
	...
	$res_classe=mysql_query($sql);
	if(mysql_num_rows($res_classe)==0) {
		echo "<p style='color:red'>The selected class does not exist.</p>";
		require("../lib/footer.inc.php");
		die();
	}
	...
	if(mysql_num_rows($res_classe)==0) {
		echo "<p>All the periods are closed for this class.<br />More no modification is possible.</p>";
		require("../lib/footer.inc.php");
		die();
	}
	...
	echo "<p>Choose the period for which you wish to seize the synthesis &nbsp;: <br />\n";
	$cpt=0;
	...
	echo "<p>Choose periods to be posted in the bulletin of class &nbsp;: <br />\n";
