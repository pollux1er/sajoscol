...
(($_SESSION['statut']=='secours')&&($current_group["classe"]["ver_periode"]['all'][$periode_num]==0))) {
	echo "<p class = 'grand'>Importation of averages and appreciations - $nom_periode[$periode_num]</p>";
	echo "<p class = 'bold'>Group : " . $current_group["description"] . " " . $current_group["classlist_string"] . " | Subject : " . $current_group["matiere"]["nom_complet"]."</p>\n";

	echo "<p>The period is closed.</p>\n";
...
echo "<p class = 'grand'>Importation of averages and appreciations - $nom_periode[$periode_num]</p>";
echo "<p class = 'bold'>Group : " . $current_group["description"] . " " . $current_group["classlist_string"] . " | Subject : " . $current_group["matiere"]["nom_complet"];
echo "<p>";
$modif = 'no';
$nb_row++;
for ($row=1; $row<$nb_row; $row++) {
	...
	if($temoin_periode_close=="y") {
		echo "<font color='red'>The period is closed for the user $reg_login !</font><br />\n";
	}
	else {
		if ((!$reg_data1) or (!$reg_data2)) {
				echo "<font color='red'>Error during the modification of data of the user $reg_login !</font><br />\n";
		} else {
			echo "Data of the user $reg_login were modified successfully !<br />\n";
		}
	}
}
...
echo "</p>\n";
echo "<p><a href='saisie_notes.php?id_groupe=$id_groupe&amp;order_by=nom'>Reach the page of seizure of the averages for checking</a>";
echo "<br /><a href='saisie_appreciations.php?id_groupe=$id_groupe&amp;order_by=nom'>Reach the page of seizure of the appreciations for checking</a></p>\n";
require("../lib/footer.inc.php");
?>