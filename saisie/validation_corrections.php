...
if(isset($_POST['action_corrections'])) {
	...
	for($i=0;$i<count($enregistrement);$i++) {
		...
		(strlen(my_ereg_replace('[0-9]','',$current_periode))==0)) {
			...
			if ((isset($action[$i]))&&(in_array($action[$i],$tab_actions_valides))) {
				if (isset($NON_PROTECT["appreciation".$i])) {
					...
					if($action[$i]=='supprimer') {
						...
						$del=mysql_query($sql);
						if($del) {
							$msg.="Suppression of the temporary recording $enregistrement[$i].<br />";
							//$nb_reg++;
							...
							$texte_email[$current_id_groupe].="Your proposal for a correction for ".$current_nom_prenom_eleve." in ".$current_group['name']." (".$current_group["description"]." in ".$current_group["classlist_string"].") over the period $current_periode was refused/removed.\n";
						}
						else {
							$msg.="Error during the suppression of the temporary recording $enregistrement[$i].<br />";
						}
					}
					elseif($action[$i]=='valider') {
						...
						if($insert) {
							...
							if($del) {
								...
								// Envoyer un mail... problème... il serait bien de n'envoyer qu'un seul mail par destinataire, plutôt que un mail par correction
								if(!isset($texte_email[$current_id_groupe])) {$texte_email[$current_id_groupe]="";}
								$texte_email[$current_id_groupe].="Your proposal for a correction for ".$current_nom_prenom_eleve." in ".$current_group['name']." (".$current_group["description"]." in ".$current_group["classlist_string"].") over the period $current_periode was validated.\n";
							...
							$msg.="Error at the time of the update of the recording $enregistrement[$i] on the bulletin.<br />";
						}
					}
			...
			$msg.="Invalid characters are proposed for $enregistrement[$i].<br />";
		}
	}

	if(($nb_reg>0)||(count($texte_email)>0)) {
		if($nb_reg>0) {$msg.="$nb_reg recording carried out.<br />";}
			...
			foreach($texte_email as $id_groupe => $texte) {
				if($texte!='') {
					...
					if($email_destinataires!='') {
						$sujet_mail="[GEPI] Groupe n°$id_groupe: Response to your request for correction";
...
//**************** EN-TETE *****************
$titre_page = "Validation of the corrections";
require_once("../lib/header.inc");
//**************** FIN EN-TETE *****************
//debug_var();
?>

<p class='bold'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Reception</a>
<?php
...
if(!isset($tab_id_classe)) {
	...
	if($nb_classes==0) {
		echo "<p>No propostion of correction requires your attention.</p>\n";
		echo "<p><br /></p>\n";
	...
	echo "<p class='bold'>Please choose the classes for which you wish to consult/Validate the proposals for corrections.</p>\n";
	...
	echo "</table>\n";
	echo "<p><a href='#' onClick='ModifCase(true); return false;'>All tick</a> / <a href='#' onClick='ModifCase(false)'>All shoot</a></p>\n";
	...
	echo " | <a href='".$_SERVER['PHP_SELF']."'>Choose other classes</a></p>\n";
	...
	if((isset($reimprimer_bulletins))&&(count($reimprimer_bulletins)>0)) {
		...
		echo "<p><span style='font-weight:bold; color:red'>CAUTION&nbsp;:</span> Modifications were carried out.<br />Undoubtedly should be reprinted the corresponding bulletins.</p>\n";
	...
	for($i=0;$i<count($tab_id_classe);$i++) {
		...
		if($nb_grp==0) {
			echo "<p>No teaching associated with this class presents (more) of proposal for
a correction.</p>\n";
		}
		else {
			echo "<table class='boireaus' width='100%' summary=\"Corrections suggested in $classe\">\n";
			echo "<tr>\n";
			echo "<th rowspan='2'>Teaching</th>\n";
			echo "<th rowspan='2'>student</th>\n";
			echo "<th rowspan='2'>Period</th>\n";
			echo "<th rowspan='2'>Proposition</th>\n";
			echo "<th colspan='3'>Action</th>\n";
			echo "</tr>\n";

			echo "<tr>\n";
			echo "<th>On standby</th>\n";
			echo "<th>Validate</th>\n";
			echo "<th>Remove</th>\n";
			echo "</tr>\n";
			...
			while($lig=mysql_fetch_object($res)) {
				...
				while($lig_ele=mysql_fetch_object($res_ele)) {
					...
					echo "<b>Recorded appreciation&nbsp;:</b> ";
					...
					if(mysql_num_rows($res_old_app)>0) {
						...
						echo "<span style='color:red'>No appreciation was recorded before the proposal for a correction.</span>\n";
					}
					echo "</div>\n";
					echo "<div style='border: 1px solid black; margin: 2px;'>\n";
					echo "<b>Correction suggested&nbsp;:</b> ";
	...
	echo "<p><a href='#' onClick=\"ToutCocher('action_valider_'); return false;\">All Validate</a> / <a href='#' onClick=\"ToutCocher('action_supprimer_'); return false;\">All Remove</a> / <a href='#' onClick=\"ToutCocher('action_attente_'); return false;\">All to give on standby</a></p>\n";
