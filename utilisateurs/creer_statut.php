...
function verifChecked($id){
	...
	for($i = 1 ; $i < $iter ; $i++){
		...
		$query_ds = mysql_query($sql_ds) OR trigger_error('Error in the function verifChecked ', E_USER_ERROR);
...
if ($action == 'ajouter') {
	...
	if ($verif >= 1) {
		...
		$msg .= "<h3 class='red'>This private statute already exists !</h3>";

	}else{

		$sql = "INSERT INTO droits_statut (id, nom_statut) VALUES ('', '".$insert_statut."')";
		$enregistre = mysql_query($sql) OR trigger_error('Impossible to record this new statute', E_USER_WARNING);
		updateOnline($sql);
		...
		if ($enregistre) {
			...
			for($a = 0 ; $a < $iter ; $a++){ // $iter est définie dans creer_statut_autorisation.php
 			...
 			$autorise_b = mysql_query("INSERT INTO droits_speciaux (id, id_statut, nom_fichier, autorisation) VALUES ".$values_b."")
			 										OR trigger_error('Impossible to record : '.$values_b.' : '.mysql_error(), E_USER_WARNING);
updateOnline("INSERT INTO droits_speciaux (id, id_statut, nom_fichier, autorisation) VALUES ".$values_b."");
			if ($autorise_b) {
				$msg .= '<h4 style="color: green;">This statute is recorded!</h4>';
			}
...
if ($action == 'modifier') {
	...
	for($a = 0; $a < $nbre; $a++){
		...
		if ($test[$a][0] == 'on') {
			...
			$query_d = mysql_query($sql_d) OR trigger_error('Impossible to remove this statute : '.mysql_error(), E_USER_NOTICE);
			updateOnline($sql_d);
			...
			$query_d = mysql_query($sql_d) OR trigger_error('Impossible to remove this statute of : '.mysql_error(), E_USER_NOTICE);
			updateOnline($sql_d);
			...
			updateOnline($sql_d);
                  ...
                  for($m = 1 ; $m < $iter ; $m++){
                    ...
                    for($i = 0 ; $i < $nbre2 ; $i++){
                     ...
                     updateOnline("UPDATE droits_speciaux SET autorisation = '".$vf."' WHERE id_statut = '".$b."' AND nom_fichier = '".$autorise[$m][$i]."'");
					 }else{
                        $query2_maj = mysql_query("INSERT INTO `droits_speciaux` VALUES ('','".$b."','".$autorise[$m][$i]."','".$vf."')");
						updateOnline("INSERT INTO `droits_speciaux` VALUES ('','".$b."','".$autorise[$m][$i]."','".$vf."')");                     
					 }

                      if (!$query_maj) {
			$msg3 .= '<span class="red">Error</span>';
                      }
                    }
                  } // for($m = 1 ; $m < $iter ; $m++){
		}
...
//print_r($test);
	...
	if ($msg3 === NULL) {
		$msg3 .= '<p style="color: green">The modifications are well recorded.</p>';
	}
}
...
if ($query) {
	...
	for($b = 0 ; $b <= $iter ; $b++){
		...
		if ($b == 0) {
			...
			$aff_tableau2[$b] = '<tr style="border: 1px solid white; text-align: center;"><td style="font-weight: bold;">List of rights</td>';

		}elseif($b == $iter){

			// On ajoute une ligne pour la suppression
			$aff_tableau2[$b] = '<tr style="border: 1px solid white; background-color: silver; text-align: center;"><td>Remove this statute</td>';
...
// On traite la partie sur les utilisateurs 'autre' pour leur définir le bon statut
	...
	if ($action == "defStatut") {
		...
		$query_v2 = mysql_query("SELECT id_statut FROM droits_utilisateurs WHERE login_user = '".$login_user."'")
									OR trigger_error('Impossible to check the private statute of this user.', E_USER_WARNING);
		...
		$query_statut = mysql_query($sql_d) OR trigger_error('Impossible to record in the base.'.mysql_error(), E_USER_WARNING);
		updateOnline($sql_d);
		if ($query_statut) {
			$msg2 .= '<h4 style="color: green;">Recorded modification.</h4>';
		}
	...
	while($tab = mysql_fetch_array($query_u)){
		...
		$aff_users .= '
			...
			<select name="userstat" onchange=\'document.getElementById("form'.$i.'").submit();\'>
				<option value="rien">Choice of statute</option>';
...
//======================================================
$titre_page = 'Management of personalized statutes';
...
<p>You can define personalized statutes, having a particular combination of rights.
 To then be able to allot (below) a statute personalized to a user, it should initially be recorded with a generic statute "other"
 (<a href="./modify_user.php">CREATE a personnel</a> ou <a href="./index.php?mode=personnels">MODIFY a personnel</a>).</p>
...
<div id="userStatut" style="border: 5px solid silver; width: 22em; margin: 5px 5px 5px 5px;">
	...
	<p style="text-align: right; font-style: italic; color: grey; background-color: lightblue;">Management of the personalized statutes&nbsp;&nbsp;</p>
...
<p style="color: grey; text-align: right; font-style: italic;">Management of the rights of the personalized statutes&nbsp;&nbsp;</p>
...
<table style="border: 1px solid white; background: #CCFFFF;">
	...
	<tfoot>
		<tr><td><?php echo $msg3; ?></td><td colspan="<?php echo $nbre_statuts; ?>"><input type="submit" name="modifier" value="Record and update" /></td></tr>
	</tfoot>
...
<p class="ajoutSt" onclick="changementDisplay('ajoutStatut', '');">Add a personalized statute</p>
...
?>
		<p>
		<label for="new">Name of the new statute</label>
		...
		<p style="color: grey; font-style: italic; margin-left: 10em;">Letters and figures only. Maximum length : 12 characters.</p>
