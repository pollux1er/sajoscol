<?php
/*
 * $Id: edit_eleve.php 6926 2011-05-14 13:29:13Z jjacquard $
 *
	...
	if ($action == "rendre_inactif") {
		...
		if ($mode == "individual") {
			...
			if ($test == "0") {
				...
				updateOnline("UPDATE utilisateurs SET etat='inactif' WHERE (login = '".$_GET['eleve_login']."')");
				if ($res) {
					$msg .= "The user ".$_GET['eleve_login'] . " was deactivated.";
				} else {
					$msg .= "Error during desactivation of the user.";
				}
			}
		} elseif ($mode == "classe" and !$error) {
			// Pour tous les ÈlËves qu'on a dÈj‡ sÈlectionnÈs un peu plus haut, on dÈsactive les comptes
			while ($current_eleve = mysql_fetch_object($quels_eleves)) {
				...
				if ($test > 0) {
					...
					updateOnline("UPDATE utilisateurs SET etat = 'inactif' WHERE login = '" . $current_eleve->login . "'");
					if (!$res) {
						$msg .= "Error during desactivation of the account ".$current_eleve->login."<br/>";
					} else {
						$nb_comptes++;
					}
				}
		...
		if ($mode == "individual") {
			...
			if ($test == "0") {
				...
				updateOnline("UPDATE utilisateurs SET etat='actif' WHERE (login = '".$_GET['eleve_login']."')");
				if ($res) {
					$msg .= "The user ".$_GET['eleve_login'] . " was activated.";
				} else {
			...
			while ($current_eleve = mysql_fetch_object($quels_eleves)) {
				...
				if ($test > 0) {
					...
					updateOnline("UPDATE utilisateurs SET etat = 'actif' WHERE login = '" . $current_eleve->login . "'");
					if (!$res) {
						$msg .= "Error during activation of the account ".$current_eleve->login."<br/>";
					} else {
						$nb_comptes++;
					}
				}
		...
		if ($mode == "individual") {
			...
			if ($test == "0") {
				...
				updateOnline("DELETE FROM utilisateurs WHERE (login = '".$_GET['eleve_login']."')");
				if ($res) {
					$msg .= "The user ".$_GET['eleve_login'] . " was removed.";
					if ($ldap_write_access) {
						if ($ldap_server->test_user($_GET['eleve_login'])) {
							...
							if ($write_ldap_success) {
								$msg .= "<br/>The user was removed from directory LDAP.";
							}
						}
					}
				} else {
			...
			while ($current_eleve = mysql_fetch_object($quels_eleves)) {
				...
				if ($test > 0) {
					...
					updateOnline("DELETE FROM utilisateurs WHERE login = '" . $current_eleve->login . "'");
					if (!$res) {
						$msg .= "Error during suppression of the account ".$current_eleve->login."<br/>";
					} else {
						$nb_comptes++;
		...
		if ($mode != "classe") {
			...
			if ($_POST['classe'] == "all") {
				$msg .= "You are going to reinitialize the passwords of all the users having the statute 'eleve'.<br/>If you are really sure to want to carry out this operation, click on the link below :";
				$msg .= "<br/><a href=\"reset_passwords.php?user_status=eleve&amp;mode=html$chaine_mdp_INE".add_token_in_url()."\" target='_blank'>Reinitialize the passwords (Impression HTML)</a>";
				$msg .= "<br/><a href=\"reset_passwords.php?user_status=eleve&amp;mode=csv$chaine_mdp_INE".add_token_in_url()."\" target='_blank'>Reinitialize the passwords (Export CSV)</a>";
				$msg .= "<br/><a href=\"reset_passwords.php?user_status=eleve&amp;mode=pdf$chaine_mdp_INE".add_token_in_url()."\" target='_blank'>Reinitialize the passwords(Impression PDF)</a>";
			} else if (is_numeric($_POST['classe'])) {
				$msg .= "You are going to reinitialize the passwords of all the users having the statute 'eleve' for this class.<br/>If you are really sure to want to carry out this operation, click on the link below :";
				$msg .= "<br/><a href=\"reset_passwords.php?user_status=eleve&amp;user_classe=".$_POST['classe']."&amp;mode=html$chaine_mdp_INE".add_token_in_url()."\" target='_blank'>Reinitialize the passwords (Impression HTML)</a>";
				$msg .= "<br/><a href=\"reset_passwords.php?user_status=eleve&amp;user_classe=".$_POST['classe']."&amp;mode=csv$chaine_mdp_INE".add_token_in_url()."\" target='_blank'>Reinitialize the passwords (Export CSV)</a>";
				$msg .= "<br/><a href=\"reset_passwords.php?user_status=eleve&amp;user_classe=".$_POST['classe']."&amp;mode=pdf$chaine_mdp_INE".add_token_in_url()."\" target='_blank'>Reinitialize the passwords (Impression PDF)</a>";
			}
		}
		...
		if ($mode != "classe") {
			...
			while ($current_eleve = mysql_fetch_object($quels_eleves)) {
				...
				if ($test > 0) {
					...
					if ($_POST['reg_auth_mode'] != $old_auth_mode) {
	                  ...
	                  updateOnline("UPDATE utilisateurs SET auth_mode = '".$reg_auth_mode."' WHERE login = '".$current_eleve->login."'");
						...
						if ($old_auth_mode == "gepi" && ($_POST['reg_auth_mode'] == "ldap" || $_POST['reg_auth_mode'] == "sso")) {
							...
							updateOnline("UPDATE utilisateurs SET password = '', salt = '' WHERE login = '".$current_eleve->login."'");
			...
			$msg .= "$nb_comptes accounts were modifieds.";
			if (isset($pass_init_required) && $pass_init_required) {
				$msg .= "<br/>Caution ! Modifications applied require a reinitialization of passwords !";
			}
		}
	}
}
...
if(mysql_num_rows($quels_eleves)==0){
	echo "<p>Aucun compte ÈlËve n'existe encore.<br />You can add students accounts using the link above.</p>\n";
	require("../lib/footer.inc.php");
	die;
}
echo " | <a href='impression_bienvenue.php?mode=eleve'>Welcome Cards </a>";
...
echo "<p style='font-weight:bold;'>Actions by batch for the existing students accounts  : </p>\n";
echo "<blockquote>\n";
echo "<p>\n";
...
echo "<br />\n";
echo "<input type='radio' name='action' id='action_rendre_inactif' value='rendre_inactif' onchange='traite_p_mdp_INE()' /><label for='action_rendre_inactif' style='cursor:pointer;'> Make inactive</label>\n";
echo "<input type='radio' name='action' id='action_rendre_actif' value='rendre_actif' style='margin-left: 20px;' onchange='traite_p_mdp_INE()' /><label for='action_rendre_actif' style='cursor:pointer;'> Make active</label> \n";
if ($session_gepi->auth_locale || $gepiSettings['ldap_write_access']) {
    echo "<input type='radio' name='action' id='action_reinit_password' value='reinit_password' style='margin-left: 20px;' onchange='traite_p_mdp_INE()' /><label for='action_reinit_password' style='cursor:pointer;'> Reinitialize passwords</label>\n";
}
echo "<input type='radio' name='action' id='action_supprimer' value='supprimer' style='margin-left: 20px;' onchange='traite_p_mdp_INE()' /><label for='action_supprimer' style='cursor:pointer;'> Delete</label><br />\n";
echo "<input type='radio' name='action' id='action_change_auth_mode' value='change_auth_mode' onchange='traite_p_mdp_INE()' /><label for='action_change_auth_mode' style='cursor:pointer;'> Modify authentification : </label>";
?>
<select id="select_auth_mode" name="reg_auth_mode" size="1">
<option value='gepi'>Local (Gepi base )</option>
...
echo "<p id='p_mdp_INE' style='font-size:x-small;'><input type='checkbox' name='mdp_INE' id='mdp_INE' value='y' /> <label for='mdp_INE' style='cursor:pointer'>Use the national number of the student (<i>INE</i>) as password when it is indicated for the reinitialization of the passwords of the class.</label></p>\n";
...
echo "<p><b>List existing students accounts</b> :</p>\n";
echo "<blockquote>\n";
...
echo "</td>\n";
echo "<td>\n";
echo "<input type='submit' name='filtrage' value='Display' /> students having a login of which the <b>name</b> contains: ";
...
echo "<tr>\n";
echo "<td>\n";
echo "<input type='button' name='afficher_tous' value='Display all students having a login' onClick=\"document.getElementById('afficher_tous_les_eleves').value='y'; document.form_rech.submit();\" />\n";
echo "</td>\n";
echo "</tr>\n";
echo "</table>\n";
...
<tr>
	<th>Identifier</th><th>Name First name</th><th>State</th><th>Actions</th><th>Class</th>
</tr>
<?php
	...
	echo "<tr class='lig$alt'>\n";
		...
		echo "<td align='center'>\n";
			...
			if ($current_eleve->etat == "actif") {
				...
				echo "<br />\n";
				echo "<a href='edit_eleve.php?action=rendre_inactif&amp;mode=individual&amp;eleve_login=".$current_eleve->login.add_token_in_url()."'>Deactivate";
			} else {
				echo "<font color='red'>".$current_eleve->etat."</font>";
				echo "<br />\n";
				echo "<a href='edit_eleve.php?action=rendre_actif&amp;mode=individual&amp;eleve_login=".$current_eleve->login.add_token_in_url()."'>Activate";
			}
			echo "</a>\n";
		echo "</td>\n";
		echo "<td>\n";
		echo "<a href='edit_eleve.php?action=supprimer&amp;mode=individual&amp;eleve_login=".$current_eleve->login.add_token_in_url()."' onclick=\"javascript:return confirm('Are you sure to want to remove the user ?')\">Delete</a>\n";

		if($current_eleve->etat == "actif" && ($current_eleve->auth_mode == "gepi" || $gepiSettings['ldap_write_access'] == "yes")) {
			echo "<br />";
			echo "Reinitialize the password : <a href=\"reset_passwords.php?user_login=".$current_eleve->login."&amp;user_status=eleve&amp;mode=html".add_token_in_url()."\" onclick=\"javascript:return confirm('Are you sure to want to carry out this operation ?\\n This one is irreversible, and will reinitialize the password of the user with an alpha-numeric password generated randomly.\\n While clicking on OK, you will launch the procedure, who will generate a page containing the card-welcome to  print immediately for distribution to the user concerned.')\" target='_blank'>Randomly</a>";
			echo " - <a href=\"change_pwd.php?user_login=".$current_eleve->login."\" onclick=\"javascript:return confirm('Are you sure to want to carry out this operation ?\\n This one will reinitialize the password of the user with a password which you will choose.\\n While clicking on OK, you will launch a page which will require of you to type a password and to validate it.')\" target='_blank'>chosen </a>";
		}
		echo "</td>\n";
...
if (mysql_num_rows($quels_eleves) == "0") {
	echo "<p>To create new accesses accounts associated to the students
defined in Gepi, you must click on the link ' Add new accounts' above.</p>\n";
}
