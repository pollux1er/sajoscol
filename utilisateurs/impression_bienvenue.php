...
if(!isset($user_login)) {
	...
	if($mode=="personnels") {
		...
		if(!isset($user_statut)) {
			...
			//**************** EN-TETE *****************************
			$titre_page = "Management of users | Impression users cards ";
			require_once("../lib/header.inc");
			//**************** FIN EN-TETE *****************
	
			echo "<p class='bold'>";
			echo "<a href='$url_retour_index_utilisateurs'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return</a>";
			echo " | <a href='".$_SERVER['PHP_SELF']."'> Welcome Cards </a>";
			echo "</p>\n";
	
			echo "<form action='".$_SERVER['PHP_SELF']."' method='post' target='_blank'>\n";
			echo "<p>Print the welcome cards for one or more categories</p>\n";
	...
	elseif($mode=="responsable") {
		if(!isset($id_classe)) {
			//**************** EN-TETE *****************************
			$titre_page = "Management of the users | Impression of responsibles cards ";
			require_once("../lib/header.inc");
			//**************** FIN EN-TETE *****************
			...
			$res=mysql_query($sql);
			if(mysql_num_rows($res)==0) {
				echo "<p>No responsible account was still created.</p>\n";
			}
			else {
				echo "<form action='".$_SERVER['PHP_SELF']."' method='post' target='_blank'>\n";
				echo "<p>Choose the classes for which to generate the welcome cards &nbsp;:<br />\n";
				...
				echo "<input type='checkbox' name='affiche_adresse_resp' id='affiche_adresse_resp' value='y' /><label for='affiche_adresse_resp'> with the address of the responsible</label><br />\n";
				echo "<input type='submit' value='Validate' /></p>\n";
				echo "<input type='hidden' name='mode' value='$mode' />\n";
				echo "</form>\n";
	...
	function change_style_classe(num) {
		if(document.getElementById('id_classe_'+num)) {
			...
			elseif(is_numeric($id_classe)) {
				...
				$msg="The identifier of class is erroneous: '$id_classe'.";
			}
		}
	}
	elseif($mode=="eleve") {
		if(!isset($id_classe)) {
			//**************** EN-TETE *****************************
			$titre_page = "Management of the users | Impression students cards ";
			require_once("../lib/header.inc");
			//**************** FIN EN-TETE *****************
		...
		//**************** EN-TETE *****************************
		$titre_page = "Management of the users | Impression users cards ";
		require_once("../lib/header.inc");
		//**************** FIN EN-TETE *****************

		echo "<p class='bold'>";
		echo "<a href='$url_retour_index_utilisateurs'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return index users</a>";
		echo "</p>\n";

		echo "<p>welcome Cards &nbsp;:</p>";
		echo "<ul>\n";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?mode=personnels'>personnel</a></li>";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?mode=responsable'>responsibles</a></li>";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?mode=eleve'>students</a></li>";
		echo "</ul>\n";
	...
	if(count($user_login)==0) {
		...
		echo "<p>No user (<i>$mode</i>) was selected.</p>\n";
...
width:".$largeur1."%;\n";
	...
	echo "<table border='0' summary='Recipient welcome card $user_login'>\n";
	echo "<tr>\n";
	echo "<td>\n";
	echo "To the attention of&nbsp;: \n";
	echo "</td>\n";
	echo "<td>\n";
	...
	if($impression=='') {
		echo "<div class='info_fiche_bienvenue'><div align='center'>Information (<i>nonprinted</i>) : The welcome card for <b
	>$user_statut</b> is not indicated.<br />You can parameterize the welcome cards in the following page&nbsp;: <a href='../gestion/modify_impression.php?fiche=";
