			...
			if($_FILES['photo']['type'][$cpt_photo] != "") {
				...
				*/
				...
				if (!isset($sav_photo['tmp_name'][$cpt_photo]) or ($sav_photo['tmp_name'][$cpt_photo] =='')) {
					...
					$msg .= "Error of download level 1 for the photo  $cpt_photo: '".$sav_photo['name'][$cpt_photo]."'<br />\n";
				} else if (!file_exists($sav_photo['tmp_name'][$cpt_photo])) {
					//$msg = "Erreur de téléchargement niveau 2.";
					$msg .= "Error of download level 2 for the photo $cpt_photo: '".$sav_photo['name'][$cpt_photo]."'<br />\n";
				//} else if ((!preg_match('/jpg$/',$sav_photo['name'][$cpt_photo])) and $sav_photo['type'][$cpt_photo] == "image/jpeg"){
					...
					$msg .= "Error: only the files having the jpg extension are authorized: '".$sav_photo['name'][$cpt_photo]."'<br />\n";
				} else {
					$dest = $rep_photos;
					$n = 0;
					...
					if (!deplacer_fichier_upload($sav_photo['tmp_name'][$cpt_photo], $rep_photos.$quiestce[$cpt_photo].".jpg")) {
						...
						$msg = "Problem of transfer: the file '".$sav_photo['name'][$cpt_photo]."' could not be transferred on the repertory photos/personnels/<br />\n";
					} else {
...
*/
	...
	$titre_page = "Management of users";
}
...
<br/><br/>
<p>On this page, you can manage the accounts of access of the users having access to Gepi using an identifier and a password.</p>
<p>Click on the type of users whom you want to manage :</p>
<p style='padding-left: 10%; margin-top: 15px;'><a href="index.php?mode=personnels"><img src='../images/icons/forward.png' alt='Personnels' class='back_link' /> Personnel of the school (professors, schooling, CPE, administrators)</a></p>
<p style='padding-left: 10%; margin-top: 15px;'><a href="edit_responsable.php"><img src='../images/icons/forward.png' alt='Responsables' class='back_link' /> Responsibles of students (parents)</a></p>
<p style='padding-left: 10%; margin-top: 15px;'><a href="edit_eleve.php"><img src='../images/icons/forward.png' alt='Eleves' class='back_link' /> Students</a></p>
<?php
} else {
?>
<p class="bold">
<a href="index.php"><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Return</a>
 | <a href="modify_user.php">Add a personnel</a>
<?php
...
if ((getSettingValue('use_sso') != "cas" and getSettingValue("use_sso") != "lemon" and getSettingValue('use_sso') != "lcs" and getSettingValue("use_sso") != "ldap_scribe") OR $block_sso) {
	...
	*/
    echo " | Reinitialize passwords : " .
    		"<a href=\"reset_passwords.php?mode=html".add_token_in_url()."\" onclick=\"javascript:return confirm('Are you sure to want to carry out this operation ?\\n This one is irreversible, and will reinitialize the passwords of all the users marked active , with an alpha-numeric password generated randomly.\\n While clicking on OK, you will launch the procedure, which will generate a page containing the welcome card to be printed immediately for distribution to the users concerned.')\" target='_blank'>HTML</a>" .
    		" - <a href=\"reset_passwords.php?mode=csv".add_token_in_url()."\" onclick=\"javascript:return confirm('Are you sure to want to carry out this operation ?\\n This one is irreversible, and will reinitialize the passwords of all the users marked active  , with an alpha-numeric password generated randomly.\\n While clicking on OK, you will launch the procedure, which will generate a CSV file containing information necessary to an automated treatment.')\" target='_blank'>CSV</a>";

	echo " | <a href='impression_bienvenue.php?mode=personnels'>welcome Cards </a>";
}
?>
 | <a href="tab_profs_matieres.php">Assign courses to professors</a>
...
if (getSettingValue("statuts_prives") == "y") {
	echo '
	&nbsp;|&nbsp;<a href="./creer_statut.php">Personalized statutes</a>';
}
?>
</p>
...
<p class='small'>Download in the format csv (<i>name - first name - GEPI identifier </i>) the file of <a href="import_prof_csv.php?export_statut=professeur">professors</a>, <a href="import_prof_csv.php?export_statut=scolarite">"schooling"</a>, <a href="import_prof_csv.php?export_statut=cpe">cpe</a>, <a href="import_prof_csv.php?export_statut=secours">emergency</a>, <a href="import_prof_csv.php?export_statut=administrateur">administrateurs</a>, <a href="import_prof_csv.php?export_statut=autre">others</a>, <a href="import_prof_csv.php?export_statut=personnels">personnel</a></p>
...
<tr>
<td><p>Display : </p></td>
<td><p><label for='display_tous' style='cursor: pointer;'>all users</label> <input type="radio" name="display" id='display_tous' value='tous' <?php if ($display=='tous') {echo " checked";} ?> onchange="document.forms['form1'].submit();" /></p></td>
<td><p>
 &nbsp;&nbsp;<label for='display_actifs' style='cursor: pointer;'>active users</label> <input type="radio" id='display_actifs' name="display" value='actifs' <?php if ($display=='actifs') {echo " checked";} ?> onchange="document.forms['form1'].submit();" /></p></td>
 <td><p>
 &nbsp;&nbsp;<label for='display_inactifs' style='cursor: pointer;'>inactive users</label> <input type="radio" name="display" id='display_inactifs' value='inactifs' <?php if ($display=='inactifs') {echo " checked";} ?> onchange="document.forms['form1'].submit();" /></p></td>
...
if(mysql_num_rows($test_auth_mode)==1) {
	...
	<option value=''>All auth_mode</option>\n";
...
?>
 ...
 <td>
	<p><input type='submit' id='bouton_valider' value='Validate' /></p>
...
echo "<table class='boireaus' cellpadding='3' summary='Table of the users'>\n";
...
//    $date_verrouillage[$i] = mysql_result($calldata, $i, "date_verrouillage");
    ...
    while ($k < $nb_mat) {
		...
		if(mysql_num_rows($res_test_matiere)>0) {
			...
			echo "<span style='color:red;'>Anomaly:</span> Course '$user_matiere_id' do not exist any more but remains asociated to '$user_login'.<br />Recreate the course (<i>then remove it properly if necessary</i>)<br />\n";
		}
		$k++;
    }
    ...
    if (getSettingValue("use_sso") == "lcs")
        if ($user_pwd != "")
            $col[$i][3] .= '<br />(local user)';
        else
            $col[$i][3] .= '<br />(LCS user)';
    //if (($display == 'tous') and ($user_etat[$i]=='inactif')) $col[$i][3] .= '<br />(inactif)';
    if (($display == 'tous') and ($user_etat[$i]=='inactif')) $col[$i][3] .= '<br />(<span style="color:red;">inactive</span>)';
	...
	*/
	...
	if ($col[$i][7] == "professeur") {
		...
		//echo "<br /><a href='creer_remplacant.php?login_prof_remplace=$user_login'>Créer un remplaçant</a>";
		echo "<br /><a href='creer_remplacant.php?login_prof_remplace=$user_login'><img src='../images/remplacant.png' width='29' height='16' alt='Create a substitute' title='Create a substitute' /></a>";
		echo "</td>\n";
	} else {
    ...
    if ($col[$i][7] == "autre" AND getSettingValue("statuts_prives") == "y") {
		...
		if ($special["nom_statut"] == '') {
			...
			$special["nom_statut"] = '<span style="color: red; font-style: italic;">nondefinite</span>';
    ...
    // Affichage du lien 'supprimer'
    echo "<td><p class='small'><span class='bold'><a href='../lib/confirm_query.php?liste_cible={$col[$i][1]}&amp;action=del_utilisateur&amp;chemin_retour=$chemin_retour".add_token_in_url()."'>Delete</a></span></p></td>\n";
    // Affichage du lien pour l'impression des paramètres
    echo "<td><p class='small'><span class='bold'><a target=\"_blank\" href='impression_bienvenue.php?user_login={$col[$i][1]}'>print the ' welcome card '</a></span></p></td>\n";
	...
	if (getSettingValue("active_module_trombinoscopes")=='y') {
			...
			if(file_exists($photo)) {
				...
				echo "' width='32' height='32'  align='middle' border='0' alt='photo present' title='photo present' /></a>\n";
			}
			echo "</td>\n";
