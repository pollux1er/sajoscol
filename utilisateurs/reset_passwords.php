...
if ($user_login AND strtoupper($user_login) == strtoupper($_SESSION['login'])) {
	$user_login = false;
	echo "<p>ERROR ! Use the interface 'Manage my account' to change your password !</p>";
	echo "</div></body></html>";
	die();
}

if ($user_status and !in_array($user_status, array("scolarite", "professeur", "cpe", "secours", "responsable", "eleve", "autre"))) {
	echo "<p>ERROR ! The identifier of statute is erroneous. The operation cannot continue.</p>";
	echo "</div></body></html>";
	die();
}

if ($user_classe AND !is_numeric($user_classe)) {
	echo "<p>ERROR ! The identifier of the class is erroneous. The operation cannot continue.</p>";
	echo "</div></body></html>";
	die();
}
//----
...
width:".$largeur1."%;\n";
		...
		echo "<tr><td>To the attention of </td><td><span class = \"bold\">" . $user_prenom . " " . $user_nom . "</span></td></tr>\n";
		//echo "<tr><td>Nom de login : </td><td><span class = \"bold\">" . $user_login . "</span></td></tr>\n";
		echo "<tr><td>Identifier : </td><td><span class = \"bold\">" . $user_login . "</span></td></tr>\n";
		if ($user_auth_mode != "gepi" && $gepiSettings['ldap_write_access'] != 'yes') {
			...
			echo "<tr><td>The password of this user is not managed by Gepi.</td></tr>\n";
		}
		else {
			echo "<tr><td>Password : </td><td><span class = \"bold\">" . $new_password . "</span></td></tr>\n";
		}//if($cas_traite!=0){
		if ($user_statut == "responsable") {
			echo "<tr><td>Responsible of : </td><td><span class = \"bold\">";
			if($liste_elv_resp==""){
				echo "&nbsp;";
			}
			else{
		...
		echo "<tr><td>Email Address : </td><td><span class = \"bold\">" . $user_email . "&nbsp;</span></td></tr>\n";
		echo "</table>\n";
...
width:".$largeur1."%;\n";
		...
		//else{
		elseif ($user_statut == "eleve") {
			echo "<tr><td>Class : </td><td><span class = \"bold\">";
			if(count($tab_tmp_classe)>0){
				$chaine="";
				foreach ($tab_tmp_classe as $key => $value){
					//echo "\$key=$key et \$value=$value et my_ereg_replace(\"[0-9]\",\"\",$key)=".my_ereg_replace("[0-9]","",$key)."<br />";
		...
		*/
		...
		echo "<tr><td>Email Adress  : </td><td><span class = \"bold\">" . $user_email . "&nbsp;</span></td></tr>\n";
		echo "</table>";
...
switch ($mode_impression) {
	case 'csv' :
		if(isset($donnees_personne_csv)){
			...
			echo "<p>All the accounts are already initialized.<br />One does not modify the passwords.</p>\n";
		}
		break;
	case 'pdf' :
		if(isset($donnees_personne_csv)){
...
if(count($tab_non_INE_password)>0) {
	if(count($tab_non_INE_password)==1) {
		$chaine="The following student does not have number INE well informed .<br />It thus obtained a random password:<br />";
	}
	else {
		$chaine="The following students do not have well informed INE number .<br />They thus obtained a random password:<br />";
	}
