<?php
/*
* $Id: affiche_eleve.php 8633 2011-11-17 21:34:51Z jjacquard $
*
		...
		if($_POST['save_params']=="y") {
			...
			function save_params_graphe($nom,$valeur) {
				...
				if(!saveSetting("$nom", $valeur)) {
					$msg.="Error during recording of the parameter $nom<br />";
				}
			}
...
if(isset($_POST['parametrage_affichage'])) {
	...
	if($msg=='') {
		$msg.="personal preferences Recorded .";
	}
...
if(
	...
	if(strlen(preg_replace("/[0-9]/","",$num_periode_saisie))==0) {
		...
		if (mysql_num_rows($verif)==0) {
			tentative_intrusion(2, "Attempt of typing of opinion of the staff meeting for a student not registered in the class.");
			$mess=rawurlencode("Attempt at typing of opinion of the staff meeting for a student not registered in the class.");
		...
		if($_SESSION['statut']=='professeur') {
			...
			$verif=mysql_query($sql);
			if (mysql_num_rows($verif)==0) {
				tentative_intrusion(2, "Attempt of typing of opinion of the staff meeting for a student of which you are not a principal professor.");
				$mess=rawurlencode("Attempt of typing of opinion of the staff meeting for a student not registered in the class.");
				header("Location: ../accueil.php?msg=$mess");
				die();
			}
		}
			...
			$verif=mysql_query($sql);
			if (mysql_num_rows($verif)==0) {
				tentative_intrusion(2, "Attempt of typing of opinion of the staff meeting for a student of a class whose account schooling is not responsible.");
				$mess=rawurlencode("Attempt of typing of opinion of the staff meeting for a student of a class of which you are not responsible.");
				header("Location: ../accueil.php?msg=$mess");
				die();
			}
		}
		...
		if($lig_verr_per->verouiller!='O') {
			...
			if (!$register) {
				...
				$msg="Recording of the opinion carried out.";
			}
		}
		else {
			$msg = "The period on which you want to record is locked";
		}
	}
	else {echo "Nonnumerical period: $num_periode_saisie<br />";}
...
if (
	...
	tentative_intrusion(1, "Attempt of access to the graphic tool for visualization without being authorized there.");
...
if ($_SESSION['statut'] == "responsable") {
	...
	if (mysql_num_rows($get_eleves) == 1) {
		...
		echo "<p>It seems that you are associated no student. Contact the administrator to solve this error.</p>";
		require "../lib/footer.inc.php";
		die();
	} else {
		if ($login_eleve != null) {
			...
			if (mysql_result($test, 0) == 0) {
			    tentative_intrusion(2, "Attempt by a parent of graphic visualization of the results of a student of which it is not legal responsible.");
			    echo "<p>You can visualize only the graphs of the students for which you are legal responsible.</p>\n";
			    require("../lib/footer.inc.php");
				die();
			}
		}
	...
	if ($login_eleve != null and (strtoupper($login_eleve) != strtoupper($_SESSION['login']))) {
		tentative_intrusion(2, "Attempt by a student of graphic visualization of the results of another student.");
	}
...
if (!isset($id_classe) and $_SESSION['statut'] != "responsable" AND $_SESSION['statut'] != "eleve") {
	echo "<div class='noprint'>\n";
	echo "<p class='bold'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return to home</a> | <a href='index.php'>Other tool for visualization</a></p>\n";
	echo "</div>\n";
	...
	*/
	...
	echo "<p>Click on the name of the student for which you want to visualize the averages :</p>";
	...
	if ($_SESSION['statut'] != "responsable" and $_SESSION['statut'] != "eleve") {
		...
		*/
		...
		echo "<p class='bold'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return home</a> | <a href='index.php'>Other tool for visualization</a>";
		...
		*/
		...
		echo "<p class='bold'><a href='../accueil.php'><img src='../images/icons/back.png' alt='Return' class='back_link'/> Return home</a>";
	}
	...
	*/
	...
	if ($_SESSION['statut'] == "responsable") {
		...
		$test = mysql_query("SELECT count(e.login) " .
				...
				"r.login = '" . $_SESSION['login'] . "' AND (re.resp_legal='1' OR re.resp_legal='2'))");
		if (mysql_result($test, 0) == 0) {
		    tentative_intrusion(3, "Attempt (strong) of a parent of graphic visualization of the results of a student of which it is not legal responsible.");
		    echo "<p>You can visualize only the graphs of the students for which you are legal responsible.\n";
		    require("../lib/footer.inc.php");
			die();
		}
	}
	if ($_SESSION['statut'] == "eleve" OR $_SESSION['statut'] == "responsable") {
		...
		if ($eleve2 != "moyclasse" and $eleve2 != "moymin" and $eleve2 != "moymax") {
			tentative_intrusion(3, "Attempt at handling of the second source of data on graphic visualization of the results (diversion of _eleve2_, who cannot, in the case of a user parent or eleve,  correspond only to one average and not another student).");
			$eleve2 = "moyclasse";
		}
	}
...
*/
	...
	if(isset($_POST['parametrer_affichage'])) {
		if($_POST['parametrer_affichage']=='y') {
			...
			*/
			...
			echo "<h2>Parameter setting of the display of the graph</h2>\n";
			...
			// Paramètres:
			echo "<p><b>Averages and periods</b></p>\n";
			echo "<blockquote>\n";
			...
			echo "<tr valign='top'><td>Display the general average:</td><td>";
			if($affiche_mgen=='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<input type='radio' name='affiche_mgen' id='affiche_mgen_oui' value='oui'$checked /><label for='affiche_mgen_oui' style='cursor: pointer;'> Yes </label>/";
			if($affiche_mgen!='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<label for='affiche_mgen_non' style='cursor: pointer;'> No </label><input type='radio' name='affiche_mgen' id='affiche_mgen_non' value='non'$checked /></td></tr>\n";
			...
			echo "<tr valign='top'><td>Display the bands minimal/maximum average:<br />(<i>this display is not applied in mode 'All periodes'</i>)</td><td>";
			if($affiche_minmax=='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<input type='radio' name='affiche_minmax' id='affiche_minmax_oui' value='oui'$checked /><label for='affiche_minmax_oui' style='cursor: pointer;'> Yes </label>/\n";
			if($affiche_minmax!='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<label for='affiche_minmax_non' style='cursor: pointer;'> No</label><input type='radio' name='affiche_minmax' id='affiche_minmax_non' value='non'$checked /></label>";
			echo "</td></tr>\n";
			...
			echo "<tr valign='top'><td>Display annual averages:<br />(<i>in mode 'All periodes' only</i>)</td><td>";
			if($affiche_moy_annuelle=='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<input type='radio' name='affiche_moy_annuelle' id='affiche_moy_annuelle_oui' value='oui'$checked /><label for='affiche_moy_annuelle_oui' style='cursor: pointer;'> Yes </label>/\n";
			if($affiche_moy_annuelle!='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<label for='affiche_moy_annuelle_non' style='cursor: pointer;'> No </label><input type='radio' name='affiche_moy_annuelle' id='affiche_moy_annuelle_non' value='non'$checked /></td></tr>\n";
			...
			echo "<td><label for='type_graphe_courbe' style='cursor: pointer;'><input type='radio' name='type_graphe' id='type_graphe_courbe' value='courbe'$checked /> curve</label><br />\n";
			if($type_graphe=='etoile') {$checked=" checked='yes'";} else {$checked="";}
			echo "<label for='type_graphe_etoile' style='cursor: pointer;'><input type='radio' name='type_graphe' id='type_graphe_etoile' value='etoile'$checked /> star</label>\n";
			echo "</td></tr>\n";

			// - dimensions de l'image
			echo "<tr><td><label for='largeur_graphe' style='cursor: pointer;'>Width (<i>in pixels</i>):</label></td><td><input type='text' name='largeur_graphe' id='largeur_graphe' value='$largeur_graphe' size='3' onkeydown=\"clavier_2(this.id,event,0,2000);\" /></td></tr>\n";
			//echo " - \n";
			echo "<tr><td><label for='hauteur_graphe' style='cursor: pointer;'>Height (<i>in pixels</i>):</label></td><td><input type='text' name='hauteur_graphe' id='hauteur_graphe' value='$hauteur_graphe' size='3' onkeydown=\"clavier_2(this.id,event,0,2000);\" /></td></tr>\n";

			// - taille des polices
			echo "<tr><td><label for='taille_police' style='cursor: pointer;'>Size of police :</label></td><td><select name='taille_police' id='taille_police'>\n";
			for($i=1;$i<=6;$i++) {
			...
			echo "<tr><td>Increasing thickness of the curves  period per period:</td><td>\n";
			if($epaisseur_croissante_traits_periodes=='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<input type='radio' name='epaisseur_croissante_traits_periodes' id='epaisseur_croissante_traits_periodes_oui' value='oui'$checked /><label for='epaisseur_croissante_traits_periodes_oui' style='cursor: pointer;'> Yes </label>/";
			if($epaisseur_croissante_traits_periodes!='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<label for='epaisseur_croissante_traits_periodes_non' style='cursor: pointer;'> No </label><input type='radio' name='epaisseur_croissante_traits_periodes' id='epaisseur_croissante_traits_periodes_non' value='non'$checked />";
			echo "</td></tr>\n";
			...
			echo "<tr><td>Display long names of course:<br />(<i>in legend under the graph</i>)</td><td>";
			if($temoin_image_escalier=='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<input type='radio' name='temoin_image_escalier' id='temoin_image_escalier_oui' value='oui'$checked /><label for='temoin_image_escalier_oui' style='cursor: pointer;'> Yes </label>/";
			if($temoin_image_escalier!='oui') {$checked=" checked='yes'";} else {$checked="";}
			echo "<label for='temoin_image_escalier_non' style='cursor: pointer;'> No </label><input type='radio' name='temoin_image_escalier' id='temoin_image_escalier_non' value='non'$checked />";
			echo "</td></tr>\n";
			...
			echo "<tr><td><label for='tronquer_nom_court' style='cursor: pointer;'>Truncate the short name of the course to <a href='#' onclick='alert(\"zero characters, one does not truncate the shorts name of course displayed in top of the graph.\");return false;'>X</a> characters:<br />(<i>to avoid collisions of legends in top of the graph</i>)</label></td><td><select name='tronquer_nom_court' id='tronquer_nom_court'>\n";
			for($i=0;$i<=10;$i++) {
			...
			echo "<tr>\n";
			echo "<td>\n";
			echo "<label for='click_plutot_que_survol_aff_app' style='cursor: pointer;'>Display appreciations in infobulles during of&nbsp;:</label>\n";
			echo "</td>\n";
			echo "<td>\n";
			...
			if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {
				echo "<tr>\n";
				echo "<td valign='top'>Allow the typing of the opinion of the council:</td>\n";
				echo "<td>\n";
				if($graphe_champ_saisie_avis_fixe!="y") {$checked=" checked";} else {$checked="";}
				echo "<input type='radio' name='graphe_champ_saisie_avis_fixe' id='graphe_champ_saisie_avis_fixe_n' value='n'$checked /> <label for='graphe_champ_saisie_avis_fixe_n' style='cursor: pointer;'>in infobulle</label><br />\n";
				if($graphe_champ_saisie_avis_fixe=="y") {$checked=" checked";} else {$checked="";}
				echo "<input type='radio' name='graphe_champ_saisie_avis_fixe' id='graphe_champ_saisie_avis_fixe_y' value='y'$checked /> <label for='graphe_champ_saisie_avis_fixe_y' style='cursor: pointer;'>in fixed field under the graph</label>\n";
				echo "</td>\n";
				echo "</tr>\n";
			}
			//========================
			...
			// Graphe PNG ou SVG
			echo "<tr><td valign='top'>Generate graphs in png or SVG<br />\n";
			echo "(<i>Graphs SVG give an aspect more smoothed,<br />but require, with certain navigators,<br />the installation of a plugin.<br />Only available for the graphs<br />in curve for the moment</i>)";
			echo "</td>\n";
			...
			echo "<tr><td>Display an unrolling window containing the appreciations:</td><td><label for='affiche_deroulant_appreciations_oui' style='cursor: pointer;'><input type='radio' name='graphe_affiche_deroulant_appreciations' id='affiche_deroulant_appreciations_oui' value='oui'$checked />Yes</label> / \n";
			...
			echo "<tr><td><label for='graphe_hauteur_affichage_deroulant' style='cursor: pointer;'>Height of the unrolling zone (<i>in pixels</i>):</label></td><td><input type='text' name='graphe_hauteur_affichage_deroulant' id='graphe_hauteur_affichage_deroulant' value='$graphe_hauteur_affichage_deroulant' size='3' onkeydown=\"clavier_2(this.id,event,0,2000);\" /></td></tr>\n";
			...
			echo "<p><b>Parameters of the photos</b></p>\n";
			echo "<blockquote>\n";
			echo "<table border='0' summary='Parameters of the photos'>\n";
			if(($affiche_photo=='')||($affiche_photo=='oui')) {$checked=" checked='yes'";} else {$checked="";}
			echo "<tr><td>Display the photograph of the student if it exists:</td><td><label for='affiche_photo_oui' style='cursor: pointer;'><input type='radio' name='affiche_photo' id='affiche_photo_oui' value='oui'$checked />Yes</label> / \n";
			...
			if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {
				...
				//echo "<hr width='150' />\n";
				//echo "<p>\n";
				echo "<a href='choix_couleurs.php' target='blank'>Modify the colors</a>\n";
				//echo "</p>\n";
				echo "</blockquote>\n";
			}
			...
			echo "<input type='submit' name='Valider' value='Validate' /></p>\n";
			...
			echo "<p align='center'>If, after tests, you want to give up your personal parameters and return to the parameters recorded in the base, validate below&nbsp;:<br />";
			echo "<input type='hidden' name='id_classe' value='$id_classe' />\n";
			echo "<input type='hidden' name='is_posted' value='y' />\n";
			...
			echo "<input type='submit' name=\"valider_raz_param\" value=\"Take the by default parameters of the school\" /></p>\n";
	...
	*/
	...
	echo "<table summary='Presentation'>\n";
	echo "<tr valign='top'>\n";
	//====================================================================
	// Bande de pilotage:
	...
	if ($_SESSION['statut'] != "responsable" and $_SESSION['statut'] != "eleve") {
		...
		echo "<option value='moyclasse'$selected>class Average  </option>\n";
		if($eleve2=='moymax') {$selected=" selected='yes'";}else{$selected="";}
		echo "<option value='moymax'$selected> max. Average </option>\n";
		if($eleve2=='moymin') {$selected=" selected='yes'";}else{$selected="";}
		echo "<option value='moymin'$selected> min. Average</option>\n";
		echo "</select>\n";
		echo "<br />\n";
...
function eleve_suivant() {
	if(document.getElementById('numeleve1').value<$nombreligne) {";
	    ...
	    if($precedent>0) {
			...
			echo "<a href='javascript:eleve_precedent();'>Previous student</a><br />\n";
		}

		//echo "<input type='submit' name='choix_eleves' value='Afficher' />\n";
		echo "<a href=\"javascript:document.forms['form_choix_eleves'].submit();\">Refresh</a>\n";

	    if($suivant<$nombreligne+1) {
			echo "<br />\n";
			//echo "<input type='button' name='suivant' value='>>' onClick='eleve_suivant();' />\n";
			echo "<a href='javascript:eleve_suivant();'>Next student</a>";
		}
		echo "</p>\n";
		...
		echo "<p>Student : ".$prenom_eleve . " " .$nom_eleve."</p>\n";
		echo "<input type='hidden' name='eleve1' value='".$login_eleve."'/>\n";
		echo "<input type='hidden' name='login_eleve' value='".$login_eleve."'/>\n";
		echo "and <select name='eleve2'>\n";
		if($eleve2=='moyclasse') {$selected=" selected='yes'";}else{$selected="";}
		if(!isset($eleve2)) {$selected=" selected='yes'";}
		echo "<option value='moyclasse'$selected> class Average</option>\n";
		if($eleve2=='moymax') {$selected=" selected='yes'";}else{$selected="";}
	...
	if ($graphe_affiche_deroulant_appreciations=='oui') {
		...
		echo "<b><i><center>Appreciations - $periode</center></i></b>";
	...
	echo "<label for='choix_toutes_periodes' style='cursor: pointer;'><input type='radio' name='choix_periode' id='choix_toutes_periodes' value='toutes_periodes'$checked onchange=\"document.forms['form_choix_eleves'].submit();\" /> All periods</label>\n";
	...
	echo "<a href='".$_SERVER['PHP_SELF']."' onClick='document.forms[\"form_choix_eleves\"].parametrer_affichage.value=\"y\";document.forms[\"form_choix_eleves\"].submit();return false;'>Parameterize the display</a>.<br />\n";
...
*/
	...
	echo "/> Deactivate display of the appreciations</label>\n";
		...
		function save_avis(mode) {
			...
			if(mode=='suivant') {
				...
				document.forms['form_choix_eleves'].submit();
						...
						//================
							echo "<br />\n<a href=\"#graph\" onClick=\"afficher_div('saisie_avis','y',100,100);return false;\"> Type the opinion of the council</a>\n";
	
							$titre="Opinion of the staff meeting: $lig_verr_per->nom_periode";
							...
							$texte.="<input type='button' NAME='ok1' value='Save' onClick=\"save_avis('');\" />\n";
							if($suivant<$nombreligne+1) {
								$texte.=" <input type='button' NAME='ok1' value='Save and pass to the next' onClick=\"save_avis('suivant');\" />\n";
							}
							...
							$texte_saisie_avis_fixe.="<p class='bold' style='text-align:center;'>Typing of the opinion of the council</p>\n";
							...
							$texte_saisie_avis_fixe.="<br /><input type='button' NAME='ok1' value='Save' onClick=\"save_avis('');\" />\n";
							if($suivant<$nombreligne+1) {
								$texte_saisie_avis_fixe.=" <input type='button' NAME='ok1' value='Save and pass to the next' onClick=\"save_avis('suivant');\" />\n";
							}
...
<input type='hidden' name='enregistrer_avis' id='enregistrer_avis' value='' />
	...
	*/
	if((isset($eleve1) AND $_SESSION['statut'] != "responsable" AND $_SESSION['statut'] != "eleve")
		...
		if($mois>$moisdatenais1) {
			...
			$precision1="years and $precision1 month";
		}
		else{
			if($mois<$moisdatenais1) {
				...
				if($jour>=$jourdatenais1) {
					$age1=$annee-$anneedatenais1;
					$precision1="years this month";
				}
				else{
					$age1=$annee-$anneedatenais1-1;
					$precision1="years and 1 more this month";
				}
			}
		}
	...
	//}
		...
		if($choix_periode=="periode") {
			...
			if($indice_eleve1==-1) {
				...
				echo "</div>\n";
				echo "<p>The student has changed class, arrived in the course of year<br /> or the left the school , but it is not in the class of $classe<br />for the period $periode.</p>\n";
				echo "<p>If this information seems erroneous to you ,<br />\n";
				echo "you can <a href=\"javascript:centrerpopup('$gepiPath/gestion/contacter_admin.php',600, 480,'scrollbars=yes,statusbar=no,resizable=yes')\">contact the administrator</a>.</p>\n";
				echo "</div>\n";
			...
			if($tab_acces_app[$num_periode]=="y") {
				...
				if(mysql_num_rows($res_avis)>0) {
					$lig_avis=mysql_fetch_object($res_avis);
					if($lig_avis->avis!="") {
						$titre_bulle="Opinion of the Staff meeting";
			...
			*/
			...
			if(mysql_num_rows($res_avis)>0) {
				...
				$texte_bulle="<table class='boireaus' style='margin:2px;' width='99%' summary='Opinion'>\n";
	...
	*/
		...
		if(isset($prenom1)) {
			...
			echo " born";
			if($sexe1=="F") {echo "e";}
			echo " the $naissance1[2]/$naissance1[1]/$naissance1[0] (<i>ie $age1 $precision1</i>).</p>";



			$acces_bull_simp="n";
			if(($_SESSION['statut']=="responsable")&&(getSettingValue('GepiAccesReport cardSimpleParent')=='yes')) {
				$acces_bull_simp="y";
			}
			elseif(($_SESSION['statut']=="eleve")&&(getSettingValue('GepiAccesReport cardSimpleEleve')=='yes')) {
				$acces_bull_simp="y";
			}
			elseif($_SESSION['statut']=="professeur") {
				...
				if(getSettingValue('GepiAccesReport cardSimpleProfToutesClasses')=='yes') {
					$acces_bull_simp="y";
				}
				elseif(getSettingValue('GepiAccesReport cardSimpleProfTousEleves')=='yes') {
				...
				elseif(getSettingValue('GepiAccesReport cardSimpleProf')=='yes') {
			...
			if($acces_bull_simp=="y") {
				...
				if($choix_periode=='toutes_periodes') {
					...
					echo "See the simplified bulletin";
					//echo "<img src='../images/icons/bulletin_simp.png' width='17' height='17' alt='Report card simple toutes périodes en infobulle' title='Report card simple toutes périodes en infobulle' />";
					echo "</a>";
				}
				else {
					...
					if (isset($tab_ele['absences'][$num_periode_choisie-1])) {
						if($choix_periode!="toutes_periodes") {
							...
							if((count($tab_ele['absences'])==0)) {
								$info_absence.="No assessment of absences is recorded.";
							}
							else {
								if($tab_ele['absences'][$num_periode_choisie-1]['nb_absences'] == '0')
								{
									$info_absence.="No half-day of absence.";
								} else {
									$info_absence.="Number of half-days of absence ";
									if ($tab_ele['absences'][$num_periode_choisie-1]['nb_absences'] == '0') {
									...
									{
										$info_absence = $info_absence." (<em>dont <b>".$tab_ele['absences'][$num_periode_choisie-1]['non_justifie']."</b> not justified";
								...
								{
									$info_absence = $info_absence."<i> Number of delays : </i><b>".$tab_ele['absences'][$num_periode_choisie-1]['nb_retards']."</b>";
								}
							}
							...
							echo "<table class='boireaus' summary='Assessment of the absences'>\n";
							echo "<tr>\n";
							echo "<th>Numbers 1/2 days of absence over the period</th>\n";
							echo "<th>of which not justified</th>\n";
							echo "<th>Number of delays</th>\n";
							echo "</tr>\n";
							$alt=-1;
...
*/
		...
		if ($_SESSION['statut'] == "eleve" OR $_SESSION['statut'] == "responsable") {
			echo "<p align='center'>Choose a period and validate.</p>\n";
		} else {
			echo "<p align='center'>Choose a student and validate.</p>\n";
		}
	}
	echo "</td>\n";
	//====================================================================
...
echo "<div id='div_bull_simp' class='infobulle_corps' style='position: absolute; top: 220px; right: 20px; width: 700px; text-align:center; color: black; padding: 0px; border:1px solid black; display:none;'>\n";
	...
	echo "<div class='infobulle_entete' style='color: #ffffff; cursor: move; width: 700px; font-weight: bold; padding: 0px;' onmousedown=\"dragStart(event, 'div_bull_simp')\">\n";
		...
		echo "Simplified bulletin  of $prenom1 $nom1 ";
		if($choix_periode=='periode') {
			echo "in period $num_periode_choisie";
		}
		else {
			echo "from period 1 to $nb_periode";
		}
		echo "</div>\n";
	echo "</div>\n";
...
echo "<script type='text/javascript'>
	...
	function affiche_bull_simp(login_eleve,id_classe,num_per1,num_per2) {
		//document.getElementById('titre_entete_bull_simp').innerHTML='Report card simplifié de '+login_eleve+' période '+num_per1+' à '+num_per2;
